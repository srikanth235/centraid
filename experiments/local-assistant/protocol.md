...
### 2026-09-19 — blind re-check set added; the frozen evaluation is unchanged

**Reason.** `j-05`'s training data gained group-phrase templates written after reading `e2e-j03`'s suite failures, so its `.811` is informed by the suite and is not a blind estimate.

`blind/` was added: 40 cases (`blind/blind_suite.json`, ids `b01`…`b40`) in the frozen suite's seven categories and proportions, with the per-turn reference calls in `blind/reference_blind.py` and `blind/run_reference_blind.py` proving all 40 reachable through the executor and the scorer at 0 failures (`runs/reference-blind.json`). The cases were written from the catalogue, `world.py`, `scoring.py` and the first three cases of `suite.json` alone — no template file, generator or training corpus was read before they were committed. `suite.json`, `reference.py`, the world, the resolvers, the executor and the scorer are untouched; `run_suite_joint.py` and `needle/suite_run_selector.py` gained a `--suite blind` flag and nothing else. New run labels, append-only: `reference-blind`, `j05-blind-e2e` (joint `j-05`, .600 outcome, .872 operation) and `needle-base-blind-m03` (Needle base, selector-driven at margin 0.3, .375). Cross-lane tables, ceilings, costs and the recommendation: [`RESULTS.md`](RESULTS.md).

### 2026-09-19 — joint span decoding, realistic invented names, int8 export; the frozen evaluation is unchanged

**Reason.** Eleven of `j-05`'s fourteen frozen-suite failures were spans over proper nouns the model had never seen, and the blind re-check set put it at 24/40. Both were attacked, and everything below is scored on **both** suites, with the blind one deciding.

- **Span decoding is word-aware** (`joint/model.decode_spans`). Token-level dumps of the failing turns showed the tagger was mostly right and the decoder was wrong: `Initech` came back `in/B-event ##ite/O ##ch/I-event` and the piece-by-piece decoder stopped at the first `O` (`"In"`); `Offsite debrief` came back with a spurious `B-` on each piece (`"Offs"`). Pieces are now grouped into words — touching offsets with alphanumeric characters on both sides, so `Neha` + `'s` stay two words — and adjacent same-slot words merge. **Frozen 60 → 63/74, blind 24 → 25/40, with no retraining.** `JOINT_DECODE=legacy` restores the old decoder; `j05-blind-legacy` is that measurement.
- **Invented names now look like proper nouns** (`joint/gen_joint_data.invented`). `j-05`'s were syllable strings (`Karomin`); the suite's are ordinary names that WordPiece fragments. The generator now builds stem+suffix coinages, multi-word titles with a lowercase tail and a share carrying a digit, and **rejects a candidate the tokenizer keeps whole**. Run `j-06`, 0.35 unseen share: **frozen 62/74, blind 26/40, operation .891** — the best blind result in the lane. The overlap guard stays at 0 offenders and 0 proper-noun leaks; `Initech` and two suite surnames that the first draft of the pools produced were removed before any data was kept.
- **A wider phrasing register did not help.** `--register-wide` adds openers, trailers, whole-request lowercasing and shouting, and doubles the typo rate. Run `j-07`: frozen 61/74, blind 25/40, operation .870 — slot accuracy rose (.652) but operation accuracy fell, and outcome fell with it. Recorded as a negative result, not adopted.
- **`--onnx {fp32,int8}`** on `run_suite_joint.py` routes prediction through `joint/predict_onnx.py`, so a quantised graph is scored end to end rather than only compared logit-by-logit. `joint/quantise_onnx.py` writes `joint-int8.onnx` with `quantize_dynamic` over `MatMul` and `Gather`: **90.7 → 23.0 MB**. Quantising `MatMul` alone was no more faithful and 58.8 MB, so it was not kept.

`suite.json`, `reference.py`, `blind/`, the world, the resolvers, the executor and the scorer are untouched. New run labels, append-only: `j05r-e2e`, `d01-e2e`, `d02-e2e`, `j05-blind-legacy`, `j05-blind-d02`, `j-06`, `j06-frozen`, `j06-blind`, `j-07`, `j07-frozen`, `j07-blind`, `j-08`, `j08-frozen`, `j08-blind`, `j05-int8-frozen`, `j05-int8-blind`.
