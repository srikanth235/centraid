...
Restore is two verbs. `backup restore` materializes a snapshot into a fresh side dir (lazy by default; `--at` for point-in-time; never swaps the live vault). The zero-vault founding route `vaults:restore` consumes one short-lived founding capability, a passphrase-wrapped recovery kit, and a provider API key; `recover()` then discovers, fetches, replays, fences, adopts, and warms the vault. DEKs and the backup keyring come only from the wrapped kit and enter custody through `KeyStore.import()`—snapshots never contain loose keys. There is no separate recovery admin mount.
