...
**Sharing is a placement recorded as an edge, not a filter.** `share_edges` (`packages/gateway/src/serve/gateway-schema.ts`) succeeds the old `placement_intents` table outright — dropped, not migrated, since both are pre-1.0. One edge (`{edgeId, originVaultId, audienceVaultId, kind: 'add' | 'move', itemType, itemIds[]}`) can carry a whole set of items — three photographs moved together are one row and one reconciler pass — and `mode: 'live'` is accepted by the schema but the route still refuses it typed `not_yet_supported` (a later phase's lend model, not yet built). `window.centraid.place` keeps its exact client-facing signature; the wire translation to edges lives only in the gateway's own placement-wire layer, so blueprint and mobile callers needed no changes.

**The closure splits into an origin-side read and an audience-side projection** (`packages/vault/src/share/closure.ts`, `read-closure.ts`, `project-closure.ts`). `readShareClosure(origin, itemType, itemIds[])` is read-only and produces a `WireClosure` — a plain JSON-serializable shape (no Buffers, Dates, or class instances) pooling every content item, derivative, and blob the item set touches exactly once, with cross-vault foreign keys absent from the read rather than merely nulled at the far end. `projectShareClosure(audience, closure, opts)` is one `BEGIN IMMEDIATE` against the audience database alone, sha-deduping through `core_content_item.sha256 UNIQUE` and writing `core_share_origin` lineage. The split exists so a same-machine share and a cross-machine one run the identical audience-side code: local composition (`shareToVault`) is just read-then-project in one process with a hardlink for the bytes, and the P3 peer plane replaces only that last hardlink step with a network pull — the closure shape itself never changes for a tunnel to sit under it.

**Projection is ingest, never a second-class arrival.** After a projection commits, still inside the same transaction and the same replica commit bracket, the audience runs the identical post-ingest door an authored row takes — for `media.media_asset`, re-deriving `place_id` from the asset's own EXIF GPS under the audience's own `keep`/`strip` location policy, and enqueuing one enrichment request so a just-shared photo is findable without waiting for the audience's own sweep. The hook table is keyed by vault entity type, never by app id (`packages/vault/src/share/projection-ingest.ts`), so vault core cannot branch on "Photos" — an app reaches it only by declaring which entity types it mints, through the scope kit (see [docs/blueprint-seats.md](docs/blueprint-seats.md)). Captions and faces are deliberately not requested here: those are consent-gated enrichers, and a placement must never manufacture an owner's consent.
