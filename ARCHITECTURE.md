...
Those slots are also the **accounting** vocabulary the owner sees. `packages/gateway/src/serve/local-usage.ts` walks them into named components — per vault `ledger` (`journal.db` + `-wal`), `vault-db`, `attachments` (`blobs/`), `apps`, `code`, and `cache` (the runner scratch, billed to its vault though it lives outside the tree); gateway-wide `backup`, `logs`, `templates`, and `storage` — and `GET /centraid/_gateway/storage/local` serves that alongside the volume's `statfs` (issue #544). The walk is TTL-cached and never runs on a timer: a recursive pass over a blob CAS is not something to schedule. Two owner-set limits live beside the storage connections in `<storageDir>/storage-limits.json`: `totalLimitBytes` is **warn-only** (it degrades a `storage-limit` health component and never refuses a write — free-space pressure remains `disk-health.ts`'s job), while `journalLimitBytes` **actuates**, and safely, because what it triggers is archival rather than deletion (below).
