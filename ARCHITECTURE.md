...
OCR, transcription, image/text embeddings, and faces are bundled deterministic automations on the same engine as every other background workflow. Their handlers take bounded batches of 16, acquire vault content through `ctx.fetch` attachments, call the loopback enrichment service through the host-owned executor, invoke typed vault commands, and stamp `enrich_derivation` with the pinned `model@version`. Cursor watermarks replace gateway-private backfill sweeps; a model/prompt revision re-arms only the affected template. Faces is the exception to ambient cursors: it drains only open consent-tagged `enrich_request` rows or items with prior consent stamps.

The synchronous capture and automation “Test run” surfaces use the same invoke-and-await fire path, so success, policy skips, service failures, and optional OCR agent turns all produce ordinary automation ledger turns. Only OCR declares an agent variant. It is reached through `ctx.agent`, requires a pinned runner model and provider-egress consent, preserves optional confidence rather than inventing it, and stamps only ACP-confirmed model identity. The enrichment service remains an executor behind its frozen wire; it is not a scheduler.
