...
function unionMembers(source: string): Map<string, Field[]> {
  ...
  for (const block of body.split(/\|\s*\{/u).slice(1)) {
    ...
    const name = /type:\s*"([^"]+)"/u.exec(text)?.[1];
    if (!name) continue;
    const fields: Field[] = [];
    for (const entry of text.split(";")) {
      const match = /^\s*([A-Za-z]+)(\?)?:\s*(.+?)\s*$/u.exec(entry);
      if (!match || match[1] === "type") continue;
      fields.push({
        name: match[1]!,
        type: match[3]!.replace(/\s+/gu, " "),
        optional: match[2] === "?",
      });
    }
...
function localFunctions(source: string): Map<string, string> {
  ...
  for (const found of source.matchAll(
    /\n(?:export )?(?:async )?function ([A-Za-z]+)\(/gu
  )) {
    let depth = 0;
    let started = false;
    let at = found.index;
    ...
    bodies.set(found[1]!, source.slice(found.index, at));
  }
  return bodies;
}
...
function reachableBody(arm: string, locals: Map<string, string>): string {
  ...
  let text = arm;
  for (;;) {
    const next = [...text.matchAll(/\b([A-Za-z]+)\s*(?:<[^(]*>)?\(/gu)]
      .map((call) => call[1]!)
...
function switchArms(source: string): Array<{ name: string; body: string }> {
  ...
  for (const [index, found] of cases.entries()) {
    ...
    arms.push({ name: found[1]!, body: body.slice(from, to) });
  }
  return arms;
}
...
function httpMethods(body: string): string[] {
  ...
  for (const call of body.matchAll(
    /\b(appRead|appWrite|companionJson|pairOverIroh)\s*(?:<[^(]*>)?\(/gu
  )) {
    const args = callArguments(body, call.index + call[0].length - 1);
    if (call[1] === "companionJson") {
      methods.push(/method:\s*"([A-Z]+)"/u.exec(args)?.[1] ?? "GET");
    } else {
...
function methodFrom(name: string, body: string, fields: Field[]): Method {
  const writeCall = /appWrite\(\s*"([^"]+)",\s*"([^"]+)"/u.exec(body);
  ...
  return {
    ...
    stages_bytes: stages,
    writes: writeCall ? { app: writeCall[1]!, action: writeCall[2]! } : null,
    fields,
  };
}
