 ...
 * "close enough"). The gateway's enrichment client
 * (`packages/gateway/src/enrich/service-client.ts`) rejects any box that
 * overshoots the item's declared `originalWidth`/`originalHeight` as a
 * per-item error, so every box this service returns MUST go through this —
