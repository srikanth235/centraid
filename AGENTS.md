...
Centraid is a personal app builder. Its backend is a host-agnostic **gateway** ([`packages/gateway`](packages/gateway)) that wires the app engine ([`packages/app-engine`](packages/app-engine)), the agent runtime ([`packages/agent-runtime`](packages/agent-runtime)), the automation engine ([`packages/automation`](packages/automation)), the SQLite stores, and the chat/automation runners together. The desktop controls that gateway as a detached child by default (the in-process path is test-only); the standalone `centraid-gateway` daemon uses the same core. Desktop and the installable web PWA ([`apps/web`](apps/web)) share the React shell and browser-safe HTTP client in [`packages/client`](packages/client); the Expo mobile app ([`apps/mobile`](apps/mobile)) connects to a gateway over HTTP. App scaffolding + templates live in [`packages/blueprints`](packages/blueprints), which renders on the kit layer of [`packages/design`](packages/design) and agent grounding in the gateway's [`packages/gateway/src/skills`](packages/gateway/src/skills); all clients share visual identity through [`packages/design`](packages/design) and per-runtime TypeScript settings through the root `tsconfig.base.json` / `tsconfig.electron.json` / `tsconfig.expo.json` files. The full layout, runtime model, on-disk layout, and build orchestration live in [ARCHITECTURE.md](ARCHITECTURE.md).
...
| [docs/photos-places.md](docs/photos-places.md) | Places on both surfaces: the three layers (names → geometry → cartography), one shared projection, and why there is no basemap |
...
| Doc | Topic |
| --- | --- |
| [docs/traps/design-tokens.md](docs/traps/design-tokens.md) | Token source of truth vs hardcoded CSS; the two layers of `packages/design` |
...
- **The gate loop runs itself (#576, #668).** **pre-commit** checks staged files without rewriting them. **pre-push** runs `bun run check:push` (~55s, every gate concurrently, reports _all_ failures in one pass); shared infrastructure requires **`bun run check:full`**. `check:pr` remains the full local mirror of CI. Tier budgets and escape hatches are in [docs/dev-environment.md](docs/dev-environment.md#the-local-gate-loop). Vitest green alone is not enough — package typecheck catches TS errors that still execute under vitest.
