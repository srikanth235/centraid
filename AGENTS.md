...
Centraid is a personal, local-first superapp: one shell wrapping many first-party apps over a sovereign vault. A Rust workspace ([`crates/`](crates)) holds the vault, the ontology, the wire protocol, the iroh network and byte planes, the on-device seat, the first-party apps ([`crates/apps`](crates/apps)) and the one `centraid` binary (gateway and seat). Mobile is Kotlin Multiplatform with a SwiftUI iOS shell ([`mobile/`](mobile)), linking the core through [`crates/core-ffi`](crates/core-ffi); desktop is Electron over a `centraid seat` sidecar ([`desktop/`](desktop)); the browser Companion is [`extension/`](extension). Shared contracts and fixtures live in [`contracts/`](contracts) and visual identity in [`packages/design`](packages/design). Wiring, runtime model, and layout: [ARCHITECTURE.md](ARCHITECTURE.md). Stack: Rust (cargo), Kotlin/Gradle and Swift/XcodeGen, and Bun + TypeScript for `packages/design`, `packages/test-kit`, `desktop/`, `extension/` and repo tooling; oxlint/oxfmt; vitest. The gate is `cargo xtask gate --profile <local|pr|nightly|release|mobile-jvm>` ([crates/xtask](crates/xtask/README.md)). Commands: [README.md](README.md).
...
- **Release + ops**: [docs/release.md](docs/release.md) · [docs/release/v1-handoffs.md](docs/release/v1-handoffs.md) (every v1 hand-off and open owner question, with its command and its destination) · [docs/release/oauth-assist-google.md](docs/release/oauth-assist-google.md) · [docs/identifiers.md](docs/identifiers.md) · [docs/enrollment.md](docs/enrollment.md) · [docs/external-review-scope.md](docs/external-review-scope.md) · [docs/logs.md](docs/logs.md) (**start every debug session here**) · [docs/recovery/](docs/recovery/)
