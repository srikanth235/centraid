...
Centraid is a personal app builder. Its backend is a host-agnostic **gateway** ([`packages/gateway`](packages/gateway)) that wires the app engine ([`packages/app-engine`](packages/app-engine)), the agent runtime ([`packages/agent-runtime`](packages/agent-runtime)), the automation engine ([`packages/automation`](packages/automation)), the SQLite stores, and the chat/automation runners together. The same gateway runs embedded in the Electron desktop ([`apps/desktop`](apps/desktop)) and as the standalone `centraid-gateway` daemon. Desktop and the installable web PWA ([`apps/web`](apps/web)) share the React shell and browser-safe HTTP client in [`packages/client`](packages/client); the Expo mobile app ([`apps/mobile`](apps/mobile)) connects to a gateway over HTTP. App scaffolding + templates live in [`packages/blueprints`](packages/blueprints) and agent grounding in the gateway's [`packages/gateway/src/skills`](packages/gateway/src/skills); all clients share visual identity through [`packages/design-tokens`](packages/design-tokens) and per-runtime TypeScript settings through the root `tsconfig.base.json` / `tsconfig.electron.json` / `tsconfig.expo.json` files. The full layout, runtime model, on-disk layout, and build orchestration live in [ARCHITECTURE.md](ARCHITECTURE.md).
...
- **Pre-push PR gates (do not skip).** Governance hooks do **not** run format/lint/typecheck. Before every `git push` to a PR, agents **must** run `bun run check:pr` (or `bun run ci`) and fix failures locally. That script is a **superset** of the **`static`** job in [`.github/workflows/ci.yml`](.github/workflows/ci.yml): `format:check`, `oxlint`, `lint:packages` (sherif — monorepo package.json hygiene), turbo `lint`, `typecheck` (includes test files), `lint:types`, `knip`, `lint:css`, `lint:e2e-flows`, **`lint:workflow-pins`** (#557 — every third-party `uses:` SHA-pinned; no hardcoded `bun-version:` and no hand-rolled `bun install` outside `.github/actions/setup`; every job carries `timeout-minutes`; **only `ci.yml` may listen on `pull_request`**, and **only `release.yml` may listen on `push: tags`**), `test:matrix`, **`test:ratchet`** (coverage floors / `minimumTests` / mutation floors up-only + perf budgets tighten-only vs `origin/main`; deletions count as decreases), **`test:ratchet:unit`**, **`test:affected`** (`turbo --filter='[origin/main]'` — changed packages only, not dependents), and **`check:diff-coverage`** (#568 — `coverage` then `test:diff-coverage`). Use **`bun run check:pr:full`** before requesting merge when a shared package changed — same gates with **`test:affected:full`** (`turbo --filter='...[origin/main]'`, dependents included). CI `static` runs matrix + ratchet but **not** `test:affected` (full package vitest lives under **`verify`**, which also runs **diff-coverage** ≥80% on changed instrumentable lines after `coverage`). **`check:diff-coverage` is the slow tail of `check:pr`** — it runs the repo-wide instrumented vitest, so it costs minutes that the lint/typecheck gates do not. Run it on its own (`bun run check:diff-coverage`) when iterating on test coverage rather than re-running the whole script. The `static` job additionally runs **actionlint** (pinned container, shellcheck gated at error severity) to lint the workflow YAML — that check is CI-only because it needs an external binary; run `bun run lint:actions` locally if you have actionlint installed. CI also runs a parallel **`verify`** job (build, native tunnel, data-plane, gateway perf, coverage, diff-coverage) and a thin required **`check`** aggregator (ruleset-required on `main` since #496). Since #557 `check` aggregates **every** PR gate, not just static/verify/mutation: the path-filtered lanes that used to be their own `pull_request` workflows (`client-e2e`, `gateway-package`, `docs`, `web-build`, `iroh-wasm`, `companion-static`, `oauth-worker`, `dependency-review`) are now jobs in `ci.yml` behind a single `changes` paths-filter. A lane the diff does not touch reports `skipped`, which `check` treats as satisfied; `cancelled` is a failure. This is why the one-PR-workflow rule is mechanical — a lane in its own path-filtered workflow reports no status on unrelated PRs and therefore can never be a required check. Dead-code / unused-dependency scanning (**`knip`**) is now part of `check:pr` and the `static` job — it enforces unused files, unused/undeclared deps, unused exports/types (with `ignoreExportsUsedInFile`, so same-file-only `Props`/`Deps` interfaces don't count), and duplicate exports; only enum/class members stay excluded. Exports that are genuinely used but invisible to knip (dynamic import, script-level `cp`) are tagged `@public` or listed in `knip.json` with a reason. **Vitest green alone is not enough** — package typecheck catches TS errors that still execute under vitest. Pushing without `check:pr` wastes Actions minutes on fixable local gates.
