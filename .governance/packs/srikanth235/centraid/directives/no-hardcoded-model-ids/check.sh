...
# survives.
#
# Allowlist: packages/server/src/engine/model-pricing.ts is the single
...
while IFS=: read -r file line_no match; do
    ...
    violation "$file:$line_no - hardcoded model id '$model_id' (use capability-tier resolver; only packages/server/src/engine/model-pricing.ts is allowlisted)"
