...
//
// Usage:
//   node .governance/law/parity.ts [--last N] [--range-only] [--quiet]
...
export function expectedDivergences() {
  ...
  return (JSON.parse(readFileSync(file, "utf8")) as { divergences: { directive: string; old: string; new: string; reason: string }[] }).divergences;
}
 ...
 */
export function explainedBy(directive: string, oldStatus: number, newStatus: number) {
 ...
 * @returns {{status: number, output: string}} The result.
 */
function verdict(file: string, args: string[], cwd: string) {
  try {
 ...
 * @returns {void}
 */
function materialiseOldRunner(tree: string, rev: string) {
 ...
 * @returns {void}
 */
function materialiseNewRunner(tree: string) {
 ...
 */
export function verdictsIn(tree: string, door: string = "window") {
  const old = {};
  ...
  const result = verdict("node", [".governance/law/run.ts", "--door", door, "--staged", "--json"], tree);
  let report: { rules: { id: string; verdict: string }[]; crashed?: string } = { rules: [] };
  try {
    report = JSON.parse(result.output.slice(result.output.indexOf("{"), result.output.lastIndexOf("}") + 1)) as typeof report;
  } catch {
 ...
 * @returns {{rows: Record<string, unknown>[], disagreements: object[]}} The table and the diffs.
 */
export function replay(count: number) {
 ...
 * @returns {{rows: Record<string, unknown>[], disagreements: object[], explained: object[], oldRunnerRevision: string}}
 *   The table and the diffs.
 */
export function replayAt(commits: string[]) {
 ...
 */
export function renderTable(rows: Record<string, unknown>[]) {
