...
import { dirDigest } from "./lib/digest.ts";
import { treeSource } from "./lib/git.ts";
import { RECORDED_PATHS } from "./digest.ts";
import { asArrival } from "./lib/types.ts";
import { oldRunnerRevision } from "./parity.ts";
...
test("a --range record is a function of the range, not of the checkout", async (t) => {
  ...
  const generateThere = () => {
    execFileSync("node", [path.join(HERE, "arrival.ts"), "--range", RANGE, "--out", out], {
      cwd: probe,
      stdio: "ignore",
    });
    return readFileSync(out, "utf8");
  ...
  assert.equal(asArrival(JSON.parse(fixture) as unknown).range?.onDefaultBranch, null, "a range run knows no branch");
...
test("the record has every section, including the ones no rule reads yet", () => {
  const arrival = asArrival(JSON.parse(readFileSync(FIXTURE, "utf8")) as unknown);
...
test("the docket is registry, not law, even though it lives inside .governance", () => {
  ...
  assert.equal(classify(".governance/law/rules/doc-integrity.ts"), "law");
