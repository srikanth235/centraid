...
import { runLaw } from "./run.ts";
import type { LawReport } from "./lib/types.ts";
 ...
 */
function comparable(report: LawReport) {
...
test("replaying #1002's squash reproduces the recorded findings exactly", async () => {
  ...
  assert.deepEqual(comparable(report), (JSON.parse(readFileSync(EXPECTED, "utf8")) as unknown));
});
