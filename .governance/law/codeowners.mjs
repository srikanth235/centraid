...
//
// Usage:
//   node .governance/law/codeowners.mjs           # print the generated file
//   node .governance/law/codeowners.mjs --write   # write it
//   node .governance/law/codeowners.mjs --check   # exit 1 on drift
import { readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { readPacks } from "./eslint.config.mjs";
import { byteCompare } from "./lib/digest.mjs";
 ...
 */
export function lawPaths(packs) {
...
export function render(paths) {
  ...
  return [
    ...
    "#",
    "# Written by `node .governance/law/codeowners.mjs --write` from the union of",
...
export function main(argv) {
  ...
  if (argv.includes("--check")) {
    ...
    process.stdout.write(
      drifted
        ? `✗ ${CODEOWNERS_PATH} has drifted from the law estate — run 'node .governance/law/codeowners.mjs --write'\n`
