...
// `arrival.mjs` could make every rule downstream say whatever the editor
...
//
// Usage:
//   node .governance/law/digest.mjs            # report: recorded vs actual
//   node .governance/law/digest.mjs --record   # rewrite those two rows
 ...
 *
 * The generator is `arrival.mjs` plus every module under `lib/` it is built
 ...
 */
export function record(manifest, digests) {
  ...
  const block = lines
    .slice(start + 1, end)
    .filter((line) => !RECORDED_PATHS.some((file) => line.startsWith(`  ${file}:`)));
