...
import rule from "./amendment-pairing.ts";
import { ruleTester } from "../lib/rule.ts";

const ruleFile = (id, status = "M") => ({
  path: `.governance/law/rules/${id}.ts`,
  status,
  estate: "law",
});
const testFile = (id, status = "M") => ({
  path: `.governance/law/rules/${id}.test.ts`,
  status,
  estate: "law",
});
  ...
  invalid: [
    {
      ...
      errors: [{ message: /changes \.governance\/law\/rules\/estate-separation\.ts but not/u }],
    },
    {
      code: record({
