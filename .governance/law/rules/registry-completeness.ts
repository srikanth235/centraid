 ...
 */
export function issuesOf(arrival: Arrival) {
  ...
  if (fromReceipts.length > 0) return [...new Set(fromReceipts)].sort((a, b) => Number(a) - Number(b));
  const fromSubjects = (arrival.commits ?? [])
    .map((commit) => SUBJECT_ISSUE.exec(commit.subject)?.groups.number)
    .filter(Boolean)
    .map(Number);
  return [...new Set(fromSubjects)].sort((a, b) => Number(a) - Number(b));
}
