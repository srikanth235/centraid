...
//
//   rule → its test    a changed `rules/<id>.mjs` without `rules/<id>.test.mjs`
...
const RULE_FILE = /^\.governance\/law\/rules\/(?<id>[a-z0-9-]+)\.mjs$/u;
const TEST_FILE = /^\.governance\/law\/rules\/(?<id>[a-z0-9-]+)\.test\.mjs$/u;
    ...
    const judge = (files, at, label) => {
      ...
      for (const [id, status] of rules) {
        ...
        if (!tests.has(id) && !pendingPairs.tests.has(id)) {
          ctx.report(
            at,
            `${label} changes .governance/law/rules/${id}.mjs but not ${id}.test.mjs. A rule changed without its cases is a claim, not a reviewable change — CONSTITUTION.md's amendment process is one commit, three edits.`
          );
        }
        if (status === "A" && !declared.has(id)) {
          ctx.report(
            at,
            `${label} adds .governance/law/rules/${id}.mjs, which no pack enables. A rule nothing enables is dead code that reads as law; add a row to .governance/law/packs/<pack>.json, or 'severity: "off"' to repeal it on the record.`
          );
        }
      }
    };
