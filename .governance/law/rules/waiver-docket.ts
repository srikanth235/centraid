...
import { defineRule, locate } from "../lib/rule.ts";
import { asArrival } from "../lib/types.ts";
import type { Arrival, DocketRow } from "../lib/types.ts";
 ...
 */
export function rowFor(rows: Record<string, unknown>[], waiver: Record<string, unknown>) {
  const candidates = (rows ?? []).filter(
    (row) => row.rule === waiver.directive && covers(row, (waiver.path as string | null) ?? null)
  );
 ...
 * @returns {void}
 */
function judgeDocket(arrival: Arrival | DocketRow[], today: string, report: (pointer: (string | number)[], message: string) => void) {
  const rows = Array.isArray(arrival) ? arrival : (((arrival as { rows?: DocketRow[] }).rows) ?? []);
  const seen = new Set();
 ...
 * @returns {void}
 */
function judgeSpending(arrival: Arrival, report: (pointer: (string | number)[], message: string) => void) {
...
export default defineRule({
  ...
  create(context) {
    ...
    if (file.endsWith("docket.json")) {
      return {
        Document(node) {
          const parsed = (JSON.parse(context.sourceCode.getText()) as unknown);
