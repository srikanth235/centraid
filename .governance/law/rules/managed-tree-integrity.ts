...
export default defineArrivalRule({
  ...
  check(arrival, ctx) {
    const waived = new Set(ctx.options.waivedUnits as Iterable<unknown> | undefined);
    ...
    for (const [index, row] of tree.files.entries()) {
      ...
      if (row.actual !== row.recorded) {
        ctx.report(
          at,
          `${row.path}: drifted from the digest recorded at apply time. If you meant to change it, re-record it (\`node .governance/law/digest.ts --record\` for the law's own generator) — otherwise restore it`
        );
        continue;
      }
