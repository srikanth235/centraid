...
import { git, gitBytes, parseNameStatus } from "./git.ts";
import { byteCompare } from "./digest.ts";
import { readPacks } from "../eslint.config.ts";
import type { GitRange, PendingCommit, TreeSource } from "./types.ts";
...
export function documentIntegrityRules() {
  const declared = readPacks().flatMap(
    (pack) => (pack.options?.["doc-integrity"] as { rules?: string[] } | undefined)?.rules ?? []
  );
 ...
 */
function shellGlob(glob: string) {
 ...
 * @returns {string[]} The section's lines, in order.
 */
export function extractReceiptSection(text: string, heading: string) {
  const out = [];
  let inside = false;
  for (const line of text.split("\n")) {
 ...
 */
export function collectFrozen(range: GitRange, pending: boolean) {
  ...
  const treeOids = (rev: string): Map<string, string> => {
      ...
      rev === ""
        ? git(["ls-files", "-s", "-z"])
            ...
            .map((line) => {
              ...
              return [file, meta.split(" ")[1]] as [string, string];
            })
        : git(["ls-tree", "-r", "-z", rev])
            .split("\0")
            .filter(Boolean)
            .map((line) => {
              const [meta, file] = line.split("\t");
              return [file, meta.split(" ")[2]] as [string, string];
            });
    return new Map(rows);
  };
  ...
  for (const rule of documentIntegrityRules()) {
    const targets =
      rule.mode === "frozen-files"
        ? baseFiles.filter((file) => shellGlob(String(rule.target)).test(file))
        : [rule.target];
 ...
 */
export function collectCites(text: string) {
  const cites = new Set();
  ...
  return [...cites].sort((a, b) => byteCompare(String(a), String(b)));
}
 ...
 */
export function collectRulings(text: string) {
    ...
    for (const [offset, line] of paragraph.lines.entries()) {
      ...
      for (const match of line.matchAll(RULING_ID)) {
        rulings.push({
          ...
          cites: [...cites].sort((a, b) => byteCompare(String(a), String(b))),
        });
      }
    }
  }
 ...
 */
export function collectReceipts(range: GitRange, pending: PendingCommit | null, source: TreeSource) {
  const dir = "receipts";
 ...
 */
function arrivalFilesIn(range: GitRange) {
 ...
 */
function addedLines(range: GitRange, staged: boolean, file: string) {
  const collect = (args) => {
    let raw = "";
    try {
 ...
 */
export function collectDocument(range: GitRange, pending: PendingCommit | null, file: string) {
  ...
  return {
    ...
    issues: [...issues].sort((a, b) => Number(a) - Number(b)),
    lines: added.length,
  };
}
 ...
 * @param {object} source The tree reader for the side being judged.
 * @returns {{path: string, exists: boolean, rows: Record<string, unknown>[], rowsOnBase: string[]}} The docket.
 */
export function collectDocket(range: GitRange | null, source: TreeSource) {
  ...
  const parse = (text: string): Record<string, unknown>[] => {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      const rows = rec.rows ?? rec.entries ?? [];
