...
// `scripts/check-ledgers.mjs`, which is pure data behind a `main()` guard, so
...
export async function ledgerSections() {
  try {
    const module = await import("../../../scripts/check-ledgers.mjs");
 ...
 */
export function judgeSection(before, after, direction) {
...
function jsonAt(rev, file) {
  try {
    return JSON.parse(git(["cat-file", "blob", `${rev}:${file}`]));
  } catch {
    return null;
  }
}
 ...
 */
export async function collectGates(range, paths, commits, staged) {
