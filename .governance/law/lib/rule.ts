...
// needs git, the network or the clock belongs in the generator (`arrival.ts`),
...
import { asArrival } from "./types.ts";
import type { Arrival, Door, Surface } from "./types.ts";

/** Doors a rule can be enforced at. See README.md § The two doors. */
export const DOORS: readonly Door[] = Object.freeze(["hook", "window", "owner"]);
 ...
 */
export const SURFACES: readonly Surface[] = Object.freeze(["arrival", "documents", "all"]);

export type LawRuleSpec = {
  id: string;
  statute: string;
  door: Door;
  description: string;
...
export type ArrivalContext = {
  ...
  report: (pointer: (string | number)[], message: string) => void;
};

export type ArrivalRuleSpec = Omit<LawRuleSpec, "create" | "surface"> & {
  check: (arrival: Arrival, ctx: ArrivalContext) => void;
};

type JsonAstNode = {
  type?: string;
  body?: JsonAstNode;
  members?: { name: { value: string }; value: JsonAstNode }[];
 ...
 */
export function defineRule(spec: LawRuleSpec): Rule.RuleModule {
  const {
    id,
    statute,
    door,
 ...
 */
export function ruleTester(surface: "arrival" | "documents" = "arrival") {
 ...
 */
export function locate(document: JsonAstNode, pointer: (string | number)[]): JsonAstNode | null {
  let node: JsonAstNode | undefined = document.body;
  for (const step of pointer) {
    ...
    if (node?.type === "Array") {
      node = typeof step === "number" ? node.elements?.[step] : undefined;
      if (!node) return null;
      continue;
    }
    return null;
 ...
 * @param {(arrival: Arrival, ctx: object) => void} spec.check The judgement.
 * @returns {object} An ESLint rule object.
 */
export function defineArrivalRule(spec: ArrivalRuleSpec): Rule.RuleModule {
  ...
  return defineRule({
    ...
    create(context) {
      return {
        Document(node: JsonAstNode) {
          const arrival = asArrival(JSON.parse(context.sourceCode.getText()) as unknown);
          check(arrival, {
            options: (context.options[0] as Record<string, unknown> | undefined) ?? {},
            report: (pointer, message) =>
              context.report({ node: (locate(node, pointer) ?? node) as never, message }),
          });
        },
      };
    },
