...
import { byteCompare, globToRegExp } from "./digest.ts";
import { DOCUMENT_PATTERNS } from "../eslint.config.ts";
import type { CommitRow, GitRange, PendingCommit, Waiver } from "./types.ts";
 ...
 */
function closeComment(text: string) {
 ...
 */
function readReason(lines: string[], index: number, rest: string) {
 ...
 */
function waiversIn(text: string, source: string, options: Record<string, unknown> = {}) {
 ...
 */
function disablesIn(line: string, file: string) {
 ...
 */
export function addedLinesByFile(range: GitRange, staged: boolean) {
  const out = new Map();
  const absorb = (raw) => {
    let file = null;
 ...
 */
export function collectWaiversFromLines(entries: Iterable<[string, string[]]>) {
  const rows = [];
 ...
 */
function sortWaivers(a: Record<string, unknown>, b: Record<string, unknown>) {
  return byteCompare(
 ...
 */
export function collectWaivers(commits: CommitRow[], pending: PendingCommit | null, range: GitRange | null = null) {
  const waivers = [];
