...
import { globToRegExp } from "./digest.ts";
 ...
 */
export function estateClassifier(lawPaths?: string[]) {
 ...
 */
export function tagEstates(rows: {path: string, status: string}[], classify: (path: string) => string) {
 ...
 */
export function estatesOf(rows: {estate: string}[]) {
