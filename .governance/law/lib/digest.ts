...
// pure, synchronous rules; `arrival.test.ts` pins the two implementations
 ...
 */
export function byteCompare(a: string, b: string) {
 ...
 */
export function digestEntries(entries: Iterable<[string, Buffer]>) {
  const rows = [...entries]
    .filter(([rel]) => kept(rel))
 ...
 */
export function dirDigest(dir: string) {
  let stat;
  try {
    stat = statSync(dir);
  } catch {
  ...
  return digestEntries(
    walk(dir)
      .filter((rel) => kept(rel))
      .map((rel): [string, Buffer] => [rel, readFileSync(path.join(dir, rel))])
  );
}
 ...
 */
export function matchesAny(file: string, matchers: RegExp[]) {
 ...
 */
export function lawDigest(entries: {path: string, sha: string}[], matchers: RegExp[]) {
