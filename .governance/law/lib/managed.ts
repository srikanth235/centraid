...
// on every run.
import { byteCompare, digestEntries, sha256 } from "./digest.ts";
import type { PackDeclaration, TreeSource } from "./types.ts";
 ...
 */
export function parsePacksLock(text: string) {
  const packs = [];
  let current = null;
  let inDigest = false;
 ...
 */
export function parseManagedDigests(text: string) {
  const out = {};
  let inBlock = false;
  for (const line of text.split("\n")) {
 ...
 * @returns {{packs: PackDeclaration[], unrecorded: object[], files: object[], kitVersion: string}} The section.
 */
export function collectManagedTree(source: TreeSource) {
