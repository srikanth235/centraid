...
import type { FileRow, TreeSource } from "./types.ts";
 ...
 */
export function gitBytes(args: string[]) {
  return execFileSync("git", args, {
    cwd: ROOT,
    maxBuffer: 64 * 1024 * 1024,
    stdio: ["ignore", "pipe", "pipe"],
 ...
 */
export function parseNameStatus(raw: string): FileRow[] {
  ...
  for (let i = 0; i < fields.length; i += 1) {
    ...
    rows.push({ path: file, status: status.charAt(0) });
    i += extra;
  }
 ...
 */
export function readBlobs(specs: string[]): Map<string, Buffer | null> {
 ...
 *   The reader.
 */
export function treeSource(rev: string | null): TreeSource {
