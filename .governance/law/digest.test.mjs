...
import { RECORDED_PATHS, record } from "./digest.mjs";
...
test("it inserts its own rows, sorted, and touches no other line", () => {
  ...
  assert.ok(lines.includes("  .governance/law/arrival.mjs: 0"));
...
test("re-recording replaces its own rows rather than duplicating them", () => {
  ...
  const twice = record(once, { ...all, ".governance/law/arrival.mjs": "999" });
  assert.equal(twice.match(/law\/arrival\.mjs/gu).length, 1);
  assert.match(twice, /\.governance\/law\/arrival\.mjs: 999/u);
...
test("it owns the generator and every module it is built from", () => {
  ...
  assert.equal(RECORDED_PATHS[0], ".governance/law/arrival.mjs");
  for (const file of [
    ".governance/law/lib/digest.mjs",
    ".governance/law/lib/git.mjs",
    ".governance/law/lib/managed.mjs",
    ".governance/law/lib/registries.mjs",
    ".governance/law/lib/rule.mjs",
  ]) {
