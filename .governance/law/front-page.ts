import type { Arrival, LawReport } from "./lib/types.ts";
 ...
 */
export function renderRegistries(arrival: Arrival | null) {
  const { changelog, decisions, docket, receipts } = arrival?.registries ?? {};
  const cite = (issues: unknown[]) => issues.map((issue) => `#${issue}`).join(", ");
 ...
 */
export function nameFiles(files: string[], limit: number = 12) {
 ...
 * Render a run.
 *
 * @param {object} report The object `run.ts --json` prints.
 * @param {object} [arrival] The record the run linted, for the registry lines.
 * @returns {string} Markdown, with a trailing newline.
 */
export function renderFrontPage(report: LawReport, arrival: Arrival = null) {
  ...
  if (report.codeowners) {
    lines.push(
      "",
      `law estate: ${report.codeowners.paths} paths, CODEOWNERS ${report.codeowners.inSync ? "in sync" : "**drifted** — run `node .governance/law/codeowners.ts --write`"}`
    );
  }
