...
import { CODEOWNERS_PATH, OWNER, lawPaths, main, render, status } from "./codeowners.ts";
...
test("generation is one line per law glob, sorted, with a header saying it is generated", () => {
  ...
  assert.ok(text.includes("codeowners.ts --write"), "the header names how to regenerate it");
