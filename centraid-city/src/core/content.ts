...
export const tour = [
  {
    ...
    pages: [
      {
        body: 'Nothing in this city can happen until a vault exists, and founding one is deliberately not an event you attend. There is no ceremony, no founding ticket, and no first-run wall: a gateway started over a fresh data directory founds itself. It creates one marked Personal vault, the default any unscoped request lands in, and then enrols the host\'s own device in a single gateway.db transaction: member label "You", admin on that vault, because there is no separate owner role, only admin, write, and read. Shared vaults are created later by an explicit owner action. All of that happens synchronously, before any route can observe a gateway with no vaults.',
      },
      {
