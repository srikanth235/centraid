...
**The shell was the last holdout.** The rung shipped in #686 and the blueprint apps moved onto it, but `packages/client` still painted the raw fills as `color:` at 39 sites across 25 stylesheets, plus five identity variables (`--au-hue`, `--notice-hue`, `--tk-hue`, `--plan-tone`, three health/status tones) that laundered a hue into a `color:` one indirection away. On the **shell's own** surfaces — a different emitter, `--bg-l: 5%` rather than the app layer's 10%, and two surfaces (`--bg`, `--bg-app`) the blueprint grid cannot see — those fills measure **2.04–5.03:1** on light and **3.12–8.44:1** on dark: 17 of 32 cells below AA, every hue failing on at least one theme, and amber missing even the 3:1 non-text floor an icon glyph owes. Rebinding the ink puts the worst cell at **4.55:1** (light) / **6.22:1** (dark).

Where a surface used one variable for _both_ jobs it now carries two — `--au-hue` / `--au-ink`, `--notice-hue` / `--notice-ink`, `--tk-hue` / `--tk-ink`, `--plan-tone` / `--plan-ink`. **The fill stays the fill**: washes, dots, rails, bars and 2px edges keep `--c-<name>`, because deepening those would dull the UI to buy contrast nothing is reading. Only the `color:` moves.

Two gates hold it. `scripts/lint-design-tokens.mjs` counts `paletteHueAsText` — a `color:` (or `-webkit-text-fill-color:`) naming a bare `--c-<hue>` anywhere under `packages/client/src`, `packages/blueprints/apps` or `packages/design/kit` — as ratcheted debt, now at **zero**; `background-color`, `border-color` and the rest are deliberately outside its reach. `packages/design/src/contrast-shell-palette.test.ts` re-measures every rung against all four shell surfaces bare _and_ on self-washes from 6% to 16%, per theme.

**16% is the wash ceiling under a palette ink**, not just the 12% the solve targets: at 18% indigo and violet fall back to 4.44 / 4.49, which is why `ApprovalsScreen`'s identity tile came down to 12% — the same normalisation #686 applied to the state washes.

A highlighting scheme owes a second contract the contrast grids cannot see. The builder's syntax tokens were tag=rose, attr=**violet**, str=forest, key=**indigo** — two hues already only 0.068 apart as fills, which solving to one floor pulled to **0.075** (light) / **0.040** (dark), two token classes the eye reads as one colour. `.tokAttr` moved to amber, taking the set's closest pair to **0.119 / 0.126**; `packages/client/src/react/shell/routes/builder/BuilderCode.tokens.test.ts` pins it where the scheme is chosen, the way `kind-colours.test.ts` pins the `docs` app's.
...
**`--t-tiny` is not the eyebrow rung.** It is sans/500 in the chrome by design, and the five shell rules that take the shorthand are `<select>` chrome, "Save"/"Cancel" buttons and a pill holding an agent's title — prose and controls, which the clause above puts on sans. The chrome's eyebrow idiom is mono at **8–10.5px**, below this rung: 94 of the 120 `text-transform: uppercase` rules in `packages/client/src` set a mono family themselves, and 90 of those sit under 11px. That sub-11px mono rung is **unnamed** — recorded as debt in the #686 entry of [docs/decisions.md](docs/decisions.md) — which is why an eyebrow here spells mono by hand rather than reaching for `--t-tiny`. The blueprint surface's `--t-tiny` _is_ mono, because its rung is 0.6rem (9.6px) and lands inside that band and ten of its twelve consumers are uppercase eyebrows.

That makes `--t-tiny` the **one** role whose family and weight differ between the two emitters, and that is a **known violation held under a waiver, not a permitted divergence**. The rule — _size and line-height may differ per surface; family and weight may not_ — is enforced for every `--t-*` role by `packages/design/src/type-role-parity.test.ts`; `--t-tiny` sits in that test's `ROLE_PARITY_ALLOWLIST` because neither side can move without visible collateral, and the resolution is to name the eyebrow role rather than to pick a winner. See the #686 entries in [docs/decisions.md](docs/decisions.md).
