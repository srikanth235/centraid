...
name: Centraid
description: >-
  The Binding Layer: one ink-on-paper superapp shell wrapping many first-party apps across a desktop shell, an installable web PWA, and native mobile. Values are pinned to packages/design/src.
colors:
...
| Where | Before | After |
| --- | --- | --- |
| [ApprovalsScreen.tsx](packages/client/src/react/screens/ApprovalsScreen.tsx) empty state | "Staged writes, lapsed connections and requests for wider access appear here. This page is empty most of the time, and that is the healthy state." | "Anything that needs your OK — a message waiting to send, an account to reconnect, an app asking for access — shows up here." |
