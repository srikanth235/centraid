...
Since [#890](https://github.com/srikanth235/centraid/issues/890) the mobile layer holds five properties the rest of this document assumes:
...
2. **Device signal lands before merge.** `mobile-device-gate` in `ci.yml` runs the critical five on Android within twelve minutes warm; `mobile-canary.yml` runs the full roster per merge; `e2e.yml` owns nightly depth. Android gates PRs per D1 in [docs/decisions.md](docs/decisions.md#mobile-testing-890).
3. **Three linters make a green run unfakeable.** `lint:e2e-wiring` (a flow the ledger claims and no lane runs is a hard failure), `lint:mobile-testids` (every id a flow references exists in the app source), and `lint:seat-verbs` (every act only a phone can perform has a journey or a dated gap). Each carries a self-test and silent-no-op guards.
