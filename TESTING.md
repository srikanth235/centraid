...
It has two halves, because the repository currently holds two trees. **[The v1 gate profiles](#the-v1-gate-profiles-1020)** are the strategy for `crates/`, `contracts/`, `mobile/`, `desktop/`, `extension/` and `deploy/`; everything after that section is v0's, and stays current until wave 6 deletes that tree ([#1020](https://github.com/srikanth235/centraid/issues/1020)). The axiom below governs both.
...
Everything below this section is v0's testing strategy, and it stays current until wave 6 deletes that tree. The v1 tree's strategy is not a second taxonomy — **it is the gate**. `cargo xtask gate --profile <name>` is the only entrypoint CI runs for `crates/`, `contracts/`, `mobile/`, `desktop/`, `extension/` and `deploy/`, each profile is stated as a **concatenation of the one before it**, and each has a feedback-time budget in a down-only ledger ([R-1020-10](docs/decisions.md#v1-platform--rust-core-kmp-shell-electron-seat-gateway-anywhere-1020), [D-1020-B2](docs/decisions.md#decisions--lane-b2-1020)).

| Profile | Budget | Steps it adds | What it is for |
| --- | --- | --- | --- |
| `local` | 120 s **warm** | `fmt`, `clippy`, `test`, `rules`, `ledgers` | The edit-run loop. Scored on a warm tree — the tree the loop runs on — and a cold run is charged to `coldLocalProfileSeconds` instead and **fails** where that key states no ceiling. Being cold is never what makes a slow gate green. |
| `pr` | 1500 s | `buf`, `deny`, `ci-policy`, `secrets`, `osv`, `release-build`, `ts-static`, `desktop-unit`, `extension-unit`, `advisory`, `lockfile`, `prompt-injection`, `sim` (25 seeds), `call-budget`, `fault-door` | What every pull request satisfies. 1500 s is the cache-miss tail; a Cargo-cache hit is the p95 the rung table budgets. |
| `nightly` | unbounded | `sim-nightly` (250 seeds), `v0-oracle`, `desktop-e2e`, `extension-e2e`, `device-lanes`, `lane-health` | The deeper sweep, plus everything that needs a display or a phone. |
| `release` | unbounded | `restore-drill`, `artifact-identity`, `prebuilt-core-required`, `vps-smoke` | What a tag must survive. `no_step_in_the_release_profile_is_a_stub` greps the gate's own source for the stub sentence, so **the profile cannot pass vacuously**. |
| `mobile-jvm` | 420 s | `mobile-jvm` | The Kotlin JVM suites and the generated-artifact drift check, in their own profile rather than charged to `pr` — a different toolchain with a different cold cost, and one number answering for two build systems answers for neither. |

### What each kind of proof covers, and where it stops

- **Parity fixtures are the app-level spec, and they are generated by executing v0.** `contracts/tools/export-<app>-parity.ts` runs the real v0 handlers over a freshly founded vault with the app's seed and a scripted command set, and dumps `{query, input, output}` plus the canonicalised rows and the committed DDL — never a `.db.gz`, because a founded vault mints ids off the clock and is not byte-reproducible. Regeneration is idempotent and the gate diffs it. The Rust parity test compares **values**, with ordering as v0's; a fixture is **never edited by hand**, so a divergence is either a port bug or a recorded ruling. While the v0 tree exists an oracle test proves v0 still answers its own fixture, which is what `v0-oracle` runs nightly.
- **The simulation is the sync proof, and it is the primary one** ([R-1020-30](docs/decisions.md#v1-platform--rust-core-kmp-shell-electron-seat-gateway-anywhere-1020)). One gateway and N seats in one process under `turmoil` — partitions, reordering, duplication, latency, crashes, clock skew — with a fixed seed, asserting seven invariants after every schedule. Every host is real except the network: real vault, real doors, real applier, real outbox, real sync driver, real on-disk SQLite. **A failing seed becomes a test and is replayed for ever.** It speaks UDP rather than TCP, because production is QUIC and because the product may not open a listening TCP socket at all.
- **The drills run the real binary.** `restore-drill` founds a vault, enrols a seat, writes commits, takes a generation, **deletes the live data directory**, runs the real `centraid recover` into a fresh one, compares every table and row, and proves the old seat is told epoch-mismatch, re-pairs and converges. It refuses a census of zero rows outright, so a vault with nothing in it cannot pass by having nothing to lose. What it does **not** yet prove is the iroh pairing handshake running again — the seat's half is played by the gateway's applier, and that gap is stated rather than implied.
- **Structural rules are gates, not conventions.** `sql-confinement` (SQL only under `crates/{ontology,vault,seat,search}` and `crates/apps/kit`), `abi-five-symbols`, `no-listening-socket` (outside `#[cfg(feature = "blob-door")]`), `commonmain-no-platform-import`. Two of them are checked twice at different layers: the no-listener rule catches the _shape_ in source, and a test spawns the real gateway and reads the kernel's own answer out of `/proc/net/tcp` — a dependency could open a listener without the string ever appearing in this repository.
- **Ledgers only fall.** `gate-budgets`, `compile-time`, `library-size`. `cargo xtask measure --write` is the only writer and only ever lowers a number; a gate run measures and reports but never writes, because a ratchet that records whatever the last run cost is a log. A raise needs an owner ruling, spent once — it has been spent once, on 2026-09-12, and the ledgers are down-only from that baseline.

### What only a device proves, and how that is kept honest

A gate cell with no hardware behind it is the failure mode this whole document is about, so v1's answer is a **loud skip with a runner contract**. `CENTRAID_DEVICE_RUNNER` flips `device-lanes` from skipping to **requiring** `xcrun devicectl list devices` (a physical iPhone — not `simctl`, which lists simulators) and `adb devices`: a runner that claims the label and has no phone goes **red**, because that is an infrastructure fault rather than a skip. `--lane <name>` narrows a run to one cell, and a name matching nothing is an error rather than an empty run reporting PASS over zero steps.

The measurements that only a device can take, and are therefore not taken yet:

| Claim | Why no gate here can make it | Where the command lives |
| --- | --- | --- |
| The six absolute mobile targets — p95 interaction latency, frame misses during concurrent sync, Notes typing latency, memory at a 50k-asset library, battery per background pass, cold start at year-3 volume | They are stated on two named reference devices and this container has neither. Every one is parked `_intended` in [`tests/journeys.json`](tests/journeys.json) so the ratchet cannot see it and it gates nothing. | [docs/release/v1-handoffs.md](docs/release/v1-handoffs.md) |
| The iOS per-state transfer promise (foreground, backgrounded, locked, suspended, relaunched) | An iroh connection does not survive app suspension, and Apple's system-managed background transfers are HTTP only. The protocol, the five states, the four fixed evidence file names and the decision table are written; the run is not. | `mobile/maestro/ios-transfer-experiment.md` |
| Open question 10's criteria (b) and (c) | Both are device measurements. Criterion (a) has tripped at 1.72×; a second trip flips the ruling with no further debate. | [docs/decisions.md](docs/decisions.md#decisions--wave-3-close-lane-x3-1020) |
| `kotlinNativeLinkSeconds` | No machine in CI links Kotlin/Native. Filling it with a JVM number is the one thing the key exists to prevent, so it stays `null` — and a null cannot be ratcheted, which costs the down-only discipline nothing. | `mobile/README.md` |
| The Swift fixture test, the iOS build, the Android device build, the Maestro flows | No Xcode, no Android SDK, no device. Each is written in full and **fails loudly** rather than skipping green; four iOS platform services are deliberately unimplemented and refuse rather than, for instance, writing a credential to a plist. | `mobile/README.md`, [docs/release/v1-handoffs.md](docs/release/v1-handoffs.md) |

### Two reds inherited from `main`, named and not hidden

`pr` is red on two findings that predate this work, and neither was answered by widening an allowlist — **a gate whose first act is to widen its own allowlist has gated nothing**:

- `secrets` — gitleaks on `packages/model-runtime/LICENSES.md`, and on `contracts/golden/format-golden.json`'s `dataKeyHex`, which is the test vector the cross-language format golden is sealed with and has to be in the file for the golden to be checkable at all. Whether a published test vector belongs in `.gitleaks.toml` or the fixture should carry its key out of band is the owner's call.
- `osv` — `astro@7.1.5`, CRITICAL, in `bun.lock`. A dependency bump outside this umbrella's scope.

Both are owner hand-offs in [docs/release/v1-handoffs.md](docs/release/v1-handoffs.md). A false positive is a different thing and is fixed **in the gate**: the `secrets` step's own walk of `target/` was fixed by classifying gitleaks' report through git's answer to "is this a file we wrote?", with `.gitleaks.toml` untouched.
...
| Boot-the-artifact smoke | `scripts/gateway-package/smoke.mjs` (+ `--base-url` for containers) + path-filtered `gateway-package` workflow | "builds but doesn't start" (host binary **and** Docker image with `/data` volume) | **PR path-filtered** (gateway/protocol/`deploy/docker`/scripts) + manual `bun run gateway:package:smoke` |
...
| Mobile boot-condition | `tests/integration-mobile/*.integration.test.ts` | the app × state matrix (`dayone`/`pending`/`offline`/`stale`/`conflict`/`parked`) produced by a **real replica session against a real gateway process**, on Linux with no device — plus the seat **fault lanes**, which kill the process at each boundary of a re-bootstrap swap and ask whether the member's queued intent is still there ([#1014](https://github.com/srikanth235/centraid/issues/1014) C5/T6, `seat-swap-faults`) | per PR |
...
4. **A ceiling states its volume and its hardware, or it is not a budget (#927).** [`tests/journeys.json`](tests/journeys.json) replaced four per-surface experience files, the rig register and the query-count file, whose keys named the SURFACE and nothing else — several of them silently meant "empty vault". Every entry is keyed `surface / journey / volume / hardware` against a declared vocabulary, and names the trace SPANS it is measured from and the CONSUMERS that assert it, so a ceiling with no reader and a reader with no ceiling are both visible. `bun run lint:journey-ledger` holds that shape and fails on any surviving reference to the five files it replaced. **Two surfaces gate on nothing yet, and the ledger says so out loud**: every `desktop/*` row is keyed to the `mock-gateway` volume and every `mobile/*` seat row to `ci-ios-sim` or `device-fixture`, because Electron does not launch on a display-less runner and an emulator has neither a phone's thermal envelope nor its flash. Those ceilings are parked under a leading underscore (`_intendedCeilingMs`), which the ratchet cannot see, so they state intent without gating vacuously — the rung that ends the park is the device rung ([PS-device-rung](docs/decisions.md#perf-and-scale-infrastructure-927)), and until one of its cells runs, an emulator number is a lower bound and never a budget.
...
Three seeded chaos lanes ask the same question of different planes: does adversity change _whether_ work is applied, or only _when_? `tests/quality/network-chaos.integration.test.ts` injects eight fault shapes over a **real** iroh connection against the replica intent plane — latency, jitter, fragmentation, asymmetric bandwidth, mid-request abort, mid-response disconnect, endpoint restart, address rebind — and asserts no-loss, apply-exactly-once and convergence on real `schedule_task` and `replica_intent_outcome` rows. `tests/quality/component-chaos.integration.test.ts` kills a gateway, replica, automation worker or model runtime mid-work and asserts the same laws across the restart. `tests/quality/share-path-chaos.integration.test.ts` faults the #929 subscription plane's peer wire — a tail page cut, a blob chunk cut, a duplicated delivery, cataloged as `SHARE_PATH_FAULTS` — and asserts the six deterministic apply rules in [docs/protocol.md](docs/protocol.md) hold under each, plus the offline case: an audience cut while the origin edits a shared subject twice converges on the latest value with no lost update and no double ([#1014](https://github.com/srikanth235/centraid/issues/1014), T16/S3). Both lease-shaped faults use the primitives' injectable clocks, so neither lane waits on a wall clock or asserts on timing.
...
`HOME_READY_MARKER` is the Home band's accessibility label, and the band renders over `DayOne` as well as over the launcher grid. The same band also draws at the foot of every frame place root (R-NY-1, [#1015](https://github.com/srikanth235/centraid/issues/1015)), so the marker proves that a Home band is up, not that the member is on Home: a flow that must prove Home after leaving a place waits for `home-screen` (`TEST_IDS.home.screen`) or `home-grid`. A flow that waits only for it walks into an empty-vault Home and then fails on its own tile selector — which is exactly what the 2026-09-01 nightly reported twelve times: `Element not found: Open <App>`, naming the app, while the app was correct and the demo corpus had simply arrived after the phone cloned. `AWAIT_LAUNCHER` waits for `home-grid`, which `LauncherGrid` alone publishes, and `lint:e2e-flows`'s RULE `launcher-await` now requires it in every chunk that reaches a launcher tile. Flows that deliberately face an empty vault mark the chunk `# e2e-lint-allow: launcher-await — <reason>`.
...
| `bun run governance:law:test` | the law's `RuleTester` suites — `valid` and `invalid` for every rule in `.governance/law/rules/` — plus the arrival generator's own cases, under `node --test`. Runs at rung 0 inside `bash .governance/run.sh` and again in CI's `scripts:test`; the `invalid` case is a directive's demonstrated red ([#1005](https://github.com/srikanth235/centraid/issues/1005)) |
| `bun run governance:law` | the law over the current range at the window door: the arrival record is generated, every enabled rule reports one line, and `--front-page <path>` renders the run as a PR body |
| `node .governance/law/parity.mjs --last 50` | the port's parity replay: the retired shell audit pack and the rules over the last fifty trunk commits, divergences declared in `parity-expectations.json`. Kept as the evidence the four ported directives check what they always checked |
| `node .governance/law/run.mjs --range bb964a7e..3df6d552` | the #1002 replay, whose expected findings are pinned in `.governance/law/fixtures/replay/1002.json` and asserted by `replay.test.mjs` |
| `.github/workflows/ci.yml` (rung 2) | parallel **static** + **gates** + **verify** + **coverage-shard**×4 → **coverage**, plus **new-test-burn-in** and a one-leg **mobile-device-gate**; required **check** aggregator (ruleset-required), which also enforces the `pr-gate` wall-clock budget; **publish-report** on main only; Bun/Turbo/Cargo caches. `governance.yml` is kit-managed and rolls up into NO aggregate — it needs its own required-check entry (see [docs/decisions.md](docs/decisions.md#the-pr-gate-loop-892)), and since [#1005](https://github.com/srikanth235/centraid/issues/1005) it is also the only lane that runs the law at its window door, so the required-check entry is what makes `registry-completeness` and `receipt-per-issue` block a merge |
