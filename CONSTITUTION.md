...
- Every commit is agent-authored: token spend and human-steering events are recorded as durable, append-only ledgers (`COSTS.md`, `STEERING.md`) so provenance survives squash merges.
- Issues are the unit of work; receipts under `receipts/` are the durable proof of completion and crosswalk back to the issue they close.
...
1. Open a PR that modifies this file **and** the directive folder under `.governance/packs/srikanth235/centraid/directives/` (or the source pack's directive folder when amending a pack-installed directive — see `.governance/packs.lock`) in the same commit.
...
- 2026-05-07 — @srikanth — Initial constitution bootstrapped via `governance init` (preset: standard, hook strategy: githooks, pack: governance-kit/core).
