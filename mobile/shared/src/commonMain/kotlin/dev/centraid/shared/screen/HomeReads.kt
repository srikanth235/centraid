...
public object HomeReads {
    ...
    public val READS: List<TileRead> = listOf(
        TileRead(
            ...
            query = PageQuery(
                name = "home.photos",
                // `content_id` IS SELECTED FOR THE BYTE DOOR, not for the body:
                // a thumbnail is located by content and read as the ASSET, so
                // the runtime needs both ids on the same row. See
                // `HomeRuntime.thumbnails`.
                select = listOf("asset_id", "title", "captured_at", "content_id"),
                from = "media_asset",
     ...
     * them.
     *
     * [thumbnails] maps an asset id to a local file the platform can open, and
     * is empty for every app but Photos. It is resolved by the runtime through
     * the core's byte door BEFORE this is called, because a tile body is a
     * finished message: a cell that arrived without its path and acquired one
     * later would be a second state for one photograph.
     */
    public fun arrived(
        appId: String,
        rows: List<Row>,
        ...
        return HomeEvent(
            tile = HomeEvent.TileArrived(
                ...
                count_label = label,
                body = bodyFor(appId, rows, thumbnails),
            ),
        )
    }
                ...
                photos = TileBody.Photos(
                    ...
                    cells = rows.take(PHOTO_CELLS).map { row ->
                        ...
                        TileBody.Photos.Cell(
                            ...
                            // they are a type no surface may embed. Neither is
                            // an error and both draw the same cell.
                            thumbnail_path = thumbnails[assetId],
                        )
                    },
                ),
            )
