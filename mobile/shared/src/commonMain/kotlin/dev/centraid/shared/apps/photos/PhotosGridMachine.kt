...
public object PhotosGridMachine : ScreenMachine<PhotosGridState, PhotosGridEvent> {
    ...
    override fun reduce(state: PhotosGridState, event: PhotosGridEvent): Step<PhotosGridState> =
        when {
            ...
            // D-1025-S7-20 is the same event over the same table. Re-read the
            // first page rather than patching a cell in place: the page's ORDER is by
            ...
            event.rows_changed != null ->
                if (event.rows_changed.asset_ids.none { it in shownIds(state) }) {
                    Step(state)
                } else {
                    firstLoad(state)
                }
            ...
            // move a photograph in time — and a first-page re-read here would
            // throw away every page the member had scrolled.
