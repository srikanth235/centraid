...
public object TallyListMachine : ScreenMachine<TallyListState, TallyListEvent> {
    ...
    /**
     * THE TABLE THIS LIST IS. `tally_expense` and nothing else: the split,
     * payer and allocation tables hang off an expense and move with it, so a
     * row this screen shows cannot change without its `tally_expense` row
     * changing too — and listing them here would be four re-reads of one page
     * for one edit.
     */
    ...
    private const val TABLE: String = "tally_expense"

    override fun seatChanged(seat: SeatState): TallyListEvent = TallyListEvent(seat_changed = TallyListEvent.SeatChanged(seat = seat))
}
