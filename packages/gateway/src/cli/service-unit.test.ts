...
describe('service-unit', () => {
  test('launchAgentPlistPath mounts under ~/Library/LaunchAgents with the label as filename', () => {
    ...
    expect(p).toBe(
      path.join('/Users/land', 'Library', 'LaunchAgents', 'dev.centraid.gateway.plist'),
    );
  });
  ...
  test('systemdUnitPath mounts under ~/.config/systemd/user', () => {
    ...
    expect(p).toBe(
      path.join('/home/land', '.config', 'systemd', 'user', 'centraid-gateway.service'),
    );
  });
  ...
  test('buildSystemdUnit quotes argv tokens containing spaces', () => {
    const spaced: ServiceUnitSpec = {
      ...spec,
      args: ['serve', '--data-dir', '/a path/with spaces'],
    };
