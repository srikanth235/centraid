...
vi.mock(import('node:child_process'), () => ({
  spawnSync: mocked.spawnSync as unknown as typeof import('node:child_process').spawnSync,
}));
...
describe('key-store', () => {
  beforeEach(() => {
    ...
    mocked.spawnSync.mockReturnValue(spawnResult({ status: 1, stderr: 'not found', stdout: '' }));
  });
  ...
  test('explicit environment and service-manager credentials wrap the key store', async () => {
    ...
    mocked.spawnSync.mockReturnValue(spawnResult({ status: 0, stdout: `${wrappingKey}\n` }));
  ...
  test('credential loading reports platform failures and malformed wrapping keys', async () => {
    ...
    vi.spyOn(process, 'platform', 'get').mockReturnValue('linux');
    mocked.spawnSync.mockReturnValue(spawnResult({ status: 1, stderr: 'decrypt failed' }));
    expect(() =>
