...
test('adoption reads every key already wrapped under the host credential', async () => {
  ...
  for (const [name, secret] of secrets) expect(store.load(name)).toEqual(secret);
});
...
test('a credential that cannot decrypt the existing keys aborts before anything is committed', async () => {
  ...
  expect(await fs.readFile(path.join(keysDir, 'endpoint-key.bin'), 'utf8')).toBe(before);
});
...
test('a malformed wrapping key is rejected as a bad install argument', async () => {
  ...
  await expect(
    adoptKeyStoreCredential(failing(), credentialFor(keysDir, Buffer.alloc(4).toString('base64'))),
...
test('adoption rewraps an unprotected envelope and skips partial temp files', async () => {
  ...
  expect(await fs.readFile(path.join(keysDir, 'endpoint-key.bin.tmp1234'), 'utf8')).toBe(
    'partial write',
  );
});
