...
test('deriving never mints the endpoint identity and never throws on unreadable custody', async () => {
  ...
  expect(landlordBearerForDataDir(dataDir, { masterKey: Buffer.alloc(32, 0x42) })).toBeUndefined();
...
test('an endpoint key stored outside the layout is not mistaken for this data dir', async () => {
  ...
  new KeyStore(path.join(dataDir, 'not-keys'), { protector: aesGcmKeyProtector(masterKey) }).create(
    'endpoint-key.bin',
  );
