...
function parseMemberArgs(args: string[], fail: (msg: string, code?: number) => never): MemberArgs {
...
export function commandMembers(args: string[], fail: (msg: string, code?: number) => never): void {
  ...
  try {
    if (action === 'list') {
      for (const member of members.list()) {
        const devices = enrollments.list().filter((row) => row.memberId === member.memberId);
    ...
    if (!selector) {
      fail(`usage: members ${action} --data-dir <path> <member-id-or-label> …`, 2);
    }
    ...
    if (action === 'rename') {
      ...
      if (!label) {
        fail('usage: members rename --data-dir <path> <member-id-or-label> --label <new-label>', 2);
      }
      // Renaming keeps the id, so every binding, grant, and prior journal
      // attribution follows the person rather than the label.
      process.stdout.write(`${JSON.stringify(members.rename(member.memberId, label))}\n`);
      return;
    }

    const orphaned = members.vaultsLosingLastAdmin(member.memberId);
    const firstOrphaned = orphaned[0];
    if (firstOrphaned !== undefined && parsed.confirmLastAdmin !== firstOrphaned) {
      fail(
