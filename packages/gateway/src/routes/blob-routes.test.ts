...
describe("blob-routes", () => {
  ...
  test("open-ended byte ranges are clamped to one bounded streaming window", () => {
    expect(parseRange("bytes=0-", MAX_OPEN_RANGE_BYTES * 3)).toStrictEqual({
      start: 0,
      end: MAX_OPEN_RANGE_BYTES - 1,
    });
    expect(
      parseRange(`bytes=${MAX_OPEN_RANGE_BYTES}-`, MAX_OPEN_RANGE_BYTES * 3)
    ).toStrictEqual({
  ...
  test("bare raw JPEG ingress defers preview CPU to the bounded backstop sweep", async () => {
    ...
    expect(derivatives.map((row) => ({ ...row }))).toStrictEqual([
  ...
  test("raw curl-style upload makes a Flate-compressed PDF searchable through cheap text", async () => {
    ...
    expect(outcome.status).toBe("executed");
    const ids = (
      outcome as { output: { content_id: string; document_id: string } }
    ).output;
    ...
    expect(derivative.byte_size).toBe(
      Buffer.byteLength(derivative.text_content)
    );
    expect(derivative.text_content).toContain(
      "midnight narwhal renewal clause"
    );
    expect(
      plane.gateway
  ...
  test("typed contribution door accepts every derivative class and rejects malformed input", async () => {
    ...
    await Promise.all(
      ["thumb", "preview", "poster"].map(async (variant) => {
        expect(
          (await post(variant, PNG_BYTES, "image/png")).status,
          variant
        ).toBe(200);
      })
    );
    expect(
      (
        await post(
          "text",
          Buffer.from("PDF.js found a quasar clause"),
          "text/plain"
        )
      ).status
    ).toBe(200);
    expect(
      (
        await post(
          "transcript",
          Buffer.from("spoken cobalt detail"),
          "text/plain"
        )
      ).status
    ).toBe(200);
    ...
    const variants = plane.db.vault
      .prepare(
        "SELECT variant FROM core_content_derivative WHERE content_id = ? ORDER BY variant"
      )
      .all(contentId) as { variant: string }[];
    expect(variants.map((row) => row.variant)).toStrictEqual([
      "embedding",
      "phash",
      "poster",
      "preview",
    ...
    expect((await fetch(`${base}/${contentId}?variant=poster`)).status).toBe(
      200
    );
    expect(
      plane.gateway
      ...
      (outcome as { output: { document_id: string } }).output.document_id
    );

    expect(
      (await post("poster", Buffer.from("not an image"), "image/jpeg")).status
    ).toBe(400);
    expect((await post("unknown", Buffer.from("x"), "text/plain")).status).toBe(
      400
    );
    expect(
      (
