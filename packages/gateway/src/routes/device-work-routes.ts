...
function requestedCapabilities(
  ...
  return [...new Set(body.capabilities)].filter(
    (value): value is EnrichmentCapability =>
      typeof value === "string" && known.includes(value)
  );
}

function vaultIdFrom(body: Record<string, unknown>): string | undefined {
  return typeof body.vaultId === "string" && body.vaultId.length > 0
    ? body.vaultId
    : undefined;
}

export function makeDeviceWorkRouteHandler(
  deps: DeviceWorkRouteDeps
): RouteHandler {
  return async (
    req: IncomingMessage,
    res: ServerResponse
    ...
    if (url.pathname !== WORK_PATH && !url.pathname.startsWith(`${WORK_PATH}/`))
      return false;

    const method = req.method ?? "GET";
    const callerKey = callerDeviceKey(req);
    const allowed = callerKey
      ? new Set(deps.enrollments.vaultsFor(callerKey))
      : undefined;
    ...
    if (!requestId || !token)
      return sendJson(res, 400, { error: "invalid_lease" });
