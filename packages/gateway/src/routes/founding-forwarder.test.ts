...
test('a phone on the desktop tunnel cannot mint a founding ticket; the host can', async () => {
  ...
  try {
    ...
    const seen = JSON.parse(Buffer.from(mirrored.body).toString('utf8')) as Record<string, string>;
  ...
  expect(await direct.json()).toMatchObject({ ok: true, ticket: expect.any(String) });
});
