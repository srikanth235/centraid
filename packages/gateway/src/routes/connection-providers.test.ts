...
describe("PROVIDER_PRESETS capabilities", () => {
  ...
  it("maps pull connectors to syncs and send templates to actions", () => {
    ...
    expect(caps.actions.some((a) => a.templateId === "google-gmail-send")).toBe(
      true
    );
    expect(
      caps.actions.some((a) => a.toolName === "connector.pull_github.list")
    ).toBe(true);
  });

  it("google preset includes gmail sync and send action", () => {
    const google = PROVIDER_PRESETS.find((p) => p.id === "google")!;
    expect(google.capabilities.syncs.some((s) => s.kind === "pull.gmail")).toBe(
      true
    );
    expect(
    ...
    expect(
      new Set(google.capabilities.actions.map((action) => action.toolName)).size
    ).toBe(google.capabilities.actions.length);
  });
