...
describe('backup-observability-routes', () => {
  afterEach(() => {
  ...
  function harness(
    target: BackupTargetState | undefined,
    serviceOver: Partial<BackupService> = {},
  ) {
    ...
    const plane = {
      ...
      name: 'Main',
      db,
      rescheduleWalCapture: vi.fn<VaultPlane['rescheduleWalCapture']>(),
    };
    const vaults = {
  ...
  test('backup status exposes persisted policy echo and reconciliation evidence', async () => {
    ...
    const out = response();
    await expect(handler(request('/centraid/_gateway/backup'), out.res)).resolves.toBe(true);
  ...
  test('policy update surfaces policy_unmet while retaining the local desired policy', async () => {
    const refreshWalSchedule = vi.fn<BackupService['refreshWalSchedule']>(async () => undefined);
    const syncPolicy = vi.fn<BackupService['syncPolicy']>(async () => ({
  ...
  test('CAS-only status and verify-bucket work when no snapshot backup is configured', async () => {
    ...
    await handler(
      request('/centraid/_gateway/backup/verify-bucket/vault-a', 'POST'),
      verifyOut.res,
    );
    expect(verifyAgainstBucket).toHaveBeenCalledWith('vault-a');
