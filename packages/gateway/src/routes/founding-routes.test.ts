...
describe('founding-routes suite', () => {
  afterEach(async () => {
  ...
  test('founding ticket is single-outstanding and initialize enrolls one verified owner', async () => {
    ...
    const first = (await (
      await fetch(`${base}/centraid/_gateway/founding/ticket`, {
        method: 'POST',
      })
    ...
    expect(registry.list()).toStrictEqual([initialized.vault]);
    ...
    expect(enrollments.members.list().map((member) => member.label)).toStrictEqual(['You']);
    const owner = enrollments.members.list()[0]!;
    expect(enrollments.members.grants(owner.memberId)).toStrictEqual([
      { vaultId: initialized.vault.vaultId, role: 'admin' },
    ]);
    expect(enrollments.list().every((row) => row.memberId === owner.memberId)).toBe(true);
    expect(tickets.pendingFoundingVaults()).toStrictEqual([]);
    expect(parseRecoveryKit(initialized.kit, 'correct horse battery staple').targets).toStrictEqual(
      [],
    );
  ...
  test('founding routes fail closed at every ceremony boundary and accept safe defaults', async () => {
    ...
    await expect(emptyRestore.json()).resolves.toMatchObject({
    ...
    expect(initializedBody.enrollment.label).toMatch(/^founder founder-de/u);
