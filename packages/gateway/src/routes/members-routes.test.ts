...
import { makeMembersRouteHandler, type MembersRouteDeps } from './members-routes.js';
...
describe('members-routes suite', () => {
  ...
  async function harness(): Promise<{
    ...
    onEndpointRevoked: ReturnType<typeof vi.fn<NonNullable<MembersRouteDeps['onEndpointRevoked']>>>;
  }> {
    ...
    const onEndpointRevoked = vi.fn<NonNullable<MembersRouteDeps['onEndpointRevoked']>>();
  ...
  test('the roster is people-first: members carry their roles and device counts', async () => {
    ...
    expect(body.members.map((member) => member.memberId)).toStrictEqual([ownerId, sidId]);
  ...
  test('renaming keeps the id, so every binding and grant follows the person', async () => {
    ...
    expect([...new Set(rows.map((row) => row.endpointId))].sort()).toStrictEqual([
      'sid-phone',
      'sid-tablet',
    ]);
  ...
  test('removing the last admin member of a vault demands the vault name', async () => {
    ...
    expect(f.enrollments.members.list().map((member) => member.memberId)).toStrictEqual([sidId]);
  });
});
