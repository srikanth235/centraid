...
const VAULT_NAMES: Record<string, string> = { 'vault-a': 'Personal', 'vault-b': 'Family' };
...
async function harness(): Promise<{
  ...
  const vaultName = (vaultId: string): string | undefined => VAULT_NAMES[vaultId];
  const membersHandler = makeMembersRouteHandler({ enrollments, vaultName, onEndpointRevoked });
...
function seed(enrollments: EnrollmentStore): { ownerId: string; sidId: string } {
  ...
  enrollments.enroll({ endpointId: 'sid-tablet', label: 'Sid tablet', memberId: sid.memberId });
...
test('the roster is people-first: members carry their roles and device counts', async () => {
  ...
  const response = await fetch(f.members, { headers: deviceHeaders('owner-key') });
  expect(response.status).toBe(200);
  const body = (await response.json()) as {
    members: Array<{ memberId: string; label: string; roles: unknown[]; deviceCount: number }>;
  };
  expect(body.members.map((member) => member.memberId)).toEqual([ownerId, sidId]);
...
test('renaming keeps the id, so every binding and grant follows the person', async () => {
  ...
  expect(await renamed.json()).toMatchObject({ member: { memberId: sidId, label: 'Siddharth' } });

  // Same id, same bindings, same authority — a rename is display only.
  const rows = f.enrollments.list().filter((row) => row.memberId === sidId);
  expect([...new Set(rows.map((row) => row.endpointId))].sort()).toEqual([
    'sid-phone',
    'sid-tablet',
  ]);
  expect(rows.every((row) => row.memberLabel === 'Siddharth')).toBe(true);
  expect(f.enrollments.members.grants(sidId)).toEqual([{ vaultId: 'vault-b', role: 'write' }]);
  expect(f.enrollments.vaultsFor('sid-tablet')).toEqual(['vault-b']);
});

test('revoke device leaves the person; remove member kills every binding at once', async () => {
  const f = await harness();
  const { sidId } = seed(f.enrollments);
  const phone = f.enrollments.list().find((row) => row.endpointId === 'sid-phone')!;
  ...
  const revoked = await fetch(`${f.devices}/${encodeURIComponent(phone.enrollmentId)}`, {
    method: 'DELETE',
    headers: deviceHeaders('owner-key'),
  });
  ...
  expect(f.enrollments.list().every((row) => row.memberId !== sidId)).toBe(true);
...
test('removing the last admin member of a vault demands the vault name', async () => {
  ...
  const missing = await fetch(url, { method: 'DELETE', headers: deviceHeaders('owner-key') });
  ...
  expect(f.enrollments.members.list().map((member) => member.memberId)).toEqual([sidId]);
});
