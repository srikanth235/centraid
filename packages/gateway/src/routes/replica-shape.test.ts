...
describe('replica-shape suite', () => {
  ...
  test('unmasked shapes hide protocol credential names and values', async () => {
    ...
    expect(JSON.stringify(wire)).not.toMatch(/signing_key|host_key|public_key/u);
  ...
  test("keeps one app's purpose grants in independent row and column shapes", async () => {
    ...
    expect(service.entityMap.get('schedule.task')?.columns).toStrictEqual(['task_id', 'title']);
    expect(billing.entityMap.get('schedule.task')?.columns).toStrictEqual(['task_id', 'priority']);
    ...
    expect(shapeReplicaRow(service, 'schedule.task', serviceTask)?.values).toStrictEqual({
      task_id: 'service-task',
      title: 'Plan',
    });
    expect(shapeReplicaRow(service, 'schedule.task', billingTask)).toBeUndefined();
    expect(shapeReplicaRow(billing, 'schedule.task', billingTask)?.values).toStrictEqual({
  ...
  test('uses the exact first grant/scope selected by canonical online consent', async () => {
    ...
    expect(onlineRows).toStrictEqual([{ task_id: 'both-grants', title: 'Canonical first' }]);
    ...
    expect(shape.entityMap.get('schedule.task')?.columns).toStrictEqual(['task_id', 'title']);
    const row = readReplicaRow(vault.db.vault, 'schedule.task', 'both-grants')!;
    expect(shapeReplicaRow(shape, 'schedule.task', row)?.values).toStrictEqual(onlineRows[0]);
  });
  ...
  test('expires within-days membership without requiring a database write', async () => {
    ...
    expect(currentReplicaLogState(vault.db.vault).watermark).toStrictEqual(watermark);
