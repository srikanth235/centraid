...
describe('agents-routes', () => {
  test('reports one entry per registered runner kind', async () => {
    ...
    expect(s.agents.map((a) => a.kind).sort()).toStrictEqual([...RUNNER_KINDS].sort());
  ...
  test('attaches each agent’s models + status from the resolver', async () => {
    ...
    expect(calls.map(([k]) => k).sort()).toStrictEqual([...RUNNER_KINDS].sort());
    const codex = s.agents.find((a) => a.kind === 'codex');
    expect(codex?.models).toStrictEqual([{ id: 'codex-x', name: 'X', default: true }]);
