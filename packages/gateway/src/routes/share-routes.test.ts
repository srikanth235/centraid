...
import { bootstrapVault, blobUriFor, openVaultDb, type VaultDb } from '@centraid/vault';
...
function seedItem(db: VaultDb, ownerPartyId: string, deviceId: string, label: string): string {
...
async function harness(): Promise<Harness> {
  ...
  const priyaBoot = bootstrapVault(priyaVault, { ownerName: 'Priya', vaultId: 'vault-priya' });
  bootstrapVault(familyVault, { ownerName: 'Family', vaultId: 'vault-family' });
  const contentId = seedItem(priyaVault, priyaBoot.ownerPartyId, priyaBoot.deviceId, 'a');
  ...
  const handler = makeShareRouteHandler({
    enrollments,
    vaultFor: (vaultId) =>
      vaultId === 'vault-priya' ? priyaVault : vaultId === 'vault-family' ? familyVault : undefined,
...
function headers(endpointId?: string, hostCustody = false): Record<string, string> {
  return {
...
function countIn(db: VaultDb, table: string): number {
  return (db.vault.prepare(`SELECT count(*) AS n FROM ${table}`).get() as { n: number }).n;
}
...
test('a write-in-audience member places the item, and the origin is untouched', async () => {
  ...
  const result = (await response.json()) as { itemId: string; deduped: boolean };
  expect(result.deduped).toBe(false);

  // The audience can READ it: row and bytes both landed.
  const placed = f.familyVault.vault
    .prepare('SELECT sha256, title FROM core_content_item WHERE content_id = ?')
    .get(result.itemId) as { sha256: string; title: string };
  expect(placed.title).toBe('Item a');
  expect(f.familyVault.blobs.getSync(placed.sha256)).toEqual(f.bytes);
  // Provenance names the acting MEMBER, not the device.
  expect(
    f.familyVault.vault
      .prepare('SELECT origin_vault_id, shared_by_member FROM core_share_origin')
      .get(),
  ).toEqual({ origin_vault_id: 'vault-priya', shared_by_member: f.priya });
...
test('re-sharing the same item dedupes onto the placed row', async () => {
  ...
  const first = (await (await post()).json()) as { itemId: string; deduped: boolean };
  const again = (await (await post()).json()) as { itemId: string; deduped: boolean };
...
test('unshare removes the projection; the origin row and bytes stay readable', async () => {
  ...
  expect(countIn(f.priyaVault, 'core_content_item')).toBe(1);
  expect(f.priyaVault.blobs.getSync(f.priyaVault.blobs.local.listSync()[0]!)).toEqual(f.bytes);
});
...
test('an unshareable item type is refused by name, with nothing placed', async () => {
  ...
  expect(await response.json()).toMatchObject({ error: 'invalid_item_type' });
  expect(f.familyVault.blobs.local.listSync()).toEqual([]);
});
...
test('an item that is not in the origin vault refuses before anything is placed', async () => {
  ...
  expect(await response.json()).toMatchObject({ error: 'share_placement_failed' });
  expect(f.familyVault.blobs.local.listSync()).toEqual([]);
});
