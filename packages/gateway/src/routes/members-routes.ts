...
export function makeMembersRouteHandler(deps: MembersRouteDeps): RouteHandler {
  return async (
    ...
    if (
      url.pathname !== MEMBERS_PATH &&
      !url.pathname.startsWith(`${MEMBERS_PATH}/`)
    ) {
      return false;
    }
    ...
    const caller = callerKey
      ? deps.enrollments.memberFor(callerKey)
      : undefined;
    ...
    const visible = (member: Member): boolean =>
      ...
      members
        ...
        .some((grant) => callerVaults.has(grant.vaultId));
    ...
    const mayAdminister = (member: Member): boolean => {
      ...
      return grants.every(
        (grant) => members.roleIn(caller.memberId, grant.vaultId) === "admin"
      );
    };
    ...
    if (url.pathname === MEMBERS_PATH) {
      ...
      if (
        !hostCustody &&
        !members.grants(caller?.memberId ?? "").some((g) => g.role === "admin")
      ) {
        return sendJson(res, 403, {
          error: "not_admin",
    ...
    if (!mayAdminister(member)) {
      return sendJson(res, 403, {
        error: "not_admin",
        message:
          "removing a person requires admin in every vault they hold a role in",
      });
    }
