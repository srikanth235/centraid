...
test('PUT storage/limits round-trips both limits and clears with null', async () => {
  ...
  expect(((await read.json()) as { limits: { journalLimitBytes: number } }).limits).toMatchObject({
    journalLimitBytes: 1024 ** 3,
  });
  ...
  expect(((await cleared.json()) as { limits: Record<string, unknown> }).limits).toMatchObject({
