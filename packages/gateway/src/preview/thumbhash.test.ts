...
test('rgbaToThumbHash rejects rasters larger than 100×100', () => {
  expect(() => rgbaToThumbHash(101, 10, solid(101, 10, 0, 0, 0))).toThrow(/doesn't fit in 100x100/);
  expect(() => rgbaToThumbHash(10, 101, solid(10, 101, 0, 0, 0))).toThrow(/doesn't fit in 100x100/);
});
...
test('rgbaToThumbHash encodes a solid color to a stable compact hash', () => {
  ...
  expect(Buffer.from(rgbaToThumbHash(8, 8, solid(8, 8, 255, 0, 0)))).toEqual(Buffer.from(hash));
});
