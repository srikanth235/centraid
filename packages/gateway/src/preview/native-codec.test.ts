...
describe('native-codec', () => {
  ...
  test('createNativeImagePreviewCodec returns null for non-image bytes', async () => {
    const codec = createNativeImagePreviewCodec();
    await expect(codec.downscale(Buffer.from('not an image'), 'image/png', 64)).resolves.toBeNull();
    await expect(codec.thumbhash(Buffer.from('nope'), 'image/png')).resolves.toBeNull();
  });
});
