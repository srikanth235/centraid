...
function renderDesignTokensBlock(): string {
  ...
  return [
    ...
    "- Type: `font: var(--t-title|--t-body|--t-body-strong|--t-small|--t-tiny|--t-mono)` shorthands; mono (`var(--mono)`) for counts, dates, metadata. **Never write `font-size:`** — and note a `font` shorthand also RESETS `font-family`, so follow it with `font-family: inherit;` wherever the app-font knob must keep flowing down.",
...
function renderComponentPrimitivesBlock(): string {
  return [
    ...
    "```",
    "",
    '**Card / surface** — wrap in `<div class="surface">` for an elevated panel; the scaffold styles it with `background: var(--bg-elev); border: 1px solid var(--line); border-radius: var(--r-card); padding: var(--sp-4);`.',
    "",
