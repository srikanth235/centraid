...
function renderDesignTokensBlock(): string {
  ...
  return [
    ...
    "**Rules:**",
    "",
    "- App identity: set `--app-hue` (your app's slot on the identity wheel — an oklch hue resolved at build time; it parameterises nothing else) and `--app-identity` (one of the palette vars `--c-amber`/`--c-forest`/`--c-indigo`/`--c-ochre`/`--c-rose`/`--c-slate`/`--c-teal`/`--c-violet`) in your app.css `:root`. The shell spends no colour: `--accent` IS the ink (`--text`) in both themes, so paint actions and the one primary fill with it, and reserve `--app-identity` for the icon chip and a content marker — never a control.",
    ...
    "- Radii: `var(--r-sm)`, `var(--r-md)`, `var(--r-lg)`, `var(--r-pill)` — do not pick px values by feel.",
    "- Spacing: `var(--sp-1)`…`var(--sp-6)` = 4 · 8 · 12 · 16 · 24 · 32px. **Every** padding, margin, and gap comes off this scale — never pick a px or rem gutter by feel. A gutter that is not on a rung is a bug, not a nuance.",
    "- Type: `font: var(--t-title|--t-body|--t-body-strong|--t-small|--t-control|--t-mono)` shorthands. `--t-mono` is the NUMERIC role — counts, dates, sizes, metadata — and it is Instrument Sans with tabular figures, not a monospace face; take it with its modifiers (`font-variant-numeric: var(--t-mono-numeric)`). `var(--font-code)` is the platform CODE stack and belongs only on code, an inline literal, or a file path — never on a number. **Never write `font-size:`**; the role owns family, weight, size and leading together.",
  ...
  const entries = inAppIcons
    ...
    .map((name) => {
      const paths = icons[name]
        .map(
          (p) => `    <path d="${p.d}"${p.fill ? ` fill="${p.fill}"` : ""} />`
        )
        .join("\n");
...
function renderComponentPrimitivesBlock(): string {
  return [
    ...
    "empty states, the status line, skeletons. **Never hand-roll a second button or input**",
    ...
    "| State reported in place (never a toast) | `.kit-status-line` |",
    "",
    "**Page shell**",
    "",
    "```html",
    ...
    "```",
    "",
    '**Card / surface** — wrap in `<div class="surface">` for an elevated panel; the scaffold styles it with `background: var(--bg-elev); border: 1px solid var(--line); border-radius: var(--r-lg); padding: var(--sp-4);`.',
    "",
...
function renderUxRulesBlock(): string {
  const lines = [
    ...
    "",
    "- Keep the blueprint kit when the app ships one (`kit.js` / `kit.css` — template clones do): its status line, confirm-to-act, states, and charts are the shared UX substrate. Never delete, fork, or stop importing these files.",
