...
function renderDesignTokensBlock(): string {
  ...
  return [
    ...
    "- Type: `font: var(--t-title|--t-body|--t-body-strong|--t-small|--t-control|--t-mono)` shorthands. `--t-mono` is the NUMERIC role — counts, dates, sizes, metadata — and it is the sans with tabular figures, not a monospace face; take it with its modifiers (`font-variant-numeric: var(--t-mono-numeric)`). `var(--font-mono)` is the platform CODE stack and belongs only on code, a path, or a key shown verbatim — never on a number. **Never write `font-size:`** — and note a `font` shorthand also RESETS `font-family`, so follow it with `font-family: inherit;` wherever the app-font knob must keep flowing down.",
