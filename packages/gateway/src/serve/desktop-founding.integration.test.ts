...
async function mount(gateway: BuiltGateway): Promise<{ url: string; close: () => Promise<void> }> {
  ...
  if (!address || typeof address === 'string') throw new Error('desktop founding server not bound');
  return {
...
test('desktop founds through the shared gate and re-adopts its protected owner on restart', async () => {
  ...
  const minted = (await (
    await fetch(`${server.url}/centraid/_gateway/founding/ticket`, { method: 'POST' })
  ...
  expect({ status: initializedResponse.status, error: initialized.error }).toEqual({
    status: 201,
    error: undefined,
  });
