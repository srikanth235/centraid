...
describe('gateway-log-store', () => {
  ...
  test('append assigns monotonic seqs and snapshot honors after', () => {
    ...
    expect(all.map((e) => e.message)).toStrictEqual(['one', 'two', 'three']);
    expect(all.map((e) => e.seq)).toStrictEqual([1, 2, 3]);
    expect(all.map((e) => e.level)).toStrictEqual(['info', 'warn', 'error']);

    expect(store.snapshot(2).map((e) => e.message)).toStrictEqual(['three']);
    expect(store.snapshot(3)).toStrictEqual([]);
  });
  ...
  test('ring buffer evicts oldest past capacity but seqs keep counting', () => {
    ...
    const all = store.snapshot();
    expect(all.map((e) => e.message)).toStrictEqual(['line 3', 'line 4', 'line 5']);
  ...
  test('subscribers get live entries; unsubscribe is idempotent', () => {
    ...
    expect(seen.map((e) => e.message)).toStrictEqual(['live']);
  ...
  test('wrap tees into the store and forwards to the inner logger', () => {
    ...
    expect(forwarded).toStrictEqual(['info:a', 'warn:b', 'error:c']);
    expect(store.snapshot().map((e) => `${e.level}:${e.message}`)).toStrictEqual([
      'info:a',
      'warn:b',
      'error:c',
    ]);
  ...
  test('boot-tail load: a new store on an existing dir sees prior entries', () => {
    ...
    expect(tail.map((e) => e.message)).toStrictEqual([
      'before restart 1',
      'before restart 2',
      'before restart 3',
    ]);
  ...
  test('boot-tail load respects ring capacity — only the newest lines are kept', () => {
    ...
    expect(second.snapshot().map((e) => e.message)).toStrictEqual(['line 3', 'line 4', 'line 5']);
  });
  ...
  test('dropped-writes counter increments on an unwritable dir, never throws', () => {
    ...
    expect(store.snapshot().map((e) => e.message)).toStrictEqual(['should not throw', 'another']);
  });
  ...
  test('disk-full: fails open to the in-memory ring, never throws, and stops hammering the disk', () => {
    ...
    try {
      ...
      expect(store.snapshot().map((e) => e.message)).toStrictEqual(['one', 'two']);
    } finally {
