...
describe('device-plane suite', () => {
  ...
  test("enrollment: a device inherits its member's grants; revoke kills the binding", async () => {
    ...
    expect([...new Set(orphaned.map((row) => row.vaultId))].sort()).toStrictEqual(['v1', 'v2']);
    expect([...new Set(orphaned.map((row) => row.endpointId))].sort()).toStrictEqual([
      'ep-laptop',
      'ep-tablet',
    ]);
    expect(store.isEnrolled('ep-tablet')).toBe(false);
    expect(store.listByVault('v2').map((row) => row.endpointId)).toStrictEqual(['ep-phone']);
  });
  ...
  test('enrollment: remember, role, and Companion grants persist across re-pair', async () => {
    ...
    expect(EnrollmentStore.open(file).get('ep-session', 'v1')?.grantProfile).toStrictEqual([
      'tasks',
    ]);
    expect(
      store.enroll({
  ...
  test('ticket redemption and enrollment commit atomically and one row wins concurrency', async () => {
    ...
    expect(enrolled?.map((row) => ({ vaultId: row.vaultId, role: row.role }))).toStrictEqual([
      { vaultId: 'v1', role: 'write' },
      { vaultId: 'v2', role: 'read' },
    ]);
