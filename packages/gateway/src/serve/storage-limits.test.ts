...
describe('evaluateStorageLimit', () => {
  ...
  it('classifies ok / degraded / error across the warn threshold and the limit', () => {
    ...
    expect(evaluateStorageLimit(4000, limits)).toMatchObject({ status: 'error', fractionUsed: 4 });
  });
});
...
describe('StorageLimitsStore', () => {
  it('round-trips through an atomic write and leaves no temp file behind', async () => {
    ...
    expect(await store.load()).toMatchObject(OFF);

    await store.update({ totalLimitBytes: 10 * 1024 ** 3, journalLimitBytes: 1024 ** 3 });
    expect(await loadStorageLimits(dir)).toMatchObject({
