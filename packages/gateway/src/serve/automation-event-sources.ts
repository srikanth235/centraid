...
import type { ConnectionBinding, EventTrigger } from '@centraid/automation';
...
function githubCursor(value: unknown): GitHubCursor | undefined {
  ...
  return {
    ...
    ...(string(row.lastModified) ? { lastModified: string(row.lastModified) } : {}),
...
function githubPayload(
  ...
  return {
    ...
    payload: {
      ...
      ...(createdAt ? { createdAt } : {}),
      ...(string(subject.updated_at) ? { updatedAt: string(subject.updated_at) } : {}),
    },
  };
}
...
async function githubPoll(input: PollProviderEventSourceInput): Promise<ProviderPollResult> {
  ...
  const response = await input.pollJson(input.connection, url, conditionalHeaders);
  ...
  if (response.status === 304) return { events: [], cursor: { ...cursor, ...next } };
  if (response.status !== 200) throw new Error(`GitHub events poll failed (${response.status})`);
...
export function pollProviderEventSource(
  ...
  throw new Error(`no event cursor adapter for "${input.trigger.connectorKind}"`);
}
