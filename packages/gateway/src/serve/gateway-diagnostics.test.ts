...
describe("gateway-diagnostics", () => {
  ...
  test("assembles version, runtime, health, logs, and vault sizes", async () => {
    ...
    expect(bundle.logs.map((e) => e.message)).toStrictEqual([
      "boot",
      "something noisy",
    ]);
    expect(bundle.vaults).toStrictEqual([
      {
        vaultId: "v1",
        name: "Personal",
        files: {
    ...
    expect(bundle.config).toStrictEqual({ foo: "bar" });
  ...
  test("reads statSync sizes without touching anything beyond the four DB files", async () => {
    ...
    fs.writeFileSync(
      path.join(vaultDir, "blobs", "aa", "huge-blob"),
      Buffer.alloc(100)
    );
    ...
    expect(bundle.vaults[0]!.files).toStrictEqual({
      vaultDbBytes: 10,
      vaultDbWalBytes: 20,
      journalDbBytes: 30,
  ...
  test("redaction: secret-shaped keys never appear in the serialized bundle, however deep", async () => {
    ...
    expect(
      (bundle.config as { nested: { deeper: { authToken: string } } }).nested
        .deeper.authToken
    ).toBe("[REDACTED]");
  });
  ...
  test("a mounted vault with live handles gets a dbstat table breakdown and an inline-body violation scan", async () => {
    ...
    const bundle = await buildDiagnosticsBundle({
      ...
      logs: new GatewayLogStore(),
      vaults: fakeVaultRegistry([
        fakePlaneWithDb("v1", "Personal", vaultDir, db),
      ]),
    });
