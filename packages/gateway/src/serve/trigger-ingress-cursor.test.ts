...
type Row = Pick<TriggerIngressRecord, 'id' | 'receivedAt' | 'payloadJson' | 'payloadRef'>;
...
type IngressStore = Pick<AutomationTriggerStore, 'listIngressAfter' | 'pruneIngress'>;

/** Minimal `listIngressAfter` / `pruneIngress` over an in-memory ingress table. */
function storeOf(rows: readonly Row[], prune?: PruneIngressResult): IngressStore {
  return {
...
test('the cursor advances only to the last row actually returned', () => {
  const result = readIngressCursor(storeOf(rows(60)), 'hook-1', undefined, 50, NOW);
...
test('the surplus is delivered on the next read instead of being dropped', () => {
  ...
  const second = readIngressCursor(store, 'hook-1', first.positionJson, 50, NOW);
  expect(second.elements.map((element) => element.position)).toEqual(
    Array.from({ length: 10 }, (_, index) => String(51 + index)),
  );
  expect(second.positionJson).toBe('60');
  const third = readIngressCursor(store, 'hook-1', second.positionJson, 50, NOW);
  expect(third.elements).toEqual([]);
...
test('a corrupt or absent position restarts from the beginning of the source', () => {
  for (const position of [undefined, 'not-json', '-3', '{"ingressId":4}']) {
    const result = readIngressCursor(storeOf(rows(2)), 'hook-1', position, 50, NOW);
    expect(result.elements.map((element) => element.position)).toEqual(['1', '2']);
  }
});
...
test('payloads decode as JSON, fall back to raw text, and expose a blob ref', () => {
  ...
  expect(result.elements.map((element) => element.payload)).toEqual([
    { a: 1 },
    'not json',
    { payloadRef: 'blob-9' },
  ]);
...
test('retention that drops undelivered rows is an accounted gap, not a silent loss', () => {
  ...
  const result = readIngressCursor(storeOf(rows(5, 31), prune), 'hook-1', '20', 50, NOW);
  expect(result.skipped).toBe(7);
  expect(result.gapReason).toBe('ingress_retention');
  // The surviving rows still ride through on the same read.
  expect(result.elements.map((element) => element.position)).toEqual([
    '31',
    '32',
    '33',
    '34',
...
test('retention that only drops already-delivered rows costs nothing', () => {
  ...
  const result = readIngressCursor(storeOf(rows(2, 21), prune), 'hook-1', '20', 50, NOW);
  expect(result.skipped).toBeUndefined();
  expect(result.gapReason).toBeUndefined();
});
...
test('another source key never charges its retention loss to this reader', () => {
  ...
  expect(ingressRetentionGap(prune, 'hook-2', 0)).toEqual({
    skipped: 9,
