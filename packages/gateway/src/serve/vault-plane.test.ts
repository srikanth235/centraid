...
describe('vault-plane', () => {
  ...
  test('WAL ownership stays unconditional while capture follows backup configuration', async () => {
    ...
    await expect(directoryBytes(path.join(dir, 'wal-ship'))).resolves.toBe(shipBytesBeforeStop);
  });
  ...
  test('one journal failure preserves its canonical marker while sibling writes commit', async () => {
    ...
    expect(
      plane.db.vault
        ...
        .all()
        .map((row) => ({ ...row })),
    ).toStrictEqual([{ invocation_id: 'queue-fail', journal_finalized_at: null }]);
    expect(
      plane.db.journal
        .prepare(
        ...
        .all()
        .map((row) => ({ ...row })),
    ).toStrictEqual([{ invocation_id: 'queue-ok-a' }, { invocation_id: 'queue-ok-b' }]);
  });
  ...
  test('full stack: a real handler file reaches the canon through ctx.vault', async () => {
    ...
    expect(events.map((row) => ({ ...row }))).toStrictEqual([{ summary: 'Cross-plane standup' }]);
  ...
  test('cross-referencing (issue #272): shell pick → owner link → app resolves the far end without a scope', async () => {
    ...
    expect((revoked.result as { cards: Array<{ status: string }> }).cards[0]!.status).toBe(
      'denied',
    );
