...
function deps(overrides: Partial<Parameters<typeof reviseAutomationInstructions>[0]> = {}): {
...
test('a successful revise publishes once and keeps the new instructions', async () => {
  ...
  expect(out.published).toEqual(['Revised: only customer messages.']);
...
test('a failed compile restores the previous instructions and says so in the thread', async () => {
  ...
  expect(out.published).toEqual(['Revised: only customer messages.', 'Summarize account changes.']);
  expect(out.rolledBack).toEqual([expect.stringContaining('handler.js did not validate')]);
...
test('a rewrite that throws AFTER publishing still rolls back', async () => {
  ...
  await reviseAutomationInstructions(input);
  expect(out.published).toEqual(['Revised: only customer messages.', 'Summarize account changes.']);
  expect(out.failed).toEqual(['ledger write failed']);
...
test('a rewrite that fails BEFORE publishing reports the failure and publishes nothing', async () => {
  ...
  expect(out.rolledBack).toEqual([]);
  expect(out.failed).toEqual(['No automation agent is configured.']);
});
