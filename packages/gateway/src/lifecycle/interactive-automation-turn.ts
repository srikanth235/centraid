  ...
  return withConversationLock(
    ...
    async () => {
      ...
      try {
        ...
        const preamble = automationContextPreamble(
          opts.row,
          recentTurns,
          opts.message
        );
        ...
        const closeTool = (
          ...
          event:
            | Extract<TurnStreamEvent, { type: "tool.result" }>
            | { ok: false; errorText: string }
        ): void => {
          const endedAt = Date.now();
          const result = "result" in event ? event.result : undefined;
          const rawJson = boundedRawJson(
            "rawJson" in event ? event.rawJson : undefined
          );
          const error = event.ok
            ? undefined
            : event.errorText || "Tool failed.";
          if (
            "locations" in event &&
        ...
        const onEvent = (incoming: TurnStreamEvent): void => {
          const event =
            incoming.type === "usage" ? pricedUsage(incoming) : incoming;
        ...
        const attachNextCandidate = async (index: number): Promise<void> => {
          ...
          if (artifacts.length > 0)
            artifactsByItem.set(candidate.itemId, artifacts);
