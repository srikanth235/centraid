...
describe('rewrite-automation-instructions', () => {
  ...
  describe('rewrite instruction helpers', () => {
    it('keeps the current prompt and steering request explicit', () => {
      expect(rewriteWorkOrder('Old prompt', 'Only urgent mail')).toContain('Old prompt');
      expect(rewriteWorkOrder('Old prompt', 'Only urgent mail')).toContain('Only urgent mail');
    });

    it('strips wrappers without changing the rewritten content', () => {
      expect(cleanRewrittenInstructions('```markdown\nOnly urgent mail.\n```')).toBe(
        'Only urgent mail.',
      );
      expect(cleanRewrittenInstructions('Revised instructions: Only urgent mail.')).toBe(
        'Only urgent mail.',
      );
    });
  });
  ...
  describe(rewriteAutomationInstructions, () => {
    it('uses a tool-less denied-permission turn, persists, and records a thread card', async () => {
      ...
      const persistPrompt = vi.fn<(prompt: string) => Promise<void>>().mockResolvedValue(undefined);
