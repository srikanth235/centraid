...
function verifyTicket(secret: string, encoded: string): BlobTicket | undefined {
  ...
  try {
    ...
    return ticket.expiresAtMs >= Date.now() && ticket.nonce
      ? ticket
      : undefined;
  } catch {
    return undefined;
...
async function serveBlob(
  ...
  if (!ticket || nonces.has(ticket.nonce))
  ...
  const range = rawRange
    ? parseRange(rawRange, size)
    : ([0, size - 1] as [number, number]);
  if (!range) {
...
async function pump(
  ...
  return await new Promise((resolve, reject) => {
    ...
    if (length === 0) request.end();
      ...
      createReadStream(file, { start: offset, end: offset + length - 1 }).pipe(
        request
      );
  });
}
  ...
  const server = http.createServer((req, res) => {
    void (async () => {
      ...
      if (url.pathname === "/v1/preview" && req.method === "POST") {
        ...
        const output = await previewCodec.downscale(
          await collect(req),
          "image/png",
          edge
        );
        ...
        res
          .writeHead(200, { "content-type": output.mediaType })
          .end(output.bytes);
        return;
      }
      if (url.pathname === "/v1/pump" && req.method === "POST") {
        const result = await pump(
          root,
          JSON.parse((await collect(req)).toString()) as PumpInput
        );
        sendJson(
          res,
          result.providerStatus >= 200 && result.providerStatus < 300
            ? 200
            : 502,
          result
        );
