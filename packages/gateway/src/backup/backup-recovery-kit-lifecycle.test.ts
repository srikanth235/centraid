...
describe('backup-recovery-kit-lifecycle', () => {
  ...
  test('target and epoch changes stale the kit once; local-only vaults and unchanged exports do not', async () => {
    ...
    expect(oneTarget.targets.map((target) => target.vaultId)).toStrictEqual([h.vaultId]);
    await expect(h.service.recoveryKitStatus()).resolves.toStrictEqual(firstTargetStale);

    await verifyExportedKit(h);
    const confirmedOneTarget = await h.service.recoveryKitStatus();
    await h.service.recoveryKitDocument();
    await expect(h.service.recoveryKitStatus()).resolves.toStrictEqual(confirmedOneTarget);

    const secondVault = h.registry.list().find((vault) => vault.name === 'Local only')!;
