...
- **No tool allowlist.** Deterministic work goes through the built-in `ctx.vault` / `ctx.fetch` / `ctx.state` rails, which need no declaration — there is no `requires.tools` field (it was removed with the `ctx.tool` rail). Leave `requires` to just `model` unless you need an MCP for `ctx.delegate`.
- `requires.model` is the capability tier `ctx.delegate` routes through (`provider/model-id`). Pick the **cheapest tier that does the inference** — a small/cheap tier for summarize/classify/extract; reserve a stronger tier for genuinely hard reasoning. `ctx.delegate` is the only billed path (see _Two cost rails_ below), so the tier you declare is the per-fire cost.
...
- `ctx.delegate({ prompt, json?, harness?, model?, configPins? })` — one constrained, billed model turn against the user's real provider. Always pass a `json` schema when the result is consumed structurally — it both parses the result and detects a model failure. `harness`/`model`/`configPins` are OPTIONAL per-call overrides (#743 Part 2 item c, absorbing #740): omit them and the call rides the automation's fixed harness/model (`requires.harness` / `requires.model`, or the automations subsystem default). Name them only when the user's instructions actually named a harness/model for that specific step ("use opencode deepseek-ocr for the OCR step") — never invent one. A step naming a harness that fails surfaces that failure; it never silently retries a different harness. Repeating the same `harness` across calls in the same automation resumes that harness's own conversation history automatically — there is no session handle to pass.
