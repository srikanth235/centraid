 ...
 * path-registered ones), the `centraid` CLI preamble spliced into the
 * system prompt so the agent can read/write `./data.sqlite`, and the
 * per-window `ChatStore` lookup for the previous adapter session id
 * (round-tripped to resume the conversation).
 *
