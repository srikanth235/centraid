...
describe(acpBlockFor, () => {
  it("maps an image MIME to an ACP image block (flat data/mimeType)", () => {
    expect(
      acpBlockFor({ mime: "image/png", dataBase64: "AAAA" }, FULL)
    ).toStrictEqual({
      type: "image",
      data: "AAAA",
      mimeType: "image/png",
  ...
  it("renders a text/plain attachment as a delimited text block, capability-free", () => {
    ...
    expect(
      acpBlockFor(
        { mime: "text/plain", dataBase64, filename: "notes.txt" },
        TEXT_ONLY
      )
    ).toStrictEqual({
      type: "text",
  ...
  it("renders application/json as text", () => {
    ...
    expect(
      acpBlockFor(
        { mime: "application/json", dataBase64, filename: "data.json" },
        FULL
      )
    ).toStrictEqual({
      type: "text",
  ...
  it("falls back to filename extension when the MIME is generic", () => {
    ...
    expect(
      acpBlockFor(
        { mime: "application/octet-stream", dataBase64, filename: "script.js" },
        FULL
      )
    ).toStrictEqual({
      type: "text",
  ...
  it("truncates textual attachments over the size cap with an explicit marker", () => {
    ...
    const block = acpBlockFor(
      { mime: "text/plain", dataBase64, filename: "huge.txt" },
      FULL
    );
