...
describe('catalog', () => {
  test('readRunnerModels returns [] when the catalog file is absent', async () => {
    ...
    await expect(readRunnerModels(catalogPath, 'claude-code')).resolves.toStrictEqual([]);
  });
  ...
  test('readRunnerModels returns the cached list when present', async () => {
    ...
    expect((await readRunnerModels(catalogPath, 'claude-code')).map((m) => m.id)).toStrictEqual([
      'claude-opus-4-8',
      'claude-haiku-4-5',
    ]);
  });
