...
export function buildReplayDispatchers(
  ...
  /** Take the next unconsumed node of `kind` preferring an exact-args match. */
  const take = (
    kind: AutomationRunNodeRow['kind'],
    name: string,
    argsJson: string | undefined,
  ): AutomationRunNodeRow | undefined => {
    const candidates = nodes.filter((n) => n.kind === kind && !consumed.has(n.nodeId));
    const exact =
      argsJson !== undefined
        ? candidates.find((n) => n.name === name && n.argsJson === argsJson)
        : undefined;
    const byName = exact ?? candidates.find((n) => n.name === name);
    const picked = byName ?? candidates[0];
  ...
  const invokeDispatcher: AutomationInvokeDispatcher = (name) => {
    const node = take('invoke', name, undefined);
    if (!node) {
      return Promise.reject(
        new Error(
