...
test('codexProviderOverrideArgs emits -c overrides for provider + endpoint', () => {
  ...
  expect(overrides.includes('model_provider="centraid-mock"')).toBeTruthy();
  expect(
    overrides.includes('model_providers.centraid-mock.name="Centraid Automation Mock"'),
  ).toBeTruthy();
  expect(
  ...
  expect(overrides.includes('model_providers.centraid-mock.wire_api="responses"')).toBeTruthy();
  expect(
    overrides.includes('model_providers.centraid-mock.env_key="CENTRAID_MOCK_KEY"'),
  ).toBeTruthy();
});
