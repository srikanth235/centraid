...
describe('stream-events suite', () => {
  ...
  test('agent_message_chunk emits assistant.start once then deltas, accumulating finalText', () => {
    ...
    expect(types()).toStrictEqual(['assistant.start', 'assistant.delta', 'assistant.delta']);
  ...
  test('tool_call falls back to kind, then "tool", for its title', () => {
    ...
    expect(starts.map((e) => e.type === 'tool.start' && e.toolName)).toStrictEqual([
      'edit',
      'tool',
    ]);
  });
  ...
  test('a tool_call that arrives already completed emits both start and result', () => {
    ...
    expect(result && result.type === 'tool.result' && result.result).toStrictEqual({ hits: 2 });
    expect(
  ...
  test('an agent’s own rawOutput.content survives the renderable-content merge', () => {
    ...
    expect(merged.content).toStrictEqual([{ type: 'text', text: 'found 1 match' }]);
    // …and the agent's own payload is still there, not overwritten.
    expect(merged.rawOutputContent).toStrictEqual([{ path: 'a.txt', score: 0.9 }]);
  });
  ...
  test('plan emits a phase event, with detail only when entries are present', () => {
    ...
    expect(withEntries && withEntries.type === 'phase' && withEntries.detail).toStrictEqual([
      { content: 'step 1', status: 'pending' },
    ]);
    expect(withEntries && withEntries.type === 'phase' && withEntries.plan).toStrictEqual([
      { content: 'step 1', status: 'pending' },
    ]);
  ...
  test('tool result extracts diff content blocks', () => {
    ...
    const result = events.find((e) => e.type === 'tool.result');
    expect(result && result.type === 'tool.result' && result.diffs).toStrictEqual([
  ...
  test('tool result preserves renderable content and terminal output as CAS candidates', () => {
    ...
    expect(result?.locations).toStrictEqual([{ path: '/workspace/report.txt', line: 7 }]);
