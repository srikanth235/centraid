...
describe("session-config suite", () => {
  ...
  test("modeAvailable matches currentModeId and availableModes entries", () => {
    ...
    expect(
      modeAvailable({ currentModeId: "bypassPermissions" }, "bypassPermissions")
    ).toBe(true);
    expect(
      modeAvailable(
        {
    ...
    expect(
      modeAvailable({ availableModes: "not-array" as unknown as [] }, "x")
    ).toBe(false);
    expect(
      modeAvailable({ availableModes: [{ id: "default" }] }, "missing")
    ).toBe(false);
  });
  ...
  test("readConfigOptionUpdate reads the schema’s full-set notification", () => {
    ...
    expect(
      readConfigOptionUpdate({ update: { sessionUpdate: "other" } })
    ).toBeUndefined();
    expect(
      readConfigOptionUpdate({
        update: {
  ...
  test("readOfferedModels flattens groups and reports currentValue", () => {
    ...
    expect(
      readOfferedModels([{ category: "model", options: [{ value: "x" }] }])
        .models
  ...
  test("pinModel records a successful pin that echoes nothing, and warns on a contradicting echo", async () => {
    ...
    await expect(
      pinModel({
        ...
        emit: (event) =>
          notices.push(event as { code?: string; level?: string }),
        sessionId: "s1",
        configOptions: modelOptions,
        requested: "m-other",
      })
