/**
 * Native-blueprint agent parity (#630).
 *
...
describe("native blueprint agent parity", () => {
  test("ACP drives all eight blueprints through consented vault_invoke receipts", async () => {
    ...
    const vaultInvoke: VaultInvokeRunner = (call) => {
      ...
      const receiptId = `receipt-agent-parity-${expected[0]}`;
      receipts.push({
      ...
      return {
        ...
        invocationId: `invocation-agent-parity-${expected[0]}`,
        receiptId,
        ...(parked
