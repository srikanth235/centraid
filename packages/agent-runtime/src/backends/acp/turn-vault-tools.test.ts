...
vi.mock('./vault-mcp-server.ts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('./vault-mcp-server.ts')>();
  return {
    ...actual,
    startVaultMcpServer: vi.fn(actual.startVaultMcpServer),
  };
});
...
afterEach(async () => {
  ...
  vi.mocked(startVaultMcpServer).mockImplementation(
    (...args) =>
      (
        vi.importActual('./vault-mcp-server.ts') as Promise<typeof import('./vault-mcp-server.ts')>
      ).then((m) => m.startVaultMcpServer(...args)) as never,
  );
  // Reset to real implementation for subsequent tests.
  const real =
    await vi.importActual<typeof import('./vault-mcp-server.ts')>('./vault-mcp-server.ts');
...
test('start failure emits vault_tools_unavailable and returns empty servers', async () => {
  vi.mocked(startVaultMcpServer).mockRejectedValueOnce(new Error('bind failed'));
  const events: TurnStreamEvent[] = [];
  const out = await startTurnVaultTools({
    toolContext: toolContext() as never,
    httpMcp: true,
