  ...
  describe("backend.attachments", () => {
    ...
    test("an agent without the image capability gets a notice naming what was skipped", async () => {
      ...
      const notice = events.find(
        (e) => e.type === "notice" && e.code === "attachment_unsupported"
      );
      expect(notice && notice.type === "notice" && notice.message).toContain(
        "shot.png"
      );
    });
    ...
    test("a PDF rides an embedded resource when the agent takes embedded context", async () => {
      ...
      const { blocks, events } = await promptBlocksFor({
        ...
        attachments: [
          { path: pdf, mime: "application/pdf", filename: "spec.pdf" },
        ],
        promptMarker,
      });
  ...
  test("a switch re-attaches a prior image only when target B advertises image input", async () => {
    ...
    const capable = await promptBlocksFor({
      extraArgs: [
        "--mode=normal",
        "--prompt-caps=image",
        `--prompt-marker=${capableMarker}`,
      ],
      hydrationContext:
        "[Centraid session handoff]\nUser: inspect this\nAttachments: prior.png (image/png)",
      hydrationAttachments: [
        { path: png, mime: "image/png", filename: "prior.png" },
      ],
      forceHydration: true,
