...
import type { LiveDispatch, LiveDispatchOptions } from './run-automation-live-dispatch.js';

const { runFire } = vi.hoisted(() => ({
  runFire: vi.fn<typeof import('@centraid/automation').runFire>(),
}));
type AutomationFailover = NonNullable<
  Parameters<typeof import('./run-automation.ts').runAutomation>[0]['onFailover']
>;
...
const liveDispatch = vi.hoisted(() => ({
  start: vi.fn<(options: LiveDispatchOptions) => Promise<LiveDispatch>>(async () => ({
...
vi.mock(import('./run-automation-live-dispatch.js'), async (importOriginal) => ({
...
describe('run-automation suite', () => {
  beforeEach(() => {
    ...
    runFire.mockResolvedValue(fireResult({ ok: true, runId: 'r1', value: { summary: 'done' } }));
  });
  ...
  describe(runAutomation, () => {
    ...
    it('re-enters a failed automation on the next ladder rung as a new ledger turn', async () => {
      ...
      runFire
        .mockResolvedValueOnce(fireResult({ ok: false, runId: 'run-fire', error: failure }))
