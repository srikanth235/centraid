...
async function runLive(
  ...
  try {
    ...
    return {
      ...
      events,
      error: events.find(
        (event): event is Extract<TurnStreamEvent, { type: 'error' }> => event.type === 'error',
      ),
      final: events.findLast(
        (event): event is Extract<TurnStreamEvent, { type: 'final' }> => event.type === 'final',
      ),
    };
  } finally {
...
const runNextAdapter = async (index: number): Promise<void> => {
  ...
  try {
    ...
    const thought = caps.configOptions.find((option) => option.category === 'thought_level');
    ...
    const usage = attempt.events.findLast(
      (event): event is Extract<TurnStreamEvent, { type: 'usage' }> => event.type === 'usage',
    );
    const effortRoundTrip = !requestedEffort || usage?.effort === requestedEffort;
...
if (requested.length >= 2) {
  ...
  try {
    const a = await runLive(first, cwd, 'Reply with exactly SWITCH_A_TOKEN_567. Do not use tools.');
    const b = await runLive(
      ...
      {
        hydrationContext:
          'Centraid handoff from another agent. Prior user requested the exact marker ' +
    ...
    const aReturn = await runLive(
      ...
      'Reply with exactly SWITCH_A_RETURN_567. Do not use tools.',
      {
        ...
        ...(a.result.usageSnapshot ? { prevUsageSnapshot: a.result.usageSnapshot } : {}),
        hydrationContext:
          'Centraid delta since this agent last ran. The other agent answered ' +
          'SWITCH_B_SEES_A_567.',
        recoveryHydrationContext:
    ...
    const ok =
      ...
      b.events.some((event) => event.type === 'notice' && event.code === 'session_hydrated') &&
      aReturn.events.some((event) => event.type === 'notice' && event.code === 'session_hydrated');
    process.stdout.write(
      `${JSON.stringify({
        check: 'switch',
        ...
        ...(a.error || b.error || aReturn.error
          ? {
              error:
                a.error?.message ?? b.error?.message ?? aReturn.error?.message ?? 'switch failed',
            }
          : {}),
      })}\n`,
    );
