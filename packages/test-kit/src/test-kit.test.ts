...
test('jsdomProject enables automatic JSX and jsdom setup', () => {
  ...
  expect(cfg.test?.setupFiles?.some((p) => p.includes('jsdom-setup'))).toBe(true);
});
...
test('recordQualityResult writes a stable artifact with rolling history', async () => {
  ...
  try {
    ...
    const file = path.join(scratch, 'artifacts', 'perf', 'gateway-low-end.json');
    ...
    expect(body.history.map((h) => h.value)).toEqual([12.5, 11]);
  } finally {
    process.chdir(cwd);
