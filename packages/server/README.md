...
Host-agnostic centraid gateway. `buildGateway()` wires [`@centraid/server/engine`](src/engine) + [`@centraid/server/acp`](src/acp) + stores + a conversation runner + an in-process cron scheduler against injected paths, and `serve()` starts an HTTP server in front of it.
