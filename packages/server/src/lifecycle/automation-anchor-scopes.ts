  ...
  for (const anchor of anchors) {
    ...
    if (!table || !idColumn || !fieldMask) {
      throw new AutomationAnchorError(
        `anchor ${anchor.anchorId} did not derive a narrow scope`
      );
    }
    ...
    if (group.idColumn !== idColumn) {
      throw new AutomationAnchorError(
        `anchors for ${key} disagree on their row key`
      );
    }
    group.ids.add(anchor.sourceId);
    const rowFields =
      group.fieldsById.get(anchor.sourceId) ?? new Set<string>();
...
function parseSelector(
  raw: string,
  anchorId: string
): AutomationAnchorSelector {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new AutomationAnchorError(
      `anchor ${anchorId} has an invalid selector`
    );
  }
...
function selectorMatches(
  ...
  if (text.slice(at, at + selector.exact.length) !== selector.exact)
    return false;
...
function sourceFieldFor(
  ...
  const idColumn =
    searchable?.idColumn ??
    (Object.hasOwn(CARD_PK, sourceType) ? CARD_PK[sourceType] : undefined);
  ...
  const row = readRows(
    vault,
    sourceType,
    [{ column: idColumn, op: "eq", value: sourceId }],
    1
  )[0];
  if (!row) {
    throw new AutomationAnchorError(
      `anchor source ${sourceType}/${sourceId} no longer exists`
    );
  }
...
export function resolveAutomationAnchors(
  ...
  const ids = [
    ...new Set(
      [...instructions.matchAll(ANCHOR_TOKEN_RE)].flatMap(
        (match) => match.groups?.anchorId ?? []
      )
    ),
  ];
  ...
  return ids.map((anchorId) => {
    ...
    if (!found)
      throw new AutomationAnchorError(
        `anchor ${anchorId} is missing or no longer live`
      );
    const row = {
    ...
    const { idColumn, field } = sourceFieldFor(
      vault,
      row.from_type,
      row.from_id,
      selector
    ...
    if (split.length !== 2 || !split[0] || !split[1]) {
      throw new AutomationAnchorError(
        `anchor ${anchorId} has an invalid source type`
      );
    }
    return {
