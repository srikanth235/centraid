...
describe("gateway-db-lock", () => {
  ...
  async function waitForReady(child: ChildProcess): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(
        () => reject(new Error(`lock holder timed out: ${output}`)),
        10_000
      );
      ...
      child.once("exit", (code, signal) => {
        ...
        reject(
          new Error(
            `lock holder exited before ready (${code ?? signal}): ${output}${errors}`
          )
        );
      });
    });
  ...
  test("SIGKILL releases gateway.db immediately and sqlite3 reads during restart recovery", async () => {
    ...
    expect(() => GatewayDatabase.open(dataDir, { lock: "exclusive" })).toThrow(
      GatewayLockError
    );
    ...
    const sqlite = spawnSync(
      ...
      [
        ...
        "SELECT value FROM gateway_meta WHERE key='schema';",
      ],
      { encoding: "utf8" }
    );
