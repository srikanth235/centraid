...
export class VaultPlane {
  ...
  private runSweep(): void {
    ...
    try {
      ...
      // THE OUTCOME LEDGER'S OWN WINDOW, ACTUALLY SWEPT (#1014, X1).
      // `pruneReplicaIntentOutcomes` shipped with no production caller at all
      // — the table's `expires_at` was a promise nothing kept — and X1 makes
      // that load-bearing: a revoked device's rows are now RETAINED for
      // dedupe, so the only thing bounding them is this. Held at the slowest
      // seat's cursor: an outcome a seat has not caught up to is a pending
      // badge that would never clear.
      const outcomeHold = lowestSeatCursor(this.db.vault);
      const outcomePrune = pruneReplicaIntentOutcomes(
        this.db.vault,
        outcomeHold === undefined ? {} : { holdAtOrAbove: outcomeHold }
      );
      if (outcomePrune.pruned > 0) {
        this.logger.info(
          `vault plane: replica intent outcomes pruned=${outcomePrune.pruned} ` +
            `heldBySeat=${outcomePrune.heldBySeat ?? "none"}`
        );
      }
