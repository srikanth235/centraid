...
function insertConnection(
  ...
  db.prepare(
    `INSERT INTO sync_connection (connection_id, label, status) VALUES (?, ?, ?)`
  ).run(row.id, row.label, row.status);
  db.prepare(
  ...
  if (row.authNote) {
    db.prepare(
      `INSERT INTO sync_connection_health (connection_id, auth_note) VALUES (?, ?)`
    ).run(row.id, row.authNote);
  }
}
