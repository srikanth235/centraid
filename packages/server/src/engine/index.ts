 ...
 *
 * Hosts: the standalone daemon in `@centraid/server` and the
...
// Harness-turn contract — the host-agnostic interface between a run spine
...
// `@centraid/server/acp`; hosts inject a `RunTurnFn` satisfying it.
export type {
  HarnessKind,
  HarnessPrefs,
  ToolContext,
...
// Per-model token pricing. Prefer harness/ACP USD when present (`resolveItemCost`);
