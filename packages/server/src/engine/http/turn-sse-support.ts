...
/** Realpath owner-selected extra roots; persisting them is the consent receipt against later symlink widening. */
...
/** Keep only refs naming a real CAS file; size receipts must match stored bytes so forged/stale refs never ride. */
 ...
 * tail. The lock map is per-runtime, not module-level: two gateways can share
 * an `appId` (#113).
 */
export async function withConversationLock<T>(
  ...
  // Chained tail (previous → next): newer callers await everything ahead of them; identity lets cleanup know nobody queued after.
