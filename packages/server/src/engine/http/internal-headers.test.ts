...
describe("Companion capability profile", () => {
  ...
  it("allows only the module’s pinned query and action bundle", () => {
    expect(
      companionHandlerAllowed(locker, "query", "locker", "autofill-item")
    ).toBe(true);
    expect(
      companionHandlerAllowed(locker, "action", "locker", "add-item")
    ).toBe(true);
    expect(
      companionHandlerAllowed(locker, "action", "locker", "trash-item")
    ).toBe(false);
  });
