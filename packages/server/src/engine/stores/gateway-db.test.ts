...
describe("openLedgerDb (the conversation-ledger band of vault.db)", () => {
  ...
  it("ISSUES NO DDL — the vault composed the band, and the ladder is its own", () => {
    ...
    // Eleven rungs: #929's three, the #928 ask tables (#972), #996's Locker
    // key plane and its unlock-credential drop, and #1014's replica floor
    // split, poison registers, one-log retirement and transient blob holds;
    // the number is the VAULT's, which is the point.
