...
describe("selector edges", () => {
  it("keeps a live automation thread’s newest turn and any unfinished turn", () => {
    ...
    const remaining = (
      journal
        .prepare(
          `SELECT id FROM turns WHERE conversation_id = 'a/x' ORDER BY seq`
        )
        .all() as {
        id: string;
      }[]
  ...
  it("re-running never double-archives an already-covered range (idempotent) and leaves automation_state", () => {
    ...
    expect(
      (
        journal
          .prepare(`SELECT COUNT(*) AS n FROM conversation_archive`)
          .get() as { n: number }
      ).n
    ).toBe(1);
    expect(
      (
        journal
          .prepare(
            `SELECT COUNT(*) AS n FROM automation_state WHERE automation_id = 'a/x'`
          )
          .get() as {
          n: number;
        }
