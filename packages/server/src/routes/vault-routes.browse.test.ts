...
describe("vault-routes.browse", () => {
  ...
  test("GET /browse/tables lists vault tables with pack classification", async () => {
    ...
    expect(
      body.tables.some((t) => t.logical === "blob.custody_state" && t.machinery)
    ).toBe(true);
  });
  ...
  test("POST /browse/insert writes through the journalled path and reads back", async () => {
    ...
    expect(((await insert.json()) as { ok: boolean; id: string }).id).toBe(
      "S9"
    );

    const row = await fetch(`${base}${B}/row?table=core.concept_scheme&id=S9`);
    expect(row.status).toBe(200);
    expect(((await row.json()) as { row: { title: string } }).row.title).toBe(
      "Browse Scheme"
    );
  });
  ...
  test("POST /browse/delete refuses with 409 + dependents when engine FKs point in", async () => {
    ...
    expect(
      body.dependents.some(
        (d) =>
          d.mechanism === "fk" && d.via === "core_party_identifier.party_id"
      )
    ).toBe(true);
    expect(
      body.dependents.some(
