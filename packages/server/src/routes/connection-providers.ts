...
function preset(
  ...
  return {
    ...base,
    capabilities:
      base.capabilities ?? capabilitiesFromConnectors(base.connectors),
  };
}
...
export const PROVIDER_PRESETS: readonly ProviderPreset[] = [
  preset({
    ...
    setup: [
      ...
      'Press "Publish app" so the status reads In production — Testing status expires your refresh token every 7 days.',
      'Unverified production costs one "unverified app" screen, clicked through once: Advanced → continue.',
      "APIs & Services → Credentials → Create credentials → OAuth client ID → type Web application.",
      "Add the redirect URI Centraid shows you on the Connect screen, then copy the Client ID and Client secret here.",
      "Google Photos needs Takeout file-drop, not OAuth — the Photos API closed in 2025.",
    ],
    connectors: [
      {
  ...
  preset({
    ...
    setup: [
      ...
      'Name it, then set supported account types to "Accounts in any organizational directory and personal Microsoft accounts".',
      "Add the redirect URI Centraid shows you, platform Web.",
      "Certificates & secrets → New client secret — copy the Value once (it is not shown again).",
      "API permissions → Add → Microsoft Graph → delegated: User.Read, Mail.Read, Calendars.Read, Contacts.Read, Files.Read.",
      "Grant admin consent only if your tenant requires it — personal accounts do not.",
  ...
  preset({
    ...
    setup: [
      ...
      "Scope it to the repositories you want, read-only (Contents, Issues, Pull requests, Metadata).",
      "Set a long expiry (or no expiry) — when the token does expire, the connection flips to needs-auth and tells you.",
      "Paste the token here as the credential.",
    ],
  ...
  preset({
    ...
    setup: [
      ...
      "This preset targets gitlab.com — self-hosted needs a host pin change.",
    ],
  ...
  preset({
    ...
    setup: [
      ...
      "Paste the key here — the connector lists only issues you already see.",
    ],
  ...
  preset({
    ...
    setup: [
      ...
      "Paste it here — the connector lists active tasks only.",
    ],
  ...
  preset({
    ...
    setup: [
      ...
      "Paste the token here — read-only, the connector never posts.",
    ],
  ...
  preset({
    ...
    setup: [
      ...
      "Paste them here and authorize — folder metadata only, never file bytes.",
    ],
