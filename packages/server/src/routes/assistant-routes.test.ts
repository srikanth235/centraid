...
describe("assistant-routes suite", () => {
  ...
  test("_turn streams the harness and records under the _assistant scope", async () => {
    ...
    expect(text).toMatch(/event: tool.start/u);
    expect(text).toMatch(/"sql":"SELECT 1"/u);
    ...
    const kinds = loaded?.messages.map(
      (m) => (m.payload as { kind: string }).kind
    );
  ...
  test("_turn threads valid attachment refs to the harness as resolved blob paths and records them on the ledger", async () => {
    ...
    const uploaded = await store.uploadBlob(
      ASSISTANT_APP_ID,
    ...
    const resolved = seenAttachments as Array<{
      path: string;
      mime: string;
      filename?: string;
    }>;
