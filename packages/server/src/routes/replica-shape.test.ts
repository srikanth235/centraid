...
describe("replica-shape suite", () => {
  ...
  test("sealed names remain sticky metadata while values never enter a replica row", async () => {
    ...
    vault.ensureAppInstallGrant("passwords", {
    ...
    expect(JSON.stringify(replicaShapesWire([shape!]))).not.toContain(
      '"password"'
    );
  ...
  test("consent-masked columns remain sticky unavailable for local handler fallback", async () => {
    const vault = await plane();
    vault.ensureAppInstallGrant("tasks", {
      purpose: "dpv:ServiceProvision",
      scopes: [
        {
          schema: "schedule",
  ...
  test("unmasked shapes hide protocol credential names and values", async () => {
    const vault = await plane();
    vault.ensureAppInstallGrant("credential-auditor", {
      purpose: "dpv:ServiceProvision",
      scopes: [
    ...
    expect(JSON.stringify(shape)).not.toMatch(/public-never-replicate/u);
  });

  test("credential predicates and credential-only masks fail closed", async () => {
    const vault = await plane();
    vault.ensureAppInstallGrant("credential-filter", {
      purpose: "dpv:ServiceProvision",
      scopes: [
        {
          schema: "access",
          ...
          verbs: "read",
          rowFilter: [
            { column: "signing_key", op: "eq", value: "must-not-evaluate" },
          ],
        },
      ],
    });
    vault.ensureAppInstallGrant("credential-mask", {
      purpose: "dpv:ServiceProvision",
      scopes: [
        {
          schema: "access",
  ...
  test("keeps a declared scope's row filter and field mask in the shape", async () => {
    ...
    const billingTask = readReplicaRow(
      vault.db.vault,
      "schedule.task",
      "billing-task"
    )!;
    expect(
      shapeReplicaRow(service, "schedule.task", serviceTask)?.values
    ).toStrictEqual({
  ...
  test("uses the exact first declared scope the canonical online read selects", async () => {
    const vault = await plane();
    vault.ensureAppInstallGrant("planner", {
      purpose: "dpv:ServiceProvision",
      scopes: [
    ...
    expect(onlineRows).toStrictEqual([
      { task_id: "both-scopes", title: "Canonical first" },
    ]);
  ...
  test.each([
    ...
    expect(replica ? [replica.values] : []).toStrictEqual(onlineRows);
  });
  ...
  test("changes temporal shape identity exactly when a row enters and leaves its window", async () => {
    ...
    const before = buildReplicaShapes(
      vault.db.vault,
      access,
      "2026-07-08T23:59:59.999Z"
    )[0]!;
    const entered = buildReplicaShapes(
      vault.db.vault,
      access,
      "2026-07-09T00:00:00.000Z"
    )[0]!;
    const leaving = buildReplicaShapes(
      vault.db.vault,
      access,
      "2026-07-10T00:00:00.000Z"
    )[0]!;
    const left = buildReplicaShapes(
      vault.db.vault,
      access,
    ...
    expect(
      shapeReplicaRow(
        ...
        "schedule.task",
        row,
        Date.parse("2026-07-10T00:00:00.001Z")
      )
    ).toBeUndefined();
  });
  ...
  test("expires within-days membership without requiring a database write", async () => {
    ...
    const atBoundary = buildReplicaShapes(
      vault.db.vault,
      access,
      "2026-07-10T00:00:00.000Z"
    )[0]!;
    const expired = buildReplicaShapes(
      vault.db.vault,
      access,
      "2026-07-10T00:00:00.001Z"
    )[0]!;
    ...
    expect(currentReplicaLogState(vault.db.vault).watermark).toStrictEqual(
      watermark
    );
  ...
  test("omits an unrelated filtered delete without rebootstrap or row-id disclosure", async () => {
    ...
    vault.db.vault
      .prepare(`DELETE FROM schedule_task WHERE task_id = 'private-task-id'`)
      .run();
    ...
    expect(projected.batch.changes).toStrictEqual([]);
  ...
  test("uses one stable opaque id through snapshot, update-out and delete", async () => {
    ...
    expect(snapshot.rowId).toMatch(/^r_[A-Za-z0-9_-]+$/u);
  ...
  test("never serializes either component of a masked composite primary key", async () => {
    const vault = await plane();
    vault.ensureAppInstallGrant("tally", {
      purpose: "dpv:ServiceProvision",
      scopes: [
        {
          schema: "tally",
    ...
    expect(shaped.rowId).toMatch(/^r_/u);
    expect(shaped.values).toStrictEqual({
      __centraid_row_id: shaped.rowId,
      share_minor: 100,
    });
  ...
  test("doorbells name only the shapes that received the projected row", async () => {
    ...
    vault.ensureAppInstallGrant("biller", {
      purpose: "dpv:ServiceProvision",
      scopes: [
        {
          schema: "schedule",
  ...
  test("the vault standing sweep enforces replica retention at startup", async () => {
    ...
    ).toBeGreaterThanOrEqual(old.seq);
  });
  ...
  test("the photos grant yields a self-contained shape a native client can render from", async () => {
    ...
    vault.ensureAppInstallGrant("photos", {
      purpose: "dpv:ServiceProvision",
      scopes: [
  ...
  test("docs and agenda grants multiplex as additive self-contained native shapes", async () => {
    const vault = await plane();
    vault.ensureAppInstallGrant("docs", {
      purpose: "dpv:ServiceProvision",
      scopes: [
    ...
    vault.ensureAppInstallGrant("agenda", {
      purpose: "dpv:ServiceProvision",
      scopes: [
    ...
    const docsByEntity = new Map(
      docs.entities.map((entity) => [entity.entity, entity])
    );
    const agendaByEntity = new Map(
      agenda.entities.map((entity) => [entity.entity, entity])
    );
    expect([...docsByEntity.keys()]).toStrictEqual(
    ...
    expect([...agendaByEntity.keys()]).toStrictEqual(
      expect.arrayContaining([
        "core.event",
        "schedule.attendee",
        "schedule.calendar",
    ...
    expect(agendaByEntity.get("core.event")!.columns).toStrictEqual(
      expect.arrayContaining([
        "event_id",
        "summary",
        "dtstart",
    ...
    expect(agendaByEntity.get("schedule.attendee")!.columns).toStrictEqual(
      expect.arrayContaining([
        "attendee_id",
        "event_id",
        "party_id",
    ...
    expect(agendaByEntity.get("core.party")!.columns).toStrictEqual(
      expect.arrayContaining(["party_id", "display_name"])
    );
    expect(new Set(shapes.map((shape) => shape.shapeId)).size).toBe(
      shapes.length
    );
  });
});
