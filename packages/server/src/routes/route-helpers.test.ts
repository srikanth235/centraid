...
describe("route-helpers scenarios", () => {
  ...
  describe("writeFileMap / readFileMap", () => {
    it("round-trips a file map, creating parent dirs and skipping non-editable/dotfiles", async () => {
      ...
      expect(readFileSync(path.join(dir, "nested/app.js"), "utf8")).toBe(
        "export const x = 1;"
      );
    ...
    it("refuses to write outside the app dir", async () => {
      const dir = tmp();
      await expect(
        writeFileMap(dir, [{ path: "../escape.ts", content: "x" }])
      ).rejects.toThrow(/outside the app/u);
    });

    it("reads a missing dir as an empty map", async () => {
      await expect(
        readFileMap(path.join(tmp(), "does-not-exist"))
  ...
  describe("readBody / readJson", () => {
    ...
    it("throws when the body exceeds the cap", async () => {
      await expect(readBody(mockReq("way too long"), 4)).rejects.toThrow(
        /too large/u
      );
    });
    ...
    it("rejects a non-object JSON body", async () => {
      await expect(readJson(mockReq("[1,2,3]"))).rejects.toThrow(
