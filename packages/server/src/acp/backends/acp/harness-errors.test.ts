...
describe("harness-errors suite", () => {
  test("AUTH_REQUIRED uses install hint", () => {
    const msg = classifyHarnessFailure(
  ...
  test("unrelated errors keep message + stderr tail", () => {
    const msg = classifyHarnessFailure(new Error("boom"), "stack line", {
      kind: "acp",
      acpArgs: [],
    });
  ...
  test.each([
    ...
    ["spawn ENOENT", "spawn"],
    ["harness exited with signal", "exit"],
  ] as const)("classifies %s as %s", (message, failureClass) => {
    expect(
      classifyHarnessFailureDetail(new Error(message), "", config).failureClass
    ).toBe(failureClass);
  });
  ...
  test("a crash whose stderr mentions a timeout is not classified as a timeout", () => {
    const detail = classifyHarnessFailureDetail(
      new Error("ACP harness exited with code 101"),
      CRASH_STDERR,
      config
    );
  ...
  test("stderr is still consulted when the message decides nothing", () => {
    expect(
      classifyHarnessFailureDetail(
        new Error("harness stopped responding"),
        CRASH_STDERR,
        config
      ).failureClass
    ).toBe("timeout");
