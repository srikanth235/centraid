/** Package-local Stryker options (types from root @stryker-mutator/core). */
export default {
  packageManager: "npm",
  inPlace: true,
  testRunner: "vitest",
  vitest: {
    configFile: "vitest.automation.mutation.config.ts",
    related: false,
  },
  testFiles: ["src/automation/fire/scheduler-ledger.contract.test.ts"],
  mutate: ["src/automation/fire/scheduler-ledger.ts"],
  reporters: ["clear-text", "json"],
  jsonReporter: { fileName: "../../artifacts/mutation/automation-report.json" },
  thresholds: { high: 80, low: 50, break: null },
  timeoutMS: 60_000,
  concurrency: 2,
  ignoreStatic: true,
  disableTypeChecks: true,
};
