...
describe("remote-provider", () => {
  ...
  describe("RemoteBackupProvider against the fake gateway", () => {
    ...
    test("envelope: capabilities unwraps the {data} envelope", async () => {
      ...
      expect(caps.capabilities).toStrictEqual([
        "backup",
        "cas",
        "derived",
        "usage",
      ...
      expect(caps.backup?.retention).toStrictEqual({
        kind: "ladder",
        keepAllDays: 7,
        dailyDays: 30,
        weeklyDays: 365,
    ...
    test("error mapping: not_found target maps to BackupProviderError with the right code/status", async () => {
      const { provider } = await fixture();
      await expect(
        provider.getSnapshot("unknown-target", 1)
    ...
    test("credential modes round-trip through the fake S3 data plane", async () => {
      ...
      expect(new TextDecoder().decode(await rw.get("chunks/x"))).toBe(
        "remote hello"
      );

      const ro = await provider.openDataPlane(targetId, "backup", "read");
      expect(new TextDecoder().decode(await ro.get("chunks/x"))).toBe(
        "remote hello"
      );
      await expect(ro.put("chunks/y", new Uint8Array(1))).rejects.toThrow(
        'object store opened in "read" mode; put refused for "chunks/y"'
      );
    });
    ...
    test("policy, inventory, and audit routes preserve their wire envelopes", async () => {
      ...
      await expect(provider.getPolicy(targetId)).resolves.toStrictEqual(policy);
      ...
      expect(inventory.objects).toMatchObject([
        { key: "blobs/a", sizeBytes: 3, state: "live" },
      ]);
    ...
    test("SigV4 PUT/GET requests carry authorization and payload hashes", async () => {
      ...
      const putReq = gateway.s3.requests.find(
        (request) => request.method === "PUT"
      );
      const getReq = gateway.s3.requests.find(
        (request) =>
          request.method === "GET" && !request.path.includes("list-type")
      );
      for (const request of [putReq!, getReq!]) {
        ...
        expect(request.headers.authorization).toMatch(
          /Signature=[0-9a-f]{64}/u
        );
        expect(request.headers["x-amz-content-sha256"]).toMatch(
          /^[0-9a-f]{64}$/u
        );
  ...
  describe("full conformance run against RemoteBackupProvider + fake gateway", () => {
    ...
    test.each(
      providerConformanceCases(makeHarness).map(
