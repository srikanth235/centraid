...
describe('wal-format', () => {
  ...
  describe('sealWalSegment / openWalSegment', () => {
    ...
    test('roundtrips', () => {
      ...
      expect([...openWalSegment(DATA_KEY, VAULT_ID, addr, sealed)]).toStrictEqual([...plain100]);
    });
    ...
    test('is deterministic: same (address, bytes) seals to byte-identical output (G7)', () => {
      ...
      expect([...openWalSegment(DATA_KEY, VAULT_ID, addr, b)]).toStrictEqual([...plain]);
    });
    ...
    test('a longer crash-retry range from the same start gets a DIFFERENT nonce', () => {
      ...
      expect([...sealedShort.subarray(0, 12)]).not.toStrictEqual([...sealedLong.subarray(0, 12)]);
    });
    ...
    test('pins the FORMAT.md nonce info and AAD strings verbatim', () => {
      ...
      expect([...sealWalSegment(DATA_KEY, VAULT_ID, addr, plain100)]).toStrictEqual([...expected]);
    });
  ...
  describe('sealWalCloser / openWalCloser', () => {
    ...
    test('pins the FORMAT.md closer info and AAD strings verbatim', () => {
      ...
      expect([...sealWalCloser(DATA_KEY, VAULT_ID, c)]).toStrictEqual([...expected]);
    });
