...
describe('scaffoldProjectFiles', () => {
  it('emits the canonical file set with the id/name baked in', () => {
    const files = byPath(scaffoldProjectFiles('todos', { name: 'Todos' }));
  ...
  it('defaults name to the id and carries a trimmed description', () => {
    const files = byPath(scaffoldProjectFiles('notes', { description: '  jot things  ' }));
  ...
  it('rejects an invalid id', () => {
    assert.throws(() => scaffoldProjectFiles('_bad'), /Invalid app id/);
  });
});

describe('updateProjectMetaFiles', () => {
  const base = (): ScaffoldFile[] => scaffoldProjectFiles('todos', { name: 'Todos' });

  it('rewrites app.json#name and the index.html <title> on rename', () => {
    const changed = byPath(updateProjectMetaFiles(base(), 'todos', { name: 'Tasks' }));
  ...
  it('clears description on empty patch and only touches app.json', () => {
    const start = scaffoldProjectFiles('todos', { name: 'Todos', description: 'x' });
    const changed = byPath(updateProjectMetaFiles(start, 'todos', { description: '   ' }));
  ...
  it('rejects an empty name and a duplicate display name', () => {
    assert.throws(() => updateProjectMetaFiles(base(), 'todos', { name: '  ' }), /cannot be empty/);
    assert.throws(
      () =>
        updateProjectMetaFiles(base(), 'todos', { name: 'Other' }, [{ id: 'x', name: 'other' }]),
      /already exists/,
    );
  });

  it('allows renaming to the apps own current name (self excluded)', () => {
    const changed = updateProjectMetaFiles(base(), 'todos', { name: 'Todos' }, [
      { id: 'todos', name: 'Todos' },
    ]);
