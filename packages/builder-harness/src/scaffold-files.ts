/*
 * Filesystem-free project scaffolding (issue #141).
 *
...
import { rewriteTitleInHtml, applyManifestName } from './project-rewrites.js';
...
/**
 * Build the file map for a fresh project (issue #141): package.json,
 ...
 */
export function scaffoldProjectFiles(id: string, opts: ScaffoldProjectOpts = {}): ScaffoldFile[] {
  validateAppId(id);
 ...
 * `updateProjectMeta` behavior without touching disk.
 *
 ...
 */
export function updateProjectMetaFiles(
...
/** The per-app package.json a fresh project ships (no compile step). */
