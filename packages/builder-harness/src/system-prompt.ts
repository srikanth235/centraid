...
- **Webhook triggers** (\`{ "kind": "webhook" }\`) need a server-minted \`id\` + \`secretHash\` you cannot generate — author only cron (or manual-only) automations. If the app genuinely needs to react to inbound HTTP, say so in chat instead of writing a webhook trigger.
...
- **Webhook triggers: never write a webhook trigger object into the manifest.** A webhook trigger needs a server-minted \`id\` + \`secretHash\` you cannot generate. The chat builder supports only cron schedules and manual runs — if the user asks for a webhook, explain that and offer a cron schedule (or a manual-only automation, \`"triggers": []\`) instead.
