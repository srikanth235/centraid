...
export function toCss(): string {
  ...
  // Tile values are shared between Home and Discover; the `--tile-` namespace
  // is deliberately not per-surface.
  ...
  // carries no `data-theme` until the renderer has read the member's prefs, so
  // BOTH themes must be reachable from this block. Do not paper over a light
  // flash by hardcoding `data-theme` in index.html: a stamped attribute always
  // wins, which makes "follow the system" unreachable. `blueprint.ts` emits
  ...
  const blocks = [
    ...
    // under a pointer (v7 §C: `CTL = touch ? 44 : 34`). Never a literal 32 —
    // it is off the scale, smaller than the control height every other token
    // agrees on, and reached by a number rather than by the metric.
    [
      "@media (pointer: fine) {",
