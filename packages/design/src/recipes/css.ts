...
export function emitRecipeCss(scope = ":root"): string {
  const lines = [
    ...
    `${scope} .kit-btn { min-height: var(--target-min, 44px); border-radius: var(--r-md); transition: background var(--dur-1) var(--ease), color var(--dur-1) var(--ease), border-color var(--dur-1) var(--ease); }`,
