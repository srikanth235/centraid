...
function themeProps(theme: Theme): Record<string, string> {
  const props: Record<string, string> = {
    ...
    "--net": theme.net,
    "--on-accent": "#FDFDFC",
    // Same literal in both themes — see css.ts's themeProps for why.
    "--on-stage": ON_STAGE,
...
export function toBlueprintCss(): string {
  ...
  return [
    ...
    // Same role classes the shell sheet publishes, so an app inside the pane
    // reaches a role the way the shell does — and the way native's `t()` does.
    // Scoped to the roles this profile supports, so a blueprint cannot compose
    // a class whose `--t-*` the blueprint sheet never defined.
    typeRoleClasses(typeForProfile("blueprint")),
    emitRecipeCss(":root"),
