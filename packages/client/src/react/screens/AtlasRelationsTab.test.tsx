...
describe("AtlasRelationsTab", () => {
  ...
  describe("AtlasRelationsTab", () => {
    it("renders a quiet empty state when the graph is null", async () => {
      ...
      expect(
        el.querySelector('[data-testid="atlas-relations-empty"]')
      ).not.toBeNull();
    ...
    it("renders nodes and FK edges from the payload", async () => {
      ...
      expect(
        el.querySelector(
          '[data-testid="atlas-edge"][data-to="sync_connection"]'
        )
      ).toBeNull();
    });
    ...
    it("marks self-referencing kinds with a curl glyph, not a loop edge", async () => {
      ...
      expect(
        concept?.querySelector('[data-testid="atlas-selfref-glyph"]')
      ).not.toBeNull();
    });
    ...
    it("puts unreached ontology kinds on the unreached ring, agreeing with island", async () => {
      ...
      for (const p of unreachedRendered)
        expect(makeGraph().island).toContain(p);
    });
    ...
    it("toggles the authored-link overlay from a relation chip", async () => {
      ...
      const chip = el.querySelector(
        '[data-testid="atlas-relation-chip"][data-relation="mentions"]'
      );
      expect(chip?.getAttribute("aria-pressed")).toBe("false");

      await fire(chip, "click");
      expect(
        el.querySelector('[data-testid="atlas-authored-arc"]')
      ).not.toBeNull();
      expect(
        el
          .querySelector(
    ...
    it('shows "Nothing here yet." when the centre table has no rows', async () => {
      ...
      await flush();
      expect(
        el.querySelector('[data-testid="atlas-samples-empty"]')?.textContent
    ...
    it("omits the samples section entirely when the fetch fails (never invents)", async () => {
      ...
      expect(
        el.querySelector('[data-testid="atlas-samples-empty"]')
      ).toBeNull();
    });
    ...
    it("wraps every layer in one viewport group at identity by default", async () => {
      ...
      expect(
        viewport?.querySelector('[data-testid="atlas-node"]')
      ).not.toBeNull();
      expect(
        viewport?.querySelector('[data-testid="atlas-edge"]')
      ).not.toBeNull();
    });
