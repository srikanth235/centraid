...
import HouseholdScreen, { type HouseholdScreenProps } from './HouseholdScreen.js';
...
function scope(over: Partial<MemberScope> = {}): MemberScope {
  return { id: 'v1', label: 'Personal', role: 'admin', canWrite: true, ...over };
}

async function mount(props: Partial<HouseholdScreenProps> = {}): Promise<HTMLDivElement> {
...
describe('HouseholdScreen', () => {
  it('leads with people and devices, then the spaces they can reach', async () => {
    ...
    expect(text.indexOf('People & devices')).toBeLessThan(text.indexOf('Spaces'));
  });
  ...
  it('badges exactly one card as the default and offers space settings only there', async () => {
    ...
    expect(onOpenSpaceSettings).toHaveBeenCalled();
  });
  ...
  it('renders the roster card when the host can list devices', async () => {
    const loadDevices = vi.fn(() => Promise.resolve([]));
    const el = await mount({
      loadDevices,
