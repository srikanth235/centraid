...
describe("resource-summary", () => {
  ...
  describe(formatBudgetSummary, () => {
    ...
    it("singularizes a lone worker/core", () => {
      ...
      expect(formatBudgetSummary(solo)).toBe(
        "Up to ~1.3 GB memory · 1 background worker on 1 core"
      );
    });
  });
  ...
  describe("L2 fact rows", () => {
    it('lists host facts, "not measured" when fsync is null', () => {
      expect(hostFactRows(sample)).toStrictEqual([
      ...
      expect(hostFactRows(noFsync)[2]).toStrictEqual({
        label: "Storage fsync",
        value: "not measured",
      });
    });
