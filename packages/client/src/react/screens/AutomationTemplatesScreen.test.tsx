...
function makeProps(
  ...
  return {
    templates,
    onPreview: vi.fn<AutomationTemplatesBridgeProps["onPreview"]>(),
    onStartFromScratch:
      vi.fn<AutomationTemplatesBridgeProps["onStartFromScratch"]>(),
    ...over,
  };
}
...
describe("screens/AutomationTemplatesScreen", () => {
  afterEach(() => {
    act(() => root?.unmount());
    root = null;
    container?.remove();
  ...
  describe(AutomationTemplatesScreen, () => {
    it("renders a card per template grouped by category, with the trigger filter", () => {
      const html = renderToStaticMarkup(
        <AutomationTemplatesScreen {...makeProps()} />
      );
    ...
    it("offers Data and Condition as segmented filter options and filters by them", () => {
      ...
      void act(() =>
        conditionTab.dispatchEvent(new MouseEvent("click", { bubbles: true }))
      );
      expect(el.querySelectorAll(".card")).toHaveLength(1);
    ...
    it("filters by trigger kind", () => {
      ...
      void act(() =>
        webhookTab.dispatchEvent(new MouseEvent("click", { bubbles: true }))
      );
      expect(el.querySelectorAll(".card")).toHaveLength(1);
    ...
    it("shows the empty state + Start-from-scratch when a search matches nothing", () => {
      ...
      void act(() =>
        scratch.dispatchEvent(new MouseEvent("click", { bubbles: true }))
      );
      expect(props.onStartFromScratch).toHaveBeenCalledOnce();
    });
  });
});
