...
function makeProps(
  over: Partial<SettingsLayoutBridgeProps> = {}
): SettingsLayoutBridgeProps {
  return {
    density: "regular",
...
describe("screens/SettingsLayoutScreen", () => {
  ...
  const seg = (el: HTMLElement, group: number): HTMLButtonElement[] =>
    [
      ...el.querySelectorAll(".seg")[group]!.querySelectorAll("button"),
  ...
  describe(SettingsLayoutScreen, () => {
    it("renders density (3) + cards (3) segmented + a sidebar switch, with active reflecting props", async () => {
      ...
      expect(
        seg(el, 0).find((b) => b.textContent === "regular")?.dataset.active
      ).toBe("true");
      expect((el.querySelector(".switch") as HTMLElement).dataset.on).toBe(
        "true"
      );
      expect(
