...
describe("screens/StorageScreen", () => {
  afterEach(() => {
    act(() => root?.unmount());
    host?.remove();
    root = null;
  ...
  const noop = (): void => {};
  const noRunBackupNow = (): Promise<{ accepted: boolean }> =>
    new Promise(() => {});
  const noConfirmRecoveryKit = (): Promise<{ confirmedAt: number }> =>
    new Promise(() => {});

  const GB = 1024 ** 3;

  function report(
    over: Partial<LocalUsageReportDTO> = {}
  ): LocalUsageReportDTO {
    return {
      scannedAt: NOW,
      totalBytes: 3 * GB,
  ...
  async function mount(
    ...
    const props: StorageScreenProps = {
      ...
      loadBackupStatus: () =>
        Promise.resolve({ configured: false, vaults: [] }),
  ...
  describe(StorageScreen, () => {
    it("renders all three cards, each wired to its own loader", async () => {
      const loadBackupStatus = vi
        .fn<StorageScreenProps["loadBackupStatus"]>()
      ...
      expect(loadBackupStatus).toHaveBeenCalledWith();
    ...
    it("refuses an unparseable custom limit without calling the gateway", async () => {
      const saveStorageLimits =
        vi.fn<StorageScreenProps["saveStorageLimits"]>();
      ...
      expect(
        ledger.querySelector('[data-testid="limit-error-ledger"]')?.textContent
      ).toContain("Enter a size");
    });

    it("pulls aggregate provider usage for the Cost metric when a loader is supplied", async () => {
      const loadStorageUsage = vi
        .fn<NonNullable<StorageScreenProps["loadStorageUsage"]>>()
        .mockResolvedValue(null);
      await mount({
        loadBackupStatus: vi
          .fn<StorageScreenProps["loadBackupStatus"]>()
          .mockResolvedValue({ configured: true, vaults: [] }),
        loadStorageUsage,
      });
      expect(loadStorageUsage).toHaveBeenCalledWith();
    });
    ...
    it("does not gate on a gateway heartbeat — it paints with no runtime snapshot", async () => {
      ...
      expect(el.textContent).not.toContain(
        "Listening for the gateway heartbeat"
      );
    ...
    it("routes the backup card’s Manage link to the settings handler", async () => {
      const onOpenStorageSettings =
        vi.fn<NonNullable<StorageScreenProps["onOpenStorageSettings"]>>();
      ...
      expect(onOpenStorageSettings).toHaveBeenCalledWith(
