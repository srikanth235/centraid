...
describe('screens/ResourceModeCard', () => {
  ...
  describe(ResourceModeCard, () => {
    it('renders the four modes and highlights the loaded selection', async () => {
      const el = await mount({
        loadMode: vi.fn<ResourceModeCardProps['loadMode']>().mockResolvedValue('conserve'),
    ...
    it('saves a new mode through the bridge', async () => {
      const saveMode = vi.fn<ResourceModeCardProps['saveMode']>().mockResolvedValue(undefined);
      const el = await mount({
  ...
  describe('ResourceModeCard — L1 budget summary + L2 disclosure', () => {
    it('renders the budget line + host attribution from resourceProfile', async () => {
      const el = await mount({
        loadMode: vi.fn<ResourceModeCardProps['loadMode']>().mockResolvedValue('balanced'),
        saveMode: vi.fn<ResourceModeCardProps['saveMode']>(),
