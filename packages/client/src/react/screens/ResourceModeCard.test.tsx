...
describe("screens/ResourceModeCard", () => {
  afterEach(() => {
    act(() => root?.unmount());
    root = null;
    container?.remove();
  ...
  function checkedRadioText(el: HTMLElement): string | undefined {
    return (
      el.querySelector('input[type="radio"]:checked')?.closest("label")
  ...
  describe(parseResourceModePref, () => {
    it("reads the durable prefs key and defaults to auto", () => {
      expect(
        parseResourceModePref({ [RESOURCE_MODE_PREF_KEY]: "conserve" })
      ).toBe("conserve");
      expect(parseResourceModePref({})).toBe("auto");
      expect(parseResourceModePref({ [RESOURCE_MODE_PREF_KEY]: "nope" })).toBe(
        "auto"
      );
    });
  });
  ...
  describe(ResourceModeCard, () => {
    ...
    it("saves a new mode through the bridge", async () => {
      const saveMode = vi
        .fn<ResourceModeCardProps["saveMode"]>()
    ...
    it("does not re-fetch when the parent re-renders with a stable loadMode", async () => {
      ...
      await act(async () => {
        root?.render(
          <ResourceModeCard loadMode={loadMode} saveMode={saveMode} />
        );
      });
      await act(async () => {
        await Promise.resolve();
    ...
    it("ignores a late loadMode resolve while a save is in flight", async () => {
      ...
      const loadMode = vi.fn<ResourceModeCardProps["loadMode"]>(
        () =>
      ...
      const saveMode = vi.fn<ResourceModeCardProps["saveMode"]>(
        () =>
  ...
  describe("ResourceModeCard — L1 budget summary + L2 disclosure", () => {
    ...
    it('opens the "How we sized this" dialog with host facts + resolved knobs', async () => {
      ...
      expect(
        el.querySelector('[data-testid="resource-details-body"]')
      ).toBeNull();
  ...
  describe("ResourceModeCard — pause background work (L0)", () => {
    it("hides the pause control when backgroundPause is absent", async () => {
      const el = await mount({
        ...
        onResume: vi.fn<NonNullable<ResourceModeCardProps["onResume"]>>(),
      });
    ...
    it("reveals durations and pauses with the chosen ms", async () => {
      ...
      expect(
        el.querySelector('[data-testid="resource-pause-active"]')?.textContent
      ).toContain("Paused until 15:00");
    });
    ...
    it('passes the computed ms for "Until tonight"', async () => {
      ...
      const onPause = vi
        .fn<NonNullable<ResourceModeCardProps["onPause"]>>()
        .mockResolvedValue({ paused: true, until: null });
      const el = await mount({
        loadMode: vi
          .fn<ResourceModeCardProps["loadMode"]>()
          .mockResolvedValue("auto"),
      ...
      expect(onPause).toHaveBeenCalledWith(
        msUntilTonight(new Date(2026, 6, 23, 10, 0, 0).getTime())
      );
    });

    it("shows the paused state and resumes", async () => {
      const onResume = vi
        .fn<NonNullable<ResourceModeCardProps["onResume"]>>()
