...
function LimitControl({
  ...
  const [draft, setDraft] = useState(() => (value === null ? '' : formatBytes(value)));
  ...
  const submitDraft = (): void => {
    ...
    if (bytes < floorBytes) {
      setError(`The smallest usable ${floorLabel} is ${formatBytes(floorBytes)}.`);
      return;
    }
    commit(bytes);
  };
  ...
  return (
    <div className={styles.control} data-testid={`limit-control-${id}`}>
      <div className={styles.controlHead}>
        ...
        <span className={styles.controlState} data-on={value !== null || undefined}>
        ...
        <div className={styles.presets}>
          {presets.map((preset) => (
            <button
              key={preset.label}
              type="button"
              className={cx(controlsCss.chip, value === preset.bytes && styles.presetActive)}
              disabled={busy}
...
export default function StorageLimitsPanel({
  ...
  return (
    <section className={cx(gwStyles.panel, styles.card)} data-testid="storage-limits-panel">
