...
describe("screens/BackupCard", () => {
  ...
  const neverConfirmKit = (): Promise<{ confirmedAt: number }> =>
    new Promise(() => {});
  ...
  describe("BackupCard — configured", () => {
    ...
    it("surfaces a vault-level lastError without hiding the row", async () => {
      ...
      expect(
        el.querySelectorAll('[data-testid="backup-vault-row"]')
      ).toHaveLength(1);
    });
  ...
  describe("BackupCard — recovery-kit gate", () => {
    it("requires a password before a configured gateway can export its wrapped kit", async () => {
      ...
      expect(
        el.querySelector('[data-testid="recovery-kit-confirmed"]')
      ).toBeNull();
    ...
    it("renders the quiet confirmed state with the date when already confirmed", async () => {
      ...
      const confirmed = el.querySelector(
    ...
    it("verifies the re-selected wrapped kit and flips to the confirmed state", async () => {
      ...
      const password = el.querySelector(
        'input[type="password"]'
      ) as HTMLInputElement;
      await act(async () => {
      ...
      await act(async () =>
        selectRecoveryKit(el, { format: "centraid-recovery-kit/2" })
      );
      const consent = el.querySelector(
        'input[type="checkbox"]'
      ) as HTMLInputElement;
