...
describe("AutomationsOverviewScreen suite", () => {
  ...
  describe(AutomationsOverviewScreen, () => {
    it("renders the header, live-count subtitle, and both automation tiles", async () => {
      ...
      const names = [
        ...el.querySelectorAll('[data-testid="automation-row-name"]'),
      ].map((n) => n.textContent);
      ...
      expect(
        grid?.querySelectorAll('[data-testid="automation-row"]').length
      ).toBe(2);
    });
    ...
    it("exposes data-au-status on each tile and the attention badge only when pending", async () => {
      ...
      const invoiceRow = rows.find((r) =>
        r.textContent?.includes("Invoice Sync")
      );
    ...
    it("renders suggested starters on the empty state and adopts via onUseSuggestion", async () => {
      ...
      expect(
        el.querySelector('[data-testid="automation-suggestions"]')
      ).toBeTruthy();
