...
function makeProps(
  ...
  return {
    loadData: vi.fn<AutomationsOverviewBridgeProps['loadData']>().mockResolvedValue(makeData()),
    onOpenAutomation: vi.fn<AutomationsOverviewBridgeProps['onOpenAutomation']>(),
    onOpenRun: vi.fn<AutomationsOverviewBridgeProps['onOpenRun']>(),
    onBrowseTemplates: vi.fn<AutomationsOverviewBridgeProps['onBrowseTemplates']>(),
    onNewAutomation: vi.fn<AutomationsOverviewBridgeProps['onNewAutomation']>(),
    ...over,
  };
}
...
describe('AutomationsOverviewScreen suite', () => {
  ...
  describe(AutomationsOverviewScreen, () => {
    ...
    it('exposes data-au-status on each tile and the attention badge only when pending', async () => {
      ...
      expect(el.querySelectorAll('.attentionBadge')).toHaveLength(1);
    ...
    it('renders the recent-activity feed grouped by date', async () => {
      ...
      expect(el.querySelectorAll('.activityRow')).toHaveLength(2);
    ...
    it('fires the header actions', async () => {
      ...
      expect(props.onNewAutomation).toHaveBeenCalledOnce();
    });
    ...
    it('shows the empty state with both CTAs when there are no automations', async () => {
      ...
      expect(
        [...el.querySelectorAll('button')].some((b) => b.textContent === 'New automation'),
      ).toBe(true);
      expect(
      ...
      const el = await mount(
        makeProps({
          loadData: vi
            ...
            .mockResolvedValue(makeData({ rows: [], runs: [] })),
          loadSuggestions: vi
            .fn<NonNullable<AutomationsOverviewBridgeProps['loadSuggestions']>>()
            .mockResolvedValue([
              {
