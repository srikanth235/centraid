...
function makeProps(over: Partial<PhoneBridgeProps> = {}): PhoneBridgeProps {
  return {
    loadStatus: vi.fn<PhoneBridgeProps['loadStatus']>().mockResolvedValue(emptyStatus),
    beginPairing: vi.fn<PhoneBridgeProps['beginPairing']>().mockResolvedValue(null),
...
describe('screens/PhoneScreen', () => {
  ...
  describe(PhoneScreen, () => {
    ...
    it('begins pairing and shows the QR + expiry, then clears on pairing completion', async () => {
      ...
      const beginPairing = vi.fn<PhoneBridgeProps['beginPairing']>(async (onPaired) => {
        firePaired = onPaired;
        return {
    ...
    it('renders the error note when the status cannot be read', async () => {
      const loadStatus = vi.fn<PhoneBridgeProps['loadStatus']>().mockResolvedValue(null);
