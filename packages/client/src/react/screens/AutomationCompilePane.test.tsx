...
function makeProps(over: Partial<AutomationCompilePaneProps> = {}): AutomationCompilePaneProps {
  return {
    ...
    dirty: false,
    compileNonce: 0,
    onCompile: vi.fn().mockResolvedValue('c-3'),
    onTestRun: vi.fn().mockResolvedValue('t-1'),
...
describe('AutomationCompilePane', () => {
  it('shows the last failed compile with its steps and the failure text', async () => {
    const el = await mount(
      makeProps({
        loadAttempts: vi.fn().mockResolvedValue([failedAttempt]),
        loadTurnSteps: vi.fn().mockResolvedValue(steps),
      }),
    );
    ...
    expect(rows.map((r) => (r as HTMLElement).dataset.status)).toEqual(['ok', 'fail']);
    expect(rows[1]?.textContent).toContain('typecheck');
  });

  it('compiles in place — it never navigates, and streams the new attempt', async () => {
    const onCompile = vi.fn().mockResolvedValue('c-3');
    const watchTurnSteps = vi.fn(async (_id: string, onSteps: (s: CompileStepDTO[]) => void) => {
      onSteps(steps);
    ...
    const el = await mount(
      makeProps({
        onCompile,
        watchTurnSteps,
        loadAttempts: vi.fn().mockResolvedValue([okAttempt]),
      }),
    );
    ...
    expect(el.querySelectorAll('[data-testid="compile-step"]').length).toBe(2);
  });

  it('falls back to a cold read when the compile stream drops mid-turn', async () => {
    const loadTurnSteps = vi.fn().mockResolvedValue(steps);
    const el = await mount(
      makeProps({
        loadAttempts: vi.fn().mockResolvedValue([]),
        loadTurnSteps,
        ...
        watchTurnSteps: vi.fn().mockResolvedValue({ settled: false, ok: false }),
      }),
    );
    await click(el, 'compile-now');
  ...
  it('hands a failure back to the instructions instead of offering a second editor', async () => {
    ...
    const el = await mount(
      makeProps({ onEditInstructions, loadAttempts: vi.fn().mockResolvedValue([failedAttempt]) }),
    );
  ...
  it('gates Test run behind a successful compile', async () => {
    const failing = await mount(
      makeProps({ loadAttempts: vi.fn().mockResolvedValue([failedAttempt]) }),
    );
    expect(
    ...
    const onTestRun = vi.fn().mockResolvedValue('t-1');
    const passing = await mount(
      makeProps({ onTestRun, loadAttempts: vi.fn().mockResolvedValue([okAttempt]) }),
    );
    expect(
  ...
  it('calls a stale plan stale when the instructions moved on', async () => {
    const el = await mount(
      makeProps({ dirty: true, loadAttempts: vi.fn().mockResolvedValue([okAttempt]) }),
    );
  ...
  it('counts up while a compile is open, and shows no clock once it settles', async () => {
    ...
    try {
      ...
      const el = await mount(makeProps({ loadAttempts: vi.fn().mockResolvedValue([running]) }));
  ...
  it('shows no elapsed clock when nothing is running', async () => {
    const el = await mount(makeProps({ loadAttempts: vi.fn().mockResolvedValue([okAttempt]) }));
