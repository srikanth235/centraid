...
function makeProps(
  over: Partial<AutomationCompilePaneProps> = {}
): AutomationCompilePaneProps {
  return {
    mode: "edit",
    dirty: false,
    compileNonce: 0,
    onCompile: vi
      .fn<AutomationCompilePaneProps["onCompile"]>()
      .mockResolvedValue("c-3"),
    onTestRun: vi
      .fn<AutomationCompilePaneProps["onTestRun"]>()
      .mockResolvedValue("t-1"),
    onEditInstructions:
      vi.fn<AutomationCompilePaneProps["onEditInstructions"]>(),
    loadAttempts: vi
...
describe("screens/AutomationCompilePane", () => {
  ...
  async function mount(
    props: AutomationCompilePaneProps
  ...
  describe(AutomationCompilePane, () => {
    it("shows the last failed compile with its steps and the failure text", async () => {
      ...
      expect(rows.map((r) => (r as HTMLElement).dataset.status)).toStrictEqual([
        "ok",
        "fail",
      ]);
    ...
    it("compiles in place — it never navigates, and streams the new attempt", async () => {
      const onCompile = vi
        .fn<AutomationCompilePaneProps["onCompile"]>()
        .mockResolvedValue("c-3");
      const watchTurnSteps = vi.fn<
        AutomationCompilePaneProps["watchTurnSteps"]
      >(async (_id, onSteps) => {
        onSteps(steps);
      ...
      expect(el.querySelectorAll('[data-testid="compile-step"]')).toHaveLength(
        2
      );
    });
    ...
    it("gates Test run behind a successful compile", async () => {
      ...
      expect(
        failing.querySelector<HTMLButtonElement>(
      ...
      expect(
        passing.querySelector<HTMLButtonElement>(
          '[data-testid="compile-test-run"]'
        )?.disabled
      ).toBe(false);
