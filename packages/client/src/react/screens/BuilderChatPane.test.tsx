...
function makeProps(
  over: Partial<BuilderChatBridgeProps> = {}
): BuilderChatBridgeProps {
  return {
    onReady: vi.fn<BuilderChatBridgeProps["onReady"]>(),
    onSend: vi.fn<BuilderChatBridgeProps["onSend"]>(),
    onCancel: vi.fn<BuilderChatBridgeProps["onCancel"]>(),
...
describe("BuilderChatPane suite", () => {
  afterEach(() => {
    act(() => root?.unmount());
    root = null;
    container?.remove();
  ...
  describe(BuilderChatPane, () => {
    it("renders user, ai, thinking, status, and divider messages", () => {
      ...
      expect(
        (el.querySelector(".chatThinking") as HTMLElement).dataset.streaming
      ).toBe("true");
      expect(el.querySelectorAll(".aiText p")).toHaveLength(2);
    });
    ...
    it("renders a collapsed tool group with a change card, toggles on click", () => {
      ...
      expect((el.querySelector(".group") as HTMLElement).dataset.open).toBe(
        "false"
      );
      expect(el.querySelector(".label")?.textContent).toBe(
        "Editing ×2, Reading"
      );
      expect(el.querySelector(".tgCardTitle")?.textContent).toContain(
        "2 files updated"
      );
    ...
    it("shows the agent-progress strip only while generating; cancel fires", () => {
      ...
      expect(el.querySelectorAll(".abProgressDots i[data-on]")).toHaveLength(3);
      void act(() =>
        (
          el.querySelector(".abProgressCancel") as HTMLButtonElement
    ...
    it("does not send while the composer is disabled", () => {
      ...
      void act(() =>
        (
          el.querySelector('button[aria-label="Send"]') as HTMLButtonElement
        ).dispatchEvent(new MouseEvent("click", { bubbles: true }))
      );
    ...
    it("loads a suggestion into the composer draft", () => {
      ...
      push(snap());
      const chip = [
        ...el.querySelectorAll<HTMLButtonElement>(".promptStarter"),
      ].find((b) => b.textContent === "Prepare to publish")!;
      void act(() =>
    ...
    it("shows the durable primary workspace choices and reports a change", () => {
      ...
      expect([...picker.options].map((option) => option.value)).toStrictEqual([
        "draft",
        "app",
        "vault-data",
      ]);
    ...
    it("switches to the history view and mounts the vanilla renderer", () => {
      ...
      expect(el.querySelector(".chatpaneHeadTitle")?.textContent).toBe(
        "Version history"
      );
      expect(props.onMountHistory).toHaveBeenCalledExactlyOnceWith(
        expect.any(HTMLElement)
      );
      void act(() =>
        (
          el.querySelector(
            '.chatpaneHead button[aria-label="Back to chat"]'
    ...
    it("re-fetches history when the nonce bumps", () => {
      ...
      expect(props.onMountHistory).toHaveBeenCalledOnce();
