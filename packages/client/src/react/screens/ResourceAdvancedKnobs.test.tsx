...
describe("screens/ResourceAdvancedKnobs", () => {
  afterEach(() => {
    act(() => root?.unmount());
    root = null;
    container?.remove();
  ...
  async function mount(
    props: ResourceAdvancedKnobsProps
  ...
  describe("resource knob prefs helpers", () => {
    it("maps knob keys to the gateway.resource.* pref namespace", () => {
      expect(knobPrefKey("workerMaxConcurrent")).toBe(
        "gateway.resource.workerMaxConcurrent"
      );
      expect(knobPrefKey("replicationConcurrency")).toBe(
        "gateway.resource.replicationConcurrency"
      );
    });
  ...
  describe(ResourceAdvancedKnobs, () => {
    it("is collapsed by default and reveals four rows on expand", async () => {
      const el = await mount({
        ...
        loadKnobPrefs: vi
          .fn<ResourceAdvancedKnobsProps["loadKnobPrefs"]>()
          .mockResolvedValue(NO_OVERRIDES),
        saveKnobPrefs: vi.fn<ResourceAdvancedKnobsProps["saveKnobPrefs"]>(),
      });
      const toggle = el.querySelector(
        '[data-testid="resource-advanced-toggle"]'
      ) as HTMLButtonElement;
      ...
      expect(
        el.querySelector('[data-testid="resource-advanced-body"]')
      ).toBeNull();
      ...
      expect(
        el.querySelector('[data-testid="knob-workerMaxOldGenerationMb"]')
      ).not.toBeNull();
      expect(
        el.querySelector('[data-testid="knob-workerPoolSize"]')
      ).not.toBeNull();
      expect(
        el.querySelector('[data-testid="knob-replicationConcurrency"]')
      ).not.toBeNull();
    });
    ...
    it("renders Linked, Custom, and env-locked states", async () => {
      const el = await mount({
        ...
        loadKnobPrefs: vi
          .fn<ResourceAdvancedKnobsProps["loadKnobPrefs"]>()
          .mockResolvedValue({ ...NO_OVERRIDES, replicationConcurrency: 4 }),
        saveKnobPrefs: vi.fn<ResourceAdvancedKnobsProps["saveKnobPrefs"]>(),
      });
      await expand(el);
      ...
      expect(
        el.querySelector('[data-testid="knob-workerPoolSize-lock"]')
          ?.textContent
      ).toContain("CENTRAID_WORKER_POOL_SIZE");
      expect(lockedRow.textContent).toContain(
        "remove the variable to tune here"
      );
      expect(
        [...lockedRow.querySelectorAll("button")].some(
          (b) => b.textContent?.trim() === "Save"
        )
      ).toBe(false);
    });
    ...
    it("hard-rejects an out-of-bounds value: inline error, Save disabled", async () => {
      const saveKnobPrefs = vi
        .fn<ResourceAdvancedKnobsProps["saveKnobPrefs"]>()
      ...
      await expand(el);
      await act(async () =>
        setInput(inputFor(el, "workerMaxConcurrent"), "999")
      );

      expect(
        el.querySelector('[data-testid="knob-workerMaxConcurrent-error"]')
          ?.textContent
    ...
    it("shows a soft warning but still allows saving", async () => {
      ...
      expect(
        el.querySelector('[data-testid="knob-workerMaxConcurrent-warn"]')
          ?.textContent
    ...
    it("saves a valid override through the bridge", async () => {
      ...
      await act(async () =>
        saveBtn(el, "workerMaxConcurrent").dispatchEvent(
    ...
    it("renders nothing when the profile lacks sources/bounds (older gateway)", async () => {
      ...
      expect(
        el.querySelector('[data-testid="resource-advanced-toggle"]')
      ).toBeNull();
