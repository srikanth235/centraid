...
function makeProps(over: Partial<HomeBridgeProps> = {}): HomeBridgeProps {
  return {
    ...
    attention: 0,
    onBuild: vi.fn<HomeBridgeProps["onBuild"]>(),
    onOpenApp: vi.fn<HomeBridgeProps["onOpenApp"]>(),
    onEnterDraft: vi.fn<HomeBridgeProps["onEnterDraft"]>(),
    onAppContext: vi.fn<HomeBridgeProps["onAppContext"]>(),
    onOpenAutomation: vi.fn<HomeBridgeProps["onOpenAutomation"]>(),
    onAutomationMenu: vi.fn<HomeBridgeProps["onAutomationMenu"]>(),
    onBrowseTemplates: vi.fn<HomeBridgeProps["onBrowseTemplates"]>(),
    ...over,
  };
}
...
describe("screens/HomeScreen", () => {
  ...
  describe(HomeScreen, () => {
    it("renders the composer hero, suggestions, filter, and the unified grid", () => {
      ...
      expect(el.querySelectorAll(".heroSuggestions .chip")).toHaveLength(2);
      expect(el.querySelectorAll(".discSegB")).toHaveLength(3);
      // 2 apps + 1 automation card
      expect(el.querySelectorAll(".wrap")).toHaveLength(3);
    ...
    it("builds from the composer (send enabled after typing)", () => {
      ...
      typeInto(
        el.querySelector(".composerInput") as HTMLTextAreaElement,
        "a todo app"
      );
      expect(
        (el.querySelector(".composerSend") as HTMLButtonElement).disabled
      ).toBe(false);
      void act(() =>
    ...
    it("opens an app, enters a draft, and right-clicks for the context menu", () => {
      ...
      const el = mount(props);
      const cards = [
        ...el.querySelectorAll('.card[data-kind="app"]'),
    ...
    it("filters to automations and toggles the layout", () => {
      ...
      void act(() =>
        autoTab.dispatchEvent(new MouseEvent("click", { bubbles: true }))
      );
      expect(el.querySelectorAll(".wrap")).toHaveLength(1);
      ...
      void act(() =>
        rowsBtn.dispatchEvent(new MouseEvent("click", { bubbles: true }))
      );
      expect(
        (el.querySelector(".appsGrid") as HTMLElement).dataset.layout
      ).toBe("rows");
    });
