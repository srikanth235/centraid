...
beforeEach(() => {
  vi.stubGlobal('URL', {
    ...URL,
    createObjectURL: vi.fn().mockReturnValue('blob:kit'),
...
function props(over: Partial<FoundingScreenProps> = {}): FoundingScreenProps {
  return {
    ...
    verify: vi.fn().mockResolvedValue({ ok: true, vaultId: 'vault-1', fingerprint: 'fp' }),
...
describe('FoundingScreen ceremony', () => {
  it('cannot complete create until the downloaded kit is reselected and loss is acknowledged', async () => {
    ...
    expect(bridge.onComplete).toHaveBeenCalledTimes(1);
  });
