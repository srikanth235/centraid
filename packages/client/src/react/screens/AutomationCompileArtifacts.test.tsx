...
async function mount(
  ...
  await act(async () => {
    ...
    root.render(<AutomationCompileArtifacts source={source} file={file} onFile={onFile} />);
  });
  return container;
}

function tab(el: HTMLElement, name: string): HTMLButtonElement {
  const found = [...el.querySelectorAll<HTMLButtonElement>('[role="tab"]')].find(
...
describe('AutomationCompileArtifacts', () => {
  ...
  it('shows the manifest when the manifest tab is the selected file', async () => {
    const el = await mount({ handler: 'HANDLER', manifest: 'MANIFEST' }, 'manifest');
    expect(el.querySelector('[data-testid="compile-artifact"]')?.textContent).toContain('MANIFEST');
    expect(el.querySelector('[data-testid="compile-artifact"]')?.textContent).not.toContain(
      'HANDLER',
    );
    expect(tab(el, 'automation.json').getAttribute('aria-selected')).toBe('true');
  ...
  it('asks the owner to switch files rather than switching on its own', async () => {
    const onFile = vi.fn();
    const el = await mount({ handler: 'HANDLER', manifest: 'MANIFEST' }, 'handler', onFile);
    await act(async () => {
      tab(el, 'automation.json').dispatchEvent(new MouseEvent('click', { bubbles: true }));
    });
  ...
  it('disables Copy when there is nothing to copy, and copies the shown file when there is', async () => {
    ...
    const emptyCopy = [...empty.querySelectorAll<HTMLButtonElement>('button')].find((b) =>
    ...
    const copy = [...el.querySelectorAll<HTMLButtonElement>('button')].find((b) =>
      b.textContent?.includes('Copy'),
    );
