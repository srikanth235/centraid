...
function makeProps(
  ...
  return {
    ...
    onMatchSystem: vi.fn<SettingsAppearanceBridgeProps['onMatchSystem']>().mockReturnValue('dark'),
    ...over,
  };
}
...
describe('screens/SettingsAppearanceScreen', () => {
  ...
  describe(SettingsAppearanceScreen, () => {
    it('renders a theme card per preset, 5 accents, 4 tile treatments, and 4 preview tiles', () => {
      ...
      expect(el.querySelectorAll('.themeCard')).toHaveLength(THEME_PRESETS.length);
      ...
      // active accent
      expect(el.querySelectorAll('.swatch[data-active="true"]')).toHaveLength(1);
    });
