...
function makeProps(
  ...
  return {
    ...
    accent: "teal",
    tileVariant: "solid",
    onSetTheme: vi.fn<SettingsAppearanceBridgeProps["onSetTheme"]>(),
    onSetCoolCast: vi.fn<SettingsAppearanceBridgeProps["onSetCoolCast"]>(),
    onSetAccent: vi.fn<SettingsAppearanceBridgeProps["onSetAccent"]>(),
    onSetTile: vi.fn<SettingsAppearanceBridgeProps["onSetTile"]>(),
    onMatchSystem: vi
      .fn<SettingsAppearanceBridgeProps["onMatchSystem"]>()
...
describe("screens/SettingsAppearanceScreen", () => {
  ...
  describe(SettingsAppearanceScreen, () => {
    it("renders a theme card per preset, 5 accents, 4 tile treatments, and 4 preview tiles", () => {
      ...
      expect(el.querySelectorAll(".themeCard")).toHaveLength(
        THEME_PRESETS.length
      );
      expect(el.querySelectorAll(".swatch")).toHaveLength(5);
      expect(el.querySelectorAll(".previewTile")).toHaveLength(4);
      // active theme marked
      expect(
        (el.querySelector('.themeCard[data-name="light"]') as HTMLElement)
          .dataset.active
      ).toBe("true");
      // active accent
      expect(el.querySelectorAll('.swatch[data-active="true"]')).toHaveLength(
        1
      );
    });
    ...
    it("selects a theme card", () => {
      ...
      const card = el.querySelector(
        `.themeCard[data-name="${other.name}"]`
    ...
    it("applies match-system and cool-cast", () => {
      ...
      expect(props.onMatchSystem).toHaveBeenCalledOnce();
