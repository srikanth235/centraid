...
describe('multi-scope inline client', () => {
  ...
  it('refuses a scope that is not mounted rather than falling back', async () => {
    ...
    await expect(client.read({ query: 'library', scope: 'vault-nope' })).rejects.toMatchObject({
      code: 'UNKNOWN_SCOPE',
    });
  });
  ...
  it('addInlineScope ignores an object it did not build', () => {
    expect(addInlineScope({}, { scope: family, session: fakeSession() })).toBe(false);
  });
  ...
  it('the online-read fallback names the scope so the gateway cannot answer for another', async () => {
    ...
    const init = call[2];
    expect((init.headers as Record<string, string>)['x-centraid-vault']).toBe('vault-family');
  });
});
