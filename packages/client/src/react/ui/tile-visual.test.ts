...
describe(tileVisual, () => {
  ...
  it('defaults to the solid finish', () => {
    expect(tileVisual(app).finish).toStrictEqual(tileFinish(app.color, 'solid'));
  });

  it('threads the requested variant to tileFinish', () => {
    expect(tileVisual(app, 'gradient').finish).toStrictEqual(tileFinish(app.color, 'gradient'));
