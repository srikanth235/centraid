...
const { getAgentsStatus, getUserPrefs, saveUserPrefs } = vi.hoisted(() => ({
  getAgentsStatus: vi.fn<typeof import('../../../gateway-client.js').getAgentsStatus>(),
...
describe('settingsProvidersData suite', () => {
  ...
  it('renders one card per agent the gateway lists, in the gateway’s order', async () => {
    ...
    expect(dto.cards.map((c) => c.kind)).toStrictEqual(['codex', 'gemini', 'qwen']);
    expect(dto.cards.map((c) => c.title)).toStrictEqual(['Codex', 'Gemini CLI', 'Qwen Code']);
  });
  ...
  it('reads and writes ordered subsystem failover membership', async () => {
    ...
    const dto = await loadProviders();
    expect(dto.subsystemRunnerLadders.builder).toStrictEqual(['claude-code', 'gemini']);
