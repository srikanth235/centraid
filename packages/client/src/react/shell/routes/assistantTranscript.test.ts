...
describe(hydrateMessages, () => {
  ...
  it("groups consecutive tool rows into one tools message", () => {
    ...
    expect((msgs[0] as Extract<AsstMsg, { kind: "tools" }>).calls).toHaveLength(
      2
    );
  });
...
describe(msgToDTO, () => {
  ...
  it("flags canRegenerate only on the last answer that has a turn id", () => {
    ...
    expect(
      asLast.kind === "ai" && asLast.streaming === false && asLast.canRegenerate
    ).toBe(true);
    expect(
      notLast.kind === "ai" &&
        notLast.streaming === false &&
        notLast.canRegenerate
    ).toBeFalsy();
  });
  ...
  it("flags canRetry on an error bubble that remembers its failed text", () => {
    ...
    expect(dto.kind === "ai" && dto.streaming === false && dto.error).toBe(
      true
    );
    expect(dto.kind === "ai" && dto.streaming === false && dto.canRetry).toBe(
      true
    );
  });
