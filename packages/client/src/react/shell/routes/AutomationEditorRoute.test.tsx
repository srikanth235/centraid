...
vi.mock('../../screens/AutomationEditorScreen.js', () => ({
  default: (props: AutomationEditorBridgeProps) => {
    captured.props = props;
    return null;
  },
}));
vi.mock('./automationEditorData.js', () => ({
  loadAutomationEditorData: helpers.loadEditor,
}));
vi.mock('./automationEditorAgentData.js', () => ({
  buildAutomationAgentEditorData: helpers.buildAgentData,
}));
vi.mock('./automationEditorCreateData.js', () => ({
...
vi.mock('./settingsConnectionsData.js', () => ({
...
vi.mock('./settingsProvidersData.js', () => ({ loadProviders: helpers.loadProviders }));
vi.mock('../webhookReveal.js', () => ({ openWebhookReveal: helpers.openWebhookReveal }));
vi.mock('../../screens/SettingsConnectionsScreen.js', () => ({
...
beforeEach(() => {
  ...
  api.listVaultEntityTypes.mockReset().mockResolvedValue(['business.invoice', 'core.transaction']);
...
describe('AutomationEditorRoute', () => {
  it('loads edit data and drives every editor bridge action', async () => {
    ...
    expect(data.triggers.map((trigger) => trigger.kind)).toEqual([
      'webhook',
      'cron',
      'data',
      'condition',
    ...
    await expect(bridge.onReadSource()).resolves.toEqual({
      manifest: '{}',
  ...
  it('derives narrow trigger scopes and exact multi-account connector matches', () => {
    ...
    expect(matchEditorConnection(connections.slice(0, 1), 'github-cloud', 'github')).toMatchObject({
      match: connections[0],
    });
  });
});
