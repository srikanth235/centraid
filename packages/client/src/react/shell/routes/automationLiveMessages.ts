...
export function createAutomationLiveTrace(
  message = ""
...
function reduceLiveItem(
  ...
  if (event.type === "tool.start" || event.type === "tool.result") {
    ...
    if (prior === undefined)
      toolIndex.set(event.toolCallId, tools.push(call) - 1);
...
export function finishAutomationLiveItem(
  ...
  const stopReason =
    stopReasonIn(input.result) ?? stopReasonFromJson(input.rawJson);
...
export function automationLiveMessages(
  ...
  const messages: AsstMsgDTO[] = [];
  if (state.message)
    messages.push({ kind: "user", text: state.message, msgId: "inbound" });
