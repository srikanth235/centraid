...
vi.mock('../../../replica/shell-session.js', () => ({
  ...
  replicaIdentityForGatewayAuth: (gatewayAuth: { gatewayId?: string; vaultId?: string }) => ({
    gatewayId: gatewayAuth.gatewayId,
    vaultId: gatewayAuth.vaultId,
  }),
}));

const { MAX_MOUNTED_SCOPES, resolveAppScopes, scopeSetKey } = await import('./useAppScopes.js');
...
describe('resolveAppScopes', () => {
  ...
  it('caps how many scopes are hydrated at once', async () => {
    ...
    await expect(resolveAppScopes('photos')).resolves.toHaveLength(MAX_MOUNTED_SCOPES);
  });
