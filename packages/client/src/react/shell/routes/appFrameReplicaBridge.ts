...
export function attachAppFrameReplicaBridge(
  ...
  const startLegacyInvalidations = (expectedGeneration: number): void => {
    ...
    const starting = getSession()
      .then((session) => {
        if (!active || expectedGeneration !== generation || !port) return;
        legacyUnsubscribe = session.subscribe(
          appId,
          undefined,
          (invalidations) => {
            for (const invalidation of invalidations) {
              if (invalidation.source === "purge") {
                ...
                post(
                  { type: "centraid:vault-change", detail: invalidation },
                  expectedGeneration
                );
              }
            }
  ...
  const result = async (
    ...
    try {
      ...
      post(
        { type: "centraid:replica-result", id, ok: true, result: value },
        expectedGeneration
      );
    } catch (error) {
      const shaped = serializeReplicaError(error);
      post(
        { type: "centraid:replica-result", id, ok: false, ...shaped },
        expectedGeneration
      );
    }
  };
  ...
  const onPortMessage = (value: unknown, expectedGeneration: number): void => {
    if (
      expectedGeneration !== generation ||
      !isReplicaPortMessage(value, appId)
    )
      return;
    const message = value;
  ...
  const establishPort = (): void => {
    ...
    port.addEventListener("message", (event) =>
      onPortMessage(event.data, expectedGeneration)
    );
    port.start();
...
function isReadyWindowMessage(
  ...
  return (
    ...
    (message.type === "centraid:replica-ready" ||
      message.type === "centraid:changes-ready")
  );
}

function isReplicaPortMessage(
  value: unknown,
  appId: string
): value is ReplicaPortMessage {
  if (!value || typeof value !== "object") return false;
  const message = value as Record<string, unknown>;
  if (message.appId !== appId || typeof message.type !== "string") return false;
  if (
    message.type === "centraid:replica-read" ||
    message.type === "centraid:replica-search"
  ) {
    return (
      validId(message.id) &&
