...
vi.mock(import('../../../gateway-client.js') as Promise<unknown>, () => ({
  ...
  updateAppMeta: vi.fn<typeof import('../../../gateway-client.js').updateAppMeta>(),
  ...
  vaultParked: vi.fn<typeof import('../../../gateway-client.js').vaultParked>(async () => []),
  confirmVaultParked: vi.fn<typeof import('../../../gateway-client.js').confirmVaultParked>(),
}));
...
vi.mock(import('./templatesData.js') as Promise<unknown>, () => ({
  loadAppTemplates: vi.fn<typeof import('./templatesData.js').loadAppTemplates>(async () => []),
}));
vi.mock(import('./appSettingsData.js') as Promise<unknown>, () => ({
  fetchAppKnobValues: vi.fn<typeof import('./appSettingsData.js').fetchAppKnobValues>(
    async () => ({}),
  ),
  pushKnobToInlineRoot: vi.fn<typeof import('./appSettingsData.js').pushKnobToInlineRoot>(),
}));
...
} satisfies Pick<ReplicaShellSession, 'read' | 'search' | 'write' | 'subscribe'>;
...
describe('InlineAppRoute suite', () => {
  ...
  describe(InlineAppRoute, () => {
        ...
        useEffect(() => {
          void (
            window as unknown as {
              centraid: {
                read: (o: unknown) => Promise<{ open: Array<{ title: string }> }>;
              };
            }
          ).centraid
            .read({ query: 'board' })
    ...
    it('catches a failed chunk load and Retry re-imports + remounts', async () => {
      ...
      let calls = 0;
      const loader = vi.fn<() => Promise<{ default: InlineAppModule }>>(async () => {
