...
describe("automationTurnMessages cold projection", () => {
  it("coalesces only consecutive tools and preserves later agent/tool groups", () => {
    ...
    expect(messages.filter((message) => message.kind === "tools")).toHaveLength(
      2
    );
    expect(
      messages
        ...
        .map((message) =>
          message.kind === "ai" && !message.streaming ? message.copyText : ""
        )
...
describe("compile-turn inbound bubble (#541)", () => {
  ...
  it("agrees with the live seed on the same turn", () => {
    ...
    expect(
      automationTurnMessages(turn({ triggerKind: "compile" }), items)[0]
    ).toStrictEqual(
      expect.objectContaining({ text: COMPILE_TURN_INBOUND_TEXT })
    );
    expect(
      automationTurnInboundText(turn({ triggerKind: "manual" }), items)
    ).toBe(WORK_ORDER);
  });
