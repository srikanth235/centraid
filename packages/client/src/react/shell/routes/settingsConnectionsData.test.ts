...
const listConnectionProviders =
  vi.fn<typeof GatewayClient.listConnectionProviders>();
const listAutomations = vi.fn<typeof GatewayClient.listAutomations>(() =>
  Promise.resolve([])
);
...
const configureAssistConnection = vi.fn<
  typeof GatewayClient.configureAssistConnection
>(() =>
  Promise.resolve({
...
const beginConnectionAuthorization = vi.fn<
  typeof GatewayClient.beginConnectionAuthorization
>(() =>
  Promise.resolve({
...
vi.mock(import("../../../gateway-client.js"), () => ({
  ...
  oauthCallbackUri: () =>
    Promise.resolve("http://127.0.0.1:17832/centraid/_vault/oauth/callback"),
...
vi.mock(import("../../../assist-oauth-handoff.js"), () => ({
  completeAssistReturnLink:
    vi.fn<typeof AssistOAuthHandoff.completeAssistReturnLink>(),
}));

describe("settingsConnectionsData", () => {
  beforeEach(() => {
    window.CentraidApi = {
      getHostCapabilities: vi
        .fn<() => Promise<{ platform: "desktop" }>>()
        .mockResolvedValue({
          platform: "desktop",
        }),
  ...
  describe("settingsConnectionsData", () => {
    ...
    it("loadConnectionProvidersData passes the preset catalog through with capabilities", async () => {
      ...
      expect(
        providers[0]!.capabilities.syncs.some((s) => s.kind === "pull.github")
      ).toBe(true);
      expect(
        providers[0]!.capabilities.actions.some((a) =>
          a.toolName.includes("github")
        )
      ).toBe(true);
    });
    ...
    it("loadConnectorToolDescriptors only exposes healthy connections and never secrets", async () => {
      ...
      expect(
        tools.some((t) => t.toolName === "connector.pull_github.list")
      ).toBe(true);
      const json = JSON.stringify(tools);
      expect(json).not.toMatch(
        /api_key|access_token|client_secret|refresh_token/u
      );
    });
    ...
    describe(makeDetachConnection, () => {
      ...
      it("propagates a server refusal so the caller can toast the reason", async () => {
        ...
        const detach = makeDetachConnection(confirm);
        await expect(
          detach("c1", "pull.gmail", "Google · Gmail")
        ).rejects.toThrow(/awaiting a decision/u);
      });
    });
  });
});
