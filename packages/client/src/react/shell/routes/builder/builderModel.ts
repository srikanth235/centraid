...
export function toBuilderMsg(
  m: ConversationMsg,
  appVersionCount: number
): BuilderMsgDTO {
  if (m.kind === "divider") return { kind: "divider", text: m.text };
  if (m.kind === "status")
    return { kind: "status", text: m.text, spinning: !!m.spinning };
  ...
  if (m.kind === "toolGroup") {
    ...
    let change: { count: number; subtitle: string; version: string } | null =
      null;
    if (writes.length > 0) {
      const basenames = writes.map(
        (c) => (c.summary ?? "").split("/").pop() ?? ""
      );
...
export function parseVersionTime(versionId: string): number | undefined {
  const m = versionId.match(
    /^v_(?<date>\d{4}-\d{2}-\d{2})T(?<hour>\d{2})-(?<minute>\d{2})/u
  );
