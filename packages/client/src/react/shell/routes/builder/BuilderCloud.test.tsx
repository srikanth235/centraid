...
describe('BuilderCloud', () => {
  it('drives overview, logs, and every automation lifecycle action', async () => {
    ...
    expect(await vi.waitFor(() => el.textContent?.includes('invoice loaded'))).toBe(true);
    ...
    expect(await vi.waitFor(() => el.textContent?.includes('Invoice watcher'))).toBe(true);
    ...
    expect(await vi.waitFor(() => el.textContent?.includes('OK in 25ms'))).toBe(true);
  ...
  it('renders empty and failed fetch states without leaking raw exceptions', async () => {
    ...
    await click(button(el, 'Logs'));
    expect(await vi.waitFor(() => el.textContent?.includes('Could not load logs.'))).toBe(true);
    expect(el.textContent).toContain('logs unavailable');

    await click(button(el, 'Automations'));
    expect(await vi.waitFor(() => el.textContent?.includes('Could not load automations.'))).toBe(
      true,
    );
