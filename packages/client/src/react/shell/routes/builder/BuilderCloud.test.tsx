  ...
  describe("BuilderCloud", () => {
    it("drives overview, logs, and every automation lifecycle action", async () => {
      ...
      expect(window.open).toHaveBeenCalledWith(
        "https://gateway.test/centraid/invoices/",
        "_blank"
      );
      ...
      await expect(
        vi.waitFor(() => el.textContent?.includes("invoice loaded"))
      ).resolves.toBe(true);
      expect(
        el.querySelectorAll('[data-testid="cloud-logs-row"]')
      ).toHaveLength(2);
      ...
      await expect(
        vi.waitFor(() => el.textContent?.includes("Invoice watcher"))
      ).resolves.toBe(true);
      ...
      await expect(
        vi.waitFor(() => el.textContent?.includes("OK in 25ms"))
      ).resolves.toBe(true);
    ...
    it("renders empty and failed fetch states without leaking raw exceptions", async () => {
      ...
      await expect(
        vi.waitFor(() =>
          el.textContent?.includes("Could not load automations.")
        )
      ).resolves.toBe(true);
