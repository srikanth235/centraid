...
describe('Sidebar suite', () => {
  ...
  describe(Sidebar, () => {
    ...
    it('groups Gateway and Storage under an Operations section', () => {
      const el = render(<Sidebar {...base} onGateway={() => {}} onStorage={() => {}} />);
      ...
      expect(
        gateway.compareDocumentPosition(storage) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    });
    ...
    it('fires onStorage and highlights the Storage item on its route', () => {
      ...
      expect(el.querySelector('[data-active="true"]')?.textContent).toContain('Storage');
    });
    ...
    it('disables Storage when no handler is provided, and keeps the Gateway pill to itself', () => {
      ...
      const items = [...el.querySelectorAll('.sbItem')];
      const storage = items.find((b) => b.textContent?.includes('Storage')) as HTMLButtonElement;
    ...
    it('renders the head slot above Build new — the profile switcher leads the column', () => {
      ...
      expect(
        head.compareDocumentPosition(buildNew) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    });
    ...
    it('shows the relaunch pill with the new version and fires the handler', () => {
      const onRelaunchToUpdate = vi.fn<NonNullable<SidebarProps['onRelaunchToUpdate']>>();
      const el = render(
    ...
    it('renders the relaunch pill above Settings, below the stretch spacer', () => {
      ...
      expect(
        pill.compareDocumentPosition(settings) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    });
  });
});
