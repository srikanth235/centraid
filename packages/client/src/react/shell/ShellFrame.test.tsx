...
describe("shell/ShellFrame", () => {
  ...
  describe(ShellFrame, () => {
    it("renders the window grid with sidebar + main content", () => {
      ...
      expect(el.querySelector<HTMLElement>(".tlMain")?.dataset.layout).toBe(
        "flat"
      );
    });
    ...
    it("marks the grid closed and shows the collapsed sidebar toggle in tlMain", () => {
      ...
      expect(
        tlMain.querySelector('[aria-label="Show sidebar"]')
      ).not.toBeNull();
    });
    ...
    it("fires nav callbacks", () => {
      ...
      const onToggleSidebar = vi.fn<ShellFrameProps["onToggleSidebar"]>();
      const el = render(
        <ShellFrame
          {...base}
          canGoBack
      ...
      // These fire straight off a native button `onClick`, so the callback
      // receives the click SyntheticEvent, not a bare invocation.
      expect(onBack).toHaveBeenCalledWith(
        expect.objectContaining({ type: "click" })
      );
      expect(onToggleSidebar).toHaveBeenCalledWith(
        expect.objectContaining({ type: "click" })
      );
    });
    ...
    it("shows the New app pencil only when collapsed + showNewChat", () => {
      ...
      host?.remove();
      const closed = render(
        <ShellFrame {...base} sidebarOpen={false} showNewChat />
      );
