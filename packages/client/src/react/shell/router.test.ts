...
describe("shell router", () => {
  ...
  it("keys parameterized routes by their identity", () => {
    expect(routeKey({ kind: "run-view", automationId: "a", runId: "r1" })).toBe(
      "run-view:r1"
    );
    expect(routeKey({ kind: "automation-view", automationId: "a2" })).toBe(
      "automation-view:a2"
    );
    ...
    expect(routeKey({ kind: "automation-editor" })).toBe(
      "automation-editor:new"
    );
