...
export function openAutomationTemplatePreview(
  ...
  const trigIcon = iconSvg(
    template.triggerKind === "webhook" ? "Webhook" : "Clock",
    14
  );
  ...
  trig.append(
    trigIco,
    document.createTextNode(template.triggerLabel ?? "Manual")
  );
  ...
  if (integrations.length > 0) {
    ...
    for (const name of integrations) {
      ...
      const hue = (INTEGRATION_HUES as Record<string, string>)[
        name.toLowerCase()
      ];
