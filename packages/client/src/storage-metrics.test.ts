...
function input(
  overrides: Partial<StorageMetricsInput> = {}
): StorageMetricsInput {
  return {
    now: NOW,
...
describe("storage-metrics", () => {
  describe("freshness", () => {
    ...
    it("echoes the four input clocks for the diagnostics disclosure", () => {
      ...
      const m = deriveStorageMetrics(
        input({ freshness: { declaredCadenceMs: DAY, clocks: c } })
      );
      expect(m.freshness.clocks).toStrictEqual(c);
    ...
    describe("threshold edges", () => {
      const at = (
        ageMs: number
      ): ReturnType<typeof deriveStorageMetrics>["freshness"] =>
  ...
  describe("recovery window", () => {
    ...
    it("is null when the provider promises no retention", () => {
      ...
      expect(m.recoveryWindow.retention).toStrictEqual({ kind: "none" });
    });
  });
  ...
  describe("privacy", () => {
    it("is a structural constant — sealed bytes, zero key custody", () => {
      ...
      expect(m.privacy.description).toBeTypeOf("string");
    });
  });
  ...
  describe("exit", () => {
    it("export is always available and restoreCostClass passes through honestly", () => {
      const free = deriveStorageMetrics(
        input({ restoreCostClass: "free-egress" })
      );
      expect(free.exit).toStrictEqual({
        exportAlwaysAvailable: true,
        restoreCostClass: "free-egress",
      });
      const metered = deriveStorageMetrics(
        input({ restoreCostClass: "metered-egress" })
      );
  ...
  it("is pure — same input yields deeply-equal output", () => {
    const i = input();
    expect(deriveStorageMetrics(i)).toStrictEqual(deriveStorageMetrics(i));
  });
});
