...
describe(tokenize, () => {
  it("wraps a JS keyword in a tok-key span", () => {
    expect(tokenize("const x = 1", "js")).toContain(
...
describe(shortVersionTitle, () => {
  ...
  it("parses the embedded date-time out of a generated version id", () => {
    expect(
      shortVersionTitle({ versionId: "v_2026-05-08T14-30-00-000Z_a1b2c3" })
    ).toBe("2026-05-08 14-30");
  });

  it("falls back to a 24-char prefix for an unrecognised id", () => {
    expect(
      shortVersionTitle({ versionId: "manual-tag-without-timestamp-xyz" })
    ).toBe("manual-tag-without-times");
  });
});
