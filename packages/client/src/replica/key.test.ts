...
describe("replica storage keys", () => {
  test("keys stable gateway identities and isolates every vault", async () => {
    const first = await replicaStorageKey({
      gatewayId: "gateway-one",
      vaultId: "one",
    });
    const otherGateway = await replicaStorageKey({
    ...
    await expect(
      replicaDatabaseName({ gatewayId: "gateway-one", vaultId: "one" })
    ).resolves.toBe(`/centraid-replica-${first}.sqlite3`);
  });
});
