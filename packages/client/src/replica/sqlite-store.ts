...
const LOCAL_REPLICA_SCHEMA_VERSION = 3;
...
export class SqliteReplicaStore {
  ...
  /** Shape metadata survives OPFS reopen and rebuilds the app/entity lookup offline. */
  ...
  private initializeSchema(): void {
    ...
    this.transaction(() => {
      ...
      `);
      this.db.exec(DDL);
      this.db.exec(`PRAGMA user_version = ${LOCAL_REPLICA_SCHEMA_VERSION};`);
    });
  }
  ...
  private validateSnapshot(snapshot: ReplicaSnapshot): void {
    if (snapshot.protocolVersion !== REPLICA_PROTOCOL_VERSION) {
      throw new ReplicaRebootstrapRequiredError('protocol-mismatch');
    }
    if (snapshot.vaultId !== this.expectedVaultId) {
      throw new ReplicaRebootstrapRequiredError('vault-mismatch');
    }
    if (!snapshot.schemaEpoch) throw new ReplicaProtocolError('Schema epoch is required');
  ...
  private run(sql: string, bind: BindingSpec = []): void {
    const statement = this.db.prepare(sql);
    try {
      if (Array.isArray(bind) ? bind.length > 0 : true) statement.bind(bind);
      statement.step();
    } finally {
      statement.finalize();
    }
  ...
  private all<T extends object>(sql: string, bind: BindingSpec = []): T[] {
    return this.db.exec({ sql, bind, rowMode: 'object', returnValue: 'resultRows' }) as T[];
  }

  private one<T extends object>(sql: string, bind: BindingSpec = []): T | undefined {
