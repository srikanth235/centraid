    ...
    async activate(identity) {
      ...
      try {
        ...
        const existing = (await requestResult(
          store.get(identityKey(identity))
        )) as ReplicaIdentityInventoryEntry | undefined;
    ...
    async deferTerminal(identity, failedAt, baseDelayMs, maxDelayMs) {
      ...
      try {
        ...
        const delay = Math.min(
          baseDelayMs * 2 ** Math.min(attempts - 1, 10),
          maxDelayMs
        );
        store.put(
          inventoryRow(identity, "terminal-pending", attempts, failedAt + delay)
        );
        await transactionDone(tx);
      } finally {
        db.close();
