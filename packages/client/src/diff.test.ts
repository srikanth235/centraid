...
describe(lineDiff, () => {
  ...
  it('keeps the common subsequence maximal across interleaved edits', () => {
    ...
    expect(rows.filter((r) => r.type === 'same').map((r) => r.text)).toStrictEqual(['a', 'c', 'd']);
    expect(rows.filter((r) => r.type === 'del').map((r) => r.text)).toStrictEqual(['b']);
    expect(rows.filter((r) => r.type === 'add').map((r) => r.text)).toStrictEqual(['x', 'y']);
  });

  it('preserves line ordering and renumbers both sides independently', () => {
    const rows = lineDiff('keep\ndrop', 'keep\nadd1\nadd2');
    expect(rows.map((r) => [r.type, r.aNum, r.bNum])).toStrictEqual([
      ['same', 1, 1],
      ['del', 2, undefined],
      ['add', undefined, 2],
      ['add', undefined, 3],
