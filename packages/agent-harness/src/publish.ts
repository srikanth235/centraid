...
/**
 * Build the app (unless `skipBuild`), then tarball + upload to the gateway.
 *
...
export async function publishApp(
  ...
  if (!(await dirExists(appDir))) {
    throw new HarnessError('no_app', `App directory not found: ${appDir}`);
  }

  if (!options.skipBuild) {
    await runBuild(appDir, options.buildCommand);
  }
  ...
  await assertManifestValid(appDir);

  const tarStream = tar.create(
    {
      gzip: true,
      cwd: appDir,
      // Preserve relative paths; refuse anything outside the app dir.
      preservePaths: false,
      filter: (relPath) => {
...
async function runBuild(appDir: string, override?: PublishOptions['buildCommand']): Promise<void> {
  const cmd = override ?? (await pickBuildCommand(appDir));
...
async function pickBuildCommand(
  ...
  if (await fileExists(path.join(appDir, 'tsconfig.json'))) {
...
/**
 * Read `app.json` from the app, parse it through the app-engine
 ...
 */
async function assertManifestValid(appDir: string): Promise<void> {
  ...
  for (const a of manifest.actions) {
    const file = path.join(appDir, 'actions', `${a.name}.js`);
    if (!(await fileExists(file))) {
      throw new HarnessError(
        'invalid_manifest',
        `app.json declares action "${a.name}" but ${path.relative(appDir, file)} does not exist`,
      );
    }
  }
  for (const q of manifest.queries) {
    const file = path.join(appDir, 'queries', `${q.name}.js`);
    if (!(await fileExists(file))) {
      throw new HarnessError(
        'invalid_manifest',
        `app.json declares query "${q.name}" but ${path.relative(appDir, file)} does not exist`,
      );
    }
  }
}
