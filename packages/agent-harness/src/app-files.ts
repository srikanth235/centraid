...
export interface AppFile {
  /** Path relative to the app root, posix style (e.g. "queries/foo.ts"). */
  path: string;
  content: string;
  /** UTF-8 byte length. */
  size: number;
...
export type AppFileLanguage = 'ts' | 'js' | 'html' | 'css' | 'json' | 'md' | 'other';
...
/**
 * Read every text file in an app folder for the desktop's code viewer.
 * Skips build artifacts, hidden files, and binaries by extension. Returns a
 * stable list sorted by path.
 */
export async function readAppFiles(appDir: string): Promise<AppFile[]> {
  const out: AppFile[] = [];
...
async function walk(root: string, rel: string, out: AppFile[]): Promise<void> {
...
/**
 * Write a single text file inside an app folder, for the desktop's
 * editable code workspace (§B5). Guards against path traversal — the
 * resolved target must stay inside `appDir` — and rejects extensions
...
export async function writeAppFile(
  ...
  if (abs !== root && !abs.startsWith(root + path.sep)) {
    throw new Error(`Refusing to write outside the app: ${relPath}`);
  }
...
function languageOf(ext: string): AppFileLanguage {
  switch (ext) {
    case '.ts':
      return 'ts';
    case '.js':
