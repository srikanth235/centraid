...
beforeEach(async () => {
  ...
  projectDir = path.join(tmpRoot, 'sample');
  ...
  await fs.writeFile(path.join(projectDir, 'queries', 'ping.js'), 'export default ()=>({ok:true})');
  // Things that should be filtered out:
  await fs.writeFile(path.join(projectDir, '.env'), 'SECRET=nope\n');
  await fs.mkdir(path.join(projectDir, 'node_modules', 'junk'), { recursive: true });
  await fs.writeFile(path.join(projectDir, 'node_modules', 'junk', 'a.js'), '// junk');
  await fs.mkdir(path.join(projectDir, '.git'));
  await fs.writeFile(path.join(projectDir, '.git', 'HEAD'), 'ref: refs/heads/main');
...
test('publishProject ships a non-empty gzip tar (regression: cwd marker not filtered out)', async () => {
  await publishProject(
    projectDir,
    'sample',
    { projectsDir: tmpRoot, gatewayUrl: 'http://127.0.0.1:1', gatewayToken: '' },
    { skipBuild: true },
  );
