...
export async function prepareBundledIndex(
  ...
  async function rewriteNextScript(index: number): Promise<void> {
    ...
    if (!type || type.toLowerCase() !== 'module') return rewriteNextScript(index + 1);
    ...
    if (bundle) out = out.replace(tag, tag.replace(src!, `./_bundle.${bundle.hash}.js`));
    return rewriteNextScript(index + 1);
  }
  ...
  const inlined = (
    await Promise.all(
      links.map(async (m): Promise<{ tag: string; css: string } | undefined> => {
        const tag = m[0];
        const relAttr = attrOf(tag, 'rel');
        if (!relAttr || relAttr.toLowerCase() !== 'stylesheet') return undefined;
  ...
  ).filter((entry): entry is { tag: string; css: string } => entry !== undefined);
  if (inlined.length > 0) {
