...
describe("conversation workspace parsing", () => {
  ...
  it("persists canonical, deduplicated non-root additional directories", async () => {
    ...
    await expect(
      parseAdditionalDirectories([target, alias])
    ).resolves.toStrictEqual([await fs.realpath(target)]);
  });
  ...
  it("rejects relative paths, files, and the filesystem root", async () => {
    ...
    await expect(parseAdditionalDirectories(["relative"])).rejects.toThrow(
      /absolute/u
    );
    await expect(parseAdditionalDirectories([file])).rejects.toThrow(
      /non-root directory/u
    );
    await expect(
      parseAdditionalDirectories([path.parse(dir).root])
    ).rejects.toThrow(/non-root directory/u);
  });
});
