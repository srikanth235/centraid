...
describe('query-bundle', () => {
  ...
  describe('query-only browser bundles', () => {
    ...
    it('returns stable hashes and ETags, 304s matches, and invalidates on source edits', async () => {
      ...
      await expect(evaluateDefault(code, edited.headers.get('etag')!)).resolves.toStrictEqual({
    ...
    it('resolves live and draft query graphs through their respective code directories', async () => {
      ...
      await expect(evaluateDefault(liveCode, live.headers.get('etag')!)).resolves.toStrictEqual({
        value: 'live-v1',
      });
      await expect(evaluateDefault(draftCode, draft.headers.get('etag')!)).resolves.toStrictEqual({
        value: 'draft-v1',
      });
