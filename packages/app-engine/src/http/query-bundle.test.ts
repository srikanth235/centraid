...
async function writeCodeDir(directory: string, value: string): Promise<void> {
  ...
  await fs.writeFile(
    path.join(directory, "queries", "broken.js"),
...
describe("query-bundle", () => {
  beforeEach(async () => {
    ...
    runtime = new Runtime({
      ...
      codeDirOverride: async (appId) =>
        appId === "demo" ? liveDir : undefined,
  ...
  describe("query-only browser bundles", () => {
    it("serves a directly importable declared query with its helper", async () => {
      ...
      expect(response.headers.get(QUERY_SOURCE_HASH_HEADER)).toMatch(
        /^[0-9a-f]{64}$/u
      );
      ...
      await expect(
        evaluateDefault(code, response.headers.get("etag")!)
    ...
    it("keeps action sources inaccessible and refuses query imports that leave queries/", async () => {
      ...
      expect((await request("/centraid/demo/actions/mutate.js")).status).toBe(
        404
      );
      expect((await request("/centraid/demo/queries/list.js")).status).toBe(
        404
      );
    ...
    it("rejects undeclared and traversal-shaped query names with typed errors", async () => {
      const undeclared = await request(
        "/centraid/demo/_query/not-declared.mjs"
      );
      ...
      const traversal = await request(
        "/centraid/demo/_query/%2E%2E%2Faction%2Fmutate.mjs"
      );
    ...
    it("returns stable hashes and ETags, 304s matches, and invalidates on source edits", async () => {
      ...
      expect(revalidated.headers.get(QUERY_SOURCE_HASH_HEADER)).toBe(
        firstSourceHash
      );
      ...
      expect(edited.headers.get(QUERY_SOURCE_HASH_HEADER)).not.toBe(
        firstSourceHash
      );
      const code = await edited.text();
      await expect(
        evaluateDefault(code, edited.headers.get("etag")!)
      ).resolves.toStrictEqual({
    ...
    it("resolves live and draft query graphs through their respective code directories", async () => {
      ...
      const draft = await request(
        "/centraid/_draft/session-1/demo/_query/list.mjs"
      );
      ...
      await expect(
        evaluateDefault(liveCode, live.headers.get("etag")!)
      ).resolves.toStrictEqual({
        value: "live-v1",
      });
      await expect(
        evaluateDefault(draftCode, draft.headers.get("etag")!)
      ).resolves.toStrictEqual({
        value: "draft-v1",
      });
