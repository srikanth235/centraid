...
function newApp(): string {
  return newDir({
    ...
    "jsx-runtime.js":
      "export { Fragment, jsx, jsxs } from './react-runtime.js';\n",
    "app.jsx": [
...
describe("app-bundle", () => {
  ...
  describe("live index.html — bundle rewrite + CSS inlining", () => {
    it("rewrites the module entry to a hashed bundle URL and inlines every stylesheet", async () => {
      ...
      expect(
        m,
        "entry <script> was not rewritten to a bundle URL"
      ).toBeTruthy();
      ...
      const styleBlocks =
        html.match(/<style data-centraid-inlined-css>/gu) ?? [];
      ...
      expect(html.indexOf("display: grid")).toBeLessThan(
        html.indexOf("color: teal")
      );
    });
  ...
  describe("bundle serving — ETag, immutability, 304", () => {
    it("prebuilds the bundle and both wire encodings before the first request", async () => {
      ...
      await expect(prewarmAppAssets(app, shared)).resolves.toStrictEqual({
        bundles: 1,
        variants: 2,
      });

      const html = (
        await serve(app, "index.html", { sharedAssetsDir: shared })
      ).body.toString();
    ...
    it("serves the bundle with a sha256 ETag, immutable cache-control, and honors If-None-Match", async () => {
      ...
      expect(data.headers["Cache-Control"]).toBe(
  ...
  describe("TypeScript entry + CSS-module in the whole-graph bundle", () => {
    ...
    it("bundles a .tsx entry importing a .module.css: hashed class name + injected CSS present, no second output dropped", async () => {
      ...
      const body = (await serve(app, `_bundle.${m![1]}.js`)).body.toString(
        "utf8"
      );
    ...
    it("re-keys the bundle when the .module.css changes (module CSS invalidates the manifest)", async () => {
      ...
      const hash1 = BUNDLE_URL_RE.exec(
        (await serve(app, "index.html")).body.toString()
      )![1]!;
      ...
      const hash2 = BUNDLE_URL_RE.exec(
        (await serve(app, "index.html")).body.toString()
      )![1]!;
      expect(hash2).not.toBe(hash1);
  ...
  async function countBootRequests(
    ...
    const pending = queue.map((u) =>
      resolveRel("index.html", u.replace(/^\.\//u, ""))
    );
    async function serveNextModule(): Promise<void> {
      const rel = pending.pop()!;
      if (!rel || seen.has(rel))
        return pending.length > 0 ? serveNextModule() : undefined;
  ...
  describe("request count over a served app", () => {
    ...
    it.skipIf(!existsSync(photosDir) || !existsSync(kitDir))(
      ...
      async () => {
        ...
        expect(after).toBe(1);
        await expect(
          prewarmAppAssets(photosDir, kitDir)
