...
const entryArb: fc.Arbitrary<PricingEntry> = fc.record({
  input_cost_per_token: fc.option(
    fc.double({ min: 0, max: 1e-3, noNaN: true }),
    {
      nil: undefined,
    }
  ),
  output_cost_per_token: fc.option(
    fc.double({ min: 0, max: 1e-3, noNaN: true }),
    {
      nil: undefined,
    }
  ),
  cache_read_input_token_cost: fc.option(
    fc.double({ min: 0, max: 1e-3, noNaN: true }),
    {
      nil: undefined,
    }
  ),
  cache_creation_input_token_cost: fc.option(
    fc.double({ min: 0, max: 1e-3, noNaN: true }),
    {
      nil: undefined,
    }
  ...
  test("missing rate fields contribute zero for that bucket", () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 10_000 }), (tokens) => {
        expect(
          costFromEntry({}, { inputTokens: tokens, outputTokens: tokens })
        ).toBe(0);
      }),
  ...
  test("entryToModelPrice scales per-token rates by 1e6", () => {
    fc.assert(
      fc.property(entryArb, (entry) => {
        ...
        expect(price.inputPerMtok).toBeCloseTo(
          (entry.input_cost_per_token ?? 0) * 1_000_000,
          8
        );
        expect(price.outputPerMtok).toBeCloseTo(
          (entry.output_cost_per_token ?? 0) * 1_000_000,
          8
        );
