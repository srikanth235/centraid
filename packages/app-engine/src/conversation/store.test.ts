...
describe("ConversationStore — conversations", () => {
  ...
  it("ensureAutomationConversation reuses one conversation and refreshes its name", () => {
    const store = newStore();
    const first = store.ensureAutomationConversation(
      "app/digest",
      "app",
      "Digest"
    );
    const second = store.ensureAutomationConversation(
      "app/digest",
      "app",
      "Morning digest"
    );
  ...
  it("keeps one durable conversation when an automation switches harness", () => {
    const store = newStore();
    const codex = store.ensureAutomationConversation(
      "app/digest",
      "app",
      "Digest",
      "codex"
    );
    const claude = store.ensureAutomationConversation(
      "app/digest",
      "app",
      "Digest",
      "claude-code"
...
describe("ConversationStore — search / pin / archive (issue #420)", () => {
  ...
  it("searchConversations matches on title and on inbound message text with a snippet", () => {
    const store = newStore();
    const budgetId = seedChat(
      store,
      "u1",
      "Budget review",
      "help me plan the quarterly budget"
    );
  ...
  it("search is prefix-based, user-scoped, and skips archived threads", () => {
    ...
    expect(
      store.searchConversations("u1", "trav").map((h) => h.id)
    ).toStrictEqual([mine]);
  ...
  it("search reflects a renamed title and a blank query returns nothing", () => {
    ...
    expect(
      store.searchConversations("u1", "groceries").map((h) => h.id)
    ).toStrictEqual([id]);
  ...
  it("the FTS index survives a store reopen and backfills pre-existing rows", () => {
    ...
    expect(
      s2.searchConversations("u1", "needle").map((h) => h.id)
    ).toStrictEqual([id]);
    s2.close();
  });
});
...
describe("ConversationStore — turns", () => {
  ...
  it("renews a turn-lock lease only for its current owner", () => {
    ...
    expect(store.acquireTurnLock(conversation.id, "owner-a", startedAt)).toBe(
      true
    );
    expect(store.refreshTurnLock(conversation.id, "owner-a", refreshedAt)).toBe(
      true
    );
    expect(
      store.acquireTurnLock(conversation.id, "owner-b", startedAt + 31 * 60_000)
    ).toBe(false);
    expect(
      store.acquireTurnLock(
        conversation.id,
        "owner-b",
        refreshedAt + 30 * 60_000 + 1
      )
    ).toBe(true);
    expect(
...
describe("ConversationStore — automation state", () => {
  it("get/set round-trips across reopens and is scoped per automation", () => {
    ...
    s1.close();
    const s2 = new ConversationStore(provider);
    expect(s2.stateGet("auto-foo", "cursor")?.valueJson).toBe(
      JSON.stringify({ since: 42 })
    );
    expect(s2.stateGet("auto-bar", "cursor")?.valueJson).toBe(
      JSON.stringify("B")
    );
