...
function compileTurns(
  ...
  for (const message of messages) {
    ...
    if (payload.kind === 'user') {
      ...
      if (includeAttachmentReferences && Array.isArray(payload.attachments)) {
        const names = payload.attachments.flatMap((attachment) => {
          ...
          if (typeof record.filename === 'string' && typeof record.mime === 'string') {
            return [`${record.filename} (${record.mime})`];
          }
          if (typeof record.filename === 'string') return [record.filename];
          if (typeof record.mime === 'string') return [`attachment (${record.mime})`];
          return ['attachment'];
        });
    ...
    if (payload.kind === 'ai') {
      lines.push(`Assistant: ${typeof payload.text === 'string' ? payload.text : ''}`);
      flush();
    }
  }
  flush();
...
export function compileHydrationPlan(
  ...
  const turns = compileTurns(messages, options.includeAttachmentReferences ?? false);
  const omittedPrefix = Math.max(0, turns.length - Math.min(minTurns, turns.length));
  const headerFor = (omittedTurns: number): string =>
    [
      ...
      omittedTurns > 0 ? `${omittedTurns} older turn(s) omitted to fit the context budget.` : '',
    ]
      .filter(Boolean)
      .join('\n');
  ...
  for (let index = turns.length - 1; index >= 0; index -= 1) {
    ...
    if (selected.length >= minTurns && estimatedTokens + turn.estimatedTokens > bodyBudget) break;
  ...
  const body = selected
    .map((turn, index) => [`Turn ${omittedTurns + index + 1}:`, ...turn.lines].join('\n'))
    .join('\n\n');
