...
describe.skipIf(process.env.CENTRAID_RUN_NATIVE_TUNNEL !== "1")(
  "native gateway relay",
  () => {
    let server: http.Server;
    let endpoint: GatewayEndpointHandle;
    let client: TunnelClient;
    const boxedAuthorize = vi.fn<GatewayEndpointOptions["authorize"]>(
      () => false
    );
      ...
      server = http.createServer((req, res) => {
        void (async () => {
          ...
          if (url.pathname.startsWith("/centraid/_gateway/tunnel/")) {
            if (
              req.headers["x-centraid-data-plane-secret"] !== CONTROL_SECRET
            ) {
