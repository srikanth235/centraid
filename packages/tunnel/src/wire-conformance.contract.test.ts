...
function buildFixture(): GoldenFixture {
  ...
  return {
    ...
    alpns: {
      ...
      gwPairBytesBase64: Buffer.from(alpnBytes(GW_PAIR_ALPN)).toString(
        "base64"
      ),
    },
    caps: {
...
describe("framing bounds", () => {
  it("rejects a header frame whose length prefix exceeds the cap", async () => {
    ...
    await expect(readHeaderFrame(bufferRecv(prefix))).rejects.toThrow(
      /out of bounds/u
    );
  });
  ...
  it("rejects a zero-length header frame on read", async () => {
    ...
    await expect(readHeaderFrame(bufferRecv(zero))).rejects.toThrow(
      /out of bounds/u
    );
  });
});
