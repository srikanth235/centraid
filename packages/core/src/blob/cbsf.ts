...
export function decodeCbsfDirectory(
  ...
  if (bytes.byteLength !== 16 + frameCount * 4)
    throw new Error("CBSF directory size mismatch");
...
export function encodeCbsfDirectory(
  ...
  for (const [index, length] of sealedLens.entries())
    view.setUint32(16 + index * 4, length, false);
  return bytes;
}
