...
const FILING_SCHEMA = {
  ...
  properties: {
    title: {
      type: "string",
      description:
        'A clear human title for this document, e.g. "Home insurance policy 2026".',
    },
    folder: {
      type: "string",
      description:
        "The folder to file it in — prefer one of the existing folders when it fits.",
    },
    doctype: {
      type: "string",
      description:
        'A short document-type label, e.g. "invoice", "policy", "warranty".',
    },
...
export default async function handler({ ctx, log }) {
  ...
  if (derivatives.length === 0)
    return { summary: "no newly readable documents to file" };
  ...
  const folderLabels = folderScheme
    ? (folders.rows ?? [])
        .filter(
          (c) => c.scheme_id === folderScheme.scheme_id && c.notation !== "root"
        )
        .map((c) => c.pref_label)
    : [];
  ...
  for (const derivative of derivatives) {
    ...
    if (!out || typeof out.title !== "string" || typeof out.folder !== "string")
      continue;
    rows.push({
