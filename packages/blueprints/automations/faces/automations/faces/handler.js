...
async function deriveAsset(ctx, asset, model) {
  ...
  const response = await ctx.fetch({
    ...
    content: [
      { contentId: asset.content_id, variant: "preview", maxBytes: 8388608 },
    ],
  });
...
export default async function handler({ ctx }) {
  ...
  if (!model)
    return { summary: "faces skipped — deterministic service unavailable" };
  ...
  for (const request of requests.rows ?? []) {
    ...
    // A target-less owner request is vault-wide consent, not ambient work.
    // Its private cursor drains the live photo library in bounded passes and
    // the request remains open until the consent-scoped walk reaches the end.
  ...
  // Past face stamps are durable evidence of consent. A model bump walks only
  // those stamped targets; it never opens an ambient library scan.
  if (remaining > 0) {
