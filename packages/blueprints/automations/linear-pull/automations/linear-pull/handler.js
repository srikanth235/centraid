...
async function gql(ctx, query, variables) {
  ...
  if (res.status !== 200) {
    throw new Error(
      "linear graphql answered " + res.status + ": " + res.text.slice(0, 200)
    );
  }
  const body = JSON.parse(res.text);
  if (body.errors && body.errors.length) {
    throw new Error(
      "linear graphql error: " + JSON.stringify(body.errors[0]).slice(0, 200)
    );
  }
  return body.data;
}
...
export default {
  ...
  async principal({ ctx }) {
    ...
    return (
      (viewer.viewer && (viewer.viewer.email || viewer.viewer.name)) || "linear"
    );
  },
