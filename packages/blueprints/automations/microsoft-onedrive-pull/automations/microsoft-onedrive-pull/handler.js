...
function toRow(item) {
  ...
  return {
    ...
    payload: {
      ...
      mediaType:
        (item.file && item.file.mimeType) || "application/octet-stream",
  ...
  async pull({ ctx, cursor }) {
    ...
    for (let page = 0; page < MAX_PAGES_PER_RUN; page += 1) {
      const listing = await api(
        ctx,
        nextLink || "/me/drive/root/delta?$top=50"
      );
      if (listing.gone) {
        if (resetTried)
          throw new Error(`${KIND} delta token remained invalid after reset`);
        traversal.clear();
        nextLink = null;
        resetTried = true;
        page -= 1;
      ...
      nextLink =
        listing["@odata.nextLink"] || listing["@odata.deltaLink"] || null;
