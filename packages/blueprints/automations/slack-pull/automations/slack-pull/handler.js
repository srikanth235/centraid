...
export default {
  ...
  async pull({ ctx, log, cursor }) {
    ...
    for (let page = 0; page < MAX_PAGES_PER_RUN; page += 1) {
      const listing = await api(
        ...
        "/conversations.list?types=im,mpim,public_channel,private_channel&limit=100&exclude_archived=true" +
          (channelsCursor
            ? "&cursor=" + encodeURIComponent(String(channelsCursor))
            : "")
      );
      if (listing.ok === false) {
        throw new Error(
          "slack conversations.list failed: " + (listing.error || "unknown")
        );
      }
      channels.push(...(listing.channels || []));
      channelsCursor =
        (listing.response_metadata && listing.response_metadata.next_cursor) ||
        null;
    ...
    for (const channel of channels) {
      ...
      const historyTraversal = cursor.provider(
        "slack.history_cursor." + channel.id
      );
      ...
      for (let page = 0; page < MAX_PAGES_PER_RUN; page += 1) {
        const history = await api(
          ...
          "/conversations.history?channel=" +
            ...
            (historyCursor
              ? "&cursor=" + encodeURIComponent(String(historyCursor))
              : "")
        );
        if (history.ok === false) {
          log.warn(
