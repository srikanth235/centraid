...
async function loadPull(id: string): Promise<PullSpec> {
  ...
  const loaded = (await import(
    `${pathToFileURL(handler).href}?test=${id}`
  )) as {
    default: PullSpec;
  };
    ...
    return {
      ...
      observe(value) {
        ...
        if (
          ...
          (previous === undefined || comparable > previous)
        ) {
          next = value;
          updates.set(key, value);
        }
...
describe("bundled pull handler correctness", () => {
  ...
  it("drains GitLab pages with independent issue and merge-request watermarks", async () => {
    ...
    expect(
      requested.filter((url) => url.includes("/merge_requests?"))
    ).toHaveLength(1);
    expect(harness.updates.has("gitlab.issues.updated_after")).toBe(true);
    expect(harness.updates.has("gitlab.merge_requests.updated_after")).toBe(
      true
    );
    expect(requested.find((url) => url.includes("/issues?"))).toContain(
      "updated_after=2026-07-20"
    );
    ...
    const ctx: PullContext = {
      ...
      async fetch({ url }) {
        if (url.endsWith("/auth.test"))
          return json({ ok: true, user_id: "U1" });
  ...
  it("pins Todoist identity to the account-specific Inbox project", async () => {
    ...
    await expect(spec.principal({ ctx })).resolves.toBe(
