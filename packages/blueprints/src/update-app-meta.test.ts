...
describe(updateAppMeta, () => {
  ...
  it("renames an app in place when the name is free", async () => {
    ...
    const appJson = JSON.parse(
      await fs.readFile(path.join(dir, "todos", "app.json"), "utf8")
    ) as {
      name: string;
    };
  ...
  it("description-only updates skip the duplicate-name check", async () => {
    ...
    const appJson = JSON.parse(
      await fs.readFile(path.join(dir, "a", "app.json"), "utf8")
    ) as {
      name: string;
      description?: string;
    };
  ...
  it("propagates rename to index.html's <title> tag", async () => {
    ...
    const before = await fs.readFile(
      path.join(dir, "todos", "index.html"),
      "utf8"
    );
  ...
  it("propagates rename to automations/<sub>/automation.json#name", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        { name: "Briefing", prompt: "do", generated: originalGenerated },
        null,
        2
      )
    );
    ...
    const appJson = JSON.parse(
      await fs.readFile(path.join(dir, appId, "app.json"), "utf8")
    ) as {
      name: string;
    };
