...
describe(scaffoldAppFiles, () => {
  it("emits the canonical file set with the id/name baked in", () => {
    ...
    // The token baseline is generated into app.css; there is no second
    // tokens.css/wall.css transport to keep in sync.
    expect(files.has("tokens.css")).toBeFalsy();
    expect(files.has("wall.css")).toBeFalsy();
    ...
    expect(files.get("index.html")).not.toMatch(/(?:tokens|wall)\.css/u);
    expect(files.has("app.js")).toBeTruthy();
    expect(files.has("app.jsx")).toBeFalsy();
  ...
  it("defaults name to the id and carries a trimmed description", () => {
    const files = byPath(
      scaffoldAppFiles("notes", { description: "  jot things  " })
    );
    const appJson = JSON.parse(files.get("app.json")!) as {
      name: string;
      description?: string;
  ...
  it("rejects an invalid id", () => {
    expect(() => scaffoldAppFiles("_bad")).toThrow(/Invalid app id/u);
  });
  ...
  it("stamps an explicit tile identity into app.json", () => {
    const files = byPath(
      scaffoldAppFiles("todos", { iconKey: "Todo", colorKey: "indigo" })
    );
    const appJson = JSON.parse(files.get("app.json")!) as {
      iconKey: string;
      colorKey: string;
...
describe(updateAppMetaFiles, () => {
  ...
  it("rewrites app.json#name and the index.html <title> on rename", () => {
    ...
    expect(changed.get("index.html")!).toMatch(/<title>Tasks<\/title>/u);
  });
  ...
  it("clears description on empty patch and only touches app.json", () => {
    ...
    const changed = byPath(
      updateAppMetaFiles(start, "todos", { description: "   " })
    );
...
describe(cloneTemplateFiles, () => {
  const template = (): ScaffoldFile[] => [
    ...
    {
      path: "index.html",
      content: "<!doctype html><head><title>Hydrate</title></head>",
    },
  ];
  ...
  it("rewrites id, name, version, package name, and title", () => {
    ...
    expect(out.get("index.html")!).toMatch(/<title>Hydrate 2<\/title>/u);
  });
