...
.del:hover { color: var(--text-soft); }
.del:active { color: var(--danger); }
.del:focus-visible { outline: 2px solid var(--app-color, var(--accent)); outline-offset: 2px; }

/* --- State triad --- */
.empty { color: var(--text-faint); font-size: 0.9rem; text-align: center; padding: 2rem 0; }
.loading { color: var(--text-faint); font-size: 0.9rem; text-align: center; padding: 1rem 0; }
.error {
  color: var(--danger);
