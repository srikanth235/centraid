...
const { sha256FileStream, stageDirectFile, stageFallbackFile } = await import(
  moduleUrl
);
...
describe("edge-upload", () => {
  ...
  describe("edge-sealed direct upload", () => {
    ...
    it("resumes the fallback door from the gateway durable offset before committing", async () => {
      ...
      const fetchMock = vi.fn<FetchTestSeam>(
      ...
      expect(uploaded).toStrictEqual([Uint8Array.of(4, 5, 6, 7, 8)]);
    ...
    it("hands an unavailable direct door back to the permanent gateway fallback", async () => {
      ...
      await expect(
        stageDirectFile(
          new NodeFile([Uint8Array.of(1)], "thin.bin"),
          "34".repeat(32)
        )
      ).resolves.toBeNull();
    ...
    it("hands a first provider PUT failure back to the durable gateway door", async () => {
      ...
      await expect(
        stageDirectFile(
          new NodeFile([Uint8Array.of(1, 2)], "offline.bin"),
          "56".repeat(32)
        )
      ).resolves.toBeNull();
    ...
    it("marks a failed completion as resumable after the provider accepted ciphertext", async () => {
      ...
      await expect(
        stageDirectFile(
          new NodeFile([Uint8Array.of(1, 2)], "retry.bin"),
          "78".repeat(32)
        )
      ).rejects.toMatchObject({ resumable: true });
    });
    ...
    it("resumes multipart work by sealing only provider-missing parts and recording each ETag", async () => {
      ...
      const fetchMock = vi.fn<FetchTestSeam>(async (input: string | URL) => {
        ...
        if (url.endsWith("/direct")) {
          return Response.json({
            ...
            upload: {
              ...
              uploadId: "upload-1",
              parts: [
                { partNumber: 2, url: "https://provider.example/part-2" },
              ],
            },
          });
        }
        ...
        if (url.endsWith("/parts/2"))
          return Response.json({ completedParts: [1, 2] });
      ...
      const file = new NodeFile(
        [block, block, block, block, Uint8Array.of(1)],
        "large.bin"
      );
      ...
      expect(
        calls.filter((url) => url.includes("provider.example"))
      ).toStrictEqual(["https://provider.example/part-2"]);
    ...
    it("schedules every multipart PUT with the native background session before awaiting suspension-safe completion", async () => {
      ...
      expect(
        fetchMock.mock.calls.some(([url]) => String(url).includes("/parts/"))
      ).toBe(false);
      ...
      expect(
        fetchMock.mock.calls.filter(([url]) => String(url).includes("/parts/"))
      ).toHaveLength(2);
