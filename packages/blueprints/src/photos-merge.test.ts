...
const moduleUrl = pathToFileURL(
  path.resolve(import.meta.dirname, "../apps/photos/merge.ts")
).href;
const { mergeScopePages } = (await import(moduleUrl)) as {
  mergeScopePages: (
    pages: readonly Page[],
    options: { ownScopeId: string }
  ) => Result;
};
...
const page = (scopeId: string, assets: Asset[], truncated = false): Page => ({
  ...
  assets,
  tail:
    assets.length > 0 ? (assets[assets.length - 1]!.taken_at ?? null) : null,
  truncated,
});
...
describe("Photos multi-scope merge (#599)", () => {
  it("interleaves three scopes newest-first", () => {
    ...
    expect(result.assets.map((a) => a.scope_id)).toStrictEqual([
      "own",
      "family",
      "club",
      "own",
