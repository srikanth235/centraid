 ...
 * {@link scaffoldAppFiles}: writes the file map to disk and adds the
...
export async function scaffoldApp(
  ...
  await Promise.all(
    scaffoldAppFiles(id, opts).map(async (file) => {
...
export async function listAppsOnDisk(appsDir: string): Promise<AppInfo[]> {
  ...
  const out = await Promise.all(
    candidates.map(async (entry): Promise<AppInfo> => {
      ...
      const [stat, built, meta, hasIndex] = await Promise.all([
        fs.stat(dir),
        hasAnyBuiltJs(dir),
        readAppMeta(dir),
...
/**
 * Patch an app's `app.json` with new `name` and/or `description`.
...
export async function updateAppMeta(
  ...
  if (patch.name !== undefined) {
    ...
    await assertDisplayNameUnique(appsDir, id, renameTo);
  ...
  if (renameTo !== undefined) {
    await Promise.all([
      rewriteIndexHtmlTitle(dir, renameTo),
      rewriteAutomationManifestNames(dir, renameTo),
    ]);
  }
}

/**
 * True when any sibling app under `appsDir` (other than `selfId`,
...
/**
 * Recursively delete an app directory. Refuses to act outside `appsDir`
 * so a malformed `id` can't escalate into deleting unrelated paths.
 */
export async function deleteApp(appsDir: string, id: string): Promise<void> {
  validateAppId(id);
