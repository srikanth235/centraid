...
describe('photos-asset-key suite', () => {
  ...
  describe('visibleAssets', () => {
    it('keeps both colliding rows instead of collapsing them into one tile', () => {
      ...
      expect(shown.map((a) => a.content_id).sort()).toStrictEqual(['c-family', 'c-own']);
    });
  });
  ...
  describe('batch commands', () => {
    ...
    it('addresses each key at its own scope across a mixed selection', async () => {
      ...
      expect(acts.map((call) => call.scope)).toStrictEqual(['vault-own', 'vault-family']);
      ...
      expect(new Set(acts.map((call) => call.input.asset_id))).toStrictEqual(new Set([COLLIDING]));
    });
