...
interface Store {
  ...
  handleChange(detail: { tables?: string[]; source?: string; scope?: string } | undefined): void;
  merged(): { assets: (Asset & { scope_id: string })[]; horizonScopeIds: string[] };
  scope(scopeId: string): { assets: Asset[]; tail: string | null; error: string | null };
  ...
  return createLibraryStore({
    async readScopes(scopeIds, input) {
      ...
      return scopeIds.map((scope) => ({
        scope,
        ok: true as const,
        data: answers.get(scope) ?? { assets: [], tail: null, truncated: false },
      }));
    },
    scopeIds: () => [...SCOPES],
    ownScopeId: () => 'own',
...
beforeEach(() => {
  ...
  paints = 0;
  answers = new Map([
    ['own', { assets: [asset('o1', '2026-07-10')], tail: '2026-07-10', truncated: false }],
    ['family', { assets: [asset('f1', '2026-07-09')], tail: '2026-07-09', truncated: false }],
    ['club', { assets: [asset('k1', '2026-07-08')], tail: '2026-07-08', truncated: false }],
  ]);
});
...
describe('Photos library store — per-scope refetch (#599)', () => {
  it('reads every mounted scope in one fan-out on a full refresh', async () => {
    ...
    expect(calls).toEqual([{ scopes: SCOPES, input: { limit: 500 } }]);
    expect(store.merged().assets.map((a) => a.asset_id)).toEqual(['o1', 'f1', 'k1']);
    expect(store.merged().assets.map((a) => a.scope_id)).toEqual(['own', 'family', 'club']);
  });
  ...
  it('refetches ONLY the scope a change burst is tagged with', async () => {
    ...
    expect(calls[0]?.scopes).toEqual(['family']);
  });
  ...
  it('fetches just the newly hydrated scope when one arrives after first paint', async () => {
    ...
    expect(calls).toHaveLength(1);
    expect(calls[0]?.scopes).toEqual(['club']);
  });
  ...
  it('keeps a failing audience from blanking the scopes that answered', async () => {
    ...
    await store.refreshAll();
    expect(store.merged().assets.map((a) => a.asset_id)).toEqual(['o1', 'k1']);
...
describe('Photos library store — show more (#599)', () => {
  beforeEach(() => {
    ...
    answers = new Map([
      ...
      ['family', { assets: [asset('f1', '2026-07-09')], tail: '2026-07-09', truncated: true }],
      ['club', { assets: [asset('k1', '2026-07-08')], tail: '2026-07-08', truncated: false }],
    ]);
  });
  ...
  it('queries only the horizon scopes, each from its own cursor', async () => {
    const store = makeStore();
    await store.refreshAll();
    expect(store.merged().horizonScopeIds).toEqual(['family']);
    calls = [];
    ...
    expect(calls).toEqual([{ scopes: ['family'], input: { limit: 500, before: '2026-07-09' } }]);
  });
  ...
  it('appends the deeper page instead of replacing what the scope held', async () => {
    ...
    expect(store.scope('family').assets.map((a) => a.asset_id)).toEqual(['f1', 'f2']);
    ...
    expect(store.merged().assets.map((a) => a.asset_id)).toEqual(['o1', 'f1', 'k1', 'o2', 'f2']);
  });

  it('does nothing when no scope is at a horizon', async () => {
    answers.set('family', { assets: [asset('f1', '2026-07-09')], tail: '2026-07-09' });
    const store = makeStore();
    await store.refreshAll();
    calls = [];
    await store.showMore();
  ...
  it('re-reads a scope at the depth it already paged to, not back at page one', async () => {
    ...
    expect(calls[0]).toEqual({ scopes: ['family'], input: { limit: 2 } });
  });
});
