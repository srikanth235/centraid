...
// media_type LIKE 'text/%' — format.js's isTextEditable mirrors the vault's
// own precondition exactly, and Details.jsx only ever offers the Edit
// affordance that opens this for a doc that passes it). Mounted keyed by
// document_id at the call site (app.jsx), so opening a different document
// remounts this component fresh — same idiom notes/components/Editor.jsx
// uses for its own note_id key. The doc's content_id/content_uri changing
...
// same debounce/flush shape notes/components/Editor.jsx's performSave/
...
// rewrite to a blob route) decodes directly via format.js's decodeDataUri —
...
import { decodeDataUri, fmtFull } from '../format.js';
import { I } from '../icons.js';
import { Icon } from './Shared.jsx';

export function Editor({ doc, registerFlush, onClose, onSave }) {
    ...
    >
      ...
      >
        <div className="d-editor-top">
          ...
          <span className="d-editor-title">{doc.title ?? 'Untitled'}</span>
          <span className="d-editor-save">{saveLabel}</span>
        </div>
        <div className="d-editor-body">
          {loadState === 'loading' ? (
            <div className="d-editor-status">Loading…</div>
          ) : loadState === 'error' ? (
            <div className="d-editor-status">Could not load this document's text.</div>
          ) : (
            <textarea
              ref={textareaRef}
              className="d-editor-textarea"
