...
// Docs, inline (issue #505). The served entry (app.tsx) stays byte-for-byte for
// mobile WebViews + the visual harness; this co-located sibling re-expresses the
// SAME orchestration as ONE React tree for the shell's inline route. The
// non-visual factories are reused verbatim — `logic.ts` (data/selection helpers,
// vault writes, upload, plain-DOM popovers), `nav.ts` (nav + overlay open/close
// transitions), the `queries/*`, and every `components/*` — with `chrome.ts`'s
// imperative listeners re-expressed as hooks + JSX handlers here. app.tsx's
// fourteen `createRoot` islands + imperative `render()`/textContent/hidden writes
// collapse into this component's JSX. Reads/writes flow through the shell-
// installed `window.centraid` (backed by the replica), so mount awaits nothing
// over the network — first paint is local. Uploads ride kit-inline's authed
// `stageFileBytes`/`stageDerivative` large-file path (`./upload.ts` → `./kit.js`),
// so >256 KiB documents upload inline; the PDF text derivative degrades to the
// gateway's own extractor exactly as served.
//
// `logic.ts`/`nav.ts` close over `state`/`data` (mutated in place, never
// reassigned) plus render entry points app.tsx defines; here those entry points
// funnel to a `bump` reducer (one React tree, no per-container roots). The only
// DOM ids the reused factories touch are kept on real elements Chrome renders:
// `#noticeBanner` (logic.notice / readFailed), `#searchInput` (nav.selectNav
// clears its value; applySearch reads it), `#uploadInput` (nav.triggerUpload
// clicks it). nav.selectNav also toggles a class on `#root` — the host page's own
// mount div, a harmless no-op here — and its drawer intent is wrapped to drive
// React `sideOpen` state instead (trap #5).
import {
  useCallback,
  useEffect,
  useLayoutEffect,
