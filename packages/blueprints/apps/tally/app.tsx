...
// per stable container — the same tasks/notes/app.tsx pattern. `logic.ts`
// holds the non-visual business logic (vault IO, the people directory, every
// modal's open/patch/save/close flow); `chrome.ts` wires the toolbar/
...
// static `.tally`/`.s-*` chrome, the narrow-viewport rules on the static
// #root); the JSX-only `.s-*` component rules moved into co-located
// components/*.module.css (shared atoms in components/shared.module.css).
...
import type { AppState, Dash, DashboardPayload, LedgerRow, ViewData } from './types.ts';
...
// reassigned, only its fields are mutated, so logic.ts's closure over it
...
// further down the file — same trick tasks/app.tsx's `createLogic(...)` call
// relies on.

const logic = createLogic({ state, dash, render, renderModals, loadView, refreshAll });

let setThemeIcon: (() => void) | undefined;

// ---------- Roots ----------

let smartNavRoot: ReturnType<typeof createRoot>;
let groupsNavRoot: ReturnType<typeof createRoot>;
let friendsNavRoot: ReturnType<typeof createRoot>;
let wrapRoot: ReturnType<typeof createRoot>;
...
function pendingForView(): LedgerRow[] {
