...
// keep app.tsx under the file-size cap, same shape as tasks/chrome.ts.
import { observeWidth, onFocusRefresh, wireThemeToggle } from './kit.js';
import type { createLogic } from './logic.ts';
import type { AppState } from './types.ts';

type Logic = ReturnType<typeof createLogic>;
...
export function wireChrome({ state, logic, renderModals, refreshAll }: ChromeDeps): {
  setThemeIcon: () => void;
} {
  ...
  // Kit theme toggle; app.tsx's render() calls the returned setter too, so
