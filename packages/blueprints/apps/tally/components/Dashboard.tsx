...
function BalRow({
  ...
  return (
    ...
    >
      ...
      <span className={`${styles.balAmt} ${TONE[amtCls]}`}>
        {money(p.net_minor, currency)}
      </span>
    </button>
  );
}
        ...
        {expenses.map((expense) => (
          <div className={styles.trashRow} key={expense.expense_id}>
            ...
            <button
              type="button"
              className="kit-btn"
              onClick={() => onRestore(expense.expense_id)}
            >
              Restore
            </button>
          </div>
...
export function Dashboard({
  ...
  const net = owed - owe;
  const netCls: "" | "pos" | "neg" =
    Math.abs(net) < 1 ? "" : net > 0 ? "pos" : "neg";
    ...
    <>
      <div className={styles.summary}>
        <div className={styles.stat}>
          ...
          <div className={`${styles.v} ${netCls ? TONE[netCls] : ""}`}>
            {netLabel}
          </div>
        </div>
        <div className={styles.stat}>
          <div className={styles.k}>You owe</div>
          <div className={`${styles.v} ${TONE.neg}`}>
            {money(owe, currency)}
          </div>
        </div>
        <div className={styles.stat}>
          <div className={styles.k}>You are owed</div>
          <div className={`${styles.v} ${TONE.pos}`}>
            {money(owed, currency)}
          </div>
        </div>
      </div>
