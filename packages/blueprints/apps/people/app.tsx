// governance: allow-repo-hygiene file-size-limit (#363) blueprint apps ship a single app.tsx entry per the kit's convention (see sibling docs/photos/tally); splitting would break the served-as-one-module contract static-server.ts assumes
...
// never Lit-rendered either. `logic.ts` holds the non-visual business logic
// (vault IO, row derivation, the popover, every write, nav transitions);
// `chrome.ts` wires the toolbar/keyboard/resize listeners; `format.ts`/
// `icons.ts` are stateless. `components/` holds pure functions of props.
//
// TS + CSS-modules conversion: the state/data shapes live in `types.ts`
// (imported type-only); component styles moved to co-located
// `components/*.module.css`, with the global shell/kit remainder still in
// `app.css` (linked, in order).
...
import { avatarColor, listName, hashInt, PALETTE } from './format.ts';
import { I } from './icons.ts';
import { ListList, JournalNav, SmartNav, Storage } from './components/Sidebar.tsx';
...
import type { AppData, AppState, Person, PersonList } from './types.ts';

// The two `window.centraid.read<T>` projections this page consumes. Naming the
// shape keeps the read results typed without an `any` (same idiom as tasks).
...
const data: AppData = { people: [], lists: [] };
...
let smartNavRoot: ReturnType<typeof createRoot>;
let listListRoot: ReturnType<typeof createRoot>;
let journalNavRoot: ReturnType<typeof createRoot>;
let storageRoot: ReturnType<typeof createRoot>;
let statusChipsRoot: ReturnType<typeof createRoot>;
let bulkBarRoot: ReturnType<typeof createRoot>;
let gridRoot: ReturnType<typeof createRoot>;
let listRoot: ReturnType<typeof createRoot>;
let listHeadRoot: ReturnType<typeof createRoot>;
let windowFootRoot: ReturnType<typeof createRoot>;
let journalRoot: ReturnType<typeof createRoot>;
let activityRoot: ReturnType<typeof createRoot>;
let detailsRootReact: ReturnType<typeof createRoot>;
let modalRootReact: ReturnType<typeof createRoot>;
let newMenuRoot: ReturnType<typeof createRoot>;
...
function makeMounter(containerId: string, getRoot: () => ReturnType<typeof createRoot>) {
  let mounted = false;
...
function renderSidebar() {
  ...
  listListRoot.render(
    <ListList
      ...
      navListId={state.nav.kind === 'list' ? state.nav.listId : undefined}
...
function render() {
  ...
  // Capture `listId` before the closure: narrowing doesn't flow into the
  // `.some()` callback (it could run after `state.nav` changed).
...
const applySearch = debounce(async () => {
  ...
  let rows: Person[] = [];
  try {
    const res = await window.centraid.read<SearchPayload>({ query: 'search', input: { term: q } });
    rows = res?.people ?? [];
  } catch {
    rows = [];
  }
