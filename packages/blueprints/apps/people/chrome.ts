...
// JSX — factored out purely to keep app.tsx under the file-size cap. Every
// callback it invokes is passed in by app.tsx, which is the only module that
// defines them (same shape as docs/tasks/notes' chrome.ts).
import {
  closePopover,
  isPopoverOpen,
  observeWidth,
...
import type { ChromeDeps, SortKey } from './types.ts';
...
export function wireChrome({
  ...
  document.addEventListener('click', (e) => {
    if (state.newMenuOpen && !(e.target instanceof Element && e.target.closest('.d-new-wrap'))) {
  ...
  $('sortBtn').addEventListener('click', () => {
    const keys: SortKey[] = ['last', 'name', 'cadence'];
