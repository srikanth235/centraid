...
// optimistic mutation, so a denied `add-person`'s row left the list entirely
// and a denied trash snapped the person back out of the trash. That is
// exactly the silent-disappearance defect the issue exists to end, so the
...
const ACTION_LABEL: Record<string, string> = {
  ...
  "edit-person": "Profile edit",
  "trash-person": "Move to trash",
  "restore-person": "Restore",
  "log-interaction": "Logged catch-up",
};

function rowLabel(row: PendingRowState): string {
  const name = row.input?.display_name;
...
export function Attention({
   ...
   *  — the "Add someone" modal, for a refused `add-person`. The profile
   *  drawer is bound to a canonical person and saves field by field, so it is
   *  deliberately not offered as an edit surface for a refused payload. */
