...
export function shortFmt(d) {
  if (d <= 0) return 'now';
  if (d < 7) return d + 'd';
  if (d < 31) return Math.round(d / 7) + 'w';
  return Math.round(d / 30) + 'mo';
}
export function inFmt(d) {
  if (d === 0) return 'today';
...
export function daysUntilAnnual(monthDay) {
...
export function fmtMonthDay(monthDay) {
  const parts = String(monthDay ?? '').split('-');
...
export function dateInputToMonthDay(v) {
...
export function fmtJournalDate(v) {
  if (!v) return '';
  const s = String(v);
