...
export default async function journalHandler({ ctx }: HandlerArgs) {
  ...
  try {
    ...
    const [notes, activities, annotations, parties, profiles] =
      await Promise.all([
        noteIds.length > 0
          ? ctx.vault.read({
    ...
    const partyById = new Map(
      ((parties.rows ?? []) as unknown as RawParty[]).map((party) => [
        party.party_id,
        party,
      ])
    );
    ...
    const textByActivity = new Map(
      ((annotations.rows ?? []) as unknown as RawAnnotation[]).map(
        (annotation) => [annotation.target_id, annotation.body_text]
      )
    );
    ...
    const auto = ((activities.rows ?? []) as unknown as RawActivity[]).map(
      (activity) => {
        ...
        return {
          ...
          touch:
            conceptById.get(activity.kind_concept_id)?.notation ??
            "interaction",
