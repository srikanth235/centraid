...
// (app.jsx) so opening a different note remounts this component fresh — the
...
// app.js's own performSave/scheduleSave. `registerFlush` lets app.jsx await
...
import { deriveTitle, parseBlocks, stripInline } from '../format.js';
import { I } from '../icons.js';
import { Icon } from './Shared.jsx';

function AttachStrip({ note, onRemove }) {
...
function DeleteButton({ onDelete }) {
  ...
  return (
    ...
    >
      {armed ? <span className="nt-armed-label">Sure?</span> : <Icon svg={I.trashLg} />}
    </button>
  );
}

function Blocks({ body, onToggleCheck, onEnter }) {
  ...
  return (
    <div
      className="nt-body-render"
      tabIndex={0}
      role="textbox"
      aria-multiline="true"
      aria-label="Note body — press Enter to edit"
      onClick={(e) => {
        if (e.target.closest('.nt-check-box')) return;
    ...
    >
      {blocks.length === 0 ? <p className="nt-md-gap" /> : null}
      {blocks.map((b) => {
        if (b.kind === 'gap') return <p key={b.line} className="nt-md-gap" data-line={b.line} />;
        if (b.kind === 'h') {
          const Tag = `h${b.level + 2}`;
          return (
            <Tag key={b.line} className={`nt-md-h${b.level}`} data-line={b.line}>
              {stripInline(b.text)}
            </Tag>
          );
        }
        if (b.kind === 'check') {
          return (
            ...
            >
              ...
              <span className="nt-check-text">{stripInline(b.text)}</span>
            </div>
          );
        }
        if (b.kind === 'li') {
          return (
            <div key={b.line} className="nt-md-li" data-line={b.line}>
              <span className="nt-md-bullet" aria-hidden="true">
                •
              </span>
        ...
        return (
          <p key={b.line} className="nt-md-p" data-line={b.line}>
            {stripInline(b.text)}
          </p>
        );
      })}
...
export function Editor({
  ...
  function scheduleSave() {
    ...
    saveTimerRef.current = setTimeout(performSave, 700);
  }
  ...
  return (
    <div
      className="nt-editor-backdrop"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div className={pending ? 'nt-editor kit-pending' : 'nt-editor'}>
        <div className="nt-editor-top">
          <button type="button" className="kit-icon-btn" aria-label="Back" onClick={onClose}>
            <Icon svg={I.back} />
          </button>
          <span className="nt-save-label">{saveLabel}</span>
          <div className="nt-editor-tools">
            <select
              className="nt-nb-select"
              aria-label="Notebook"
              value={notebookId}
              onChange={(e) => {
            ...
            <button
              ...
              aria-pressed={String(note.pinned === 1)}
              onClick={() => {
                flush();
                onTogglePin(note);
              }}
        ...
        <div className="nt-editor-foot">
          <span className="nt-receipt-chip">receipt</span>
          <span className="nt-activity-line">{activity.join('  ·  ')}</span>
        </div>
      </div>
    </div>
  );
