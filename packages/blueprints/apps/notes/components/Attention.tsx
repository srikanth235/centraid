...
//
// Every other pending status renders as the chip on the card itself, because
// the replica still overlays it. These do not: settlement removed the
// optimistic mutation, so a denied `create-note`'s card left the wall
// entirely and a denied delete snapped the note back out of the trash. That
...
const ACTION_LABEL: Record<string, string> = {
  ...
  "edit-note": "Note edit",
  "move-note": "Note move",
  "delete-note": "Note deletion",
  "rename-notebook": "Notebook rename",
  "delete-notebook": "Notebook deletion",
};

function rowLabel(row: PendingRowState): string {
  const title = row.input?.title ?? row.input?.name;
...
export function Attention({
   ...
   *  — the quick-add card, for a refused `create-note`. The editor is bound
   *  to a canonical note and autosaves what it shows, so it is deliberately
   *  not offered as an edit surface for a refused payload. */
