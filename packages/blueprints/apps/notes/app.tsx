...
// per stable container — the same tasks/app.tsx pattern. `logic.ts` holds
// the non-visual business logic (vault IO, notebook CRUD, parked-write
// tracking, search); `chrome.ts` wires the toolbar/keyboard/resize
// listeners; `format.ts`/`icons.ts` are stateless. `components/` holds pure
// functions of props.
...
const data: AppData = { notes: [], notebooks: [], tags: [], window: 200 };
...
const initialView: AppState['view'] =
  knobView && VALID_VIEWS.has(knobView as AppState['view'])
    ? (knobView as AppState['view'])
    : 'masonry';
...
let focusQuickAddFn: (() => void) | null = null;
...
function registerEditorFlush(fn: () => Promise<void>) {
  editorFlush = fn;
}
...
let sidebarNavRoot: ReactRoot;
let sidebarFootRoot: ReactRoot;
let toolbarRoot: ReactRoot;
let wallRoot: ReactRoot;
let editorRoot: ReactRoot;
...
async function showMore() {
  const btn = $('wall').querySelector('.kit-foot button') as HTMLButtonElement | null;
