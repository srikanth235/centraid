// Notes, inline (issue #505). The served entry (app.tsx) stays byte-for-byte for
...
// (vault IO, notebook CRUD, parked tracking, search) and the `components/*` are
// reused verbatim; `chrome.ts`'s imperative listeners become hooks + JSX
// handlers; app.tsx's five `createRoot` islands + imperative `render()` collapse
// into this component's JSX. Reads/writes flow through the shell-installed
// `window.centraid` (backed by the replica), so mount awaits nothing over the
// network — first paint is local. The attach flow's large-file upload rides the
// authed `wireAttachInput` override (kit-inline), so >256 KiB attachments work
// inline; small files still travel inline as data: URIs.
