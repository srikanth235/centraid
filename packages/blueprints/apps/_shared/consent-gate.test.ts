 ...
 * honest with a source scan of `packages/server/src/automation/fire/enrich-gate.ts`,
...
const ENRICH_GATE_PATH = path.resolve(
  import.meta.dirname,
  "../../../server/src/automation/fire/enrich-gate.ts"
);
...
function automationEnrichDomains(): string[] {
  ...
  if (!match) {
    throw new Error(
      "ENRICH_DOMAINS not found in packages/server/src/automation/fire/enrich-gate.ts " +
