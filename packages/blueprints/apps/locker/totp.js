...
export async function computeTotp(seed, step) {
...
export function totpOffset() {
  const rem = 30 - (Math.floor(Date.now() / 1000) % 30);
  return 94.2 * (1 - rem / 30);
}

const OTP_CACHE = new Map(); // `${seed}|${step}` -> "123 456" | null

function cacheKey(seed, step) {
  return `${seed}|${step}`;
}
 ...
 */
export function useTotp(seed) {
  const [tick, setTick] = useState(0);
  const [code, setCode] = useState(() => {
    if (!seed) return null;
    const key = cacheKey(seed, Math.floor(Date.now() / 30000));
    return OTP_CACHE.has(key) ? OTP_CACHE.get(key) : null;
  });
...
export function genPassword({ len, num, sym }) {
