...
// modal idiom (CreateModal.jsx et al). Mounted only while `state.edit` is
// set and unmounts on close, so its local state always starts fresh from the
// `edit` seed app.jsx built in `openNew()`/`openEdit()`. The password
...
import { CAT_ORDER, TYPE_LABEL } from '../format.js';
...
function EditFieldRow({ f, value, onChange, onGenerate }) {
  const input = (
    <input
      className={f.mono ? 'v-in mono' : 'v-in'}
      placeholder={f.ph || ''}
      value={value || ''}
  ...
  return (
    <div className="v-field-lg">
      <div className="v-flabel">{f.label}</div>
      {f.gen ? (
...
export function EditModal({ edit, onClose, onSave, onOpenGenerator }) {
  const { mode } = edit;
  ...
  const generate = (key) => onOpenGenerator((password) => setField(key, password));
  ...
  return (
    ...
    >
      <div className="kit-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {mode === 'new' ? (
          <div className="v-field-lg">
            <div className="v-flabel">Type</div>
            <div className="v-typerow">
              {CAT_ORDER.map((t) => (
                <button
                  key={t}
                  type="button"
                  className="kit-chip quiet"
                  aria-pressed={String(type === t)}
                  onClick={() => {
                    setType(t);
                    setFields({});
                  }}
        ...
        <div className="v-field-lg">
          <div className="v-flabel">Title</div>
          <input
            className="v-in"
        ...
        <div className="v-field-lg">
          <div className="v-flabel">Tags (comma-separated)</div>
          <input
            className="v-in"
        ...
        manifest edit. */}
        <div className="v-field-lg">
          <div className="v-flabel">Connector alias (optional)</div>
          <input
            className="v-in mono"
