...
// Photos, inline (issue #505). The served entry (app.tsx) stays byte-for-byte
// for mobile WebViews + the visual harness; this co-located sibling re-expresses
// the SAME orchestration for the shell's inline route as ONE React tree.
//
// Photos is the largest bundled app and, unlike the pilot apps, keeps NO
// `logic.ts` — its orchestration lives in app.tsx alongside eight `createRoot`
// islands (sidebar/toolbar/main/selection-bar/lightbox/slideshow/picker/
// enrichment) and the region factories (`createSidebar`/`createLightbox`/… in
// sidebar.tsx/lightbox.tsx/picker.tsx/slideshow.tsx/duplicates.tsx) that each own
// a private slice of state and render into an INJECTED root. That injection seam
// is the whole conversion: instead of `createRoot($('…'))`, each factory (and
// app.tsx's own `render*` orchestrators) is handed a SLOT root whose `.render()`
// writes a `slots.*` ReactNode and bumps this component — so every region renders
// inside Chrome.tsx's one tree, and `sidebar.tsx`/`lightbox.tsx`/… /`media.ts`/
// `visibility.ts`/`upload.ts`/`search.ts` and every `components/*` are reused
// VERBATIM. app.tsx's module-level state + boot sequence are reproduced in the
// mount effect below (they cannot be imported — app.tsx runs that boot at module
// eval against the served DOM); index.html's body becomes Chrome.tsx.
//
// Reads/writes flow through the shell-installed `window.centraid` (backed by the
// replica); uploads + derivative contributions ride the app's normal `./kit.js`
// imports, which the inline Vite alias rewrites to the authed kit-inline
// (`stageFileBytes`/`stageDerivative`). See the file-foot note on blob DISPLAY.
import {
  useCallback,
  useEffect,
  useLayoutEffect,
