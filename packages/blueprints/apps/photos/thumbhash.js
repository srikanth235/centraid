 ...
 */
export function thumbHashFromRgba(w, h, rgba) {
  ...
  const encodeChannel = (channel, nx, ny) => {
    ...
    for (let cy = 0; cy < ny; cy += 1) {
      for (let cx = 0; cx * ny < nx * (ny - cy); cx += 1) {
        ...
        for (let y = 0; y < h; y += 1) {
          ...
          for (let x = 0; x < w; x += 1) f += channel[x + y * w] * fx[x] * fy;
        }
        f /= w * h;
        if (cx || cy) {
          ac.push(f);
    ...
    if (scale) for (let i = 0; i < ac.length; i += 1) ac[i] = 0.5 + (0.5 / scale) * ac[i];
    return [dc, ac, scale];
  };
 ...
 */
export function thumbHashFromImage(img) {
  try {
