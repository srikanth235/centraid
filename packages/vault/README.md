...
**The schema is a baseline, then rungs.** `VAULT_MIGRATIONS` in `src/schema/migrate.ts` holds rung one — the #916 baseline, stated rather than reconstructed, because v0 had no files in the field — and rung two, added by [#929](https://github.com/srikanth235/centraid/issues/929), the first change that had to reach a file that already existed. A fresh vault stamps `PRAGMA user_version = 2`. Rung one is history and does not grow; a change that must reach an existing file adds the next rung.
