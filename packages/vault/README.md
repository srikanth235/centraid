...
**The schema is ONE baseline, not a ladder.** `VAULT_MIGRATIONS` in `src/schema/migrate.ts` holds a single entry and a fresh vault stamps `PRAGMA user_version = 1`. v0 has no files in the field, so a rung that reconstructs a shape the baseline can state is compatibility code for a problem nobody has. The first shipped release that must reach an existing file adds rung two — and the baseline text becomes history at that moment, not before.
