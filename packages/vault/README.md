...
**The schema is a baseline, then rungs.** `VAULT_MIGRATIONS` in `src/schema/migrate.ts` holds rung one — the #916 baseline, stated rather than reconstructed, because v0 had no files in the field — rungs two through four, added by [#929](https://github.com/srikanth235/centraid/issues/929), the first change that had to reach a file that already existed — the subscription seat, `share_delivery_config`'s re-cut with `departure_policy`, and the drop of `core_share_origin` — and rung five, added by [#972](https://github.com/srikanth235/centraid/issues/972) so `share_authority_request` and `share_authority_use` reach files that had already climbed past rung 1. A fresh vault stamps `PRAGMA user_version = 5`. Rung one is history and does not grow; a change that must reach an existing file adds the next rung.
