...
describe('rrule', () => {
  test('parseRrule reads FREQ/INTERVAL/COUNT/UNTIL/BYDAY', () => {
    expect(parseRrule('FREQ=WEEKLY;INTERVAL=2;BYDAY=MO,WE;COUNT=5')).toStrictEqual({
      freq: 'WEEKLY',
      interval: 2,
      count: 5,
      until: undefined,
  ...
  test('expandRrule WEEKLY+BYDAY lands only on named weekdays', () => {
    ...
    expect(weekdays).toStrictEqual([1, 3, 5, 1, 3, 5]);
  });
  ...
  test('expandRrule MONTHLY clamps day-of-month into shorter months', () => {
    ...
    expect(hits.map((h) => h.slice(0, 10))).toStrictEqual([
      '2026-01-31',
      '2026-02-28',
      '2026-03-31',
      '2026-04-30',
  ...
  test.each(PARITY)('server projection matches the native expansion: $name', (fixture) => {
    ...
    expect(expandRrule(fixture.rrule, fixture.start, fixture.from, fixture.to, 200)).toStrictEqual(
      fixture.expected,
    );
  });
