...
describe('placement suite', () => {
  ...
  test('a share projects the item into the audience vault and leaves the origin untouched', () => {
    ...
    expect(readShareOrigin(audience.vault, 'media.media_asset', result.itemId)).toStrictEqual({
      itemType: 'media.media_asset',
    ...
    expect(
      plainSqliteRow(origin.vault.prepare('SELECT COUNT(*) AS n FROM core_share_origin').get()),
    ).toStrictEqual({
      n: 0,
    });
  ...
  test('linkFromSync classifies a refusing link as unsupported and rethrows anything else', () => {
    ...
    expect(() => store.linkFromSync('b'.repeat(64), path.join(root, 'nope'))).toThrow(/ENOENT/u);
  });
  ...
  test('re-sharing the same item is idempotent — same member and a different member', () => {
    ...
    expect(
      plainSqliteRow(
        audience.vault.prepare('SELECT COUNT(*) AS n FROM core_content_derivative').get(),
      ),
    ).toStrictEqual({
      n: 1,
    });
    ...
    expect(bySid.blobs.map((b) => b.mode)).toStrictEqual(['present', 'present']);
  });
});
