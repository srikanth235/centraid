...
test('an injected mid-share failure leaves the origin clean and only a reclaimable orphan', () => {
  ...
  expect(origin.vault.prepare('SELECT COUNT(*) AS n FROM media_media_asset').get()).toEqual({
    n: 1,
  });
  ...
  expect(audience.vault.prepare('SELECT COUNT(*) AS n FROM core_share_origin').get()).toEqual({
    n: 0,
  });
...
test('unshare removes the projection; the origin row and bytes stay readable', () => {
  ...
  expect(result.orphanedShas.sort()).toEqual([photo.sha256, photo.thumbSha].sort());
  expect(audience.vault.prepare('SELECT COUNT(*) AS n FROM media_media_asset').get()).toEqual({
    n: 0,
  });
  expect(audience.vault.prepare('SELECT COUNT(*) AS n FROM core_content_item').get()).toEqual({
...
test('re-sharing after an unshare works again, idempotently', () => {
  ...
  share();
  unshareFromVault({ audience, itemType: 'media.media_asset', itemId: photo.assetId });
  reclaimOrphans(audience);
  const again = share();

  expect(again.deduped).toBe(false);
  expect(again.blobs.map((b) => b.mode)).toEqual(['linked', 'linked']);
...
test('unshare refuses a row the audience authored itself — no provenance, no delete', () => {
  ...
  expect(result).toEqual({ removed: false, orphanedShas: [] });
  expect(audience.vault.prepare('SELECT COUNT(*) AS n FROM media_media_asset').get()).toEqual({
    n: 1,
  });
});
...
test('sharing an unknown item is refused before anything is placed', () => {
  ...
  ).toThrow(/is not in the origin vault/);
  expect(audience.blobs.local.listSync()).toEqual([]);
});
