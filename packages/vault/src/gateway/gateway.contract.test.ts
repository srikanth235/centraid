...
describe("gateway", () => {
  ...
  describe("S3/S4 command execution", () => {
    test("the provenance doorbell rings only after journal commit and cannot fail the write", () => {
      ...
      expect(observed[0]?.entityTypes).toStrictEqual(
    ...
    test("group commit crosses exactly one vault + journal commit pair", () => {
      ...
      expect(
        vaultExec.mock.calls.filter(([sql]) => sql === "COMMIT")
      ).toHaveLength(1);
      expect(
    ...
    test("invocation ids are bound to command, caller, and grant before execution", () => {
      ...
      expect({
        ...db.vault
          .prepare(
            "SELECT count(*) AS n FROM replica_invocation_commit WHERE invocation_id = ?"
          )
          .get(invocationId),
      }).toStrictEqual({ n: 0 });
    ...
    `);
      ...
      expect(journalRows()).toStrictEqual({
        postChecks: 0,
        provenance: 0,
        receipts: 0,
        evidence: 0,
    ...
    test("ordinary post-canonical recovery preserves receipt replay output", () => {
      ...
      const event = db.vault
        .prepare("SELECT event_id FROM core_event")
        .get() as {
        event_id: string;
      };
      ...
      expect(
        db.vault
          .prepare(
            `SELECT 1 AS present FROM replica_invocation_commit WHERE invocation_id = ?`
          )
          .get(invocationId)
      ).toBeUndefined();
    });
  ...
  describe("confirmation routing + revocation + sweeps", () => {
    ...
    test('listParked resolves callerKind "assistant" for the vault assistant identity, distinct from an automation agent (issue: parked-invocation trust legibility)', () => {
      ...
      const assistantDevice = enrollDevice(
        db,
        boot.ownerPartyId,
        "assistant-host"
      );
      ...
      const listed = gw.listParked();
      expect(
        listed.find((p) => p.invocationId === assistantParked.invocationId)
      ).toMatchObject({
        callerKind: "assistant",
        caller: "Assistant",
      });
      expect(
        listed.find((p) => p.invocationId === automationParked.invocationId)
      ).toMatchObject({
        callerKind: "agent",
      });
    });
    ...
    test("revoking a replica device makes all of its parked payloads unapprovable", () => {
      ...
      expect(readDurableParkedPayload(db, parked.invocationId)).toBeUndefined();
      expect(() => gw.confirm(owner, parked.invocationId, true)).toThrow(
        /no parked invocation/u
      );
      expect({
        ...db.vault.prepare("SELECT count(*) AS n FROM core_event").get(),
      }).toStrictEqual({
    ...
    test("sweep expires lapsed grants and purges scheduled content", () => {
      ...
      const gone = db.vault
        .prepare(
          `SELECT count(*) AS n FROM core_content_item WHERE content_id='c-old'`
        )
        .get() as {
        n: number;
      };
    ...
    test("sweep purges a lapsed trashed asset on its own clock while its rented bytes live on", () => {
      ...
      const contentStays = db.vault
        .prepare(
          `SELECT count(*) AS n FROM core_content_item WHERE content_id='c-live'`
        )
        .get() as { n: number };
  ...
  test("within-next-days filters to the forward horizon window (due soon, not overdue-forever)", () => {
    ...
    const now = Date.now();
    const iso = (deltaDays: number): string =>
      new Date(now + deltaDays * 86_400_000).toISOString();
