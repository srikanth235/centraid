...
- Canonical bytes/text live in core_content_item (content_uri is a data: URI). What those bytes ARE lives on core_content_representation, one row per (owner_type, owner_id) — core_content_item carries no media_type and no title. Use vault_content_text(<the owner's media_type>, content_uri) to decode a body to text. Note/message bodies hang off *_content_id columns.
- A document's identity is core_document, not core_content_item (issue #352): current_content_id names the HEAD content item; version history is core_entity_revision OCCURRENCES for that document, walked from core_document.current_revision_id through parent_revision_id, each naming the content that became current (#996, R20(a)). knowledge_note's body_content_id repoints the same way and carries its own current_revision_id. There is no 'revises' link and no second history table.
