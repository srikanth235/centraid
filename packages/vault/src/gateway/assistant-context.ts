...
- Canonical bytes/text live in core_content_item (content_uri is a data: URI). What those bytes ARE lives on core_content_representation, one row per (owner_type, owner_id) — core_content_item carries no media_type and no title. Decoded body text is a column: join core_content_text on content_id. Note/message bodies hang off *_content_id columns.
