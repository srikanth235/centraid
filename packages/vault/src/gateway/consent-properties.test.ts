...
// `evaluateConsent`'s own doc comments claim, driven over generated grants,
// grant_scopes, execution clamps, and consent_policy rows rather than
...
// policy never leaks into the next: `evaluateConsent` only ever SELECTs, so a
  ...
  db.vault
    .prepare(
      `INSERT INTO consent_policy
       ...
       VALUES (?, ?, ?, ?, ?, NULL, NULL, '2020-01-01T00:00:00Z', 1)`
    )
 ...
 */
function normDecision(decision: ConsentDecision): Record<string, unknown> {
...
/** The field-mask intersection `evaluateConsent`/`executionClamp` compute. */
...
describe("consent chain property", () => {
  ...
  // narrowing a clamp verb, or adding a consent_policy row can only turn
        ...
        (sc, mutation) => {
          inTxn(() => {
            ...
            switch (mutation) {
              case "delete-grant":
                db.vault
                  .prepare(`DELETE FROM consent_grant_scope WHERE grant_id = ?`)
                  .run(grantId);
                db.vault
                  .prepare(
                    `DELETE FROM consent_access_grant WHERE grant_id = ?`
                  )
                  .run(grantId);
                break;
              case "remove-scope":
                db.vault
                  .prepare(
                    `DELETE FROM consent_grant_scope
                      WHERE scope_id = (
                        SELECT scope_id FROM consent_grant_scope
        ...
        (sc) => {
          inTxn(() => {
            ...
            db.vault
              ...
              .run(wholeGrant);
            db.vault
              .prepare(`DELETE FROM consent_access_grant WHERE grant_id = ?`)
              .run(wholeGrant);
            seedGrant([
