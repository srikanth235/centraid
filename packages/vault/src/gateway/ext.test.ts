...
describe('ext', () => {
  ...
  describe('apply + the write trio', () => {
    ...
    test('an app with an ext scope inserts, updates, reads and deletes; unknown columns refuse', () => {
      ...
      expect(gw.read(cred, { entity: `ext.${APP}.workout`, purpose: PURPOSE }).rows).toHaveLength(
        0,
      );
    });
  ...
  describe('uninstall and purge', () => {
    test('retain keeps rows but deregisters commands; purge drops everything', () => {
      ...
      expect(() => gw.sql(owner, { sql: 'SELECT * FROM ext_gym_log_workout' })).toThrow(
        'sql failed: no such table: ext_gym_log_workout',
      );
    });
  ...
  describe('export / import', () => {
    test('ext rows round-trip: tables recreated from specs, rows and hash intact', () => {
      ...
      expect(rows.map((row) => ({ ...row }))).toStrictEqual([{ notes: 'portable' }]);
      fresh.close();
    });
  });
    ...
    test('generated table DDL ends STRICT; a type-violating insert is rejected', () => {
      ...
      expect(() =>
        db.vault
          ...
          .run('w1', 'not-a-number'),
      ).toThrow('cannot store TEXT value in INTEGER column ext_gym_log_workout.reps');
    });
