...
describe("ext", () => {
  ...
  describe("apply + the write trio", () => {
    test("apply creates tables, registry rows and the typed commands", () => {
      ...
      expect(outcome.created.sort()).toStrictEqual(["gear", "workout"]);
      expect(resolveEntity(`ext.${APP}.workout`, db.vault)?.physical).toBe(
        "ext_gym_log_workout"
      );
  ...
  describe("drafts", () => {
    test("seed copies live rows; draft writes stay scratch; drop discards", () => {
      ...
      expect(
        resolveEntity(`extdraft.${APP}.workout`, db.vault)
      ).toBeUndefined();
    });
    ...
    test("a draft with a NEW table publishes as a live DDL diff", () => {
      ...
      const next = [
        ...specs(),
        {
          name: "plan",
          columns: [
            { name: "plan_id", type: "text" as const, primaryKey: true },
          ],
        },
      ];
  ...
  describe("uninstall and purge", () => {
    test("retain keeps rows but deregisters commands; purge drops everything", () => {
      ...
      expect(purge.purged.sort()).toStrictEqual(["gear", "workout"]);
      expect(resolveEntity(`ext.${APP}.workout`, db.vault)).toBeUndefined();
      expect(() =>
        gw.sql(owner, { sql: "SELECT * FROM ext_gym_log_workout" })
      ).toThrow("sql failed: no such table: ext_gym_log_workout");
    });
  ...
  describe("export / import", () => {
    test("ext rows round-trip: tables recreated from specs, rows and hash intact", () => {
      ...
      const rows = fresh.vault
        .prepare("SELECT notes FROM ext_gym_log_workout")
        .all() as {
        notes: string;
      }[];
  ...
  describe("STRICT + the JS-safe-integer bound", () => {
    test("generated table DDL ends STRICT; a type-violating insert is rejected", () => {
      ...
      expect(() =>
        db.vault
          .prepare(
            `INSERT INTO ext_gym_log_workout (workout_id, reps) VALUES (?, ?)`
          )
          .run("w1", "not-a-number")
      ).toThrow(
        "cannot store TEXT value in INTEGER column ext_gym_log_workout.reps"
      );
    });
    ...
    test("an ALTER-added integer column carries the same CHECK bound", () => {
      ...
      expect(() =>
        db.vault
          .prepare(
            `INSERT INTO ext_gym_log_workout (workout_id, duration_s) VALUES (?, ?)`
          )
