...
import { GatewayError, type ExecutionScopeSpec, type Identity } from './types.js';
...
test('a clamp that does not cover the entity denies before any grant is consulted', () => {
  grant([{ schema: 'core', verbs: 'read' }]);
  const decision = readTask(caller([{ schema: 'core', table: 'core_note', verbs: 'read' }]));
  expect(decision).toMatchObject({ decision: 'deny', grantId: null });
  if (decision.decision === 'deny') expect(decision.failing).toContain('execution manifest');
});
...
test('a clamp scope only covers the verb it grades for', () => {
  ...
  const identity = caller([{ schema: 'core', table: 'core_task', verbs: 'read' }]);
  expect(readTask(identity)).toMatchObject({ decision: 'allow' });
  expect(evaluateConsent(db.vault, identity, 'core', 'core_task', 'act', PURPOSE)).toMatchObject({
    decision: 'deny',
  });
});
