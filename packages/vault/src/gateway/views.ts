...
export function registerAppView(
  ...
  if (!base || base.file !== "vault") {
    throw new GatewayError(
      "contract",
      `unknown base entity ${options.baseEntity}`
    );
  }
  ...
  for (const join of options.definition.joins ?? []) {
    ...
    if (!joined || joined.file !== "vault") {
      throw new GatewayError(
        "contract",
        `unknown joined entity ${join.entity}`
      );
    }
...
export function queryAppView(
  ...
  if (!base)
    return deny(`view base entity ${view.base_entity} no longer resolves`);
  const definition = JSON.parse(view.definition_json) as ViewDefinition;

  const baseConsent = evaluateConsent(
    db.vault,
    identity,
    base.schema,
    base.table,
  ...
  if (baseConsent.decision === "deny")
    return deny(baseConsent.failing, baseConsent.grantId);

  // Clamp base columns: view columns ∩ grant field mask.
  const mask = baseConsent.fieldMask;
  const baseColumns = definition.columns.filter(
    (c) => mask === null || mask.includes(c)
  );
  ...
  for (const [i, join] of (definition.joins ?? []).entries()) {
    ...
    if (!joined)
      return deny(
        `joined entity ${join.entity} no longer resolves`,
        baseConsent.grantId
      );
    ...
    if (joinConsent.decision === "deny") {
      return deny(
        `join ${join.entity}: ${joinConsent.failing}`,
        baseConsent.grantId
      );
    }
    const joinMask = joinConsent.fieldMask;
    const joinColumns = join.columns.filter(
      (c) => joinMask === null || joinMask.includes(c)
    );
    ...
    if (!edge)
      return deny(
        `join ${join.entity} no longer follows a declared FK`,
        baseConsent.grantId
      );
    const alias = `j${i}`;
    joins.push(
    ...
    for (const c of joinColumns)
      selects.push(`${alias}."${c}" AS "${joined.table}_${c}"`);
  }
  const now = nowIso();
  const scopeFilter = compileFilters(
    db.vault,
    base.physical,
    baseConsent.rowFilter,
    now,
    "b"
  );
  const viewFilter = compileFilters(
    db.vault,
    base.physical,
    definition.where ?? [],
    now,
  ...
  const rows = db.vault
    ...
    .all(...scopeFilter.params, ...viewFilter.params) as Record<
    string,
    unknown
  >[];
  const receiptId = writeReceipt(db.journal, {
