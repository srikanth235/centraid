...
function assistant(onBehalfOfMember?: { memberId: string; mayAct: boolean }): Credential {
...
test('a write records the acting member id, and the id is what survives a rename', () => {
  ...
  expect(receiptDetail('act schedule.propose_event')).toMatchObject({ actingMember: SID });
  ...
  expect(JSON.stringify(receiptDetail('act schedule.propose_event'))).not.toContain('Sid');
});
...
test('an unattributed write journals no member rather than inventing one', () => {
  ...
  expect(receiptDetail('act schedule.propose_event')).not.toHaveProperty('actingMember');
});
...
test('an agent for a read-role member is refused the write the member could not make', () => {
  ...
  if (outcome.status === 'denied') expect(outcome.reason).toContain(SID);
...
test('an agent for a write-role member acts, and the receipt names both', () => {
  ...
  const provenance = db.journal
    .prepare(`SELECT agent_kind FROM consent_provenance ORDER BY prov_id DESC LIMIT 1`)
...
test('the cap bites reads never — a read-role member still reads through their agent', () => {
  ...
  expect(() => gw.read(cred, { entity: 'schedule.calendar', purpose: PURPOSE })).not.toThrow();
});
