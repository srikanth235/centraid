...
describe("links", () => {
  ...
  function liveLink(linkId: string) {
    return db.vault
      ...
      .get(linkId) as
      | { valid_to: string | null; asserted_by: string }
      | undefined;
  }
  ...
  test("an unknown relation notation is refused by precondition — vocabulary, never caller-invented", () => {
    ...
    expect(out.predicate).toContain("relation_in_scheme");
  });
  ...
  test("an identical live link is refused; after unlink the relation may be asserted again", () => {
    ...
    expect(dup.predicate).toContain("no_identical_live_link");
  ...
  test("linking to a missing endpoint, an unknown entity, itself, or another link is refused", () => {
    ...
    expect(ghost.reason).toContain("no schedule.task");
    ...
    expect(unknown.reason).toContain("unknown entity");
    ...
    expect(self.reason).toContain("cannot link to itself");
  ...
  function anchorOf(linkId: string) {
    return db.vault
      .prepare(
        "SELECT anchor_id, selector_json FROM core_link_anchor WHERE link_id = ?"
      )
  ...
  test("link_entities with a selector writes the anchor atomically with the link", () => {
    ...
    expect(JSON.parse(anchor?.selector_json ?? "{}")).toStrictEqual(SELECTOR);
  });
  ...
  test("a malformed selector is refused at the input schema, link unwritten", () => {
    ...
    const n = db.vault
      .prepare("SELECT count(*) AS n FROM core_link_anchor")
      .get() as { n: number };
    expect(n.n).toBe(0);
  });
  ...
  test("anchor_link upserts the one anchor a link carries — a re-baseline moves it in place", () => {
    ...
    expect(JSON.parse(after?.selector_json ?? "{}")).toStrictEqual(moved);
    const n = db.vault
      .prepare("SELECT count(*) AS n FROM core_link_anchor")
      .get() as { n: number };
    expect(n.n).toBe(1);
  ...
  test("anchor_link can attach an anchor to a link created without one (re-anchor an orphaned edge)", () => {
    ...
    expect(out.status).toBe("executed");
    expect(
      (out as { output: { anchor_id?: string } }).output.anchor_id
    ).toBeTruthy();
  ...
  test("a link_anchor is not a linkable endpoint — locators are not entities", () => {
    ...
    expect(out.status).toBe("failed");
    assert(out.status === "failed");
    expect(out.reason).toContain("links do not link links");
  });
});
