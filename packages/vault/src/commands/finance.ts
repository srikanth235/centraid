...
function categorizeTxn(ctx: HandlerCtx): Record<string, unknown> {
  ...
  ctx.db
    .prepare(
      "UPDATE core_transaction SET category_concept_id = ? WHERE txn_id = ?"
    )
...
function splitTxn(ctx: HandlerCtx): Record<string, unknown> {
  ...
  ctx.db
    .prepare("DELETE FROM finance_txn_split WHERE txn_id = ?")
    .run(input.txn_id);
...
function setBudget(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (existing) {
    ...
    ctx.db
      .prepare(
        "UPDATE finance_budget SET limit_minor = ?, currency = ? WHERE budget_id = ?"
      )
...
function removeBudget(ctx: HandlerCtx): Record<string, unknown> {
  ...
  ctx.db
    .prepare("DELETE FROM finance_budget WHERE budget_id = ?")
    .run(input.budget_id);
...
function flagAnomaly(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (!concept)
    throw new Error(
      `vocabulary missing 'anomaly' concept — reseed flags scheme`
    );
