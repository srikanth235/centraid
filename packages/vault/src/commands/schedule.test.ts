...
describe("schedule", () => {
  ...
  test("propose_event stores rrule, conferencing_uri and reminders", () => {
    ...
    const ext = db.vault
      .prepare(
        "SELECT conferencing_uri, reminders_json FROM schedule_event_ext WHERE event_id = ?"
      )
    ...
    expect(JSON.parse(ext.reminders_json)).toStrictEqual([
      { minutes_before: 10 },
      { minutes_before: 0 },
    ]);
  });
  ...
  test("an iCalendar chair, when present, must be the event organizer", () => {
    ...
    expect(() =>
      db.vault
        .prepare(
          "UPDATE core_event SET organizer_party_id = ? WHERE event_id = ?"
        )
