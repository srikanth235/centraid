...
describe("health", () => {
  ...
  test("log_vital: the reading IS an observation, plus clinical columns (R02)", () => {
    ...
    const vital = db.vault
      .prepare(
        "SELECT vital_type, loinc_code, context FROM health_vital WHERE vital_id = ?"
      )
      .get(vital_id);
  ...
  test("log_vital refuses non-positive values at the contract", () => {
    ...
    expect(outcome.predicate).toContain("value_is_positive");
  });
  ...
  test("void_vital: a second void fails loudly — entered-in-error is terminal", () => {
    ...
    expect(again.predicate).toContain("not_already_voided");
  });
  ...
  test("void_vital refuses an observation that does not exist", () => {
    ...
    expect(outcome.status).toBe("failed");
    assert(outcome.status === "failed");
    expect(outcome.predicate).toContain("observation_exists");
  });
  ...
  test("import_workout: one span, two lenses — activity in core, load in health", () => {
    ...
    expect(outcome.status).toBe("executed");
    if (outcome.status !== "executed") return;
    const { activity_id, workout_id } = outcome.output as {
      activity_id: string;
      workout_id: string;
    };
    const activity = db.vault
      .prepare(
        "SELECT actor_party_id, kind_concept_id FROM core_activity WHERE activity_id = ?"
      )
      .get(activity_id);
    ...
    const workout = db.vault
      .prepare(
        "SELECT activity_id, distance_m, avg_hr FROM health_workout WHERE workout_id = ?"
      )
      .get(workout_id);
  ...
  test("adjust_course: dose change keeps the course active and flags reminders stale", () => {
    ...
    const course = db.vault
      .prepare(
        "SELECT dose_text, ended_at FROM health_medication_course WHERE course_id = ?"
      )
      .get(courseId);
  ...
  test("adjust_course: stop ends the course; a stopped course never transitions again", () => {
    ...
    expect(again.status).toBe("failed");
    assert(again.status === "failed");
    expect(again.predicate).toContain("course_active");
  });
  ...
  test("summarize_trends: the 90-day picture becomes an owned content item with cited stats", () => {
    ...
    const content = db.vault
      .prepare(
        "SELECT media_type, title FROM core_content_item WHERE content_id = ?"
      )
      .get(output.content_id);
    expect(content).toMatchObject({
      media_type: "application/json",
      title: "heart_rate trend, 90d",
    });
    const evidence = db.journal
      .prepare("SELECT claim FROM agent_evidence WHERE invocation_id = ?")
      .get(outcome.invocationId) as { claim: string };
  ...
  test("summarize_trends refuses an empty window — a trend over nothing is noise", () => {
    ...
    expect(outcome.status).toBe("failed");
    assert(outcome.status === "failed");
    expect(outcome.predicate).toContain("readings_exist_in_window");
  });
  ...
  test("condition rows are excluded from default scopes: schema-wide grant denies, explicit scope allows", () => {
    ...
    expect(
      gw.read(cred, { entity: "health.vital", purpose: "dpv:HealthMonitoring" })
        .rows
    ).toStrictEqual([]);
