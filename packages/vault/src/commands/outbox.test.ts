...
describe("outbox", () => {
  ...
  function itemRow(itemId: string): Record<string, unknown> {
    return db.vault
      .prepare("SELECT * FROM outbox_item WHERE item_id = ?")
  ...
  describe("outbox.stage", () => {
    ...
    test("staging toward an unknown connection fails — no orphan artifacts", () => {
      const outcome = invoke(
        agent,
        "outbox.stage",
        stageInput({ label: "nope" })
      );
      expect(outcome.status).toBe("failed");
      const n = db.vault
        .prepare("SELECT count(*) AS n FROM outbox_item")
        .get() as { n: number };
      expect(n.n).toBe(0);
    });
  });
  ...
  describe("outbox.decide", () => {
    ...
    test("edit-then-send: the decision may replace the artifact and request", () => {
      ...
      expect(JSON.parse(String(row.request_json)).body).toBe(
        '{"raw":"edited"}'
      );
    });
  ...
  describe("standing grants (issue #306 phase 3)", () => {
    test("always-allow mints the (actor, verb, target) rule; the next matching item auto-approves", () => {
      ...
      const other = invoke(
        agent,
        "outbox.stage",
        stageInput({ target: "meera@example.com" })
      );
    ...
    test("revocation retro-invalidates: approved-but-undrained items park back to pending (issue #308 A8)", () => {
      ...
      expect(revoked.status).toBe("executed");
      expect(
        (revoked as { output?: { reparked?: number } }).output?.reparked
      ).toBe(3);
      for (const outcome of [first, second, third]) {
  ...
  describe("outbox graph joins", () => {
    ...
    test("stage resolves the wire address to a known party and validates subject refs", () => {
      ...
      expect(
        invoke(
          agent,
          "outbox.stage",
          stageInput({ subject_type: "core.party" })
        ).status
      ).toBe("failed");
      expect(
        invoke(
          agent,
          "outbox.stage",
          stageInput({ subject_type: "no.such", subject_id: "x" })
        ).status
      ).toBe("failed");
      expect(
        invoke(
          agent,
          "outbox.stage",
          stageInput({ subject_type: "core.party", subject_id: "ghost" })
        ).status
      ).toBe("failed");
    });
    ...
    test("a sent message-shaped artifact publishes into the social spine", () => {
      ...
      const thread = db.vault
        .prepare(
          "SELECT channel, external_ref FROM social_thread WHERE thread_id = ?"
        )
      ...
      expect(
        parts.some(
          (p) => p.party_id === null && p.handle === "ravi@example.com"
        )
      ).toBe(true);
    });
    ...
    test("a non-message artifact (no body text) drains without publishing", () => {
      ...
      const n = db.vault
        .prepare("SELECT count(*) AS n FROM social_message")
        .get() as { n: number };
      expect(n.n).toBe(0);
    });
  });
