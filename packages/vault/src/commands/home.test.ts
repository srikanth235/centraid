...
describe("home", () => {
  ...
  test("add_item lands an owned, undisposed asset item", () => {
    ...
    const item = db.vault
      .prepare("SELECT * FROM home_asset_item WHERE item_id = ?")
      .get(itemId);
  ...
  test("update_item revises fields it was asked to and nothing else", () => {
    ...
    const item = db.vault
      .prepare(
        "SELECT name, serial_no, acquired_on FROM home_asset_item WHERE item_id = ?"
      )
      .get(itemId);
    expect(item).toMatchObject({
      name: "Road bike",
  ...
  test("dispose_item stamps the lifecycle date once; a second disposal is refused", () => {
    ...
    expect(again.predicate).toContain("item_exists_and_owned");
  });
  ...
  test("add_warranty decorates an item and refuses an inverted term", () => {
    ...
    expect(outcome.status).toBe("executed");
    const warrantyId = (outcome as { output: { warranty_id: string } }).output
      .warranty_id;
    const warranty = db.vault
      .prepare(
        "SELECT item_id, ends_on FROM home_warranty WHERE warranty_id = ?"
      )
      .get(warrantyId);
    ...
    expect(inverted.predicate).toContain("ends_not_before_starts");
  });
  ...
  test("a bound purchase is a faithful transaction projection and cannot drift", () => {
    ...
    expect(() =>
      db.vault
        .prepare(
          "UPDATE core_transaction SET currency = 'USD' WHERE txn_id = 'home-purchase'"
        )
        .run()
    ...
    expect(() =>
      db.vault
        .prepare(
          "UPDATE home_asset_item SET purchase_price_minor = 1 WHERE item_id = ?"
        )
        .run(bound)
  ...
  test("complete_maintenance stamps last_done_on; a missing plan is refused", () => {
    ...
    const plan = db.vault
      .prepare(
        "SELECT last_done_on FROM home_maintenance_plan WHERE plan_id = ?"
      )
