...
describe("attachments", () => {
  ...
  test("identical bytes dedupe on sha256; a second attachment reuses the content item and is not primary", () => {
    ...
    const so = (
      second as { output: { content_id: string; is_primary: number } }
    ).output;
  ...
  test("a second file on the same subject is not primary", () => {
    ...
    const roles = db.vault
      .prepare(
        "SELECT role FROM core_attachment WHERE target_id = ? ORDER BY is_primary DESC"
      )
      .all(taskId) as { role: string }[];
    expect(roles.map((r) => r.role)).toStrictEqual(["photo", "other"]); // text defaults to 'other'
  });
  ...
  test("attaching to a non-existent subject is a receipted refusal", () => {
    ...
    expect(out.reason).toContain("no schedule.task");
  });
  ...
  test("a non-data URI and an oversized file are both refused by precondition", () => {
    ...
    assert(notData.status === "failed");
    expect(notData.predicate).toContain("is_data_uri");
    ...
    assert(tooBig.status === "failed");
    expect(tooBig.predicate).toContain("within_size_cap");
  });
  ...
  test("detach removes the edge but leaves the canonical content item", () => {
    ...
    const gone = db.vault
      .prepare(
        "SELECT count(*) AS n FROM core_attachment WHERE attachment_id = ?"
      )
      .get(attachment_id) as { n: number };
    expect(gone.n).toBe(0);
    const kept = db.vault
      .prepare(
        "SELECT count(*) AS n FROM core_content_item WHERE content_id = ?"
      )
  ...
  test("detach on an unknown attachment is refused by precondition", () => {
    ...
    expect(out.status).toBe("failed");
    assert(out.status === "failed");
    expect(out.predicate).toContain("attachment_exists");
  });
  ...
  test("attach by content_id pins an EXISTING content item — no re-upload (issue #272)", () => {
    ...
    const contentId = (minted as { output: { content_id: string } }).output
      .content_id;
    ...
    const out = (
      reused as { output: { content_id: string; is_primary: number } }
    ).output;
    expect(out.content_id).toBe(contentId);
    expect(out.is_primary).toBe(1); // first edge on subject B
    const role = db.vault
      .prepare("SELECT role FROM core_attachment WHERE target_id = ?")
      .get(b) as {
      role: string;
    };
  ...
  test("attach requires exactly one source: neither and both are refused", () => {
    ...
    expect(both.predicate).toContain("exactly_one_source");
  });
  ...
  test("attach by content_id refuses unknown and trashed content items", () => {
    ...
    expect(ghost.predicate).toContain("content_exists");
    ...
    assert(trashed.status === "failed");
    expect(trashed.predicate).toContain("content_exists");
  });
});
