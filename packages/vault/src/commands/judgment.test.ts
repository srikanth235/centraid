...
describe("judgment", () => {
  ...
  test("record_correction validates the target and stamps the acting party", () => {
    ...
    const row = db.vault
      .prepare(
        "SELECT corrected_by_party_id, reason FROM agent_correction WHERE correction_id = ?"
      )
