...
function addItem(ctx: HandlerCtx): Record<string, unknown> {
  ...
  const transaction = input.acquired_txn_id
    ? (ctx.db
        .prepare(
          "SELECT amount_minor, currency FROM core_transaction WHERE txn_id = ?"
        )
...
function updateItem(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (
    input.purchase_price_minor !== undefined &&
    input.acquired_txn_id === undefined
  ) {
    sets.push("purchase_price_minor = ?");
    values.push(input.purchase_price_minor);
  }
  if (
    input.purchase_currency !== undefined &&
    input.acquired_txn_id === undefined
  ) {
  ...
  if (sets.length > 0) {
    ctx.db
      .prepare(
        `UPDATE home_asset_item SET ${sets.join(", ")} WHERE item_id = ?`
      )
...
function completeMaintenance(ctx: HandlerCtx): Record<string, unknown> {
  ...
  ctx.db
    .prepare(
      "UPDATE home_maintenance_plan SET last_done_on = ? WHERE plan_id = ?"
    )
