...
describe("business", () => {
  ...
  >(command: string, input: Record<string, unknown>): T {
  ...
  function logTime(
    ...
    return expectExecuted<{ entry_id: string; activity_id: string }>(
      "business.log_time",
      {
  ...
  test("update_client moves a lead through the pipeline and revises the rate", () => {
    ...
    const row = db.vault
      .prepare(
        "SELECT status, default_rate_minor FROM business_client WHERE client_id = ?"
      )
      .get(clientId);
  ...
  test("log_time refuses inactive projects and inverted intervals", () => {
    const clientId = addClient();
    const proposed = expectExecuted<{ project_id: string }>(
      "business.add_project",
      {
        client_id: clientId,
        name: "Maybe later",
        status: "proposed",
    ...
    expect(early.predicate).toContain("project_exists_and_active");
    ...
    assert(inverted.status === "failed");
    expect(inverted.predicate).toContain("interval_is_positive");
  });
  ...
  test("create_draft_invoice bills unbilled entries, totals reconcile, entries get marked", () => {
    ...
    const invoice = db.vault
      .prepare(
        "SELECT status, total_minor, currency FROM business_invoice WHERE invoice_id = ?"
      )
      .get(out.invoice_id);
  ...
  test("invoice totals stay derived when lines change and reject direct drift", () => {
    ...
    const { invoice_id } = expectExecuted<InvoiceDraft>(
    ...
    db.vault
      .prepare(
        "UPDATE business_invoice_line SET amount_minor = 12345 WHERE line_id = ?"
      )
      .run(line.line_id);
    expect(
      (
        db.vault
          .prepare(
            "SELECT total_minor FROM business_invoice WHERE invoice_id = ?"
          )
    ...
    expect(() =>
      db.vault
        .prepare(
          "UPDATE business_invoice SET total_minor = 1 WHERE invoice_id = ?"
        )
        .run(invoice_id)
  ...
  test("business_invoice_line carries a NOT NULL qty_scale and CHECKs its scale + amount (issue #441 A3)", () => {
    const cols = db.vault
      .prepare("PRAGMA table_info('business_invoice_line')")
      .all() as {
      name: string;
      notnull: number;
    }[];
    ...
    const ddl = (
      db.vault
        .prepare(
          "SELECT sql FROM sqlite_master WHERE type='table' AND name='business_invoice_line'"
        )
        .get() as { sql: string }
    ).sql;
  ...
  test("double-billing a time entry is a receipted refusal that rolls everything back", () => {
    ...
    expect(again.status).toBe("failed");
    assert(again.status === "failed");
    expect(again.reason).toContain("already invoiced");
  ...
  test("invoice numbers are sequential per year", () => {
    ...
    const first = expectExecuted<InvoiceDraft>(
      "business.create_draft_invoice",
      {
        client_id: clientId,
        entry_ids: [e1.entry_id],
    ...
    const second = expectExecuted<InvoiceDraft>(
      "business.create_draft_invoice",
      {
        client_id: clientId,
  ...
  test("send_invoice moves draft → sent once; a second send is refused", () => {
    ...
    expect(again.status).toBe("failed");
    assert(again.status === "failed");
    expect(again.predicate).toContain("invoice_is_draft");
  });
  ...
  test("mark_invoice_paid links a plausible posted credit and refuses implausible ones", () => {
    ...
    expect(short.predicate).toContain("transaction_plausibly_settles");
    ...
    const row = db.vault
      .prepare(
        "SELECT status, paid_txn_id FROM business_invoice WHERE invoice_id = ?"
      )
      .get(invoice_id);
