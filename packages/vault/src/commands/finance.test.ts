...
describe("finance", () => {
  ...
  test("categorize_txn changes classification, never the amount", () => {
    ...
    const txn = db.vault
      .prepare(
        "SELECT category_concept_id, amount_minor FROM core_transaction WHERE txn_id = ?"
      )
      .get(txnId);
  ...
  test("split_txn: the ₹4,200 order that was groceries + a gift (Σ holds)", () => {
    ...
    const sum = db.vault
      .prepare(
        "SELECT SUM(amount_minor) AS s, count(*) AS n FROM finance_txn_split WHERE txn_id = ?"
      )
      .get(txnId);
  ...
  test("split_txn Σ-invariant: splits that do not sum to the parent roll back entirely", () => {
    ...
    expect(outcome.predicate).toContain("splits_sum_to_parent");
    // Rollback proof: nothing landed.
    const splits = db.vault
      .prepare("SELECT count(*) AS n FROM finance_txn_split")
      .get() as {
      n: number;
    };
    ...
    const inv = db.journal
      .prepare(
        `SELECT count(*) AS n FROM agent_command_invocation WHERE status='rolled_back'`
      )
      .get() as { n: number };
    expect(inv.n).toBe(1);
  });
  ...
  test("set_budget upserts on (owner, category, period, starts_on)", () => {
    ...
    const budgets = db.vault
      .prepare("SELECT limit_minor FROM finance_budget")
      .all();
    // node:sqlite hands back null-prototype rows; spreading compares the column
    // data (which is the contract) without asserting the driver's prototype.
    expect(budgets.map((row) => ({ ...row }))).toStrictEqual([
      { limit_minor: 1200000 },
    ]);
  });
  ...
  test("remove_budget deletes the cap; the ledger is untouched", () => {
    ...
    const budgetId = (set as { output: { budget_id: string } }).output
      .budget_id;
    const outcome = gw.invoke(owner, {
    ...
    expect(outcome.status).toBe("executed");
    const budgets = db.vault
      .prepare("SELECT count(*) AS n FROM finance_budget")
      .get() as {
      n: number;
    };
  ...
  test("remove_budget refuses a budget that does not exist", () => {
    ...
    expect(outcome.status).toBe("failed");
    assert(outcome.status === "failed");
    expect(outcome.predicate).toContain("budget_exists");
  });
  ...
  test("a removed budget can be set again (upsert inserts a fresh row)", () => {
    ...
    const firstId = (first as { output: { budget_id: string } }).output
      .budget_id;
    gw.invoke(owner, {
      command: "finance.remove_budget",
    ...
    expect(second.status).toBe("executed");
    const budgets = db.vault
      .prepare("SELECT budget_id, limit_minor FROM finance_budget")
      .all() as {
      budget_id: string;
      limit_minor: number;
    }[];
  ...
  test("flag_anomaly tags the transaction once; a second flag is refused", () => {
    const outcome = gw.invoke(owner, {
      ...
      input: {
        txn_id: txnId,
        reason: "amount 3x the recurring series expectation",
        confidence: 0.9,
      },
      purpose: "dpv:ServiceProvision",
    });
    ...
    expect(again.predicate).toContain("not_already_flagged");
  });
});
