...
function updateClient(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (sets.length > 0) {
    ctx.db
      .prepare(
        `UPDATE business_client SET ${sets.join(", ")} WHERE client_id = ?`
      )
...
function createDraftInvoice(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (input.entry_ids.length === 0)
    throw new Error("an invoice needs at least one time entry");
  const client = ctx.db
  ...
  const entryRows = input.entry_ids.map((entryId) => {
    ...
    if (row.billable !== 1)
      throw new Error(`time entry ${entryId} is not billable`);
    if (row.invoice_line_id !== null)
      throw new Error(`time entry ${entryId} is already invoiced`);
    if (row.rate_minor === null)
      throw new Error(
        `time entry ${entryId} has no rate and the client has no default`
      );
  ...
  for (const row of entryRows) {
    ...
    ctx.db
      .prepare(
        "UPDATE business_time_entry SET invoice_line_id = ? WHERE entry_id = ?"
      )
...
function markInvoicePaid(ctx: HandlerCtx): Record<string, unknown> {
  ...
  ctx.db
    .prepare(
      `UPDATE business_invoice SET status = 'paid', paid_txn_id = ? WHERE invoice_id = ?`
    )
