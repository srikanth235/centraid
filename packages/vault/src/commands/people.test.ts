...
describe("people", () => {
  ...
  function addPerson(input: Record<string, unknown> = {}): string {
    const o = invoke("people.add_person", {
  ...
  function createList(name: string): string {
    return out<{ list_id: string }>(invoke("people.create_list", { name }))
      .list_id;
  }
  ...
  test("add_person mints a canonical person party plus its 1:1 profile", () => {
    ...
    const profile = db.vault
      .prepare(
        "SELECT role, avatar_color, cadence_days FROM people_profile WHERE party_id = ?"
      )
      .get(partyId);
  ...
  test("set_cadence updates the keep-in-touch interval", () => {
    const partyId = addPerson();
    expect(
      invoke("people.set_cadence", { party_id: partyId, cadence_days: 7 })
        .status
    ).toBe("executed");
    const profile = db.vault
  ...
  test("star/unstar are the canonical flags-scheme tag on the party (idempotent)", () => {
    const partyId = addPerson();
    expect(invoke("people.star_person", { party_id: partyId }).status).toBe(
      "executed"
    );
    expect(invoke("people.star_person", { party_id: partyId }).status).toBe(
      "executed"
    ...
    expect(invoke("people.unstar_person", { party_id: partyId }).status).toBe(
      "executed"
    );
  ...
  test("move_person re-files into one list and un-lists when omitted", () => {
    ...
    expect(
      invoke("people.move_person", { party_id: partyId, list_id: close }).status
    ).toBe("executed");
    expect(listOf(partyId)).toBe(close);
    expect(
      invoke("people.move_person", { party_id: partyId, list_id: family })
        .status
    ).toBe("executed");
    ...
    expect(invoke("people.move_person", { party_id: partyId }).status).toBe(
      "executed"
    );
  ...
  test("add_note lands an annotation on the party (searchable owner memo)", () => {
    const partyId = addPerson();
    expect(
      invoke("people.add_note", {
        party_id: partyId,
        text: "Loves ceramics — ask about the studio.",
      }).status
    ).toBe("executed");
    const note = db.vault
      .prepare(
  ...
  test("important dates: a birthday auto-creates its reminder; toggle flips it", () => {
    ...
    const reminderOf = () =>
      (
        db.vault
          .prepare(
            "SELECT reminder_on FROM people_important_date WHERE date_id = ?"
          )
          .get(dateId) as { reminder_on: number }
      ).reminder_on;
    expect(reminderOf()).toBe(1);
    expect(invoke("people.toggle_reminder", { date_id: dateId }).status).toBe(
      "executed"
    );
  ...
  test("lists create with unique names, rename, and delete only when empty", () => {
    ...
    expect(twin.predicate).toContain("name_unused");
    expect(
      invoke("people.rename_list", { list_id: work, name: "Colleagues" }).status
    ).toBe("executed");
    const partyId = addPerson({ list_id: work });
    const nonEmpty = invoke("people.delete_list", { list_id: work });
    expect(nonEmpty.status).toBe("failed");
    assert(nonEmpty.status === "failed");
    expect(nonEmpty.predicate).toBe(
      "This list still has people in it — move them out first."
    );
    expect(invoke("people.move_person", { party_id: partyId }).status).toBe(
      "executed"
    );
    expect(invoke("people.delete_list", { list_id: work }).status).toBe(
      "executed"
    );
  });
