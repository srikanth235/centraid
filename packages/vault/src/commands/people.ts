...
function editPerson(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (input.display_name !== undefined) {
    ctx.db
      .prepare(
        "UPDATE core_party SET display_name = ?, updated_at = ? WHERE party_id = ?"
      )
  ...
  if (sets.length > 0) {
    ctx.db
      .prepare(
        `UPDATE people_profile SET ${sets.join(", ")} WHERE party_id = ?`
      )
...
function logInteraction(ctx: HandlerCtx): Record<string, unknown> {
  ...
  ctx.db
    .prepare(
      "UPDATE people_profile SET last_contacted_at = ? WHERE party_id = ?"
    )
...
const ADD_IMPORTANT_DATE: CommandDefinition = {
  ...
  handler: (ctx) => {
    ...
    if (isBirthday) {
      ...
      const yearPrefix =
        existing && /^\d{4}-\d{2}-\d{2}$/u.test(existing)
          ? existing.slice(0, 4)
          : "-";
      ...
      if (birthDate !== existing) {
        ctx.db
          .prepare(
            "UPDATE core_party SET birth_date = ?, updated_at = ? WHERE party_id = ?"
          )
...
const TOGGLE_REMINDER: CommandDefinition = {
  ...
  handler: (ctx) => {
    ...
    ctx.db
      .prepare(
        "UPDATE people_important_date SET reminder_on = 1 - reminder_on WHERE date_id = ?"
      )
      .run(input.date_id);
...
const SETTLE_DEBT: CommandDefinition = {
  ...
  handler: (ctx) => {
    ...
    ctx.db
      .prepare(
        "UPDATE tally_obligation SET settled_at = ? WHERE obligation_id = ?"
      )
