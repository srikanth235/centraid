...
describe("tags", () => {
  ...
  test("tag_item creates a shared Tags scheme + concept and records the edge", () => {
    ...
    const concept = db.vault
      .prepare(
        "SELECT pref_label, notation FROM core_concept WHERE concept_id = ?"
      )
    ...
    const tag = db.vault
      .prepare(
        "SELECT target_type, target_id, tagged_by_party_id FROM core_tag WHERE tag_id = ?"
      )
      .get(output.tag_id);
  ...
  test("the same label tags a task, a note, a document and a media asset under one shared concept", () => {
    ...
    for (const out of [onTask, onNote, onDocument, onAsset])
      expect(out.status).toBe("executed");
  ...
  test("labels are readable through the standard entity read path (no dedicated query needed)", () => {
    ...
    const labels = concepts
      .filter((c) => labelIds.has(c.concept_id))
      .map((c) => c.pref_label);
    expect(labels).toStrictEqual(["Taxes"]);
  });
});
