...
describe("sync", () => {
  ...
  test("agent stages freely; publish parks; owner approval lands the rows", () => {
    ...
    expect((confirmed as { output: { created: number } }).output.created).toBe(
      1
    );
  ...
  test("remote files publish as content items rather than fabricated messages", () => {
    ...
    const published = gw.confirm(
      owner,
      (proposed as { invocationId: string }).invocationId,
      true
    );
  ...
  describe("connection lifecycle (phase 4)", () => {
    ...
    test("an explicit connection id survives label changes without creating a shadow row", () => {
      ...
      const connectionId = (first as { output: { connection_id: string } })
      ...
      expect(
        (rebound as { output: { connection_id: string } }).output.connection_id
      ).toBe(connectionId);
      const staged = gw.invoke(agent, {
        command: "sync.stage_rows",
      ...
      expect(
        (
          db.vault
            .prepare(
              `SELECT count(*) AS n FROM sync_connection WHERE kind = 'mcp.gmail'`
            )
            .get() as { n: number }
        ).n
      ).toBe(1);
    ...
    test("a failed run flips the connection to failing; a good one restores it", () => {
      ...
      const run = db.vault
        .prepare(
          "SELECT status, error FROM sync_connection_run WHERE run_id = ?"
        )
    ...
    test("paused means paused — begin_run refuses until the owner resumes", () => {
      ...
      expect((refused as { output: { refused: string } }).output.refused).toBe(
        "paused"
      );
      gw.invoke(owner, {
        command: "sync.set_connection_status",
    ...
    test("cursors persist across runs and come back on begin_run", () => {
      ...
      expect(
        (next as { output: { cursors: Record<string, unknown> } }).output
          .cursors
      ).toStrictEqual({
        history_id: { id: 42017 },
      });
  ...
  describe("sync.configure_credential + sync.store_tokens (issue #304)", () => {
    test("oauth2 configure seals the client secret, pins hosts, starts in needs-auth", () => {
      ...
      const cred = db.vault
        .prepare(
          "SELECT cred_kind, client_secret, allowed_hosts FROM sync_connection_credential"
        )
        .get() as {
        cred_kind: string;
      ...
      const conn = db.vault
        .prepare("SELECT status FROM sync_connection")
        .get() as {
        status: string;
      };
    ...
    test("Assist records its mode without accepting or storing a shared client secret", () => {
      ...
      const row = db.vault
        .prepare(
          "SELECT oauth_mode, client_id, client_secret FROM sync_connection_credential"
        )
        .get() as {
        oauth_mode: string;
        client_id: string;
    ...
    test("api_key configure seals the key and the connection is live immediately", () => {
      ...
      expect(outcome.status).toBe("executed");
      const cred = db.vault
        .prepare("SELECT api_key FROM sync_connection_credential")
        .get() as {
        api_key: string;
      };
  ...
  describe("credential commands are confirm-gated (issue #308 A1/A2)", () => {
    test("an agent proposing configure_credential parks; owner approval lands it", () => {
      ...
      expect(
        (
          db.vault
            .prepare("SELECT allowed_hosts FROM sync_connection_credential")
            .get() as {
            allowed_hosts: string;
          }
        ).allowed_hosts
  ...
  describe("sync.remove_connection", () => {
    ...
    test("happy path: deletes the connection, its credential, health and cursor rows, and receipts the act", () => {
      ...
      expect(
        db.vault.prepare("SELECT * FROM sync_connection_cursor").get()
      ).toBeUndefined();
    });
