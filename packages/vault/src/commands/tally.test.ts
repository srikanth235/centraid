...
describe("tally", () => {
  ...
  test("delete_group is refused while it holds an expense — a TRASHED expense still blocks until it purges", () => {
    ...
    expect(
      (
        db.vault
          .prepare(
            "SELECT count(*) AS n FROM tally_expense WHERE expense_id = ?"
          )
          .get(xid) as {
          n: number;
        }
    ...
    expect(
      (
        db.vault
          .prepare(
            "SELECT count(*) AS n FROM tally_expense_split WHERE expense_id = ?"
          )
          .get(xid) as { n: number }
      ).n
    ).toBe(0);
  ...
  test("settle_up toward the owner posts a credit; friend-to-friend stays tally-only", () => {
    ...
    const count = db.vault
      .prepare(
        `SELECT count(*) AS n FROM core_transaction WHERE external_id LIKE 'tally:%'`
      )
      .get() as { n: number };
