...
const VITAL_CODES: Record<
  string,
  { code: string; loinc: string; unit: string }
> = {
...
function subjectPartyId(ctx: HandlerCtx): string {
  if (ctx.identity.kind === "owner-device" && ctx.identity.partyId)
    return ctx.identity.partyId;
  const owner = ctx.db
...
function logVital(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (!mapping)
    throw new Error(
      `no code mapping for vital_type ${String(input.vital_type)}`
    );
...
function voidVital(ctx: HandlerCtx): Record<string, unknown> {
  ...
  ctx.db
    .prepare(
      `UPDATE core_observation SET status = 'entered-in-error' WHERE observation_id = ?`
    )
...
function adjustCourse(ctx: HandlerCtx): Record<string, unknown> {
  ...
  if (input.action === "stop") {
    ctx.db
      .prepare(
        "UPDATE health_medication_course SET ended_at = ? WHERE course_id = ?"
      )
    ...
    ctx.db
      ...
      .run(
        input.dose_text ?? null,
        input.schedule_rrule ?? null,
        input.course_id
      );
  }
...
function summarizeTrends(ctx: HandlerCtx): Record<string, unknown> {
  ...
  const cutoff = new Date(
    Date.parse(ctx.now) - input.days * 86_400_000
  ).toISOString();
  const stats = ctx.db
    .prepare(
