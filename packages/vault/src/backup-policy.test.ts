...
describe("BackupPolicy", () => {
  ...
  it("rejects an RPO below the declared floor and invalid acknowledgement modes", () => {
    expect(() =>
      resolveBackupPolicy({ rpoSeconds: MIN_RPO_SECONDS - 1 })
    ).toThrow(BackupPolicyError);
  ...
  it("treats an empty/whitespace storageClass as unset, not an explicit class", () => {
    ...
    expect(() => resolveBackupPolicy({ storageClass: 7 })).toThrow(
      /storageClass/u
    );
  });
  ...
  it("validates the directToColdOriginals shape", () => {
    ...
    expect(() =>
      resolveBackupPolicy({ directToColdOriginals: { enabled: "yes" } })
    ).toThrow(/enabled/u);
    expect(() =>
      resolveBackupPolicy({ directToColdOriginals: { minBytes: -1 } })
    ).toThrow(/minBytes/u);
    expect(() =>
      resolveBackupPolicy({ directToColdOriginals: { minBytes: 1.5 } })
    ).toThrow(/minBytes/u);
    expect(() =>
      resolveBackupPolicy({
