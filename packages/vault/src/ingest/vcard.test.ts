...
test('normalizeHandle lowercases emails and strips tel separators', () => {
  expect(normalizeHandle('email', '  Ravi@Example.COM ')).toBe('ravi@example.com');
...
test('parseVcards extracts FN, N, BDAY, EMAIL, TEL with labels', () => {
  ...
  expect(cards[1]).toEqual({
    ...
    sortName: null,
    bday: null,
    identifiers: [{ scheme: 'email', value: 'meera@example.com', label: null }],
  });
});

test('parseVcards unfolds lines and strips grouping prefixes', () => {
  const vcf = ['BEGIN:VCARD', 'FN:Long', '  Name', 'item1.EMAIL;TYPE=HOME:a@b.c', 'END:VCARD'].join(
    '\r\n',
  );
  const [card] = parseVcards(vcf);
  expect(card?.fn).toBe('Long Name');
  expect(card?.identifiers).toEqual([{ scheme: 'email', value: 'a@b.c', label: 'home' }]);
});

test('parseVcards drops cards without FN', () => {
  expect(parseVcards(['BEGIN:VCARD', 'EMAIL:x@y.z', 'END:VCARD'].join('\n'))).toEqual([]);
});
