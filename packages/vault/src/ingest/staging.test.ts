...
describe("staging", () => {
  ...
  test("stage → review → publish: first contact is a draft, publish lands rows + map", () => {
    ...
    const map = db.vault
      .prepare(
        "SELECT target_type, gone_upstream FROM sync_external_entity WHERE external_id = ?"
      )
  ...
  test("discard: a draft batch publishes nothing and cannot be re-published", () => {
    ...
    expect(() => gw.publishImport(owner, staged.batchId)).toThrow(
      /discarded, not draft/u
    );
  });
  ...
  test("MBOX: messages thread by normalized subject, senders resolve to one party", () => {
    ...
    const threads = db.vault
      .prepare("SELECT count(*) AS n FROM social_thread")
      .get() as {
      n: number;
    };
    ...
    const messages = db.vault
      .prepare(
        "SELECT count(*) AS n FROM social_message WHERE sender_party_id IS NOT NULL"
      )
      .get() as { n: number };
    expect(messages.n).toBe(2);
    const meera = db.vault
      .prepare(
        `SELECT count(*) AS n FROM core_party WHERE display_name = 'Meera Pillai'`
      )
      .get() as { n: number };
  ...
  test("CSV: statement rows land as transactions on a named account", () => {
    ...
    const account = db.vault
      .prepare(
        `SELECT account_id, currency FROM core_account WHERE name = 'HDFC Savings'`
      )
  ...
  test("Takeout zip: inner ics + vcf route to one mixed batch; strays reported", () => {
    ...
    const party = db.vault
      .prepare(
        `SELECT count(*) AS n FROM core_party WHERE display_name = 'Ravi Kumar'`
      )
      .get() as { n: number };
