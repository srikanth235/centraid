...
describe("s3", () => {
  ...
  test("storage class: unset ⇒ no header ⇒ today's behavior (byte-for-byte)", async () => {
    ...
    expect(signedHeadersOf(put!.authorization)).not.toContain(
      "x-amz-storage-class"
    );
  });
  ...
  test("storage class: multipart CREATE carries it, UploadPart does not (§6)", async () => {
    ...
    const create = fake.requests.find(
      (r) => r.method === "POST" && r.search.includes("uploads")
    );
    expect(create?.storageClass).toBe("STANDARD_IA");
    const parts = fake.requests.filter(
      (r) => r.method === "PUT" && r.search.includes("uploadId")
    );
  ...
  test("retry: a 400 is definitive — no retry, the op throws on the first answer", async () => {
    ...
    await expect(store.put(sha256OfBytes(bytes), bytes)).rejects.toThrow(
      /s3 put .*: 400/u
    );
