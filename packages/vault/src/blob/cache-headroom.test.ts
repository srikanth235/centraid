...
describe("cache-headroom", () => {
  ...
  test("physical headroom pressure evicts a replicated preview below its logical budget", () => {
    ...
    const used = () =>
      local
        .listSync()
        .reduce((total, sha) => total + (local.statSync(sha)?.size ?? 0), 0);
  ...
  test("a promoted pending-outbox blob stays local after staging is consumed", async () => {
    ...
    expect(
      db.vault.prepare("SELECT 1 FROM blob_staging WHERE sha256 = ?").get(sha)
    ).toBeUndefined();
