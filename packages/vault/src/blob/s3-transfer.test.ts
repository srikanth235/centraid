...
describe("s3-transfer", () => {
  test("temporary multipart enumeration paginates inside the vault prefix and can abort results", async () => {
    ...
    await transfer.abortTemporaryUpload(
      uploads[0]!.tempId,
      uploads[0]!.uploadId
    );
    expect(urls.at(-1)?.pathname).toBe(
      "/test-bucket/vault/one/tmp/blobs/temp-a"
    );
  ...
  test("final-SHA multipart writes every operation against the content-addressed key", async () => {
    ...
    const etag = await transfer.uploadShaPart(
      sha,
      uploadId,
      1,
      Buffer.from("sealed")
    ...
    for (const call of calls) {
      expect(call.url.pathname).toBe(
        `/test-bucket/vault/one/blobs/sha256/${sha}`
      );
