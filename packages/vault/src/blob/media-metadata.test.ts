...
function mp4Fixture(): Buffer {
  ...
  const captured =
    Math.floor(Date.UTC(2024, 0, 2, 3, 4, 5) / 1000) + 2_082_844_800;
...
function webmFixture(): Buffer {
  ...
  const segment = element(
    [0x18, 0x53, 0x80, 0x67],
    Buffer.concat([info, tracks])
  );
  return Buffer.concat([
    element([0x1a, 0x45, 0xdf, 0xa3], Buffer.alloc(0)),
    segment,
  ]);
}
...
describe("bounded media metadata parsers", () => {
  ...
  test("pipeline integrates parsers and malformed containers degrade honestly", () => {
    ...
    expect(
      parseMediaMetadata(Buffer.from("not a container"), "video/mp4")
    ).toStrictEqual({});
