...
import {
  ...
  ORPHAN_MULTIPART_GRACE_MS,
} from "./orphan-multipart.js";
import type {
  RemoteBlobTransfer,
...
describe("orphan-multipart", () => {
  ...
  test("active session/outbox upload ids stay protected while stale sibling ids are reaped", async () => {
    ...
    const transfer: CleanupTransfer = {
      ...
      abortTemporaryUpload: async (tempId, uploadId) =>
        void aborted.push(`${tempId}:${uploadId}`),
    };
