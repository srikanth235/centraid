...
describe("transfers", () => {
  ...
  function stagingCount(db: VaultDb, sha256: string): number {
    return (
      db.vault
        .prepare(
          "SELECT COUNT(*) AS count FROM blob_staging WHERE sha256 = ? AND variant IS NULL"
        )
  ...
  test("fallback ingress persists offsets once per 4 MiB durability batch (#456 I7)", async () => {
    ...
    expect(run.coordinator.state.session(begin.sessionId)?.received_bytes).toBe(
      0
    );

    const remainder = Buffer.alloc(INGRESS_FSYNC_BATCH_BYTES - first.length, 2);
    await run.coordinator.appendIngress(
      begin.sessionId,
      first.length,
      remainder
    );
  ...
  test("restart commit truncates a non-durable tail before adopting unknown-size ingress", async () => {
    ...
    const nonDurableTail = Buffer.from(
      "tail that survived close but not the offset transaction"
    );
    const tempPath = first.coordinator.state.session(
      begin.sessionId
    )?.temp_path;
    ...
    expect(
      first.coordinator.state.session(begin.sessionId)?.received_bytes
    ).toBe(durable.length);
  ...
  test("strict acknowledgment returns a durable pending receipt while provider is down, then transitions", async () => {
    ...
    expect((await coordinator.preflight(committed.sha256)).custody).toBe(
      "replicated"
    );
    expect(
      unsealBlob(
  ...
  test("fallback commit resumes after restart from committing before temp adoption", async () => {
    ...
    const plain = Buffer.from(
      "GIF89a fallback commit survives the first crash boundary"
    );
  ...
  test("fallback commit resumes after restart when adoption removed the temp file", async () => {
    ...
    const plain = Buffer.from(
      "unknown hash is recovered entirely from the persisted digest state"
    );
    const sha = createHash("sha256").update(plain).digest("hex");
    ...
    const committing = firstRun.coordinator.state.beginFallbackCommit(
      begin.sessionId,
      sha
    );
    expect(committing.temp_path).not.toBeNull();
    expect(firstRun.local.adoptTempSync?.(sha, committing.temp_path!)).toBe(
      true
    );
    ...
    expect(second.local.getSync(sha)).toStrictEqual(plain);
    expect(second.coordinator.state.session(begin.sessionId)?.state).toBe(
      "complete"
    );
    expect(second.coordinator.state.outbox(sha)).not.toBeNull();
    expect(stagingCount(second.db, sha)).toBe(1);
  ...
  test("fallback commit replay keeps staged and enqueued finalize steps idempotent", async () => {
    ...
    const plain = Buffer.from(
      "staging and custody enqueue may both precede a process crash"
    );
    const sha = createHash("sha256").update(plain).digest("hex");
    const firstRun = await h.restart();
    const begin = await firstRun.coordinator.beginIngress({
    ...
    if (begin.mode !== "spool") throw new Error("expected spool ingress");
    await firstRun.coordinator.appendIngress(begin.sessionId, 0, plain);
    const row = firstRun.coordinator.state.beginFallbackCommit(
      begin.sessionId,
      sha
    );
    ...
    expect(firstRun.coordinator.state.session(begin.sessionId)?.state).toBe(
      "committing"
    );
  ...
  test("completed fallback replay repairs the former crash window before outbox enqueue", async () => {
    ...
    const plain = Buffer.from(
      "complete must never mean the remote custody obligation was forgotten"
    );
    const sha = createHash("sha256").update(plain).digest("hex");
    const firstRun = await h.restart();
    const begin = await firstRun.coordinator.beginIngress({
