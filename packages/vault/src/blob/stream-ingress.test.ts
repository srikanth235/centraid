...
describe("stream-ingress", () => {
  ...
  function fakeRemote(keys: BlobContentKeyRegistry): FakeRemote {
    ...
    const transfer: RemoteBlobTransfer = {
      ...
      completeTemporaryUpload: async (tempId, _uploadId, parts) => {
        ...
        temporary.set(
          tempId,
          Buffer.concat(parts.map((part) => saved.get(part.partNumber)!))
        );
      },
      ...
      putTemporaryStream: async (tempId, source) =>
        void temporary.set(tempId, await collect(source)),
  ...
  test("a 500 MiB bare stream on a 200 MiB volume selects encrypted hash-pending stream-through", async () => {
    ...
    const begin = await h
      .coordinator()
      .beginIngress({ expectedSize: 500 * MiB });
  ...
  test("hash-pending stream re-keys provider temp frames into the final per-blob CBSF object", async () => {
    ...
    const plain = Buffer.from(
      "hash this while the encrypted stream is already leaving disk"
    );
    const committed = await h
      .coordinator()
      .streamThrough(
        { expectedSize: plain.length, filename: "note.bin" },
        Readable.from([plain])
      );
    const sealed = h.fake.objects.get(committed.sha256)!;
    expect(
      unsealBlob(h.keys.getOrCreate(committed.sha256), committed.sha256, sealed)
    ).toStrictEqual(plain);
  ...
  test("hash-pending duplicate authenticates the existing final object and discards only its temp", async () => {
    ...
    expect(h.fake.objects.get(sha)).toStrictEqual(sealed);
  ...
  test("declared-SHA preflight returns an existing receipt without opening an upload", async () => {
    const h = await harness({ budgetBytes: 1 });
    const plain = Buffer.from(
      "preflight means the request body never needs to leave the client"
    );
    const sha = sha256OfBytes(plain);
    h.fake.objects.set(
      sha,
      sealBlob(h.keys.getOrCreate(sha), sha, plain, 1024 * 1024)
    );
    ...
    expect(h.fake.temporary.size).toBe(0);
    expect(h.fake.partUploads).toStrictEqual([]);
  });

  test("declared-SHA stream rejects mismatched plaintext before promoting a final object", async () => {
    const h = await harness({ budgetBytes: 1 });
    const plain = Buffer.from(
      "these bytes do not match the declared content address"
    );
    await expect(
      h
        .coordinator()
  ...
  test("direct completion HEAD- and AEAD-verifies CBSF before accepting the metadata claim", async () => {
    ...
    const committed = await coordinator.completeDirect(
      initiated.sessionId!,
      "paired-phone-key"
    );
    expect(committed).toMatchObject({ sha256: sha, custody: "remote-only" });
    expect(coordinator.state.session(initiated.sessionId!)?.state).toBe(
      "complete"
    );
  ...
  test("direct completion rejects missing, corrupt, and declared-plaintext-size-mismatched objects", async () => {
    ...
    const wrongSize = await begin(
      Buffer.from("declared size must bind the directory"),
      999
    );
  ...
  test("beginDirect on a local-only blob dedupes but settles as unreplicated (casAck receipt)", async () => {
    ...
    expect(
      result.sessionId,
      "a durable local copy needs no re-upload"
    ).toBeUndefined();
  ...
  test("stream-through resumes open and committing sessions without retransmitting completed parts", async () => {
    ...
    if (begun.mode !== "stream-through")
      throw new Error("expected stream-through");
    await first.appendIngress(
      begun.sessionId,
      0,
      plain.subarray(0, TEST_CHUNK_BYTES)
    );
    ...
    if (resumed.mode !== "stream-through")
      throw new Error("expected resumed stream-through");
    await second.appendIngress(
      resumed.sessionId,
      resumed.offset,
      plain.subarray(TEST_CHUNK_BYTES)
    );
    ...
    if (committing.mode !== "stream-through")
      throw new Error("expected committing session");
