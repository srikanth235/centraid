...
export async function verifyRemoteSealedObject(input: {
  ...
  if (!header)
    throw new Error("provider returned no CBSF header after completion");
  ...
  if (!trailerBytes)
    throw new Error("provider returned no CBSF trailer after completion");
  ...
  if (directoryStart < HEADER_BYTES)
    throw new Error("provider CBSF directory overruns frames");
  const sealedDirectory = await store.get(sha256, {
    start: directoryStart,
    end: sealedSize - TRAILER_BYTES - 1,
  });
  if (!sealedDirectory)
    throw new Error("provider returned no CBSF directory after completion");
  ...
  if (
    ...
    directory.totalSize !== expectedPlaintextSize
  ) {
    throw new Error(
  ...
  if (!first)
    throw new Error("provider returned no sampled CBSF frame after completion");
