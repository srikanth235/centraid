...
describe("read", () => {
  ...
  test("resolves the requested rung for many ids, one map, absent ids omitted", () => {
    ...
    const map = resolveDerivativeShas(
      db.vault,
      [...withThumb, "no-rung", "never-seen"],
      "thumb"
    );
  ...
  test("variant filter: a preview request never returns thumb rows", () => {
    ...
    const thumbs = resolveDerivativeShas(
      db.vault,
      ["only-thumb", "only-preview"],
      "thumb"
    );
    expect([...thumbs.keys()]).toStrictEqual(["only-thumb"]);
    const previews = resolveDerivativeShas(
      db.vault,
      ["only-thumb", "only-preview"],
      "preview"
    );
    expect([...previews.keys()]).toStrictEqual(["only-preview"]);
  });
