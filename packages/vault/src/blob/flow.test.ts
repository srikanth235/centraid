...
describe("flow", () => {
  ...
  test("inline data_uri: text stays in the row, binary spills to the CAS, big refuses", () => {
    const taskOut = executed<{ task_id: string }>(
      invoke("schedule.add_task", { title: "T" })
    );
    ...
    expect(textRow.sha256).toBe(
      sha256OfBytes(Buffer.from("inline body", "utf8"))
    );
    ...
    expect((refused as { predicate?: string }).predicate).toContain(
      "within_size_cap"
    );
  });
  ...
  test("blob maintenance sweep refreshes the app-readable custody-state mirror (issue #352)", async () => {
    ...
    const row = db.vault
      .prepare(
        "SELECT sha256, custody_state FROM blob_custody_state WHERE content_id = ?"
      )
      .get(doc.content_id) as
      | { sha256: string; custody_state: string }
      | undefined;
    ...
    expect(
      (
        db.vault
          .prepare("SELECT count(*) AS n FROM blob_custody_state")
          .get() as { n: number }
      ).n
    ).toBe(1);
  });
  ...
  test("custodyStateCounts groups the mirror by state, zero-filled (issue #351 wave 4)", async () => {
    ...
    expect(custodyStateCounts(db.vault)).toStrictEqual({
      "pending-offsite": 0,
      "local-only": 0,
      replicated: 0,
      "remote-only": 0,
