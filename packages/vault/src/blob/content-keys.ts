...
export class BlobContentKeyRegistry {
  ...
  constructor(
    ...
    if (wrappingKey.length !== KEY_BYTES)
      throw new Error("blob key wrapping key must be 32 bytes");
  ...
  getOrCreate(sha256: string): Buffer {
    ...
    const row = this.db
      .prepare(
        "SELECT wrapped_key, wrap_nonce, key_epoch FROM blob_content_key WHERE sha256 = ?"
      )
      .get(sha) as
  ...
  grantToDevice(sha256: string, deviceId: string): DeviceWrappedContentKey {
    ...
    const deviceKey = this.deriveDeviceWrapKey(
      device.public_key,
      wrap.salt,
      wrap.key_epoch
    );
      ...
      for (const device of devices) {
        ...
        if (device.device_id === row.device_id) {
          revoked = Number(
            this.db
              .prepare(
                "DELETE FROM blob_device_content_key WHERE device_id = ?"
              )
        ...
        const grants = this.db
          .prepare(
            "SELECT sha256 FROM blob_device_content_key WHERE device_id = ?"
          )
  ...
  rewrapAll(nextWrappingKey: Buffer): number {
    ...
    try {
      for (const row of rows) {
        ...
        const wrapped = seal(
          nextKey,
          `blob-key:${row.sha256}:epoch:${nextEpoch}`,
          contentKey
        );
        this.db
          .prepare(
