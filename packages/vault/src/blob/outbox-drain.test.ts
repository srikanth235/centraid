...
describe("outbox-drain", () => {
  ...
  test("known-sha outbox writes CBSF straight to the final CAS key", async () => {
    ...
    const store: BlobStore = {
      ...
      put: async () => {
        throw new Error(
          "ordinary known-sha upload must stay on the streaming seam"
        );
      },
      putStream: async (targetSha, source) =>
        void final.set(targetSha, await collect(source)),
    ...
    const transfer: RemoteBlobTransfer = {
      beginTemporaryUpload: async () => {
        throw new Error(
          "ordinary known-sha upload must not create a temp multipart"
        );
      },
      ...
      completeTemporaryUpload: async () => {
        throw new Error(
          "ordinary known-sha upload must not complete a temp object"
        );
      },
      abortTemporaryUpload: async () => undefined,
      putTemporary: async () => {
        throw new Error(
          "ordinary known-sha upload must not write a temp object"
        );
      },
      putTemporaryStream: async () => {
        throw new Error(
          "ordinary known-sha upload must not stream to a temp object"
        );
      },
    ...
    expect([...final.keys()]).toStrictEqual([sha]);
  ...
  test("outbox-resident multipart resumes directly at the final SHA without CopyObject", async () => {
    ...
    const transfer: RemoteBlobTransfer = {
      ...
      completeShaUpload: async (targetSha, _uploadId, parts) => {
        final.set(
          targetSha,
          Buffer.concat(parts.map((part) => uploaded.get(part.partNumber)!))
        );
      },
      abortShaUpload: async () => undefined,
      beginTemporaryUpload: async () => {
        throw new Error(
          "outbox-resident bytes must not create a temporary object"
        );
      },
      uploadTemporaryPart: async () => {
        throw new Error(
          "outbox-resident bytes must not upload a temporary part"
        );
      },
      completeTemporaryUpload: async () => {
        throw new Error(
          "outbox-resident bytes must not complete a temporary object"
        );
      },
      abortTemporaryUpload: async () => undefined,
      putTemporary: async () => {
        throw new Error(
          "outbox-resident bytes must not PUT a temporary object"
        );
      },
      putTemporaryStream: async () => {
    ...
    expect(JSON.parse(state.outbox(sha)!.parts_json)).toStrictEqual([
  ...
  test("existing corrupt, zero, or size-stale provider objects are replaced before custody ack", async () => {
    ...
    const drainBadObject = async (index: number): Promise<void> => {
      ...
      const plain = Buffer.from(
        `authoritative local object ${index} with enough bytes`
      );
      const sha = sha256OfBytes(plain);
      local.putSync(sha, plain);
      state.enqueue(sha, plain.length);
  ...
  test("close fencing is rechecked after remote verification before SQLite settlement", async () => {
    ...
    const state = new BlobTransferState(db.vault);
    const plain = Buffer.from(
      "provider-confirmed bytes whose range read outlives close"
    );
    const sha = sha256OfBytes(plain);
    local.putSync(sha, plain);
    state.enqueue(sha, plain.length);
