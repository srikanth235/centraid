...
function isoTime(seconds: number): string | undefined {
  if (!Number.isFinite(seconds) || seconds <= MP4_EPOCH_OFFSET_S)
    return undefined;
    ...
    for (const box of isoBoxes(bytes, start, end)) {
      ...
      try {
        if (box.type === "mvhd" && box.payload + 20 <= box.end) {
          ...
          for (
            let i = 0;
            i < Math.min(entries, 64) && entry + 8 <= box.end;
            i += 1
          ) {
...
const EBML_MASTER = new Set([
  0x18538067, 0x1549a966, 0x1654ae6b, 0xae, 0xe0, 0xe1,
]);
...
export function parseWebmMetadata(bytes: Buffer): MediaMetadata {
  ...
  const walk = (
    ...
    while (at < end && visited < MAX_ELEMENTS) {
      ...
      if (payload > elementEnd || elementEnd > end || elementEnd > bytes.length)
        return;
      visited += 1;
      ...
      try {
        if (id.value === 0x2ad7b1) {
          timecodeScale =
            unsignedBe(bytes, payload, elementEnd) ?? timecodeScale;
...
export function parseId3Metadata(bytes: Buffer): MediaMetadata {
  ...
  for (let frames = 0; frames < 2048 && at + 10 <= end; frames += 1) {
    ...
    const size =
      version === 4 ? syncSafe(bytes, at + 4) : bytes.readUInt32BE(at + 4);
    const payload = at + 10;
    if (size <= 0 || payload + size > end) break;
    if (id === "TIT2")
      out.title = decodeTagText(bytes.subarray(payload, payload + size));
    if (id === "TPE1")
      out.artist = decodeTagText(bytes.subarray(payload, payload + size));
    at = payload + size;
  }
...
function parseVorbisComment(
  bytes: Buffer,
  start: number
): Pick<MediaMetadata, "title" | "artist"> {
...
export function parseMediaMetadata(
  ...
  try {
    ...
    if (mediaType === "audio/ogg" || mediaType === "audio/flac")
