...
describe("replica invocation commit receipt", () => {
  test("rolls back with the canonical mutation", () => {
    ...
    expect(
      db.vault
        .prepare(
          "SELECT 1 AS present FROM schedule_calendar WHERE calendar_id = ?"
        )
        .get(calendarId)
    ).toBeUndefined();
  });
  ...
  test("repairs then reclaims a terminal intent marker on reopen", () => {
    ...
    try {
      ...
      expect(
        readReplicaInvocationCommit(db.vault, "invocation-intent-reopen")
      ).toBeDefined();
      db.close();
      db = undefined;
      ...
      expect(
        readReplicaIntentOutcome(db.vault, "intent-reopen", "device-1")?.status
      ).toBe("executed");
    } finally {
      db?.close();
      rmSync(dir, { recursive: true, force: true });
  ...
  test("revocation retains detached proof until reopen repairs the journal", () => {
    ...
    try {
      ...
      expect(deleteReplicaIntentOutcomesForDevice(db.vault, "device-1")).toBe(
        1
      );
      expect(
        readReplicaInvocationCommit(db.vault, "invocation-revoked")
      ).toMatchObject({
        invocationId: "invocation-revoked",
      });
      expect(
        readReplicaInvocationCommit(db.vault, "invocation-revoked")?.intentId
      ).toBeUndefined();
      db.close();
      db = undefined;
  ...
  test("bounded pages repair later proof while retaining and rejecting an unprovable marker", () => {
    ...
    expect(
      readReplicaInvocationCommit(db.vault, "invocation-missing")
    ).toBeDefined();
    expect(
