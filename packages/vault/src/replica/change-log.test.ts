...
describe('change-log', () => {
  ...
  test('live ext rows join the log while draft rows remain scratch-only', () => {
    ...
    opened.vault
      ...
      .run();
    expect(readReplicaChanges(opened.vault, { since: beforeDraftRow }).changes).toStrictEqual([]);
  });
  ...
  test('retention applies age then count while advancing through a deleted prefix', () => {
    ...
    expect(() => readReplicaChanges(vault, { since: cursorBeforeRetention })).toThrow(
      ReplicaRebootstrapRequiredError,
    );
  });
  ...
  test('count pressure compacts the full window to the latest entry per row', () => {
    ...
    expect(readReplicaChanges(vault, { since: alreadyCurrent }).changes).toStrictEqual([]);
    expect(() => readReplicaChanges(vault, { since: { epoch, seq: 999 } })).toThrow(
      ReplicaRebootstrapRequiredError,
    );
  });
  ...
  test('epoch bump invalidates old cursors and new changes continue above the prior watermark', () => {
    ...
    expect(() => readReplicaChanges(vault, { since: before.watermark })).toThrow(
      ReplicaRebootstrapRequiredError,
    );
