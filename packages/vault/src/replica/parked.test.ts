...
describe("parked", () => {
  ...
  test("parked command payloads survive gateway recreation encrypted at rest", () => {
    ...
    const stored = db.vault
      .prepare(
        "SELECT request_sealed FROM replica_parked_payload WHERE invocation_id = ?"
      )
      .get("inv-1") as { request_sealed: string };
    expect(stored.request_sealed).not.toContain("never-plaintext-at-rest");
    expect(readDurableParkedPayload(db, "inv-1")?.request.input).toStrictEqual({
  ...
  test("withdrawing an answer drops every durable parked payload on it", () => {
    ...
    expect(deleteDurableParkedPayloadsForGrant(db, "grant-1")).toStrictEqual([
      "inv-1",
      "inv-2",
    ]);
    expect(listDurableParkedPayloads(db)).toStrictEqual([]);
  });
  ...
  test("settlement rolls payload deletion back when the terminal outcome cannot commit", () => {
    ...
    expect(
      readReplicaIntentOutcome(db.vault, "intent-1", "device-1")
    ).toMatchObject({
      status: "parked",
      invocationId: "inv-1",
    });
