...
describe("fts-index-budget", () => {
  test("truncateForIndex passes short values through untouched", () => {
    const db = openVaultDb();
    const row = db.vault
      .prepare(`SELECT ${truncateForIndex("'hello'", 100)} AS v`)
      .get() as {
      v: string;
    };
  ...
  test("truncateForIndex passes NULL through as NULL, not the marker", () => {
    const db = openVaultDb();
    const row = db.vault
      .prepare(`SELECT ${truncateForIndex("NULL", 10)} AS v`)
      .get() as {
      v: null;
    };
  ...
  test("rebuildFtsIndex refuses core.document and points at the derivative-aware rebuild", () => {
    const db = openVaultDb();
    expect(() => rebuildFtsIndex(db.vault, "core.document")).toThrow(
      /rebuildDocumentFtsIndex/u
    );
  });
  ...
  test("rebuildDocumentFtsIndex re-derives fts_core_document, extracted text still wins", () => {
    ...
    db.vault.exec(
      `UPDATE fts_core_document SET body = 'stale' WHERE document_id = '${document_id}'`
    );
