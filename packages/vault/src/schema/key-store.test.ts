...
describe("key-store", () => {
  ...
  test("legacy file custody writes an atomic self-describing 0600 envelope", () => {
    ...
    expect(
      readFileSync(file, "utf8").startsWith(KEY_STORE_ENVELOPE_MAGIC)
    ).toBe(true);
  ...
  test("an interrupted first mint leaves no short key and retry commits a complete envelope", () => {
    ...
    expect(() => keys.loadOrCreate("endpoint-key.bin")).toThrow(
    ...
    expect(readFileSync(retry.file("endpoint-key.bin"), "utf8")).toMatch(
      /^CENTRAID-KEY-V1\n/u
    );
  });
  ...
  test("adopts a legacy raw key without changing its value", () => {
    ...
    expect(
      readFileSync(keys.file("vault.sealkey"), "utf8").startsWith(
        KEY_STORE_ENVELOPE_MAGIC
      )
    ).toBe(true);
    expect(readFileSync(keys.file("vault.sealkey"), "utf8")).toContain(
  ...
  test("routes payloads through a supplied OS-custody protector", () => {
    ...
    expect(readFileSync(keys.file("keyring.key"), "utf8")).toContain(
      '"scheme":"test-os-v1"'
    );
  });
  ...
  test("AES protector requires the device wrapping key and rewraps file custody", () => {
    ...
    const structured = Buffer.from(
      JSON.stringify({ version: 1, epochs: ["first"] })
    );
    ...
    expect(readFileSync(plain.file("keyring.key"), "utf8")).toContain(
      '"scheme":"aes-256-gcm-v1"'
    );
    expect(() =>
  ...
  test("corrupt, unsupported, and invalid names fail closed", () => {
    ...
    expect(() => keys.load("foreign.key")).toThrow(
      /unavailable custody scheme/u
    );
  });
