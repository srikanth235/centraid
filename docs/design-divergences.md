...
**The commons-era `shared_with_them` projection is no longer drawn on either person screen.** `queries/person.ts` still answers it (and the phone's replica projection still mirrors that query), but the section above it feeds on is the grant plane now — one fact, one source. The mobile vault-link screen (`VaultLink.tsx`, reachable only by deep link) is the one surface still drawing the old projection; repointing or retiring it belongs with the copy-as-share sweep.
