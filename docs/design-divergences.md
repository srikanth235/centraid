...
| --- | --- |
| The `Share` and `Link vault` commits, and the Share sheet from People | **`Share` is live**, on the `Shared with them` section head, opening the shared `GrantSheet` already on this person. `Link vault` is **retired rather than restored**: a grant to an unlinked person parks at `awaiting_channel` and mints the channel invitation as its own first step, so a `Link vault` commit would name a ceremony the product no longer has. `Log` remains the one filled commit. |
...
- **The vault-link screen is reachable only by deep link.** The two doors the handoff gives `PersonLink` (the roster `Link` verb, the person screen's `Link vault` commit) named the link ceremony, and #825 retired the ceremony rather than the screen: sharing mints the invitation itself, so nothing draws a door to a standing the phone can only read. The two ceremony sentences ("One approval each, once…") stay undrawn for the same reason — they narrate an act no control performs any more.
...
`/test-report/` renders in the product's design ([decisions.md](decisions.md#product-grammar-and-block-composition), 2026-08-23, [#853](https://github.com/srikanth235/centraid/issues/853)): the same `scripts/site-tokens.mjs` that feeds the two sites emits `scripts/test-report/report-tokens.css`, and `bun run lint:site-tokens` fails on drift from it, on a `var()` that resolves to nothing, on a literal font family, on a colour literal, and on the withdrawn face. The report also takes every row in [the public web surfaces](#the-public-web-surfaces) above — it is a long-form page and uses the same site layer. The rows below are what a **heat map** does that neither a product screen nor a marketing page does.

| Divergence | Decision | Enforcement / reason |
| --- | --- | --- |
| Four matrix states — passed, partial, failed, flaky — paint as a **fill** on a cell that is also a focusable target, two of them (`--c-teal`, `--c-violet`) from the identity-hue axis that is never permitted on a control. | Keep, bounded to `.cell` and `.dot` inside the matrix. | A heat map's whole job is to answer "which of twelve states is this" across 15×11 cells at a glance, and a 52×40 cell has no room for a label. The bound is what keeps it honest: only the four states a lane actually MEASURED are fills, the ink on them is `--st-on-fill` (`--text-inv`, which inverts with the theme as the hues do), and every cell carries its state as text as well — in its class, its `title`, the legend, and the evidence inspector below — so nothing on this page is legible only to a reader who sees hue. No control on the page takes a hue. |
| The other eight states — n/a by design, tracked gap, evidence unmatched, owner silent, missing, stale, lane did not run, named absence — paint as a **recessed trough** (`--bg-app` + `--line-strong`) with their tone as a 3px inline-start rule and as their own numerals. | Keep. This is the rule, not a departure from it. | It is what DESIGN.md already requires of `--warning`, `--seam` and `--attention`: type, a border or a 2px rule, never a fill. It is also the report's own thesis rendered rather than described — absence stays fully visible and stays classified, and what it stops doing is out-shouting the proof. Before this pass every one of the 165 cells was a fill and the 136 unproven ones were a flat `#46534c` block, so the page was a wall of solid colour in which the cells carrying actual evidence had nothing to distinguish them but hue. |
| "n/a by design" takes `--c-slate`, a content marker, rather than `--warning`. | Keep. | An exclusion an app's own manifest declares is a classification, not a degraded reading. It is also forced: in dark, `--warning` (`#d9a75b`) and `--attention` (`#D8A64E`) land within a hair of each other, so "n/a by design" and "owner silent" sharing that family would be one state to any reader. `--warning` keeps this page's one honest use of itself, `--st-degraded`, on the verdict strip. |
...
| `bun run lint:design-tokens` is not pointed at `scripts/test-report/`. | Open, and deliberately not closed by widening a budget. | Same reasoning as `scripts/*-site` in the row above: that gate's checked-in budget is empty and must stay empty, and the report still carries bespoke layout dimensions (the 40px cell, the 98px header band, the 230px surface column) that a heat map's geometry genuinely needs and no `--sp-*` rung expresses. `lint:site-tokens` plus `report-theme.test.mjs` fence what this pass actually closed — the palette, the face, the type scale, the missing light theme, and unresolvable `var()`s. |
