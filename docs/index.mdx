...
- **Apps as folders.** _Source tree:_ `index.html` + `app.css` + `app.js` + `queries/*.js` + `actions/*.js` + `migrations/*.sql` + `automations/<id>/` + `app.json`. _Mounted tree_ on the gateway adds `data.sqlite`, `runtime.sqlite`, `current.json`, and `versions/<v_id>/` snapshots of every uploaded build.
