...
| KMP shells (v1) | the same `toNativeTheme()`, **emitted as data** — `contracts/tools/export-native-theme.ts` writes `design/native-theme.json`, `mobile/shared/.../design/Tokens.kt` and `mobile/iosApp/Design/Theme.swift` from one command | Compose and SwiftUI read the emitted table; neither holds a literal | `git diff --exit-code design copy mobile` in the `mobile-jvm` gate step, plus `NativeThemeSpec` over the emitted table in both schemes and `NativeAccessibilityLintSpec` over both view trees ([#1020](https://github.com/srikanth235/centraid/issues/1020)) |
| Rust (v1) | [`crates/design`](../crates/design/README.md) — the same three functions, **generated from a corpus the real TypeScript emits** over 252 hostile rows rather than retyped | none; it exists so a parity fixture's presentation fields can be compared at all | `crates/design/tests/corpus.rs` asserts every row, and `copy::route_gaps` names a route id present on one side and not the other ([#1020](https://github.com/srikanth235/centraid/issues/1020), D-1020-T1) |
