...
- **Journey ownership**: origin `tests/agent-e2e-mobile/flows/tally-derived.mjs`; custodian `apps/desktop/tests/e2e/tally.spec.ts`; viewer `skip` in `tests/claims.json#appSeats` pending the web seat's own journey.
- **Structural exclusions**: see `tests/claims.json#appEngines` (record-only: custody skipped).
