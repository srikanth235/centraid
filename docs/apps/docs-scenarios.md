...
| title-only edit on phone does not write an empty body | — | — | — | **product-bug** (#864): editing the title while the body is still fetching overwrites the document with an empty body (`apps/mobile/src/apps/docs/DocumentEditor.tsx`, `useDocumentText.ts`) |
| export filename cannot traverse out of the share folder | — | — | — | **product-bug** (#864): `docs-export.ts` interpolates the title into a path, so `../../x` escapes |
