...
Tally splits the plane the other way round: its WRITES are ordinary and its derived READS are not. Every Tally act carries an optimistic pending projection (`packages/blueprints/apps/tally/pending-projection.ts`), so the native cover issues all of them through `session.write` and they queue in the durable outbox when the gateway is out of reach — that is what "record-only and fully offline-capable" means on a phone. The one exception is `materialize-recurring-expense`, whose occurrence id is minted by the canonical recurrence engine: the projection excludes it by construction, so the seat WITHHOLDS the verb while offline rather than queueing a copy of an expense nobody issued. Reads take the gateway's query handlers (`apps/mobile/src/apps/tally/tally-gateway.ts`) because `queries/dashboard.ts` holds the app's ONE balance engine; folding balances out of replica rows on the phone would be a second derivation of "who owes whom", free to drift from the first. A refused grant and an unreachable gateway are therefore different screens, and `apps/mobile/src/apps/tally/tally-store.test.ts` pins that a failed read never reads as an empty ledger.
