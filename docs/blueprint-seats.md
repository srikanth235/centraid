...
8. **One pending-write overlay engine.** `packages/blueprints/apps/_shared/pending-overlay.ts` + per-app `pending-projection.ts` own the unsettled-write model every write-bearing app renders — reload-proof by construction because it rebuilds only from the durable outbox, never from app memory or a gateway fetch. Tally's `pendingExpenses`, Tasks'/Agenda's `pendingAdds`/`pendingIds`/`pendingByIntent`, and Notes' `pendingNoteIds`/`pendingCreates` were four hand-rolled copies of the same state machine; the engine replaces all four.
...
**H — pending-write overlay.** `packages/blueprints/apps/_shared/pending-overlay.ts` + per-app `pending-projection.ts` are the one answer to "what does an unsettled write render as" (issue #738). Verbs: `begin` (mint the optimistic mutations a write issues), `applyOutcome` (fold a write/settlement result), `applyChangeDetail` (fold one change-feed event), `restore` (rebuild overlay-status rows from the durable outbox — the reload path, and the only source of truth for it), `enrichCommons` (fold the commons rail in as enrichment only — steward label and per-grant status, never a removal), `dismiss` (discard an attention row the member has read), and `takeForRetry` (hand back the cached action/input for a fresh-`intentId` re-issue). Row surfaces: `rows()` (every tracked write), `byRowId()` (decorate a query row with its pending state), `attention()` (denied/conflict/failed), `unsettled()` (queued/sending/parked, the rows still overlaying reads). Reason grammar: `queued`/`sending` render a quiet chip; `parked` renders a chip, its reason, and the owner-approval affordance; `denied`/`conflict`/`failed` persist the row with its explanation and edit/retry/discard — silent disappearance is a defect class, not a styling choice. A conflict names the expected and actual versions verbatim (`PendingConflictDetail`), never a generic transport error. Offline copy says "waiting for a connection" — naming a steward would be a lie, nothing has been submitted; online commons copy names the steward the write waits on. A gateway-supplied reason (`pendingReasonCopy`'s `reason` opt) is printed verbatim, same as every other engine's grammar. Structural exclusions: **Locker × pending-overlay** — Locker's reveal is online-only by design (no offline unlock to queue), so it declares no `pending-projection.ts` actions, the same absent-registry-row shape placement uses for Locker. **Photos byte-custody** — bytes ride engine B end to end; this engine covers only the row-shaped writes an app's queries return (an upload's row metadata may project, its bytes never do).

Coverage, stated precisely because the grammar above is the contract and not every seat satisfies all of it. An attention row (denied/conflict/failed) is durable everywhere: it is journaled into the outbox in the same transaction that scrubs the settled intent (`IntentAttentionRecord`), so it survives reload on browser and native alike, and a discard clears the durable record rather than only the session's memory. **Retry** — re-issuing the cached payload under a fresh `intentId` — is offered by every adopting blueprint except Photos (see below) and, on native, from the device-global pending-changes sheet rather than inline on the originating row, and only for a row whose journal retained its payload. **Edit** — reopening a compose surface seeded from the refused payload — is offered only where an app has a surface bound to the payload rather than to the canonical row: Tally's expense modal, Agenda's create modal, Notes' quick-add, Docs' rename and folder fields, People's add-person modal. Agenda's event drawer, Notes' autosaving editor and People's profile drawer are deliberately **not** seeded, because they edit the stored row and would either resend on the next keystroke or diverge from disk; actions carrying no correctable text (deletes, moves, trash/restore) offer retry and discard only. Photos announces a refusal on the frame's single status line with discard as its inline action and offers neither retry nor edit, since every declared Photos write is a one-tap toggle whose control is still on the tile — respecting `Tile.tsx`'s four-slot budget rather than adding a surface. Conflicts are reachable only where a seat can obtain a row version: every adopting blueprint sends `baseVersions` for writes against an existing row via `window.centraid.rowVersion()`, an opaque-identity shape supplies none rather than guessing, and no mobile screen sends them yet.
