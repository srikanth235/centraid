...
One gate per engine, all five in `scripts/lint-engine-conformance.mjs` (wired into `check:push` as `lint:engine-conformance`), with the two single-package halves left where they already were: `packages/blueprints/src/placement-registry.test.ts` (A's union mirrors vault's) and `packages/blueprints/src/no-inference-client.test.ts` (C's provider-SDK ban, now scanning the native tree too).

**A — placement.** `packages/blueprints/apps/_shared/placement-registry.ts` is the one answer to "what can be placed into an audience vault, and which app owns it". One verb: `window.centraid.place({ kind, itemType, itemId, sourceVaultId, targetVaultId })`, reached through `AudiencePlacement.tsx` (web) or `AudiencePlacementSheet` (native, over `apps/mobile/src/kit/share/`); the target list comes from `InlineScope.audience` and `MemberStore.membersOf`, never from an app's own guess at who is in the household. Reason grammar: a placement that does not execute returns `result.reason` and the control prints it verbatim — the surface never invents a sentence for a refusal the gateway described. Structural exclusion: **Locker × placement**. `locker.item` is left out of `PlaceableItemType` rather than filtered, so a Locker placement control is a type error at the call site and a lint failure in the browser-served tree that `tsc` never sees.
...
**D — triage.** One verb answers a proposal: `media.answer_face_proposal` (`packages/vault/src/commands/enrich.ts`), with three answers — confirm, reject, dismiss — writing `media_face_region.review_state`. The `media.confirm_face` / `media.reject_face` pair it replaced is **retired, not deprecated beside it**: reject was a DELETE, which is not a state, so the enricher was free to re-propose the same stranger for ever and a review queue could never be finished. The session state machine is `packages/blueprints/apps/photos/triage-session.ts` (`openTriage`, `triageCurrent`, `triageSkip`, `triageAnswer`, `triageRefill`, `triageProgress`), pure and imported verbatim by the native twin; its frozen denominator is why the numerator counts up instead of the total sliding around mid-session. Reason grammar: skip records nothing and says so; a queue control that goes inert must say why, since a member stuck at "1 of 54" with a grey control has no next move. Structural exclusion: an outcome vocabulary is per-app — nothing in `triage-session.ts` knows what a face or a duplicate cluster is — so a third consumer joins by naming its outcomes, not by widening the engine.

**The fifth shared thing is the search scaffold**, already row 6 of the list above (`apps/_shared/search-scaffold.ts` + `SearchScaffold.tsx`). It is not written up as a contract here because it has no verbs, no refusals and no exclusions: it is a pure combinator plus a four-state renderer, and matching stays app-owned by design. Row 6 is its entry; this section deliberately does not restate it.
