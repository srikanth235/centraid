...
**A — placement.** `packages/blueprints/apps/_shared/placement-registry.ts` is the one answer to "what can be placed into an audience vault, and which app owns it". One verb: `window.centraid.place({ kind, itemType, itemId, sourceVaultId, targetVaultId })`, reached through the one give/lend `ShareSheet.tsx` (web) or its native twin (`apps/mobile/src/kit/share/ShareSheet.tsx`) — both replaced the older single-purpose `AudiencePlacement.tsx` / `AudiencePlacementSheet` controls in #726 P6; the target list comes from the caller's own mounted, writable `window.centraid.scopes` plus every linked person, never from an app's own guess at who is in the household. Reason grammar: a placement that does not execute returns `result.reason` and the control prints it verbatim — the surface never invents a sentence for a refusal the gateway described. Structural exclusion: **Locker × placement**. `locker.item` is left out of `PlaceableItemType` rather than filtered, so a Locker placement control is a type error at the call site and a lint failure in the browser-served tree that `tsc` never sees.
...
**F — enrichment boundary.** Apps enqueue consent-scoped `enrich_request` rows and read model-versioned vault projections. The reference service is reached only by `packages/gateway/src/enrich/service-client.ts`; blueprint, native, and automation trees may import neither that client/service nor a provider SDK. Reason grammar is the capability result itself: unconfigured service, missing capability, or model failure remains capability-scoped and visible rather than being re-labelled success. Structural exclusion is the closed `EnrichDomain` union and the app's absent consent/control surface. The app×engine grid records those N/A cells against this section.

**G — scope kit.** `packages/blueprints/apps/_shared/scope-kit.ts` is how an app becomes shareable without writing sharing code (issue #726 D11). A `ScopeAppDeclaration` names three facts, nothing more: `mergeKey` (the field `mergeScopePages`, `scope-merge.ts`, orders and windows a cross-scope read on — Photos: `taken_at`; a record-only app with nothing else to order by: its own row id), `mintedIdFamilies` (the vault entity kinds this app's rows belong to, e.g. `"schedule.task"`, so the closure/edge split knows which rows are this app's to project when a share crosses a scope), and `projectionIngest` (the name of the audience-side post-ingest door this app's rows should take, or `"none"` for an app with no derived state to refresh). It is a STRING TAG, not a function reference — the kit is a browser ES module with no bundler and must never import vault-side code, and the projection engine resolves ingest doors server-side, keyed by vault entity type rather than by app id (see [ARCHITECTURE.md](../ARCHITECTURE.md#vault-ownership-and-sharing-726)). Tasks is the proof the kit generalizes past the single app (Photos) it was extracted from: its declaration (`tasksScopeDeclaration`) merges on its own row id, mints `schedule.task` rows, and declares no projection ingest (a task carries no bytes of its own — attachments are `core.content_item` rows Tasks only ever references, never mints — so a projected task has nothing to re-ingest); it reads its board over two mounted scopes through `scope-fanout.ts` with zero sharing-specific code of its own.
...
3. **The gateway enrichment lane** — `packages/gateway/src/enrich/service-client.ts` drains the same consent-scoped requests to the optional `CENTRAID_ENRICH_URL` reference service and stamps the returned capability/model provenance. This is one gateway-owned client, not an app-facing transport.

No blueprint app, native seat, or automation imports a provider SDK or the enrichment service/client directly (`packages/blueprints/src/no-inference-client.test.ts` is the conformance check). Apps express intent as rows and consume vault projections; choosing device versus gateway execution is host policy, never a client network call.
