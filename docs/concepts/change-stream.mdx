...
> **TODO(#120)** — capture the exact event payload shape (event name, JSON body, whether it's one event per table or one event with a table list). The README documents the surface but not the wire format; I'd need to read `packages/app-engine/src/` for the exact serialization.
...
> **TODO(#120)** — verify the event name (`change` vs `message`) and field name (`tables` vs `tableNames`) against `app-engine` source. Pattern is correct; field names are placeholder.
