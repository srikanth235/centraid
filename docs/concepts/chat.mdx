...
The desktop renderer streams these routes directly — `apps/desktop/src/renderer/gateway-client-chat.ts` posts the turn and reads the SSE with a `fetch` + `ReadableStream` reader. The mobile companion talks to the same routes over HTTP. There is no shared client package.
...
The `_chat` routes live in `@centraid/app-engine` and are served identically on both hosts. What differs is which agent backend the host wires in:
...
> **TODO(#120)** — list the exact normalized event types. The README references "normalized events" but I haven't read `app-engine/src/chat/events.ts` (or wherever the event schema lives) to enumerate them.
