...
**Known gaps.** The reference service's `setup` was not run in the environment it was built in, so three tensor-layout assumptions are documented in code but not yet verified against a real forward pass: the CLIP embedding output tensor index, the OCR detector's output shape and recognizer class layout, and YuNet's output count/order and box parametrization. See the service's own [README](../tools/enrichment-service/README.md) ("Known gaps for whoever wires this up against real weights") before trusting its output on first real run. The pure-TS math underneath each capability (BPE tokenizer, CTC decode, NMS, DB postprocess, Umeyama alignment) is unit-tested without the native runtime, so those gaps are isolated to the tensor-shape assumptions specifically, not the surrounding logic.
