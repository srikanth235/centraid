...
- state the shape in the baseline. Since [#916](https://github.com/srikanth235/centraid/issues/916) rung one is the whole baseline, stated rather than reconstructed, because v0 had no files in the field; [#929](https://github.com/srikanth235/centraid/issues/929) added **rungs two through four**, the first changes that had to reach a file that already existed, and [#972](https://github.com/srikanth235/centraid/issues/972) added **rung five** so `share_authority_request` / `share_authority_use` reach files frozen at `user_version` 2. A fresh vault stamps `PRAGMA user_version = 5`. `migrate.test.ts` proves the fresh file lands on 5, that reopening it is a no-op with no schema drift, and that `foreign_key_check` is clean. The forward-only ladder and its downgrade refusal (`VaultSchemaAheadError`) stay — they are what makes the FIRST post-v0 rung safe;
