...
| A **snapshot never carries a key** | Long-lived key material stays out of every snapshot by construction (`packages/server/src/backup/backup-sources.ts`). Locker key custody rides the kit, and only the kit ([#996](https://github.com/srikanth235/centraid/issues/996) R13) |
