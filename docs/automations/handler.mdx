...
The fallback automation receives the failure info as its `input` and can log to a table or post to Slack via `ctx.tool`. It runs as a separate audited run, not as a child of the failed one.
