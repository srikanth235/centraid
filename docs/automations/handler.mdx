...
The conventional filename is `handler.js`. The conventional location is `<appCodeDir>/automations/<id>/handler.js`. Both are constants in [`automation-manifest.ts`](https://github.com/srikanthsrungarapu/centraid/blob/main/packages/runtime-core/src/automation-manifest.ts) (`AUTOMATION_HANDLER_FILE`, `AUTOMATION_MANIFEST_FILE`).
