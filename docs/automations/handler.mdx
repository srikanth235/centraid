...
Invoke another automation as a **subagent** — a child conversation that runs to completion and returns. Unlike a handoff (which transfers control and never comes back), `ctx.invoke` is call-and-return: the child runs its own `run` and hands its `output` back to the caller. Returns `{ output, childRunId }`.
...
The subagent call is recorded as a `run_nodes` row tagged `kind: "invoke"` on the parent, with the target handle, input, output, and the child's `runId`. The child run (sub-conversation) itself shows up in the run list as a normal entry with `trigger_origin: "manual"` (or however the host tags invoked fires).
