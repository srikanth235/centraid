...
The implementation lives in [`packages/automation/src/scaffold/webhook.ts`](https://github.com/srikanthsrungarapu/centraid/blob/main/packages/automation/src/scaffold/webhook.ts).
...
The route resolves the slug **globally** — webhook ids are unique across the gateway, so `list(appsDir)` scans every app's automations for a matching `webhookTriggerOf(triggers)?.id === slug`. First match wins.
...
There is no webhook _listener_ on the desktop. Only the remote gateway (`@centraid/openclaw-plugin`) mounts the `/_centraid-hook` route via `makeWebhookRouteHandler`; the local in-process scheduler (`InProcessScheduler`) handles cron triggers only and ignores webhook entries entirely. The manifest is preserved verbatim — if the user uploads the app to a remote gateway later, the webhook is already provisioned and ready. Locally, the only way to fire a webhook-triggered automation is the "Run now" button, which fires with empty `input`.

This means the conversational builder can still ask the user "do you want this triggered by a GitHub webhook?" on the desktop — the provisioning pass (`provisionAppPendingWebhooks` / `provisionPendingWebhooksInFiles`) mints the slug + secret, the desktop shows the secret to the user, but actual webhook fires happen only after the app moves to a gateway.
