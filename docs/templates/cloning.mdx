...
The desktop renders the contents of `packages/app-templates/index.json` as a gallery. Picking a template + clicking _Clone_ runs `cloneTemplate()` from [`@centraid/agent-harness`](https://github.com/srikanthsrungarapu/centraid/tree/main/packages/agent-harness), which copies the template's files into the user's apps directory under a new id and hands off to the existing publish pipeline.
