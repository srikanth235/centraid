...
**Those figures are superseded and the deferral is gone.** governance-kit audit 0.11.0 retired `repo-hygiene` outright and rewrote `receipt-per-issue` to a much cheaper check; [#1005](https://github.com/srikanth235/centraid/issues/1005) then ported all four vendored directives to rules and deleted the pack. The repo runs **nine** directives, one of which — `law` — is the whole ported catalog behind one door. `.governance/conf/srikanth235/centraid/pre-commit-deferred.conf` is empty; nothing runs at pre-push that does not run at pre-commit. Re-measure on the reference machine before treating any number in this section as current.
