...
**The v1 tree has its own loop, and it is one command.** The ladder above is v0's. From wave 1 ([#1020](https://github.com/srikanth235/centraid/issues/1020)) the v1 tree is gated by `cargo xtask gate --profile <local|pr|nightly|release>` — `local` under 2 minutes **on a warm tree**, which is the tree the edit-run loop runs on (a cold first build is charged to `coldLocalProfileSeconds` instead, 3200 s, and is 1562.4 s at ten crates), `pr` under 25 in [`gate.yml`](../.github/workflows/gate.yml) on every pull request, `nightly` carrying the v0 `contracts/` oracle suite, `release` failing on the two placeholders that wave 2 and wave 3 fill. The budgets are enforced and down-only; they were re-based once for the Rust workspace under an owner ruling ([D-1020-B2](decisions.md#decisions--lane-b2-1020)) and only fall from that baseline, and `mobile-jvm` is a fifth profile with its own 420 s ceiling rather than a charge against `pr` ([D-1020-B2-3](decisions.md#decisions--lane-b2-1020)). **Every cargo and xtask command needs its own `CARGO_TARGET_DIR`** when more than one worktree is in flight — see the worktree rules above and [traps/shared-cargo-target.md](traps/shared-cargo-target.md). What that means for rung 2 is that `ci.yml` no longer runs on pull requests at all and **branch protection must be repointed at `gate` and `dependency-review`**; the profiles, the ledgers and that hand-off are in [docs/toolchain.md](toolchain.md#v1-cargo-xtask-gate-1020) and [`crates/xtask/README.md`](../crates/xtask/README.md).
