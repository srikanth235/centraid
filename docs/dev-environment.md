...
**Those figures are superseded and the deferral is now near-empty.** governance-kit audit 0.11.0 retired `repo-hygiene` outright and rewrote `receipt-per-issue` to a much cheaper check. The bundled catalog is four directives, the repo runs thirteen in total, and the whole suite completes in ~7s in one process — under the 5s rung-0 budget the deferral was built to escape. `receipt-per-issue` is the only id still on the deferred list. Re-measure on the reference machine before treating any number in this section as current.
