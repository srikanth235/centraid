...
The automation fire gate checks the owner's `enrich_policy` tier before any handler traffic. Image automation reads the **thumbnail/preview derivative, never the original** — nobody needs a 48MP RAW to produce an embedding tensor. Deterministic computation is still issue #724's **enrichment service**, the loopback-only HTTP executor configured via `CENTRAID_ENRICH_URL`; the template reaches it through governed `ctx.fetch`, not through a second client or scheduler. See [docs/enrichment-service.md](enrichment-service.md) for the wire contract, config, and capability table.
