...
11. **On the native shells, the `(gatewayId, vaultId)` pair is UNREPRESENTABLE as a placeholder** ([#1020](https://github.com/srikanth235/centraid/issues/1020) wave 3 lane E, D-1020-E4). Rule 10 is a rule a reviewer enforces; in `mobile/shared` it is a constructor. `dev.centraid.shared.mount.MountKey` validates both halves non-blank and **refuses the literal `"manual"` by name**, so the file-naming bug that orphaned every queued write cannot be written rather than merely being caught. The waiting state carries no key at all — `Mount.Waiting(reason)`, with a reason a member can read — because a state that held a key would be a state that could name a file. And switching vaults is a FUNCTION from a mount to a mount (`remount`), not a setter on an active-vault pointer: there is no API shape on the native side that a filter could take.

12. **A screen's state is keyed on its screen instance, and the revision is monotonic per instance** ([#1020](https://github.com/srikanth235/centraid/issues/1020) wave 3 lane E). `ScreenHost` increments a revision on every reduction and `centraid.screen.v1.ScreenState` carries it, so a renderer drops a state it has already surpassed rather than re-rendering out of order. It is per INSTANCE and not global: two Photos covers over two bands are two instances with two revisions, and a global counter would make one of them look stale whenever the other moved.
