...
5. **A multi-scope mount keys on the whole scope SET, not the focused scope** (issue #599). An inline app can be mounted over several scopes at once — the owner's own plus every audience vault they belong to (`GET /_vault/scopes`). The mount owns one replica lease per scope and one `window.centraid` built from that exact set, so the set is a real key axis: `appId : attempt : <sorted scope ids>`. Derive it with `scopeSetKey` so ordering never churns the key, and never key such a mount on "the active vault" — that pointer moving is not a reason to re-mount, while a scope joining or leaving the set is.
