...
| **auto-found** | What a gateway does to **itself** when constructed over a **fresh data dir**: creates the one marked-default `Personal` vault, enrolls the host device's owner, and records that owner in `vault_owners` — silently, with no ceremony, ticket, kit, or screen. Founding is simply the first **mint** (#726 D2). An existing data dir is never modified. | `buildGateway()` in `serve/build-gateway.ts`; `VaultRegistry.isFresh()` |
| **Personal** | The **name** of the one auto-founded vault, not a type: the owner's default vault, renamed to their display name once the desktop profile step completes (headless keeps `Personal`). Founding no longer creates a second `Shared` vault — #726 made the vault the unit of sharing, so a household vault is one a person creates and shares like any other. | `build-gateway.ts`; rename in `packages/client/src/react/boot.tsx` |
...
- **Auto-founding** enrolls the founding owner with no prompt (#603): a fresh data dir gets one vault, **Personal**, recorded in `vault_owners` for that owner in one transaction. There is no auto-founded `Shared` vault — a shared space is an ordinary vault its owner creates.
...
| Term | Meaning | Code |
| --- | --- | --- |
| **give** | A snapshot edge: a one-time copy that becomes the receiver's data. On the product surface it is the receiver-side “Save to my vault” gesture. | `packages/gateway/src/routes/edges-routes.ts`; `packages/vault/src/share/project-closure.ts` |
...
| **edge** | A row in `share_edges`: one snapshot crossing of a fixed item set from an origin vault to an audience vault. `kind='add'` keeps the origin item; `kind='move'` removes it after target receipt, only within one owner's vaults. Live lending was shipped in #726 and deleted in #731. | `packages/gateway/src/serve/gateway-schema.ts`; `routes/edges-routes.ts` |
