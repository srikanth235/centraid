...
| **served app** | An app rendered as an **opaque, same-origin iframe document** the gateway bakes and serves, under the blueprint CSP. Builder preview only — inline since #505, and mobile dropped its WebView cover in #799. | `packages/app-engine/src/http/static-server.ts`; `AppFrame.tsx` |
