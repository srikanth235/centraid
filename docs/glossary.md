...
| **outbox** | The device's durable store of unsettled intents — the second of the two local truths a replica read composes (`replica ⊕ outbox`, #738). Its four unsettled states (`queued`/`sending`/`parked` plus the `awaiting-change` transition) are the only ones a reload can rebuild; a settled or discarded intent leaves it. | `packages/client/src/replica/intents.ts` (`overlayMutations`); `window.centraid.pendingWrites()` |
| **pending-write overlay** | The shared engine (H) that composes an app's unsettled outbox writes into the rows its queries return and tracks their status/reason for rendering. Not a second replica: it is the app-facing declaration over the same outbox composition the replica layer already performs. | `packages/blueprints/apps/_shared/pending-overlay.ts`; per-app `pending-projection.ts`; [blueprint-seats.md](blueprint-seats.md#engine-contracts) engine H |
