...
| **served app** | An app rendered as an **opaque, same-origin iframe document** the gateway bakes and serves, under the blueprint CSP. Retired by #799: mobile dropped its WebView cover, then the desktop/PWA iframe host and the builder that authored for it went, leaving the gateway's serving code with no caller. | `packages/app-engine/src/http/static-server.ts` |
