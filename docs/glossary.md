...
| **delegate step** | An optional alternate step in the same recognition template using `ctx.delegate`, a per-call harness/model pin, and provider-egress consent. Only OCR declares one; it is not a provider kind or second engine. | `packages/server/src/automation/manifest/manifest.ts`; `packages/server/src/acp/automation/run-automation.ts` |
