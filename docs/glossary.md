...
| "the active vault" in user-facing copy | **active space**. The combined sidebar switcher (#608) lists every reachable space before the registered gateways, changes the active/default vault pointer, and is always present so Add gateway remains reachable. Creation flows still name explicit targets where ambiguity matters; Household still lists the full access model. |
