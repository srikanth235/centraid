...
| **automation** | Headless conversation + manifest + handler that fires on schedule, webhook, condition, or vault data change. | [`crates/automations`](../crates/automations/README.md) |
| **harness** | An installed model-capable CLI Centraid can drive for a turn, such as `codex`, `claude-code`, or `opencode`. Code and wire identifiers use `Harness*`; Settings keeps the market-facing label **Agents**. | `crates/assist/src/registry.rs`; [harnesses.md](harnesses.md) |
| **delegate** | A bounded judgment step requested by a handler through `ctx.delegate`; recorded as an item with `kind='delegate'`. It is an act, not a principal. | `crates/automations/src/fire/mod.rs` |
| **agent** | An autonomous L2 principal with an enrolled credential. Never a harness, model call, handler rail, or ledger item kind. | `access_agent` |
| **adapter** | A first-party npm shim that makes a CLI without a native ACP mode speak ACP. No other integration or persistence field uses this word. | `crates/assist/src/adapters.rs` |
| **provider** | The external vendor that receives egress. Use only in egress/consent/model-vendor language, never as the installed CLI's name. | `conversation_provider_consent`; `crates/vault/src/ledger/consent.rs` |
| **Needs you** | The owner-facing place that holds open **decisions**. Informational **notices** stand in Activity, not here ([decisions.md](decisions.md#mobile-ux-consistency-1015) R-NY-2, R-NY-4). | `BandPolicy.kt` in `mobile/shared` |
| **decision** | An item that needs the owner to act — a staged write, a parked high-risk act, a lapsed connection. Only these count toward Needs you. | `replica_parked_payload`; `crates/seat/src/settlement.rs` |
| **notice** | A durable, non-decision update. Repeats collapse by `(kind, source_ref)` and carry read/archive state. | `notifications_notice` |
...
| **ledger band** | The conversation ledger inside `vault.db` — conversations, turns, items, attachments, automation state, archive and digest rows. Same file, same exclusions, 90-day retention window. | [`crates/vault/src/ledger`](../crates/vault/src/ledger) |
...
| **replica** | A seat's full copy of `vault.db`, kept current by the gateway's `replica_log`; writes are intents in the seat's outbox and the gateway is the sole canonical writer ([#996](https://github.com/srikanth235/centraid/issues/996)). | [`crates/seat`](../crates/seat/README.md); `replica_log`, `outbox_item` |
| **predicted write** | A seat runs the real handler for a queued intent and applies what it produced to its replica, so the row changes on screen before any network; settlement later confirms or undoes it. Never "optimistic state" owned by an app component. | `crates/seat/src/pending.rs` |
| **pairing** | One-time ticket ceremony that enrolls a device's iroh EndpointId with a gateway. Redemption burns the ticket and enrolls the device in one call; only a hash of the ticket secret is stored. | [`crates/net/src/pairing.rs`](../crates/net/src/pairing.rs); `crates/centraid/src/allowlist.rs` |
| **pair ticket** | The **only** ticket kind (#603). Always means _join an existing gateway_. Minted by the gateway (`centraid gateway --print-qr`) or `centraid pair`, one-time, burns on redeem. | [`crates/net/src/ticket.rs`](../crates/net/src/ticket.rs) |
| **auto-found** | What a gateway does to **itself** when started over an empty data dir: creates one vault (named by `--vault-name`) and its owner party — silently, with no ceremony, ticket, kit, or screen. An existing data dir is never modified. | `open_or_found_vault` in `crates/centraid/src/run.rs`; `Vault::found` |
...
| **host** | The gateway whose disk and process a vault currently sits on. Hosting is a location, not an authority. See [Owners](#owners-gateway-726). | — |
| **transport / relay** | The iroh QUIC path between a seat and its gateway, one ALPN (`centraid/v1`). Hole-punching falls back to relays over outbound TCP 443 ([R-1020-3](decisions.md#v1-platform--rust-core-kmp-shell-electron-seat-gateway-anywhere-1020)). | [`crates/net`](../crates/net/README.md); `crates/protocol/src/alpn.rs` |
| **byte plane** | Content-addressed blob storage, keyed by BLAKE3, with custody accounting per blob. | `crates/blobs`; `blob_*` tables |
| **skill** | Harness grounding unit (`SKILL.md`) loaded by the harness runtime. | `crates/assist` |
| **recognition automation** | Bundled handler that owns its ML implementation, reads bytes/text with `ctx.vault.content`, and writes model-versioned derivatives with `ctx.vault.invoke`. There is no separate enrichment service or generic inference context. | `crates/automations/src/handler/recipes.rs` |
| **deterministic step** | The self-contained, non-delegate branch of a recognition automation: same input + pinned local specialist model gives the same canonical result without provider egress. | `crates/automations` |
| **delegate step** | An optional alternate step in the same recognition template using `ctx.delegate`, a harness/model pin, and provider-egress consent. `photo-ocr` and `doc-text-extractor` declare one; it is not a provider kind or second engine. | `crates/automations/src/manifest.rs` |
...
| **egress class** | Where an engine's work happens — `on-device`, `gateway` (the member's own infrastructure, not egress), or `provider`. Computed from the engine, never user-set, and the axis egress consent is keyed on. | `crates/automations/src/fire/enrich_gate.rs` |
| **policy cascade** | The scoped enrichment rules — vault, domain, collection, item — stating per capability whether it is enabled, which engine profile runs it, and its trigger. `NULL` inherits, most specific wins, and one resolver folds it inside the gate. | `enrich_policy_rule`; `crates/automations/src/fire/enrich_gate.rs` |
...
| Term | Meaning |
| --- | --- |
| **gateway** | A _role_ of the one core, not a machine: the vault's authority — canonical SQLite file, receipts, retention, backup, iroh endpoint, QR pairing. A headless daemon the member runs on a VPS, a home server, Docker or their own laptop (`centraid gateway install`). It has no admin UI. |
| **seat** | The other role of the same core: a full replica with the applier, the outbox, settlement, and every app's queries and commands. Every device, desktop included, is a seat and pairs by scanning the gateway's QR ticket. A phone is never a gateway. Not the byte seat below (`origin` / `custodian`), which is about where bytes live; the runtime seat's axis is whether it is replicated or thin. |
...
| **desktop** | The Electron app (`desktop/electron`, renderer in `desktop/renderer`). Electron main spawns a `centraid seat` sidecar; the renderer is a thin React client over a local socket to it. |
...
| **Companion** | The browser extension (`extension/`). It reaches the local seat through native messaging to the `centraid` binary (`centraid native-host`); no iroh or WASM in the browser. |
...
| **`centraid` binary** | The one shipped program, with `gateway`, `seat`, `pair`, `devices`, `backup`, `doctor`, `recover`, `export`, `native-host`, `assist`, `automations` and `mcp` subcommands. The CLI is a _client_: every admin verb sends the same command message an owner seat sends. |
...
| **kit** | [`crates/apps/kit`](../crates/apps/kit/README.md) — everything an app crate may import, and deliberately not much: the paged-read grammar as data, `Money`, the door's statement builder, the year-3 fixture generators. An app is a manifest plus two sets of pure functions, and the kit is the vocabulary those functions are written in. |
...
| **band** | A region of the one vault file with its own rules: the **model** (the ontology's schemas), the **audit band** (receipts, provenance, invocations, checks, evidence — append-only by trigger), and the **ledger band** (conversations, turns, items, attachments, automation state). Bands are not databases; they are one ACID boundary and one migration ladder. |
...
| **member key** | `K` — the key Locker secret cells are sealed under, **minted on a seat and never held by a gateway**. The vault learns that a generation exists and when (`locker_key(key_id, created_at, retired_at)` carries no material); a vault with no seat holds no key and therefore no secrets. Not the vault DEK, which a gateway does hold and which seals connector tokens. |
| **drill** | A test that runs the **real binary** against a real artifact rather than calling a library. The restore drill founds a vault, enrols a seat, takes a generation, deletes the live data directory, runs `centraid recover` into a fresh one, compares every table and row, and proves the old seat is told epoch-mismatch, re-pairs and converges. It refuses a census of zero rows, so an empty vault cannot pass by having nothing to lose. |
...
| **daemon** | A `centraid gateway` process under a `--data-dir`, usually installed as a launchd, systemd or Windows service. |

## Seats and byte custody ([blueprint-seats.md](blueprint-seats.md))

| Term | Meaning |
| --- | --- |
| **byte seat** | A device's role in byte custody — `origin` (mobile: content born and cached here) or `custodian` (desktop: the console beside the gateway that holds the bytes). Orthogonal to form factor: never branch custody on width or layout on seat. |
...
| **simulation** | The deterministic sync proof: one gateway and N seats in one process under a scripted network, with a fixed seed. A failing seed is a reproducible test case and stays in the suite ([R-1020-30](decisions.md#v1-platform--rust-core-kmp-shell-electron-seat-gateway-anywhere-1020)). The `sim` step runs 25 seeds; `sim-nightly` runs 250. | [`crates/sim`](../crates/sim/README.md) |
...
**Minting is not owning, and neither is hosting.** The host can run the process that serves a vault and read its rows — a gateway is trusted for data and blind for secrets ([R-1020-9](decisions.md#v1-platform--rust-core-kmp-shell-electron-seat-gateway-anywhere-1020)) — but hosting confers no authority over the vault's relationships or its devices. Full posture is in [SECURITY.md](../SECURITY.md).
...
| **subscription** | The audience vault's standing subscription to a grant-keyed shape the origin serves. Consent lasts with membership, not a clock; derivatives remain seat-local. Retires **commons** as the word for co-owned resident data. | `share_subscription`, `share_subscription_member`, `share_subscription_lineage` |
...
| --- | --- | --- | --- |
| host / gateway | **gateway** for the product process | "host" for the machine a vault sits on | See [Owners](#owners-gateway-726) |
