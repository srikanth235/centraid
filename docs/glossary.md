...
| **device-only claim** | A fact about the **operating system's** behaviour rather than the product's logic, so no unit, component, or Playwright layer can falsify it. Each is written to be adoptable verbatim as a claims-ledger cell owner, and only the `origin` seat can hold one. | `tests/agent-e2e-mobile/roster.json#flows`; the flow each row names |
| **suite** (mobile) | A named, ordered set of device journeys with ONE aggregate wall-clock ceiling, one canary rule and one pairing-reuse rule. It is the unit of parallelism on a device lane and the unit a budget prices — never a dynamic shard, because a shard computed at dispatch time is not readable by the linter that proves a flow is scheduled. | `tests/agent-e2e-mobile/roster.json#suites`; `run-roster.mjs --suite <id>` |
| **candidate** | A `main` SHA that passed the whole rung-3 ladder. `refs/candidates/latest` points at it and `artifacts/candidate.json` describes it; the nightly, the weeklies and the release lane all run on the candidate rather than on the tip of `main`, so a red distinguishes "the product regressed" from "the 04:00 merge broke the build" ([decisions.md](decisions.md#the-quality-ladder-915)). | `.github/workflows/candidate.yml`; `artifacts/candidate.json` |
| **lane** | One GitHub **job id**, and the unit the report has a row for. A matrix leg is `<job> (<leg>)`; a display name is never a lane identity. Every lane on rungs 2–5 is registered in `tests/claims.json#lanes` with its rung, platform, budget and the qualities × surfaces it claims. | `tests/claims.json#lanes`; `bun run lint:evidence-mapping` |
| **evidence** | One `artifacts/evidence/<lane>.json` per lane per run: what it proved, on which candidate, in how long, and which cases it ran. The nightly report is a pure function of a directory of these plus the claims file, so a lane that writes nothing renders `no evidence` rather than disappearing ([TESTING.md](../TESTING.md#the-evidence-contract)). | `scripts/test-report/write-evidence.mjs`; `scripts/test-report/evidence-schema.mjs` |
| **park** | A lane that has been red long enough to stop being information, given an **expiry** (≤ 14 days) and a rolling issue. A parked lane still runs and still writes evidence; the report renders it `parked` and leaves it out of the verdict until the expiry, at which point it counts as red again. More than three parks, or any park older than 30 days, is itself a HOLD. | `tests/quarantine.json#lanes`; `scripts/test-report/write-evidence.mjs` |
| **claim** | A row in `tests/claims.json#claims`: what the product promises, the file that owns the proof, the lane that runs it, its severity `S1`–`S4`, and the date it was last demonstrated red. The 45 rows are the retired user-facing qualities panel; `bun run lint:quality-knobs` refuses a removed claim without an approved deviation. | `tests/claims.json`; `scripts/check-quality-knobs.mjs` |
| **rung** | Which of the ladder's six questions a check answers: 0 commit, 1 push, 2 merge, 3 candidate, 4 nightly, 5 weekly. A mobile suite declares its rungs and a lane declares one; rung 2 is the only rung that blocks a merge, and only rung 2 lanes are `blocking` ([decisions.md](decisions.md#the-quality-ladder-915)). | `tests/agent-e2e-mobile/roster.json`; `run-roster.mjs --rung <n>` |
| **marginal budget** (mobile flow) | One journey's own cost with **no fresh pairing in it**, which is why a suite's members may sum past the suite's aggregate ceiling: the suite ceiling prices the pairings, the flow budgets price the work. Both are tighten-only. | `tests/agent-e2e-mobile/roster.json#flows[].budgetMs` and `#suites[].budgetMs` |
| **harness failure** | A device journey whose OWN script threw — an unimported helper, a bad reference — so no product claim was ever exercised. Neither `infrastructure` (nothing flaked; a retry reproduces it) nor `product` (nothing was asserted); never retried, and named as a defect in the flow. | `tests/agent-e2e-mobile/lib/failure-class.mjs`, signal `flow-crash` |
| **provisional-local floor** | A mutation floor seeded from a local Stryker run rather than CI, set at (local measured − 11) and re-seeded to (CI measured − 3) by the first green nightly that measures it. The exception the file declares at the number, never a silent one ([decisions.md](decisions.md#adversary-lanes-and-provisional-evidence-839)). | `tests/floors.json#mutation` |
| **ledger** | One of the four tighten-only JSON files under `tests/` ([#915](https://github.com/srikanth235/centraid/issues/915) Wave 4): `floors.json` (up-only), `budgets.json` (down-only), `inventory.json` (down-only, issue and expiry per exception) and `quarantine.json` (flaky tests and parked lanes, expiry). Each is a set of named **sections**, each section has one direction and its own waiver. | `tests/floors.json` |
| **section** (ledger) | A top-level key of a ledger — the unit a direction, an `approvedDeviation` and a base-side fallback path all attach to. Cited as `tests/budgets.json#suiteWallClock`. A widen is waived only by a CHANGED note in the section being widened. | `scripts/check-ledgers.mjs` |
| **derived mirror** | A ledger section whose numbers are copied from the document that also holds their context and asserted equal by `bun run lint:ledgers`, so the ledger indexes them without becoming a second source of truth. Two exist: `floors.minimumTests` from `tests/claims.json` and `budgets.mobileSuites` from the mobile roster. | `node scripts/check-ledgers.mjs --write` |
...
| **fulfillment** | Per-audience-vault delivery state under one grant — `awaiting_channel` (the peer vault is known and the link to it has ended), `syncing`, `delivered`, `remove_sent`, `removed`. Mechanism, never meaning: a member reads grants, never fulfillment rows. | `share_fulfillment` |
| **channel** | How to reach a party — the vault link, read as a property of the party, with two states: `live` and `severed`. There is no third: the People link ceremony is the only thing that opens one, and a grant naming a party without a live one is refused (#903). | `share_party_vault_binding` + `vault_links` |
...
| "invitation" for the act that makes a person shareable | **link** — the People ceremony that writes `share_party_vault_binding`. Nothing is sent on the member's behalf, and a commons invitation is a roster ask among people already linked, never a way to reach a new one (#903) |
