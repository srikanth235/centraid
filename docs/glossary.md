...
| **Personal / Shared** | Names, not types. `Personal` is the auto-founded owner's default vault, renamed to their display name once the desktop profile step completes (headless keeps `Personal`). `Shared` is an ordinary vault an owner may create later; it is not created by founding and nothing on its record means "sharing". | `build-gateway.ts`; rename in `packages/client/src/react/boot.tsx` |
