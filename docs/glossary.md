...
| **recognition automation** | **Retired in v0.** The worker and every `enrich.*` command were deleted in [#1029](https://github.com/srikanth235/centraid/issues/1029); the `enrich_*` and face tables stay in the schema with no production writer. Earlier meaning: a bundled deterministic derivation (OCR, transcription, embeddings, faces) stamping `enrich_derivation` with its pinned `model@version`. | [recognition-automations.md](recognition-automations.md) (superseded) | | **deterministic step** | **Retired with recognition** ([#1029](https://github.com/srikanth235/centraid/issues/1029)): same input plus a pinned local model giving the same canonical result. | — |
