...
`migrations/<NNNN>_<name>.sql` is where your app's SQLite schema lives. Migrations run forward, are ordered by the numeric prefix, and are the authoritative source — the `tables[]` array in `app.json` is documentation only.
