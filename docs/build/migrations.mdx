...
> **TODO(#120)** — document the exact lifecycle: do migrations run at upload time, at first read after a version flip, or on gateway start? `current.json` records `activeVersion` and `history`; whether the runtime tracks `appliedMigrations` separately matters here. Read `app-engine/src/migrations/` (or wherever) to confirm.
