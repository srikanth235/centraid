...
- [Automations](/automations/index) — handlers that run on a trigger instead of a UI click. Different file (`handler.js`), different ctx (`ctx.tool`, `ctx.agent`, `ctx.state`, `ctx.runs`), different audit trail (`runtime.sqlite`).
