...
An automation can pin its authoring/interactive runner, default model, and thought level in `automation.json`: `requires.runner` is an open, non-empty registry key, `requires.model` is the model id/alias, and `requires.thoughtLevel` is the semantic effort value. `requires.runner` selects headless compilation and interactive automation turns; it also remains the compatibility fire-level default for existing handlers whose `ctx.agent` calls omit `runner`. It is not a step-routing constraint: compiled handler code names a step-specific harness/model with `ctx.agent({ runner, model, ... })`, and those call arguments take precedence over the fire defaults. An unknown future/removed manifest key falls back to `runner.automations` (then the default agent); an explicit call key instead fails typed before egress. Per category, preference resolution remains manifest default, runner+subsystem preference, runner-wide preference, then backend default. Any automation-specific conversational route must reuse this resolver rather than reading subsystem prefs directly.
...
- **Billed rail** — `ctx.agent({ runner?, model?, prompt, json?, content? })` — a bounded one-shot turn routed through the **same single ACP backend as chat** (`getRunnerBackend(kind).runTurn`). A supplied runner or model overrides that dimension for the call. Omitting a runner preserves the fire runner; omitting a model preserves the selected runner's existing fire/config-pin model resolution. A named runner must be registered and enrolled in the user's automations primary/failover ladder before anything egresses. Each runner keeps its own resumable harness binding, breaker state, and confirmed model/usage on that call's `agent` ledger item.
