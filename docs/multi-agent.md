...
- **Heavy suites are serialised.** Two full suites in parallel on a small machine both time out, so they run through a shared lock. Sibling agents prefer crate- or file-scoped runs (`cargo test -p <crate>`, `cargo test -p <crate> --test <file>`, and for the remaining TypeScript packages `bunx vitest run path/to/file.test.ts`) and bail on first failure; the full run belongs to the agent that owns the push. See [dev-environment.md](dev-environment.md#the-local-gate-loop) for which rung runs what.
...
- **Never restart a shared long-running service** — a gateway daemon, a `centraid seat` sidecar, a simulator or emulator, a database under a shared `dataDir` — without permission. Stop only what you started, in your own worktree and on your own ports; if you need a port someone else holds, ask or pick another rather than killing by pid.
