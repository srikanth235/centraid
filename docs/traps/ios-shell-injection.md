...
- **`hermesc` is not in the cache by default.** It lives at `ios/Pods/hermes-engine/destroot/bin/hermesc`, and Pods are deliberately **not** cached (a Pods cache hit makes Expo skip `pod install`, which is the only thing that runs the `centraid-tunnel` podspec's `prepare_command` — see the note in `e2e.yml`). So the cold path banks `hermesc` **beside** the `.app`, under the same content-addressed key. That is sound because the key carries the native fingerprint and `hermes-engine`'s version follows the pinned `react-native` version, which is one of that fingerprint's inputs (`package:react-native`): a banked `hermesc` is always the one that built the banked shell. Under CNG the path itself only exists after `bun run --cwd apps/mobile native:prebuild` + `pod install`, which every iOS lane runs in a step of its own ([#996](https://github.com/srikanth235/centraid/issues/996)). A shell banked before this path existed has no `hermesc`, and `ios-shell-cache.mjs` rebuilds rather than injecting.
