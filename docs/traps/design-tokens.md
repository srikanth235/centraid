...
- **Kit layer** (`kit/`, referenced as `@centraid/design/kit`) — the component substrate app surfaces load: `kit.css`, `kit.ts`, the chart and status-line elements, and the `@`-mention controllers. Apps import it as a sibling (`./kit.ts`); each shell's bundler rewrites that specifier to this dir through `inline-vite-aliases.ts`. The raw-`.js` shape it still carries is a leftover of the retired serving plane (#799) and dissolves into the token layer under the same issue.
