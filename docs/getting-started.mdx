...
Centraid ships as a monorepo. The desktop app embeds a local gateway out of the box — no separate server to run. The same gateway code also runs as the standalone `centraid-gateway` daemon for headless or always-on use, and can be mounted on an OpenClaw instance to host apps remotely; all three modes share one upload-and-version-flip contract.
