...
| `POST /centraid/<app>/_turn` (+ `GET/PUT /_turn/model`) SSE chat | `router.ts` `app-chat` → `turn-routes.ts` | **all 8 apps**: `window.KIT_ASK` + `[data-ask-mount]`; `kit.js` ask panel + `kit/conversation-client.js`/`turn-stream.js` | shell conversation surface (`gateway-client-conversation.ts` / `useAssistantConversations`) scoped to the app; parked writes appear only in Inbox (#647), never as inline consent cards |
...
| Decision surface `/centraid/_vault/inbox` (+ canonical decision endpoints) | gateway vault plane | none — the Ask panel deliberately has no decision controls | shell-native Inbox; decisions remain canonical and one owner surface renders them (#647) |
