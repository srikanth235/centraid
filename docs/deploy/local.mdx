...
The default. When you launch the desktop app with `bun run dev:desktop`, the Centraid gateway runs **inside the Electron main process**. The renderer talks to it over IPC; app iframes load from the in-process gateway as if it were a remote URL.
