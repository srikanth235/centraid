...
> **TODO(#120)** — document the .jsonl event shape. From the README: "SSE stream of normalized events" describes the wire format; the .jsonl transcript almost certainly stores the same events. Confirm against `app-engine/src/chat/`.
...
- **No cross-app handler database.** Each app's `data.sqlite` and `runtime.sqlite` are isolated; no handler ever opens another app's files. The gateway-level `centraid-analytics.sqlite` is a _rollup_ (one summary row per run) — handlers can't write to it.
