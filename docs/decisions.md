...
- **Recognition is self-contained automation.** `photo-ocr`, `transcript`, `embed-image`, `embed-text`, and `faces` are deterministic templates whose bundled handlers read through `ctx.vault.content`, run their own ML implementation, and persist through `ctx.vault.invoke`. Model assets may be local. The automation engine owns scheduling, bounded cursor watermarks, policy, retries, Test run, and ledger history. There is no service wire, `ctx.infer`, or `ctx.enrich`; `enrich_derivation` stamps remain vault provenance. Only OCR may replace the deterministic step with a consented, pinned, billed `ctx.delegate` step.
