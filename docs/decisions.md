...
| **A-pinned** | An adversary lane that finds a defect it is not chartered to fix **pins** it. A characterisation test asserts the current, wrong behaviour and names the ruling or documented sentence it contradicts, so the day the product moves — fixed or made worse — the pin goes red and the record is revisited deliberately. Tolerating the failure quietly and deleting the finding are both refused, and a register entry is never added to go green. The standing pins are the three registered classes in `scripts/fuzz/known-findings.json`, the fall-back double fire against [cron-timezone.md](cron-timezone.md#dst-policy), defect D1 against [G-revoke](#sharing-v1--the-grant-plane-825), and the two peer-target guard disagreements pinned by `packages/tunnel/src/peer-target-differential.test.ts` (#842 W2.1): a bare peer-plane prefix plus a query or fragment is wrongly admitted (the length test reads the whole target, not the path), and a lone surrogate is admitted JS-side but unrepresentable in the Rust `&str` guard; the owner-facing `GET /centraid/_gateway/diagnostics` bundle emitting the owner-authored vault name verbatim, pinned by `tests/quality/diagnostics-redaction-canary.test.ts` against [logs.md](logs.md), which tells a person to attach that document to a support request (#842 W8.1); and an automation worker started with no parent-chosen sandbox lane still reaching the filesystem, subprocesses and the environment, pinned by `packages/server/src/engine/sandbox/sandbox-escape.test.ts` against [SECURITY.md](../SECURITY.md)'s consent-scoped-handler framing — the default cannot flip until `packages/model-runtime/src/onnx.ts` stops resolving through `createRequire`, which every lane refuses (#842 W7.1). |

**Defect D1 qualifies G-revoke; it does not amend it.** The commons simulator reaches a schedule where `fulfillShareGrant` drops a `delivered` fulfillment row back to `syncing` because the host could not reach the peer for one pass, and `propagateShareGrantRevocation` then reads that `syncing` as never-delivered: it settles `removed` — nothing had been delivered, so there was nothing to remove — while the audience vault still holds the whole projection. The owner is told the share is gone and it is not. G-revoke's sentence is unchanged, because it states the intent and the intent is right: revoke is honestly best-effort against a peer's disk. What is wrong is the engine, on a reachable path, in the one direction the copy does not warn about — the admitted failure is a peer that keeps a copy it was asked to drop, not a host that never asks. The divergence is carried by the `test.fails` case in `packages/vault/src/share/commons-sim.test.ts`; the day the engine remembers what it delivered, that case passes, turns red, and the pin is deleted there and in its grant-plane fixture.
