...
- **`consent_share` dropped outright, not rebuilt.** The rejected filtering model's one vestigial writer is removed with its DDL, poly-refs entry, and tests. `share_edges` — a durable ledger for give/lend edges — arrives in a later #726 phase as its own thing, not a resurrection of `consent_share`.
...
- **Remote give ships as a snapshot; lend is deferred, not half-built.** An edge's `mode` is `snapshot` (a one-time copy — shipped, including across the peer plane) or `live` (a continuously synced view — accepted by the schema, refused by the route typed `not_yet_supported`) so the column exists once rather than being added later as a migration; the lend semantics themselves are a later phase's work, not simulated here.
