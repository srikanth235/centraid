...
- **Recognition is automation orchestration.** `photo-ocr`, `transcript`, `embed-image`, `embed-text`, and `faces` are deterministic templates using governed `ctx.fetch` to the unchanged enrichment-service wire. The automation engine owns scheduling, bounded cursor watermarks, policy, retries, Test run, and ledger history; the service only executes model batches. The gateway-private capability sweeps and generic `supersededTargets` selector are deleted; `enrich_derivation` stamps remain vault provenance. Only OCR may replace the deterministic step with a consented, pinned, billed `ctx.agent` variant.
...
| Local OCR | **Superseded by #724/#731.** Traditional OCR runs through the loopback enrichment service as a deterministic automation step; OCR alone also offers an explicit provider-egress-consented agent variant. |
...
| **724-2** | **One enrichment service, and the queue is the app API.** A single HTTP seam (`packages/gateway/src/enrich/service-client.ts`) serves embeddings, OCR, faces, and transcripts. Apps never call it directly. #731 moved scheduling from private sweeps to bundled deterministic automations without changing this wire. |
