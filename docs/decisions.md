...
| **T1** ([#555](https://github.com/srikanth235/centraid/issues/555), supersedes #505) | **Gateway connections are iroh-only and identified by EndpointId.** Per-device HTTP tokens, direct URL connections, and `POST /centraid/_gateway/pair` are removed. Relay hints are refreshable cache. Every request resolves a real vault enrollment; there is no wildcard admin plane. |
