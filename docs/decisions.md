...
- **Every new vault gets an Ed25519 identity keypair, minted once; existing vaults never backfill on mount.** `VaultRegistry.create()` mints the seed and directory record as one new-vault operation. A later mount must load the same seed and match its public key to both vault and gateway directory; loss or mismatch fails closed per #750. The seed lives beside the vault's DEK in the same `KeyStore` envelope, and the **recovery kit — not the portable export — carries it.** `RecoveryKitTarget` carries `identitySeed` alongside `sealKey`, both mandatory for `recover()`; the portable export deliberately does not, because an identity seed is signing authority, not vault content.
...
One historical implementation delta remains: founding is not literally the one-person `ProvisionPerson` shape because its two-vault, marker-tagged transaction is different. Both paths do use the same new-vault identity invariant; neither may mint for an existing vault. P1's backup-target ownership check was scoped to the backup **policy** route (`PUT /centraid/_gateway/backup/policy/:vaultId`) rather than the CAS blob-store attach route, because the policy route sits on the same admin-tier family as the other P1 refusals and had no ownership check at all before this change — the blob-store route is flagged as a follow-up, not silently skipped.
...
## #750 — vault sharing lifecycle consolidation

Settled **2026-08-12** in [#750](https://github.com/srikanth235/centraid/issues/750), amending #726/#731 without preserving pre-v0 formats:

- **Stable identity, replaceable location.** New vaults mint once. Existing vaults never mint on mount: missing or mismatched key material is a named, fail-closed fault. `vault_directory` owns public key/label, `vault_routes` owns one signed current location, and `vault_links` owns approval only. Endpoint start, rotation, mount, and recovery invoke route assertion delivery in production.
- **Owner authority, device provenance.** Request context resolves `{ ownerId, deviceId }` once. Lists and repositories scope authority to the owner; `created_by_device` remains audit provenance. Human link labels are contractual client fields.
- **One effect grammar.** `share_effects` replaces the specialized give-decision, blob-pull, refusal, and Commons-invitation queues. Local versus peer is executor selection, not a second reducer. Commons intents use the same state vocabulary; they remain in `vault.db` because they are signed member inputs and part of backup-resident grant state, while effects are gateway-local transport work.
- **Borrowed/live is deleted, not consolidated through adapters.** #731 already removed its schema, stores, routes, leases, change log, and CAS lifecycle. #750 does not recreate a compatibility or migration path; its borrowed acceptance cases are structural deletion proofs.
- **Person provisioning is one durable operation.** Endpoint/ticket capability is preflighted before planning. Stable ids and secret key name are recorded under the caller's operation id; filesystem creation is idempotent and owner/ownership/ticket commit atomically. Replay resumes; it never duplicates.
- **Commons domain catch-up is executable tail.** One operation behind means one canonical command on each seat, with deterministic per-sequence row ids. The full closure is reserved for baseline/laggard/recovery, so unchanged seat-local OCR, embeddings, and FTS survive. Blob access is authorized once per `(peer pair, grant, sha)` and chunks stream through a promotion temp file. Command routing and capability exposure are declared data with production-call conformance checks.
- **Steward recovery is explicit ceremony.** Presence and recovery are reachable on web/native; re-founding enqueues invitations. Pairwise link consent is not widened transitively, so N≥3 missing direct links require the ordinary link ceremony documented in [the steward-loss runbook](recovery/commons-steward-loss.md). Seat-local supersession can split-brain; the runbook makes the human winner and cleanup explicit.
