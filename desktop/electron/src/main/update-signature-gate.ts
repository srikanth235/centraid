 ...
 *   oversight; widening it to make a v1 update path work is exactly the "never
 *   weaken policy to go green" case. A v1 update path is enrolment-gated the
 *   same way signing is — see `.github/workflows/lane-release-desktop.yml`.
 * - `RELEASE_ASSET_BASE` stays a constant, so a compromised feed cannot
 *   redirect where the manifest is fetched from. */
