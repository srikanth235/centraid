...
- **F8 — the SonarCloud gate named two ratings and no findings, so the rules were run locally instead.** `sonarcloud.io` is refused by this environment's egress policy and the analysis is server-side Autoscan, so the D Security / C Reliability verdict arrived with nothing to act on. `eslint-plugin-sonarjs` **is** the SonarJS analyser, so the full rule set was run — untyped and type-aware — over exactly the PR's non-excluded changed files. Four defects, all real: `gateway/execution.ts` built a 14-entry `ENTITY_POINTERS` walk on every foreign-key failure and fed it to `(pairs.length > 0 ? "" : "")`, both branches identical (S3923). The walk was deleted rather than completed, because it could never have produced anything — it passes `pointer.table`, a PHYSICAL name, to `resolveEntity`, which returns `undefined` for anything without a dot, so `pairs` is always empty; finishing the sentence would have been writing new code to fix a second latent bug, not restoring a lost one. `gateway/sealed-artifact.ts` cast `JSON.parse(value)` to `Record<string, unknown>` (S3403); the `null` guard the rule calls dead is correct at runtime and stays, but the ARRAY case passed `typeof === "object"`, was iterated, and reported `payload_json:0` as an unexpected sealed value — a legitimate import refused for a numeric field name that does not exist. `install-memory.ts`'s `scope.table === null` is unreachable by type and real at runtime (manifest JSON can express `table: null`), so it collapses to `== null`, which `oxlint.config.ts` permits. And `commands/atlas.ts` interpolated caller-supplied JSON keys as SQL identifiers: `guardWriteTarget` checked the table and refused sealed column NAMES but never checked the keys against the real column set, so a key carrying a quote escaped the identifier and wrote a sealed column — reproduced end to end, the `password` cell going from `NULL` to a caller-supplied value. It now rejects any column not in `tableInfo`, ahead of the sealed check. Those `prepare()` lines are not new in this wave; the fix is folded in because the bypass is reachable and the diff is small. `schema/ontology-doc.ts` and `doctor.ts` route their PRAGMA identifiers through `JSON.stringify` like every sibling. The Security D was left unresolved in that pass, on the reasoning that no Vulnerability-class rule fired locally — see F9, which is what it actually was.
- **F9 — the D Security was two `runInNewContext` calls, and the page is parsed now instead of run.** The dashboard, read directly, named them: `schema/ontology-doc.ts` L223 and L228, *"Make sure that this dynamic injection or execution of code is safe"*, Critical Vulnerability, tagged `former-hotspot` — a review hotspot SonarCloud had promoted, which is exactly the class `eslint-plugin-sonarjs` does not reproduce and why the local sweep came back empty. `parseOntologyDocSchemas` and `parseOntologyDocMachinery` sliced two array literals out of `ontology-body.html` and handed each to `node:vm`. Whatever the slice held was EXECUTED, and an empty sandbox is not a boundary — it shares the process, so a `while(true)`, a `process.mainModule` walk or an `fs` reach authored into a 1,600-line hand-edited HTML page ran with the test runner's privileges, in a lane that runs on every PR. Nothing about the data ever needed evaluation. `parseDataLiteral` is a recursive-descent reader for the one grammar §03 is written in — arrays, objects with bare keys, single-quoted strings with escapes, numbers, `true`/`false`/`null` — and every other token is a parse error, so a call, an identifier, a template literal or a trailing statement is refused where it used to run. Numbers are scanned character by character rather than by regular expression, because the input is 91 KB and a scanner has no backtracking to reason about. `__proto__` is defined as a plain data key, so a page entry cannot reach a prototype. Proved equivalent, not assumed: both arrays parse to output byte-identical to the `vm` path (`JSON.stringify` equality, 89,324 and 1,943 characters), which is what says this is a swap of mechanism and not of meaning; the structured-clone realm dance the sandbox needed is retired with it, since the parser builds its values in this realm. `extractArray` stops at the `]` rather than the `;` — what comes back is one value, never a statement. Four tests cover the grammar, the refusals, the malformed inputs and the `__proto__` key. The C Reliability finding on the same dashboard (`gateway/execution.ts` L144, *"returns the same value whether the condition is true or false"*) is the dead ternary F8 already deleted in `5996fbff`; the dashboard's snapshot predates that commit.
- **F12 — the bundled-manifest scope census still counted Locker's two dead scopes.** With F10's planner fix in place, `new-test-burn-in (7)` ran `packages/server/src/serve/manifest-scope-denial.sweep.test.ts` and it failed all three runs on `declaredScopes: 278` against an actual 277. The census is right and the pin was stale: F-Locker replaced the two scopes naming the dropped `locker.item_history` (`read` and `reveal`) with a single `core.entity_revision` read, a net loss of one. The reveal scope did NOT move — nothing may reveal a revision, which is the same rule the pane's copy and `sidecarAskOf`'s deleted `history:` branch enforce, so a previous password leaves only through a confirmed export. `apps`, `automations` and `withScopes` are unchanged, which is what says the manifest tree did not move under the sweep: only the scope count did. The pin is updated to 277 with the reason beside it. This is the burn-in lane doing its job on the first run where it could see the file at all — under the old planner this suite was one of the 104 that reported broken without ever running.
- **F11 — the Android build died at `lintVitalRelease`, on an exclusion list that named two of three tasks.** The gate reached a cold gradle build for the first time (F5 made a cache miss rebuild), ran 18m19s of real compile, and then failed validation: `:app:lintVitalRelease` is an `AndroidLintTextOutputTask` whose `returnValueInputFile` and `textReportInputFile` are produced by `lintVitalAnalyzeRelease` and `lintVitalReportRelease` — the two tasks `android-emulator-install.sh` excludes. Excluding the producers and leaving the consumer in the graph does not skip it: gradle refuses the task outright with "An input file was expected to be present but it doesn't exist" and a `WorkValidationException`, so the whole build fails after paying for the compile. The third `-x` is added; the exclusion is still by unqualified task name, so an AGP rename still fails loudly rather than quietly restoring a 4-minute lint. **This is not this branch's defect and it is not the caching restructure's** — `#905` added the two exclusions, and it has been latent on `main` ever since, because the lane hard-failed on an apk-cache miss until this wave reversed that (F5) and because a warm build skips all three tasks. This branch is simply the first thing to reach a cold build. Fixed here rather than reported, since the reversal is what made the path reachable and the fix is one flag. **Not verified locally** — there is no Android SDK in this environment; the change is derived from the failure's own text and AGP's task wiring, and run 33711841127 is the reproduction.
- **F10 — `new-test-burn-in` reported eight red legs, and not one of them was a test.** Every shard failed on a different `packages/blueprints/apps/**` file with "failed all 3 runs — the test is broken, not flaky". They were the same defect: `burn-in.mjs` runs each file from "the nearest ancestor with a `package.json`", and every blueprint — `agenda`, `docs`, `locker`, `notes`, `people`, `photos`, `tally`, `tasks` — carries a private one declaring a dev dependency, with no Vitest config. So the runner started vitest in a directory whose `include` cannot see the file, got `No test files found, exiting with code 1`, and reported a suite that never ran as broken. **104 test files across all eight apps**, measured by replaying the old rule over the real tree. This is #915's own regression one level down: that fix taught the planner `scripts/**` is not a Vitest project and never asked the same question of a nested package. The predicate is now `package.json` AND a Vitest/Vite config — what vitest actually needs to collect the file it was handed — and `nearestPackageDir` is renamed `nearestVitestProjectDir` to stop the next reader making the same assumption. Adding a config to each blueprint would have silenced it too, and much worse: `packages/blueprints/vitest.config.ts` owns `apps/**` on purpose, carrying the design-package SOURCE aliases and the `*.module.css.js` harness its apps need, so eight copies would be eight things to drift and each app's isolated run would differ from the suite run it is meant to reproduce. Verified: the four files the shards named burn in 3/3 in isolation, and a new property test walks all 1,541 real suites and asserts every plan lands in a directory that has a config — 104 stranded under the old rule, none under this one. The sharding was not the cause and is not reverted; it is what let the lane reach these files at all, where before it was cancelled at 25 minutes having finished 116 of 237.
...
**Locker's history pane reads the shared revision ledger, and no longer offers a previous password.** `locker.item_history` is dropped and its rotation record lives in a `core_entity_revision` snapshot; the Locker blueprint's readers were the last thing still naming the dropped table, and they are ported here — so the pane works again, and this wave's one user-visible regression is closed rather than carried.

What a member sees changed with it, and the change is a real narrowing. Before, each revision row carried a `Previous password` beneath it with `Reveal` and `Copy`, addressed by a permit against `locker.item_history.password`. Now a row names the operation, the columns that changed and the instant, and a rotation adds "Previous password kept · sealed in this revision, and readable only through a confirmed export." There is no reveal affordance and no permit address: `historyPasswordKey` and the `history:` branch of `sidecarAskOf` are gone, so nothing can address a revision at all. That is not a downgrade dressed up as a feature — the one path to a previous password is now `locker-export.ts`, which unseals each snapshot under the export's own confirmation and receipt, and the pane's copy points at exactly that.

The pane reads through the same `core.entity_revision` shape as People, Tally and Photos rather than a Locker-shaped query, and it stays a section of the one `LockerDetail` payload — Docs and Tally lazily land a separate `revisions` read because they have an undo action acting on `undo_until`, and Locker has no undo command, so a second authenticated round trip would buy nothing. `undo_until`/`undone_at` are deliberately not surfaced for the same reason: an undo countdown with no undo behind it is inert copy.

One correction to the brief this was ported under: **`snapshot_json` is not sealed-free.** `revision-capture.ts` builds it with `json_object(… OLD."password" …)`, so a snapshot carries the item's CIPHERTEXT, and the gateway's sealed masking covers declared sealed columns — which `core_entity_revision.snapshot_json` is not. So the handler opens the snapshot and never forwards it: the diff runs off `REVISION_COLUMNS`, an allow-list of plain columns, and a rotation is named by `password_set_at`, the plain column the vault re-stamps exactly when a password is set. Comparing two ciphertexts would answer "was this cell rewritten", not "did the value change". A test asserts no ciphertext and no `snapshot` key rides the payload.
...
- **The `verify` job's perf lane runs now, and it fails one budget.** F4 above got the harness past warmup; with `CENTRAID_BENCH_REQUIRE_FSYNC=1 CENTRAID_HARDWARE_PROFILE=constrained bun run test:perf:pr`, seven of eight budgets pass — `request.p99Ms` 43.1 ms of 250, `memory.rssPeakBytes` 224 MB of 512, `eventLoop.peakP99Ms` 85.5 ms of 150, `idle.contextSwitchesPerHour` 190 k of 500 k, `idle.liveDataGrowthBytesPerHour` 0 of 10 MiB, `storage.diskWriteBytesPerWrite` 101 KB of 128 KB, and `storage.fsyncPerWrite` 0 of 6 (strace ran; the trace epoch recorded no fsync). The eighth, `idle.diskWriteBytesPerHour`, measured **12.0 MB/h against a 10 MiB/h ceiling** and was red — reproduced across two runs (12.02 MB/h and 12.48 MB/h). **It was this change set's doing, and the earlier reading of it here — "not this change set's doing, what an idle gateway writes" — was wrong.** Measured on a built worktree of the merge-base `07465a3b`, the same harness and the same 65-second window report `idle.diskWriteBytesPerHour` **0**; on the branch, 13.16 MB/h. The delta is the receipt plane: `computeDueReminders`/`nextReminderFireAt` now read through the gateway (F-review-A 8.1), so `PushWakeRelay.armDue` — a debounced timer that fires on the idle path — went from five raw SELECTs to five `gateway.read`s, and every gateway read appends an `access.receipt`. `access_receipt` spans five b-trees, so each single-row append dirties five 8 KiB pages and commits them on its own: 41,080 WAL bytes and an fsync per read, 205,400 bytes per idle scan, traced statement-by-statement with `strace -e trace=pwrite64,fsync` and a `node:sqlite` statement probe. Fixed at the source in this change set, not by moving the ceiling: `armDue` now takes one scan instead of two (`scanReminders` reads `schedule.task` and `schedule.event_ext` once and derives both the due list and the next fire instant, where the two calls used to re-read them at the same instant), and that scan runs inside `Gateway.readBatch`, which lands the same receipts in one commit instead of three. Same evidence, a fifth of the WAL: **3.63 MB/h**, stable across three runs (3,629,671 / 3,629,648 / 3,629,652), all eight budgets green.
...
### packages/vault/src/gateway — the access plane and the duties (46)
