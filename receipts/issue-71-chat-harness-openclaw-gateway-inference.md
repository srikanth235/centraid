...
- [packages/local-chat-runner/src/local-chat-runner.ts](../packages/agent-runtime/src/conversation-adapter.ts): `makeLocalChatRunner` becomes a thin wrapper that constructs `cwd = <appsDir>/<appId>`, splices the chat preamble into `extraSystemPrompt`, reads/writes the per-window `ChatStore` for resume, and delegates to the primitive. Public surface unchanged — chat path is byte-for-byte equivalent.
- [packages/local-chat-runner/src/types.ts](../packages/agent-runtime/src/types.ts): dropped `RunOneTurnArgs` / `AdapterCtx` (only used by the old chat-shaped adapter signatures); kept `RunnerKind` + `RunnerPrefs`.
- [packages/local-chat-runner/src/index.ts](../packages/agent-runtime/src/index.ts): now exports the primitives (`runCodexTurn`, `runClaudeTurn`) and their input/config/result types alongside the existing `makeLocalChatRunner`, so a future builder-harness session can import them directly without a circular dep.
- [packages/local-chat-runner/src/centraid-cli.ts](../packages/agent-runtime/src/cli/centraid-cli.ts): added `centraid preview snapshot` subcommand returning `{ path, exists, sizeBytes?, mtimeMs?, ageMs? }` for `./.preview/snapshot.png`. The builder will keep this file fresh from the Electron main process; the agent reads the metadata to know whether a preview is ready, and uses its native `Read` tool for the image bytes.
- [packages/local-chat-runner/src/centraid-cli.test.ts](../packages/agent-runtime/src/cli/centraid-cli.test.ts): added four tests for the new subcommand (missing file → `exists:false`, present file → size/mtime/age, no-subcommand usage error, extra-args rejection).
...
- New [packages/local-chat-runner/src/codex-app-server.ts](../packages/agent-runtime/src/backends/codex/backend.ts) — JSON-RPC client for `codex app-server`. Handshake is `initialize` (with `experimentalApi: true` capability) → `initialized` notification → `thread/start` (or `thread/resume`) → `turn/start`. cwd / `developerInstructions` / `sandbox` / `approvalPolicy: 'never'` are passed inside `thread/start` params (app-server does **not** honor `-c key=value` — that flag is parsed by the root `codex` CLI and only forwarded to opt-in subcommands). Auto-accepts any `*/requestApproval` server requests so the turn doesn't stall under `approvalPolicy: 'never'`. `turn/interrupt { threadId }` on abort.
- New [packages/local-chat-runner/src/claude-sdk.ts](../packages/agent-runtime/src/backends/claude/backend.ts) — wraps `mod.query({ prompt, options })`. Lazy-imports the SDK so codex-only paths don't pay the dependency cost (the SDK pulls a bundled `claude` binary into `node_modules`). Translates `SDKMessage` events — `partial_assistant_message` content-block deltas → `assistant.delta` / `reasoning.delta`; assistant `tool_use` blocks → `tool.start`; user `tool_result` blocks → `tool.result`; `result` → `final`.
- New [packages/local-chat-runner/src/runtime.ts](../packages/agent-runtime/src/runtime.ts) — exports `runAgentTurn(input, config)`, the unified primitive both chat and builder consume. Dispatches by `prefs.kind` ∈ `'codex' | 'claude-code'`. Returns `{ adapterKind, sessionId? }` for resume.
- [packages/local-chat-runner/src/local-chat-runner.ts](../packages/agent-runtime/src/conversation-adapter.ts) — `makeLocalChatRunner` is now a thin chat wrapper over `runAgentTurn`: derives `cwd = <appsDir>/<appId>`, splices the centraid-CLI preamble into `extraSystemPrompt`, reads/writes the per-window `ChatStore` for resume, prepends the centraid bin dir to `PATH` so codex's shell tool can invoke `centraid` by bare name.
- [packages/local-chat-runner/src/index.ts](../packages/agent-runtime/src/index.ts) — exports the new primitives + the backend-specific input/result types for direct consumers (builder-harness).
- [packages/local-chat-runner/package.json](../packages/agent-runtime/package.json) — adds `@anthropic-ai/claude-agent-sdk@^0.3.143`.
...
### Rename pass (follow-up commit)

The earlier modules carried legacy names from when the chat path was the only consumer; this commit aligns the names with the unified shape.

- **Package**: `@centraid/local-chat-runner` → `@centraid/agent-runtime`. `git mv packages/local-chat-runner packages/agent-runtime`; the directory rename preserves history. The package is the unified agent-turn runtime used by both the per-app chat and the builder, so the "chat-runner" label was actively misleading.
- **Source file**: `packages/agent-runtime/src/local-chat-runner.ts` → `chat-adapter.ts` (via interim `chat-runner.ts`). Function `makeLocalChatRunner` → `makeChatRunner`; type `MakeLocalChatRunnerOptions` → `MakeChatRunnerOptions`. `chat-adapter.ts` is the more accurate name — the file is the chat-specific adapter layered on top of `runAgentTurn` (the mode-agnostic engine primitive), not a runner in its own right. The exported factory keeps the name `makeChatRunner` because what it *returns* is a `ChatRunner` (the host-agnostic interface in `runtime-core` that `/_chat` injects).
- **Header comments**: `packages/agent-runtime/src/index.ts` now spells out the actual matrix in one paragraph — two coding-agent backends (codex / claude-code), `runAgentTurn` as the engine primitive, `makeChatRunner` as one of two `ChatRunner` implementations (the other being `@centraid/openclaw-plugin`'s in-process runner). Goal: any reader landing in the package can map "where does X go" without reading the implementation.
- **Pref keys**: `chat.runner.kind` / `chat.runner.binPath` / `chat.runner.extraArgs` → `agent.runner.*`. Both the desktop's local-runtime prefsLoader and ipc.ts's `loadRunnerPrefs` were reading these keys for chat AND builder turns, so the `chat.` prefix wasn't accurate. Free swap — this is v0, no migration needed.
- **Pi residue**: removed stale references to `pi-coding-agent` / `pi-mono` / `@earendil-works/pi-agent-core` / "pi's AgentEvent" / "pi-shaped" from `packages/chat-harness/src/index.ts`, `packages/builder-harness/src/{agent-session,system-prompt}.ts`, `apps/desktop/src/{main.ts,main/{auth-import,chat,ipc}.ts,renderer/{app,builder,centraid-api.d}.ts}`, and `packages/openclaw-plugin/README.md`. Each comment was rewritten to describe what the code *is* rather than what it *came from*.
- Workspace dep updates: `apps/desktop/package.json`, `packages/builder-harness/package.json` now depend on `@centraid/agent-runtime`. `bun install` refreshes the lockfile.

### Verification (rename pass)

- `bun run typecheck`: 16/16 succeed.
- `bun run test`: 12/12 turbo tasks succeed (187 unit tests in runtime-core; the agent-runtime suites move under the new path unchanged).
- `bun run lint`: 0 warnings.
- `bunx oxfmt --check`: clean across 244 files.
