...
- **`include-hidden-files: true` on the blob upload, and it is load-bearing.** The lane's first real run failed on all four shards: vitest writes its blob reporter output to `.vitest-reports/`, and `actions/upload-artifact` skips any path with a dot-prefixed segment unless told otherwise, so each shard wrote `blob-<i>-4.json` and then uploaded nothing. `if-no-files-found: error` turned that into four named red shards at the source instead of the alternative — four green shards, a merge over an empty directory, and every floor below scoring a world that never ran. Both guards were pointing the right way before either was needed, which is the only reason this cost one cycle rather than shipping a silent hole.
...
- **The connected-session bootstrap has one owner.** `apps/web/tests/e2e/connect.ts` holds `mintControlSession` / `applyConnection` / `connectPwa` and the harness constants; `accessibility.spec.ts`, `pwa-offline-journey.spec.ts` and `web-pwa.spec.ts` import them instead of each keeping a byte-identical copy. It is two steps plus a composition rather than one call because `web-pwa` asserts the control cookie **between** them — that spec's claim is that the POST establishes an httpOnly session, so the assertion had to stay where it was, before the reload. Nothing about what any of the three asserts changed.
...
| PWA offline journey in `client-e2e / web-e2e`: `context.setOffline(true)` | `apps/web/tests/e2e/pwa-offline-journey.spec.ts`, `apps/web/tests/e2e/connect.ts` |
| Runtime accessibility scan (axe) riding the Playwright lanes already paid for | `apps/web/tests/e2e/accessibility.spec.ts`, `apps/web/tests/e2e/connect.ts`, `apps/web/tests/e2e/web-pwa.spec.ts` |
...
14. **SonarCloud's "5.4% duplication on new code" was a real finding on this change, and it was fixed by extraction, not by moving the gate.** The offending block was 48 lines of connected-session bootstrap copied verbatim from `accessibility.spec.ts` into `pwa-offline-journey.spec.ts` — a copy this receipt's own first draft was on record as having made deliberately. It now lives in `apps/web/tests/e2e/connect.ts`. `web-pwa.spec.ts` carried the same bytes and was converted too, which is why the helper is two steps plus a composition. **The ten remaining copies are NOT converted here**: they use an earlier variant (a hand-added cookie, the vault id read back from `/_vault/vaults`), they are untouched by this change and so are not new code, and rewriting ten browser-lane specs inside a CI PR is the widening this repo's review rules tell me not to do on my own. Logged in [QUALITY.md](../QUALITY.md) instead.

    The extraction moved two comment-density pins UP without adding a comment character, and the reason is arithmetic: `web-pwa.spec.ts` carries the identical 384 comment characters it always did (11.14% → 16.07%) and `accessibility.spec.ts` **fell** from 766 to 716 (17.93% → 24.60%) — both rose in *share* only because 48 lines of code left the denominator. Trimming real prose to pay for that would be deleting rationale to buy back a number the extraction earned; the pins are re-pinned to their measured values with that stated in `approvedDeviation`. `connect.ts` is allowlisted from the 15% **cap only**, never from a pin, with its own reason.
