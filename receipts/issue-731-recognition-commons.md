...
- PR #735 CI repair: the shared runtime now pins PDF.js `6.2.108`, the mutation seed/report pairing and recognition self-containment law registry are aligned, the existing `enrichment-service-contract` matrix identity is retained for the self-contained recognition contract so the minimum-test ratchet remains continuous, and the governed matrix/schema fingerprints are re-pinned after audit. The gateway health fixture now reflects the ten bundled enricher templates installed disabled by default (`0 of 10` enabled). The documented Sonar Autoscan policy excludes only the five generated recognition bundles. `bun run test:ratchet`, the model-lock test, recognition typecheck, repository format check, and type-aware lint pass; the Sonar project exclusion settings were applied through `scripts/ci/configure-sonarcloud.mjs`.
- CI coverage's `tools/recognition-automations/src/**` floor was just under its 66% ratchet; the OCR model-weight preflight now has a focused unit owner, restoring coverage without lowering the floor.
...
| 2026-08-10 | codex | 019fecc2-f3e4-7d00-8249-950351d3417c |
