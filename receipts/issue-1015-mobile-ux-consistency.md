...
## Round NY — Lane A: Needs you holds decisions, alerts stand in Activity, notice headlines are sentences

The follow-up round the section above scoped ([#1015](https://github.com/srikanth235/centraid/issues/1015)), Lane A's half: the rulings R-NY-1..R-NY-7 recorded in [docs/decisions.md § Mobile UX consistency (#1015)](../docs/decisions.md#mobile-ux-consistency-1015), and R-NY-2, R-NY-4 and R-NY-5 built.

- **R-NY-5, server.** A notice headline is a sentence the server writes: an automation reads "<name> did not finish", "<name> is working again" or "<name> completed", and an outbox write reads "<target> was sent", "was not sent" or "needs your approval again". The exception gist moves to `detail.gist`, and #647's D4 headline rule is superseded on the R-NY-5 row. I chose "did not finish" over "stopped" because a failed run has not stopped the rule, and its next fire still runs.
- **R-NY-2, Needs you.** The queue counts and lists decisions only. Every notice leaves with its Mark read and Archive verbs. The Standing grants tail, Updates and Archived go, and Settings → Access is the record. "Review all" and "History" go, and the empty state carries no verb. The health line states one sentence and no count. Every waiting row is one tap target: `RowsBlock` gains an optional row `onPress`, drawn beside the verb and never around it. Shared copy loses its jargon: "consent store", "rebuilder", "approving is the act" and "mints". A back key via `useShellParent()` landed in `87bb56245` at the root's request and was reverted when the root withdrew it: lane B's `usePlaceFrame(place)` gives a place its band or its back key, including Needs you opened from an app, which `useShellParent` gets wrong. Wiring `usePlaceFrame("notifs")` here and the band into `GatewayAlerts.tsx` is the root's follow-up after the merge.
- **R-NY-2, Activity.** `GatewayAlerts` lists every active notice, not only gateway-health ones, as one standing line per source. Each line carries one state word said once, a plain time, and a count only for a problem. The whole line opens its source and marks it read, and a failed rule offers "Try again" through `runAutomation`. `memberFacingError` stays on the headline (R-SH-9).
- **R-NY-4, the name.** The place, the band tab, the title, the Settings row, the push button and the comments say "Needs you". The route ids `Approvals` and `notifs` stay.

**Files.** Added: `apps/mobile/src/apps/insights/alerts-model.ts`, `apps/mobile/src/apps/insights/alerts-model.test.ts`, `apps/mobile/src/apps/insights/GatewayAlerts.test.tsx`. Deleted: `apps/mobile/src/screens/approvals/ApprovalsTail.tsx`. Changed: `apps/mobile/src/apps/insights/GatewayAlerts.tsx`, `apps/mobile/src/apps/insights/Insights.tsx`, `apps/mobile/src/kit/components/PlaceHeader.tsx`, `apps/mobile/src/kit/components/PlaceHeader.test.tsx`, `apps/mobile/src/kit/components/RowsBlock.tsx`, `apps/mobile/src/kit/components/RowsBlock.styles.ts`, `apps/mobile/src/kit/components/RowsBlock.test.tsx`, `apps/mobile/src/lib/notifications-core.ts`, `apps/mobile/src/screens/Approvals.tsx`, `apps/mobile/src/screens/BackupHealth.tsx`, `apps/mobile/src/screens/PhoneStorage.tsx`, `apps/mobile/src/screens/Settings.tsx`, `apps/mobile/src/screens/SignalNotification.tsx`, `apps/mobile/src/screens/shell-copy.ts`, `apps/mobile/src/screens/shell-places.ts`, `apps/mobile/src/screens/approvals/Approvals.styles.ts`, `apps/mobile/src/screens/approvals/Approvals.test.tsx`, `apps/mobile/src/screens/approvals/ApprovalsQueue.tsx`, `apps/mobile/src/screens/approvals/RowParts.tsx`, `apps/mobile/src/screens/approvals/approvals-model.ts`, `apps/mobile/src/screens/approvals/approvals-model.test.ts`, `apps/mobile/src/screens/approvals/useApprovals.ts`, `apps/mobile/src/screens/approvals/view-types.ts`, `apps/mobile/src/screens/home/places.ts`, `apps/mobile/src/screens/home/places.test.ts`, `apps/mobile/src/screens/home/band.test.ts`, `docs/decisions.md`, `packages/client/src/approvals-copy.ts`, `packages/client/src/react/shell/routes/ApprovalsRoute.test.tsx`, `packages/client/src/react/shell/routes/approvalsData.test.ts`, `packages/server/src/serve/notices.ts`, `packages/server/src/serve/notices.test.ts`, `packages/server/src/serve/build-gateway.ts`, `packages/server/src/serve/build-gateway.test.ts`, `packages/server/src/serve/outbox-executor.ts`, `packages/server/src/serve/outbox-executor.test.ts`.

**Not done, and why.**
- Comments in `apps/mobile/src/screens/home/HomeBand.tsx:114` and `apps/mobile/src/kit/rooms/SystemPlace.tsx:5` still use the old nouns. Both files are lane B's this round.
- Desktop's `ApprovalsScreen` keeps its Standing grants, its "Review all" and its notices. The desktop seat is a non-goal of #1015, but the shared copy it reads was fixed at the source.
- `bodyRef={scroller}` stays on Needs you with nothing scrolling it any more. I left it so the prop list lane B edits merges cleanly.

### Verification

1. `bun run --cwd packages/server typecheck` → 0. `cd packages/server && bunx vitest run src/serve/notices.test.ts src/serve/serve-scheduler-reconcile.test.ts src/serve/outbox-executor.test.ts src/serve/build-gateway.test.ts` → all four files green.
2. `bun run --cwd apps/mobile typecheck` → 0, and `bun run --cwd packages/client typecheck` → 0.
3. `cd packages/client && bunx vitest run src/react/shell/routes/approvalsData.test.ts src/react/shell/routes/ApprovalsRoute.test.tsx src/react/screens/ApprovalsScreen.test.tsx` → 3 files, 84 passed.
4. `cd apps/mobile && bunx vitest run src/screens/home src/screens/shell-copy.test.ts src/kit/components/PlaceHeader.test.tsx src/screens/Settings src/screens/approvals src/lib/notification` → 25 files, 256 passed. `bunx vitest run src/apps/insights` → 5 files, 62 passed.
5. `grep -n "failed — " packages/server/src/serve/build-gateway.ts` → nothing.
6. `node scripts/lint-mobile-rooms.mjs` → 35 findings (`screen-root 33 · back-literal 0 · page-margin 2 · identity-tint 0 · copy-title-case 0 · error-detail 0`), unchanged from the base. `lint-mobile-design` and `lint-container-opacity` → ok.

### Falsification

Each check planted a regression in a clean tree, ran the tests that should catch it, and restored the file with `git checkout`.

1. **Claim: no notice, a failing rule included, enters the Needs you queue.** I planted `data.notices.filter((n) => n.severity !== "info").length` back into `waitingTotal`, then ran `bunx vitest run src/screens/approvals`. Result: red. Two tests failed ("counts decisions only…" and "keeps every notice out of the queue…"), with 20 passing.
2. **Claim: the exception gist never lands in a headline.** I planted ``+ (gist ? ` — ${gist}` : "")`` onto the build-gateway headline call, then ran `bunx vitest run src/serve/build-gateway.test.ts`. Result: red. One test failed ("a failed automation fire writes and collapses its Notifications notice"), with 23 passing.

## Round NY — post-merge (lane B, on umbrella 37df8b2bf)

- **F1.** `place-frame.test.ts` named `SHELL_TITLES.alerts`, which lane A renamed. Rather than swap the key, the test now pins R-NY-4 itself: a back key to Needs you says the band's own noun (`getPlace("notifs").name`). Its old path, Needs you → Activity through History, left with lane A, so the test now pins the path that exists: an alert opens Needs you over Activity, which makes Needs you a sub-page of Activity.
- **F2.** Needs you takes `usePlaceFrame("notifs")` like every other place root: the band when it stands on Home, and a back key when it is opened from Settings, an app or an alert. Its `onHome`, and a `bodyRef` that nothing scrolls any more, are gone.
- **F3, the decision.** Activity's alerts view is a view of the Activity place and lives on Activity's own route, so it takes `usePlaceFrame("stats")`. Pushed over Activity, it is a sub-page with a back key to Activity. Standing on Home, it is Activity's root and draws the band. Its `onLeave` prop is gone. **Finding: nothing in the product opens this view any more.** No caller sets `initialTab: "alerts"`, because lane A removed History and the notice links. R-NY-2 says notices stand in this tab, so Activity needs a door to it; that is an owner question and was not added here.
- **F4.** HomeBand's one-noun comment says Needs you. `SystemPlace.tsx`'s header already did.
- **F5, one gutter.** RoomBody's error, loading and empty states now take the room gutter (`rooms.styles.ts` `state`). `EmptyBlock` carries none, so an empty inside a padded place body is inset once, not twice.
  - Census of `EmptyBlock` call sites, by the container each sits in:
    - Padded by a place body, so doubled before and inset once now: Needs you, Connectors, Activity, Rules, Vault's two sections, and People's day one.
    - Padded by their page already: Locker review and Locker items.
    - Full-bleed, so they now pass the new `inset` prop: Tally's export surface, Locker trash, access and search, Docs folders, and the Drive list.
  - `rooms.test.tsx` ("the room gutter") and `EmptyBlock.test.tsx` pin this.
- **F6.** Needs you's empty body is in plain words, desktop and phone alike: "Anything that needs your OK — a message waiting to send, an account to reconnect, an app asking for access — shows up here." Every pin is updated, including DESIGN.md's copy table.
- **F7.** Activity's room error no longer passes the gateway's reason as a "what happened" fact, since that is how "vault host returned HTTP 404" reached a member. `Insights.test.tsx` now asserts that the transport string is absent. **The one detail kept** is Needs you's unpaired sentence, "This phone is not paired with a gateway yet.", which a member can act on through Open Settings. `INSIGHTS_ERROR_BODY` is in plain words, desktop pin updated. Vault's copies section still shows a "the connection said" fact built from a device message; it is not a room error, so it is listed here as a finding for the next round.

## Round NY — post-merge: Lane A makes background failures reachable again

On umbrella head `26ef7e962` the Activity alerts view had become unreachable. Nothing navigated to `Insights { initialTab: "alerts" }` once History and the notice rows on Needs you were gone. A failed-rule push also still opened Needs you, which no longer holds notices, so it opened empty. That broke R-NY-2's own intent ([docs/decisions.md § Mobile UX consistency (#1015)](../docs/decisions.md#mobile-ux-consistency-1015)).

- **G1.** Activity's overview draws a standing "Alerts" row in every state that has a page, including a window where nothing ran. Its sub-line reads "Nothing needs a look" or "N need(s) a look", and it stays blank until the read lands rather than showing an invented zero. It pushes the alerts view over Activity, so lane B's frame gives that view its back key. Files: `apps/mobile/src/apps/insights/Insights.tsx`, `apps/mobile/src/apps/insights/Insights.test.tsx`, `apps/mobile/src/apps/insights/alerts-model.ts`, `apps/mobile/src/apps/insights/alerts-model.test.ts`, and the new `apps/mobile/src/apps/insights/useAlertCount.ts`.
- **G2.** A push opens the place it is about. The census found that the server sends only a content-free wake (`data: { centraid: "replica-wake" }`, `packages/server/src/routes/push-wake-routes.ts`). Every visible push is composed on the phone from the authenticated pull, so the distinction is added there and no server field is needed. Each composed row carries `about`. A notice push gets the `CENTRAID_ALERTS` category ("Open alerts"), `url: centraid://insights?initialTab=alerts` and an `open-alerts` plan, so the plan and the linking table agree. Decisions, and any push without `about`, keep Needs you. Files: `apps/mobile/src/lib/notification-model.ts`, `apps/mobile/src/lib/notifications-plan.ts`, `apps/mobile/src/lib/notifications-core.ts`, `apps/mobile/src/lib/notifications.tsx`, and the new `apps/mobile/src/lib/notification-routing.test.ts`.
- **I.** When Activity's run-log read fails, the room error replaces the body and takes the Alerts row with it, so the error now offers "Open alerts" as its secondary, using the same push; `apps/mobile/src/apps/insights/Insights.tsx`, `apps/mobile/src/apps/insights/Insights.test.tsx`.
- **Not done.** The web seat's `composeWebNotifications` in `packages/client/src/notifications-model.ts` has the same undifferentiated shape. The desktop and PWA seats are a non-goal of #1015.
...
## Owner items — Lane A: the rulings, "Needs you" on every seat, "rule" on the desktop

Branch `lane/1015-apps-a` from `cb997a7c9`. Four commits: `6d3f6ae90` (A1, rulings), `c8debf768` (A2, "Needs you" on every seat), `1bc7c92cc` (the ledger the rename left behind), `11d6e1534` (A3, "rule" on the desktop).

- **A1 — the rulings.** R-NY-8 … R-NY-17 are dated rows in the "Mobile UX consistency (#1015)" table of `docs/decisions.md`, and R-SH-9's two rows are struck through and marked superseded by R-NY-10 in that table's existing supersession style.
- **A2 — "Needs you" on every seat (R-NY-16).** The screen, route kind (`needs-you`), launcher and ops-bar labels, copy module and the blueprint door are renamed with no alias, and `composeWebNotifications` plus the service worker tag each push row `decision` or `notice`. The full file list is in `c8debf768`.
- **A3 — "rule" on the desktop (R-NY-9).** Member sentences on the editor, viewer, templates and run screens now say "rule"/"rules" in sentence case. Changed: `packages/client/src/react/screens/AutomationCompilePane.tsx`, `AutomationEditorConnectorsPicker.tsx`, `AutomationEditorHarnessPicker.tsx`, `AutomationEditorScreen.tsx`, `AutomationThreadScreen.tsx`, `AutomationsOverviewScreen.tsx`, `RunViewScreen.tsx`; `packages/client/src/react/shell/routes/AutomationEditorRoute.tsx`, `AutomationViewRoute.tsx`, `RunViewRoute.tsx`, `TemplatesRoute.tsx`, `automationEditorTriggers.ts`, `automationLiveMessages.ts`, `automationThreadData.ts`, `automationTurnMessages.ts`, `automationsData.ts`, `runViewData.ts`, `needsYouPhrasing.ts`; the tests that assert those strings (`AutomationEditorAccountChoice.test.tsx`, `AutomationEditorScreen.test.tsx`, `AutomationEditorTriggers.test.tsx`, `AutomationThreadScreen.test.tsx`, `AutomationsOverviewScreen.test.tsx`, `RunViewScreen.test.tsx`, `NeedsYouScreen.test.tsx`, `automationEditorVault.test.ts`) and `apps/desktop/tests/e2e/automations.spec.ts`. New: `packages/client/src/react/rule-copy.sweep.test.ts`.
- **The one R-SH-11 item found unfinished.** `needsYouPhrasing.ts`'s `callerPhrase` said "the automation <name>" for an `agent` caller while the phone's `apps/mobile/src/screens/needs-you/needs-you-model.ts` already said "the rule <name>" — R-SH-11 named "Approvals' caller phrase" by hand and only one seat followed. The sweep now reads that module so the two seats cannot drift again.
- **Identifiers left standing, deliberately (R-NY-9 names them).** The wire flag and capability id `automations`; route kinds `automations`, `automation-view`, `automation-builder`, `automation-editor`, `run-view`, `templates`; the deep-link path; every `Automation*`/`automation*` file and module name; `data-testid` values (`automation-thread`, `automation-menu-delete`); `automationRef`, `automationId`, `automationName`, `automationTurns` fields; tally's `approvals` verb. No member reads any of them.
- **The ledger the A2 rename left behind.** `c8debf768` renamed `ApprovalsScreen.tsx` and its test but left their `tests/inventory.json#fileSize` rows under the old paths, so both files silently lost the exemption that keeps them past the 625-line ceiling and `max-lines` failed on the first commit to stage one. The rows follow the files in `1bc7c92cc`; `_budget` is unchanged at 131 (a rename adds no exemption) and the test file's recorded length ratchets 1282 → 1280. Law and ledger stay in their own commit, per `estate-separation`.

### Findings outside the slice

- **Member-copy "automation" survives on 14 desktop modules R-NY-9 does not name**, while the place is called Rules: `AppSettingsPanel.tsx` (the "Automations" tab, "No automations linked to this app yet.", "Open Automations →", "…its scheduled automations."), `capabilities.ts` + `CapabilityWall.tsx`, `assistantContextLabel.ts`, `LibraryCards.tsx`'s kind badge, `NeedsYouScreen.tsx`'s holder line, `SettingsAppearanceScreen.tsx`, `SettingsConnectionsScreen.tsx`, `SettingsDiagnosticsScreen.tsx`, `SettingsHarnessLanes.tsx`, `BackupCopyCards.tsx`, `AssistantScreen.tsx`, `Gallery.tsx`. R-SH-11's headline ("the ITEM noun is rule, on both seats") is unqualified, so these read as the same divergence; they are left standing because R-NY-9 scopes itself to the editor, viewer, templates and run screens. **Recommendation:** extend the item noun to every seat surface in one pass, and widen this sweep to all of `packages/client/src/react`.
- **`gateway-client*.ts` fallback strings** ("Automation finished unsuccessfully.", "interactive automation turn failed (HTTP …)") are seat-manufactured error text, which R-NY-10 says the gateway should own. Not touched: R-NY-10's server work is another lane's.
- **`tests/inventory.json#commentDensity.files` is stale wholesale** — ~190 pinned paths no longer exist (the `packages/vault/src/share/commons-*` family and `packages/client/src/replica/*` from earlier landed renames, plus A2's), and `bun run test:comment-density` reports 389 files over the 15% cap. `check-ledgers` does not name-check the section and the ratchet is not in `check:push`, so nothing caught it. Repo-wide, not this lane's.
- **D-11 is still `"pending owner grant"` in `.governance/law/docket.json`**, so `law` keeps its one `waiver-docket` warning. R-NY-8 says the owner granted it, but a waiver is granted by the owner's own act; an agent flipping the enforcement docket on a relayed claim is the thing the warning exists to prevent. The ruling row is recorded; the docket line is the owner's to change.

### Verification

A-x1. `grep -rn "Approvals" packages/client/src apps/mobile/src packages/blueprints --include='*.ts' --include='*.tsx'` → 2 lines, both the common noun for consent grants in a code comment (`AutomationViewRoute.tsx:172`, `apps/mobile/src/apps/assistant/useAssistant.ts:71`), neither the screen. No `ApprovalsScreen`, `ApprovalsRoute`, `approvalsData`, `approvalsPhrasing` or `approvals-copy` remains anywhere outside `receipts/` (evidence, append-only).

A-x2. `bunx vitest run src/gateway-client-push.test.ts` with `notifications-model.ts` restored to `cb997a7c9`: `× tags every row decision or notice, so a tap lands where it can be acted on (R-NY-16)` — `AssertionError: expected [ [ 'outbox', undefined ], …(4) ] to strictly equal [ [ 'outbox', 'decision' ], …(4) ]`, `Tests 1 failed | 6 passed (7)`. At HEAD: `Tests 7 passed (7)`.

A-x3. The sweep over the same 31 modules: **49 offenders before → 0 after**. Red proof at HEAD with `AutomationEditorScreen.tsx` and `needsYouPhrasing.ts` reverted: `expected [ …(11) ] to strictly equal []`, listing "Loading automation…", "Create automation", "Delete automation", "the automation ${caller}" and seven more.

E1. `bun run --cwd <pkg> typecheck` → 0 for `packages/client`, `packages/blueprints`, `packages/server`, `apps/mobile`, `apps/web`, `apps/desktop`.

E2. `cd apps/mobile && bunx vitest run` → **329 files, 327 passed, 2 failed; 2801 tests, 2800 passed, 1 failed.** Both failures are the inherited ones the brief names: `src/lib/replica/expo-seat-driver.test.ts` (0 tests — a rolldown parse error, red on main) and `scripts/verify-native-state.test.mjs` (lane B owns it under R-NY-17). Other packages: `packages/client` 276 files / 2482 passed, 0 failed; `apps/web` 11 files / 65 passed; `packages/blueprints` 217 files / 214 passed, 3 failed — the three inherited reds the brief names (`one-computation`, `pending-projection-tripwire`, `photos-vocabulary`); `packages/server` — the two files A2 touched, 21 passed.

E3. `node scripts/lint-mobile-rooms.mjs --enforce` → exit 0. `screen-root 1` (PhotoLightbox, lane C's R-NY-14), `back-literal 0`, `page-margin 0`, `identity-tint 0`, `copy-title-case 0`, `error-detail 0` — 1 finding over 606 files.

E4. `bun run format` → 5688 files. `bun run check:push:static` → **4/4** (`turbo:lint`, `lint`, `format:check`, `typecheck:affected`) in 140.0s.

E5. `node .governance/law/run.mjs --brief-digest bf847b5c4982` → `law (window door): 10 rule(s), 0 error(s), 1 warning(s)`; the warning is `waiver-docket` on D-11, unchanged from the brief's state and explained in the findings above.

E6. `git status` clean but for the untracked `NOTE-lane.md`. `git log --oneline cb997a7c9..HEAD` → `11d6e1534`, `1bc7c92cc`, `c8debf768`, `6d3f6ae90`.

### Falsification

The two riskiest claims, and the throwaway check run against each:

1. **"The sweep would actually catch a regression."** A passing sweep proves nothing on its own — the file filter could be matching no files, or `memberTexts` could be dropping every string. Check: `git stash` of `AutomationEditorScreen.tsx` and `needsYouPhrasing.ts` at HEAD, then re-run. **Result: 11 offenders reported, naming the exact reverted sentences in both files** — the glob reaches the screens and the extractor reads both string literals and template bodies. A second guard (`SOURCES.length > 20`, measured 31) fails if the directory filter ever stops matching.
2. **"Only member copy changed; no identifier moved."** A copy sweep that quietly renames a route key or a `data-testid` breaks the seat silently, since a `getByTestId` miss is a runtime failure, not a type error. Check: `bun run --cwd packages/client typecheck` → 0, the full client suite → 2482 passed with 0 failures, and `grep` for every renamed string across `apps/desktop/tests` → the only hits are test titles and comments. **Result: no identifier, flag, route key, deep-link path, module or file name changed** — the diff is 71 lines changed in 71 places, all inside string literals and JSX text.
## Owner items — Lane C: one statement per fact, member names for every kind, and the gateway's member sentence (#1015)

Three owner items from the #1017 body, as answered by R-NY-11, R-NY-13 and R-NY-10 ([docs/decisions.md § Mobile UX consistency (#1015)](../docs/decisions.md#mobile-ux-consistency-1015)). Three commits off `cb997a7c9`: `049f1d5fa`, `ba26f4312`, then `931442d59` (red) and `45e392951` (green) for the third.

### C1 — each fact once, one noun per destination (R-NY-11), `049f1d5fa`

Docs #8: "a folder is a label on the document, not a place it sits" is stated once per seat, in the shared `foldersCaption`; the 30-day purge once, in the shared trash caption. Docs #11: the create entry is "Add a document", the detail screen "Properties", the inbound shelf "Shared", on every seat. Photos #15: the phone keeps the 30-day rule only in the trash header subtitle.

Mobile: `apps/mobile/src/apps/docs/AddToDocs.tsx`, `apps/mobile/src/apps/docs/DocsFoldersView.tsx`, `apps/mobile/src/apps/docs/DocsHome.tsx`, `apps/mobile/src/apps/docs/DocsHome.test.tsx`, `apps/mobile/src/apps/docs/DocsMoreSheet.tsx`, `apps/mobile/src/apps/docs/DocumentRead.tsx`, `apps/mobile/src/apps/docs/doc-menu.ts`, `apps/mobile/src/apps/docs/doc-menu.test.ts`, `apps/mobile/src/apps/docs/docs-copy.ts`, `apps/mobile/src/apps/docs/docs-copy-case.test.ts`, `apps/mobile/src/apps/docs/docs-places.ts`, `apps/mobile/src/apps/docs/docs-places.test.ts`, `apps/mobile/src/apps/photos/PhotoStateView.tsx`, `apps/mobile/src/apps/photos/photos-collections.ts`. Blueprints: `packages/blueprints/apps/docs/components/Details.tsx`, `packages/blueprints/apps/docs/components/DetailsTabs.tsx`, `packages/blueprints/apps/docs/components/InfoToggle.tsx`, `packages/blueprints/apps/docs/components/NewDocRoute.tsx`, `packages/blueprints/apps/docs/document-copy.ts`, `packages/blueprints/apps/docs/nav-rail.ts`, `packages/blueprints/apps/docs/nav-rail.test.ts`, `packages/blueprints/apps/docs/popovers.ts`, `packages/blueprints/apps/docs/shelves.ts`, `packages/blueprints/apps/docs/view-copy.ts`, `packages/blueprints/apps/photos/components/DuplicateReview.tsx`, `packages/blueprints/src/docs-shelves.test.ts`, `packages/blueprints/src/photos-duplicate-review.test.ts`.

### C2 — Vault names what it holds in member words (R-NY-13), `ba26f4312`

The registry is the one owner of what a kind is called (#883 O-label). The census and Browse payloads carry `friendly` beside the mechanical `label`; the phone's Vault reads only `friendly`, so "Content Derivative" and "Asset Phash" become "Derivatives" and "Fingerprints", and the humanized fallback is gone from the client and from the mobile types. The machinery chip "The engine's own" is **"Kept by Centraid"**.

`apps/mobile/src/lib/atlas.ts`, `apps/mobile/src/screens/data/Data.tsx`, `apps/mobile/src/screens/data/data-model.ts`, `apps/mobile/src/screens/data/data-model.test.ts`, `packages/client/src/gateway-client-atlas.ts`, `packages/client/src/react/screens/AtlasScreen.test.tsx`, `packages/client/src/react/screens/atlasRelationsTestKit.tsx`, `packages/client/src/react/screens/atlasScreenModel.test.ts`, `packages/vault/src/schema/atlas-browse.ts`, `packages/vault/src/schema/atlas-census.ts`, `packages/vault/src/schema/atlas-census.test.ts`, `packages/vault/src/schema/atlas-graph.ts`, `packages/vault/src/schema/entity-labels.test.ts`, `docs/vault-ontology.md`.

### C3 — the producer owes the member sentence (R-NY-10), `931442d59` red → `45e392951` green

Census of the six `kit/member-error.ts` callers, and where each string is built now:

| Caller | The string it received | Producer today |
| --- | --- | --- |
| `apps/mobile/src/apps/insights/GatewayAlerts.tsx:141` | `notice.headline` | Server, a member sentence since R-NY-5 (`automationNoticeHeadline` / `outboxNoticeHeadline` / `enrichRefusalNotice`, `packages/server/src/serve/notices.ts`), with the failure on `detail.gist`. Printed verbatim. |
| `apps/mobile/src/apps/insights/Insights.tsx:331` | `describe(error)` — the exception's own text | `ExportFailureError(member, detail)`, thrown by `shareCsv`. |
| `apps/mobile/src/kit/transfer/transfer-queue.ts:59` | `upload_item.last_error` — the exception's own text | `memberTransferFailure` at the drainer's catch. |
| `apps/mobile/src/kit/transfer/backup-verdict.ts:102` | the same row | the same producer |
| `apps/mobile/src/screens/home/origin-health.ts:52` | the same row | the same producer |
| `apps/mobile/src/screens/data/VaultSections.tsx:65,156` | `memberDeviceError(...)` / `SHELL_ERROR.sharing` — already member sentences | the frame was the defect, not the value: both "the connection said" facts are gone, and the panel body says it once. |

A seventh surface the census turned up: `apps/mobile/src/screens/BackupHealth.tsx:483` prints `failure.lastError` with no filter at all, so it was showing the raw exception. It is member copy now without touching that file.

`memberTransferFailure` (`apps/mobile/src/lib/upload/transfer-failure.ts`) is five outcomes over the refusal's status, with `DirectTransferError.member` for a thrower that knows better than the status does (a local file that changed under the queue). The raw text goes to `[centraid] upload: <itemId> was not sent — <reason>` and `[centraid] insights: the CSV was not shared — <reason>`, both documented in `docs/logs.md`. `apps/mobile/src/kit/member-error.ts` and `apps/mobile/src/kit/member-error.test.ts` are deleted. The envelope is `docs/protocol.md` § "The member sentence and its detail".

Two dead fields the deletions exposed, removed rather than left: the devices read's `message` (`apps/mobile/src/screens/devices/useDevices.ts`) and the sharing read's `error` (`apps/mobile/src/screens/data/VaultSections.tsx`), whose only consumers were the two quoted facts.

Files: `apps/mobile/src/apps/insights/GatewayAlerts.tsx`, `apps/mobile/src/apps/insights/GatewayAlerts.test.tsx`, `apps/mobile/src/apps/insights/Insights.tsx`, `apps/mobile/src/apps/insights/Insights.test.tsx`, `apps/mobile/src/apps/insights/insights-export.ts`, `apps/mobile/src/apps/insights/useInsights.ts`, `apps/mobile/src/kit/transfer/backup-verdict.ts`, `apps/mobile/src/kit/transfer/backup-verdict.test.ts`, `apps/mobile/src/kit/transfer/transfer-queue.ts`, `apps/mobile/src/kit/transfer/transfer-queue.test.ts`, `apps/mobile/src/lib/upload/gateway-client.ts`, `apps/mobile/src/lib/upload/transfer-failure.ts` (new), `apps/mobile/src/lib/upload/transfer-failure.test.ts` (new), `apps/mobile/src/lib/upload/uploader.ts`, `apps/mobile/src/lib/upload/uploader.test.ts`, `apps/mobile/src/screens/data/VaultSections.tsx`, `apps/mobile/src/screens/devices/useDevices.ts`, `apps/mobile/src/screens/home/origin-health.ts`, `apps/mobile/src/screens/home/origin-health.test.ts`, `docs/logs.md`, `docs/protocol.md`. Deleted: `apps/mobile/src/kit/member-error.ts`, `apps/mobile/src/kit/member-error.test.ts`.

### Verification

- C-x1 `grep -rn "not a place\|30 days" apps/mobile/src/apps/{docs,photos} packages/blueprints/apps/{docs,photos} --include='*.ts' --include='*.tsx' | grep -v test` → one statement of the folder fact (`packages/blueprints/apps/docs/drive-copy.ts:321-322`, shared by both seats) and one of the purge per seat (`packages/blueprints/apps/docs/view-copy.ts:90` for Docs; `apps/mobile/src/apps/photos/PhotoStateView.tsx:140` and `packages/blueprints/apps/photos/view-copy.ts:93` for Photos, phone and web). The rest of the hits are code comments citing the caption and the unrelated "Last 30 days" date filter.
- C-x2 `grep -rn "member-error\|memberFacingError\|the connection said" apps packages --include='*.ts' --include='*.tsx' | grep -v node_modules` → nothing.
- C-x3 the curated-name test, red against a planted uncurated kind: `entity-catalog.ts`'s `label: "Derivatives"` set to `"Content Derivative"` → `entity-labels.test.ts:130` fails with `["core.content_derivative is called \"Content Derivative\""]`. Reverted; tree clean.
- E1 `bun run --cwd <pkg> typecheck` → 0 for `apps/mobile`, `packages/blueprints`, `packages/client`, `packages/vault`.
- E2 `cd apps/mobile && bunx vitest run` → 329 files, 2819 passed, 1 failed + 1 file that fails to load, both inherited and named in the brief: `apps/mobile/scripts/verify-native-state.test.mjs` (lane B's R-NY-17) and `apps/mobile/src/lib/replica/expo-seat-driver.test.ts` (red on main). `packages/client` 275/275 files, 2479 passed. `packages/vault` 211/211 files, 1775 passed. `packages/blueprints` 214/217 files, 7313 passed, the three inherited reds: `photos-vocabulary` (`PHOTOS_ERROR_FREE_UP_PAUSED` in `shared-copy.ts`, untouched here), `one-computation`, `pending-projection-tripwire`.
- E3 `node scripts/lint-mobile-rooms.mjs --enforce` → exit 0; `screen-root 1` (PhotoLightbox, lane B's, at its baseline), `back-literal 0`, `page-margin 0`, `identity-tint 0`, `copy-title-case 0`, `error-detail 0`, over 605 files.
