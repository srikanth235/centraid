...
- [x] Split overloaded mobile owners: update-wall contract vs compat into separate owners, version skew proven in **both** directions; add hostile-input cases to `mobile.security` and interleaved-writer cases to `mobile.concurrency`. **All three landed.** `mobile.contracts` and `mobile.compat` named one file; compat moved to `mobile-gateway-skew.test.ts`, which states skew as range-overlap arithmetic in both directions (newer gateway, older gateway, containment, one-version overlap at each edge, the disjoint boundary either side, and a capability gap that must never blame the app). `mobile.concurrency` gained four interleaved cases where *different* operations overlap. `mobile.security` gained eleven cases — nine rejections and two counterweights (an ordinary presigned object, and a legitimate key containing an encoded `%`) — and **three of them found real holes**, fixed rather than pinned (see What changed).
...
- [ ] Photos **free-up-space**: heavy logic at unit/integration tier; one device journey proving real eviction + tile state. **The device half rests on a false premise and is left unchecked rather than faked:** on mobile `Free up space` is a permanently `disabled` Pressable in `BackupHealth.custody.tsx` whose hint reads "Release the copies from the app that holds them". The phone never evicts, so there is no real eviction on a device to prove. Recorded in QUALITY.md; the same discovery shape as the custodian-seat item below.
- [ ] Locker unlock on Android via `adb -e emu finger touch`; gate-open read of a secret. **Tooling landed, journey did not.** `ctx.device` now wraps exactly that adb call, so the blocker recorded in `origin-acts.json` is no longer "out-of-band tooling" — it is that the Locker gate is a *passphrase* gate (`locker-gate-field`), and whether an emulated fingerprint satisfies `SecureStore`'s `requireAuthentication` prompt is not knowable without running it. Writing the flow blind would be speculation shaped like a test.
- [x] One frame-level capture/share-in journey via Android intent synthesis. `flows/share-intent-in.mjs` synthesises ACTION_SEND through the new `ctx.device` escape and asserts Quick capture opens **carrying the shared text verbatim** — the payload half, which is what a build that opened the right screen while mangling `encodeURIComponent` would lose. Staged `promoting` (never run; five nights) so it cannot gate a PR.
- [ ] Assistant journey: pair, send, first stub token renders — requires a fixed-delay stub turn in `ci-gateway.mjs`, which also unlocks the `sendToFirstToken` experience budget. **The stub turn shipped; the journey did not.** `lib/fixed-delay-agent.mjs` is a real ACP agent streaming its first token after a known delay, wired as the CI gateway's provider through the registry's `acp` custom-agent kind, with the launch plan proved by `tests/integration-mobile/assistant-provider.integration.test.ts`. The budget entry moves from `NONE TODAY` to a named rig — but `status` stays `unmeasured`, because a rig is not a measurement and no device has run it.
- [ ] Upgrade-with-data journey: install previous release artifact → write state → install current over it → data intact.
- [x] On-device op-sqlite probe. `flows/op-sqlite-probe.mjs` — the only evidence in the repo about the engine the product ships, since all 52 W3 cells run against the in-memory `NodeSqliteDriver` stand-in. Proves reads stay coherent while writes land and that every write of a five-note burst survives process death. Explicitly **not** an exactly-once proof: it detects an intent re-executed on recovery, not a row duplicated verbatim, and both the flow header and the roster claim say so. Staged `promoting` (never run; ten nights).
...
- [ ] Notification permission + delivery: `simctl push` / `adb broadcast` payload carrying a `centraid://` deep link lands on the named screen. **Still blocked, but on a different thing.** `ctx.device` dispatches the call; what it cannot conjure is the payload — Android delivery runs through FCM and `simctl push` needs a committed `.apns` fixture, so this needs a payload-plumbing decision rather than a line of flow code.
- [ ] Granted camera-roll path (incl. iOS limited library) with `simctl addmedia`. **Mechanically unblocked, materially not.** `ctx.device` dispatches `xcrun simctl addmedia <udid> …` in one call; the lane has no committed image to add, and putting a binary in the repo for it is a decision about where test media lives. iOS's third `limited` authorization state still has no simctl verb at all.
...
### The follow-up pass: three deferred items taken, and two security holes found
...
- **Percent-encoded traversal was accepted.** `new URL()` resolves a literal `../` before that test runs — which is what made the existing traversal case pass and the check look sound — but leaves `%2e%2e%2f` untouched, so `…/tmp/blobs/%2e%2e%2f%2e%2e%2fblobs/sha256/<secret>` satisfied the prefix. The prefix and a `..`-segment test now apply to **every** percent-decoding of the path, bounded, with a malformed escape refused rather than guessed at.
- **Credentials embedded in the URL were accepted.** `URL.origin` omits userinfo, so `https://evil:pw@provider.example/…` matched the provider origin and the credentials would ride to it. Now rejected outright.

**A third hole was in the first version of the fix itself**, and it is the one most worth recording: segments were split on `/` alone, so `..%5c..%5c` — which `URL` does not normalise, because it rewrites only a *literal* backslash — walked straight through the check that had just been written to stop exactly this. An independent audit found it; the tests that shipped alongside the fix did not. The same audit found that the fix rejected a legitimate key containing an encoded `%` (`%25` decodes once to `%`, which cannot decode again — treated as malformed), and that its decoding bound of four contradicted a comment promising "however many times the far side unescapes". All three are fixed, and each is verified load-bearing by reverting it and watching the matching test go red. The general lesson is in QUALITY.md: **a normalization the parser performs for you hides the cases it does not perform.**

**Interleaved writers.** The concurrency owner raced `claimNext` against itself and nothing else. Four cases now overlap *different* operations — concurrent admissions taking unique strictly-increasing order, an admission racing a claim, a settle racing a transition, and four drain workers settling every intent exactly once. Three of the four have a demonstrated killing mutation — splitting `add`'s order allocation, `claimNext`'s claim, or `transition`'s patch across an `await`; the drain-worker case catches the double-claim, which is the failure that actually reaches a user. The fourth, the admission/claim pair, is a coherence assertion with no such mutation, and is recorded as that rather than counted among them. The scope limit is stated in the file and in the matrix note rather than left to be assumed: `node:sqlite` is synchronous and JS single-threaded, so these pin atomicity across `await` boundaries, **not** OS-thread contention on a file.

**The Assistant finally has a model provider.** `tests/agent-e2e-mobile/lib/fixed-delay-agent.mjs` is a small, real ACP agent that streams its first token after a known delay — a constant rather than an instant reply, so the measurement has a floor to subtract and does not exercise a path no real turn takes. `ci-gateway.mjs` configures it through the registry's existing `acp` "Custom ACP agent" kind, which needs no new harness kind and no change to shipped product surface. The prefs are **exported from the agent module and imported by the test**, so the wiring test asserts the values the gateway actually writes instead of a copy that could drift.

**`ctx.device` — the escape that unblocked the seat-verb register.** Four origin acts were recorded as blocked on "out-of-band tooling the harness does not wrap". Each was one `adb` or `simctl` call; the only thing missing was somewhere to put it. `ctx.device` takes an argv array (never a shell string) targeted at `state.udid` (never "the device", which on a runner hosting two emulators is a coin flip). The blanket excuse is gone from every row of `origin-acts.json`, replaced by a specific remaining blocker per act.

**Two new device journeys, both staged, neither run.** Two bars are in play here and they are deliberately different, which is why the checklist ticks these while `origin-acts.json` still refuses to name either as an owner. The checklist item is *write the journey*, and it is written. The register's bar is *a scheduled owner*, and a `promoting` flow is not scheduled — it has never run, and an owner that has never run is precisely the defect that register exists to catch. Neither position concedes to the other; a flow crosses from one to the other by completing its nights. `flows/share-intent-in.mjs` synthesises the frame-level share-in and asserts the payload arrives verbatim; `flows/op-sqlite-probe.mjs` is the repo's only evidence about the shipped database engine, since all 52 W3 cells run against an in-memory stand-in. Both are `promoting` in `roster.json` with `since`/`nights`, which makes them the **first real users of the D3 pipeline** — until now every flow was `scheduled` and the pipeline had a rule but no road. Both linter rules were verified to bite on these entries (a `promoting` flow on the blocking lane, and one missing its `since`), not just on the self-test fixtures. They run under a new `run-promoting-suite.mjs` rather than joining `run-probes-suite.mjs`, whose 35-minute ceiling has ninety seconds of slack and whose own budget doc forbids raising it.

**What this pass deliberately did not produce: four more device flows.** Locker's biometric unlock, notification delivery, the granted camera roll and upgrade-with-data all now have the tooling and none has a journey. Writing them without an emulator would mean guessing at surfaces I cannot read confidently — whether an emulated fingerprint satisfies `SecureStore`'s prompt, what payload Android delivery expects — and shipping four unrunnable flows that assert guesses is the failure this whole issue was opened about, in a worse form. The gaps stay dated, with blockers now precise.
...
- Split overloaded mobile owners: update-wall contract vs compat into separate owners, version skew proven in **both** directions; add hostile-input cases to `mobile.security` and interleaved-writer cases to `mobile.concurrency`. **All three landed.** `mobile.contracts` and `mobile.compat` named one file; compat moved to `mobile-gateway-skew.test.ts`, which states skew as range-overlap arithmetic in both directions (newer gateway, older gateway, containment, one-version overlap at each edge, the disjoint boundary either side, and a capability gap that must never blame the app). `mobile.concurrency` gained four interleaved cases where *different* operations overlap. `mobile.security` gained eleven cases — nine rejections and two counterweights (an ordinary presigned object, and a legitimate key containing an encoded `%`) — and **three of them found real holes**, fixed rather than pinned (see What changed).
  → `apps/mobile/src/lib/replica/mobile-gateway-skew.test.ts` (new compat owner, repointed in `tests/matrix.json`), `apps/mobile/src/lib/replica/sqlite-intent-store.test.ts` (four interleaved cases), `apps/mobile/src/lib/upload/transfer-policy.test.ts` (nine hostile-input cases) and `transfer-policy.ts` (the two fixes).
- One frame-level capture/share-in journey via Android intent synthesis. `flows/share-intent-in.mjs` synthesises ACTION_SEND through the new `ctx.device` escape and asserts Quick capture opens **carrying the shared text verbatim** — the payload half, which is what a build that opened the right screen while mangling `encodeURIComponent` would lose. Staged `promoting` (never run; five nights) so it cannot gate a PR.
  → `tests/agent-e2e-mobile/flows/share-intent-in.mjs`, the `ctx.device` escape in `lib/harness.mjs`, staged in `roster.json` and run by `run-promoting-suite.mjs`.
- On-device op-sqlite probe. `flows/op-sqlite-probe.mjs` — the only evidence in the repo about the engine the product ships, since all 52 W3 cells run against the in-memory `NodeSqliteDriver` stand-in. Proves reads stay coherent while writes land and that every write of a five-note burst survives process death. Explicitly **not** an exactly-once proof: it detects an intent re-executed on recovery, not a row duplicated verbatim, and both the flow header and the roster claim say so. Staged `promoting` (never run; ten nights).
  → `tests/agent-e2e-mobile/flows/op-sqlite-probe.mjs`, staged in `roster.json`, run by `run-promoting-suite.mjs` under `flows/promoting-budget.md`.
...
**The follow-up pass** — 22 files (10 new)

Listed apart from the waves above because they landed after the first push, and folding them into those counts would misreport what each wave delivered. The counts come from `git status --short`, not from memory: an earlier version of this section said "12 files (8 new)" while omitting six changed files including `lib/harness.mjs`, which is where `ctx.device` — a headline item of this very pass — actually lives.

*New (10)* — `apps/mobile/src/lib/replica/mobile-gateway-skew.test.ts` · `scripts/lint-e2e-wiring.cases.mjs` · `tests/agent-e2e-mobile/flows/op-sqlite-probe.mjs` · `tests/agent-e2e-mobile/flows/promoting-budget.md` · `tests/agent-e2e-mobile/flows/share-intent-in.mjs` · `tests/agent-e2e-mobile/lib/fixed-delay-agent.mjs` · `tests/agent-e2e-mobile/lib/fixed-delay-agent.test.mjs` · `tests/agent-e2e-mobile/lib/sh-quote.test.mjs` · `tests/agent-e2e-mobile/run-promoting-suite.mjs` · `tests/integration-mobile/assistant-provider.integration.test.ts`

*Changed (12)* — `CHANGELOG.md` · `QUALITY.md` · `apps/mobile/scripts/android-emulator-roster.sh` · `apps/mobile/src/lib/replica/mobile-gateway-compatibility.test.ts` · `apps/mobile/src/lib/replica/sqlite-intent-store.test.ts` · `apps/mobile/src/lib/upload/transfer-policy.ts` · `apps/mobile/src/lib/upload/transfer-policy.test.ts` · `scripts/lint-e2e-wiring.mjs` · `tests/agent-e2e-mobile/lib/ci-gateway.mjs` · `tests/agent-e2e-mobile/lib/harness.mjs` · `tests/agent-e2e-mobile/origin-acts.json` · `tests/agent-e2e-mobile/roster.json` · plus `receipts/issue-890-mobile-testing-rebuild.md`, `tests/matrix.json` and `tests/experience-budgets/mobile.json`.

`transfer-policy.ts` is the only **product** source file in this pass; everything else is test layer, harness or registry. That asymmetry is the point: a test-layer change that has to edit shipped code has either found a bug or is bending the product to suit a test, and here it is the first.

`scripts/lint-e2e-wiring.cases.mjs` exists for a different reason: the wiring linter had grown to 656 lines against the repo's 625-line god-file ceiling — a violation the first push carried and CI reported. The fixtures moved out and **the assertion stayed in**, so running the linter directly still exercises every case; a rule engine whose cases only run under `bun run test` rots into an always-passing gate the first time someone runs it alone. Verified by disabling the `promoting` rule and watching the linter's own self-test name the case that broke.
...
- **Five of the seven W5 journeys remain**, and the count is stated carefully because the previous two drafts of this bullet both got it wrong. The seven are Photos free-up-space, Locker biometric unlock, the frame-level share-in, upgrade-with-data, notification delivery, the granted camera-roll path, and the Assistant first-token. **One landed** — the frame-level share-in. The Assistant's *blocker* was removed but its journey was not written, so it is counted here as remaining. The op-sqlite probe also landed but is **not** one of these seven: it was its own separate item, and folding it in to reach "two of seven" was the miscount an audit caught. The blanket excuse the remaining five shared is gone: `ctx.device` in `lib/harness.mjs` wraps one targeted `adb -s <udid> …` / `xcrun simctl … <udid> …`, which is what `emu finger touch`, `am start`, `simctl push` and `simctl addmedia` needed. What is left is **specific and different per journey**, and `origin-acts.json` says so per row: free-up-space has a FALSE PREMISE on mobile (the button is permanently `disabled` and redirects — the phone never evicts); the camera roll needs a committed test image and iOS's `limited` state has no simctl verb; notification delivery needs FCM plumbing or an `.apns` fixture, not a flow; Locker's gate is a passphrase gate and whether an emulated fingerprint satisfies `SecureStore`'s prompt is unknowable without running it; upgrade-with-data needs a previous release artifact the lane does not build.
- **The Assistant journey.** Its stated blocker is gone: `lib/fixed-delay-agent.mjs` ships as a real ACP agent with a known first-token delay, wired as the CI gateway's provider and proved to resolve by `tests/integration-mobile/assistant-provider.integration.test.ts`. The journey that taps send and waits for first paint still needs a device, so `sendToFirstToken` moves from "no rig exists" to "rig exists, no number yet" and its `status` deliberately stays `unmeasured`.
...
- ~~**Splitting the overloaded mobile owners.**~~ **Done** — this bullet previously deferred the work on the grounds that it "belongs to a wave that can measure the resulting cells". That reasoning did not survive contact: all three owners are ordinary Node-tier files, every case is runnable and was demonstrated red against a mutation, and the hostile-input half found two live security holes. Deferring it would have left those shipped. The superseded reasoning is kept here rather than deleted, because a deferral that turned out to be wrong is worth being able to find later.
...
### Second audit — the follow-up pass

The follow-up pass was audited the same way, by a fresh-context sub-agent given only the diff and asked to find claims exceeding their evidence, tests that cannot fail, factual errors, and broken wiring — with instructions to mutate source and re-run rather than reason from the prose.

**`VERDICT: REFUTED`, with eighteen findings, and the severe ones were severe.** All are fixed; the four that matter most:

1. **The headline "split" had not happened.** `mobile.compat` was repointed at the new skew file while `mobile-gateway-compatibility.test.ts` kept its `judgeMobileGatewayCompatibility` test — so the double-ownership the change claimed to remove was still there under a new name. **Fixed:** the judge test is deleted from the old owner, which now carries a pointer to where skew lives.
2. **The security fix left a hole of exactly the shape it was written to close.** Segments were split on `/` alone, so `..%5c..%5c` — which `URL` does not normalise, because it rewrites only a *literal* backslash — was accepted. The audit constructed the URL and ran it against the shipped predicate. **Fixed**, with a test that goes red when the separator class is narrowed back. Two smaller defects in the same function came with it: a legitimate key containing an encoded `%` was rejected as malformed, and a decoding bound of four contradicted a comment promising "however many".
3. **Two tests could not fail.** "Rejects an encoded prefix that only decodes into scope" passed against the *pre-fix* implementation and its premise was false — no decoding put that path in scope, so it distinguished nothing. And the settle/transition orphan check looped over an empty array, asserting **zero times** in exactly the case it was written for, while hard-coding the one ordering under which the defect cannot appear. **Fixed:** the first is replaced by cases that bite, the second is a set comparison driven in both orderings — and the reversed ordering catches the `transition` mutation the audit proved survived.
4. **`flows/share-intent-in.mjs` could not have worked.** `adb shell` joins argv with spaces and hands the result to the device's `sh` without escaping, so the payload — chosen to contain a space and an apostrophe — would have word-split and left an unterminated quote, failing before any assertion. The `ctx.device` comment asserting the opposite ("nothing here interpolates a flow's own data through a shell parser") was true of the host and false of the layer that matters. **Fixed:** `shQuote` with its own spec, applied at the call site, and the comment now names the trap instead of denying it.

The rest were prose defects of one family — counts and mechanisms stated more confidently than checked: a "four of these rows" that was one, a "two of the seven" that was one, an inventory of "12 files" that omitted six including the file holding this pass's headline feature, a budget table whose enumeration summed to 18 under a heading of 17, an off-by-one claim disproved by mutation, and a stated mechanism for an uncoverable comparison that was simply the wrong mechanism. Each is corrected in place, and where the correction is interesting the superseded version is named rather than erased.
