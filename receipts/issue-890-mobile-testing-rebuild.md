...
- **Seven of the W5 journeys** — Photos free-up-space, Locker biometric unlock, the frame-level share-in, upgrade-with-data, notification delivery, the granted camera-roll path, and the Assistant first-token. **Six** of the seven need out-of-band tooling the harness does not wrap (a seeded simulator library, `adb -e emu finger touch`, intent synthesis, a previous release artifact to install over, `simctl push`, `simctl addmedia`), and each is now a **dated gap with its blocker stated** in `tests/agent-e2e-mobile/origin-acts.json` or the README's gap table, which is a materially better state than the invisible absence they were in. The seventh, the Assistant journey, is blocked on something else entirely: a fixed-delay stub turn in `lib/ci-gateway.mjs`, without which the `sendToFirstToken` budget stays `NONE TODAY`. (An earlier draft of this bullet said "six" over a list of seven and then "five of the six" — corrected here rather than quietly, because a miscount in the register of what is NOT done is the one place it does the most damage.)
- **The on-device op-sqlite probe.** This one is not blocked on tooling, and it is the most load-bearing omission in the change: W3's boot-condition tier proves 52 of 56 cells against `NodeSqliteDriver`, the repo's `node:sqlite` stand-in (`tests/integration-mobile/lib/seat.ts` says so in its own header), so **every one of those cells is evidence about the stand-in, not about the native module the app actually ships**. Same SQL, but not the same driver: statement lifetime, concurrent-reader behaviour and error surfaces are exactly where the two could diverge, and nothing in this PR would notice if they did. Writing the probe needs no out-of-band tooling — a device journey that opens the real replica and exercises those seams would do it — so it is left undone for size, not for impossibility, which makes it the first thing to pick up after this lands.
