...
Review-driven follow-up (second commit):
- `packages/app-engine/src/http/http-server.ts` — reverted the auth-selection fallthrough: a present `Authorization` header selects the bearer path outright (invalid bearer → 401), so the cookie `authorizeRequest` (which mutates `req.url`/headers) never runs alongside a bearer. Added regression tests in `packages/app-engine/src/http/http-server.test.ts`.
- `packages/vault/src/blob/pdf-text.ts` — `extractTJArrayStrings` now buffers candidate strings and only commits them after the `] TJ` terminator is confirmed, and an unbalanced `(` no longer aborts the remaining `Tj` scan. Added characterization tests in `packages/vault/src/blob/pdf-text.test.ts`.
- `packages/app-engine/src/conversation/auto-title.ts` — restored the pre-refactor collapse → strip → trim order; characterization tests in `auto-title.test.ts`.
- `packages/app-engine/src/handlers/dispatcher.ts` — `loadManifest` stats first and only reads the manifest bytes on a cache miss.
- `packages/vault/src/gateway/sql.ts` — replaced the trailing `;+\s*$` regex with a manual terminator strip; tests in `sql.test.ts`.
- `packages/client/src/gateway-client-editing.ts`, `packages/client/src/gateway-client-automation-editing.ts` — added `// lgtm[js/insecure-randomness]` suppressions with justification: the editing-session id is the deterministic `desktop-<appId>` worktree key, not a random security token.
- `packages/gateway/src/cli/key-store.ts`, `packages/backup/src/recovery-kit.ts`, `packages/vault/src/ingest/staging.ts` — moved `// lgtm[js/insufficient-password-hash]` suppressions onto the flagged line, keeping the justification.
- Minor: corrected stale comments in `route-helpers.ts` / `apps-store-draft-files.ts`, hardened preview temp cleanup in `stream-ingress.ts`, matched old `parseAddress` edge cases in `mbox.ts` (plus `const`), treated directory owners as missing in `validate-matrix.mjs`, and re-validated the on-disk WAL bytes in `wal-restore.ts`.
- Added characterization tests pinning pre-refactor behaviour: `packages/vault/src/ingest/mbox.test.ts` (new), plus cases in `pdf-text.test.ts`, `auto-title.test.ts`, `sql.test.ts`, `preflight.test.ts`, and `enrich-publishers.test.ts`.
...
Review follow-up files: `packages/app-engine/src/http/http-server.test.ts`, `packages/vault/src/blob/pdf-text.test.ts`, `packages/app-engine/src/conversation/auto-title.test.ts`, `packages/vault/src/gateway/sql.test.ts`, `packages/agent-runtime/src/preflight.test.ts`, `packages/vault/src/ingest/enrich-publishers.test.ts`, `packages/vault/src/ingest/mbox.test.ts`, `packages/client/src/gateway-client-editing.ts`, `packages/client/src/gateway-client-automation-editing.ts`.
...
All of the above commands passed. After verification, the `security-fix-high` branch was committed and pushed, and a PR was opened linking issue #597. After PR review, the follow-up fixes above were verified with `bun run format`, `bun run lint`, `bunx turbo run typecheck` (app-engine, vault, agent-runtime, client, backup), per-package `bun run test` for the same five packages, and `bun run test:report:smoke`; the `security` workflow was dispatched against `security-fix-high` to confirm the CodeQL alerts clear.
