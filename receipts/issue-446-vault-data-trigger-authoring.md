...
- `packages/client/src/react/shell/routes/AutomationEditorRoute.tsx` — `loadEntityTypes` wiring; `vaultForTriggers` exported.
...
Results: `bun run ci` (format:check, oxlint + turbo lint, typecheck, lint:types, lint:css) green; gateway work-order tests 5/5 (4 existing + 1 new); client editor + vault tests 17/17 across the three files; a full touched-dir sweep of `src/react/screens src/react/shell/routes` in `packages/client` passed 65 files / 513 tests; skills compose tests 4/4 (SKILL.md edits don't break skill composition).
...
- Per-app "automate this" entry points (deep-link from entity views into the editor).
- `@[entity/id]`-token pickers in the trigger UI; plain datalist ships in v1.
