...
3. **Amendment after the verifier's PASS: the pragma bracket is replaced by a file split.** The verifier raised, and the root accepted, that `synchronous=NORMAL` under `journal_mode=DELETE` is a **rollback journal** — a power loss can corrupt the file, not merely drop the last commits — so with the mobile outbox in the same file the `FULL` bracket could not protect it whatever the pragma said. `SB-replica-sync` now rules the file split instead: on mobile the intent outbox **moves to its own SQLite file on its own connection at `synchronous=FULL`** (today it shares the replica file, `sqlite-intent-store.ts:68-70`, `native-session.ts:1130`); the replica store stays `journal_mode=DELETE` at `NORMAL` because the attached second reader depends on `DELETE`, and because it is derived state a corrupted replica file is **detected on open and re-bootstrapped rather than repaired**. Wave 3 (C2) lands both, with a power-loss case in the convergence suite proving detection, re-bootstrap and no outbox loss; B4 measures fsyncs per offline intent under both profiles. The same wording is mirrored in the note under the register. Both audit blocks are left untouched, including the passage in the second one that verified the now-superseded bracket against the code — it is the record of what was checked at that SHA.
