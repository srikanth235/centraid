...
- **Four files were touched outside the slice contract's named list, minimally and for one reason each.** `packages/client/src/replica/store-core.ts` and `apps/mobile/src/lib/replica/multi-vault-reader.ts` are the read plan's only two callers and the root ruled them INSIDE the contract — the plan cannot report truncation without them, and having gained a probe row would return one row too many if left alone; each change is inside one function. Genuinely outside: `packages/client/src/replica/worker-client.ts` (one spread, to carry the two fields across the worker hop), `packages/client/src/replica/search.ts` and `apps/mobile/src/lib/replica/multi-vault-session.ts` (both from verifier follow-up 1 below), and `packages/blueprints/types/centraid.d.ts` (the shared handler contract the 138 web flag additions must typecheck against). `apps/web/tests/e2e/tasks.spec.ts` and `playwright.config.ts` were added on the root's instruction, not on this slice's own initiative. All reported rather than left silent.
