...
by `apps/desktop/tests/e2e/onboarding-home.spec.ts` with Household open.
...
- `apps/desktop/tests/e2e/onboarding-home.spec.ts` — adds journey 2.12, which
  opens Household and emits `artifacts/e2e/ui-impact/issue-750-vault-sharing.png`
  as this issue's visual evidence. It anchors on the page's own unconditional
  `<h1>`, not on the sharing card's "People & circles" heading: a first run
  against the e2e mock gateway does not mount that card, and an assertion that
  depended on fixture depth would make this a sharing-data test rather than the
  evidence frame it is
