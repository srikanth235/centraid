...
- Recorded maintainer-approved file-size waivers on the four cohesive issue files that cross the 625-line hygiene ceiling: the mounted reader and its end-to-end fixture, the crash-ordering coordinator, and the cross-tree engine-conformance scanner. The waiver changes no runtime or gate threshold.
...
9. The four `repo-hygiene file-size-limit` waivers are scoped to files whose behavior is deliberately audited as one boundary; the maintainer approved those waivers after the commit hook surfaced the limit. The repository-wide 625-line threshold remains unchanged.

Gate-required deviation note (quality knobs, verbatim single line):

#738 re-pins governed fingerprints after adding the additive pending-overlay reliability flow and demonstrated-red evidence; no budget or existing gate was weakened.
