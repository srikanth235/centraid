...
**What this does NOT do:** it does not fix `cold-start`'s assertion. Closing that means asserting a launcher landmark instead of a band label, which is a strengthening — but it converts a currently-green flow into an honestly-red one for as long as the replica arrives empty, so it belongs with the replica fix rather than ahead of it. Recorded in `cold-start.md` under its own heading so the next reader finds the limit stated rather than rediscovering it.
