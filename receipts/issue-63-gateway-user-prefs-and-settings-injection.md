...
- [x] Single gateway sqlite (`centraid-gateway.sqlite`) with real cross-table FKs — UserStore + ChatHistoryStore share one connection via `gateway-db.ts`
...
**Chat-history sessions scoped to the user identity — `chat_sessions.user_id` baked into the baseline schema, every read + write filters by current `UserStore.getUserId()`.** The chat-history store and the user store were two unrelated SQLite files, so a chat session had no link back to the user identity. With the gateway now the source of truth for user prefs, the chat history needs the same scoping — otherwise two devices syncing to the same gateway can see each other's history (and a future multi-user model has no path forward without a column-add migration). Since centraid is pre-1.0, the column went into the baseline directly rather than as a follow-up slot — no append-only migration ladder yet, no backfill machinery to carry. The `ChatHistoryStore` constructor now takes a required `userIdProvider: () => string` (wired to `UserStore.getUserId` by both hosts); every prepared statement filters by `user_id`, every insert stamps it. Five new tests cover that two stores against the same SQLite file with different user UUIDs cannot see, write to, rename, or delete each other's sessions.

**Single gateway sqlite (`centraid-gateway.sqlite`) with real cross-table FKs — UserStore + ChatHistoryStore share one connection via `gateway-db.ts`.** The earlier two-file split (`centraid-user.sqlite` + `centraid-chat-history.sqlite`) was path-dependent, not principled — chat-history pre-existed, user-store followed precedent. With single-user, both files in the same directory anyway, and `chat_sessions.user_id` only an application-enforced FK (cross-attached-DB FKs aren't a thing in SQLite), consolidation is a clear win:

- New `gateway-db.ts` owns the single `MIGRATIONS` array spanning four tables — `users(id, created_at)`, `user_prefs(user_id, key, value)`, `chat_sessions(id, user_id FK→users, …)`, `chat_messages(session_id FK→chat_sessions, …)`. All cross-table relationships use `ON DELETE CASCADE`.
- `openGatewayDb(path)` opens the file with `PRAGMA foreign_keys=ON` and runs the migration tail. `makeGatewayDbProvider(path)` wraps that into a lazy `DatabaseProvider` getter the OpenClaw worker subprocesses don't trigger unless they actually serve HTTP.
- Both `UserStore` and `ChatHistoryStore` now take a `DatabaseProvider` (no more dbPath, no more per-store migrate function); they share one `DatabaseSync` handle, one prepared-statement cache per store, one transaction lock for cross-table writes. `Runtime` carries both as optional fields; `startRuntimeHttpServer` mounts the routes from those fields rather than reopening files itself.
- `users.id` is the FK target for both `user_prefs.user_id` and `chat_sessions.user_id` — application code can't insert orphan prefs or orphan sessions even if it wanted to. A new `gateway-db.test.ts` covers the consolidated migration shape, the FK-cascade behavior end-to-end (DELETE FROM users wipes prefs + sessions + messages atomically), and the lazy-provider semantics.
- v0 means no data migration — existing dev installs with the old two-file layout get a fresh `centraid-gateway.sqlite` on next launch and the old files sit orphaned on disk (delete manually if you care).
