...
**Law** · window door · range `e9a7d81a..7969869e` · law digest `53be88c22ab5` → `53be88c22ab5`
...
- rulings recorded: #272, #352, #456, #505, #555, #834, #892, #922, #996, #1014, #1020 in `docs/decisions.md`
- changelog entries: #1020
- no gates moved
- no waivers used
...
Regenerated by `node .governance/law/run.mjs --front-page <path>` and pasted between the two markers; hand edits inside that block are overwritten by the next run.
...
- [x] **Close pass**: the acceptance criteria of [#1020](https://github.com/srikanth235/centraid/issues/1020) walked one by one, the spike report and the transport experiment recorded with evidence, and the doc pass over every state document the waves changed

**Where the boxes stand at the close** (the note below is wave 0's and is kept as written, because it was true of wave 0):

- **Waves 0, 1, 2, 3 and 4 are ticked**, with one stated exception inside wave 4: the **connectors lane did not run** — the owner put it on the back burner on 2026-09-12 — so `crates/connectors` does not exist and *Connectors and OAuth assist* is the one capability-inventory row with no v1 half. The notifications lane did not run either, and nothing in the core depends on it.
- **Wave 5 (Ship) has not run.** Store lanes, the three-OS desktop service install and the docs rewrite are wave 5; this close pass did the doc half of it.
- **Wave 6 (Retire) is prepared and not merged**: `claude/1020-retire-v0` at `b318fba1`, one commit, pushed. Its gate is red on twelve tests whose subject is that the two trees agree, which is the class of test that goes with the tree. See *The prepared retirement* in the close section.
- **The close pass is ticked**: the acceptance criteria are walked one by one below, the spike report and the transport experiment are recorded with the evidence each one actually has, and the doc pass is done.

_Wave 0's note, unedited:_ Ticked by wave 0: **box 1 only**. Every other box needs code that does not exist, and no acceptance criterion of the issue is met by this slice.
...
**`296a4ea9` — `fix(blueprints): the task zone column is tz, not recurrence_tz`** — finding **R-1020-35** ([#1020](https://github.com/srikanth235/centraid/issues/1020), [docs/decisions.md](../docs/decisions.md#decisions--lane-v-1020)), fixed at source with a demonstrated red.
...
## Close — lane CL-DOCS: the doc pass, the decisions log, the hand-off book, and the prepared retirement

The close pass's docs half ([multi-agent.md](../docs/multi-agent.md#the-close-pass)): every touched doc to current state, the rulings moved out of this receipt and into the decisions log, the owner hand-offs consolidated, the acceptance boxes reconciled, and the wave 6 commit prepared on a branch that is not merged.

Doctrine digest for this lane: **`53be88c22ab5`** (`node .governance/law/brief.mjs`).

### What landed, by commit

**`fa6c296d` — `docs(traps): the shared target directory and the JSON that is not canonical`**

- `docs/traps/shared-cargo-target.md` (new) — the two mechanisms, not one: `crates/api-proto`'s build-script `OUT_DIR` is keyed by package identity, which is identical in every worktree, so a lane editing a `.proto` hands its generated Rust to every other lane; and a compiled `xtask` used to remember the tree it was built in. The xtask half is fixed in code; the `OUT_DIR` half cannot be fixed from inside the workspace, so the trap is the defence.
- `docs/traps/serde-json-preserve-order.md` (new) — `preserve_order` unifies across a build, so a crate four hops away decides whether another crate's sort key is insertion order or name order. Green under `-p`, red under `--workspace`.
- `docs/traps/README.md`, `docs/dev-environment.md` — the index rows, a sixth worktree rule, and the v1 gate-loop paragraph now saying one target directory per worktree.

**`51f56657` — `docs(decisions): every lane ruling of the umbrella, in its final form`**

- `docs/decisions.md` — four new sections under `## v1 platform …`: **Wave 1**, **Wave 2**, **Wave 3** and **Wave 4 lane rulings**, plus **The close pass**. Every `D-1020-*` row from every lane's `### Decisions` block, in its final form — statement, options weighed, recommendation adopted — with the five supersession chains inside the umbrella in one table rather than left to be reconstructed from two places. The fourteen open questions each get their current answer and the evidence or the absence of it. Five census miscounts the waves corrected are recorded. The three pre-existing anchors (`#decisions--lane-v-1020`, `#decisions--lane-b2-1020`, `#decisions--wave-3-close-lane-x3-1020`) are untouched, because four docs and the law's arrival record cite them.

**`af3406dd` — `docs: the state documents describe two trees, and the hand-off book`**

- `ARCHITECTURE.md` — a new head: `## Two trees, one repository`, then `## The v1 platform` (the two roles, a line per crate over all 25 members, the shells and what crosses between them, contracts/fixtures/ledgers), then `## The v0 gateway (the pinned oracle)` over everything that was already there, unaltered. The workspace layout shows both trees. The four anchors other files cite are unmoved.
- `README.md` — a two-trees note under the header links; a v1 layout table beside the v0 one under a **retiring** heading; `## Build / check (v1)` with the five gate profiles, the binary's verbs and the three shells' commands, and the v0 commands under `## Build / check (v0 — retiring)`.
- `TESTING.md` — `## The v1 gate profiles`: the five profiles as a concatenation with their budgets and steps, what each kind of proof covers **and where it stops** (parity fixtures, the simulation, the drills, the structural rules, the ledgers), the table of what only a device can prove, and the two inherited reds named rather than allowlisted.
- `CONTRIBUTING.md` — the green-gates row now names both trees' commands, and `## The v1 tree, and what it refuses` states the three rules that bite first: one gate command, `sql-confinement`, and what an oracle edit may and may not be.
- `SECURITY.md` — two bullets in the v1 trust premise and **two claim-register rows**: connector tokens are host-readable by design (`sync.connection_credential` is sealed under the vault DEK, which a gateway holds, because a gateway that could not read a token could not connect an account), and the member-key gate does **not** cover a host with root reading `K` out of an unlocked seat's process memory. The exit criterion's own wording is named as wanting amendment.
- `docs/protocol.md` — the two packages with two promises, the framing, the compatibility rules, the honest limit on unknown fields, and why the desktop's local channel is not on that wire.
- `docs/harnesses.md` — the registry as a generated fixture, the two refused arguments, adapter discovery, `centraid mcp` with no listener, the live fake agent and the cost of having no recorded v0 wire bytes.
- `docs/glossary.md` — eight v1 nouns the waves used and the glossary did not carry: **kit**, **door**, **band**, **artifact key**, **identity stamp**, **member key**, **drill**, **gate profile**; the reference-device row now names the two ruled models.
- `docs/blueprint-seats.md` — the v0 `origin`/`custodian`/`viewer` triple and v1's replicated/thin axis are two different questions, said out loud; what survives the port and what retires with the PWA.
- `docs/photos/README.md` — Places is a facet, the library takes one page on purpose, the recognition tier is a fourth value, and the two `media.forget_person` findings.
- `docs/design-machinery.md` — the KMP and Rust lowerings as two more consumer rows, and rule 2 extended: a v1 lowering is **generated, never written**.
- `docs/release.md` — `## What a release workflow owes the egress ratchet`, with the currently-red release lane named.
- `docs/release/v1-handoffs.md` (**new**) — below.
- `crates/centraid/README.md` — the verb table stopped saying three shipped verbs are unbuilt: `seat`, `native-host` and five new surfaces run, `devices list|revoke` are the only two still owed, and `--data-dir` is honoured.
- `AGENTS.md` — the hand-off book in the Release + ops row.

**`3b7b4f7d` — `test(journeys): the app crates' year-3 numbers get an address`**

- `tests/journeys.json` — `gateway/app-read/*` entries with one `app-read` journey and new year-3 volumes (Tally, Docs, Notes, People; Photos rides the existing `year3-50k-assets`; Agenda and Tasks follow in the next commit, for seven entries and six volumes in all). Additions only, no existing entry re-keyed, no ceiling moved, every number parked under `_observedMs` with `status: "projected"` so the ratchet reads none of them.

**`297b0664` — `docs: the schedule lane's rulings, questions and one zone source`**

- `docs/decisions.md` — `D-1020-S1` … `D-1020-S9`, and a sixth census correction: the `schedule` schema carries **four** confirm-gated commands, not three, because v0 declares them across two files and the census read one.
- `docs/release/v1-handoffs.md` — the lane's three owner questions, including the one it deliberately did not answer: whether two destructive verbs should carry manifest confirmations, where the lane's evidence for adding one was "v0 does not have one", which is an observation and not a finding.
- `docs/cron-timezone.md` — **one zone source, and it moved down**: `FireZone` is `crates/vault::time::zone` now and the automations crate re-exports it, so cron and recurrence resolve a wall time through one function. `FireZone::resolve(trigger_tz, vault_zone)` takes two tiers and no third argument — the deletion of tier 3 is in the signature.
- `ARCHITECTURE.md`, `tests/journeys.json` — Agenda and Tasks in the crate table and two more `app-read` entries, the second recording the 1,500-instance expansion cap as **reached** at the year-3 volume rather than as a constant.

### The hand-off book

`docs/release/v1-handoffs.md` is the canonical list, and the lanes' receipt sections are the evidence layer behind it. Seven areas, each row carrying the exact command, the evidence to hand back and **where the result goes** — a ledger key, a decision row, or a parked ceiling that gets promoted:

| Area | Rows | The one that blocks most |
| --- | --- | --- |
| Devices, Xcode and the Android SDK | 6 | The self-hosted macOS runner. Four device cells exist and **fail** on a runner that claims the label with no phone. |
| Signing, notarisation and the stores | 6 | The updater's release key: `TRUSTED_RELEASE_KEYS` is empty, so every packaged update refuses, and enrolling one **flips an existing test** — which is the review signal that signed updates went live. |
| The gateway on a real host | 7 | The real VPS run. The system unit has never been loaded by a real systemd. |
| Models and the network fetcher | 6 | Five sessions and one fetcher, each with its own evidence sentence rather than a shared "it works". |
| The ACP adapters | 3 | One real `initialize`: no recorded v0 wire bytes are committed, so this run is the only thing that would catch a frame divergence. |
| CI, branch protection and the inherited reds | 6 | Branch protection. A required check that never reports blocks every pull request forever. |
| The device experiment and the two rulings on it | 4 | The iOS transfer experiment: R-1020-3 stops being provisional and R-1020-15's door ships or is deleted on its four numbers. |

The **owner questions** nine lanes filed are consolidated there in one table — People 5, Locker 5, Photos 3, and twelve more across Tally, Docs, Notes and the wave 2 lanes — each with its options, a recommendation, and **the test that has to change with the answer**, so "nothing happened" is not a possible outcome. Lane CL-FIX's section links to that table rather than carrying a second copy.

### The acceptance criteria of [#1020](https://github.com/srikanth235/centraid/issues/1020), one by one

- [ ] **`cargo xtask gate` with its four profiles is the only CI entrypoint and is green on `main`; `pr` completes within its budget.** It **is** the only entrypoint — `ci.yml` came off `pull_request` in wave 1 and `gate.yml` replaced it — and there are five profiles, not four (`mobile-jvm` is not a superset of anything, by [D-1020-B2-3](../docs/decisions.md#decisions--lane-b2-1020)). It is **not green on `main`**, because the umbrella has not merged; on the umbrella tip `pr` is red on the two inherited findings named below plus `ci-policy`, and `local` is **over its 120 s budget** on a warm tree — 285.7 s at the People lane's measurement, of which `cargo test --workspace` alone is 219.7 s over 23 members. **The budget was not touched.** The remedy is structural and is hand-off 6.5.
- [ ] **`centraid gateway` runs from the Docker image on a clean VPS with no listening TCP port (blob door off) and prints a pair QR.** The device-less half is proved on every `release` run: `vps-smoke` installs the built tarball into a clean, digest-pinned container through `deploy/vps/install.sh`, founds a vault, pairs a seat over iroh, seals a WAL segment, takes a generation, runs `doctor`, restarts over the same data directory, and refuses a tampered `identity.json`. The no-listener half is proved twice — a structural rule over `crates/**` and a test that spawns the real gateway and reads `/proc/net/tcp`. **What is open is the real host**: hand-off 3.1.
- [ ] **A release-built iPhone and Android phone and an Electron seat pair to that remote gateway, work offline, and converge on reconnect; the desktop also works in thin mode and shows its connectivity and durability states.** The Electron third is done and proved end to end (`desktop-e2e`: a real window, a real sidecar over a real socket, a video that seeks inside a blob still arriving). The phone two need hand-offs 1.2 and 1.3 — and, before a phone could be judged on this box at all, the **screen surface the plan never budgeted**: three screens exist against v0's ~40 shell screens and 72 app screens (see *Corrections to the plan*). Convergence itself is proved for all three seats by the simulation rather than by a device.
- [ ] **The absolute targets in `tests/journeys.json` are met on the reference devices, including frame misses during concurrent sync and typing latency in Notes; wave 0 baselines are also beaten.** All six are parked `_intended` and gate nothing. The two reference devices are **named** (Q-1020-2) and neither has been run. The Rust half of the typing number is measured (1.13 ms for the on-open pull over the largest body the product holds); the half the target is stated on is a Kotlin state machine with no Rust twin.
- [ ] **The iOS per-state transfer promise is documented and its journey passes on a physical device.** **Documented**: eight states in [mobile-offline.md](../docs/mobile-offline.md#the-per-state-promise--the-camera-roll-backup-on-ios-provisional), with one row deliberately blank and the four sentences that may fill it already written, so the run picks one rather than composing one. **Not run**: hand-off 7.1.
- [ ] **Every app's parity fixtures pass in Rust; every inventory journey passes.** Parity: **every app lane that ran is green on both sides** — Tally (29 cases compared whole), Photos, Docs, Notes, People, Locker, automations and the assist corpus, each generated by executing v0 and each proved by the TypeScript oracle over the same files. The journeys are the open half: seven `gateway/app-read` rows now carry the Rust-side numbers as projected, and every device-side journey is hand-off 1.4 — **and the inventory's journey for a mobile app cannot be walked at all until its screens exist**, which is the plan gap above rather than a lane's shortfall.
- [x] **The ABI exports five symbols and its `CONTRACT.md` clauses each have a test; the version-window handshake refuses out-of-window seats and stale artifacts.** Five `#[unsafe(no_mangle)] pub extern "C"`, counted by a test **and** by `nm` over the built `cdylib` as a second and different question; ten clauses, one test each through the C entry points; a committed cbindgen header whose regeneration is asserted to be a no-op. The window refuses at both ends with a fifth test asserting the ends are **distinct**, so it cannot collapse to a point and pass vacuously. A stale artifact is refused at `open` against the caller's stated digest, and `vps-smoke` installs a tampered `identity.json` and asserts the refusal. Clause 9 is proved against a **real** panic by the `fault-door` step, the one place the tree is compiled with `debug-fault`.
- [x] **The restore drill runs in CI; the failure tests (process death, interrupted snapshot, disk full, duplicate delivery, restore with paired seats) pass.** `restore-drill` is a `release` step and runs the real binary: it deletes the live data directory and compares 247 tables and 209 rows after `centraid recover`, and refuses a census of zero rows outright. Five failure modes, ten tests, four demonstrated reds; process death and duplicate delivery are proved in `crates/sim`, where a real host crashes and restarts over a real file. **Its first real run failed**, and that red is what produced [D-1020-R8](../docs/decisions.md#wave-2-lane-rulings-1020). One honest gap: the drill's re-pair is a replica re-bootstrap, not an iroh pairing handshake.
- [ ] **Store builds ship from the native projects; the desktop ships with the bundled sidecar and installs a local gateway on macOS, Windows and Linux.** The desktop bundles the sidecar and `centraid gateway install` writes a unit and never enables it. **Windows is short one door**: `centraid seat --socket` exits 3 there until the named-pipe DACL is written and observed to refuse somebody. Store builds are wave 5 and hand-offs 2.1–2.4.
- [x] **The extension works over native messaging with no WASM.** Eighteen methods over one framed stdio channel, proved twice: against a real `centraid seat` over a real Unix socket in Rust, and in a real headed Chromium that loads the unpacked Companion, writes a host manifest into its own profile, fills a real login form on a real `https://` origin and **offers nothing** on a look-alike. The absence of WASM, iroh, `fetch`, `indexedDB` and every `crypto.subtle` key operation is asserted over the sources **and the built tree**. The Companion now bundles **no third-party code at all**.
- [ ] **The v0 tree and retired tools are deleted; decisions.md carries the rulings and supersessions; no broken links.** The decisions half is **done** — every ruling and every supersession chain is in `docs/decisions.md`, and every relative link this pass added resolves against the GitHub heading-slug rule. The deletion is **prepared and not merged**: see *The prepared retirement* below. Wave 6 is gated on the device evidence the boxes above are open on.
- [x] **The simulation suite runs on every PR and a recorded failing seed from wave 2 stays green.** `sim` is a `pr` step at 25 seeds and `sim-nightly` is 250. Every host is real except the network — real vault, real doors, real applier, real outbox, real sync driver, real on-disk SQLite — and it speaks **UDP**, because production is QUIC and because the product may not open a listening TCP socket at all. Two bugs were found by it in wave 2 and both seeds are in the suite for ever.
- [x] **Locker secrets are unreadable on the gateway host: a test reads the gateway's file and key store and fails to recover a sealed cell.** `member_key_gate.rs` is a **search**, not an assertion: five planted plaintexts hunted across every sealed table through the paged door, eight command answers, the seat snapshot compressed **and** inflated, the backup base, and the vault file with its WAL and SHM — plus a non-vacuity assertion that the paged door *did* serve `lk1:`. The key door is deleted **structurally** in three places rather than refused at runtime. **Two boundaries are outside it and are now in SECURITY.md with their own claim-register rows**: a host with root reading `K` from an unlocked seat's memory, and connector tokens, which are host-readable by design. The criterion's own wording wants splitting in two — hand-off 7.4.
- [ ] **Share and Autofill extensions work on iOS and Android without opening the vault file.** The **rule** is ruled and stated ([R-1020-24](../docs/decisions.md#v1-platform--rust-core-kmp-shell-electron-seat-gateway-anywhere-1020)): an extension writes an app-group inbox and Autofill reads a sealed index the main app maintains. No extension target is built; it needs the toolchains of hand-offs 1.2 and 1.3.
- [x] **The compile-time and library-size ledgers exist and only fall.** `contracts/ledgers/{gate-budgets,compile-time,library-size}.json`, checked against the merge-base on every profile, written only by `cargo xtask measure --write`, which only ever lowers. **No `approvedDeviation`, waiver or override path exists in the ratchet** — the one time these numbers rose it was an owner ruling, recorded per key with its measurement and its multiplier. `library-size` is seeded with twelve nulls, and a null cannot be ratcheted, which is why hand-off 3.4 exists.
- [x] **One receipt records every wave, the spike report and the transport experiment with evidence.** This file: wave 0 and 0b, four waves of lanes, two cross-lane fix waves and this close, each with its exit list, its demonstrated reds, its findings and its falsification. The binding spike's **host** half is measured (JNA, and not the Foreign Function & Memory API, because Android's minimum is nowhere near JDK 22 and measuring the binding the product will ship is the point of a spike); its device half is hand-off 1.3. The transport experiment is recorded as a **protocol with fixed evidence file names and a pre-written decision table** — which is the honest form of "recorded with evidence" for a run no machine here can make.

**Seven of sixteen closed.** Every one of the nine open boxes is open on the same two things: a physical device, or a host that is not this container. None is open on code nobody wrote.

### The inherited reds

Every red on the umbrella tip, what it is, since when, and whose call it is. **Not one was answered by widening an allowlist, a budget or a ledger.**

| Step | The finding | Since | Whose call |
| --- | --- | --- | --- |
| `secrets` | gitleaks `generic-api-key` on `packages/model-runtime/LICENSES.md` | predates the umbrella (arrived with [#1011](https://github.com/srikanth235/centraid/issues/1011)) | Owner: a reasoned `.gitleaks.toml` row, or move the string. Hand-off 6.2 |
| `secrets` | gitleaks on `contracts/golden/format-golden.json`'s `dataKeyHex` — the key the cross-language format golden is **sealed with**, and which has to be in the file for the golden to be checkable at all | wave 2 lane R (`c5de2b39`) | Owner. Not a false positive in `R-1020-35`'s sense: the string genuinely is a key and it is genuinely committed. Hand-off 6.2 |
| `osv` | `astro@7.1.5`, CRITICAL 9.8, in `bun.lock` | predates the umbrella | Owner: a dependency bump, outside this umbrella's scope. Hand-off 6.3 |
| `ci-policy` | `lint:ci-egress` — `lane-release-extension.yml` installs and runs dependency code with **no `step-security/harden-runner` step and no ledger entry** | wave 4 lane extension | A **release** lane with no egress control is the worst shape of this gap. The remedy is the step, not a ledger row. Hand-off 6.4, and now written up in [release.md](../docs/release.md#what-a-release-workflow-owes-the-egress-ratchet) |
| `ci-policy` | `lint:path-filters` — `copy/`, `design/` and `mobile/` are claimed by no `changes` filter in `ci.yml` and have no `tests/path-filter-ledger.json` entry | wave 3 (`d1d8787a`) | Hidden behind the egress finding, which fails first. `skipped` counts as a PASS in the required `check` job, so this is exactly the merge-green-unexercised failure that linter exists to catch |
...
Written down because a plan nobody corrects is a plan the next umbrella repeats.

- **The mobile screen surface was never budgeted, and that is a plan gap rather than a slip.** The inventory's *App UIs, mobile — replace* row was costed as **wave 3's three screens** (Tally list, Photos grid, Notes editor) and **no wave was assigned the rest**. The rest is not small: v0 ships roughly **forty shell screens** under `apps/mobile/src/screens/*` — Home alone is a 376-line graded springboard with per-tile grading, pins, first moves, a status line and an all-apps sheet, and beside it Capture, Scan, Onboarding, Settings, BackupHealth, NeedsYou, PhoneStorage, Sharing, Devices and more — plus **seventy-two app screens** across the seven app stacks in `navigators.tsx`. What v1 has today is the **navigation model** in `mobile/shared` (Home as the floor, covers, band destinations as parameters) and three state machines; **Home is a placeholder with three links in both shells**. Recorded here with the count rather than reported as done. The recommendation is a **follow-up umbrella scoped after the iOS hand-off makes Swift buildable**, so that every screen can be proven on both shells rather than one — building forty-plus screens against a toolchain that cannot compile half of them is how a surface ships twice.

- **Wave 4 was the largest wave by a wide margin, and the plan sized it as one.** "One lane per app, at most three concurrent, plus lanes for `crates/assist`, `crates/automations`, `crates/connectors` and `extension/`" turned into **ten** lanes across four slots. The connectors lane was put on the **owner's back burner on 2026-09-12** and did not run at all, so `sync_*` token custody is untouched and the inventory's *Connectors and OAuth assist — retain* row is the one capability with no v1 half.
- **The shared build cache cost the run several hours, twice, before the mechanism was understood.** Three wave-4 lanes reported workspace builds failing to compile crates they had never touched, and a gate reporting 98 scanned files where the worktree held 89. The cause was two mechanisms, not one: a build script's `OUT_DIR` keyed by package identity, and an `xtask` binary that remembered where it was compiled. The second is fixed in code; the first cannot be fixed from inside the workspace. **The correction is one target directory per worktree**, and it is now a trap rather than folklore.
- **One lane was restarted.** The Tally slot was split into lane D3 (the kit and the ceiling) and a later Tally-finish lane, because `crates/design` did not exist when D3 ran and the presentation fields of its parity fixture could not be compared at all. The cost was one deferred-comparison marker carried for two waves; the alternative — comparing by hand-editing a generated fixture — is forbidden and would have been worse.
- **The census was wrong five times, and each correction is a number a doc now carries.** Eighteen Companion methods, not seventeen. Nine share windows, not eight. The `core` schema's idempotency split summed to 25 of 27 — the `once` arm is eight. Places is a facet of Photos, not an app. And `queries/person.ts` makes **one** sharing read deny independently, not three — the sentence described the app's intent, which is why the port had to be written to the intent rather than from the sentence.
- **Two ruling ids collided.** Wave 3's close and wave 4's extension lane both used `D-1020-X3`. Named in `docs/decisions.md` and disambiguated there rather than renumbered, because a ruling id that moves is a citation that breaks.
- **The `local` budget is the one number the plan did not anticipate.** "Compile time is the new Hermes" was in the issue and the ledger exists; what the plan did not say is that a *test suite* grows faster than a *compile*, and that the profile would go over its budget without gaining a step.

### Plan versus delivered

Every row of [#1020](https://github.com/srikanth235/centraid/issues/1020)'s capability inventory, with where it landed and what its journey is worth today.

| Capability | Disposition | Landed | Journey |
| --- | --- | --- | --- |
| Vault schema, migrations, drift register | retain | `crates/ontology` (wave 1 lane A) | **green** — the golden vault opens, every digest the release froze is reproduced from Rust, 13 commitments hold |
| Commit log, receipts, audit and ledger bands, retention | retain | `crates/vault` (wave 2 lane D1; the ledger band's schema in wave 4 lane assist) | **green** — replay and retention fixtures |
| Seat replica, applier, outbox, settlement, overlay | retain | `crates/seat` (wave 2 lane D2) | **green** — offline write, reconnect, settle, proved by the simulation |
| App query handlers and action projections | replace | `crates/apps/{kit,tally,photos,docs,notes,people,locker,agenda,tasks}` — **all eight bundled apps** | **green**, by parity fixtures generated from v0's own handlers and proved in both trees |
| App UIs, desktop | retain | `desktop/renderer` over the **local JSON channel**, not protobuf-es | **partial** — the screens ship over the same named reads and the parity claim is kept by comparing the fold field by field; mounting `app-inline` is deferred with its reason ([D-1020-F16](../docs/decisions.md#wave-3-lane-rulings-1020)) |
| App UIs, mobile | replace | `mobile/shared` + Compose + SwiftUI: the navigation model (Home as the floor, covers, band destinations as parameters) and **three** screen machines | **open, and larger than the plan said** — v0 has ~40 shell screens and 72 app screens; Home is a placeholder with three links in both shells. Needs a device **and** a follow-up umbrella |
| Pairing, device enrollment, allowlist, revocation | retain | `crates/net` + `crates/vault` | **partial** — pair by QR is proved; `centraid devices list\|revoke` are the two verbs still owed and exit 3 naming their lane |
| Sharing: grants, circles, closure, projection, peer plane | retain | `crates/vault` + `crates/apps/docs`'s nine-window share fold | **partial** — the fold and the audience projection are green; the gateway-to-gateway peer plane is not built |
| Media pipeline | retain | `crates/media` + `crates/apps/photos` | **partial** — hashing, derivatives, duplicates and Places are green on fixtures; camera-roll backup needs a device |
| Media transfer, resumable upload ledger | replace | iroh-blobs in `crates/net` | **open** — the iOS policy is hand-off 7.1 |
| Full-text and vector search, recurrence | retain | `crates/search` (FTS5) + `crates/vault::time` | **partial** — the FTS door is green and Locker's absence from it is structural; recurrence is **one** engine in three call shapes with a 244-case rrule corpus and a DST corpus in six zones chosen to break a naive check. **sqlite-vec is not wired** (`grep` over `crates/**` for `sqlite-vec` and `vec0` finds nothing), and the FTS door walks **none** of v0's four consent walls yet, which it says in its README rather than stubbing them |
| Assistant plane | retain | `crates/assist` | **green on the corpus** — attended and unattended turns, delegate, artifact capture, the #842 corpus as its own gate step; `vault_sql` over the socket is hand-off 5.3 |
| Automations | retain | `crates/automations` | **green** — scheduled run, steering, anchor resolution, two tiers where v0 has three |
| Connectors and OAuth assist | retain | **nothing** | **not started** — owner's back burner, 2026-09-12. The one inventory row with no v1 half |
| Recognition | replace | `crates/automations` + `crates/media::models` | **partial** — the gateway half and the tier vocabulary are green behind a `Model` trait; five real sessions are hand-offs 4.1–4.5 and the device half is named scope, not built |
| Civil time and cron timezone | retain | `crates/automations` | **green** — two tiers, the host's zone **unreachable** rather than merely unused |
| Admin operations | replace | `centraid` subcommands as commands | **partial** — backup, doctor, recover, export, gateway install run; devices does not |
| Notifications and push wake | replace | **nothing** | **not started** — wave 4's notifications lane did not run; nothing in the core depends on it, which is what makes the ruling cheap to reverse |
| Browser extension Companion | replace | `extension/` + `centraid native-host` | **green** — eighteen methods, a real browser, no third-party code |
| OS autofill and share extensions | retain | **nothing** | **not started** — the rule is ruled, the targets need a device toolchain |
| Desktop service install | new | `centraid gateway install` + `deploy/` units | **partial** — writes a unit, never enables it; three-OS install is wave 5 |
| Backup, restore, recovery kit, `recover` | retain | `crates/vault::{backup,custody}` + `centraid recover` | **green** — the restore drill, and a **v0-made** recovery kit still opens in Rust ([D-1020-R3](../docs/decisions.md#wave-2-lane-rulings-1020)) |
| Diagnostics, logs, support bundle | retain | `tracing` in the core | **partial** — diagnostics go to stderr and nothing else does; there is no support bundle yet |
| Web PWA, worker seat, WASM transport, OPFS | retire | — | prepared for deletion |
| Expo, EAS, expo-updates, Metro, Hermes traps | retire | — | prepared for deletion |

### The numbers

| | |
| --- | --- |
| Workspace members | **25** — 16 under `crates/` plus 9 app crates named individually, because `crates/*` also matches `crates/apps` and a glob would hide the count |
| Rust | **173,803 lines** — 136,146 under `src/`, 37,535 under `tests/` |
| Protobuf | 1,712 lines across two packages with two promises |
| Kotlin and Swift | 8,319 lines in `mobile/` |
| TypeScript in the v1 trees | 21,007 lines — `desktop/`, `extension/` and `contracts/tools/`, the last of which retires with v0 |
| `contracts/` | 208 files, 78 of them JSON fixtures |
| The command registry | **156** typed commands across nine schemas — `core`, `tally`, `media`, `knowledge`, `people`, `social`, `locker`, `enrich` and `schedule` |
| The C ABI | **five** exported symbols, ten contract clauses, one test each |
| Down-only ledgers | 3 files under `contracts/ledgers/`, and the five `cargo xtask gate` checks hold 20 sections against the merge-base on every profile |
| `cargo test --workspace` | **1,947 passed, 0 failed, 6 ignored** on the umbrella tip |
| The v0 tree the deletion would remove | **1,046,086 lines across 5,395 files**, measured as the retirement commit's own diffstat |

### Exit list

Every command run at the worktree root on this branch, in this order, with its outcome.

```sh
cargo xtask rules             # FAIL — 2 findings, both INHERITED: sql-confinement on
                              #        crates/apps/photos/tests/{parity,year3}.rs, landed by
                              #        c24cc561 in wave 4 and lane CL-FIX's to fix. 205 files
                              #        scanned, 142 in the allowed crates
cargo xtask gate --profile local
                              # FAIL on `rules` ONLY, and only on those two. fmt ok (1.9s),
                              #        clippy ok (33.1s), test ok (282.1s), ledgers ok — 5 ledgers
                              #        hold against e9a7d81a. Tree COLD: 317.4s of the 3200s
                              #        coldLocalProfileSeconds ceiling, so the warm 120s budget was
                              #        not the number scored. Run in a PRIVATE target directory
                              #        (CARGO_TARGET_DIR=/home/user/cargo-target-lane-cldocs), which
                              #        is the only kind of run whose verdict is worth anything
cargo test --workspace        # PASS — 1,825 passed, 0 failed, 4 ignored at the pre-Schedule tip;
                              #        1,947 passed, 0 failed, 6 ignored after the rebase onto
                              #        d675987e, which is lane Schedule's own number
node scripts/lint-journey-ledger.mjs
                              # PASS — "every entry names its volume, hardware, spans and consumers";
                              #        run after each of the two journeys commits
node scripts/check-ledgers.mjs
                              # PASS — 20 sections across 5 ledgers hold against origin/main
node scripts/security/lint-ci-egress.mjs
                              # 1 problem — lane-release-extension.yml, the inherited red now written
                              #        up in docs/release.md. Not fixed here: it is a workflow file
bash .governance/run.sh       # PASS — all 10 directive(s) passed, on every commit
node .governance/law/run.mjs --door window --brief-digest 53be88c22ab5
                              # PASS — 10 rule(s), no findings, on the DEFAULT baseline (the
                              #        merge-base with origin/main, e9a7d81a) — the baseline CI
                              #        judges and the one doc-integrity freezes receipts against
bun install                   # needed for the format and static steps; node_modules removed after
bun run format:check          # PASS — "All matched files use the correct format". The only
                              #        oxfmt-owned file this lane touched is tests/journeys.json,
                              #        and it was edited as TEXT rather than reserialised: a
...
node .governance/law/run.mjs --front-page <path>
                              ...
                              #        between (`cmp` prints nothing)
a link sweep over every relative link this lane wrote
                              # PASS — 0 missing files and 0 missing anchors across the 20 docs
                              #        touched, resolved against the GitHub heading-slug rule.
                              #        One PRE-EXISTING broken anchor was found and fixed on the
                              #        way: docs/dev-environment.md pointed at
                              #        TESTING.md#the-four-ledgers-915-wave-4, and the heading is
                              #        `### The ledgers (#915 Wave 4, #927)`
git push -u origin claude/1020-cldocs
                              # PASS — no SKIP_*, no --no-verify on any commit of either branch
```

No gate, ledger direction, budget, allowlist, lint config or law path was touched by this lane. No waiver was spent and no docket row was filed. The five law-estate files under `tests/` were not edited on either branch.

### The prepared retirement (wave 6)

**`claude/1020-retire-v0`, one commit, head `b318fba1`, pushed and NOT merged.** Branched from the umbrella head `d675987e`.

**5,395 files, 1,046,086 deletions, 87 insertions.** What goes: `apps/**` whole; `packages/**` except `design` and `test-kit`; every `contracts/tools` generator that ran v0's real handlers, because they retire with the tree that made them answerable while the fixtures they produced stay as the frozen oracle; `.github/workflows/ci.yml`, whose v0 jobs retire with the tree they gated; and v0's test suites under `tests/{agent-e2e-*,perf,scale,quality,helpers,integration-mobile,design-gallery}` with the tsconfig that named them. Four supersession rows in `docs/decisions.md` land in the same commit.

**The gate on it, measured rather than predicted:**

| | |
| --- | --- |
| `bun run check:push:static` | **4/4** |
| `cargo xtask gate --profile local` | **FAIL** — `fmt`, `clippy` and `ledgers` pass; `rules` fails on the two inherited Photos findings; `test` fails |
| `cargo test --workspace --no-fail-fast` | **12 failed, 1,935 passed, 6 ignored.** `--no-fail-fast` matters: cargo stops at the first failing target, and the first number the plain run gave was **three** |

**The twelve, and why they are the finding rather than a fix list.** Every one is a test whose subject is *that the two trees agree* — the one class of test that has to go **with** the tree rather than be repaired:

- `crates/apps/locker`, three in the lib and four in the parity suite: `online_only_is_exactly_v0s_five`, `the_manifest_differs_from_v0_by_exactly_the_two_dead_fields`, `the_promoted_spec_is_byte_identical_to_the_extensions`, `every_shelf_declares_the_order_v0_declares`, `locker_has_no_demo_seed_and_the_manifest_says_so`, `only_the_live_shelf_carries_the_connector_alias_in_v0`, `v0s_candidate_list_never_reports_a_one_time_code`.
- `crates/apps/photos`, two: `the_contracts_sample_directory_is_the_v0_roll`, `the_bundle_is_v0s_own_answers_and_says_how_many`.
- `crates/media`, one: `models::tests::the_committed_v0_manifest_parses_and_names_its_capabilities`.
- `crates/ontology`, one, and this one is a **finding about a claim**: `every_golden_corpus_is_byte_identical_in_both_trees` fails. `golden.rs`'s own doc comment says the v0 path "exists until wave 6 deletes the v0 tree; **a caller must handle its absence**", and lane A's receipt says the test "handles the wave-6 absence rather than failing". It does not. The library function is fine; the test is not.

**Three things the branch proves that reading could not.**

1. **Wave 6 is not one commit, and the issue's plan says it is.** *"Retire — atomic: delete the v0 tree and the retired tools"* cannot hold, because the gate that would admit the commit is part of what it deletes: the pre-push gate is `check:push:static`, whose `typecheck:affected` ended in `tsc -p tests` over v0's own test tree. Making the branch pushable meant retiring that tail **with its input set** — its `include` globs now resolve to nothing, which is what tsc itself says — and that is a tool retiring with v0, not a check being dropped. Any narrower deletion is unpushable without `SKIP_*` or `--no-verify`, neither of which was used.
2. **`packages/design` surviving is not enough, and the cone is three packages deep.** `contracts/tools/export-design-corpus.ts` reaches `packages/blueprints/apps/tally/format.ts` for `figureTone`, which reaches `packages/core`'s money module and its own `types.ts`; and `packages/design`'s own tests import `@centraid/test-kit`'s fake clock, so a **fourth** package survives beside it rather than 32 runnable tests being deleted to make a deletion tidy. The owner decision is to port four functions — `toNativeTheme`, `partyHueValue`, `identityInitials`, `figureTone` — so Rust and Kotlin are the source, or to keep that cone alive for one generator.
3. **160 relative links across 190 markdown files point at deleted paths**, 16 of them in `docs/decisions.md`. Acceptance criterion 11 asks for none. The supersession row says why they are kept rather than rewritten — a path citation under a deleted tree is a citation to history, the way frozen receipts cite files they outlived — but the sweep is doc work the merge owes, and it is larger than it looks.

**What the branch deliberately does not do**, each for a stated reason: the five ledgers under `tests/` are **law estate**, so the advisory rows the plan would move out of `tests/inventory.json` need owner review and `estate-separation` forbids a commit that edits law and territory together; `tests/journeys.json` keeps every v0 entry including those whose consumer path this commit deletes, because retiring a ceiling is an owner-reviewed act under its own `approvedDeviation` and a lane that silently dropped rows to make a lint green would be doing the thing that lint exists to catch; and `scripts/**` keeps its v0 halves, because `lint-path-filters.mjs` reads the `ci.yml` this commit deletes and the path-filter ledger is a governed register.

**And what gates the merge**: the nine open acceptance boxes above, every one of them open on a physical device or a host that is not a container. `cargo xtask gate --profile nightly` would additionally need its `v0-oracle` step removed — that step runs v0's own suites against the `contracts/` fixtures, and there would be no v0 to run.

### Findings from this lane

1. **`crates/ontology`'s wave-6 absence handling is a claim its test does not keep.** Above. One line in `tests/fixtures.rs`; named rather than fixed here because `crates/**` is lane CL-FIX's this slot.
2. **Two rulings share the id `D-1020-X3`** — wave 3's close ruled open question 10 under it, and wave 4's extension lane used it for the staging chunk protocol. Disambiguated in `docs/decisions.md` rather than renumbered, because a ruling id that moves is a citation that breaks.
3. **`crates/centraid/README.md` described a build three waves old**: `seat`, `native-host` and five later surfaces were listed as exiting 3. A README that says a shipped verb is unbuilt is worse than no README, because the exit-3 rule is what a reader trusts it for. Fixed.
4. **`docs/dev-environment.md` carried a broken anchor** into `TESTING.md`. Fixed on the way past.
5. **The census was wrong six times**, each corrected in `docs/decisions.md` with the number now taken from the code: eighteen Companion methods not seventeen; nine share windows not eight; the `core` idempotency split summing to 25 of 27 where the `once` arm is eight; four confirm-gated `schedule` commands not three; Places a facet of Photos rather than an app; and one sharing read denying independently in `queries/person.ts`, not three.
6. **`lint:ci-egress` is red on a release workflow** — `lane-release-extension.yml` installs and runs dependency code with no `harden-runner` step and no ledger entry. Written up in `docs/release.md` and filed as hand-off 6.4; the remedy is the step, not a ledger row, and it is a workflow file rather than this lane's.
7. **The `local` profile is over budget on the merged tree and the trend is three lanes old.** Not touched. It is hand-off 6.5, and the remedy the issue already anticipates is scoping `local`'s `test` step to the changed crates and leaving the workspace run to `pr`.
...
The two riskiest claims in this lane's diff, the throwaway check run against each, and the result.

**Claim 1: "`tests/journeys.json` gains numbers and gates nothing."** The risk is the exact one this ledger exists to prevent — a row that reads as documentation and is quietly a ceiling, or a formatting round trip that re-flows the file and hides a real change inside 800 lines of noise. Two checks. First, the ratchet's own reader: `scripts/test-report/ratchet-floors.mjs` skips any key whose name starts with `_` at three separate call sites (`:92`, `:158`, `:373`), which is why every observed number is `_observedMs` / `_observedRows` / `_observedInstanceCap` and why `status: "projected"` carries no bare ceiling. Second, the diff itself: the first attempt reserialised the file with `json.dump` and produced **805 insertions and 178 deletions** for seven entries — oxfmt keeps short arrays on one line and Python does not. Reverted and redone as a text splice: **184 insertions, 3 deletions**, the three being the two lines that gained a trailing comma and the `approvedDeviation` string that records the addition. An additions-only claim that a formatter can falsify is not an additions-only claim.

**Claim 2: "the retirement branch's gate is red on twelve tests and no more."** The risk is that the number is an artifact of when the run stopped rather than a count of what is broken — which is precisely what happened on the first attempt. `cargo xtask gate --profile local` reported `test` failing with *"to rerun pass `-p centraid-apps-locker --lib`"*, and taking that at face value would have put **three** in this receipt. Checked by re-running `cargo test --workspace --no-fail-fast`: **twelve**, in four crates rather than one, including the `crates/ontology` failure that is the most interesting of them and that the first run never reached. The lesson is small and worth writing down: **cargo's default fail-fast makes a failure count a lower bound**, and a receipt that quotes one without `--no-fail-fast` is quoting where the run stopped.

## Close — lane CL-FIX: the wave 4 findings fixed at source, the two CI holes closed, and the ladder's second rung

Fifteen items from the wave 4 receipts, R-1020-35's mandate and the owner's close-pass list. Each row names its commit (post-rebase), the red that proved it, and what moved.

### The items

| # | Item | Commit | The red | What moved |
| --- | --- | --- | --- | --- |
| 1 | **`media.forget_person` erased everyone's confirmed faces in a one-member vault** | `f567829a9` (v0), `1e62fa5ef` (port) | `forgetting the only member clears their judgements and erases nobody else's face` — `regions_forgotten: 2` before the fix, Ana's and Sam's faces gone | Delete on `party_id` alone; the judgement is cleared in place, and `review_state` goes back to `proposed` because the schema pins the two to each other (`CHECK ((review_state = 'confirmed') = (confirmed_by_party_id IS NOT NULL))` — so "null the column" alone is unwritable). `contracts/apps/photos` unchanged: the corpus forgets a party whose only confirmed region is their own, which is the case the fixture does not have (finding below). |
| 2 | **Tally labelled a foreign-currency expense in base money** | `c6373b9ce` (v0), `b46e5fce9` (port + fixture) | `labels a foreign-currency expense in the money it was settled in` — `rate_suggestions` was `[]`, a ¥ dinner read as `original_currency: GBP` | `tally.dashboard.expenses` selected 13 columns and `ledgerRow` reads 8 more. All 8 selected on both sides; `ledger_row`, the export fold and `rate_suggestions` read them with v0's own fallbacks. **50 answers moved in `contracts/apps/tally/queries.json`**: `original_currency`/`settlement_currency` `GBP`→`JPY`/`EUR` on the two foreign expenses, and `rate_*` from `null` to the identity rate on every `export` case (v0's `?? 1_000_000 / 6 / "identity" / spent_on`, which the port had hardcoded and now reads). |
| 3 | **`shared_from.at` was `Date.parse(…) \|\| 0` → 1970** | `4265ef1d8` (v0), `ee5c08ad4` (port) | `says nothing rather than 1970 when the arrival instant cannot be read` | `at` is `number \| null`; the port already modelled the absence, and its parity mapping lowers `None` to `null` instead of `0`. Fixture unchanged — the corpus's `subscribed_at` always parses, which is why this was latent. |
| 4 | **The audit band as catalog entities (D-1020-CL1)** | — | — | **NOT DONE.** See "Not done" below. |
| 5 | **`autofill-candidates` never reported a one-time code** | `540a585a9` (v0 + door), `ee5c08ad4` (port), `115d5334f` (wire) | `answers IS NOT NULL, under a name the projection gives it` + `takes an output alias as a name and not as a column`; `reduces sealed login rows to secret-free candidates` went red on the stub | The paged door (both trees) now takes an output alias as a name and permits a sealed column **as the operand of `IS [NOT] NULL` and nowhere else**; Locker projects `otp_seed IS NOT NULL AS has_totp`. `crates/core::api::page` keyed the row image by the literal select entry, so the bit would have crossed the wire as NULL — keyed by the entry's output name now. Two Locker answers move to `has_totp: true`; the seed is on no projection and in no answer, asserted. |
| 6 | **Tally year-3 finance/revision rows + a seeded pairing case** | — | — | **NOT DONE.** See "Not done" below. |
| 7 | **Two Photos test files held SQL** (the standing `rules` red) | `ebbcff0d0` | `sql-confinement` 2 findings → `205 file(s) scanned, clean` | 18 statements became named acts in `centraid_apps_kit::fixtures` (`trash_asset`, `confirm_face_region`, `stamp_phash_cluster`, `live_asset_count`, …). `rules` is green on `local` and `pr`. |
| 8 | **`copy/`, `design/`, `mobile/` claimed by no `changes` filter** | `b48744307` | `lint:path-filters` 3 problems → clean; the new step's **first run found `copy/tasks.json` still carrying Tally's root-shelf route id** (PE-F7's artefact, never regenerated) | A new `emitters` step on the `pr` profile re-runs the copy, design-corpus and native-theme emitters, formats, and asserts `git diff --exit-code -- copy design mobile`; all three paths ledgered in `tests/path-filter-ledger.json` with the job that covers them. `ci-policy` green. |
| 9 | **The warm `local` profile** | `9693524fa` | — | Measured, not raised. **Before 150.7 s** (fmt 2.0 / clippy 0.6 / **test 147.8** / rules 0.3 / ledgers 0.0) — the whole overrun is `test`, whose summed execution is 132.9 s over **100 test binaries**, the two largest being `crates/sim/tests/seeds.rs` (22.9 s, 3 tests) and `crates/apps/notes/tests/year3.rs` (21.6 s, 5 tests). `local`'s step excludes `centraid-sim`, which `pr` runs **twice** (its own unexcluded `--workspace`, and the `sim` step at 25 seeds), so no merge-gating profile loses a second. **After 145.3 s** (test 141.4). Summed execution 132.9 → 114.4 s. **Still over 120 s — an owner question, not a raise.** |
| 10 | **The three `pending:` marks** | `5f0c3c0f8` | — | All four lists are already `[&str; 0]`; three asserted it and the native host's did not (its loop passes over an empty method table too). One assertion added. Notes' `send-to-tasks` and the automations recipes were retired by their own lanes. |
| 11 | **`lint:ci-egress` red on `lane-release-extension.yml`** | `31752e9df` | `lint:ci-egress` 1 problem → `7 workflow(s) enforce an egress policy` | `step-security/harden-runner` in `audit` mode on both jobs. **No ledger row**, and the brief's expectation of one is wrong on the mechanism: `egress-ledger.json` holds only workflows with *no* step, and a row beside a hardened workflow fails the gate as stale. |
| 12 | **`Value::to_string` is not a canonical sort key (PE-F8)** | `118bf79dc` | `the text does not depend on the order the keys arrived in`, `a row hash reaches inside a value` | `centraid_apps_kit::canonical_json` — keys sorted by UTF-16 code unit at every depth, ECMAScript number spelling. People's local copy deleted in favour of it; `automations::row_hash` was sorting only its top level and printing nested objects in insertion order, so a standing match re-fired whenever the build's feature set changed. The sweep found two other canonicalisers (`crates/vault::intents`, `crates/media::format`); both stay, each now says which job it is for. |
| 13 | **`contracts/migrations/002_revisions.sql` on the ladder** | `68c594413` | `the_proposal_is_not_on_the_ladder_yet` failed the moment `LADDER` grew | Rung two. The suite winds a file **back** to rung one to seed the malformed chain and watch the backfill check refuse, which exercises the real upgrade path rather than a hand-applied file; `baseline.rs` states the four guards as the whole of the delta from the v0 corpus; `head_version()` is 2 and a newer file is still `DowngradeRefused`. The drill's `an_upgrade_that_fails_mid_way_restores_from_its_pre_migration_snapshot` is green. |
| 14 | **`PENDING_COMMANDS` empty** | `5f0c3c0f8` | — | Verified: all four empty, all four now asserted. |
| 15 | **`every_golden_corpus_is_byte_identical_in_both_trees` fails when v0 is absent** | `b17596877` | Ran with `packages/` moved aside: `only 0 file pair(s) were compared` | Both corpus gates print a named `SKIPPED` line and return when `packages/vault/tests/golden` is gone; a label missing *while the tree is still there* is a finding rather than a skip. |
| + | **SCH-F1 — `core_event.rrule_support` had one writer and it was not the app** | `08f072be9` | `an event's rule carries the engine's own verdict, on write and on edit` | `propose_event` and `edit_event` compute it from the three-shape parser in both trees. **Agenda's corpus regenerated: five query answers and one row moved `supported` → `unsupported`**, all the same `FREQ=MONTHLY;BYSETPOS=-1` event. |
| + | **R-1020-35's `recurrence_tz` writer half** | `50a6b9d23` | `carries the row's own sort order through, never a reset zero` | Not dead weight — a live bug. `anchorWrite` sent `recurrence_tz` and `schedule.organize_task` declares `tz` with `additionalProperties: false`, so **every anchor change from the Tasks detail sheet was refused**. Five call sites and the row shape follow the column; the vault-side test pins both spellings. |
...
| `cargo fmt --all --check` | clean |
| `cargo clippy --workspace --all-targets -- -D warnings` | clean |
| `cargo test --workspace` | **1,953 passed, 0 failed, 6 ignored** |
| `cargo xtask gate --profile local` | 145.3 s against 120 s — every step green, `rules` clean (205 files), `ledgers` hold; over budget, see item 9 |
| `cargo xtask gate --profile pr --lane ci-policy` | PASS — `lint:workflow-pins, lint:ci-egress, lint:path-filters + actionlint` |
| `cargo xtask gate --profile pr --lane emitters` | PASS on a clean tree (7.9 s) |
| `bun run check:push:static` | **4/4 gates passed in 254.4 s** — lint, turbo:lint, format:check, typecheck:affected |
| generators, idempotent | `agenda` regenerated twice, byte-identical; the eight parity bundles re-run under `CENTRAID_WRITE_CONTRACTS=1` and only `locker` + `tally` + `agenda` moved |
| v0 oracle, both trees | the eight `tests/quality/*-parity.contract.test.ts` pass against the committed bundles |
| `bunx vitest run packages/vault packages/blueprints` | 9,312 passed / 4 failed before this lane, **3 of the 4 inherited** (below) and the fourth fixed here |

### Not done, and what exists in the tree

- **Item 4 — the audit band as catalog entities (D-1020-CL1).** Not landed. What the work needs, from reading it: `access.provenance` and `access.receipt` added to `packages/vault/src/schema/entity-catalog.ts`'s `access` block and to `contracts/schema/v0-registries.json` (96 → 98 entities, which moves `manifest-scope-denial.sweep.test.ts`'s declared-scope count), a `read`-only verb and an `object_type` row filter, and then Docs' `docs.activity.provenance` and Locker's `access` answer through the real gateway. **The brief's "never replicated" premise does not match the tree**: both tables are already in `REPLICATED_TABLE_NAMES` (`packages/vault/src/schema/private-tables.ts:134`) and already in `localTables` as "never exported", so the ruling needs restating against that before it is landed. Left whole rather than half-applied.
- **Item 6 — the year-3 Tally axis and the `matches` pairing case.** Not landed. `kit::fixtures::year3_tally` still writes no `core_transaction` and no `core_entity_revision`, so two of the ten measured view numbers are round because the plane is empty, and `matches` still answers `[]` in both trees.
- **SCH-F5 — v0's Agenda/Tasks demo seeds call `Date.now()` outside the fixture clock.** The generators neutralise it (`normaliseHostClock`); the seeds are still host-dependent for anyone running v0 directly.

### Findings

- **Three v0 law suites are red at the umbrella head and are not this lane's**: `packages/blueprints/src/one-computation.test.ts` (`adds no seat-local duplicate BODY, renamed or not`), `pending-projection-tripwire.test.ts` (`every destructive action on the eight apps projects a delete or a tombstone`) and `photos-vocabulary.test.ts` (`shows the storage noun only in reviewed ownership copy`). Confirmed inherited by stashing every `packages/**` edit of this lane and re-running: the same three fail. The Docs lane's finding 7 named two *different* suites, both of which now pass, so this is a second, newer set.
- **`contracts/apps/photos/` has no one-member-forget case.** The corpus forgets a party whose only confirmed region is their own, so the fixture did not move when the bug was fixed — the proof is the two unit tests instead. The Photos lane's finding 5 (no trashed-asset case) is the same gap in the same bundle.
- **The `emitters` step earned its place on its first run**, finding a committed `copy/tasks.json` that no emitter had produced since the Notes lane fixed the root-shelf id. Nothing regenerated `copy/` on any per-PR profile before this.
- **`crates/apps/kit::fixtures` is now 5,400 lines.** The Photos script is the fourth axis in it; the honest next move is one module per app under `fixtures/`, which is a rename and no behaviour.

### Decisions — lane CL-FIX

Each cites [#1020](https://github.com/srikanth235/centraid/issues/1020) and adopts a recommendation under R-1020-34.

- **D-1020-CL2 — forgetting a member clears their judgements; it does not erase the faces they judged.** Options: (a) keep both columns; (b) delete on `party_id` and null the judge; (c) two commands. **(b) adopted**, with the correction the schema forced: `confirmed_by_party_id` cannot be nulled alone, so `review_state` returns to `proposed` and the region is a proposal again.
- **D-1020-CL3 — a projection carries the columns its fold reads.** The alternative (drop the eight fields from the payload) would change what every Tally surface shows. Adopted: select them, keep v0's fallbacks.
- **D-1020-CL4 — an unreadable instant is `null`, never the epoch.**
- **D-1020-CL5 — presence is not plaintext.** A sealed column may be tested for null and may not be read; an output alias is a name, not a column. Both rules land in the v0 door, the kit's grammar and the wire's row keying. The alternative — projecting `otp_seed` and stripping it afterwards — is what the Locker lane said must not happen.
- **D-1020-CL6 — an app crate's tests script through the kit.** No second `sql-confinement` exemption.
- **D-1020-CL7 — a generated tree is regenerated by the profile that gates a merge.** Options: a `changes` filter in `ci.yml` (which no longer listens on `pull_request`); an always-on ledger row with no job behind it; a `pr` step. The third, because it is the only one that exercises anything.
- **D-1020-CL8 — one canonical JSON for the app plane**, and each of the other two canonicalisers says which job it is for rather than being merged into it.
- **D-1020-CL9 — `local` sheds `centraid-sim`; `pr` runs it twice.** Stated in the profile table, not inside a test.
- **D-1020-CL1 — not taken.** The audit band's registration is item 4 and is left to the owner with the replication correction above.

### Owner questions, consolidated

The wave 4 lanes' product questions are collected with their recommendations in [`docs/release/v1-handoffs.md`](../docs/release/v1-handoffs.md) (lane CL-DOCS). This lane adds two:

1. **The warm `local` profile is 145.3 s against 120 s** after item 9. The split is in the table above: `test` is 141.4 s of it, over 100 test binaries. Options: (a) merge each crate's integration tests into one `tests/main.rs` (one link instead of twenty; a mechanical refactor across 25 crates, no coverage change); (b) move the year-3 axes to `nightly` with a `pr` step that asserts they ran; (c) re-base the budget on measurement. **Recommendation: (a), then re-measure** — (c) is the one the doctrine forbids, and the budget was already re-based once under D-1020-B2.
2. **Should `contracts/apps/photos` grow the two cases it lacks** — a one-member forget and a trashed asset that is not purged? Both move every id in the bundle, which is why neither lane did it. **Recommendation: yes, once, in the same regeneration.**

### Doctrine digest

Law `53be88c22ab5`. `amendment-pairing` · `commit-message-format` · `constitution-coverage` · `doc-integrity` · `doctrine-citation` · `estate-separation` · `managed-tree-integrity` · `receipt-per-issue` · `registry-completeness` · `waiver-docket`. No waiver spent; no `SKIP_*`, no `--no-verify`; no gate, budget, ledger, test or allowlist weakened — the `local` overrun is reported rather than re-budgeted, the `sql-confinement` rule gained no exemption, and the sealed-column rule was narrowed to a presence test rather than opened. No generated fixture was hand-edited: every one moved through its generator. No model identifier in any file. This section is appended last, after lane CL-DOCS's.

## Close

The umbrella's last section. Everything above it is what happened; this is what the tree is, what it is not, and what the next person has to do.

**What v1 is, after six waves and two cross-lane fixes.** One Rust core in **25 crates** and 173,803 lines, running as either the vault's authority or a full replica, with **1,947 tests** green on the tip. One binary with twelve verbs, of which two — `devices list` and `devices revoke` — still exit 3 naming their lane. A five-symbol C ABI with ten contract clauses, each with its own test and one of them proved against a real panic. Two protobuf packages with two different promises, and a wire whose unknown-field invariant is held at the frame because the codec cannot hold it. A deterministic simulation that runs 25 seeds on every pull request and 250 nightly, over a real vault, a real applier, a real outbox and a real file — and which found two bugs nobody was looking for. **All eight bundled apps** as crates whose parity is generated by executing v0 and proved in both trees. A Kotlin Multiplatform shell with three screens. An Electron seat that plays and seeks a video whose bytes are still arriving. A browser Companion that bundles no third-party code at all. A gateway that opens no listening TCP port, checked in the source and again in `/proc/net/tcp`.

**What it is not.** Not on a phone: every one of the six absolute mobile targets is parked, both reference devices are named and neither has been run, and the mobile screen surface is three screens against v0's forty shell screens and seventy-two app screens — a row the plan never costed, now filed as its own proposal ([#1022](https://github.com/srikanth235/centraid/issues/1022)), gated on the iOS hand-off. Not on a rented host: the VPS smoke runs in a container and the system unit has never been loaded by a real systemd. Not connected: the connectors lane never ran, by the owner's own ruling, so one capability-inventory row has no v1 half. Not shipped: no store build, no signing key, no notarisation. And **not deleted**: v0 is still here, still the oracle, still the thing every fixture is answerable against.

**The nine open acceptance boxes are open on two things and no third.** A physical device, or a host that is not this container. Not one is open on code nobody wrote, and that is the claim this receipt is for.

**What the next person does, in order.**

1. **Repoint branch protection** at `gate` and `dependency-review`. Until that happens every pull request blocks on a required check that no longer reports — and it has been true since wave 1.
2. **Stand up the device runner** — a self-hosted macOS runner with the named iPhone and the named Android attached, `vars.CENTRAID_DEVICE_RUNNER=true`. Four cells are waiting and they **fail** rather than skip on a runner that claims the label with no phone.
3. **Make Swift buildable**, which unblocks the iOS build, the Swift fixture test that has never run, the transport experiment that decides two rulings, and the screen surface the plan never budgeted.
4. **Answer the questions**, which are consolidated with their options, a recommendation and the test that changes with each answer in [docs/release/v1-handoffs.md](../docs/release/v1-handoffs.md#the-owner-questions).
5. **Then, and only then, wave 6.** `claude/1020-retire-v0` is pushed and unmerged, and what it proves is that the deletion is not one commit: the gate that would admit it is part of what it deletes, and `crates/design` pins a three-package cone of v0 that one Rust port would cut.
