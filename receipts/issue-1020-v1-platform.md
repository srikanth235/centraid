...
**`c548f58f` — `fix(xtask)`: the repository root is resolved at run time.**
...
**`2fe4622e` — `fix(vault)`: ids are minted from the clock.**
...
**`a8ca93e5` — `fix(core)`: the wire error carries an owner-facing sentence.**
...
**`fe8f75e0` — `feat(core)`: the handshake carries the artifact identity.**
...
**`b303e8fe` — `feat(core-ffi)`: a debug-fault door for clause 9.**
...
**`d18bc8a8` — `build(gate)`: `mobile-jvm` is real, the device lanes have bodies.**
...
**`4cfd811e` — `docs(decisions)`: `D-1020-X3`.** `docs/decisions.md`, one dated
block, the OQ10 evidence row and the ruling. Summarised under Decisions below.

**`a157dd5d` — `fix(xtask)`: what has been built is read from `CARGO_TARGET_DIR`.**
...
| `SIM_SEEDS=25 cargo test -p centraid-sim` | 6 suites, **0 failing**; `the_determinism_seed_replays_byte_for_byte` and `a_seed_run_twice_reaches_the_same_state` both green, which is the check `2fe4622e` had to survive |
...
| `bun run format:check` | `All matched files use the correct format` over 5,949 files. It first found `crates/xtask/README.md` unformatted; the fix is amended into `a157dd5d` rather than left as a trailing commit |
...
green, so the count **is** five and `b303e8fe` did not add a sixth symbol; what
...
- **`c548f58f`** — `crates/xtask/tests/repo_root.rs` runs the **already
  ...
  list.
- **`2fe4622e`** — `the_first_write_after_a_reopen_does_not_collide_with_the_first_session`
...
- **`a8ca93e5`** — `the_owner_facing_sentence_carries_no_database_text_and_the_detail_still_does`
  (`crates/core/src/error.rs`). Red while `Error.sentence` did not exist and the
  screen's only text was `detail`.
- **`b303e8fe`** — its first run found `centraid_next_event` answering `TIMEOUT`
...
- **`a157dd5d`** — the red is a line from this lane's own exit list. Straight
...
1. **A shared `CARGO_TARGET_DIR` swaps compiled artifacts between worktrees of
   ...
   (`c548f58f` working as designed) while applying **its** rule definitions. The
...
2. **`measure`'s two cold keys were measuring a warm build.**
   ...
   **down-only** ledger as a first build. Fixed in `a157dd5d`. No ledger row was
