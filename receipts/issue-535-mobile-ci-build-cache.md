...
- **skip first-run onboarding after a clearState launch so Android reaches Home** (`tests/agent-e2e-mobile/lib/first-run.mjs`, `tests/agent-e2e-mobile/lib/harness.mjs`, `tests/agent-e2e-mobile/flows/home-loads.mjs`, `tests/agent-e2e-mobile/README.md`). With the app finally loading (iroh AAR), run 30090178855 rendered — but the "Welcome to Centraid" **onboarding** screen (`src/screens/Onboarding.tsx`), not Home: `App.tsx` gates the tab shell on `profile.onboarded`, which is AsyncStorage-backed and therefore wiped by `launchApp: {clearState: true}`, so every fresh Android launch starts in onboarding. iOS never surfaced this — its simulator persists the flag across the clearState the flows use, so it was already past onboarding. Added a shared `SKIP_ONBOARDING` snippet — a conditional `runFlow: when visible "Skip onboarding" → tapOn` (the onboarding header button, which calls `setOnboarded(true)` and drops straight to Home) — and inserted it right after the `clearState: true` launch in `home-loads` and in the harness `configureGateway` helper. The two dismissal snippets (`SKIP_ONBOARDING` + the existing `DISMISS_KEYBOARD_ONBOARDING`) were extracted into a new `lib/first-run.mjs`, mirroring the earlier `metro.mjs` split, so `harness.mjs` stays under the 500-line repo-hygiene cap. Conditional, so it is a no-op when a launch keeps state or the device is already onboarded (iOS). This also removes the 120s stuck-wait that let the emulator's Pixel Launcher ANR under the software GPU. `lint:e2e-flows` green (no vacuous assertions). This is a flow-only change — it does not move the native fingerprint, so the banked apk warm-hits and the re-run skips the ~34m gradle build.
