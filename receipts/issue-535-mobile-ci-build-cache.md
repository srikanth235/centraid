...
- [x] suppress Android ANR system dialogs so a background launcher ANR can't block Maestro
- [x] AVD snapshot caching + right-sized emulator (4 GB / 4 cores)
...
- **suppress Android ANR system dialogs so a background launcher ANR can't block Maestro** (`apps/mobile/scripts/android-emulator-e2e.sh`). Intermittently the emulator's Pixel Launcher ANRs while our app is foregrounded (software GPU under Metro+app load) and Android pops a "Pixel Launcher isn't responding" **system dialog OVER the app**. Maestro then queries that system window — which has no app content — so `visible`/`tapOn` against the app fail even though the app rendered correctly: run 30094372690 reached Home cleanly, run 30095300728 failed to see the onboarding "Skip" with the ANR dialog covering a screen that (per the debug screenshot) plainly showed it. Added `adb shell settings put global hide_error_dialogs 1` after the emulator boots and before Maestro runs, which lets a background ANR resolve silently instead of stealing the window. Not a native-build input, so the fingerprint and the warm cache-hit are unaffected. **Verified green: run 30096004831 passed `home-loads` end-to-end in 56s** — Skip visible → tapped, "YOUR APPS" + "Connect your computer" + "Pair desktop" all asserted — the first fully-green Android e2e run.
- **AVD snapshot caching + right-sized emulator (4 GB / 4 cores)** (`.github/workflows/e2e.yml`) — the researched GitHub-Actions best practice for Android e2e, on top of an already-green lane. Two `reactivecircus/android-emulator-runner` steps in the documented pattern: a `Create AVD + snapshot` step (cache-miss only) boots a clean emulator once and banks a booted-state snapshot into `~/.android/avd`, cached via `actions/cache` keyed on every AVD-shaping input (`avd-api34-x86_64-google_apis-pixel6-r4096-c4-v1`); the test step then boots with `-no-snapshot-save`, resuming from that snapshot in seconds instead of a ~40s cold boot. Beyond speed, resuming an already-settled launcher is the structural cure for the first-boot Pixel Launcher ANR race (`hide_error_dialogs` is the belt, snapshot-boot the suspenders). Both steps set `ram-size: 4096M` + `cores: 4` (the runner is 16 GB / 4-vCPU) so the app has real headroom under Metro + app load. This is the highest-value Android-lane change short of a managed device cloud — which was rejected as an external service against the repo's self-host posture. Arm64 hosted runners were also rejected: GitHub's ARM Linux runners still do not expose `/dev/kvm` (2026), so `ubuntu-latest` x86_64 + KVM remains the only accelerated hosted path, which is exactly why the x86_64-carrying iroh AAR was the right unlock. iOS needs no analogous change — macOS runners run simulators natively (no virtualization), which is why that lane was reliable from the start. **Verified: run 30096879423 (a cold AVD-cache miss) generated the snapshot, and the test step then booted from it — `Successfully loaded snapshot 'default_boot' using 1330 ms` — versus the ~40s cold boot, and `home-loads` passed in 47s.** Two consecutive green Android runs (30096004831 without the AVD cache, 30096879423 with it) confirm the lane is stable, not a one-off.
...
| claude-code-955653fc-da5-1784898718-1 | claude-code | 955653fc-da50-425f-95f2-bc71a62f0f63 | #535 | claude-opus-4-8 | 46 | 45605 | 13250019 | 27283 | 72934 | 7.5923 | 2534 | 8630547 | 303212463 | 1922010 |  |
| claude-code-955653fc-da5-1784899291-1 | claude-code | 955653fc-da50-425f-95f2-bc71a62f0f63 | #535 | claude-opus-4-8 | 54 | 1618164 | 14343576 | 58909 | 1677127 | 18.7583 | 2588 | 10248711 | 317556039 | 1980919 |  |
| claude-code-955653fc-da5-1784899350-1 | claude-code | 955653fc-da50-425f-95f2-bc71a62f0f63 | #535 | claude-opus-4-8 | 6 | 11100 | 1782207 | 1920 | 13026 | 1.0085 | 2594 | 10259811 | 319338246 | 1982839 |  |
| claude-code-955653fc-da5-1784899396-1 | claude-code | 955653fc-da50-425f-95f2-bc71a62f0f63 | #535 | claude-opus-4-8 | 1 | 853 | 597769 | 267 | 1121 | 0.3109 | 2595 | 10260664 | 319936015 | 1983106 |  |
| claude-code-955653fc-da5-1784899454-1 | claude-code | 955653fc-da50-425f-95f2-bc71a62f0f63 | #535 | claude-opus-4-8 | 8 | 1998 | 2394864 | 2493 | 4499 | 1.2723 | 2603 | 10262662 | 322330879 | 1985599 |  |
| claude-code-955653fc-da5-1784899529-1 | claude-code | 955653fc-da50-425f-95f2-bc71a62f0f63 | #535 | claude-opus-4-8 | 6 | 9390 | 1799604 | 3726 | 13122 | 1.0517 | 2609 | 10272052 | 324130483 | 1989325 |  |
| claude-code-955653fc-da5-1784899978-1 | claude-code | 955653fc-da50-425f-95f2-bc71a62f0f63 | #535 | claude-opus-4-8 | 23 | 13385 | 7282132 | 12589 | 25997 | 4.0396 | 2632 | 10285437 | 331412615 | 2001914 |  |
