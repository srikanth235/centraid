...
## Lane 1 — the outbox cannot be destroyed: a kill anywhere in a re-bootstrap keeps the queue

### What landed

| Commit | What |
| --- | --- |
| `4ac92e43` | `packages/client/src/replica/seat/carry-over.ts`, `worker-core.ts`, `node-staging.ts`, `opfs-staging.ts`, `seat-worker.ts`, `apps/mobile/src/lib/replica/expo-seat-staging.ts`, `native-seat.ts`, `tests/integration-mobile/lib/node-seat.ts`, `docs/mobile-offline.md` — the carry-over sidecar, its exactly-once marker, and the three hosts that write it. |
| `62dc0425` | `packages/client/src/replica/seat/applier.ts`, `seat-loop.ts`, `watermark.ts`, `docs/mobile-offline.md` — a deferred span stops the cursor instead of stepping over it; schema is never deferred. |
| `a2201079` | `packages/client/src/replica/seat/applier.ts`, `seat-intent-store.ts`, `worker-core.ts`, `packages/client/src/replica/outbox-mirror.ts`, `shell-session.ts` — the cleared-overlay news moves after COMMIT, the journal is pruned on the path that settles almost everything, and the mirror is keyed and invalidated. |
| `4ccd7435` | `packages/client/src/replica/seat/seat-intent-store.ts`, `carry-over.ts`, `outbox.ts`, `apps/mobile/src/lib/replica/pending-content-refs.ts`, `native-session.ts` — `needs_blobs_json` gets a writer; the phone's byte protection gets a vault. |
| `5eb7128d` | `packages/client/src/replica/seat/seat-intent-store.ts`, `seat-sync-loop.ts`, `apps/mobile/src/lib/coalesce.ts`, `apps/mobile/src/kit/replica/ReplicaProvider.tsx`, new `kit/replica/revoke-scope.ts` — one write lock, an awaited teardown, a real cancel, a closable loop. |
| `c2847481` | new `apps/mobile/src/lib/replica/seat-lease.ts` + `seat-lease-held-error.ts`, `native-seat.ts`, `background-sync.ts`, `kit/replica/replica-seat-mount.ts`, `lib/upload/native-queue.ts`, `lib/upload/boot.ts`, `docs/mobile-offline.md` — the seat lease and the shared upload handle. |
| `b7de1ac9` | `packages/server/src/serve/vault-plane.ts`, `build-gateway.ts`, `packages/server/src/cli/device-admin.ts`, `SECURITY.md` — `retireReplicaDevice`, and the outcome ledger's first real sweep. |
| `2acfeb07` | `packages/client/src/replica/seat/session-seat.ts` — the opened file's own cursor, before the first sync. |
| `9c86e0a8` | new `packages/client/src/replica/revoked-outbox.ts` + `opfs-revoked-outbox.ts` + `apps/mobile/src/lib/replica/revoked-outbox-file.ts` + `seat/seat-authorization-revoked-error.ts`, `shell-session-purge.ts`, `shell-session.ts`, `shell-session-scopes.ts`, `seat-loop.ts`, `seat-sync-loop.ts`, `apps/mobile/src/lib/replica/native-session.ts`, `native-session-open.ts` (new), `native-session-types.ts`, `kit/replica/revoke-scope.ts`, `replica-status.ts`, `replica-context.ts`, `ReplicaProvider.tsx`, `SECURITY.md`, `docs/mobile-offline.md` — one revocation policy on both hosts. |
| `74ae001a` | `packages/server/src/serve/gateway-schema.ts`, new `share-placement-attempts.ts`, `packages/server/src/routes/placement-routes.ts`, `docs/protocol.md` — the attempt row, and the origin's release after the receipt. |
| `b2144fe5` | new `tests/integration-mobile/seat-swap-faults.integration.test.ts`, `lib/node-seat.ts`, `packages/client/src/replica/seat/worker-core.ts`, `carry-over.test.ts`, `TESTING.md`, `tests/integration-mobile/README.md` — the fault lane, and the failed-bootstrap re-adopt it found. |
| `6ab381de` | `apps/mobile/src/lib/replica/delivery-triggers.ts`, `native-session.ts`, `native-session-open.ts`, `packages/client/src/replica/native.ts`, `docs/mobile-offline.md` — the phone consumes the cleared overlays lane A left wired. |

**C5/T6 — the queue is on disk for the whole swap.** `readSeatCarryOver` → `driver.close()` → download → `writeSeatCarryOver` held the outbox in a JS heap object across a multi-megabyte download, while `rebootstrap-copy.ts` told the member "your unsent changes stay queued"; on the phone `install()` deletes the destination before it moves the new file in, so a kill anywhere in that window left the intents nowhere. The stash goes to `<seat>.carry-over.json` before the handle is released and is dropped only after the write-back has COMMITTED. The replay is exactly-once by a token stamped in `seat_carry_over` inside that same transaction — without it a crash between the write-back and the clear would resurrect intents the seat had since drained. A sidecar for another vault is discarded (R-1014-11 read from this end, [decisions](../docs/decisions.md#replication-offline-and-sharing-1014)), and it goes with the file on purge.

**C1/C2 — a deferred span is owed, not skipped.** The applier set `cursor = group.lastSeq` on the skip branch, so rule 3 dropped those rows on every later delivery and no back-fill path existed: a 60-row import above the threshold was permanently absent from a seat reporting `behind: 0`. The cursor stays before the owed commit, the back-fill is the ordinary tail, `deferred_from` clears in the transaction that applies the span, `SeatLoop.sync` ends the pass rather than re-asking for the same page, and the watermark line carries the distance beside the wifi sentence. A commit carrying a `ddl` row is never deferred — the column it adds is what every later row binds against, and the SQLite error that produced was not a `SeatDriftError`, so the loop rethrew instead of re-bootstrapping.

**C10/C9/C11 — what is written inside the transaction and what is told after it.** `onCleared` fired before COMMIT, so a shell listener that threw rolled back an applied log page and a not-yet-durable fact was published; `seatOverlayClearingHook` is two halves now and the applier calls the second outside its own `try`. `clearSeatOverlaysAtCommit` prunes the settled journal like `settle()` does — it is the path that settles almost everything on such a seat. And the in-transaction `DELETE FROM seat_outbox` never reached the outbox mirror's proxy, so the mirror served settled intents as pending forever; `settleCleared` invalidates by store on both hosts, and `pending(states)` caches per states asked for.

**C6/C7 — the pending-bytes protection had neither data nor a vault.** `needs_blobs_json` was bound to a literal `null` by the only write path, so `shasPendingIntentsNeed()` answered `[]` on every seat that ever ran. It carries what the intent's input names now (`namedRowIds` — the same reading the chain and the phone's protection use) and is renamed `contentPendingIntentsNeed`. The phone's set was a module-global across vaults: with Personal and Family both mounted — R25's own arrangement ([decisions](../docs/decisions.md#replication-offline-and-sharing-1014)) — vault B's queue move made vault A's captured-but-unsent photograph evictable. It is a map keyed by vault, the predicate is the union, and the registry is withdrawn when the last seat closes.

**P1/C8/P12/P16/P17/P21 — one owner per handle.** expo-sqlite caches connections by database NAME, so the background pass's second `SeatWorkerCore` was the foreground's handle and its `finally` closed it. The pass asks for `useNewConnection: true` AND takes a lease — a `<seat>.lease.json` sidecar read before SQLite is touched, because a pass that has to open the file to learn it may not open the file has already done the damage — and skips a live vault as `skippedLive`. `SeatIntentStore.add/transition/settle` run inside `BEGIN IMMEDIATE` with the read inside, and `created_order` is minted under that lock. `ReplicaProvider` teardown parks an awaited close the next mount waits on. `coalesceWork.cancel()` aborts the in-flight run through an `AbortSignal` the run reads. `SeatSyncLoop.close()` ends the trailing follow-up. The upload database is one reference-counted handle in `native-queue.ts`; five entry points used to open and close it independently and the first one out closed it under the rest, with `withDrainLock` serialising only drains.

**C25/P24/X7/X8/X1 — one revocation policy, and it exports first** (R-1014-12, [decisions](../docs/decisions.md#replication-offline-and-sharing-1014)). Whatever the gateway never durably took is written beside the seat file as `revoked-outbox-<vaultId>.json` after the drain is quiesced and before anything is deleted, and the count comes back: the phone says "N unsent changes were saved to this phone", the browser logs it. A host with nowhere to write still reports the count; a failed export never abandons the purge. The seat's own log door raises `SeatAuthorizationRevokedError` on a 401 or a 403 naming `replica_device_not_enrolled`, so a read-only seat that never touches the drain purges too, and the phone stops answering a refusal with `requireBootstrap()`. On the gateway, `retireReplicaDevice` keeps `replica_intent_outcome` and its invocation-commit markers and deletes only the parked payloads — a re-paired phone's replays are deduped rather than executed twice — which makes the ledger's `expires_at` load-bearing, so the vault plane's sweep runs `pruneReplicaIntentOutcomes` (its first production caller anywhere) held at the slowest seat's cursor.

**V3 — a placement is bracketed, and the release is last.** `share_placement_attempts` records the act and its parameters before the first vault write and is dropped when every step it brackets is durable; a retry resumes and a retry that re-addresses the id answers `409 placement_id_reused`. The crash-injection test found that releasing the origin before the receipt was unrecoverable: the origin was emptied with nothing recorded and the retry's `readShareClosure` refused the items the first attempt had removed, so the outbox retried `placement_failed` forever. Released after the receipt, the window is the item in both vaults, which the retry resolves — so a receipt with an attempt row beside it does not short-circuit.

**R1 ([decisions](../docs/decisions.md#replication-offline-and-sharing-1014)) — the file's own cursor.** `SessionSeat` took its watermark only from `sync()`, so between opening a filled seat and its first catch-up the drain's ack-after-delta sweep was skipped and an intent whose commit the cursor had already passed stayed parked. Seeded from the handle, never `#filled`.

### Verification

- `bun run --cwd packages/vault typecheck` · `packages/core` · `packages/client` · `packages/server` · `apps/mobile` → PASS (exit 0), all five.
- `grep -rn "useNewConnection: true" apps/mobile/src --include=*.ts | grep -v test` → PASS: `background-sync.ts:217` is the background pass; `native-seat.ts:159` is the option it threads and `:177` the `.incoming` handle from C17; `expo-seat-driver.ts:56` and `expo-sqlite-driver.ts:90` are the driver options themselves.
- `grep -rn "deleteReplicaIntentOutcomesForDevice" packages --include=*.ts | grep -v "test\|dist"` → PASS: only `packages/vault/src/replica/intents.ts:512` (the definition) and `packages/vault/src/index.ts:717` (its export). No production caller from revoke — `vault-plane.ts` calls `retireReplicaDevice` instead. The function is left in place because `packages/vault/src/replica/**` belonged to another lane in flight; deleting it is owed to whichever wave owns that file next.
- `bun run --cwd packages/client build && bun run --cwd packages/server build && bun run --cwd packages/vault build` → PASS (exit 0).
- `flock /tmp/centraid-suite.lock bun run --cwd packages/client test` → PASS: 277 files, 2523 tests green.
- `flock /tmp/centraid-suite.lock bun run --cwd apps/mobile test` → 2419 green; two inherited failures, both reproduced with this lane's diff stashed: `src/lib/replica/expo-seat-driver.test.ts` (`RolldownError: Flow is not supported` parsing `node_modules/react-native/index.js` — a file that has never run in this environment) and `scripts/verify-native-state.test.mjs` ("generated trees are untracked", the android/ios trees committed by `af9ceac6`).
- `flock /tmp/centraid-suite.lock bun run --cwd packages/server test` → 3554 green; three inherited failures, `gateway-db-lock.integration.test.ts` and two `src/acp/backends/acp/launch.test.ts` root/`IS_SANDBOX` cases, all three reproduced with this lane's diff stashed.
- `flock /tmp/centraid-suite.lock bun run test:integration:mobile` → PASS: 16 files, 84 tests green, including the new fault lane.
- Red first, as asked. With the sidecar write disabled (`worker-core.ts`), `seat-swap-faults.integration.test.ts` → `× keeps the queued intent when the phone dies 'after the install, before the write-back' — AssertionError: expected [] to have a length of 1` and `× and the recovered intent still drains, once`; green with it. The same revert reds `carry-over.test.ts`'s `keeps the queue when the process dies between install and write-back`.
- `bun run format && bun run check:push:static` → PASS: `4/4 gates passed in 97.8s`.
- `node .governance/law/run.mjs` → PASS for this lane: `0 error(s), 7 warning(s)`, and every warning names another lane's commit (`a332d01f`, `bd7cdf85`, `d9265b8e`) or another lane's receipt section (lines 208, 442). None names a Lane 1 commit.

### What I did not do, and why

- **The `share_placement` intent record in the ORIGIN vault, which the brief's step 7 asks for, is not there.** It would be a new table in `packages/vault/src/schema/**`, and the brief forbids this lane those files. The property it was for is served instead by a gateway-side attempt row plus the reordering: each vault step is already idempotent by id (`INSERT OR IGNORE` under `share_authority_live_answer`, dedupe-by-content on the projection, a delete for the release), so what was actually missing was a record that the act had BEGUN and an order in which a retry can finish it. Both are now tested by crash injection at each step.
- **`retireReplicaDevice` lives on the vault plane, not in `packages/vault/src/replica/intents.ts`.** Same reason: that file was another lane's. The three tables it touches (`replica_intent_outcome`, `replica_invocation_commit`, `replica_parked_payload`) are all on `local-tables.ts`, so this is not a replicated write and needs no bracketing; `bracketed-replica-writes` passes on it.
- **The browser's unsent-export count reaches a console line, not a surface.** The phone has a revoked-notice store to hang the sentence on and now does; the web shell has no equivalent, so `shell-session-scopes.ts` logs it with the file name. A member-facing surface for it is a shell change this lane did not have a design for.
- **`shasPendingIntentsNeed` was renamed rather than kept as an alias.** `grep -rn "shasPendingIntentsNeed" --include=*.ts . | grep -v node_modules | grep -v "/dist/"` → only the comment in `carry-over.ts` recording the old name. It had no caller outside its own test.

### Found, not mine

- **`packages/client/src/replica/seat/blob-presence.ts` has no production writer at all.** `grep -rn "recordSeatBlob\|claimSeatBlob" --include=*.ts . | grep -v node_modules | grep -v "/dist/"` finds only `carry-over.test.ts` and `bytes.test.ts` beside the definitions. The whole `seat_blob_presence` table — pins, `captured_here`, `claim_intent_id`, `purge_seq` — is carried faithfully across a re-bootstrap and written by nothing, so R25's byte policy rests on it in name only. C6's sibling, one table over.
- **`bootstrapSeatFile` rejects a snapshot that moves mid-download and nothing retries it.** `SeatSnapshotMovedError` is correct (splicing two artifacts is worse), but `SeatLoop.sync()` re-bootstraps once per sync only for a `SeatDriftError`, so a busy gateway — and lane A's note says this one is never quiescent — turns a moved artifact into a failed catch-up the host must notice. The new fault lane has to retry it by hand for exactly this reason.
- **`grantPlacementAuthority` (`packages/vault/src/grant/grant-authority.ts:29`) writes `share_authority` with no `beginReplicaCommit`/`endReplicaCommit` around it,** unlike every other writer of that table. It is reached from `placeItemsInVault`, so a placement's authority rows land outside a bracketed pair.
- **`GatewayDatabase` has no column-migration path.** `gateway-schema.ts` is `CREATE TABLE IF NOT EXISTS` only, so a column added to an existing table would silently not exist on an installed gateway. That is why V3's progress record is a new table rather than a `status` column on `share_access_receipts`; the constraint deserves to be stated somewhere.
- **`apps/mobile/src/lib/replica/native-session.ts` and `kit/replica/ReplicaProvider.tsx` are both within a handful of lines of the 625-line cap,** and this lane spent two extractions (`kit/replica/revoke-scope.ts`, `lib/replica/native-session-open.ts`) getting back under it. The next change to either will need a third.

### Docs touched

[`docs/mobile-offline.md`](../docs/mobile-offline.md) (the sidecar under "A re-bootstrap keeps the queue", the deferred-span row in the debug table, the lease and the shared upload handle under "Background work", scoped revocation's export and count, and the cleared overlays on the change sink), [`docs/protocol.md`](../docs/protocol.md) (placement's attempt row and the release-after-receipt order), [`SECURITY.md`](../SECURITY.md) (the revocation paragraph: what the gateway keeps, what it deletes, and the export both hosts make first), [`TESTING.md`](../TESTING.md) and [`tests/integration-mobile/README.md`](../tests/integration-mobile/README.md) (the fault lanes).
