...
- `apps/mobile/package.json` + `bun.lock` — add `@expo/metro-runtime@57.0.13` as a devDependency of the mobile app (optional peer of `expo`, previously absent from the lockfile entirely); `bun add` also refreshed a few in-range transitive resolutions (`@expo/log-box` 57.0.2→57.0.3, pretty-format re-nesting). Mobile typecheck + tests verified green locally after the refresh.
...
- Second CI iteration (run 32947705439) with the Maestro 2.6.1 pin still red, but the Metro request log caught the real signature: every flow launch bundles a **1-module stub** (`Android Bundled 74ms apps/mobile/index.ts (1 module)`) instead of the 2049-module graph the prewarm fetch builds, immediately preceded by `@expo/metro-runtime not found`. The dev client therefore boots a stub and every assert times out on a blank app. `@expo/metro-runtime` is an optional peer of `expo` and is absent from the lockfile; added as a devDependency of `apps/mobile` (Expo's own monorepo guidance), plus a CI manifest probe printing the `launchAsset` so a stub-entry manifest is caught directly if the dep alone is not the fix. iOS is out of scope for #870.
