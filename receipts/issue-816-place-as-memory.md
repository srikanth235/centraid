...
- **The mobile-first and basemap rulings (Expo primary; real maps on by default via MapKit/`expo-maps` on iOS and MapLibre + OpenFreeMap on Android; no API keys anywhere; `react-native-maps` removed; web sketch-only) are recorded as supersessions in `docs/decisions.md`, and every wave's exit was demonstrated on the Expo app before or with the web surface.**
  → Wave 0 recorded all three rulings in `docs/decisions.md` (P-ladder, P-gazetteer, P-mobile-first, P-cartography, P-shared, P-egress) plus three supersession pointers; every wave's exit ran on the mobile suite, and the ui-impact screenshot is published by the mobile e2e flow.
- **`react-native-maps` is gone from `apps/mobile/package.json` and native state (or the finishing recipe is recorded and handed off), and the `knip.json` / `native-fingerprint.mjs` workarounds it required are retired with it.**
  → Wave 0. The dependency, the `knip.json` ignore and the `RNMapsDefines.h` exclusion in `native-fingerprint.mjs` went together; the macOS finishing recipe is in Decisions below.
- **No surface in Photos renders a raw coordinate except the explicit "copy exact location" action.**
  → Wave 1's ladder, enforced on every surface added since — `readableName` on headings and stamps, `placePhrase` everywhere else, and a title-sweep test in search and trips.
- **The lightbox location row shows a phrase from the ladder and links only in-app; the openstreetmap.org link is gone.**
  → Wave 1 removed the row and `ExifRow.href` with it; the panel's mini map is the existing in-app projection.
- **An unnamed place near a member-named place renders relative to it ("3.4 km NE of Home" style) on both surfaces.**
  → Wave 1's relative rung, with Home preferred as the anchor inside 25 km; asserted in `place-phrase.test.ts` and rendered by both info panels.
- **"Name this place?" flow exists, and a given name updates every surface that phrases that place.**
  → Wave 2's `PlaceNaming` prompt plus `media.name_place`; because every surface phrases through the ladder rather than a stored string, one naming re-phrases all of them with no backfill.
- **Gazetteer automation is opt-in, off by default, fully offline, and never overwrites a member name; its decision supersession is recorded in `docs/decisions.md`.**
  → Wave 3. It ships `enabled: false`, the lookup is a table in the bundle, and `media.set_place_gazetteer` cannot reach `core_place.name`.
- **Place search matches member names, gazetteer names, and relative phrases; unlocated photos have a browsable bucket.**
  → Wave 4's place vocabulary on both surfaces, plus the `no-location` bucket under one reserved key.
- **The seeded roll produces at least one auto-named Trip card with an offline route-sketch cover.**
  → Wave 5's `trips.ts`; the cover is drawn from the offline `place-map.ts` projection so a card needs no network.
- **Shared/exported photos carry place metadata only by explicit choice at a chosen precision, and never a Home-relative phrase.**
  → Wave 6b: three rungs, `none` by default, and a real EXIF strip below the exact rung; the Home-relative rung is suppressed and that is asserted of the call sites.
- **`docs/photos/places.md` describes the new current state (two-mode cartography included, egress disclosure included); the QUALITY.md dead-weight item is closed; one receipt at `receipts/issue-816-place-as-memory.md`.**
  → Wave 6a and this commit: the doc is rewritten around five layers, QUALITY.md's item moved to Resolved in Wave 0, and this file is the one receipt.
...
The invariant is now provable at the **call sites**, which is the part a test of `place-phrase.ts` alone can never discharge. `share-place-call-sites.test.ts` enforces two total rules with no allowlist over both Photos trees: every `placePhrase` call names its context, and exactly one module may hand a photograph to an OS share API. The first rule was demonstrated red — it named the two implicit callers, `search-groups.ts` and `search-place-vocabulary.ts`, which now say `context: "private"` explicitly with a comment saying why a member's own search results may keep the relative rung. `photo-share.test.ts` drives the real path with the seams mocked and asserts the payload the OS receives, including that the private ladder *would* have said "3.5 km NE of Home" for the same input — so the suppression is asserted against the thing it suppresses. `PhotoLightbox.tsx` kept its size (891 → 899 lines) because the two verbs and the sheet moved out into `viewer-export.ts` and `PhotoShareChoice.tsx`.
