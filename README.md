...
- **Live data, no plumbing**: every action invalidates the tables it touched and subscribers re-read — bundled apps render inline in the shell and refresh off the device replica; served apps (builder preview) tail SSE on `/centraid/<id>/_changes`.
