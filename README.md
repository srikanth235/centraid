...
- **Smoke:** path-filtered CI builds the image and probes it with a mounted `/data` (`scripts/gateway-package/smoke.ts --base-url …`). Host-side: `bun run gateway:package:smoke`.
