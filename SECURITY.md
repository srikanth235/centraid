...
**Credential issuance is iroh pairing-only.** There is no durable gateway bearer, per-device HTTP token, direct-URL pairing route, or wildcard admin plane — and no password/session/OIDC plane by design ([#599](https://github.com/srikanth235/centraid/issues/599); [docs/decisions.md](docs/decisions.md)): authentication is the transport, and identity-proofing for a new household member is an owner handing them a ticket. The zero→one founding ceremony consumes a host-possession ticket, auto-creates the owner **member**, and binds the proved first EndpointId to it with the `admin` role; ordinary pairing binds a device to its member, and the device inherits that member's `(member, vault)` roles (`admin`/`write`/`read`). Every **vault-scoped** request resolves a real enrollment in one vault and fails closed without a proved device identity. Two surfaces are deliberately outside that rule (issue #568 item C): `GET /centraid/_gateway/info` is public so a client can read the version/schema handshake before it can pair — it answers version, capabilities, and status to anyone, and withholds the iroh dial ticket from an unauthenticated caller; and a **fresh** gateway (no vault yet) answers a closed allowlist of gateway-scope prefixes — `_gateway/health`, `_logs`, `_templates`, `_agents` — because the founding window admits an unenrolled EndpointId by construction. Everything else 409s `uninitialized` until a vault exists. Revocation removes the enrollment, cascades its web sessions in SQLite, and makes the QUIC admission check refuse the device. The filesystem-anchored CLI remains the recovery path if the sole owner is lost.
