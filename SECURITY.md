...
| **Handler sandbox** (#842 W7.1, #846 P9) | [`packages/server/src/engine/sandbox/`](packages/server/src/engine/sandbox/), installed **unconditionally** by the engine and automation worker runners | A handler reaching `node:fs`, sockets, subprocesses, `process.env`, `process.binding`/`dlopen`, or rebuilding the loader through `node:module`. An automation handler that declares no `sandbox.lane` gets the strict floor — there is no no-sandbox path left | An OS boundary — see the honest limits below |
