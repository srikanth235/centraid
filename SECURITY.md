...
**Compression happens before encryption, and that leaks a size class** ([#1029](https://github.com/srikanth235/centraid/issues/1029) §4, W13 finding 22). A `base` range and a page `segment` are zstd'd against a trained dictionary and only then sealed, so a sealed object's length is a function of how _compressible_ its plaintext was and not only of how long it was. Anyone who can see that length — the gateway holding the object, or an observer counting bytes on the wire — learns something about the content that a fixed-size envelope would not tell them: a segment of repetitive page structure is small, one carrying high-entropy bytes is not. This is the shape of the CRIME/BREACH family of attacks, and it is a deliberate trade: without the dictionary a one-row edit's 4 KiB page segment does not compress at all, and a backup nobody can afford to take is not a security property either.

What is done about it is **Padmé** (`crates/media/src/object/pad.rs`): every object's plaintext is padded to a bucket whose width grows with its size, which caps the overhead at about 12% while collapsing many distinguishable lengths into few classes. **Padmé bounds that leak; it does not remove it.** Two objects in different buckets are still distinguishable, and an adversary who can get plaintext of their choosing sealed — an imported file, an inbound message — and watch which bucket the result lands in still learns about what was compressed alongside it. The design claims exactly what it delivers, and no more: object sizes are bounded, not exact, and nothing here makes the size of a sealed object secret.
