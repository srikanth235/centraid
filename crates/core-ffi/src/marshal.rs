...
pub fn config_from_json(bytes: &[u8]) -> Result<CoreConfig, CoreError> {
    ...
    // NO `role` KEY (#1029 §1). A shell used to name one — `gateway`,
    // `seat-replicated`, `seat-thin` — and the core branched on it. The phone
    // is the only host that opens a vault (#1029 §6), so there is one kind of
    // core and a `role` a shell still sent would be a word nothing reads. It
    // is IGNORED rather than refused: an older shell against a newer core is
    // exactly the version-skew case the handshake exists for, and refusing the
    // open would turn a stale key into a product that will not start.
    ...
    // (#1029 §1). A shell opening a vault a restore is about to write passes
    // `create: false` and is answered "there is nothing here" rather than
    // being handed a silently empty one.
...
mod tests {
    ...
    /// A CONFIGURATION IS A PATH AND A CHOICE ABOUT CREATING (#1029 §1).
    ///
    /// The enrolment record — the endpoint secret, the gateway address, the
    /// relay decision, the enrolled public key — is gone with the pairing it
    /// described, and so is `role`. What is left is what a phone actually
    /// needs to open its own vault.
    #[test]
    fn a_configuration_defaults_to_founding_and_an_explicit_choice_wins() {
        let founding = config_from_json(br#"{"path":"/tmp/v.db"}"#).expect("it parses");
        assert!(founding.create, "a shell that names a path founds a vault");
        assert!(
            !config_from_json(br#"{"path":"/tmp/v.db","create":false}"#)
                .expect("parses")
                .create,
            "a shell waiting on a restore must not be handed an empty vault"
        );
    }

    /// A `role` A SHELL STILL SENDS IS IGNORED, NOT REFUSED (#1029 §1).
    ///
    /// An older shell against a newer core is the version-skew case the
    /// handshake exists for, and refusing the open over a key nothing reads
    /// would turn a stale word into a product that will not start.
    #[test]
    fn a_role_left_over_from_an_older_shell_does_not_change_the_open() {
        let stale = config_from_json(br#"{"path":"/tmp/s.db","role":"seat-thin"}"#)
            .expect("an older shell's key is ignored");
        assert_eq!(stale.path, std::path::PathBuf::from("/tmp/s.db"));
        assert!(stale.create);
        assert!(
