...
| `representations` | What the bytes ARE, which is not a property of the bytes: `core_content_representation` keyed on the OWNER (`media.asset`), because two assets sharing one sha can read as two different things (#996 R20(b)). |
