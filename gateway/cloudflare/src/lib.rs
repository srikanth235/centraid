...
use worker::{
    Context, DurableObject, Env, Headers, Method, Request, Response, Result as WorkerResult, State,
...
fn authenticate(
    ...
    certificate.verify().map_err(|_| Refusal::SignatureInvalid)?;
...
impl VaultObject {
    ...
    async fn run_conformance(&self, forgetful: bool) -> WorkerResult<Response> {
        ...
        // asserts on.
        let mut body = report.render();
        body.push_str(if report.is_green() { "GREEN\n" } else { "RED\n" });
        Response::ok(body)
    }
...
impl DurableObject for MailboxObject {
    ...
    async fn alarm(&self) -> WorkerResult<Response> {
        ...
        // from.
        let Some(mailbox) = self.state.id().name().as_deref().and_then(|name| key_of(name)) else {
...
impl DurableObject for AccountObject {
    ...
    async fn fetch(&self, mut request: Request) -> WorkerResult<Response> {
        ...
        match (method, segments.get(2).map(String::as_str)) {
            ...
            (Method::Post, Some("receipt")) => self.verify_receipt(&store, account, &body, at).await,
            _ => Response::error("not found", 404),
        }
    }
}
...
impl DeclaredObject {
    fn parse(&self) -> Result<Declaration, Fault> {
        ...
        let checksum = hex::decode(&self.attested_checksum)
            .ok()
            .and_then(|bytes| centraid_gateway_core::checksum::AttestedChecksum::from_slice(&bytes))
