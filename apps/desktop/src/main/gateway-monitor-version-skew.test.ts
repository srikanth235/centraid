...
const fail = (at: number): GatewayProbe => ({ at, ok: false, detail: 'fetch failed' });
...
describe('applyProbe: version handshake', () => {
  it('never judges a local gateway — versionSkew stays undefined', () => {
    expect(run([ok(T0, { version: '9.9.9', schemaEpoch: 99 })]).versionSkew).toBeUndefined();
  });
  ...
  it('treats only protocol skew as unsafe and retains that verdict through outages', () => {
    ...
    expect(
      applyProbe(skewed, ok(T0 + 10_000, { version: undefined, schemaEpoch: undefined }))
        .versionSkew,
    ).toMatchObject({ skewed: true, gatewaySchemaEpoch: EXPECTED_SCHEMA_EPOCH + 1 });
  });
});
...
describe(applyVersionSkewAlert, () => {
  ...
  it('fires immediately for a protocol-skewed remote and de-dupes its episode', () => {
    ...
    expect(applyVersionSkewAlert(state, config, T0 + 60_000).action).toBeUndefined();
  });
  ...
  it('does not alert when disabled, matching, or local', () => {
    ...
    expect(
      applyVersionSkewAlert(run([ok(T0, { version: '9.9.9', schemaEpoch: 99 })]), config, T0)
        .action,
    ).toBeUndefined();
  });
  ...
  it('re-arms after a matching probe and fires again when the protocol diverges', () => {
    ...
    expect(applyVersionSkewAlert(state, config, T0 + 20_000).action).toStrictEqual({
      gatewayVersion: EXPECTED_GATEWAY_VERSION,
      gatewaySchemaEpoch: EXPECTED_SCHEMA_EPOCH + 1,
    });
  });
