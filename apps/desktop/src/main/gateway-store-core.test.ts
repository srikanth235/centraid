...
describe(normalizeProfile, () => {
  ...
  it("rejects id mismatch, bad kind, empty label, missing createdAt", () => {
    ...
    expect(
      normalizeProfile(endpointId, { ...base, kind: "cloud" as never })
    ).toBeUndefined();
    expect(
      normalizeProfile(endpointId, { ...base, label: "" })
    ).toBeUndefined();
    expect(
      normalizeProfile(endpointId, { ...base, createdAt: undefined })
    ).toBeUndefined();
...
describe(validateAddGatewayFields, () => {
  it("requires a label and valid EndpointId", () => {
    expect(
      validateAddGatewayFields({ label: "  ", endpointId: "gw-1" })
    ).toMatchObject({
      ok: false,
      code: "invalid_input",
    });
    expect(
      validateAddGatewayFields({ label: "G", endpointId: "" })
    ).toMatchObject({
      ok: false,
