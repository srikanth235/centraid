...
export async function openAppFolder(
  ...
  // string (it doesn't reject), so a bare `await` would swallow every failure
  // and the handler would always claim success. Surface it instead, so the
  // renderer's catch shows a real toast.
