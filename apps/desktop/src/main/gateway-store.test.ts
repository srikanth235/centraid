...
const fixture = vi.hoisted(() => ({
  ...
  closeDialer: vi.fn(),
  ensureProxy: vi.fn(async () => 'http://127.0.0.1:43123'),
}));
...
vi.mock('./gateway-secrets.js', () => ({ clearGatewayCredentials: fixture.clearCredentials }));
vi.mock('./iroh-dialer.js', () => ({
...
test('main owns one connection registry and renderer storage owns none of it', async () => {
  ...
  expect(profiles.map((profile) => profile.id)).toEqual(['local', endpointId]);

  const entries = await fs.readdir(path.dirname(fixture.file), { recursive: true });
  expect(entries).toEqual(['connections.json']);
  ...
  expect(Object.keys(localStorage).filter((key) => key.startsWith('centraid.v1.'))).toEqual([]);
});
...
test('remote-only add, use, and forget never creates the platform gateway directory', async () => {
  ...
  expect((await listGateways()).map((profile) => profile.id)).toEqual(['local']);
  await expect(fs.access(fixture.localDataDir)).rejects.toMatchObject({ code: 'ENOENT' });
});
