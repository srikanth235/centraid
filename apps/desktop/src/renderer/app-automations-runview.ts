...
export function createRunViewModule(ctx: ShellContext, ui: AutomationsUi): RunViewModule {
  ...
  function renderRunView(automationId: string, runId: string): void {
    ...
    // Phase 3 (#325): render via the ported React RunViewScreen when the bundle
    // is loaded. The vanilla side keeps the SSE stream + node model below and
    // pushes a derived snapshot to React on each event; `rerender` routes to
    // React when mounted, else rebuilds the vanilla DOM.
    let reactUpdate: ((s: RunViewSnapshot | null) => void) | null = null;
    const runBridge = window.CentraidReact;
    if (runBridge?.mountRunView) {
      scroll.replaceChildren();
      registerCleanup(
        runBridge.mountRunView(scroll, {
