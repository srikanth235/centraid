...
const KIND_LABEL: Record<StorageConnectionKindDTO, string> = { 'byo-s3': 'BYO S3', provider: 'Provider' };
...
function DriftLine({
  ...
  connection: StorageConnectionCardDTO;
}): JSX.Element | null {
  ...
  return (
    <div className={styles.driftLine} data-emphasis={drifted ? 'warn' : undefined} data-testid="drift-line">
...
function ConnectionPanel({ connection }: { connection: StorageConnectionCardDTO }): JSX.Element {
  const quotaBytes = connection.providerReported?.backup?.quotaBytes ?? connection.providerReported?.cas?.quotaBytes;
  ...
  return (
    <div className={styles.connectionPanel} data-testid="storage-connection-panel">
      ...
      {connection.fetchedAt ? (
        <div className={styles.fetchedAt}>provider figures as of {relativeWhen(connection.fetchedAt)}</div>
      ) : null}
...
export default function StorageCard({ now, loadStatus, onOpenSettings }: StorageCardProps): JSX.Element {
