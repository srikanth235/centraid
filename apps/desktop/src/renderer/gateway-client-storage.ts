...
export type CreateStorageConnectionInput = CreateByoS3ConnectionInput | CreateProviderConnectionInput;
...
export async function listStorageConnections(): Promise<StorageConnectionDTO[]> {
  ...
  const out = await readJson<{ connections: StorageConnectionDTO[] }>(res, 'list storage connections');
...
export async function createStorageConnection(
  ...
  const out = await readJson<{ connection: StorageConnectionDTO }>(res, 'create storage connection');
...
export async function updateStorageConnection(
  ...
  const out = await readJson<{ connection: StorageConnectionDTO }>(res, 'update storage connection');
  return out.connection;
}
...
export async function attachVaultStorageConnection(
  ...
  const out = await readJson<{ blob_store: BlobStoreSettingsDTO }>(res, 'attach storage connection');
  return out.blob_store;
}
