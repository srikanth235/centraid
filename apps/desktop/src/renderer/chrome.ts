...
(function () {
  ...
  interface SidebarOpts {
    ...
    apps: SidebarApp[];
    drafts: SidebarApp[];
    /** Element mounted at the top of the sidebar, above "Build new",
     *  followed by a hairline divider. Hosts the profile switcher. */
  ...
  function buildSidebar(opts: SidebarOpts): HTMLElement {
    ...
    // Sidebar-head slot. Renders ABOVE "Build new" because the profile
    // is the meta-context every other entry operates under (apps,
    // automations, settings — all profile-scoped). A hairline divider
    // below separates the profile-scope row from page-scope actions so
    // the hierarchy reads at a glance. app.ts mounts the profile
    // switcher head row here (window.Profiles).
    if (opts.headSlot) {
      wrap.append(opts.headSlot);
