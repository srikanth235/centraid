...
contextBridge.exposeInMainWorld("CentraidApi", {
  ...
  readProjectFiles: (input: { id: string }) =>
    ipcRenderer.invoke(Channel.PROJECTS_FILES, input),
  openProjectFolder: (input: { id: string }) =>
    ipcRenderer.invoke(Channel.PROJECTS_OPEN, input),
  deleteProject: (input: { id: string }) =>
    ipcRenderer.invoke(Channel.PROJECTS_DELETE, input),
  previewUrl: (input: { id: string }) =>
    ipcRenderer.invoke(Channel.PROJECTS_PREVIEW_URL, input),
  ...
  promptAgent: (input: { text: string }) =>
    ipcRenderer.invoke(Channel.AGENT_PROMPT, input),
  ...
  listVersions: (input: { id: string }) =>
    ipcRenderer.invoke(Channel.VERSIONS_LIST, input),
  activateVersion: (input: { id: string; versionId: string }) =>
    ipcRenderer.invoke(Channel.VERSIONS_ACTIVATE, input),
  appLiveUrl: (input: { id: string }) =>
    ipcRenderer.invoke(Channel.APP_LIVE_URL, input),
  deregisterApp: (input: { id: string }) =>
    ipcRenderer.invoke(Channel.APPS_DEREGISTER, input),
});
