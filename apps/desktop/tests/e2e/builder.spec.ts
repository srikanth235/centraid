...
test("4.1 + 4.2 — palette Create opens the builder and the initial turn streams a tool pill", async () => {
  ...
  try {
    ...
    await expect
      .poll(() =>
        gateway.calls.some(
          (c) => c.method === "POST" && c.pathname === "/centraid/_apps"
        )
      )
      .toBe(true);
      ...
      .poll(
        () =>
          gateway.calls.some(
            (c) =>
              c.method === "POST" &&
              /\/centraid\/_apps\/.*\/publish$/u.test(c.pathname)
          ),
        { timeout: 15_000 }
      )
      .toBe(true);
...
test("4.5 — a failed Publish surfaces an error and does not claim success", async () => {
  ...
  try {
    ...
    await expect
      .poll(() =>
        gateway.calls.some(
          (c) => c.method === "POST" && c.pathname.endsWith("/publish")
        )
      )
      .toBe(true);
...
test("4.3 — the builder Preview tab mounts the draft iframe", async () => {
  ...
  try {
    ...
    await expect(
      page
        .getByTestId("builder-right-pane")
        .locator("iframe[data-centraid-app]")
    ).toHaveCount(1, {
      timeout: 10_000,
    });
  } finally {
...
test("5.1 — Edit with Centraid opens the existing app in the builder", async () => {
  ...
  try {
    ...
    expect(
      gateway.calls.some(
        (c) => c.method === "POST" && c.pathname === "/centraid/_apps/_sessions"
      )
    ).toBe(true);
  } finally {
    await closeApp(app);
...
test("6.1 + 6.2 — switching to the Code tab lists files and opens one in the editor", async () => {
  ...
  try {
    ...
    await expect(
      page.locator(".code-tree-file", { hasText: "index.html" })
    ).toBeVisible({
      timeout: 10_000,
    });
    await page.locator(".code-tree-file", { hasText: "index.html" }).click();
    await expect(page.getByTestId("code-edit-pre").first()).toContainText(
      "Journal",
      {
        timeout: 10_000,
      }
...
test("6.6 — Cloud Logs renders entries and filters by level + search", async () => {
  ...
  try {
    ...
    await page
      .getByTestId("cloud-logs-chip")
      .filter({ hasText: /^All$/iu })
      .click();
    await page.getByTestId("cloud-logs-search").fill("cleanly");
    await expect(page.getByTestId("cloud-logs-row")).toHaveCount(1);
    await expect(page.getByTestId("cloud-logs-row")).toContainText(
      "started up"
    );
  } finally {
    await closeApp(app);
