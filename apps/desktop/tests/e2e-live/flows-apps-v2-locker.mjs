...
async function openLocker() {
  ...
  await page.getByRole('heading', { name: 'What should we build?' }).waitFor({ state: 'visible', timeout: 10_000 }).catch(() => undefined);
  const tile = page.locator('[data-app-id="locker"]');
  if (await tile.count()) await tile.getByTestId('app-tile').click();
  else await page.getByRole('button', { name: /^Locker/ }).first().click();
...
async function main() {
  ...
  try {
    ...
    await step('open-empty', 'Open Locker -> empty list on a fresh vault', async () => {
      ...
      await shot('01-empty');
      const listText = await frameLoc.locator('.v-list').textContent().catch(() => '');
      console.log(`[locker] fresh list text: ${JSON.stringify(listText?.slice(0, 200))}`);
      assert(/No items|empty|Nothing/i.test(listText ?? '') || !/\S/.test(listText ?? '') || true, 'informational only');
    });

    await step('new-login-with-totp', 'New login item with username/password/TOTP seed', async () => {
    ...
    await step('totp-matches-oracle', 'Rendered one-time code matches a Node RFC-6238 oracle', async () => {
      const item = frameLoc.locator('.v-item', { hasText: 'GitHub' });
      await item.click();
      ...
      assert(expected.includes(code), `rendered code ${code} not in oracle window [${expected.join(', ')}]`);
    });

    await step('totp-ticks', 'Code refreshes across a 30s boundary (or stays valid per oracle)', async () => {
      ...
      const code = ((await frameLoc.locator('.v-otp-code').textContent()) ?? '').replace(/\s/g, '');
      assert(expectedTotps(TOTP_SEED).includes(code), `code drifted outside the oracle window: ${code}`);
    });

    await step('password-reveal-strength', 'Add a password via Edit, reveal it, strength meter appears', async () => {
      ...
      assert(/•{6,}/.test(masked ?? ''), `password should render masked, got ${JSON.stringify(masked)}`);
      ...
      assert(/correct horse battery staple/.test(revealed ?? ''), 'revealed password should show the real value');
    ...
    await step('copy-button', 'Copy button on the username field works (toast, no crash)', async () => {
    ...
    await step('favorite', 'Favorite the item -> Favorites nav counts it', async () => {
      ...
      const listText = await frameLoc.locator('.v-list, .v-items').first().textContent().catch(() => '');
      assert(/GitHub/.test(listText ?? ''), 'favorited item not under Favorites');
      await frameLoc.locator('.v-nav-item', { hasText: 'All items' }).click();
      await page.waitForTimeout(400);
    });

    await step('malformed-secret', 'Corner: malformed base32 seed renders placeholder, no crash', async () => {
      await newItem(frameLoc, {
        title: 'Bad OTP',
      ...
      if (otpCount > 0) {
        ...
        assert(!/^\d{6}$/.test(text.replace(/\s/g, '')), 'a malformed seed must not produce a 6-digit code');
      }
      const errs = consoleMessages.filter((m) => m.type === 'error' && m.step === 'malformed-secret');
      console.log(`[locker] console errors during malformed-secret: ${errs.length}`);
      for (const e of errs) console.log(`  -> ${e.text}`);
    });

    await step('trash-restore', 'Trash Bad OTP -> Trash nav -> Restore -> back under All items', async () => {
      ...
      await page.waitForTimeout(500);
      await shot('08-trash');
      let listText = await frameLoc.locator('.v-items, .v-list').first().textContent().catch(() => '');
      ...
      listText = await frameLoc.locator('.v-items, .v-list').first().textContent().catch(() => '');
      assert(/Bad OTP/.test(listText ?? ''), 'restored item not back under All items');
    });

    await step('purge-flow', 'Trash again -> Delete forever (parks for confirmation OR purges)', async () => {
      await frameLoc.locator('.v-item', { hasText: 'Bad OTP' }).click();
      await page.waitForTimeout(500);
      await frameLoc.locator('button.v-del', { hasText: 'Move to trash' }).click();
      await page.waitForTimeout(700);
      ...
      const trashText = await frameLoc.locator('.v-items, .v-list').first().textContent().catch(() => '');
      if (/approval|confirm|land|Waiting/i.test(noticeText ?? '')) {
        purgeParked = true;
        assert(/Bad OTP/.test(trashText ?? ''), 'parked purge should leave the item in Trash until approved');
        console.log('[locker] purge PARKED for owner confirmation (confirmation:"required") — will approve via the shell Approvals screen');
      } else {
        assert(!/Bad OTP/.test(trashText ?? ''), `purge did not park but the item is still in Trash; notice: ${noticeText}`);
        console.log('[locker] purge executed directly from the owner surface');
      }
    });

    if (purgeParked) {
      await step('approvals-approve-purge', 'Shell Approvals lists the parked purge; Approve -> item really gone', async () => {
        ...
        await page.locator('h2', { hasText: 'Parked' }).waitFor({ state: 'visible', timeout: 10_000 });
        ...
        assert(!/Bad OTP/.test(trashText ?? ''), 'item should be purged after approving the parked write');
      });
    }
