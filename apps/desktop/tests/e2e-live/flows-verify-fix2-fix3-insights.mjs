...
async function main() {
  ...
  try {
    ...
    await step('fix2-labels', 'Insights "By source"/"Recent activity" show "System health check", not generic labels', async () => {
      await openInsights();
      await shot('fix2-insights-full-page');
      const bodyTxt = await bodyText();
      console.log(`[fix23] Insights body (first 1000 chars): ${JSON.stringify(bodyTxt.slice(0, 1000))}`);
      ...
      const bySourceSection = page.locator('text=By source').locator('xpath=ancestor::*[self::section or contains(@class,"panel")][1]');
      const bySourceText = await bySourceSection.first().textContent().catch(() => bodyTxt);
      console.log(`[fix23] By source panel text: ${JSON.stringify(bySourceText?.slice(0, 400))}`);
      assert(/System health check/.test(bySourceText ?? ''), 'By source panel missing "System health check" label');
      ...
      const recentSection = page.locator('text=Recent activity').locator('xpath=ancestor::*[self::section or contains(@class,"panel")][1]');
      const recentText = await recentSection.first().textContent().catch(() => bodyTxt);
      console.log(`[fix23] Recent activity panel text: ${JSON.stringify(recentText?.slice(0, 600))}`);
      assert(/System health check/.test(recentText ?? ''), 'Recent activity missing "System health check" label for automation runs');
      ...
      assert(!rawOkRow, 'Recent activity still shows raw handler summary "ok" instead of the automation name');
    });

    // ================= FIX 3a: baseline KPIs =================
    let baselineTokensText = '';
    let baselineSpentText = '';
    await step('fix3a-baseline-kpis', 'Note baseline Tokens/Spent KPIs before sending a new chat message', async () => {
      await openInsights();
      const tokensBlock = await page.locator('text=Tokens · 30 days').locator('xpath=..').textContent();
    ...
    await step('fix3b-send-chat', 'Assistant: send "say hi", wait up to 180s for a real codex reply', async () => {
    ...
    await step('fix3c-insights-after-turn', 'Insights KPIs nonzero, By model shows gpt-*, new Chat row in Recent activity', async () => {
      await navTo(page, 'Home');
      await page.waitForTimeout(300);
      await openInsights();
      await shot('fix3c-insights-after-llm-turn');
      const bodyTxt = await bodyText();
      console.log(`[fix23] Insights body after chat turn (first 1200 chars): ${JSON.stringify(bodyTxt.slice(0, 1200))}`);
      ...
      assert(Number.isFinite(totalTokens) && totalTokens > 0, `expected nonzero totalTokens KPI, got ${JSON.stringify(tokensBlock)}`);
    ...
    let turn1 = null;
    await step('fix3d-db-ground-truth', 'DB: newest interactive turn has nonzero tokens + non-NULL cost; newest step item has model set', async () => {
      ...
      turn1 = { id, startedAt: Number(startedAt), inTok: Number(inTok), outTok: Number(outTok), cacheTok: Number(cacheTok), cost };
      assert(turn1.inTok + turn1.outTok + turn1.cacheTok > 0, `newest interactive turn has all-zero token columns: ${turnsOut}`);
      assert(cost && cost !== '' && cost.toLowerCase() !== 'null', `newest interactive turn has NULL/empty cost: ${turnsOut}`);
      ...
      assert(itemFirst && itemFirst[0] && itemFirst[0].toLowerCase() !== 'null' && itemFirst[0] !== '', `newest step item missing model: ${itemsOut}`);
    });

    // ================= FIX 3e: second turn, own usage not cumulative =================
    await step('fix3e-second-turn-not-cumulative', 'Second chat message (thread resume) records its OWN turn usage, not cumulative doubling', async () => {
      await navTo(page, 'Assistant');
      ...
      const t2 = { id: second[0], inTok: Number(second[2]), outTok: Number(second[3]), cacheTok: Number(second[4]), cost: second[5] };
      const t1 = { id: first[0], inTok: Number(first[2]), outTok: Number(first[3]), cacheTok: Number(first[4]), cost: first[5] };
      ...
      console.log(`[fix23] turn2/turn1 total-token ratio = ${ratio.toFixed(2)} (expect roughly ~1x, NOT ~2x cumulative doubling)`);
      assert(ratio < 1.6, `second turn's tokens look cumulative (ratio ${ratio.toFixed(2)}x vs first turn) — thread-resume diffing may be broken`);
    });
