...
async function main() {
  ...
  try {
    ...
    await step(
      'pregrant-open-chip',
      "Ask panel opens immediately after install; capture the grant chip's very-first-open state",
      async () => {
    ...
    await step(
      'confirm-auto-grant-settled',
      'Confirm the Tasks app is now granted (auto-consent on its first vault call) via App settings -> Vault; click Load demo data if offered',
      async () => {
        ...
        console.log(
          `[ask01] Vault tab: already granted=${isGranted}, "Grant access" button present=${needsManualGrant}`,
        );
        if (needsManualGrant) {
    ...
    await step(
      'revoke-then-regrant',
      'Revoke access, confirm chip flips to no-grant, then manually re-grant via the button (exercises the real manual grant path)',
      async () => {
        ...
        assert(
          !(await grantBtnRetry.isDisabled()),
          'Grant access button is still disabled even after a fresh Settings dialog mount',
        );
    ...
    await step(
      'reopen-same-session-chip',
      'Reopen Ask in the SAME iframe session after revoke/re-grant — grantChecked latches true on the FIRST open of this mount, so the chip should be STALE (still showing whatever it captured on the very first open)',
      async () => {
        await frameLoc.locator('#kitAskBtn').click();
        await frameLoc
          .locator('.kit-ask-panel[role="dialog"]')
        ...
        console.log(
          `[ask01] same-session reopen chip text: "${chipText}" (first-open text was: "${firstOpenChipText}")`,
        );
    ...
    await step(
      'remount-then-reopen-chip',
      'Navigate away (Home) and back — iframe remounts kit.js, resetting grantChecked — chip should now reflect the TRUE current (re-granted) state',
      async () => {
        ...
        const nowGranted =
          /consent-gated/.test(chipText) &&
          !/no grant yet/.test(chipText) &&
          !/not enrolled/.test(chipText);
        if (!nowGranted) {
          console.log(
          ...
          console.log(
            '[ask01] chip showed a granted state after remount — did not reproduce the appId/name mismatch bug this run.',
          );
        }
        results.push({
          id: 'grant-chip-appid-name-mismatch-bug',
          label:
            'BUG: kit.js grant chip compares apps[].appId (UUID) to manifest appId string, so it can never show granted state',
          verdict: nowGranted ? 'pass' : 'bug-confirmed',
          ms: 0,
        });
      },
    );
    ...
    await step(
      'empty-input-noop',
      'Submitting empty input is a no-op (no new bubble)',
      async () => {
        ...
        assert(
          after === before,
          `expected no new bubble on empty submit, before=${before} after=${after}`,
        );
    ...
    await step(
      'unicode-html-escaped',
      'Unicode + HTML-as-text renders escaped in transcript, not interpreted',
      async () => {
        const input = frameLoc.locator('.kit-ask-compose input[aria-label="Ask"]');
        ...
        console.log(
          `[ask01] unicode bubble text: ${JSON.stringify(bubbleText)}, <b> element count: ${hasBoldElement}`,
        );
        assert(
          bubbleText.includes(text),
          `bubble text does not contain the literal sent text: ${bubbleText}`,
        );
        assert(
          hasBoldElement === 0,
          'HTML in the user message was interpreted as markup instead of escaped text',
        );
    ...
    await step(
      ...
      'Rapid double-click send: check for de-dupe/disable (report observed behavior)',
      async () => {
        const input = frameLoc.locator('.kit-ask-compose input[aria-label="Ask"]');
        ...
        console.log(
          `[ask01] rapid double-click send: user bubbles added = ${delta} (1 = de-duped/guarded, 2 = duplicate submits allowed — a minor bug worth flagging)`,
        );
    ...
    await step(
      'navigate-away-panel-open',
      'Navigate the shell to Home while Ask panel is open — no crash / console errors from the unmount',
      async () => {
        ...
        if (errCountAfter > errCountBefore) {
          console.log(
            `[ask01] new console errors after navigating away with panel open: ${JSON.stringify(newErrors)}`,
          );
        } else {
          console.log(
            '[ask01] no new console errors from navigating away with the Ask panel open.',
          );
        }
      },
    );
    ...
    await step(
      'reopen-transcript-persistence',
      'Reopen Tasks + Ask — report whether the prior transcript persisted or reset',
      async () => {
        frameLoc = await openApp('tasks');
        await frameLoc.locator('#kitAskBtn').click();
        await frameLoc
        ...
        console.log(
          `[ask01] transcript after reopen preserved prior messages: ${transcriptPreserved}. Log text starts: ${JSON.stringify(text.slice(0, 200))}`,
        );
    ...
    await step(
      'install-locker-diff-suggestions',
      'Install Locker; Ask placeholder/suggestions differ from Tasks',
      async () => {
        await frameLoc
          .locator('.kit-ask-x[aria-label="Close"]')
          .click()
          .catch(() => undefined);
        await navTo(page, 'Home');
        ...
        await lockerFrame
          .locator('.kit-ask-panel[role="dialog"]')
          .waitFor({ state: 'visible', timeout: 10_000 });
        const lockerPlaceholder = await lockerFrame
          .locator('.kit-ask-compose input[aria-label="Ask"]')
          .getAttribute('placeholder');
        const lockerChips = await lockerFrame.locator('.kit-ask-chip').allTextContents();
        console.log(
          `[ask01] Locker Ask placeholder: "${lockerPlaceholder}", suggestions: ${JSON.stringify(lockerChips)}`,
        );
        await shot('13-locker-ask-panel');
        assert(
          lockerPlaceholder === 'Ask your locker…',
