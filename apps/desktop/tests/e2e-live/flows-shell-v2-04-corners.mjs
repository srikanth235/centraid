...
async function noHorizontalOverflow(label) {
  ...
  console.log(`[v2-04] ${label}: scrollWidth=${metrics.scrollW} clientWidth=${metrics.clientW} bodyScrollWidth=${metrics.bodyScrollW}`);
  assert(
...
async function main() {
  ...
  try {
    ...
    await step('home-composer', 'Home composer: prompt box, Build control, suggestion chips all render', async () => {
      ...
      for (const chip of ['Habit tracker', 'Weekly review', 'Inbox digest', 'Invoice filer']) {
        assert(
          await page.getByRole('button', { name: chip }).isVisible().catch(() => false),
      ...
      assert((await composer.inputValue()) === 'a tiny QA scratchpad app', 'composer text did not round-trip');
      await composer.fill('');
      await shot('01-home-composer');
    });

    // ---------- Rapid-fire nav ----------
    await step('rapid-nav-storm', '15 rapid nav clicks across all routes leave the shell coherent', async () => {
      const routes = ['Discover', 'Insights', 'Automations', 'Approvals', 'Starred', 'Assistant', 'Home'];
      for (let i = 0; i < 15; i++) {
    ...
    await step('vault-switcher-settled', 'Vault switcher popover fully settles (no translucent text bleed)', async () => {
      const head = page.locator('[class*="head"]').first();
      await page.keyboard.press('Meta+Shift+G');
      const menu = page.getByRole('menu');
      const opened = await menu.waitFor({ state: 'visible', timeout: 3_000 }).then(() => true).catch(() => false);
      if (!opened) {
      ...
      if (!closedByEscape) {
        ...
        throw new Error('BUG: Escape does not close the vault switcher popover (scrim click does)');
      }
    });

    // ---------- Narrow viewports ----------
    await step('narrow-375-phone', '375x812 phone width: Home/Discover/Settings no horizontal overflow', async () => {
      ...
      await page.locator('button[data-kind]').first().waitFor({ state: 'visible', timeout: 15_000 });
    ...
    await step('sidebar-collapse-relaunch', 'Collapsed sidebar persists across relaunch', async () => {
      ...
      assert(state === 'closed', `collapsed sidebar did not persist relaunch (data-sidebar=${state})`);
    ...
    await step('escape-closes-preview-narrow', 'Escape closes a template preview at 375px too', async () => {
      await page.setViewportSize({ width: 375, height: 812 });
      await page.waitForTimeout(400);
      await navTo(page, 'Discover');
