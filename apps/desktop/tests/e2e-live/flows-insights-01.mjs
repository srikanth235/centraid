...
async function main() {
  ...
  try {
    ...
    await step(
      'flow1-empty-state',
      'Fresh vault -> Insights shows full empty-state layout',
      async () => {
        const t = Date.now();
        await openInsights();
        ...
        const spentVal = await page
          .locator('text=Spent · USD')
          .locator('xpath=..')
          .locator('div')
          .first();
        void spentVal;
    ...
    await step(
      'flow2-run-completes-on-own-feed',
      "Run completes and shows on the automation's own run feed",
      async () => {
        const t = Date.now();
        ...
        const txt = await bodyText();
        console.log(
          `[ins] run-view text snapshot (first 300 chars): ${txt.slice(0, 300).replace(/\n/g, ' | ')}`,
        );
    ...
    await step(
      'flow2-insights-shows-first-run',
      'Insights: Generations >= 1, $0.00 cost, By source + Recent activity rows',
      async () => {
        const t = Date.now();
        await openInsights();
        ...
        const bodyTxt = await bodyText();
        console.log(
          `[ins] Insights body after 1 run (first 800 chars): ${bodyTxt.slice(0, 800).replace(/\n/g, ' | ')}`,
        );
    ...
    await step(
      'flow3-run-again-x2',
      'Run the automation 2 more times (3 total) and verify counts increment',
      async () => {
        const t = Date.now();
        for (let i = 2; i <= 3; i++) {
          await navTo(page, 'Automations');
    ...
    await step(
      'flow3-insights-3-runs',
      'Insights reflects 3 total generations; Daily consumption shows something for today',
      async () => {
        const t = Date.now();
        await openInsights();
        ...
        console.log(
          `[ins] Daily consumption empty-state count after 3 runs: ${dailyEmptyStill} (0 = chart is showing)`,
        );
    ...
    await step(
      'flow4-real-llm-turn',
      'Assistant: send a trivial prompt, wait up to 180s for a real answer',
      async () => {
        const t = Date.now();
        ...
        console.log(
          `[ins] assistant settle=${settled} sawErrorHint=${sawError} elapsed=${Date.now() - t}ms`,
        );
        if (!settled) {
          llmAvailable = false;
        ...
        if (sawError) {
          llmAvailable = false;
          console.log(
            '[ins] LLM turn surfaced an error/unavailable hint — treating as LLM-unavailable, not a bug',
          );
        }
    ...
    if (llmAvailable) {
      await step(
        'flow4-insights-nonzero-tokens',
        'Insights shows nonzero tokens/USD + a By model row after the real turn',
        async () => {
          await openInsights();
      ...
      inconclusive(
        'flow4-insights-nonzero-tokens',
        'Insights shows nonzero tokens after real turn',
        'LLM turn unavailable/errored — see flow4-real-llm-turn',
      );
    }
    ...
    await step(
      'flow5a-formatting',
      'No NaN/Infinity/undefined anywhere on Insights; USD formatted sanely',
      async () => {
        await openInsights();
    ...
    await step(
      'flow5b-nav-during-run',
      'Navigate to Insights while a run is in flight -> no crash, data settles',
      async () => {
        await navTo(page, 'Automations');
    ...
    await step(
      'flow5c-rapid-renav',
      'Rapid re-navigation Insights<->Home x5 -> no crash, no stale flicker',
      async () => {
        for (let i = 0; i < 5; i++) {
    ...
    await step(
      'flow6-narrow-resize',
      'Resize to ~900x700 -> layout does not collapse/overlap illegibly',
      async () => {
