...
const FIXTURES = '/private/tmp/claude-502/-Users-srikanth-gitspace-centraid--claude-worktrees-charming-matsumoto-4872ab/51bb86f0-75f7-4678-aef4-ad31b920a377/scratchpad/docs-fixtures';
...
async function main() {
  ...
  try {
    ...
    await step('upload-basic', 'Upload a plain .txt fixture via the hidden file input', async () => {
      ...
      assert(/note\.txt|note/.test(gridText ?? ''), `uploaded note.txt not visible in grid: ${JSON.stringify(gridText)}`);
    });

    await step('upload-long-and-unicode', 'Upload a long filename and a unicode/emoji filename', async () => {
      const uploadInput = frameLoc.locator('#uploadInput');
      const longName = 'this-is-a-very-long-filename-used-to-test-docs-truncation-and-wrapping-behavior-in-the-grid-and-list-views-of-the-docs-blueprint-app.txt';
      ...
      assert(/日本語のファイル名テスト/.test(gridText ?? ''), `unicode filename not visible: ${JSON.stringify(gridText?.slice(0, 400))}`);
    });
    ...
    await step('list-toggle', 'Toggle to List view and back to Grid', async () => {
      ...
      assert((await listWrap.getAttribute('hidden')) === null, 'list view should be visible after toggle');
      await frameLoc.locator('#viewGrid').click();
      await page.waitForTimeout(300);
      await shot('04-grid-view-restored');
      const grid = frameLoc.locator('#grid');
      assert((await grid.getAttribute('hidden')) === null, 'grid view should be visible after toggling back');
    });

    await step('details-panel', 'Open details panel for note.txt, verify fields, star it', async () => {
    ...
    await step('folder-create-move-rename-delete', 'Create a folder, move a doc into it, rename it, then delete it (doc survives)', async () => {
      ...
      await page.waitForTimeout(300);
      const popoverMoveItem = page.locator('.kit-popover-item, [role="menuitem"]', { hasText: 'E2E Folder' }).first();
      // Popover lives inside the iframe too.
      const framePopoverItem = frameLoc.locator('.kit-popover-item, [role="menuitem"]', { hasText: 'E2E Folder' }).first();
      ...
      await page.waitForTimeout(400);
      const folderGridText = await frameLoc.locator('#grid').textContent().catch(() => '');
      assert(/note/.test(folderGridText ?? ''), 'moved document not visible inside the folder view');
      ...
      assert((await frameLoc.locator('.d-nav-item', { hasText: 'E2E Folder Renamed' }).count()) > 0, 'rename did not take effect');
      ...
      assert(/Empty the folder first/.test(noticeText ?? ''), `expected the friendly "Empty the folder first…" message, got: ${JSON.stringify(noticeText)}`);
      assert((await frameLoc.locator('.d-nav-item', { hasText: 'E2E Folder Renamed' }).count()) > 0, 'folder should still exist — the delete should have been refused, not applied');
      ...
      await page.waitForTimeout(300);
      const documentsTarget = frameLoc.locator('.kit-popover-item, [role="menuitem"]', { hasText: 'Documents' }).first();
      ...
      assert((await frameLoc.locator('.d-nav-item', { hasText: 'E2E Folder Renamed' }).count()) === 0, 'folder still present after delete (once emptied)');
      const allGridText = await frameLoc.locator('#grid').textContent().catch(() => '');
      assert(/note/.test(allGridText ?? ''), 'document disappeared after its (now-emptied) folder was deleted — should have survived, un-filed');
    });

    await step('trash-and-restore', 'Trash the unicode doc, verify it is gone from All, restore it', async () => {
      ...
      const trashGridText = await frameLoc.locator('#grid, #list').first().textContent().catch(() => '');
      ...
      await frameLoc.locator('.d-nav-item', { hasText: 'All documents' }).click();
      await page.waitForTimeout(400);
      const restoredGridText = await frameLoc.locator('#grid').textContent().catch(() => '');
      assert(/日本語/.test(restoredGridText ?? ''), 'restored doc not visible under All documents');
    });
    ...
    await step('search', 'Search filters to the long-filename doc', async () => {
      ...
      assert(/truncation/.test(gridText ?? ''), 'search for "truncation" did not surface the long-filename doc');
