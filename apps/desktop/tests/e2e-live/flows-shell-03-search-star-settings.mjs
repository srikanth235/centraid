...
async function main() {
  ...
  try {
    ...
    await step('search-query', 'Typing a query filters results sanely', async () => {
      ...
      const resultText = await page.locator('.results, [class*="results"]').first().textContent().catch(() => '');
      console.log(`[s3] palette results for "Notes": ${JSON.stringify(resultText?.slice(0, 300))}`);
    });

    await step('search-empty-query', 'Corner case: clearing back to an empty query does not break the palette', async () => {
      const input = page.getByPlaceholder('Search apps, chats, templates — or describe a new one…');
      await input.fill('');
      await page.waitForTimeout(400);
    ...
    await step('search-unicode-xss', 'Corner case: unicode + literal <script> query renders escaped, not executed', async () => {
      const input = page.getByPlaceholder('Search apps, chats, templates — or describe a new one…');
      ...
      const scriptTagCount = await page.evaluate(
        (marker) => document.querySelectorAll(`script`).length && document.body.innerHTML.includes(marker) && document.querySelectorAll('script[data-injected-by-test]').length,
        payload,
      );
      assert(scriptTagCount === 0, 'a <script> tag appears to have been injected from the search query');
    ...
    await step('install-agenda-for-star', 'Install Agenda template (fixture for the star flow)', async () => {
    ...
    await step('star-app-via-context-menu', 'Star Agenda via its context menu; data-starred flag flips', async () => {
    ...
    await step('starred-page-check', 'Visit the Starred sidebar page after starring an app', async () => {
    ...
    const settingsTabs = ['Appearance', 'Layout', 'Workspace', 'Spaces', 'Phone', 'Import', 'Connections', 'Agents'];
    await step('settings-tab-walk', 'Walk every Settings tab — each renders without error', async () => {
      ...
      for (const tabName of settingsTabs) {
        ...
        await tabBtn.click();
        await page.waitForTimeout(300);
        await page.getByRole('heading', { name: tabName }).waitFor({ state: 'visible', timeout: 10_000 });
    ...
    await step('automations-screen-and-template-detail', 'Automations screen renders; browse an automation template detail from Discover', async () => {
      ...
      console.log(`[s3] automation preview dialog text: ${JSON.stringify(dialogText?.slice(0, 300))}`);
