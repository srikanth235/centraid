...
async function openStarred() {
  ...
  await page.getByRole('heading', { name: 'Starred', level: 1 }).waitFor({ state: 'visible', timeout: 15_000 });
...
async function main() {
  ...
  page = session.page;
  wireConsole(page);
  console.log(`[fix1] launched + Home ready in ${Date.now() - t0}ms (fresh profile, userData=${USER_DATA_DIR})`);

  try {
    await page.setViewportSize({ width: 1400, height: 900 });

    // ---------- a. fresh profile -> Starred empty state ----------
    await step('a-empty-state', 'Fresh profile: Starred shows "Nothing starred yet..." empty state', async () => {
      await openStarred();
      await shot('a-empty-state');
      const emptyMsg = page.locator('text=Nothing starred yet. Hover an app tile and tap the star.');
    ...
    await step('b-install-and-star-app', 'Install Agenda, star it via Home right-click context menu', async () => {
      ...
      assert(starredBefore === 'false', `expected data-starred=false before starring, got ${starredBefore}`);
      ...
      assert(starredAfter === 'true', `expected data-starred=true after starring, got ${starredAfter}`);
    ...
    await step('c-starred-page-shows-card', 'Starred page renders the starred Agenda card, pixel-identical to Home', async () => {
      await openStarred();
      await shot('c-starred-page-with-agenda');
      const card = page.locator('[data-app-id="agenda"]');
      await card.waitFor({ state: 'visible', timeout: 10_000 });
      assert((await card.getAttribute('data-starred')) === 'true', 'Starred card missing data-starred=true');
    ...
    await step('d-open-and-unstar', 'Starred card context menu offers Open/Unstar; Open opens app; Unstar removes card', async () => {
      const card = page.locator('[data-app-id="agenda"]');
      ...
      assert(/Unstar/.test(menuText ?? ''), `expected "Unstar" in Starred menu, got: ${menuText}`);
      assert(!/Rename/.test(menuText ?? ''), `menu should be narrow (no Rename), got: ${menuText}`);
      assert(!/Delete/.test(menuText ?? ''), `menu should be narrow (no Delete), got: ${menuText}`);
    ...
    await step('e-star-automation', 'Adopt + star "System health check" automation; appears on Starred as an automation card', async () => {
      await navTo(page, 'Discover');
      await page.getByRole('tab', { name: /^Automations/ }).click();
      await page.waitForTimeout(300);
      const tmplCard = page.locator('button[data-kind="automation"]', { hasText: 'System health check' }).first();
      ...
      await openHome();
      const autoTile = page.locator('[data-kind="automation"]', { hasText: 'System health check' }).first();
      ...
      console.log(`[fix1] Starred page body after starring automation: ${JSON.stringify(bodyTxt.slice(0, 300))}`);
      assert(/System health check/.test(bodyTxt), 'Starred page missing the starred automation card');
      const autoCardOnStarred = page.locator('[data-kind="automation"]', { hasText: 'System health check' });
      await autoCardOnStarred.first().waitFor({ state: 'visible', timeout: 5_000 });
    });

    // ---------- f. relaunch persistence ----------
    await step('f-relaunch-persistence', 'Relaunch same userDataDir -> check whether star state persists on Starred', async () => {
      await session.close();
      ...
      const bodyTxt = await bodyText();
      console.log(`[fix1] Starred page body after relaunch: ${JSON.stringify(bodyTxt.slice(0, 300))}`);
      const emptyPresent = await page.locator('text=Nothing starred yet.').count();
      console.log(`[fix1] OBSERVATION: after relaunch, empty-state present=${emptyPresent > 0} (star Store persistence — report as observation, not necessarily a bug either way)`);
    });
