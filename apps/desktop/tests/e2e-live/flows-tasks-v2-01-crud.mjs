...
async function main() {
  ...
  try {
    ...
    await step(
      'open-empty-board',
      'Open Tasks -> empty board renders (Today view, empty state copy)',
      async () => {
    ...
    await step(
      'add-plain-task-enter-key',
      'Add a plain task via Enter key (no due/priority)',
      async () => {
        const input = frameLoc.locator('.tk-capture-input');
    ...
    await step(
      ...
      'Add a task with the "Tmrw" due chip -> shows in Upcoming, not Today',
      async () => {
    ...
    await step(
      'add-task-nl-due-parsing',
      'Natural-language due date in the title ("...+3d") previews a hint and lands the clean title with the due date',
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Today' }).click();
        await page.waitForTimeout(200);
        const input = frameLoc.locator('.tk-capture-input');
        ...
        assert(
          !/\+3d/.test(rowText ?? ''),
          `NL token "+3d" leaked into the stored title: ${rowText}`,
        );
    ...
    await step(
      ...
      'Add a task with the "High" flag chip -> flag glyph renders on the row',
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Anytime' }).click();
        await page.waitForTimeout(200);
    ...
    await step(
      ...
      'Sidebar nav counts match the actual number of open tasks in each bucket',
      async () => {
    ...
    await step(
      'open-detail-drawer',
      'Click a row -> Detail drawer opens with title/notes/due/priority/effort controls',
      async () => {
        ...
        assert(
          (await titleInput.inputValue()) === 'Write the QA report',
          'detail title input did not prefill with task title',
        );
      },
    );
    ...
    await step(
      'detail-edit-title',
      'Edit title in detail drawer, blur -> commits + row reflects new title',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
        const titleInput = detail.locator('.tk-detail-title');
    ...
    await step(
      'detail-edit-notes',
      'Set notes in detail drawer, blur -> commits + note preview shows under row',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
        ...
        assert(
          /screenshots/.test(noteText ?? ''),
          `row note preview missing expected text: ${noteText}`,
        );
      },
    );
    ...
    await step(
      'detail-clear-notes',
      'Clear notes (empty + blur) -> clear_description path, note preview disappears',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
        const notes = detail.locator('.tk-detail-notes');
        await notes.fill('');
    ...
    await step(
      'detail-edit-due-preset',
      'Pick "Next wk" due preset in detail drawer -> due chip updates + row reflects it',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
    ...
    await step(
      'detail-edit-due-datepicker',
      'Use the raw date input to set an explicit due date',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
    ...
    await step('detail-edit-priority', 'Pick "Med" priority chip in detail drawer', async () => {
      ...
      await page.waitForTimeout(700);
      const active = detail
        .locator('.tk-detail-cols > div', { hasText: 'Priority' })
        .locator('button.on', { hasText: 'Med' });
      await active.waitFor({ state: 'visible', timeout: 5_000 });
    ...
    await step(
      'detail-edit-effort',
      'Pick "30m" effort chip in detail drawer -> row shows effort meta',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
        ...
        const active = detail
          .locator('.tk-detail-cols > div', { hasText: 'Effort' })
          .locator('button.on', { hasText: '30m' });
        await active.waitFor({ state: 'visible', timeout: 5_000 });
      },
    );
    ...
    await step(
      'detail-add-subtask',
      'Add a subtask from the detail drawer -> renders in subtasks list, closes to 0/1 badge on row',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
    ...
    await step(
      'detail-complete-subtask',
      'Complete the subtask via its own circle -> counter flips to 1/1',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
        const subRow = detail.locator('.tk-subtask-row', { hasText: 'Attach screenshots' });
        ...
        assert(
          /1\/1/.test(labelText ?? ''),
          `subtasks counter did not show 1/1 after completing: ${labelText}`,
        );
      },
    );
    ...
    await step(
      'detail-activity-log',
      'Session activity log in the drawer records the edits made so far, each tagged "receipt"',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
    ...
    await step(
      'detail-start-pause',
      'Toggle "Start" -> task becomes in-process, row shows "in progress" badge; "Pause" reverts',
      async () => {
        const detail = frameLoc.locator('.tk-detail');
    ...
    await step(
      'complete-task-from-row',
      'Complete a task from the board row -> toast with Undo, row leaves the open list',
      async () => {
    ...
    await step(
      'completed-task-in-logbook',
      'Completed task appears in Logbook with the completion date',
      async () => {
    ...
    await step(
      'undo-completion-via-toast',
      'Re-complete + Undo via toast action puts the task back in the open list',
      async () => {
        ...
        console.log(
          `[tasks01] logbook row circle click: before=${before} after=${after} (expected no-op, row stays)`,
        );
    ...
    await step(
      'cancel-task-from-detail',
      'Cancel a task from the detail drawer -> lands in Logbook with the cancel glyph, Cancel button now disabled',
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Anytime' }).click();
        await page.waitForTimeout(300);
        const row = frameLoc.locator('.tk-row', { hasText: 'Fix the leaking faucet' });
    ...
    await step(
      'search-tasks-cross-view',
      'Search is global, not scoped to the current focus view: from "Anytime" (undated only), searching for a task due TOMORROW must still surface it (regression test for a bucket-allow-list bug fixed during this QA pass — see logic.js buildSections)',
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Anytime' }).click();
        await page.waitForTimeout(300);
    ...
    await step(
      ...
      'Search with no matches shows the "No matches" empty state',
      async () => {
        const searchInput = frameLoc.locator('#searchInput');
        ...
        assert(
          (await emptyTitle.textContent())?.trim() === 'No matches',
    ...
    await step(
      'clear-search-escape',
      'Escape clears the search box and restores the full board',
      async () => {
        const searchInput = frameLoc.locator('#searchInput');
