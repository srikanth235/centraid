...
async function main() {
  ...
  try {
    ...
    await step(
      'empty-title-submit-noop',
      'Add button stays disabled for an empty title; Enter on empty input is a no-op',
      async () => {
        ...
        const count = await frameLoc
          .locator('.tk-nav-item', { hasText: 'All open' })
          .locator('.tk-nav-count')
          .textContent();
        assert(
          count === '0',
          `expected 0 open tasks after empty-title Enter, sidebar shows ${count}`,
        );
    ...
    await step(
      'whitespace-only-title-noop',
      'A title of only spaces is trimmed to empty -> Add stays disabled, no task created',
      async () => {
        const input = frameLoc.locator('.tk-capture-input');
        await input.fill('    ');
        const addBtn = frameLoc.locator('.tk-capture-add');
        ...
        assert(
          disabled,
          'Add button is enabled for a whitespace-only title (should stay disabled)',
        );
        await input.fill('');
      },
    );
    ...
    await step(
      'very-long-title',
      'A very long title (500 chars) is accepted, stored, and rendered without breaking layout',
      async () => {
    ...
    await step(
      'special-chars-emoji-unicode',
      'Title with quotes, HTML tags, emoji and unicode renders as literal text, not interpreted markup',
      async () => {
        ...
        assert(
          rendered === weirdTitle,
          `special-char title did not round-trip exactly: ${JSON.stringify(rendered)}`,
        );
        const boldCount = await titleEl.locator('b').count();
        assert(
          boldCount === 0,
          'HTML <b> tag in the title was interpreted as markup instead of literal text',
        );
    ...
    await step(
      'rapid-double-click-complete',
      'Rapid double-click on a row circle does not double-fire / corrupt state',
      async () => {
        ...
        console.log(
          `[tasks02] rows matching the long title still visible in this view after rapid double-click: ${openCount}`,
        );
    ...
    await step(
      'logbook-rows-are-a-dead-end',
      "FINDING: once a task lands in Logbook (from the rapid-double-click completion above), there is NO UI path back — the row's onClick AND its circle toggle are both disabled when closed=true (Row.jsx), so clicking it is a confirmed no-op",
      async () => {
        ...
        console.log(
          `[tasks02] long-title task ended up completed (in Logbook) after the rapid double-click: ${inLogbook}`,
        );
        if (inLogbook) {
          await row.click();
          await page.waitForTimeout(400);
          ...
          console.log(
            `[tasks02] clicking the Logbook row opened the Detail drawer: ${detailOpened} (expected false — Row.jsx sets onClick=undefined when closed)`,
          );
          assert(
            !detailOpened,
            'Logbook row unexpectedly opened a Detail drawer — Row.jsx closed-guard may have regressed',
          );
        }
    ...
    await step(
      'detail-toggle-idempotency',
      "Complete/reopen/complete again via the Detail drawer's own circle (which stays open across the status change) — toggling an already-completed task back and forth is idempotent, no error",
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Anytime' }).click();
        ...
        assert(
          onState === 'true',
          `expected completed (data-on=true) after 1st toggle, got ${onState}`,
        );
        ...
        assert(
          onState === 'false',
          `expected reopened (data-on=false) after 2nd toggle, got ${onState}`,
        );
        ...
        assert(
          onState === 'true',
          `expected completed again (data-on=true) after 3rd toggle, got ${onState}`,
        );
    ...
    await step(
      'escape-mid-title-edit-discards-unsaved',
      'Escape while editing a title in the Detail drawer closes the drawer WITHOUT committing the unsaved (un-blurred) text',
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Anytime' }).click();
        await page.waitForTimeout(300);
        const input = frameLoc.locator('.tk-capture-input');
        ...
        console.log(
          `[tasks02] rows showing the unsaved Escape'd title text: ${leaked} (expected 0 — onBlur never fired)`,
        );
        assert(
          leaked === 0,
          'unsaved title edit leaked into the row despite Escape (no blur/commit expected)',
        );
      },
    );

    await step(
      'enter-key-in-capture-submits',
      'Pressing Enter in the capture bar submits the same as clicking Add',
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Anytime' }).click();
        await page.waitForTimeout(200);
        const input = frameLoc.locator('.tk-capture-input');
    ...
    await step(
      'load-demo-data',
      'Load demo data via Settings -> Vault -> "Load demo data" (real UI path, not a direct API call)',
      async () => {
    ...
    await step(
      'many-tasks-scroll-layout',
      'Add enough tasks to exceed 15 total open items; board scrolls, sections/counts stay correct, no layout breakage',
      async () => {
        await frameLoc.locator('.tk-nav-item', { hasText: 'Anytime' }).click();
        await page.waitForTimeout(300);
        for (let i = 1; i <= 10; i++) {
        ...
        assert(
          Number(allCount) >= 15,
          `expected at least 15 open tasks, sidebar shows ${allCount}`,
        );
    ...
    await step(
      ...
      async () => {
        ...
        assert(
          Number(allCount) >= 15,
          `open task count did not persist across relaunch: ${allCount}`,
        );
        ...
        const longRowOpen = await frameLoc
          .locator('.tk-row-title', { hasText: 'Extremely long task title' })
          .count();
        if (longRowOpen === 0) {
          await frameLoc.locator('.tk-nav-item', { hasText: 'Logbook' }).click();
          await page.waitForTimeout(400);
          const longRowClosed = await frameLoc
            .locator('.tk-row', { hasText: 'Extremely long task title' })
            .count();
          assert(
            longRowClosed > 0,
            'long-title task vanished entirely across relaunch (not open, not in Logbook)',
          );
          await frameLoc.locator('.tk-nav-item', { hasText: 'All open' }).click();
          await page.waitForTimeout(400);
        }
        ...
        console.log(
          `[tasks02] special-char task title after relaunch: ${JSON.stringify(specialText)}`,
        );
        assert(
          /emoji.*unicode/.test(specialText ?? ''),
          'special-char task title corrupted across relaunch',
        );
      },
    );
