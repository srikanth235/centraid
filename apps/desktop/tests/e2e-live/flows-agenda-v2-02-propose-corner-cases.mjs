...
async function main() {
  ...
  try {
    ...
    await step(
      'flow1-two-calendars-visible',
      'Seeded Personal+Work calendars show in "My calendars" sidebar',
      async () => {
    ...
    await step(
      'flow2-propose-normal-event',
      'Propose a normal timed event today on Personal -> lands tentative, pill visible',
      async () => {
    ...
    await step(
      'flow3-event-detail-drawer-no-guests',
      'Open event detail for an event proposed with no guests: no Guests section (empty attendees array)',
      async () => {
        ...
        console.log(
          `[agv2-2] "Guests" eyebrow present: ${guestsLabel > 0} (expected 0 -- no one was invited to this event)`,
        );
        assert(
          guestsLabel === 0,
          'expected NO Guests section for an event proposed without any guests',
        );
    ...
    await step(
      'flow4-all-day-event',
      'Propose an all-day event (midnight-to-midnight) -> renders as a spanning bar, not a timed pill',
      async () => {
        const modal = await openCreateModal(fl);
        ...
        assert(
          spans === 'true',
          `expected data-spans="true" for a midnight-to-midnight event, got ${spans}`,
        );
      },
    );

    await step(
      'flow5-overnight-event',
      'Propose an overnight event (10pm today -> 1am tomorrow) -> spans two day cells in month view',
      async () => {
        const modal = await openCreateModal(fl);
        const today = new Date();
        const start = new Date(
        ...
        const occurrences = await fl
          .locator('.ag-pill', { hasText: 'Overnight watch shift' })
          .count();
        assert(
          occurrences === 2,
          `expected the overnight event to render on both its start and end day cells, got ${occurrences}`,
        );
      },
    );

    await step(
      'flow6-long-emoji-title',
      'Propose events with a very long title and an emoji/special-char/HTML-ish title -> render safely, no layout break, no XSS',
      async () => {
        const longTitle =
          'Q3 cross-functional stakeholder alignment and roadmap prioritization deep-dive with the entire extended leadership team and outside counsel'.slice(
            0,
            180,
          );
        ...
        assert(
          ...
          'expected the literal <script> text to render as inert text, not execute',
        );
        // No actual alert dialog should have fired (React escapes text content by default).
        console.log(
          `[agv2-2] emoji pill text (should be literal, inert): ${JSON.stringify(renderedText)}`,
        );
    ...
    await step(
      'flow7-past-event',
      'Propose a past event (yesterday) -> month view (explicit range) still shows it',
      async () => {
        const modal = await openCreateModal(fl);
        const today = new Date();
        const start = new Date(
        ...
        assert(
          count === 1,
          `expected the past event to render in month view (in-month range), got ${count}`,
        );
        ...
        const scheduleCount = await fl
          .locator('.ag-sched-card', { hasText: 'Yesterday retro' })
          .count();
        console.log(
          `[agv2-2] "Yesterday retro" visible in Schedule (upcoming-only) view: ${scheduleCount > 0} (expected false)`,
        );
        assert(
          scheduleCount === 0,
          'a purely-past event should not appear in the forward-looking Schedule view',
        );
    ...
    await step(
      'flow8-same-calendar-conflict-refused',
      'Propose an event overlapping an EXISTING one on the SAME calendar -> vault refuses (failed, precondition no_busy_conflict)',
      async () => {
        const modal = await openCreateModal(fl);
        const today = new Date();
        ...
        const end = new Date(start.getTime() + 3600000);
        await fillAndSubmit(modal, {
          title: 'Conflicting same-cal meeting',
          start,
          end,
          calendarName: 'Personal',
        });
        ...
        assert(
          /refused|conflict|busy/i.test(noticeText ?? ''),
          `expected a vault-refused conflict message, got: ${noticeText}`,
        );
        // Modal should still be open (proposal did not silently succeed).
        const stillOpen = await modal.isVisible();
        assert(
          stillOpen,
          'modal should remain open after a refused proposal so the user can adjust',
        );
    ...
    await step(
      'flow9-cross-calendar-conflict-also-refused',
      'Propose an overlapping event on a DIFFERENT calendar (Work) -> ALSO refused (conflict check is vault-wide, not per-calendar)',
      async () => {
        const modal = await openCreateModal(fl);
        const today = new Date();
        ...
        const end = new Date(start.getTime() + 3600000);
        await fillAndSubmit(modal, {
          title: 'Cross-calendar conflict test',
          start,
          end,
          calendarName: 'Work',
        });
        ...
        assert(
          /refused|conflict|busy/i.test(noticeText ?? ''),
          `expected the SAME vault-wide conflict refusal across calendars, got: ${noticeText}`,
        );
        await page.keyboard.press('Escape');
        await modal.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => undefined);
      },
    ...
    await step(
      'flow10-invalid-end-before-start',
      'Client-side validation: end before start blocks submit with inline notice, no vault round-trip',
      async () => {
        const modal = await openCreateModal(fl);
        const today = new Date();
        const start = new Date(
        ...
        const notice = await modal
          .locator('.ag-form-notice')
          .textContent()
          .catch(() => '');
        assert(
          /later end/i.test(notice ?? ''),
          `expected "Pick a start and a later end." notice, got: ${notice}`,
        );
        const stillOpen = await modal.isVisible();
    ...
    await step(
      'flow11-escape-mid-form-discards',
      'Escape mid-form discards the draft -- reopening shows a blank composer',
      async () => {
        const modal = await openCreateModal(fl);
        ...
        assert(
          titleVal === '',
          `expected a blank title after Escape-discard + reopen, got: ${JSON.stringify(titleVal)}`,
        );
    ...
    await step(
      'flow12-rapid-double-click-create-btn',
      'Rapid double-click "Create event" opens exactly one modal, not two stacked',
      async () => {
        ...
        assert(
          modalCount === 1,
          `expected exactly 1 create modal after rapid double-click, got ${modalCount}`,
        );
        await shot('15-rapid-doubleclick-single-modal');
        await page.keyboard.press('Escape');
        await fl
          .locator('.ag-create-modal')
    ...
    await step(
      'flow13-rapid-doubleclick-submit-no-dupe',
      'Rapid double-click "Propose event" does not create a duplicate event',
      async () => {
        const modal = await openCreateModal(fl);
        const today = new Date();
        const start = new Date(
        ...
        assert(
          count === 1,
          `expected exactly 1 event from a rapid double-submit, got ${count} (possible duplicate-write bug)`,
        );
      },
    );

    await step(
      'flow14-search-finds-events',
      'Search box finds a proposed event by title (FTS5), routes to Schedule view',
      async () => {
        await fl.locator('.kit-seg button', { hasText: 'Month' }).click();
        await page.waitForTimeout(300);
