...
async function main() {
  ...
  try {
    ...
    await step('open-welcome', 'Open Tally -> fresh-vault welcome dashboard', async () => {
      ...
      assert(
        /Welcome to Tally/.test(wrapText ?? ''),
        `expected the welcome empty state, got: ${JSON.stringify(wrapText?.slice(0, 200))}`,
      );
    });

    await step(
      'expense-blocked-without-group',
      'Corner: Add expense without a group -> notice, no modal',
      async () => {
        ...
        assert(
          /Create a group first/.test(noticeText ?? ''),
          'expected "Create a group first" notice',
        );
        assert(
          (await frameLoc.locator('.kit-modal').count()) === 0,
          'no expense modal should open without a group',
        );
      },
    );
    ...
    await step('create-group', 'Create group "Road Trip" with Bob', async () => {
      ...
      assert(
        /Road Trip/.test(title ?? ''),
        `expected to land in the Road Trip group view, title: ${title}`,
      );
    });
    ...
    await step(
      'add-expense-60-equal',
      'Add $60 expense you paid, equal split -> Bob owes you $30.00',
      async () => {
        await frameLoc.locator('#addExpenseBtn').click();
        const modal = frameLoc.locator('.kit-modal');
        await modal.waitFor({ state: 'visible', timeout: 5000 });
        ...
        assert(
          /30\.00 each · 2 people/.test(norm(sumText)),
          `equal-split preview wrong: ${sumText}`,
        );
        ...
        const balText = await frameLoc
          .locator('.s-balpanel')
          .textContent()
          .catch(() => '');
        console.log(`[tally] group balance chips: ${JSON.stringify(balText)}`);
        assert(
          /You get back [^\d]*30\.00/.test(norm(balText)),
          `group chip should say "You get back 30.00": ${balText}`,
        );
        assert(
          /Bob owes [^\d]*30\.00/.test(norm(balText)),
          `group chip should say "Bob owes 30.00": ${balText}`,
        );

        // Ledger row: "you lent" $30.00, sub "you paid $60.00".
        const rowText = await frameLoc.locator('.s-exrow', { hasText: 'Gas' }).textContent();
        assert(
          /you lent/.test(rowText ?? '') && /30\.00/.test(norm(rowText)),
          `ledger row wrong: ${rowText}`,
        );
    ...
    await step(
      'dashboard-after-e1',
      'Dashboard totals: owed $30.00, owe $0.00, net +$30.00',
      async () => {
        ...
        const wrapText = await frameLoc.locator('#wrap').textContent();
        assert(
          /\+[^\d]*30\.00/.test(norm(wrapText)),
          `total balance should be +30.00: ${JSON.stringify(wrapText?.slice(0, 300))}`,
        );
        assert(
          /Bob/.test(wrapText ?? '') && /owes you/.test(wrapText ?? ''),
          'You-are-owed list should show Bob',
        );
      },
    );
    ...
    await step(
      'odd-cent-split',
      'Corner: $10.01 equal split -> shares $5.01 + $5.00 (no lost cent)',
      async () => {
        await frameLoc.locator('#addExpenseBtn').click();
        const modal = frameLoc.locator('.kit-modal');
        await modal.waitFor({ state: 'visible', timeout: 5000 });
        ...
        await modal
          .locator('.s-field', { hasText: 'Paid by' })
          .locator('select')
          .selectOption({ label: 'Bob Marley' });
        await page.waitForTimeout(300);
        const sumText = await modal.locator('.s-splitsum').textContent();
        ...
        const wrapText = await frameLoc.locator('#wrap').textContent();
        assert(
          /\+[^\d]*24\.99/.test(norm(wrapText)),
          `net should be +24.99 after odd-cent expense: ${JSON.stringify(wrapText?.slice(0, 300))}`,
        );
      },
    );
    ...
    await step(
      ...
      'Open the odd-cent expense detail: split rows $5.01 (You) + $5.00 (Bob)',
      async () => {
        ...
        assert(
          /5\.01/.test(norm(detailText)) && /5\.00/.test(norm(detailText)),
          'detail modal split rows should show 5.01 and 5.00',
        );
    ...
    await step(
      'edit-expense',
      'Edit the $60 expense to $80 (exact split 40/40) -> net +$34.99',
      async () => {
        ...
        assert(
          /You get back [^\d]*34\.99/.test(norm(balText)),
          `group chip should say "You get back 34.99": ${balText}`,
        );
      },
    );
    ...
    await step('delete-expense', 'Delete the odd-cent expense -> net back to +$40.00', async () => {
      ...
      const balText = await frameLoc
        ...
        .catch(() => '');
      assert(
        /You get back [^\d]*40\.00/.test(norm(balText)),
        `group chip should say "You get back 40.00" after delete: ${balText}`,
      );
      assert(
        (await frameLoc.locator('.s-exrow', { hasText: 'Odd cent snack' }).count()) === 0,
        'deleted expense still in the ledger',
      );
    });
    ...
    await step('settle-up', 'Settle up: Bob pays you $40.00 -> group settled', async () => {
      ...
      assert(
        /You — settled/.test(balText ?? '') && /Bob — settled/.test(balText ?? ''),
        `both chips should be settled: ${balText}`,
      );
    });

    await step(
      'large-amount',
      'Corner: $999999.99 expense renders with correct shares',
      async () => {
        await frameLoc.locator('#addExpenseBtn').click();
        const modal = frameLoc.locator('.kit-modal');
        await modal.waitFor({ state: 'visible', timeout: 5000 });
        ...
        assert(
          /500000\.00 each/.test(norm(sumText)),
          `large equal split preview wrong: ${sumText}`,
        );
        await modal.locator('button', { hasText: 'Save' }).click();
        await modal.waitFor({ state: 'hidden', timeout: 10_000 });
        await page.waitForTimeout(800);
        ...
        assert(
          /Bob owes [^\d]*499999\.99/.test(norm(balText)),
          `large-amount balance wrong: ${balText}`,
        );
    ...
    await step(
      'activity-view',
      'Activity view lists the expense, edit trail and settlement',
      async () => {
    ...
    await step('search', 'Search finds "Gas" with the group suffix', async () => {
      ...
      const wrapText = await frameLoc.locator('#wrap').textContent();
      assert(
        /Gas/.test(wrapText ?? '') && /Road Trip/.test(wrapText ?? ''),
        `search results should show Gas · Road Trip: ${JSON.stringify(wrapText?.slice(0, 300))}`,
      );
