...
async function main() {
  ...
  try {
    ...
    await step('fix3-default-calendar-present', 'Fresh vault already has a "Personal" calendar -- create modal is not a dead end (fix #3)', async () => {
      ...
      assert(calChipCount >= 1, `expected a "Personal" calendar chip in the create modal on a brand-new vault, found ${calChipCount}`);
      const noCalMsg = await modal.locator('text=import an .ics file').count();
      assert(noCalMsg === 0, 'expected NO "no calendars, import an .ics" dead-end hint now that a default calendar is minted');
    ...
    await step('propose-alpha', 'Propose "Verify Alpha (cancel target)" on Personal', async () => {
      const modal = await openCreateModal(fl);
      await fillAndSubmit(modal, { title: 'Verify Alpha (cancel target)', start: alphaStart, end: alphaEnd, calendarName: 'Personal' });
      await page.waitForTimeout(800);
      await fl.locator('.kit-seg button', { hasText: 'Month' }).click();
      await page.waitForTimeout(300);
      await fl.locator('.ag-today', { hasText: 'Today' }).click();
      await page.waitForTimeout(300);
      await fl.locator('.ag-pill', { hasText: 'Verify Alpha' }).waitFor({ state: 'visible', timeout: 10_000 });
      await shot('02-alpha-proposed');
    });

    await step('propose-beta', 'Propose "Verify Beta (reschedule target)" on Personal', async () => {
      const modal = await openCreateModal(fl);
      await fillAndSubmit(modal, { title: 'Verify Beta (reschedule target)', start: betaStart, end: betaEnd, calendarName: 'Personal' });
      await page.waitForTimeout(800);
      await fl.locator('.ag-pill', { hasText: 'Verify Beta' }).waitFor({ state: 'visible', timeout: 10_000 });
      await shot('03-beta-proposed');
    });

    await step('cancel-parks-not-executes', 'Ask-to-cancel Alpha -- must PARK (notice says waiting for approval), event stays', async () => {
      ...
      const toastTexts = await fl.locator('kit-toast').allTextContents().catch(() => []);
      ...
      assert(parkToast, `expected a "sent to the owner for confirmation" toast, got: ${JSON.stringify(toastTexts)}`);
      const executedToast = toastTexts.find((t) => /^event cancelled/i.test(t));
      assert(!executedToast, `must NOT show an "Event cancelled" toast (would mean it executed immediately), got: ${JSON.stringify(toastTexts)}`);
      ...
      assert(stillThere >= 1, 'Verify Alpha event must still be present on the calendar (parked, not executed)');
      await shot('07-alpha-still-on-calendar');
    });

    await step('approvals-lists-cancel', 'Approvals surface lists the parked cancel_event entry', async () => {
      ...
      const scheduleRows = await page.locator('text=/schedule\\.(cancel_event|cancel-event)/i').count();
      const bodyText = await page.locator('body').innerText();
      const mentionsAlpha = /Verify Alpha/i.test(bodyText);
      console.log(`[v01] Approvals schedule.cancel_event rows: ${scheduleRows}, mentions "Verify Alpha": ${mentionsAlpha}`);
      assert(scheduleRows > 0 || mentionsAlpha, 'expected an Approvals entry for the parked cancel_event');
    });

    await step('approve-actually-cancels', 'Approving the parked entry actually cancels the event in Agenda', async () => {
      ...
      assert(goneCount === 0, 'expected Verify Alpha to be actually cancelled (gone from calendar) after owner approval');
      await shot('10-alpha-gone-after-approval');
    });

    await step('reschedule-parks-not-executes', 'Move Beta via the drawer -- must PARK (parked toast + activity log, event keeps its original time)', async () => {
      ...
      const newStart = originalStart.replace(/T(\d{2}):/, (m, h) => `T${String((Number(h) + 2) % 24).padStart(2, '0')}:`);
      ...
      const parkToast = toastTexts.find((t) => /sent to the owner for confirmation/i.test(t) && /current time until approved/i.test(t));
      assert(parkToast, `expected a "sent to the owner for confirmation ... stays at its current time" toast, got: ${JSON.stringify(toastTexts)}`);
      const movedToast = toastTexts.find((t) => /^event moved/i.test(t));
      assert(!movedToast, `must NOT show "Event moved" toast (would mean it executed immediately), got: ${JSON.stringify(toastTexts)}`);
      ...
      await page.waitForTimeout(300);
      const reopenedStart = await drawer2.locator('input[type="datetime-local"]').first().inputValue();
      console.log(`[v01] Beta start before attempted move: ${originalStart}, after: ${reopenedStart}`);
      assert(reopenedStart === originalStart, `Beta's start time changed (${originalStart} -> ${reopenedStart}) -- reschedule executed immediately instead of parking`);
      const activityText = await drawer2.locator('.ag-activity, [class*="activity"]').innerText().catch(() => '');
      console.log(`[v01] activity log mentions parked move: ${/parked for the owner/i.test(activityText)}`);
