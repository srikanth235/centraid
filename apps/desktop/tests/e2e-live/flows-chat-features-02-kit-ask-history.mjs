...
async function waitForKitAskIdle(frameLoc, timeout = 60_000) {
  await frameLoc.locator('.kit-ask-compose[data-busy="false"]').waitFor({ state: 'attached', timeout });
}
...
async function main() {
  ...
  try {
    ...
    await step('history-button-toggles', 'History button toggles the in-panel history view', async () => {
    ...
    await step('send-message', 'Send a message; busy contract engages (data-busy=true, send disabled) then clears', async () => {
      ...
      await frameLoc.locator('.kit-ask-compose[data-busy="true"]').waitFor({ state: 'attached', timeout: 5_000 });
      const sendBtnDisabled = await frameLoc.locator('.kit-ask-send[aria-label="Send"]').isDisabled();
      assert(sendBtnDisabled, 'send button should be disabled while .kit-ask-compose[data-busy="true"]');
    ...
    await step('history-shows-conversation', 'Reopening History shows this conversation in the list', async () => {
      const histBtn = frameLoc.locator('.kit-ask-history-btn');
      await histBtn.click();
    ...
    await step('attach-file-in-compose', 'Attach a file via the paperclip button, chip appears', async () => {
      ...
      const pendingChip = frameLoc.locator('.kit-ask-pending-name', { hasText: 'chat02-sample.txt' });
      await pendingChip.waitFor({ state: 'visible', timeout: 10_000 });
      await shot('06-pending-attachment-chip');
    });

    await step('send-with-attachment', 'Send with the attachment; chip renders in the sent bubble', async () => {
      const input = frameLoc.locator('.kit-ask-compose input[aria-label="Ask"]');
    ...
    await step('kit-model-picker-default-label', 'Ask panel model picker (.kit-ask-model-btn) shows "Default" with no override', async () => {
      const btn = frameLoc.locator('.kit-ask-model-btn');
      await btn.waitFor({ state: 'visible', timeout: 5_000 });
      assert((await btn.getAttribute('aria-label')) === 'Model', 'kit model button missing aria-label="Model"');
      assert((await btn.getAttribute('aria-haspopup')) === 'menu', 'kit model button missing aria-haspopup="menu"');
    ...
    await step('kit-model-picker-open-menu', 'Opening the menu shows role=menu with "Use default" first', async () => {
      const btn = frameLoc.locator('.kit-ask-model-btn');
      await btn.click();
      ...
      assert(/^Use default/.test(firstText), `first kit model item should read "Use default...", got "${firstText}"`);
      assert(
    ...
    await step('kit-model-picker-select-non-default', 'Pick a non-default model; label updates', async () => {
      const items = frameLoc.locator('.kit-ask-model-item');
      const count = await items.count();
      if (count < 2) {
        console.log('[chat02] only "Use default" is available in the kit ask catalog — skipping selection/persistence steps');
      ...
      assert(label === pickedKitModelName, `label did not update to the picked model: expected "${pickedKitModelName}", got "${label}"`);
      await shot('10-kit-model-picker-selected');
    });

    if (pickedKitModelName) {
      await step('kit-model-picker-persists-on-reopen', 'Close + reopen the Ask panel; the picked model label persists (PUT + reopen-refetch works)', async () => {
        ...
        await frameLoc.locator('.kit-ask-panel[role="dialog"]').waitFor({ state: 'hidden', timeout: 5_000 }).catch(() => undefined);
        await frameLoc.locator('#kitAskBtn').click();
        await frameLoc.locator('.kit-ask-panel[role="dialog"]').waitFor({ state: 'visible', timeout: 10_000 });
        const label = (await frameLoc.locator('.kit-ask-model-label').textContent())?.trim() ?? '';
        console.log(`[chat02] kit ask model label after reopen: "${label}" (expected "${pickedKitModelName}")`);
        assert(
      ...
      await step('kit-model-picker-reset-to-default', 'Reset the picker to "Use default"', async () => {
        const btn = frameLoc.locator('.kit-ask-model-btn');
        await btn.click();
        const menu = frameLoc.locator('.kit-ask-model-menu');
        await menu.waitFor({ state: 'visible', timeout: 5_000 });
      ...
      console.log('[chat02] skipping persistence + reset steps (no non-default model was available to select)');
    }

    await step('attach-magic-token-file', 'Attach a text file with a magic token; the real model reads it back (multimodal fix)', async () => {
      const tmpFile = path.join(OUT_DIR, 'chat02-magic.txt');
      await fs.writeFile(tmpFile, 'The code word is MANGO-COMPASS-93.\n');
      const fileInput = frameLoc.locator('.kit-ask-file');
      await fileInput.setInputFiles(tmpFile);
      const pendingChip = frameLoc.locator('.kit-ask-pending-name', { hasText: 'chat02-magic.txt' });
      await pendingChip.waitFor({ state: 'visible', timeout: 10_000 });
    ...
    await step('kit-composer-visual-screenshot', 'Screenshot of the redesigned kit Ask composer for visual review', async () => {
      await shot('16-kit-composer-visual-final');
    });

    await step('new-conversation-from-history', '"+ New conversation" clears the active thread', async () => {
      const histBtn = frameLoc.locator('.kit-ask-history-btn');
      await histBtn.click();
      ...
      const log = frameLoc.locator('.kit-ask-log');
      const text = (await log.textContent()) ?? '';
      assert(!text.includes('See attached file'), 'log still shows prior conversation text after "+ New conversation"');
      await shot('08-new-conversation-cleared');
    });

    await step('reopen-past-conversation', 'Reopen the earlier conversation from history -> transcript reloads', async () => {
      const histBtn = frameLoc.locator('.kit-ask-history-btn');
      const isHistoryOpen = (await frameLoc.locator('.kit-ask-history').evaluate((el) => el.hidden)) === false;
      ...
      const log = frameLoc.locator('.kit-ask-log');
      const text = (await log.textContent()) ?? '';
      console.log(`[chat02] reloaded transcript text starts: ${JSON.stringify(text.slice(0, 200))}`);
      assert(
