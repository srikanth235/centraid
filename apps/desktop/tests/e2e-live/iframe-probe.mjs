...
async function main() {
  ...
  try {
    ...
    assert(
      h1Text?.trim() === "Agenda",
      `expected iframe h1 "Agenda", got ${h1Text}`
    );

    const kitBannerCount = await frameLoc.locator(".kit-banner").count();
    console.log(
      `[iframe-probe] iframe has ${kitBannerCount} .kit-banner node(s)`
    );
    assert(
      kitBannerCount >= 1,
      "expected at least one .kit-banner node inside the app iframe"
    );
    ...
    console.log(
      `[iframe-probe] #weekViewBtn aria-pressed: ${beforePressed} → ${afterPressed}`
    );
    assert(
      beforePressed !== "true",
      "week view should not start pressed (month is default)"
    );
    assert(
      afterPressed === "true",
    ...
    if (captured)
      console.log(`[iframe-probe]   -> ${JSON.stringify(captured)}`);
    assert(
      Boolean(captured),
      'page.on("console") did not capture the child-frame console.log'
    );
