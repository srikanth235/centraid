...
async function main() {
  ...
  try {
    ...
    await step(
      'flow1-install',
      'Discover -> preview Agenda -> Use this template installs + pins to Home',
      async () => {
    ...
    await step(
      'flow2-open-empty-month',
      'Open Agenda -> empty vault, default Month view renders with no consent banner blocking',
      async () => {
        const tile = page.locator('[data-app-id="agenda"]');
    ...
    await step(
      'flow3-empty-week',
      'Switch to Week view -> empty grid, no events, no console errors',
      async () => {
        const fl = frameLoc(page);
    ...
    await step(
      'flow4-empty-schedule',
      'Switch to Schedule view -> "Nothing coming up" empty state',
      async () => {
        const fl = frameLoc(page);
        await fl.locator('.kit-seg button', { hasText: 'Schedule' }).first().click();
        await page.waitForTimeout(400);
        await shot('05-empty-schedule-view');
        const emptyTitle = await fl
          .locator('.kit-empty-title')
          .textContent()
          .catch(() => '');
        assert(
          /Nothing coming up/.test(emptyTitle ?? ''),
          `expected "Nothing coming up" empty state, got: ${emptyTitle}`,
        );
      },
    );

    await step(
      'flow5-create-modal-default-calendar',
      'Create event modal on a fresh vault: the bootstrapped "Personal" calendar is pickable and Propose is enabled',
      async () => {
        ...
        const calChips = await modal
          .locator('.ag-cal-chips:not(.ag-guest-chips) .ag-cal-chip')
          .count();
        ...
        assert(
          !(await proposeBtn.isDisabled()),
          'Propose event should be enabled when a calendar exists',
        );
