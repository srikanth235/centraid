...
async function main() {
  ...
  console.log(
    `[smoke] launched + Home ready in ${Date.now() - t0}ms (userData=${userDataDir})`
  );
  ...
  try {
    ...
    const autoCount = await page
      .locator('button[data-kind="automation"]')
      .count();
    console.log(
      `[smoke] breakdown: ${appCount} app templates, ${autoCount} automation templates`
    );
    assert(appCount === 8, `expected 8 app templates, got ${appCount}`);
    assert(
      autoCount === 16,
      `expected 16 automation templates, got ${autoCount}`
    );
