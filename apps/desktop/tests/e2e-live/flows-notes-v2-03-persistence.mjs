...
async function main() {
  ...
  try {
    ...
    await step('install-and-seed-state', 'Install Notes; create 2 notebooks, 3 notes (1 filed, 1 pinned, 1 with an attachment + checklist)', async () => {
      await installNotes();
      ...
      await frame.locator('.nt-nb-name', { hasText: 'Work' }).waitFor({ state: 'visible', timeout: 5_000 });
      ...
      await frame.locator('.nt-card-title', { hasText: 'Persistence probe — filed note' }).waitFor({ state: 'visible', timeout: 10_000 });
      ...
      await frame.locator('.nt-qa-body').fill('- [ ] Verify this survives relaunch\n- [x] Already checked once');
      await frame.locator('.nt-qa-actions .kit-btn.primary', { hasText: 'Add note' }).click();
      const pinnedCard = frame.locator('.nt-card', { hasText: 'Persistence probe — pinned note' });
      ...
      await frame.locator('.nt-qa-actions .kit-btn.primary', { hasText: 'Add note' }).click();
      const attachCard = frame.locator('.nt-card', { hasText: 'Persistence probe — with attachment' });
    ...
    let counts = null;
    await step('capture-pre-relaunch-state', 'Capture sidebar summary + Pinned count before closing the app', async () => {
      await frame.locator('.nt-nav-item', { hasText: 'All notes' }).click();
      await page.waitForTimeout(300);
      const summaryLine = await frame.locator('.nt-summary-line').textContent();
      const pinnedCount = await frame.locator('.nt-nav-item', { hasText: 'Pinned' }).locator('.nt-nav-count').textContent();
      counts = { summaryLine, pinnedCount };
      console.log(`[notes-v2-03] pre-relaunch: summary="${summaryLine}", pinnedCount=${pinnedCount}`);
      assert(pinnedCount === '1', `expected 1 pinned note before relaunch, got ${pinnedCount}`);
    });

    await step('relaunch-same-userdata', 'Fully close and relaunch the app with the same userDataDir', async () => {
      await session.close();
    ...
    await step('reopen-and-verify-notebooks-and-notes', 'Reopen Notes; both notebooks + all 3 notes are present with correct scope/pin/checklist state', async () => {
      frame = await openNotes();
      ...
      const summaryLine = await frame.locator('.nt-summary-line').textContent();
      console.log(`[notes-v2-03] post-relaunch summary line: "${summaryLine}" (was "${counts?.summaryLine}" before)`);
      assert(summaryLine === counts?.summaryLine, `sidebar summary changed across relaunch: "${summaryLine}" vs "${counts?.summaryLine}"`);

      const pinnedCount = await frame.locator('.nt-nav-item', { hasText: 'Pinned' }).locator('.nt-nav-count').textContent();
      assert(pinnedCount === '1', `expected 1 pinned note to survive relaunch, got ${pinnedCount}`);

      const filedCard = frame.locator('.nt-card-title', { hasText: 'Persistence probe — filed note' });
      await filedCard.waitFor({ state: 'visible', timeout: 10_000 });
      const pinnedCard = frame.locator('.nt-card-title', { hasText: 'Persistence probe — pinned note' });
      await pinnedCard.waitFor({ state: 'visible', timeout: 10_000 });
      const attachCard = frame.locator('.nt-card-title', { hasText: 'Persistence probe — with attachment' });
      await attachCard.waitFor({ state: 'visible', timeout: 10_000 });
      ...
      const inWork = await frame.locator('.nt-card-title', { hasText: 'Persistence probe — filed note' }).count();
      assert(inWork === 1, 'the note filed into "Work" before relaunch is no longer scoped there after relaunch');
      ...
      console.log(`[notes-v2-03] pinned note checklist after relaunch: ${checkedBoxes}/${totalBoxes} done`);
      assert(totalBoxes === 2 && checkedBoxes === 1, `checklist state did not survive relaunch: ${checkedBoxes}/${totalBoxes}`);
      ...
      assert(naturalWidth > 0, 'the attachment image failed to load after relaunch (broken image / lost blob)');
    });
    ...
    await page.screenshot({ path: path.join(OUT_DIR, 'FATAL-persist-FAILURE.png') }).catch(() => undefined);
    process.exitCode = 1;
  } finally {
    await session.close();
  }
