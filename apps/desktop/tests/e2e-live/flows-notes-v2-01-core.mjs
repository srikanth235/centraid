...
async function recoverUi() {
  try {
    ...
    if (frame) {
      ...
      if ((await back.count().catch(() => 0)) > 0) await back.click({ timeout: 2_000 }).catch(() => undefined);
    }
  } catch {
    /* best-effort only */
  }
...
async function main() {
  ...
  try {
    ...
    await step('install', 'Install Notes via Discover -> preview -> Use this template', async () => {
      await installNotes();
      await shot('install-home-tile');
    });

    await step('open-empty-state', 'Open Notes; fresh vault shows empty state + quick-add composer', async () => {
      ...
      assert((await emptyTitle.textContent())?.trim() === 'No notes yet', `unexpected empty title: ${await emptyTitle.textContent()}`);
    ...
    await step('ask-panel-first-open', 'Open the Ask panel (first act-verb-adjacent surface) and screenshot grant state', async () => {
    ...
    await step('quick-add-unfiled', 'Create a note via quick-add while scope is "All notes" (unfiled)', async () => {
      ...
      await bodyInput.fill('The Design of Everyday Things, Salt Fat Acid Heat, Project Hail Mary.');
      ...
      assert(targetLabel?.trim() === 'Unfiled', `expected quick-add target "Unfiled", got "${targetLabel}"`);
    ...
    await step('quick-add-into-notebook', 'Navigate into "Travel" then quick-add files the note there', async () => {
      ...
      assert(/Travel/.test(targetLabel ?? ''), `expected quick-add target to say "Into Travel", got "${targetLabel}"`);
      await frame.locator('.nt-qa-actions .kit-btn.primary', { hasText: 'Add note' }).click();
      await page.waitForTimeout(700);
    ...
    await step('sidebar-counts-update', 'Sidebar "All notes" / notebook counts reflect the two created notes', async () => {
      await frame.locator('.nt-nav-item', { hasText: 'All notes' }).click();
      await page.waitForTimeout(300);
      await shot('all-notes-scope');
      const allCount = await frame.locator('.nt-nav-item', { hasText: 'All notes' }).locator('.nt-nav-count').textContent();
      console.log(`[notes-v2-01] All notes count: ${allCount}`);
      assert(Number(allCount) >= 2, `expected at least 2 notes total, got ${allCount}`);
    });

    await step('open-editor-edit-title-body', 'Open a note, edit title + body, wait for autosave -> "Saved" label', async () => {
      ...
      await textarea.fill('The Design of Everyday Things, Salt Fat Acid Heat, Project Hail Mary, Piranesi.');
    ...
    await step('close-and-reopen-verify-persisted', 'Close the editor and reopen the note to confirm the edit persisted', async () => {
      await frame.locator('.kit-icon-btn[aria-label="Back"]').click();
      await frame.locator('.nt-editor').waitFor({ state: 'hidden', timeout: 5_000 }).catch(() => undefined);
      await page.waitForTimeout(300);
    ...
    await step('checklist-add-and-toggle', 'Insert a checklist item, type it, close, toggle done -> card progress bar updates', async () => {
      ...
      await frame.locator('.nt-editor').waitFor({ state: 'hidden', timeout: 5_000 }).catch(() => undefined);
      await page.waitForTimeout(300);
      const label2 = await frame.locator('.nt-card', { hasText: 'Scratch — books to read next' }).locator('.nt-progress-label').textContent();
      console.log(`[notes-v2-01] checklist progress label after toggling done: "${label2}"`);
      assert(label2 === '1/1', `expected checklist progress "1/1" after toggling done, got "${label2}"`);
      await shot('card-checklist-done');
    });

    await step('pin-from-card-and-unpin-from-editor', 'Pin a note from its card; verify Pinned group + sidebar count; unpin from the editor', async () => {
      ...
      const pinnedCountEl = frame.locator('.nt-nav-item', { hasText: 'Pinned' }).locator('.nt-nav-count');
      ...
      await frame.locator('.kit-icon-btn[aria-label="Back"]').click();
      await page.waitForTimeout(400);
      const pinnedCountAfter = await frame.locator('.nt-nav-item', { hasText: 'Pinned' }).locator('.nt-nav-count').textContent();
      console.log(`[notes-v2-01] Pinned count after unpin from editor: ${pinnedCountAfter}`);
      assert(pinnedCountAfter === '0', `expected sidebar Pinned count 0 after unpin, got ${pinnedCountAfter}`);
      await shot('note-unpinned-from-editor');
    });

    await step('move-note-via-editor-select', 'Move a note into "Recipes" via the editor\'s notebook <select>', async () => {
      await frame.locator('.nt-nav-item', { hasText: 'All notes' }).click();
      await page.waitForTimeout(300);
      const card = frame.locator('.nt-card', { hasText: 'Scratch — books to read next' });
      await card.click();
      ...
      const cardInRecipes = frame.locator('.nt-card-title', { hasText: 'Scratch — books to read next' });
      await cardInRecipes.waitFor({ state: 'visible', timeout: 10_000 });
    });

    await step('attach-and-remove-file', 'Attach a small PNG to a note, verify it shows in the strip, then remove it', async () => {
      ...
      console.log(`[notes-v2-01] attachment remove took ${attempts} click(s) to land (2 is the expected arm+confirm)`);
      if (attempts > 2) {
        console.log(
      ...
      assert(tileCountAfter === 0, `expected 0 attachment tiles after remove, got ${tileCountAfter} after ${attempts} clicks`);
      await frame.locator('.kit-icon-btn[aria-label="Back"]').click();
    });

    await step('search-and-clear', 'Search for a term, verify filtered + highlighted results, then clear', async () => {
      ...
      await frame.locator('.nt-sub', { hasText: /match/ }).waitFor({ state: 'visible', timeout: 10_000 });
      ...
      assert(/Goa/.test(cardTitle ?? ''), `search result card title doesn't contain "Goa": ${cardTitle}`);
      ...
      assert(countAfter > 1, `expected more than 1 card after clearing search, got ${countAfter}`);
    });

    await step('view-toggle-list-and-back', 'Toggle List view then back to Card (masonry) view', async () => {
      await frame.locator('.nt-nav-item', { hasText: 'All notes' }).click();
      ...
      assert((await wallList.count()) === 1, 'wall did not get the "list" class after toggling List view');
      await frame.locator('#viewMasonryBtn').click();
      await page.waitForTimeout(300);
      await shot('view-masonry');
    });

    await step('rename-notebook', 'Rename "Recipes" notebook to "Cooking" via the toolbar', async () => {
      await frame.locator('.nt-nb-name', { hasText: 'Recipes' }).click();
      await page.waitForTimeout(300);
    ...
    await step('delete-open-editor-note', 'Delete a note from inside the editor (arm + confirm) -> editor closes, card gone', async () => {
      await frame.locator('.nt-nav-item', { hasText: 'All notes' }).click();
      await page.waitForTimeout(300);
      const card = frame.locator('.nt-card', { hasText: 'Goa long weekend' });
      await card.click();
      ...
      const cardGone = await frame.locator('.nt-card-title', { hasText: 'Goa long weekend' }).count();
      assert(cardGone === 0, 'deleted note still shows in the wall');
    });

    await step('delete-notebook-unfiles-notes', 'Delete "Cooking" notebook (has a note) -> note is unfiled, not destroyed', async () => {
      ...
      await deleteBtn.click();
      await page.waitForTimeout(200);
      const confirmBtn = frame.locator('.kit-icon-btn.danger[aria-label="Confirm delete notebook"]');
      await confirmBtn.waitFor({ state: 'visible', timeout: 3_000 });
      await confirmBtn.click();
      await page.waitForTimeout(700);
      ...
      const noteStillThere = await frame.locator('.nt-card-title', { hasText: 'Scratch — books to read next' }).count();
      assert(noteStillThere >= 1, 'note that was inside the deleted notebook was destroyed, not unfiled');
    });

    await step('create-notebook-duplicate-name-allowed', 'FINDING: knowledge.create_notebook has NO uniqueness precondition (unlike rename_notebook, which refuses via name_unused_by_owner) — creating two notebooks with the same disposable name both succeed', async () => {
      const makeOne = async () => {
        await frame.locator('button[aria-label="New notebook"]').click();
        const input = frame.locator('input[aria-label="Notebook name"]');
        await input.waitFor({ state: 'visible', timeout: 5_000 });
        ...
        await frame.locator('.nt-nb-form').waitFor({ state: 'hidden', timeout: 10_000 }).catch(() => undefined);
        await page.waitForTimeout(400);
      };
      await makeOne();
      const countAfterFirst = await frame.locator('.nt-nb-name', { hasText: 'Scratchpad' }).count();
      await makeOne();
      const countAfterSecond = await frame.locator('.nt-nb-name', { hasText: 'Scratchpad' }).count();
      console.log(
      ...
      assert(countAfterFirst === 1, `expected 1 "Scratchpad" notebook after the first create, got ${countAfterFirst}`);
      if (countAfterSecond === 2) {
        ...
        console.log(`[notes-v2-01] did not reproduce the duplicate-name-on-create finding this run (count after 2nd create: ${countAfterSecond})`);
      }
    });
