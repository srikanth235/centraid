...
async function main() {
  ...
  try {
    ...
    for (const s of screens) {
      await step(`nav-${s.label}`, `Sidebar -> ${s.label} renders, no blank pane, no error toast`, async () => {
        ...
        if (s.heading) {
          ...
          await page.getByPlaceholder('Ask your vault anything…').waitFor({ state: 'visible', timeout: 15_000 });
        }
    ...
    await step('chrome-back-forward', 'Back/Forward nav buttons walk router history', async () => {
      ...
      if (backVisible) {
        ...
        console.log('[chrome] Back/Forward buttons not present on this route — recording as observation, not failure');
      }
      await navTo(page, 'Home');
      await page.getByRole('heading', { name: 'What should we build?' }).waitFor({ state: 'visible', timeout: 10_000 });
    });
    ...
    await step('chrome-sidebar-collapse', 'Sidebar collapse/expand toggle works both directions', async () => {
      const hideBtn = page.getByRole('button', { name: 'Hide sidebar' });
      await hideBtn.waitFor({ state: 'visible', timeout: 10_000 });
      await hideBtn.click();
      await page.waitForTimeout(400);
      const windowState = await page.locator('[data-sidebar]').first().getAttribute('data-sidebar').catch(() => null);
      ...
      await showBtn.click();
      await page.waitForTimeout(400);
      const windowState2 = await page.locator('[data-sidebar]').first().getAttribute('data-sidebar').catch(() => null);
    ...
    await step('corner-rapid-double-click', 'Rapid double-click on nav items does not break the shell', async () => {
    ...
    await step('corner-nav-while-dialog-open', 'Sidebar nav while a template preview dialog is open', async () => {
      await navTo(page, 'Discover');
      ...
      await page
        ...
        .first()
        .click({ timeout: 3_000 })
        .catch((err) => console.log(`[corner] sidebar click did not land while dialog open (expected if backdrop traps clicks): ${err.message.split('\n')[0]}`));
