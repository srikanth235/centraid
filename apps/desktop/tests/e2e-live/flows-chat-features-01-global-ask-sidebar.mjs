...
async function main() {
  ...
  try {
    ...
    await step('ensure-claude-code-active', 'Force Claude Code active (codex CLI is broken in this environment)', async () => {
      await ensureClaudeCodeActive();
    });

    await step('sidebar-empty-state', 'Sidebar Chats section shows real empty state (not disabled placeholder)', async () => {
      ...
      assert(await emptyRow.isDisabled(), 'empty-state row should be disabled (not a real conversation)');
    ...
    await step('open-assistant', 'Navigate to Assistant via sidebar', async () => {
      ...
      assert((await oldAside.count()) === 0, 'AssistantScreen still renders its own internal thread aside');
      await shot('01-assistant-empty');
    });

    let firstConvTitle = null;
    await step('send-first-message', 'Send a message; conversation appears in the sidebar with live count', async () => {
      ...
      firstConvTitle = hasRow ? 'New conversation' : null;
    ...
    await step('attach-file-and-send', 'Attach a text file with a magic token; the real model reads it back (multimodal fix)', async () => {
      ...
      const input = page.getByPlaceholder('Ask your vault anything…');
      await input.fill('What is the code word in the attached file? Reply with only the code word.');
      ...
      assert(draftAfterSend === '', `composer did not clear the draft on send (busy-guard likely blocked it): "${draftAfterSend}"`);
    ...
    let pickedAssistantModelId = null;
    let assistantDefaultModelLabel = null;

    await step('model-picker-visible', 'Composer has an "Assistant model" picker button reading "Default · <model>"', async () => {
      const btn = page.locator('button[aria-label="Assistant model"]');
      await btn.waitFor({ state: 'visible', timeout: 5_000 });
      assert((await btn.getAttribute('aria-haspopup')) === 'menu', 'model button missing aria-haspopup="menu"');
      assert((await btn.getAttribute('aria-expanded')) === 'false', 'model button should start collapsed');
      const label = (await btn.textContent())?.trim() ?? '';
      console.log(`[chat01] assistant model button label: "${label}"`);
      assert(/^Default · /.test(label), `expected label to start with "Default · ", got "${label}"`);
      assistantDefaultModelLabel = label;
      await shot('06-model-picker-default-label');
    });

    await step('model-picker-open-menu', 'Opening the picker shows a role=menu with "Use default" first', async () => {
      const btn = page.locator('button[aria-label="Assistant model"]');
      await btn.click();
      assert((await btn.getAttribute('aria-expanded')) === 'true', 'aria-expanded did not flip to true when opened');
      ...
      assert(/^Use default/.test(firstText), `first menu item should read "Use default...", got "${firstText}"`);
      assert((await items.first().getAttribute('aria-checked')) === 'true', 'first item ("Use default") should be aria-checked when no override is set');
    ...
    await step('model-picker-select-non-default', 'Pick the first non-default catalog model; button label updates', async () => {
      const btn = page.locator('button[aria-label="Assistant model"]');
      await btn.click();
      const menu = page.locator('div[role="menu"][aria-label="Choose the assistant model"]');
      await menu.waitFor({ state: 'visible', timeout: 5_000 });
      ...
      const count = await items.count();
      if (count < 2) {
        console.log('[chat01] only the "Use default" item is available (empty catalog) — skipping selection/cross-surface steps');
        await page.keyboard.press('Escape');
        return;
      }
      const target = items.nth(1);
      pickedAssistantModelName = (await target.locator('span').first().textContent())?.trim() ?? null;
      ...
      assert(label === pickedAssistantModelName, `button label did not update to the picked model: expected "${pickedAssistantModelName}", got "${label}"`);
      await shot('08-model-picker-selected');
    });

    await step('model-picker-outside-click-closes', 'Clicking outside the open menu closes it', async () => {
      const btn = page.locator('button[aria-label="Assistant model"]');
      await btn.click();
      const menu = page.locator('div[role="menu"][aria-label="Choose the assistant model"]');
      await menu.waitFor({ state: 'visible', timeout: 5_000 });
    ...
    if (pickedAssistantModelName) {
      await step('model-picker-cross-surface-settings', 'Settings -> Models -> Agents select shows the same model id set via the composer picker', async () => {
        await page.getByRole('button', { name: /^Settings/ }).first().click();
        await page.getByRole('button', { name: 'Agents' }).waitFor({ state: 'visible', timeout: 10_000 });
        await page.getByRole('button', { name: 'Agents' }).click();
        await page.waitForTimeout(400);
        ...
        for (let i = 0; i < optCount; i++) {
          ...
          if (text === pickedAssistantModelName || text?.startsWith(`${pickedAssistantModelName} ·`)) {
            matchedId = await opt.getAttribute('value');
            break;
          }
        }
        assert(matchedId, `could not find a Settings option matching composer-picked model name "${pickedAssistantModelName}"`);
        pickedAssistantModelId = matchedId;
        ...
        console.log(`[chat01] Settings select value: "${currentValue}" (expected "${matchedId}")`);
        assert(
      ...
      await step('model-picker-reset-to-default', 'Navigate back to Assistant; reset the picker to "Use default"', async () => {
        await navTo(page, 'Assistant');
        await page.getByPlaceholder('Ask your vault anything…').waitFor({ state: 'visible', timeout: 10_000 });
        const btn = page.locator('button[aria-label="Assistant model"]');
        ...
        assert(/^Default · /.test(label), `expected label to read "Default · ..." after reset, got "${label}"`);
        await shot('11-model-picker-reset-to-default');
      });
    } else {
      console.log('[chat01] skipping cross-surface + reset steps (no non-default model available in the catalog)');
    }

    await step('composer-visual-screenshot', 'Full-page screenshot of the redesigned composer for visual review', async () => {
      const input = page.getByPlaceholder('Ask your vault anything…');
      await input.fill('');
    ...
    await step('new-conversation', 'Sidebar "+" starts a fresh conversation', async () => {
      const addBtn = page.locator('span', { hasText: 'Chats · ' }).locator('..').getByRole('button', { name: 'Add' });
      await addBtn.click();
      await page.getByPlaceholder('Ask your vault anything…').waitFor({ state: 'visible', timeout: 10_000 });
    ...
    await step('sidebar-navigate-back', 'Click the prior conversation row in the sidebar to switch back', async () => {
      ...
      assert(text.includes('Here is a file') || text.includes('vault'), 'switching conversation did not restore prior transcript');
      await shot('07-switched-back-to-prior-conversation');
    });

    await step('delete-conversation', 'Delete a conversation from the sidebar via the hover ✕', async () => {
      const row = page.getByRole('button', { name: /^What can you tell me/ }).first();
      ...
      if ((await delBtn.count()) > 0) {
        ...
        assert(countAfter === 'Chats · 0', `expected conversation list to be empty after delete, got "${countAfter}"`);
        await shot('08-after-delete');
      } else {
        console.log('[chat01] delete button not found via hover selector — recording informational note');
      }
    });
