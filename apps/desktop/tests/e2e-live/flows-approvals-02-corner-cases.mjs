...
async function main() {
  ...
  try {
    ...
    await step('setup-install-grant-park-4', 'Install Locker, grant access, create+trash+park 4 items', async () => {
    ...
    await step('flow4-approve-from-approvals', 'Approve one Parked row from Approvals -> row gone, item actually purged in Locker', async () => {
      await goApprovals(page);
      ...
      assert(after === before - 1, `expected row count to drop by 1 after approve, ${before} -> ${after}`);
      ...
      assert(alphaCount === 0, `expected "alpha secret" to be actually purged after Approve, found ${alphaCount}`);
      await shot('03-locker-trash-alpha-gone');
    });

    // ---- Flow 5: Deny "beta secret" from Approvals -> danger confirm overlay ----
    await step('flow5-deny-from-approvals', 'Deny a Parked row -> danger confirm overlay -> confirm -> row gone, item SURVIVES (deny != delete)', async () => {
      await goApprovals(page);
      const before = await page.locator('button', { hasText: 'locker.purge_item' }).count();
      ...
      assert(/can.t be replayed/.test(dialogText ?? ''), `unexpected deny-overlay copy: ${dialogText}`);
      ...
      assert(after === before - 1, `expected row count to drop by 1 after deny, ${before} -> ${after}`);
      ...
      assert(betaCount === 1, `expected "beta secret" to SURVIVE a deny (still in trash), found ${betaCount}`);
    ...
    await step('flow6-cross-surface-approve-from-app', 'Approve from the app Vault tab -> Approvals no longer lists it; badge lags until focus', async () => {
      const fl = await openLocker(page);
      const badgeBeforeText = await page.getByRole('button', { name: /^Approvals/ }).first().textContent();
      console.log(`[appr2] sidebar badge BEFORE app-surface decision (stale, pre-focus): ${JSON.stringify(badgeBeforeText)}`);
      ...
      const gammaCard = dialog.locator('div', { has: page.getByText('locker.purge_item') }).first();
      void gammaCard;
      ...
      console.log(`[appr2] Vault-tab "Waiting for your say-so" section present after approving last-but-one: ${waitingLeft}`);
      ...
      const badgeStaleText = await page.getByRole('button', { name: /^Approvals/ }).first().textContent();
      console.log(`[appr2] sidebar badge immediately after app-surface decision (expect STALE, no focus yet): ${JSON.stringify(badgeStaleText)}`);
      // Then trigger the focus refresh and confirm it catches up.
      await page.evaluate(() => window.dispatchEvent(new Event('focus')));
      await page.waitForTimeout(600);
      const badgeFreshText = await page.getByRole('button', { name: /^Approvals/ }).first().textContent();
      console.log(`[appr2] sidebar badge AFTER focus event: ${JSON.stringify(badgeFreshText)}`);
      assert(badgeFreshText?.trim() === 'Approvals1', `expected badge to read "Approvals1" after focus catch-up, got ${JSON.stringify(badgeFreshText)}`);
      ...
      assert(gammaCount === 0, `expected "gamma secret" purged after app-surface approve, found ${gammaCount}`);
    });

    // ---- Flow 7a: rapid double-click Approve on the last row (delta) --
    // no crash, no double-execution error. ----
    await step('flow7a-rapid-double-click-approve', 'Rapid double-click Approve on a real Parked row -> no crash, no double-execution error', async () => {
      await goApprovals(page);
      await parkedRowToggle(page, 0).click();
      await page.waitForTimeout(200);
      ...
      const emptyVisible = await page.locator('text=Nothing waiting on you.').isVisible().catch(() => false);
      ...
      assert(errorsSoFar.length === 0, `expected no console errors from rapid double-click approve, got: ${JSON.stringify(errorsSoFar)}`);
    ...
    await step('flow7d-repeated-expand-collapse-real-row', 'Park a 5th item, expand/collapse its Approvals row repeatedly -> stable, no console errors', async () => {
      ...
      const errorsSoFar = consoleMessages.filter((m) => m.type === 'error');
      assert(errorsSoFar.length === 0, `expected no console errors from expand/collapse thrash, got: ${JSON.stringify(errorsSoFar)}`);
    });
    ...
    await step('flow7b-relaunch-persistence', 'Relaunch (same userDataDir): does the parked "epsilon secret" invocation survive?', async () => {
      await session.close();
      ...
      await goApprovals(page);
      const parkedAfterRelaunch = await page.locator('button', { hasText: 'locker.purge_item' }).count();
      const emptyAfterRelaunch = await page.locator('text=Nothing waiting on you.').isVisible().catch(() => false);
      console.log(`[appr2] FINDING: parked rows visible after relaunch: ${parkedAfterRelaunch} (empty-state visible: ${emptyAfterRelaunch})`);
      ...
      console.log(`[appr2] "epsilon secret" item itself survived relaunch (durable data): ${epsilonCount === 1}`);
      assert(epsilonCount === 1, `expected "epsilon secret" to still be in trash after relaunch, got ${epsilonCount}`);
    ...
    await step('flow7c-stale-decision-after-decided-elsewhere', 'Decide a parked item out-of-band, then click its now-stale Approve -> graceful, not a crash', async () => {
      const fl = await openLocker(page);
      ...
      assert(Boolean(itemId), 'could not read item_id from the expanded row to correlate the invocation');
      ...
      const outcome = await page.evaluate(async (targetItemId) => {
        ...
        return { status: decideRes.status, body: await decideRes.json().catch(() => null), invocationId: match.invocationId };
      }, itemId);
      console.log(`[appr2] out-of-band decision result: ${JSON.stringify(outcome)}`);
      assert(!outcome.error, `failed to correlate/decide zeta out-of-band: ${JSON.stringify(outcome)}`);
      ...
      assert(outcome.body?.status === 'executed', `expected the out-of-band decision to execute, got ${JSON.stringify(outcome.body)}`);
      ...
      console.log(`[appr2] stale Approve button still visible in this view: ${staleApproveVisible}`);
      if (staleApproveVisible) {
        await staleApprove.click();
        await page.waitForTimeout(700);
        await shot('17-after-clicking-stale-approve');
        const toastText = await page.locator('[data-global-toast]').textContent().catch(() => null);
        console.log(`[appr2] toast/error copy after clicking the stale Approve: ${JSON.stringify(toastText)}`);
        const crashed = await page.evaluate(() => document.title).catch(() => null);
        assert(crashed !== null, 'page appears to have crashed after clicking a stale Approve button');
      }
      const errorsSoFar = consoleMessages.filter((m) => m.type === 'error');
      console.log(`[appr2] console errors after stale-decision corner case: ${JSON.stringify(errorsSoFar)}`);
    });
