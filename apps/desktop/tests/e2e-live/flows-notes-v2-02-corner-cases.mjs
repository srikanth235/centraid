...
async function main() {
  ...
  try {
    ...
    await step('empty-note-attempt', 'Quick-add with no title/body is a no-op (friendly notice, no note created)', async () => {
      const titleInput = frame.locator('.nt-qa-title');
      await titleInput.click();
      ...
      assert(cardCount === 0, `expected 0 notes after an empty-input submit attempt, got ${cardCount}`);
      await shot('empty-note-attempt-result');
    });

    await step('very-long-title-and-body', 'Very long title (300 chars) + multi-KB body saves cleanly, card preview stays bounded', async () => {
      const longTitle = 'L'.repeat(300);
      const longBody = Array.from({ length: 400 }, (_, i) => `Line ${i}: the quick brown fox jumps over the lazy dog.`).join('\n');
      ...
      assert(titleVal.length === 300, `expected 300-char title to round-trip, got length ${titleVal.length}`);
      await shot('long-note-opened-in-editor');
      await frame.locator('.kit-icon-btn[aria-label="Back"]').click();
      await page.waitForTimeout(300);
    });

    await step('special-characters', 'Title/body with quotes, HTML tags, emoji, unicode render as literal text, never interpreted as markup', async () => {
      const weirdTitle = '"quotes" <b>tags</b> & emoji 📝 ünïcödé';
      const weirdBody = 'Body: "double" \'single\' <script>alert(1)</script> & 中文 日本語 émigré — done.';
      ...
      assert(titleText?.includes('<b>'), `expected the literal string "<b>" to render as text, got: ${titleText}`);
      const boldEls = await card.locator('b').count();
      assert(boldEls === 0, 'special-char title was interpreted as HTML (a real <b> element rendered) instead of literal text');
      const scriptEls = await frame.locator('script:has-text("alert(1)")').count();
      assert(scriptEls === 0, 'note body containing <script> was injected into the DOM as a real <script> tag');
      await shot('special-chars-verified-escaped');
    });

    await step('rapid-double-click-submit', 'Rapid double-click "Add note" — busy-guard should prevent a duplicate note', async () => {
      const titleInput = frame.locator('.nt-qa-title');
      await titleInput.click();
      await titleInput.fill('Rapid double-click probe');
      const submit = frame.locator('.nt-qa-actions .kit-btn.primary', { hasText: 'Add note' });
      await submit.waitFor({ state: 'visible', timeout: 5_000 });
      const before = await frame.locator('.nt-card-title', { hasText: 'Rapid double-click probe' }).count();
      ...
      const after = await frame.locator('.nt-card-title', { hasText: 'Rapid double-click probe' }).count();
      console.log(`[notes-v2-02] "Rapid double-click probe" cards before=${before} after=${after}`);
      await shot('rapid-double-click-result');
      if (after > 1) {
        console.log(`[notes-v2-02] BUG-ish finding: rapid double-click on "Add note" produced ${after} duplicate notes — QuickAdd's busy useState guard did not prevent both clicks from landing (likely a React re-render race between the two synchronous click events before setBusy(true) commits).`);
      }
      assert(after >= 1, 'rapid double-click submit produced no note at all');
    });

    await step('escape-mid-quickadd-no-cancel', 'Pressing Escape while the quick-add composer is expanded does NOT close it (only its own Cancel button does) — documenting the behavior', async () => {
      const titleInput = frame.locator('.nt-qa-title');
      await titleInput.click();
      ...
      await page.waitForTimeout(300);
      const moreVisible = await frame.locator('.nt-qa-more').isVisible().catch(() => false);
      console.log(`[notes-v2-02] quick-add "more" section still visible after Escape: ${moreVisible}`);
      ...
      const leftoverCard = await frame.locator('.nt-card-title', { hasText: 'Escape probe' }).count();
      assert(leftoverCard === 0, 'the Escape-then-Cancel probe note was saved, but it should have been discarded');
    });

    await step('edit-then-navigate-away-no-explicit-save', 'Type a title edit then immediately close the editor (Back) before the 700ms debounce fires — confirms flush() saves it anyway (no silent data loss)', async () => {
      ...
      console.log(`[notes-v2-02] title edit survived an immediate close-without-waiting-for-debounce: ${found}`);
      assert(found, 'closing the editor immediately after typing lost the edit — flush() on close did not save it (data loss)');
    });

    await step('edit-then-click-backdrop-no-explicit-save', 'Same probe, but close via clicking the editor backdrop instead of the Back button', async () => {
      ...
      const renamed = frame.locator('.nt-card-title', { hasText: 'Renamed again via backdrop-click' });
      const found = (await renamed.count()) > 0;
      console.log(`[notes-v2-02] title edit survived a backdrop-click close: ${found}`);
      assert(found, 'closing the editor via backdrop click immediately after typing lost the edit');
    });

    await step('delete-currently-open-note', 'Delete the note while its own editor is open — editor should close and the note should vanish from the wall', async () => {
      ...
      const cardGone = await frame.locator('.nt-card-title', { hasText: 'Renamed again via backdrop-click' }).count();
      assert(cardGone === 0, 'deleted note still appears in the wall');
    });

    await step('fifteen-plus-notes-and-many-pins', 'Create 15 more notes (scroll/layout) and pin 6 of them (Pinned group + sidebar count)', async () => {
      ...
      await frame.locator('#scroll, .nt-scroll').first().evaluate((el) => {
      ...
      assert(pinnedCards === 6, `expected exactly 6 cards in the Pinned section, got ${pinnedCards}`);
    });
    ...
    await page.screenshot({ path: path.join(OUT_DIR, 'FATAL-corner-FAILURE.png') }).catch(() => undefined);
    process.exitCode = 1;
  } finally {
    await session.close();
  }
