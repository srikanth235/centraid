...
async function main() {
  ...
  try {
    ...
    await step('open-notes', 'Open Notes iframe', async () => {
      ...
      frame = frameLoc(page);
      await frame
        .locator('.nt-nav-item, [aria-label="New notebook"]')
        .first()
    ...
    await step('create-first-scratchpad', 'Create notebook "Scratchpad"', async () => {
      ...
      assert(
        count === 1,
        `expected exactly 1 "Scratchpad" notebook after first create, got ${count}`,
      );
    });

    await step(
      'duplicate-refused-friendly',
      'Creating a SECOND "Scratchpad" is refused with a friendly message, not a raw error or silent success',
      async () => {
        await frame
        ...
        const noticeText = await frame
          ...
          .catch(() => '');
        console.log(
          `[v02] noticeBanner after duplicate create attempt: ${JSON.stringify(noticeText)}`,
        );
        assert(
        ...
        assert(
          !/name_unused/i.test(noticeText ?? ''),
          `raw precondition name leaked into the UI: ${JSON.stringify(noticeText)}`,
        );
        ...
        assert(
          count === 1,
          `expected still exactly 1 "Scratchpad" notebook (duplicate refused), got ${count}`,
        );
      },
    );
