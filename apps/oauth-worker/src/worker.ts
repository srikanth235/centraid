...
async function callbackResponse(
  ...
  if (!state || !/^[dw]\.[A-Za-z0-9_-]{43}$/u.test(state)) {
    ...
    return finishPage(
      400,
      "Not connected",
      "The authorization return was incomplete."
    );
  }
  const binding = await readBrowserBinding(
    request,
    state,
  ...
  if (providerError || !code) {
    ...
    metric(
      env,
      "callback",
      providerError === "access_denied" ? "denied" : "invalid_code",
      400
    );
  } else {
  ...
  return clearBrowserBinding(
    desktopFinishPage(
      deepLink,
      code
        ? "Return to Centraid to finish connecting."
        : "Return to Centraid and try again."
    ),
    url,
    state
  );
...
function startPage(env: Env): Response {
  ...
  const callbackUrl = JSON.stringify(env.CALLBACK_URL).replace(
    /</gu,
    "\\u003c"
  );
  const allowedScopes = JSON.stringify([
    ...STANDARD_GOOGLE_SCOPES,
    ...(String(env.RESTRICTED_SCOPES_ENABLED) === "true"
      ? RESTRICTED_GOOGLE_SCOPES
      : []),
  ]).replace(/</gu, "\\u003c");
...
async function bindBrowser(
  ...
  const cookie = await mintBrowserBinding(
    state,
    browserBinding,
    env.CALLBACK_RECEIPT_SECRET,
    now
  ...
  return withSecurityHeaders(
    new Response(null, {
      status: 204,
      headers: {
        "set-cookie": serializeBrowserBindingCookie(
          new URL(request.url),
          state,
          cookie
        ),
...
async function verifyReceipt(
  ...
  const match = /^v1\.(?<expiresAt>\d{10})\.(?<mac>[A-Za-z0-9_-]{43})$/u.exec(
    receipt
  );
...
function validEnvironment(env: Env, requestUrl: URL): boolean {
  ...
  return (
    coordinatesValid &&
    /^[A-Za-z0-9._-]{3,512}\.apps\.googleusercontent\.com$/u.test(
...
function validatedScopes(
  ...
  if (
    ...
    value.length === 0 ||
    value.length >
      STANDARD_GOOGLE_SCOPES.size + RESTRICTED_GOOGLE_SCOPES.size ||
