...
describe(approvalBadgeForState, () => {
  it("clears when unpaired or locked", () => {
    expect(
      approvalBadgeForState({ paired: false, locked: false, count: 2 })
    ).toBe("");
    expect(
      approvalBadgeForState({ paired: true, locked: true, count: 2 })
    ).toBe("");
  });
  ...
  it("shows the count when healthy", () => {
    expect(
      approvalBadgeForState({ paired: true, locked: false, count: 4 })
    ).toBe("4");
  });
});
...
describe("isLockerFillMessage / shouldCaptureContextMenu", () => {
  ...
  it("only captures the quick-task menu on a tab with a URL", () => {
    ...
    expect(
      shouldCaptureContextMenu({ menuItemId: "other", tabUrl: "https://x" })
    ).toBe(false);
    expect(
