...
describe("popup connection state", () => {
  it("shows selected modules as paused and unselected modules as revoked", () => {
    ...
    expect(modules.find((module) => module.id === "locker")?.state).toBe(
      "paused"
    );
    expect(modules.find((module) => module.id === "notes")?.state).toBe(
      "paused"
    );
    expect(modules.find((module) => module.id === "tasks")?.state).toBe(
      "revoked"
    );
  });
