...
async function endpoint(): Promise<BrowserEndpoint> {
  if (!endpointPromise) {
    endpointPromise = (async () => {
      ...
      const [stored, pairing] = await Promise.all([
        loadDeviceKey(),
        loadPairing(),
      ]);
...
export async function companionFetch(
  ...
  const responseHeaders = new Headers(
    JSON.parse(response.headers_json) as Record<string, string>
  );
...
export async function appRead<T>(
  app: string,
  query: string,
  input: unknown = {}
): Promise<T> {
...
export async function appWrite<T>(
  app: string,
  action: string,
  input: unknown
): Promise<T> {
