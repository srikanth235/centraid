...
export function assertTopFramePage(
  ...
  if (asOrigin(pageUrl) !== asOrigin(sender.tab.url)) {
    throw new Error(
      "The requested Locker origin does not match the active page."
    );
  }
  if (!isEligiblePageUrl(pageUrl)) {
    throw new Error(
      "Locker is available only on HTTPS pages and local development origins."
    );
  }
}
...
async function candidates(pageUrl: string): Promise<LockerCandidate[]> {
  ...
  return (response.candidates ?? []).filter((candidate) =>
    matchesOrigin(candidate, pageUrl)
  );
}
...
async function fill(itemId: string, pageUrl: string): Promise<FillMaterial> {
  ...
  const response = await appRead<{ fill?: FillMaterial }>(
    "locker",
    "autofill-item",
    {
      item_id: itemId,
...
function pngBytes(dataUri: string): ArrayBuffer {
  ...
  if (!dataUri.startsWith(prefix))
    throw new Error("The tab capture was not a PNG image.");
  const bytes = Uint8Array.from(atob(dataUri.slice(prefix.length)), (char) =>
    char.charCodeAt(0)
  );
  ...
  switch (message.type) {
    ...
    case "capture:task":
      ...
      return appWrite("tasks", "add", {
        title:
          message.capture.selection?.trim() ||
    ...
    case "capture:document": {
      ...
      if (!staged.sha256)
        throw new Error("The screenshot could not be staged.");
