// The four-up ICON grid: the engraved AppIcon emblem over its name.
//
// Home's springboard is content tiles now (./LauncherGrid, issue #708 A) — this
// is what remains for the two surfaces where an app is being *identified*
// rather than *previewed*: the search overlay's filtered results, where a
// content tile would preview eight apps the user did not ask about, and any
// caller that wants the dense catalog view. It keeps the original #498 Slice B
// behaviour verbatim.
//
...
export interface LauncherIconGridProps {
  items: readonly LauncherItem[];
  onOpen: (item: LauncherItem) => void;
}

export default function LauncherIconGrid({
  items,
  onOpen,
}: LauncherIconGridProps): React.JSX.Element {
