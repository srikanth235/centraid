...
// The vault store IS the boundary to the gateway and the device keychain, and
// it is process memory rather than a React tree — so this is the read seam, the
// same place the Photos file seams its timeline. Every screen, shelf rule and
// copy table above it stays real, `openLocker` included as a no-op: on a
// device it opens a session, and a test that let it run would be asserting a
// network.
...
/** The store's shape, at whichever phase the case needs. */
function vault(
...
describe("Locker, on the real React Native host tree", () => {
  ...
  it("withdraws every item from the tree behind a lock, rather than dimming them", () => {
    ...
    // THE WALL. Not `aria-disabled`, not a lowered opacity: absence. Only the
    // real tree can prove the title is gone, and this is the one claim in
    // Locker that most matters — a secret still mounted behind a lock screen is
    // a secret a screen reader can still read out.
  ...
  it("takes the band away with the content behind a lock", () => {
    ...
    // A band drawn over the wall is a way past it. It is not there.
