...
- **The rooms** (#1015) — every Docs surface roots in `kit/rooms`: `DocsHome` in `AppPlace`, every pushed screen in `PushedPage`, the editor in `EditorRoom`. `useDocsRoom` (`docs-room.tsx`) hands a screen the band, the More sheet, the vault lockup and the back target; `DocsScreen.tsx` and `DocsShelfHeader.tsx` are gone. `DocumentViewer` passes no `band` — the stage is the document, and a band under it is a second thing to look at (deviation 2). Stage colors: `colors.stage` / `colors.onStage` / `colors.stageLine` from the theme, never literals.
- **The back target is a `PlaceRef`**, computed from the live stack over `docs-places.ts`. No screen can write it down; the thirteen that used to type it all typed "All".
...
- **Versions' who/what column** — `you` / `Docs` / `a machine` is a `access.provenance` fact this replica does not carry. The chain, dates, kinds and sizes are real; the actor column is absent with `VERSIONS_WHO_WITHHELD` under the list. A denied/failed `core.link` read renders `VERSIONS_ABSENT`, never an empty fabricated history. No diff is drawn anywhere — this seat cannot render a real one.
