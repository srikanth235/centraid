...
/**
 * Why `Copy to Sharing` cannot fire from a phone shelf today. The web says
 * this same sentence when the member's share destination is not mounted
 * (`SelectionBar.tsx`); on the phone there is no Sharing surface at all yet
 * (see `photos-band.ts` — the More sheet carries no Sharing row for exactly
 * this reason), so the target is visible, disabled and honest rather than
 * tappable and silent.
 */
export const NO_SHARE_DESTINATION_REASON =
