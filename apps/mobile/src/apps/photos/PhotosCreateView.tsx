...
const onDesktop = (feature: string): void => {
  Alert.alert(
    ...
    `${feature} lives on the Centraid desktop app for now.`
  );
};
...
export default function PhotosCreateView(): React.JSX.Element {
  ...
  return (
    ...
    >
      <View style={styles.heroCard}>
        ...
        <View style={styles.heroBody}>
          ...
          <Text style={[styles.heroCopy, { color: colors.ink2 }]}>
            Centraid picked 24 clips and photos from your trip and set them to
            music.
          </Text>
      ...
      <Text
        style={[styles.eyebrow, styles.sectionEyebrow, { color: colors.ink3 }]}
      >
        MAKE SOMETHING
      </Text>
      <View style={styles.makeGrid}>
        {MAKE.map((item) => (
          ...
          >
            {item.badge ? (
              <View
                style={[styles.badge, { backgroundColor: colors.bgSunken }]}
              >
            ...
            <View>
              <Text style={[styles.makeTitle, { color: colors.ink }]}>
                {item.title}
              </Text>
              <Text style={[styles.makeSub, { color: colors.ink3 }]}>
                {item.subtitle}
              </Text>
            </View>
          </Pressable>
      ...
      <View style={styles.getList}>
        {GET.map((item, index) => (
          ...
          >
            <View
              style={[styles.getTile, { backgroundColor: colors.bgSunken }]}
            >
              <Feather name={item.icon} size={19} color={colors.ink2} />
            </View>
            <View style={styles.getText}>
              <Text style={[styles.getTitle, { color: colors.ink }]}>
                {item.title}
              </Text>
              <Text style={[styles.getSub, { color: colors.ink3 }]}>
                {item.subtitle}
              </Text>
            </View>
