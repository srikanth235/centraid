...
describe('store', () => {
  ...
  describe(UploadQueueStore, () => {
    it('enqueues an item with one row per part', () => {
      ...
      expect(store.parts(item.itemId).map((part) => part.partNumber)).toStrictEqual([1, 2, 3]);
    ...
    it('returns non-terminal items oldest first, and drops terminal ones', () => {
      ...
      expect(store.pending().map((item) => item.itemId)).toStrictEqual(['item-3']);
    ...
    it('persists canonical follow-ups until their bytes settle and replay completes', () => {
      ...
      expect(store.pendingFollowups().map((followup) => followup.shape)).toStrictEqual(['docs']);
    });
  ...
  `;
    ...
    it('leaves foreign tables alone when it rebuilds its own schema', () => {
      ...
      expect(
        driver
          .all<{ intent_id: string }>('SELECT intent_id FROM replica_intent_outbox')
          .map((row) => ({ ...row })),
