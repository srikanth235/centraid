// One process-wide single-flight guard for EVERY drain entry point (#419.4).
//
// The queue is safe to drain from anywhere, but two drains running at once
// re-PUT the same parts and double-count attempts. Without this guard the
// foreground reconciler, the Android headless task and each media producer
// would each open their own queue and drain independently — the headless task
// in particular spawning a drain on top of a live foreground one.
//
