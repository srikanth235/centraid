...
export const approvedDeviation =
  "Binding Layer font fan-out re-baseline in #707/#708/#709. CI web-e2e on PR #709 head 88ab442f measured cold same-origin shell requests=16 transfer=495485B (PWA WATERFALL SUMMARY). The +4 requests / +~74 KB vs the prior 12 / 470_000 ceilings are the ten self-hosted woff2 faces (Schibsted Grotesk 500/700 × latin/latin-ext, Instrument Serif, Source Serif 4, Spline Sans Mono — each latin + latin-ext; the type overhaul kept the file COUNT at ten but the heavier sans cuts add ~23 KB over the Instrument Sans-era bytes, spent from the same headroom) served from /fonts by the centraid-fonts Vite plugin; they are intentional product identity, not accidental chunk bloat. Prior Vite 8 note (#565) still holds for JS chunking: going below the JS half still needs a web-host.ts source change. maxRequests widens 12 -> 17 (measured 16 + 1); maxTransferBytes widens 470_000 -> 520_000 (measured 495_485 + ~5% headroom). Tighten only if font subsets or faces are cut later.";
