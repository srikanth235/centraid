...
//
// HOW TO UPDATE (read before touching a number): the runbook lives in
// scripts/perf/README.md, "The budgets — and how to update them".
//
// The app-open budgets are measured against a real bundled system app opening
// inline in the shell (#799). Their VALUE is the methodology + the regression
...
export interface PerfBudgets {
   ...
   * (there is no second window and no navigation entry on the inline path,
   * #799). What it fences is the shell's per-app lazy-chunk
...
// ─── MEASURED BASELINE ───────────────────────────────────────────────────────
// apps/web e2e harness, headless Chromium. Per-number provenance and headroom
// rationale are inline on each ceiling; the ratchet record with maintainer
// approval is `approvedDeviation` below.
//
// Vite 8's rolldown bundler splits the shell into many small chunks (#565); a
// `shell-common` group in apps/web/vite.config.ts caps the request count.
...
// ships a BLANK PAGE — with BETTER waterfall numbers (measured: 6 requests /
// 221 KB, app dead). That is why perf-waterfall.spec.ts asserts the app
// renders, and why any chunking work must keep that assertion in front of
// these budgets.
...
export const perfBudgets: PerfBudgets = {
  shell: {
    ...
    //
    // The count includes the self-hosted woff2 faces served under /fonts:
    // resource timing counts each face; transfer includes their compressed
    // bodies. That is product identity load, not a JS-chunk regression — see
    // approvedDeviation.
    //
  ...
  appOpen: {
    ...
    // MEASURED 0.0 — a warm re-open re-pays none of the app's payload; nothing
    // on the inline path is `no-store`. The ceiling catches a change that
    // makes a re-open re-pay some of it.
