  ...
  await page.evaluate(() => {
    ...
    navigator.serviceWorker.addEventListener("message", (event) => {
      ...
      if (msg.target.includes("/gzip-fixture")) {
        ...
        void (async () => {
          const gz = await gzipBytes(
            new TextEncoder().encode("gzip-decoded-body")
          );
          port.postMessage({
            type: "head",
            status: 200,
...
test("a remote 403 revalidation evicts cached blobs for the revoked device", async ({
  page,
}) => {
  ...
  await expect
    .poll(() => page.evaluate((cache) => caches.has(cache), BLOB_CACHE))
    .toBe(true);
  await page.evaluate(() => {
...
test("prewarmed query bundles replay in airplane mode and revalidate with If-None-Match", async ({
  ...
  const url =
    "/__centraid_iroh__/d-bridge-a/centraid/app-fixture/_query/search.mjs";
  expect(
    await page.evaluate(
      (u) =>
  ...
  await expect
    .poll(async () => {
      const calls = await page.evaluate(
        () =>
          (window as unknown as { __calls: { ifNoneMatch: string | null }[] })
            .__calls
      );
...
test("gzip-encoded tunnel responses are decoded before caching", async ({
  page,
}) => {
...
test("remembered app launch, no-store document, and assets replay in airplane mode", async ({
  ...
  expect(new URL(online.url).pathname).toBe(
    "/__centraid_iroh__/d-offline/centraid/app-fixture/"
  );
  ...
  expect(new URL(offline.url).pathname).toBe(
    "/__centraid_iroh__/d-offline/centraid/app-fixture/"
  );
});
  ...
  const result = await page.evaluate(async (src) => {
    ...
    const inlineNextScript = async (index: number): Promise<void> => {
      ...
      const bundle = await fetch(
        new URL(script.getAttribute("src")!, response.url)
      );
    ...
    return new Promise<{ blocked: boolean; opaque: boolean }>(
      (resolve, reject) => {
        const timeout = window.setTimeout(
          () => reject(new Error("sandbox module timeout")),
          5000
        );
...
test("ephemeral bridge scopes neither read nor write tunnel caches", async ({
  page,
}) => {
...
test("unpair purges the tunnel caches but keeps the shell cache", async ({
  page,
}) => {
  await page.evaluate(
    (u) => fetch(u).then((r) => r.text()),
...
test("a fresh worker install purges stale-version caches", async ({ page }) => {
  await page.evaluate(() =>
    caches
      .open("centraid-shell-legacy")
      .then((c) => c.put("/legacy", new Response("old")))
  );
  expect(await page.evaluate(() => caches.has("centraid-shell-legacy"))).toBe(
    true
  );
