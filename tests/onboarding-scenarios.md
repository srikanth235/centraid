...
| C6 | Vault rename fails non-fatally | Make `updateVault` fail (e.g. kill gateway right after commit) | Onboarding still completes; space remains named "Personal"; only a console line `[first-run] renaming the Personal vault failed` — no toast. Decide if silent is acceptable |
...
| F7 | Remove a gateway | Remove the remote; try removing `local` | Remote removal falls back active to `local` (also lifts the latch) and drops its iroh key; `local` → `local_not_removable` |
...
| J5 | Forget a Space does not purge data | Switcher → remove → confirm `Remove` in the `Remove from this phone?` alert | Row + `spaces.ticket.<id>` deleted, but the replica SQLite, thumbnail packs, intent outbox, and freshness cache **remain**. Verify leftover bytes in `PhoneStorage`; candidate bug |
...
| J7 | Dangling active id repair | Corrupt `spaces.activeId` to a removed id; relaunch | `hydrateSpaces()` repairs to `registry[0]` and re-projects the active slot |
...
15. Mobile `ReplicaProvider` rewrites the Space's `gatewayId` to the desktop display name (**H3/H4**).
