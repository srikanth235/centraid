...
| G2 | **⚠️ Keychain survives app delete** (highest-value) | Complete G1 → delete the app (or Maestro `clearState`) → reinstall → launch | **Suspected bug**: `phoneLink.secretKey` / `phoneLink.ticket` are keychain items and survive. `hydrateVaultLinks()` finds an empty registry → `migrateLegacySlot()` sees a surviving hint with `desktopName === ""` → **synthesises a Space with `gatewayId: "desktop"`** and marks it active. Expected-correct behaviour is a clean connect step. Verify first; file if confirmed |
