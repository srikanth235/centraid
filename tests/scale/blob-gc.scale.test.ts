...
import {
  FsBlobStore,
  MemoryBlobStore,
} from "../../packages/vault/src/blob/local.js";
import {
  blobUriFor,
  sha256OfBytes,
...
describe("blob-gc.scale", () => {
  test("large mixed-custody CAS evicts only remotely proven, non-staged local bytes", async () => {
    ...
    try {
      for (let index = 0; index < count; index += 1) {
        ...
        const bytes = Buffer.from(
          `scale-blob-${index.toString().padStart(5, "0")}`
        );
    ...
    expect(
      shasByState.get("remote-only")!.every((sha) => !local.hasSync(sha))
    ).toBe(true);
    expect(
