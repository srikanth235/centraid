...
2. Look at the last screenshot Maestro produced. The harness's own `screenshots/` dir holds whatever your flow captured via `takeScreenshot:`. Maestro's debug dir at `~/.maestro/tests/<timestamp>/` holds the failure screenshot (filename has `❌` in it) and `commands-(*).json` with per-step status and stack traces.
...
- **Don't use `clearState: true` without acknowledging the cost.** It wipes the Expo dev client's Metro URL cache along with AsyncStorage. The shared first-run helpers automatically deep-link iOS back to Metro, accept iOS's native `Open` confirmation, and dismiss the first-use `Continue`/`Reload` developer overlays; use the same `xcrun simctl openurl <udid> "dev.centraid.mobile://expo-development-client/?url=http%3A%2F%2F127.0.0.1%3A8081"` manually when debugging an ad-hoc flow.
