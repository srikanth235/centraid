...
- `ctx.configureGateway({ gatewayUrl?, gatewayToken?, homeCommands?, session?, fresh? }?)` — clear app state, mint a pairing ticket from the declared gateway (ownership: the ticket lands the phone in whichever owner host custody resolves — a fresh gateway founds a placeholder owner, a reused one lands on the owner an earlier flow already named), redeem it through the real ticket-only onboarding UI, and complete the test profile. Optional Maestro `homeCommands` run after the helper's mandatory Home wait, letting a journey own extra evidence. `session: true` keeps the three fresh-pairing phases in one driver process and returns the receipt-derived Connect-to-Home duration; use it for a canary whose budget is about product latency. `fresh: true` is an explicit lifecycle boundary for a suite member that seeded a new gateway fixture and must create a new initial replica even when the suite normally reuses its paired profile. Journeys that need a gateway call this themselves so their prerequisites do not depend on execution order. A live ticket is never retained in Maestro diagnostics; session mode keeps only explicit screenshots from safe checkpoints.
- `ctx.ensureDemo(appId)` — idempotently load a scenario before pairing. The iOS roster proves empty Photos permission denial first, then `ios-roster-bootstrap.mjs` seeds every deterministic app scenario before a fresh full-replica pairing; later app journeys reuse that paired state so Home and content assertions observe the fixtures they claim.
