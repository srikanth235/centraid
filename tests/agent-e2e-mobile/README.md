...
- `ctx.run(yaml, hint?, options?)` — execute a Maestro YAML chunk. Each call spawns `maestro test` once, including an iOS XCUITest driver handshake, so batch directives per call rather than one-per-action. `ctx.runSession([{ yaml, label }], hint?, options?)` runs short ordered phases in one driver process when startup cost would distort the measurement. The harness uses the internal `sensitive` option only for capability-bearing input; it suppresses console/debug retention and keeps the live value in a `MAESTRO_*` variable.
...
- `ctx.configureGateway({ gatewayUrl?, gatewayToken?, homeCommands?, permissionCommands?, fillSampleContent?, session?, fresh? }?)` — clear app state, mint a pairing ticket from the declared gateway (ownership: the ticket lands the phone in whichever owner host custody resolves — a fresh gateway founds a placeholder owner, a reused one lands on the owner an earlier flow already named), redeem it through the real ticket-only onboarding UI, and complete the test profile. Optional Maestro `homeCommands` run after the helper's mandatory Home wait, letting a journey own extra evidence. `fillSampleContent: true` drives Home's real “Fill it with sample content” action when the paired replica is empty; the harness then requires the Photos tile, so fixture setup cannot silently pass. `session: true` keeps the three fresh-pairing phases in one driver process and returns the receipt-derived Connect-to-Home duration; use it for a canary whose budget is about product latency. `fresh: true` is an explicit lifecycle boundary for a suite member that must create a new initial replica even when the suite normally reuses its paired profile. Journeys that need a gateway call this themselves so their prerequisites do not depend on execution order. A live ticket is never retained in Maestro diagnostics; session mode keeps only explicit screenshots from safe checkpoints.
- `ctx.ensureDemo(appId)` — idempotently load a scenario on the gateway for a host-side fixture seam. It does not update an already-paired phone's replica; use `fillSampleContent: true` or pair fresh after seeding when the journey claims phone-visible content. The iOS roster proves empty Photos permission denial first, then `ios-roster-bootstrap.mjs` uses Home's product seed action and a fresh replica; later app journeys reuse that paired state so Home and content assertions observe the fixtures they claim.
...
| `mobile-e2e-ios` (`e2e.yml`) | nightly | iOS | `run-ios-depth-suite.mjs` — native depth plus the 19-flow app-level release roster ([budget](flows/ios-depth-budget.md)) | no |
...
| `run-ios-depth-suite.mjs` (19 flows) | the iOS Release artifact: native depth, every app-level journey, and the cold-start/scroll probes — budget in [flows/ios-depth-budget.md](flows/ios-depth-budget.md) |
