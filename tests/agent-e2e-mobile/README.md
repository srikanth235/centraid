...
- `ctx.configureGateway({ gatewayUrl?, gatewayToken?, homeCommands? }?)` — clear app state, mint a pairing ticket from the declared gateway (ownership: the ticket lands the phone in whichever owner host custody resolves — a fresh gateway founds a placeholder owner, a reused one lands on the owner an earlier flow already named), redeem it through the real ticket-only onboarding UI, and complete the test profile. Optional Maestro `homeCommands` run after the helper's mandatory Home wait in the same safe chunk, letting a journey own extra evidence without another driver launch. Journeys that need a gateway call this themselves so their prerequisites do not depend on execution order. The live ticket is isolated to one Maestro chunk whose diagnostics are never retained; setup and post-submit diagnostics remain available.
