...
For a manual iOS investigation, dispatch the `e2e` workflow with `suite: mobile` and select one `ios_suite` (`native-v0-resilience`, `volume-proof`, `cold-start`, `scroll-frames`, or `photos`). The scheduled workflow keeps `all` and runs every isolated cell; the selector only avoids spending the matrix budget on unrelated green journeys during diagnosis.
...
- **`launchApp: { clearState: true }`** wipes the Expo dev client's cached Metro URL. The harness now re-injects `dev.centraid.mobile://expo-development-client/?url=http%3A%2F%2F127.0.0.1%3A8081` on iOS and dismisses iOS's native `Open` confirmation plus Expo's first-use developer overlays (`Continue`, then `Reload`) before polling for onboarding. If an ad-hoc run still shows a red "No script URL provided" screen, the same recovery can be issued manually with `xcrun simctl openurl <udid> "dev.centraid.mobile://expo-development-client/?url=http%3A%2F%2F127.0.0.1%3A8081"`.
