...
- `ctx.configureGateway({ gatewayUrl?, gatewayToken?, homeCommands?, session? }?)` — clear app state, mint a pairing ticket from the declared gateway (ownership: the ticket lands the phone in whichever owner host custody resolves — a fresh gateway founds a placeholder owner, a reused one lands on the owner an earlier flow already named), redeem it through the real ticket-only onboarding UI, and complete the test profile. Optional Maestro `homeCommands` run after the helper's mandatory Home wait, letting a journey own extra evidence. `session: true` keeps the three fresh-pairing phases in one driver process and returns the receipt-derived Connect-to-Home duration; use it for a canary whose budget is about product latency. Journeys that need a gateway call this themselves so their prerequisites do not depend on execution order. A live ticket is never retained in Maestro diagnostics; session mode keeps only explicit screenshots from safe checkpoints.
...
| `mobile-e2e-ios` (`e2e.yml`) | nightly | iOS | `run-ios-depth-suite.mjs` — native depth plus the 18-flow app-level release roster ([budget](flows/ios-depth-budget.md)) | no |
...
| `run-ios-depth-suite.mjs` (18 flows) | the iOS Release artifact: native depth, every app-level journey, and the cold-start/scroll probes — budget in [flows/ios-depth-budget.md](flows/ios-depth-budget.md) |
