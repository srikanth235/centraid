...
There is no bottom-tab navigator (`apps/mobile/src/navigation.ts`): all eight apps are full-screen covers, and Settings opens from the vault switcher sheet's “Pair another desktop” row. The complete native matrix searches the visible “All apps and places” sheet for every app, including Photos and Docs. That distinction matters on an empty-vault Home: “Bring in photographs” and “Bring in documents” are import offers, not app-opening labels. This keeps the journey on user-visible launcher contracts instead of depending on an iOS URL handoff after an Expo development-client relaunch. Each destination is asserted on a string that only the opened screen publishes ("Search photos", "Add document or folder", "Create event", "APPEARANCE"), never on a launcher or sheet label. Covers dismiss with a native swipe-down gesture Maestro cannot drive, so each surface starts from a fresh launch; React Navigation state is not persisted, so every launch lands on Home.
...
**Automated steps:** configure the declared gateway; launch without clearing state; open all eight blueprint covers through the searchable All-apps sheet (with Settings from the vault switcher's “Pair another desktop” row); force-stop and relaunch without clearing state; assert Home returns. This catches navigation regressions and verifies that replica/upload databases outlive the process.
