...
the Home springboard, all eight blueprint covers (Photos, Docs, Agenda, Tasks,
People, Notes, Tally, and Locker), and Settings.

There is no bottom-tab navigator (`apps/mobile/src/navigation.ts`): all eight
apps are full-screen covers launched from Home's tiles, and Settings opens from
the space drawer. Each surface is therefore entered the way a user enters it —
by the tile's `Open <name>` accessibility label — and asserted on a string that
only the opened screen publishes ("Search photos", "Add document or folder",
"Create event", "APPEARANCE"), never on the tile or drawer label, which can be
visible before the destination opens. Covers dismiss with a native swipe-down
gesture Maestro cannot drive, so each surface starts from a fresh launch; React
Navigation state is not persisted, so every launch lands on Home.
...
onboarding, accepting the named-roster direct completion or the unnamed-profile
branch, before exercising the shell. For the manual matrix, grant
...
state; open all eight blueprint covers from Home plus Settings from the space
drawer; force-stop and relaunch without clearing state; assert Home returns.
This catches navigation
