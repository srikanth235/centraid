...
- Everything `home-loads.md` needs (installed dev build, booted device, Metro on `:8081`).
...
5. Assert the shell's error states ("Could not load app", "Not connected") are absent, screenshot as `<appId>.png`.

**Scope — which apps are gated.** `photos`, `docs` and `agenda` are implemented NATIVELY by the mobile shell (`NATIVE_APPS` in `apps/mobile/src/screens/Home.tsx`; `openApp` navigates to a native screen for each), so they have no web document and this gate's premise does not apply. They are excluded rather than asserted loosely — a gate that cannot fail is worse than one that does not run. That leaves five WebView apps: tasks, people, notes, locker, tally. Keep `NATIVE_ON_MOBILE` in the flow in sync with `NATIVE_APPS`; drift means a WebView app silently goes ungated.
