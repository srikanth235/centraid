...
1. Launch `com.centraid.mobile` with `clearState: true` → AsyncStorage
   wiped, so Home renders in its `no-gateway` branch.
2. Wait up to 30s for `"Open Settings"` text — only present after the
   JS bundle has downloaded from Metro and `<Home>` has mounted.
3. Take screenshot `home-fresh` (lands under `runs/<runId>/screenshots/`).
4. Assert both `"Open Settings"` and `"Connect to your desktop."` are
   visible — these are the unique markers of the `no-gateway` state.
