...
**Selectors** ([#890](https://github.com/srikanth235/centraid/issues/890) W2): the band's Waiting slot is tapped by its **key**, `tally-band-contrib` — `shelves.ts` owns the label `Waiting` and may re-word it, while the key is what `tally-band.ts` already keys on. Every assertion above stays copy, deliberately: all four claims are about the words the seat publishes, and two of them are about words it must NOT publish.
