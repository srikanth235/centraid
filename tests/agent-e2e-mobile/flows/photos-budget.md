...
The five `photos-*.mjs` journeys share one gateway process and one paired simulator profile. `run-photos-suite.mjs` pairs during the denied-permission empty-vault journey, then the library journey loads the deterministic Photos scenario and the remaining journeys reuse that paired profile. The runner fails when aggregate wall time **exceeds eight minutes**, measured from the first flow process start through the fifth verdict. Every journey still writes an independent verdict, including after an earlier failure.
