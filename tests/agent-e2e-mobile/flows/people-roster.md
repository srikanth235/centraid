...
**Selectors** ([#890](https://github.com/srikanth235/centraid/issues/890) W2): the arrival marker is now `people-row-first`, the leading row's handle — it can only exist where People drew a list, which the app's own name (drawn by the launcher tile too) could not tell apart from a tap that did nothing. All four rows, both roles and the cadence line stay asserted by their own words: those are the vault's strings and the join this flow exists to catch.
