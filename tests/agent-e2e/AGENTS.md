...
1. Read the existing renderer source for the screen you're testing — selectors live there. Don't guess class names. [`apps/desktop/src/renderer/app.ts`](../../apps/desktop/src/renderer/app.ts) and [`apps/desktop/src/renderer/builder.ts`](../../apps/desktop/src/renderer/builder.ts) are the two big ones.
