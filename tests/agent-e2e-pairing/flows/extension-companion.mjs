...
try {
  await runFlow("extension-companion", async (ctx) => {
    ...
    if (!install.ok)
      throw new Error(
        `Locker install failed: ${install.status} ${await install.text()}`
      );
    const add = await fetch(
      `${ctx.gateway.url}/centraid/locker/actions/add-item`,
      {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    try {
      ...
      if (!pairing?.endpointId)
        throw new Error("extension stored no paired endpoint identity");
      ctx.note(
        `MV3 worker paired endpoint ${pairing.endpointId.slice(0, 10)}… over browser iroh`
      );
      ...
      if (coldMs > 2_000)
        throw new Error(
          `cold fill exceeded 2s budget: ${Math.round(coldMs)}ms`
        );
      if (
      ...
      if (
        !/^\d{6}$/u.test(await page.locator('input[name="otp"]').inputValue())
      )
      ...
      if (warmMs > 500)
        throw new Error(
          `warm fill exceeded 500ms budget: ${Math.round(warmMs)}ms`
        );
      ctx.note(
        `fill budgets held: cold ${Math.round(coldMs)}ms, warm ${Math.round(warmMs)}ms`
      );

      const review = await fetch(
        `${ctx.gateway.url}/centraid/_vault/review?limit=20`,
        { headers }
      );
      ...
      if (
        !entries.some(
          (entry) =>
            entry.action === "reveal" &&
            entry.context?.origin === new URL(loginUrl).origin
        )
      ) {
        throw new Error(`review feed has no fill reveal for ${loginUrl}`);
      }
      ctx.note(
        "origin-bearing Locker reveal is visible in the owner review feed"
      );
      ...
      if (pairingAfter)
        throw new Error(
          "remote revocation did not purge extension pairing state"
        );
      if (await page.locator('input[name="password"]').inputValue())
        throw new Error("revoked fill still populated password");
      ctx.note(
        "remote revocation killed the next fill and purged local pairing state"
      );
    } finally {
      await context.close();
    }
