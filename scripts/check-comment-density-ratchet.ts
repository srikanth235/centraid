...
export type DensityPin = readonly [number, number] | number[];

export type DensityBaseline = {
  _comment?: string;
  _entries?: string;
  issue?: number;
  expires?: string;
  ...
  files?: Record<string, DensityPin>;
};
 ...
 */
export function commentRanges(text: string, fileName: string) {
...
export const capEligible = (measurement: DensityMeasurement): boolean =>
  measurement.nonBlankLines >= CAP_MIN_LINES;

export const overCap = (measurement: DensityMeasurement): boolean =>
...
export function measureTree({
  ...
  measured: Map<string, DensityMeasurement>;
  totals: {
    files: number;
    commentChars: number;
    totalChars: number;
  ...
  const measured = new Map<string, DensityMeasurement>();
  const totals = {
    files: 0,
    commentChars: 0,
    totalChars: 0,
...
export const rose = (
  ...
  const pinComments = pin[0];
  const pinTotal = pin[1];
  return (
    pinComments !== undefined &&
    pinTotal !== undefined &&
    measurement.commentChars * pinTotal > pinComments * measurement.totalChars
  );
};

const percent = (commentChars: number, totalChars: number): string =>
 ...
 */
export function reconcileRatchet(
  baseline: DensityBaseline,
  measured: Map<string, DensityMeasurement>
): { next: DensityBaseline; refused: string[] } {
  const pins = baseline.files ?? {};
  const files: Record<string, DensityPin> = {};
  ...
  for (const rel of [...measured.keys()].sort()) {
    ...
    files[rel] = [measurement.commentChars, measurement.totalChars] as const;
  }
  return {
...
export function loadBaseline(root = ROOT): DensityBaseline {
  const section = readLedgerSection(INVENTORY_PATH, BASELINE_SECTION, root);
  if (isDensityBaseline(section)) return section;
  return {
...
function isDensityBaseline(value: unknown): value is DensityBaseline {
