...
import {
  ...
  parseVocabulary,
  templateLines,
} from "./lint-mobile-testids.ts";

/** A Maestro chunk as a flow writes one: a template with an `appId`/`---` head. */
const chunk = (body: string) => `await ctx.run(\`appId: x\n---\n${body}\n\`);`;
...
const rules = (result: ReturnType<typeof lintTestIds>) =>
...
test("a referenced-but-absent id fails", () => {
  ...
  assert.match(
    ...
    /no screen in apps\/mobile\/src/u
  );
});
...
test("isAllowed does not reach back past the previous step", () => {
  ...
  assert.equal(isAllowed(text, ids[0]?.line ?? 0, "missing-id"), true);
  assert.equal(isAllowed(text, ids[1]?.line ?? 0, "missing-id"), false);
});
