...
type GateResult = { name: string; code: number; ms: number; out: string };
 ...
 */
function runOne(name: string): Promise<GateResult> {
...
): Promise<{ results: GateResult[]; failed: string[]; ms: number }> {
