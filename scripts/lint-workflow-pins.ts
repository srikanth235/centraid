...
function exemptUses(ref: string): boolean {
 ...
 */
function isClosedOnlyPullRequestTrigger(
  lines: string[],
  pullRequestLineIndex: number
): boolean {
  let sawTypes = false;
  let typesBlob = "";
    ...
    if (typesMatch) {
      ...
      if (typesBlob === "" || typesBlob.startsWith("#")) {
        ...
        while (j < lines.length) {
          ...
          if (itemLine === undefined || !/^ {6}-\s+\S/u.test(itemLine)) break;
          items.push(
            itemLine
              .replace(/^ {6}-\s+/u, "")
              .replace(/\s+#.*$/u, "")
...
export function lintWorkflowSource(name: string, source: string): string[] {
  const found: string[] = [];
...
function lintSetupAction(): string[] {
  const found: string[] = [];
...
function lintPackageManager(): string[] {
  const pkg: unknown = JSON.parse(
    readFileSync(path.join(root, "package.json"), "utf8")
  );
  const packageManager = isRecord(pkg) ? pkg.packageManager : undefined;
  const pin = typeof packageManager === "string" ? packageManager : "";
  if (!/^bun@\d+\.\d+\.\d+$/u.test(pin)) {
    return [
      `package.json packageManager must be \`bun@<x.y.z>\`, got \`${pin}\``,
    ];
  }
  return [];
}
