 ...
 *
 * Usage:
 *   node scripts/perf/send-to-first-token.ts             # enforce the ceiling
 *   node scripts/perf/send-to-first-token.ts --report    # print, never fail
 *   node scripts/perf/send-to-first-token.ts --samples 40 --warmup 5
 */
 ...
 */
export function compareToCeiling(
  p95Ms: number,
  metric: unknown
 ...
 * assistant token.
 * @param {(input: unknown, config: string) => Promise<unknown>} runAcpTurn Turn driver.
 * @returns {Promise<number>} Milliseconds from send to first token.
 */
async function sample(
  runAcpTurn: (input: unknown, config: string) => Promise<unknown>
): Promise<number> {
...
async function main(): Promise<number> {
  ...
  const driver = runAcpTurn as unknown as (
    i: unknown,
    c: unknown
  ) => Promise<unknown>;
