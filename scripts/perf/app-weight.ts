 ...
 *
 * Usage:
 *   node scripts/perf/app-weight.ts --surface desktop
 *   node scripts/perf/app-weight.ts --surface mobile
 *   node scripts/perf/app-weight.ts --surface web
 *   node scripts/perf/app-weight.ts --surface desktop --report   # print, never fail
 */
...
function isDebugSidecar(file: string, extraSuffixes: string[] = []) {
...
async function weighDirectory(absRoot: string, extraSuffixes: string[] = []) {
...
function compareWeight(
  weighed: { shippedBytes: number; largestChunkBytes: number },
  budget: { maxTotalBytes?: unknown; maxLargestChunkBytes?: unknown },
  label: string
): string[] {
  const errors: string[] = [];
  if (
...
function parseArgs(argv: string[]): { surface?: string; report: boolean } {
  const out: { surface?: string; report: boolean } = { report: false };
...
async function main() {
  ...
  const surface =
    args.surface !== undefined && args.surface in SURFACES
      ? SURFACES[args.surface as keyof typeof SURFACES]
      : undefined;
  if (!surface) {
    console.error(
  ...
  const weighed = await Promise.all(
    surface.roots.map(async (rel: string) => {
      ...
      return present
        ? {
            rel,
            ...(await weighDirectory(
              abs,
              "extraDebugSuffixes" in surface &&
                Array.isArray(surface.extraDebugSuffixes)
                ? surface.extraDebugSuffixes.filter(
    ...
    errors.push(
      ...compareWeight(
        entry,
        {
          ...
          maxLargestChunkBytes: budget.maxLargestChunkBytes,
        },
        entry.rel
      )
    );
