...
//
// Usage:
//   node scripts/perf/run-waterfall.ts            # all perf tests
//   node scripts/perf/run-waterfall.ts --shell    # just the waterfall test
//   PWDEBUG=1 node scripts/perf/run-waterfall.ts  # headed / inspector
...
console.log(
  "\nReport: apps/web/test-results/perf-waterfall-report.json" +
    "\nSummary: node scripts/perf/summarize.ts"
);
