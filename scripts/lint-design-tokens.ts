...
import { readLedgerSection, writeLedgerSection } from "./check-ledgers.ts";
...
export type DesignTokenMetric = (typeof METRICS)[number];
export type DesignTokenCounts = Record<DesignTokenMetric, number>;
...
function countPaletteHueAsText(css: string) {
...
const isTokened = (value: string): boolean => value.includes("var(--");
...
function countRawFontSize(css: string) {
...
function countRawFontWeight(css: string) {
...
function countRawRadius(css: string) {
  const matches = [
...
function walkCss(directory: string, out: string[] = []): string[] {
...
export function compareBudget(
  actual: Record<string, Partial<DesignTokenCounts>>,
  budget: unknown
): string[] {
  const findings: string[] = [];
  const budgetRecord = isRecord(budget) ? budget : {};
  for (const [file, counts] of Object.entries(actual)) {
    const allowedRaw = budgetRecord[file];
    const allowed = isRecord(allowedRaw) ? allowedRaw : {};
    for (const metric of METRICS) {
      const found = counts[metric] ?? 0;
      const limit = typeof allowed[metric] === "number" ? allowed[metric] : 0;
      if (found > limit)
  ...
  for (const file of Object.keys(budgetRecord)) {
    if (!(file in actual))
...
export function scanDesignTokenCss(
  root = ROOT
): Record<string, Partial<DesignTokenCounts>> {
  const actual: Record<string, Partial<DesignTokenCounts>> = {};
...
export function formatTotals(
  ...
  const totals: DesignTokenCounts = Object.fromEntries(
    METRICS.map((metric) => [metric, 0])
  ) as DesignTokenCounts;
...
function main() {
  ...
  if (process.argv.includes("--write")) {
    ...
    const current = readLedgerSection(BUDGET_SECTION_PATH, BUDGET_SECTION);
  ...
  const section = readLedgerSection(BUDGET_SECTION_PATH, BUDGET_SECTION);
  const budget =
    isRecord(section) && isRecord(section.budgets) ? section.budgets : {};
