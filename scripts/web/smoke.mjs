...
if (existsSync(path.join(dist, "manifest.webmanifest"))) {
  const m = JSON.parse(
    readFileSync(path.join(dist, "manifest.webmanifest"), "utf8")
  );
  ok(
    m.id === "/" || m.id === undefined || m.id === "",
    "manifest id is / (K5) or default"
  );
  if (m.id !== undefined)
    ok(m.id === "/", 'manifest.id is "/" (K5 sticky install identity)');
}
