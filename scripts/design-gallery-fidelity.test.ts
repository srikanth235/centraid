...
import type { FidelityRecord } from "./design-gallery-fidelity.ts";
import {
  BIDI_PROBE_NUMERALS,
  BIDI_PROBE_WORD,
  CJK_SAMPLE,
  ...
  normalizeStack,
  probeBidiInPage,
} from "./design-gallery-fidelity.ts";
...
function record(overrides: Partial<FidelityRecord> = {}): FidelityRecord {
  return {
...
test("margins and border widths are read on the same terms as padding", () => {
  const cases: Array<[keyof FidelityRecord, keyof FidelityRecord, RegExp]> = [
...
test("a numeric element that lost its isolate is caught", () => {
  ...
  assert.match(
    findings[0] ?? "",
    /the digits reorder against the surrounding run/u
  );
});
...
test("a latin-only stack under CJK copy is caught", () => {
  ...
  assert.match(
    findings[0] ?? "",
    /mandatory CJK fallbacks are not in the stack/u
  );
});
...
async function mount(html: string): Promise<Document> {
...
test("the CJK localizer swaps copy and leaves the structure alone", async () => {
  ...
  assert.ok(paragraph);
  assert.equal(paragraph.textContent?.includes(" "), false);
  const bold = host.querySelector("b");
  const boldText = bold?.textContent ?? "";
  assert.equal(CJK_SAMPLE.includes(boldText[0] ?? ""), true);
