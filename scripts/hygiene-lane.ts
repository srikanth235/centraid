...
import { runGates } from "./lint-product.ts";
 ...
 */
type GateRun = { name: string; code: number; ms: number; out?: string };
export type HygieneSummary = {
  ...
  verdict: string;
  red: string[];
  gates: Array<{ gate: string; verdict: string; durationMs: number }>;
};

export function summarize(
  results: readonly GateRun[],
  at: string
): HygieneSummary {
  const gates = [...results]
 ...
 */
export function gateTable(summary: HygieneSummary): string {
  return [
 ...
 */
export function summaryMarkdown(summary: HygieneSummary): string {
 ...
 */
export function issueBody(summary: HygieneSummary, runUrl: string): string {
  const lines = [
