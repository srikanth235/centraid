...
import type { Roster } from "../tests/agent-e2e-mobile/lib/roster.ts";
import { wiringSelfTestCases } from "./lint-e2e-wiring.cases.ts";
import {
  discoverFlows,
  discoverRunners,
  resolveReach,
} from "./lint-e2e-wiring.reach.ts";
import type { ReadFile, WiringLane } from "./lint-e2e-wiring.reach.ts";
import {
  corpusProblems,
  discoverApps,
  LANE_PREAMBLE,
...
} from "./lint-e2e-wiring.rules.ts";
import type { CorpusApp } from "./lint-e2e-wiring.rules.ts";

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}

export type WiringFinding = { rule: string; message: string };
...
export function matrixMobileOwners(matrix: unknown): Map<string, string[]> {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      if (
        ...
        if (match) {
          ...
          if (!flow) continue;
          const list = owners.get(flow);
          if (list) list.push(here);
          else owners.set(flow, [here]);
        }
        continue;
      }
      walk(value, here);
...
export function lintWiring(input: LintWiringInput): {
  findings: WiringFinding[];
  laneCount: number;
  flowLanes: Map<string, Set<string>>;
} {
  const { roster, flows, runners, matrix, apps, readFile } = input;
  ...
  const lanes = roster.lanes as unknown as Record<string, WiringLane>;
  ...
  let reach = {
    flowLanes: new Map<string, Set<string>>(),
    runnerLanes: new Map<string, Set<string>>(),
  };
  try {
    reach = resolveReach(lanes, readFile, roster);
  } catch (error) {
    fail("lane", error instanceof Error ? error.message : String(error));
  }
  ...
  for (const [flow, entry] of Object.entries(declared)) {
    ...
    if (entry.status === "promoting") {
      ...
      if (
        !entry.since ||
        !(typeof entry.nights === "number" && entry.nights > 0)
      ) {
        fail(
          "promoting",
    ...
    if (typeof entry.claim !== "string" || entry.claim.length < 20) {
      fail(
        "rostered",
...
function selfTest() {
  ...
  const { flows, runners, apps, readFile, cases } = wiringSelfTestCases({
    FLOWS_DIR,
    LANE_PREAMBLE,
    MOBILE_DIR,
    SEEDER,
  ...
  for (const testCase of cases) {
    ...
    const extraFiles = testCase.files;
    const caseRead: ReadFile = extraFiles
      ? (rel) => extraFiles[rel] ?? readFile(rel)
      : readFile;
    const got = [
      ...new Set(
        lintWiring({
          roster: testCase.roster as Roster,
...
function main() {
  ...
  const roster = JSON.parse(readFile(ROSTER_PATH)) as Roster;
