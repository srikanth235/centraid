...
import { lintTsconfigs } from "./lint-tsconfigs.ts";
...
const rootTypecheckScripts = json({
  ...
  scripts: {
    typecheck:
      "tsc -p tests && tsc -p scripts && tsc -p scripts/tsconfig.pricing.json && tsc -p scripts/tsconfig.design.json",
    "typecheck:affected":
      "tsc -p tests && tsc -p scripts && tsc -p scripts/tsconfig.pricing.json && tsc -p scripts/tsconfig.design.json",
  },
});
...
test("site-tokens.ts requires the design bundler program in typecheck", (t) => {
  const root = fixture(t, {
    "package.json": json({
      name: "centraid",
      scripts: {
    ...
    "tsconfig.node.json": nodeProfile,
    "scripts/tsconfig.json": scriptsProgram,
    "scripts/site-tokens.ts": "export {};\n",
  });
  assert.deepEqual(lintTsconfigs(root).sort(), [
    "package.json: typecheck must target tsc -p scripts/tsconfig.design.json",
    "package.json: typecheck:affected must target tsc -p scripts/tsconfig.design.json",
    "scripts/tsconfig.design.json: missing Node tooling program",
  ]);
});
