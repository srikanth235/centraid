...
export type DeclaredAct = { app: string; act: string; manifest: string };
export type SeatVerbFinding = { rule: string; message: string };
...
export function declaredActs(root = ROOT): Map<string, DeclaredAct> {
  const acts = new Map<string, DeclaredAct>();
  ...
  for (const app of readdirSync(appsDir).sort()) {
    ...
    const seats =
      isRecord(manifest) && isRecord(manifest.seats) ? manifest.seats : {};
    const originActs = Array.isArray(seats.originActs) ? seats.originActs : [];
    for (const act of originActs) {
...
export function ageInDays(since: unknown, now = Date.now()): number | null {
...
type TrackingIssue = { state?: unknown };
type LintSeatVerbsInput = {
  acts: Map<string, DeclaredAct>;
  register: { acts?: Record<string, unknown> };
  scheduledFlows: Set<string>;
  existingFlows: Set<string>;
  trackingIssues: Record<string, TrackingIssue>;
  now?: number;
};

export function lintSeatVerbs(input: LintSeatVerbsInput): SeatVerbFinding[] {
  const {
    acts,
    register,
    scheduledFlows,
  ...
  const findings: SeatVerbFinding[] = [];
  ...
  for (const [key, row] of Object.entries(rows)) {
    ...
    if (row.gap === true) {
      ...
      if (!(typeof row.blocker === "string" && row.blocker.length > 60)) {
        fail(
          "dated-gap",
    ...
    if (!(typeof row.assertion === "string" && row.assertion.length > 20)) {
      fail(
        "owned-is-real",
...
function main() {
  ...
  const registerRaw: unknown = JSON.parse(read(REGISTER_PATH));
  const roster: unknown = JSON.parse(read(ROSTER_PATH));
  const matrix: unknown = JSON.parse(read(MATRIX_PATH));
  const register = {
    acts:
      isRecord(registerRaw) && isRecord(registerRaw.acts)
        ? registerRaw.acts
        : {},
  };
  const rosterRecord = isRecord(roster) ? roster : {};
  const rosterFlows = isRecord(rosterRecord.flows) ? rosterRecord.flows : {};
  const matrixRecord = isRecord(matrix) ? matrix : {};
  const trackingIssuesRaw = isRecord(matrixRecord.trackingIssues)
    ? matrixRecord.trackingIssues
    : {};
  const trackingIssues: Record<string, TrackingIssue> = Object.fromEntries(
    Object.entries(trackingIssuesRaw).map(([id, entry]) => [
      id,
  ...
  const scheduledFlows = new Set(
    Object.entries(rosterFlows)
      .filter(([, entry]) => isRecord(entry) && entry.status !== "exploratory")
      .map(([flow]) => flow)
  );
  ...
  const owned = rows.filter((row) => !isRecord(row) || row.gap !== true).length;
  console.log(
