...
import {
  ...
  scanSelectionFiles,
} from "./lint-engine-conformance.ts";
...
const BUTTON_LANE = (ledger: Record<string, number>) => [
  {
    name: "class-less <button>",
    ledger,
    scope: (label: string) => label.startsWith("packages/client/src/"),
    count: (code: string) =>
...
test("component-existence ledger rejects an instance it does not carry", () => {
  ...
  assert.match(
    findings[0] ?? "",
    /1 class-less <button> where the ledger allows 0/u
  );
