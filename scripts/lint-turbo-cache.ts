 ...
 */
export function trackedMatches(
  glob: string,
...
export function lintTurboOutputs(
  turboConfig: unknown,
  packageDirs: readonly string[],
  tracked: readonly string[]
): string[] {
  const errors: string[] = [];
  const tasks =
    isRecord(turboConfig) && isRecord(turboConfig.tasks)
      ? turboConfig.tasks
      : {};
  for (const [task, definition] of Object.entries(tasks)) {
    const outputs =
      isRecord(definition) && Array.isArray(definition.outputs)
        ? definition.outputs
        : [];
...
export function workspaceDirs(
  ...
  const patterns = Array.isArray(workspaces)
    ...
    : isRecord(workspaces) && Array.isArray(workspaces.packages)
      ? workspaces.packages.filter((p): p is string => typeof p === "string")
      : [];
...
function main() {
  ...
  const config: unknown = JSON.parse(stripJsonComments(source));
  ...
  const packageDirs = workspaceDirs(pkgRecord.workspaces ?? [], (dir) => {
    try {
