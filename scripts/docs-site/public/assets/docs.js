...
(() => {
  ...
  if (searchBtn) {
    ...
    const render = async () => {
      ...
      if (!pf) {
        hits = [];
        results.innerHTML =
          '<p class="search-empty">Search is unavailable in this build.</p>';
        return;
      }
      ...
      hits = pageResults
        ...
        .filter(
          (hit, index, all) =>
            all.findIndex((other) => other.href === hit.href) === index
        )
        .slice(0, 12);
      selected = 0;
      if (!hits.length) {
  ...
  if (navToggle && mobileMenu) {
    ...
    const syncSections = () => {
      ...
      for (const a of rail.querySelectorAll("a"))
        mmRail.appendChild(a.cloneNode(true));
    ...
    window.addEventListener("resize", () => {
      if (window.innerWidth > 1000 && mobileMenu.classList.contains("open"))
        setOpen(false);
    });
  }
  ...
  document.querySelectorAll(".copy-btn").forEach((btn) => {
    ...
    btn.addEventListener("click", async () => {
      const text =
        btn.dataset.copy ||
        btn.closest(".term")?.querySelector("code")?.textContent ||
        "";
      try {
