...
async function siteExists(rel: string) {
  try {
...
async function resolves(clean: string, fromPage: string) {
...
const pages = (await walk(outDir)).filter((f: string) => f.endsWith(".html"));
...
const validatePage = async (index: number) => {
