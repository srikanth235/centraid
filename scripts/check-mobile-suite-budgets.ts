 ...
 */
export function observedSuiteMs(
  runner: { flows: string[] },
  ...
  return parts.reduce((total, samples) => {
...
export function groupByFlow(ledger: unknown): Map<string, number[]> {
  const byFlow = new Map<string, number[]>();
  const records =
    isRecord(ledger) && Array.isArray(ledger.records) ? ledger.records : [];
  for (const raw of records) {
    if (!isRecord(raw) || raw.pass !== true) continue;
    const flowName =
...
export function baseBudgetMs(
  ...
  for (const ref of ["origin/main", "main"]) {
    ...
    if (roster) {
      const parsed: unknown = JSON.parse(roster);
      const suites =
        isRecord(parsed) && isRecord(parsed.suites) ? parsed.suites : {};
      const suiteSpec = isRecord(suites[suite]) ? suites[suite] : {};
      const base = suiteSpec.budgetMs;
      if (typeof base === "number") return base;
      const inheritedSpec =
        typeof supersedes === "string" && isRecord(suites[supersedes])
          ? suites[supersedes]
          : {};
      const inherited = inheritedSpec.budgetMs;
    ...
    for (const candidate of candidates) {
      ...
      if (match) {
        ...
        if (!minutes) continue;
        return Number(minutes.replaceAll("_", "")) * 60_000;
      }
    }
...
type CheckBudgetsInput = {
  runners: SuiteRunner[];
  byFlow: Map<string, number[]>;
  baseOf: (id: string, supersedes: unknown) => number | null;
};

export function checkBudgets(input: CheckBudgetsInput): string[] {
  const { runners, byFlow, baseOf } = input;
...
function selfTest() {
  ...
  const runner = (budgetMinutes: number, flows: string[]): SuiteRunner => ({
    file: "r.mjs",
