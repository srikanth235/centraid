...
type HandshakeInfo = {
  ...
  minSupportedProtocol: unknown;
  capabilities: unknown;
} & Record<string, unknown>;

type HandshakeResult =
  | { ok: true; info: HandshakeInfo }
  | { ok: false; reason: string; detail: string };

export const PROTOCOL_TARGETS: FuzzTarget[] = [
  {
    ...
    async load() {
      const mod = await importByPath(
        "packages/core/dist/protocol/index.js",
        BUILD
      );
      const judgeGatewayInfo = moduleFn<(value: unknown) => HandshakeResult>(
        mod,
        "judgeGatewayInfo"
      );
      const readProtocolFromInfo = moduleFn<
        (value: Record<string, unknown>) => Record<string, unknown>
      >(mod, "readProtocolFromInfo");
      const isGatewayCapabilities = moduleFn<(value: unknown) => boolean>(
        mod,
      ...
      return (bytes: Uint8Array) => {
        ...
        if (
          ...
          const read = readProtocolFromInfo(value as Record<string, unknown>);
          for (const field of ["protocolVersion", "minSupportedProtocol"]) {
            const protocol = read[field];
            invariant(
              protocol === null || Number.isSafeInteger(protocol),
              "handshake.protocol-read",
              `readProtocolFromInfo returned ${String(protocol)} for ${field}`
            );
          }
        }
  ...
  },
  {
    ...
    async load() {
      const mod = await importByPath("packages/tunnel/dist/protocol.js", BUILD);
      const parsePairQrPayload = moduleFn<
        (text: string) => Record<string, unknown> | undefined
      >(mod, "parsePairQrPayload");
      const encodeHeaderFrame = moduleFn<(header: unknown) => number[]>(
        mod,
        "encodeHeaderFrame"
      );
      return (bytes: Uint8Array) => {
