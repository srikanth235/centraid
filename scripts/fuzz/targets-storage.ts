...
export const STORAGE_TARGETS: FuzzTarget[] = [
  {
    id: "cbsf-directory",
    ...
    async load() {
      const mod = await importByPath("packages/core/dist/blob/index.js", BUILD);
      const decodeCbsfDirectory = moduleFn<
        (bytes: Uint8Array, frameCount: number) => CbsfDirectory
      >(mod, "decodeCbsfDirectory");
      const encodeCbsfDirectory = moduleFn<
        (
          frameSize: number,
          totalSize: number,
          sealedLens: number[]
        ) => Uint8Array
      >(mod, "encodeCbsfDirectory");
      ...
      const frameCountsFor = (bytes: Uint8Array): number[] => {
        const exact =
      ...
      return (bytes: Uint8Array) => {
        ...
        for (const frameCount of frameCountsFor(bytes)) {
          let decoded: CbsfDirectory | null = null;
          ...
          invariant(
            decoded !== null,
            "cbsf.null-decode",
            "decode returned without a directory"
          );
          invariant(
          ...
          for (const offset of corruptionOffsets(bytes.length)) {
            ...
            corrupted[offset] = current ^ 0x80;
            let corruptDecoded: CbsfDirectory | null = null;
            let corruptFailure: unknown = null;
            try {
  ...
  },
  {
    ...
    async load() {
      const mod = await importByPath(
        "packages/backup/dist/wal-format.js",
        BUILD
      );
      const parseWalSegmentKey = moduleFn<
        (key: string) => Record<string, unknown> | null | undefined
      >(mod, "parseWalSegmentKey");
      const parseWalCloserKey = moduleFn<
        (key: string) => Record<string, unknown> | null | undefined
      >(mod, "parseWalCloserKey");
      const parseWalPairMarkerKey = moduleFn<
        (key: string) => Record<string, unknown> | null | undefined
      >(mod, "parseWalPairMarkerKey");
      const walSegmentKey = moduleFn<
        (address: Record<string, unknown>) => string
      >(mod, "walSegmentKey");
      const walGroupCloserKey = moduleFn<
        (address: Record<string, unknown>) => string
      >(mod, "walGroupCloserKey");
      const walPairMarkerKey = moduleFn<
        (address: Record<string, unknown>) => string
      >(mod, "walPairMarkerKey");
      ...
      return (bytes: Uint8Array) => {
        ...
        if (segment) {
          invariant(
            ...
            "wal.segment-db",
            `segment db "${String(segment.db)}" is not a WAL database`
          );
          invariant(
            typeof segment.generation === "string" &&
              GENERATION.test(segment.generation),
            "wal.segment-generation",
            `segment generation "${String(segment.generation)}" is not 32 lowercase hex`
          );
          invariant(
          ...
          invariant(
            typeof segment.group === "number" &&
              Number.isSafeInteger(segment.group) &&
              segment.group >= 0 &&
              typeof segment.tickMs === "number" &&
        ...
        if (closer) {
          invariant(
            (closer.db === "vault" || closer.db === "journal") &&
              typeof closer.generation === "string" &&
              GENERATION.test(closer.generation) &&
              typeof closer.group === "number" &&
        ...
        if (marker) {
          invariant(
            typeof marker.vaultGeneration === "string" &&
              GENERATION.test(marker.vaultGeneration) &&
              typeof marker.journalGeneration === "string" &&
              GENERATION.test(marker.journalGeneration) &&
              typeof marker.tickMs === "number" &&
