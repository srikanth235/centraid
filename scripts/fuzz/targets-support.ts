 ...
 * directly — see `scripts/fuzz/ts-resolve.ts`.
 */
...
export async function importByPath(
  ...
  if (relative.endsWith(".ts") && !tsHookRegistered && !process.env.VITEST) {
    register(new URL("ts-resolve.ts", import.meta.url));
    tsHookRegistered = true;
  }
  const loaded: unknown = await import(pathToFileURL(absolute).href);
  if (typeof loaded !== "object" || loaded === null) {
    throw new Error(`fuzz: ${relative} did not export a module object`);
  }
...
export function asRecord(
  ...
  if (typeof value !== "object" || value === null) {
    throw new FuzzInvariantError(className, message);
  }
  return value as Record<string, unknown>;
}

export function moduleFn<T extends (...args: never[]) => unknown>(
  exports: Record<string, unknown>,
  name: string
): T {
  const value = exports[name];
  if (typeof value !== "function") {
    throw new Error(`fuzz: ${name} is not a function`);
  }
  return value as T;
}
 ...
 */
export function corruptionOffsets(length: number): number[] {
 ...
 */
export function assertFtsGrammar(expression: string, className: string) {
  invariant(
 ...
 * @param {string} className Finding class prefix.
 */
export function assertKeyRoundtrip(
  format: (address: Record<string, unknown>) => string,
...
export interface FuzzTarget {
  ...
  iterations: number;
  smokeIterations: number;
  corpus?: string;
  load: () => Promise<(bytes: Uint8Array) => string>;
}
