 ...
 */
function extglob(glob: string, open: number) {
  const close = glob.indexOf(")", open);
  if (close === -1) throw new Error(`unterminated extglob in ${glob}`);
  const body = glob
    .slice(open + 1, close)
    .split("|")
    .map((part: string) => globToRegExp(part).source.slice(1, -1))
    .join("|");
 ...
 * @returns {{body: string, next: number}} The compiled alternation and the index after `}`.
 */
function braces(glob: string, open: number) {
  const close = glob.indexOf("}", open);
...
export async function flattenProjects(
  config: unknown,
  projectRoot: string,
  load: (rel: string, root: string) => Promise<unknown>
): Promise<Array<{ root: string; include: string[]; exclude: string[] }>> {
  const record = isRecord(config) ? config : {};
  const test = isRecord(record.test) ? record.test : {};
  const declaredRoot =
    typeof record.root === "string"
      ? path.resolve(projectRoot, record.root)
      : projectRoot;
  const children = test.projects ?? record.projects;
  ...
  const include = Array.isArray(test.include)
    ? test.include.filter((item): item is string => typeof item === "string")
    : ["**/*.{test,spec}.?(c|m)[jt]s?(x)"];
  const exclude = Array.isArray(test.exclude)
    ? test.exclude.filter((item): item is string => typeof item === "string")
    : [];
  return [
    {
      root: declaredRoot,
...
async function loadConfig(rel: string) {
 ...
 */
export function filesNamedByScripts(scripts: unknown): Set<string> {
  const named = JSON.stringify(scripts ?? {}).match(
    /[\w./@-]+\.(?:test|spec)\.[cm]?[jt]sx?/gu
  );
  return named ? new Set(named) : new Set<string>();
}
 ...
 */
export function globsNamedByScripts(scripts: unknown) {
  const named = JSON.stringify(scripts ?? {}).match(
  ...
  for (const file of files) {
    ...
    const reached = projects.some((project) => {
      ...
      if (
        ...
        matchesAny(relative, project.exclude ?? [])
      )
        return false;
 ...
 */
export function assertEveryConfigModelled(
  configs: string[],
...
export async function run() {
  ...
  const playwright = playwrightDirs(
    ...
    (rel: string) => readFileSync(path.join(ROOT, rel), "utf8")
  );
  const orphans = findOrphans({
    files: tracked,
    projects,
    named: filesNamedByScripts(isRecord(pkg) ? pkg.scripts : undefined),
    namedGlobs: globsNamedByScripts(isRecord(pkg) ? pkg.scripts : undefined),
    playwright,
  });
  return {
    orphans,
