...
export type FilterTable = Record<string, string[]>;
 ...
 * globs at 14 as `- '…'`.
 */
export function parseFilters(source: string): FilterTable | null {
  const lines = source.split("\n");
  const start = lines.findIndex((line: string) =>
    /^\s{10}filters:\s*\|/u.test(line)
  );
  ...
  for (let index = start + 1; index < lines.length; index += 1) {
    ...
    if (name) {
      const filterName = name.groups?.name;
    ...
    const globValue = glob?.groups?.glob;
    if (globValue && current) {
      const list = filters[current];
      if (list) list.push(globValue);
    }
  }
  return filters;
}
...
export function duplicateGlobs(filters: FilterTable): string[] {
 ...
 */
function scannableUnits(source: string): Array<{ line: number; text: string }> {
  const lines = source.split("\n");
  const units: Array<{ line: number; text: string }> = [];
  for (let index = 0; index < lines.length; index += 1) {
    const current = lines[index] ?? "";
    const opener = /^(?<indent>\s*)[\w-]+:\s*[>|][-+]?\s*$/u.exec(current);
    if (!opener) {
      units.push({ line: index + 1, text: current });
      continue;
    }
    const keyIndent = opener.groups?.indent?.length ?? 0;
    const body: string[] = [];
    let cursor = index + 1;
    ...
    units.push({ line: index + 1, text: `${current} ${body.join(" ")}` });
    index = cursor - 1;
  }
  return units;
}
 ...
 */
export function escapeHatchProblems(source: string): string[] {
  const problems: string[] = [];
...
export function claimedPath(glob: string): string {
 ...
 */
export function claimedPaths(filters: FilterTable): Set<string> {
 ...
 */
export function pathsRequiringClaim(
  ...
  for (const file of trackedFiles) {
    ...
    if (top === "packages" || top === "apps") {
      if (parts.length >= 3 && parts[1] !== undefined)
        required.add(`${top}/${parts[1]}`);
      continue;
    }
    if (top.startsWith(".")) continue; // .github, .governance, .githooks
    required.add(top);
  }
  return required;
}

export function lintPathFilters(
  filters: FilterTable,
  trackedFiles: readonly string[],
  ledger: unknown
): string[] {
  ...
  const alwaysOn = isRecord(ledgerRecord.alwaysOn) ? ledgerRecord.alwaysOn : {};
  const exempt = new Map(
    Object.entries(alwaysOn).filter(
...
function main() {
  ...
  const tracked = execFileSync("git", ["ls-files", "-z"], {
    ...
    .split("\0")
    .filter(Boolean);
  const ledger: unknown = JSON.parse(readFileSync(LEDGER_PATH, "utf8"));
  ...
  console.log(
    `path-filters: ${Object.keys(filters).length} filter(s) cover every workspace and top-level path (${Object.keys(isRecord(ledger) && isRecord(ledger.alwaysOn) ? ledger.alwaysOn : {}).length} ledgered as always-on), and every read of one carries the \`all\` fallback`
  );
}
