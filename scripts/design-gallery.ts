...
import { launchOptions, unrunnableVerdict } from "./design-gallery-browser.ts";
import { runFidelityLanes } from "./design-gallery-fidelity.ts";
import type { GalleryEntry, GalleryScheme } from "./design-gallery-lowering.ts";
import { captureLowering } from "./design-gallery-lowering.ts";
import { buildWebShell, diffPng, serveDist } from "./design-gallery-serve.ts";
...
function matrixTables(value: unknown): {
  surfaces: Record<string, unknown>;
  moments: Record<string, unknown>;
} {
  if (!isRecord(value) || !isRecord(value.surfaces) || !isRecord(value.moments))
    throw new Error(
      "design-gallery: tests/design-grammar-matrix.json is not a tables object"
    );
  return { moments: value.moments, surfaces: value.surfaces };
}

const MATRIX = matrixTables(
  JSON.parse(readFileSync(MATRIX_FILE, "utf8")) as unknown
);
...
function captures(): GalleryEntry[] {
...
function validateGalleryContract(entries: GalleryEntry[]): string[] {
      ...
      loaded: (() => {
        ...
        document.fonts.forEach((face) => {
          if (face.status === "loaded")
            names.push(`${face.family} ${face.weight}`);
        });
        return names;
      })(),
      product,
...
const LEGAL_TYPE_TRIPLES = new Set(
  [false, true].flatMap((touch) =>
    Object.values(typeForSurface(touch)).map(
      (style) => `${style.weight}|${style.size}|${style.lineHeight}`
    )
  )
);

async function assertLegalTypeTriples(
  page: Page,
  root: string,
  id: string
): Promise<void> {
  ...
  const illegal = [
    ...new Set(
      triples
        .filter(
          (row: { triple: string }) => !LEGAL_TYPE_TRIPLES.has(row.triple)
        )
        .map(
          (row: { element: string; triple: string }) =>
            `${row.element}=${row.triple}`
        )
    ),
  ];
  if (illegal.length > 0)
 ...
 */
async function assertControlVocabulary(
  page: Page,
  root: string,
  id: string
): Promise<void> {
  ...
  const fills = await page
    ...
    .evaluateAll((elements) =>
      elements.map((element: Element) => ({
        ...
        variant:
          element instanceof HTMLElement ? element.dataset.variant : undefined,
      }))
    );
  const primaryFills = new Set(
    fills
      .filter(
        (row: { background: string; disabled: boolean; variant?: string }) =>
          row.variant === "primary" && !row.disabled
      )
      .map((row: { background: string }) => row.background)
  );
  ...
  const trespassers = fills.filter(
    (row: { background: string; variant?: string }) =>
      row.variant !== "primary" && row.background === accent
  );
  if (trespassers.length > 0)
    throw new Error(
      `${id}: the accent fill is not the primary variant's alone — ${trespassers
        .map((row: { variant?: string }) => row.variant)
        .join(", ")} paint ${accent}`
    );
  ...
  const focused = await page.evaluate(() => {
    ...
    return active instanceof HTMLElement ? active.dataset.variant : undefined;
  });
  if (focused !== "primary")
...
async function captureShell(
  ...
  await page.evaluate((scheme: GalleryScheme) => {
...
async function main() {
  ...
  const manifest = {
    issue: 690,
    reviewIssue: 695,
    generatedBy: "scripts/design-gallery.ts",
    ...
    // pixels. See scripts/design-gallery-fidelity.ts.
    fidelityLanes: {
      ...
      (chain, entry) =>
        chain.then(async () => {
          const page = await browser.newPage({
            colorScheme: entry.scheme as "light" | "dark",
            deviceScaleFactor: 1,
