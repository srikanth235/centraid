...
export function discoverFlowFiles(root = ROOT): string[] {
  const files: string[] = [];
...
export function discoverSourceFiles(root = ROOT, dir = SOURCE_DIR): string[] {
  const out: string[] = [];
 ...
 */
export type VocabEntry = { value: string; accessor: string; line: number };

export function parseVocabulary(text: string): {
  ids: VocabEntry[];
  prefixes: VocabEntry[];
} {
  const ids: VocabEntry[] = [];
  const prefixes: VocabEntry[] = [];
 ...
 */
export function collectFlowIds(text: string): {
  ids: Array<{ id: string; line: number }>;
  chunks: number;
} {
 ...
 */
export function isAllowed(text: string, line: number, rule: string): boolean {
  const lines = text.split("\n");
...
type LintTestIdsInput = {
  flows: SourceFile[];
  vocabulary: { ids: VocabEntry[]; prefixes: VocabEntry[] };
  sources: SourceFile[];
};

export function lintTestIds(input: LintTestIdsInput): {
  findings: Array<{
    file: string;
    line: number;
    rule: string;
  ...
  const { flows, vocabulary, sources } = input;
