...
type LedgerWrite = (rel: string, value: unknown) => void;
type MergeOverrides = {
  coverage?: unknown;
  mutation?: unknown;
  minimumTests?: unknown;
  suiteWallClock?: unknown;
  mobileSuites?: unknown;
  skip?: unknown;
  lanes?: unknown;
...
function writeMerged(write: LedgerWrite, overrides: MergeOverrides = {}): void {
...
describe("derived mirrors", () => {
  test("a minimumTests mirror that drifts from the claims file fails", () => {
    ...
    assert.match(
      run(root)[0] ?? "",
      /mirrors tests\/claims\.json as 4 but reads 3/u
    );
  });
  ...
  test("a mobile suite mirror that drifts from the roster fails", () => {
    ...
    assert.match(
      errors[0] ?? "",
      /mirrors tests\/agent-e2e-mobile\/roster\.json/u
    );
  });
