...
// in lint-types-policy.ts makes an engine upgrade fail closed until this
// catalog has been regenerated and reviewed.
export const typeAwareCatalogVersion = "7.0.2001";

export const typeAwareOnlyRules: string[] = [
...
export const allFileCompatibilityRules: string[] = [
  "typescript/no-misused-promises",
  "typescript/await-thenable",
  "typescript/switch-exhaustiveness-check",
  "typescript/no-for-in-array",
...
export const sourceOnlyCompatibilityRules: string[] = [
  "typescript/no-floating-promises",
];

export const compatibilityRules: string[] = [
  ...allFileCompatibilityRules,
  ...sourceOnlyCompatibilityRules,
];

export const fixtureRules: string[] = [...compatibilityRules];
...
export const blueprintCompatibilityRules: string[] =
  allFileCompatibilityRules.filter(
...
type RuleGroup = keyof typeof RULE_GROUPS;

function isRuleGroup(value: string | undefined): value is RuleGroup {
  return value !== undefined && Object.hasOwn(RULE_GROUPS, value);
}

function printRules(group: string | undefined): void {
  if (!isRuleGroup(group)) {
    throw new Error(`unknown lint-types rule group: ${group}`);
  }
  process.stdout.write(`${RULE_GROUPS[group].join("\n")}\n`);
}
