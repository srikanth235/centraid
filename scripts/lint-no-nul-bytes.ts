 ...
 * `packages/vault/tests/golden/` (`.gz`). HEVC stills (`.heic`/`.heif`) are
 * the same class as `.jpg` — preview fixtures, not source. Every one of them
 * is already opaque to textual review and nothing about that is a defect.
 */
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  const failures = scan(present, (file: string) =>
