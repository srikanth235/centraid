...
export function commentBlocks(text: string, fileName: string): CommentBlock[] {
  ...
  let run: CommentBlock | null = null;
  const flush = () => {
  ...
  for (const range of ranges) {
    ...
    const contiguous =
      run !== null &&
      lines
        .slice((run.lastLine ?? first) + 1, first)
        .every((line: string) => line.trim() === "");
    if (!contiguous) {
      flush();
      run = {
        pos: range.pos,
...
export function lintCommentBlocks(
  ...
  const { root = ROOT, files, allowlist = {} } = options;
