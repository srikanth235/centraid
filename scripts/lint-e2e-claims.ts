...
export function claimOf(source: string) {
 ...
 */
type LintClaimsInput = {
  flows: string[];
  docs: Record<string, string | undefined>;
  pins: string[];
  rostered?: Record<string, string | undefined>;
};

export function lintClaims(input: LintClaimsInput): string[] {
  const { flows, docs, pins, rostered = {} } = input;
  const errors: string[] = [];
    ...
    const errors = lintClaims({
      ...
      rostered: Object.fromEntries(
        (testCase.input.flows ?? []).map((flow) => [
          flow,
          claimOf(
            flow in testCase.input.docs
              ? ((testCase.input.docs as Record<string, string>)[flow] ?? "")
              : ""
          ) ?? undefined,
        ])
      ),
...
function main() {
  ...
  const pinnedRaw: unknown = JSON.parse(readFileSync(PINS, "utf8"));
  const pinnedRecord =
    typeof pinnedRaw === "object" && pinnedRaw !== null
      ? (pinnedRaw as Record<string, unknown>)
      : {};
  const pinned = {
    flows: Array.isArray(pinnedRecord.flows)
      ? pinnedRecord.flows.filter(
          (entry): entry is string => typeof entry === "string"
        )
      : [],
    baseline: Array.isArray(pinnedRecord.baseline)
      ? pinnedRecord.baseline.filter(
          (entry): entry is string => typeof entry === "string"
        )
      : [],
  };
