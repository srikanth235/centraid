 ...
 *
 * SEVEN tone families, each meaning exactly ONE thing (issue #864) — the
 * register used to paint twelve states with four, so a family answered several
 * questions at once. The families now partition by MEANING: `ok` passed a solid
 * claim; `partial` passed a partial claim; `danger` tonight's run went wrong
 * (failed + infra-mismatch); `flaky` green only on retry; `gap` no test exists
 * (the matrix hole and the app grids' unowned seat are one fact, one paint);
 * `attn` integrity — the report cannot vouch for its own evidence; `grey`
 * absence of evidence (stale, no lane, missing, named). `partial`/`flaky`/`gap`
 * follow the ramp's identity hues (`--c-teal`, `--c-violet`, `--c-rose`) so the
 * two halves agree; `attn` moved off the `--seam` literal it used to duplicate
 * (at #B4441F it sat 8° from `danger` — no distance at 11px — and `--st-gap`
