...
import {
  ...
  parseInstallArgs,
} from "./gateway-npm/pack-helpers.ts";
...
function listPackFiles(fromPackDir: string): string[] {
  ...
  if (fs.existsSync(manifestPath)) {
    const m: unknown = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
    const tarballs = isRecord(m) && Array.isArray(m.tarballs) ? m.tarballs : [];
...
function main(argv: string[]): void {
  ...
  if (useGlobal) npmArgs.push("-g");
    ...
    if (prefix === null) throw new Error("install prefix is required");
