...
import { GATEWAY_WORKSPACE_PACKAGES } from "./assemble-runtime.ts";
...
const NATIVE_MODULE_DECISION = {
  sharp: {
    ...
    packaging:
      "platform-specific optional dependency; pin per docker target arch",
  },
  "wasm-vips": {
  ...
  "iroh/tunnel wasm": {
    ...
    packaging:
      "gateway image is HTTP/control-plane first; tunnel native optional via host network",
  },
};

function listDistIfPresent(pkgRel: string) {
  const dist = path.join(root, pkgRel, "dist");
  if (!existsSync(dist))
    return [`${pkgRel}/dist  (missing — run package build)`];
  const files: string[] = [];
