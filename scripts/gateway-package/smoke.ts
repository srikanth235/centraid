 ...
 *
 * Usage:
 *   node scripts/gateway-package/smoke.ts [--gateway-bin <path>] [--port 0]
 *   node scripts/gateway-package/smoke.ts --base-url http://127.0.0.1:8787
 *
 * Prefer an already-built gateway for host mode:
 *   bun run --cwd packages/server build
 *   node scripts/gateway-package/smoke.ts
 */
...
import { waitForGatewayInfo } from "./probe.ts";
...
async function probeOnly(baseUrl: string) {
...
async function hostMode() {
  ...
  const gatewayBin =
    arg("--gateway-bin") ?? path.join(root, "packages/server/dist/cli/cli.js");
