 ...
 *
 * Usage:
 *   node scripts/gateway-package/assemble-runtime.ts --root=<monorepo> --out=<dir>
 */
...
/** Must stay aligned with scripts/gateway-package/trace.ts. */
...
function walkRm(
  ...
  pred: (
    full: string,
    name: string,
    st: ReturnType<typeof lstatSync>
  ) => boolean
) {
 ...
 */
export function rewriteRuntimeSymlinks(out: string, root: string) {
  ...
  const mapIntoOut = (resolved: string) => {
...
export function assembleRuntime({
  ...
  const rootVersion =
    isRecord(rootPkg) && typeof rootPkg.version === "string"
      ? rootPkg.version
      : "0.0.0";
  writeFileSync(
  ...
  for (const pkg of GATEWAY_WORKSPACE_PACKAGES) {
    ...
    if (!isRecord(pkgJson)) {
      throw new Error(`${pkg}/package.json is not an object`);
    }
    ...
    const runtimePkg: Record<string, unknown> = { ...pkgJson };
