 ...
 * @returns {{ errors: string[], entries: Loose[] }} Violations and entries.
 */
export function validateQuarantine(
  document: unknown,
  nowMs: number
): { errors: string[]; entries: Loose[] } {
  const errors: string[] = [];
...
export function readQuarantine(file: string = QUARANTINE_PATH): Loose {
...
if (process.argv[1] === import.meta.filename) {
  ...
  console.log(
    `quarantine: ${entries.length} entr${entries.length === 1 ? "y" : "ies"}, none expired (budget ${dict(dict(document)._policy).budget})`
  );
}
