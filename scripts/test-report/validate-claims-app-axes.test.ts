 ...
 */
async function validateMatrix(claims: unknown, options: unknown = {}) {
  const flowIds = bags(dict(claims).flows)
    .map((flow) => flow.id)
    .concat(["vault-core-flow"]);
