...
import { buildSummary } from "./generate.ts";
import {
...
describe("the rolling issue body", () => {
  test("agrees with §3 about the deadline for the same lane", async () => {
    ...
    const queued = dict(
      bags(model.attention).find((entry) => entry.lane === "mobile-e2e-android")
    );
    expect(queued.lane).toBeDefined();
    ...
    expect(String(queued.deadline)).toContain(queued.issue ? "expires" : "");
    expect(body).toContain(String(queued.deadline).replace("expires ", ""));
...
describe("the real workflows", () => {
  test("every evidence step in the repo names a registered lane", async () => {
    ...
    const { loadClaims } = await import("./claims-schema.ts");
