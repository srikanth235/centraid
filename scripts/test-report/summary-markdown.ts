...
export function renderSummaryMarkdown(
  summary: Loose,
  meta: { reportUrl?: string; runUrl?: string; title?: string } | undefined = {}
) {
  ...
  const lines = [
    ...
    "| Signal | Value |",
    "| --- | ---: |",
    `| Lanes passed | ${Number(dict(deltas).passed ?? 0)} |`,
    `| Lanes failed | ${Number(dict(deltas).failed ?? 0)} |`,
    `| Lanes degraded | ${Number(dict(deltas).degraded ?? 0)} |`,
    `| Lanes parked | ${parks.length} |`,
    `| Lanes with no evidence | ${Number(dict(deltas).noEvidence ?? 0)} |`,
    `| New red since the last candidate | ${Number(dict(deltas).newRed ?? 0)} |`,
    `| New green since the last candidate | ${Number(dict(deltas).newGreen ?? 0)} |`,
 ...
 */
export function publicReportUrl({
  owner,
  repo,
  slot,
}: {
 ...
 */
export function coverageScopesBelowFloor(
  coverageRows: Array<{
    scope: string;
    lines: number | null;
    lineFloor?: number | null;
  }>
) {
  const below = [];
