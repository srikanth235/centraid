 ...
 */
export function parseArgs(argv: string[], known: Set<string>) {
 ...
 */
export function resolveVerdict(verdict: string, jobStatus: string | undefined) {
 ...
 */
export function lookupPark(
  lane: string,
  readText: (relative: string) => string | null,
  today: string
): { until: string; issue: number } | null {
  for (const source of PARK_SOURCES) {
    ...
    const entry = dict(dict(parsed).lanes)[lane];
    if (!entry) continue;
    const park = dict(entry);
    const until = String(park.expires ?? park.until ?? "");
    if (!until || until < today) continue;
    const issue = Number(String(park.issue ?? "").replace(/^#/u, ""));
...
export function buildEvidence(
  ...
  context: {
    now: Date;
    park: { until: string; issue: number } | null;
    cases: unknown[];
  }
) {
  ...
  const declared = resolveVerdict(
    String(flags.verdict ?? ""),
    flags["job-status"] === undefined ? undefined : String(flags["job-status"])
  );
  ...
  const durationMs =
    flags["duration-ms"] === undefined
      ? Math.max(
          0,
          Date.parse(String(finishedAt)) - Date.parse(String(startedAt))
        )
...
function readCases(casesPath: string) {
  if (!casesPath) return [];
...
export function main(
  ...
  const evidence = buildEvidence(flags, {
    now,
    park,
    cases: readCases(String(flags.cases ?? "")),
  });
  ...
  const file = path.join(outDir, evidenceFileName(String(evidence.lane)));
...
if (
  ...
  try {
    ...
    process.stderr.write(`write-evidence: ${errorMessage(error)}\n`);
    process.exitCode = 1;
  }
}
