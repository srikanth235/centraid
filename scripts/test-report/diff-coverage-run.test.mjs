...
describe('workspaceDirOf', () => {
  test('maps packages/ and apps/ sources to their workspace dir', () => {
    expect(workspaceDirOf('packages/gateway/src/serve/build-gateway.ts')).toBe('packages/gateway');
    expect(workspaceDirOf('apps/mobile/src/lib/upload/enqueue.ts')).toBe('apps/mobile');
  });
...
describe('projectNameOf', () => {
  ...
  test('a workspace with a vitest config reports its package name', () => {
    ...
    writeFileSync(path.join(dir, 'package.json'), JSON.stringify({ name: '@x/with-vitest' }));
    writeFileSync(path.join(dir, 'vitest.config.ts'), 'export default {}');
    expect(projectNameOf(path.join(relFixture, 'with-vitest'))).toBe('@x/with-vitest');
  });
  ...
  test('a workspace without a vitest config contributes no project — naming it would make --project fail', () => {
    ...
    mkdirSync(dir, { recursive: true });
    writeFileSync(path.join(dir, 'package.json'), JSON.stringify({ name: '@x/no-vitest' }));
