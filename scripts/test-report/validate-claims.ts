...
async function exists(base: string, relative: string) {
  try {
 ...
 * moved, go and look".
 */
export async function firedTriggers(
  claims: Loose,
  {
    root: base,
    glob,
  }: { root?: string; glob?: typeof import("node:fs/promises").glob }
) {
  const triggers = Object.entries(dict(claims.revisitTriggers));
  const scans = await Promise.all(
    triggers.map(async ([cell, raw]) => {
      const trigger = dict(raw);
      const pattern = new RegExp(String(trigger.contains ?? ""), "u");
      if (!glob) return null;
      const files = await fromAsync(
        glob(String(trigger.glob ?? ""), { cwd: base })
      );
      const sources = await Promise.all(
        files.map((file) => readFile(path.join(base ?? "", file), "utf8"))
      );
      const hit = files.find((_file, index) =>
 ...
 */
export async function validateClaimsFile(
  claims: Loose,
  options: { root?: string; checkFiles?: boolean } | undefined = {}
) {
  ...
  const flowIds = new Set(flows.map((flow: Loose) => flow.id));

  if (checkFiles) {
    const owners = new Map();
    for (const claim of bags(claims.claims))
      owners.set(claim.owner, `claim ${claim.id}`);
    for (const flow of bags(flows)) owners.set(flow.owner, `flow ${flow.id}`);
    for (const [tag, law] of Object.entries(dict(claims.laws)))
      owners.set(dict(law).owner, `law ${tag}`);
    for (const law of bags(claims.joinLaws))
      owners.set(law.owner, `join law ${law.id}`);
    for (const layer of bags(claims.consentLedger)) {
  ...
  for (const rung of [2, 3, 4, 5]) {
    if (!bags(claims.lanes).some((lane) => lane.rung === rung)) {
...
if (
  ...
  if (errors.length > 0) {
    ...
    process.stdout.write(
      `claims: ${bags(claims.claims).length} claims, ${items(lanes).length} lanes, ${flowIds.size} derived flows, ${Object.keys(dict(claims.naCells)).length} deliberate n/a cells\n`
    );
  }
}
