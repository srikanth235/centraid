...
export const METRICS: Record<
  string,
  { label: string; pattern: RegExp; remedy: string }
> = {
  /**
 ...
 */
export function countHygieneSites(source: unknown): Record<string, number> {
  const counts: Record<string, number> = Object.fromEntries(
    METRIC_KEYS.map((key) => [key, 0])
  );
  if (typeof source !== "string" || !source) return counts;

  const truthyMetric = METRICS.toBeTruthyFalsy;
  const calledMetric = METRICS.toHaveBeenCalled;
  if (!truthyMetric || !calledMetric) return counts;

  counts.toBeTruthyFalsy = [...source.matchAll(truthyMetric.pattern)].length;

  for (const match of source.matchAll(calledMetric.pattern)) {
    ...
    counts.toHaveBeenCalled = (counts.toHaveBeenCalled ?? 0) + 1;
  }
  return counts;
}
...
export async function discoverHygieneCounts({
...
}: { root?: string; include?: string[]; exclude?: string[] } = {}): Promise<{
  totals: Record<string, number>;
  files: Array<{ file: string; [metric: string]: string | number }>;
}> {
  ...
  const scanned = await Promise.all(
    [...files].sort().map(async (file) => {
      ...
      return {
        file,
        ...countHygieneSites(source ?? ""),
      } as { file: string; [metric: string]: string | number };
    })
  );
...
export function topOffenders(
  ...
  return files
    ...
    .sort(
      (a, b) => Number(b[key]) - Number(a[key]) || a.file.localeCompare(b.file)
    )
    .slice(0, limit)
 ...
 */
export function validateHygieneBudgets(
  budgets: unknown,
  discovered: {
    totals: Record<string, number>;
    files: Array<{ file: string; [metric: string]: string | number }>;
  ...
  const errors: string[] = [];
  const committed = dict(dict(budgets).budgets);

  for (const key of METRIC_KEYS) {
    const measured = discovered.totals[key] ?? 0;
    ...
    if (measured > Number(budget)) {
      const offenders = topOffenders(discovered.files, key);
      errors.push(
        `${METRICS[key]?.label} budget exceeded: ${measured} against a budget of ${budget} (+${measured - Number(budget)}). The budget is down-only — ${METRICS[key]?.remedy}. Top offenders: ${offenders.join(", ")}`
      );
    } else if (measured < Number(budget)) {
      errors.push(
        `${METRICS[key]?.label} budget is slack: ${measured} against a budget of ${budget}. Ratchet ${key} down to ${measured} (node scripts/test-report/hygiene-ratchet.ts --write).`
      );
    }
  }
 ...
 */
export function reconcileBudgets(budgets: unknown, totals: unknown): Loose {
  const committed = dict(dict(budgets).budgets);
  const measured = dict(totals);
  const next: Record<string, number> = {};
  for (const key of METRIC_KEYS) {
    const previous = committed[key];
    const current = Number(measured[key] ?? 0);
    next[key] = Number.isInteger(previous)
      ? Math.min(Number(previous), current)
      : current;
  }
  return { ...dict(budgets), budgets: next };
}
...
async function main() {
  ...
  if (write) {
    ...
    console.log(
      `hygiene: wrote budgets ${METRIC_KEYS.map((key) => `${key}=${dict(next.budgets)[key] ?? ""}`).join(" ")} to ${INVENTORY_PATH}#hygiene`
    );
    return;
  }
  ...
  console.log(
    `hygiene: ${discovered.files.length} test files at budget — ${METRIC_KEYS.map((key) => `${METRICS[key]?.label ?? key} ${totals[key]}`).join(", ")}`
  );
}
