 ...
 * Usage: node scripts/test-report/write-job-summary.ts [--summary path] [--report-url url]
 */
import { appendFile, readFile } from "node:fs/promises";
import path from "node:path";

import { dict } from "./record.ts";
import { renderSummaryMarkdown } from "./summary-markdown.ts";
...
let summary: unknown = {};
try {
  summary = JSON.parse(await readFile(summaryPath, "utf8")) as unknown;
} catch {
  summary = {
    passed: 0,
    failed: 0,
...
const md = renderSummaryMarkdown(dict(summary), {
