 ...
 * Render the body.
 *
 * @param {{lane: string, summary: Loose, evidence: Loose|null, reportUrl?: string}} input the lane, tonight's summary, its evidence and the report URL
...
export function renderRollingIssueBody({
  ...
  const blocker =
    bags(summary.blockers).find((row) => row.lane === lane) ?? null;
  const park = bags(summary.parks).find((row) => row.lane === lane) ?? null;
  ...
  const failing = bags(evidence?.cases).filter(
  ...
  if (failing.length > 0) {
    ...
    for (const entry of failing) {
      lines.push(
        `- \`${entry.id}\` — ${Math.round(Number(entry.durationMs ?? 0) / 1000)}s, ${entry.attempts ?? 1} attempt(s)`
      );
    }
    lines.push("");
  }
...
function parseFlags(argv: string[]) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (flag?.startsWith("--")) flags[flag.slice(2)] = argv[index + 1];
  }
  return flags;
}
