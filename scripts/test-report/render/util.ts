...
/** A state pill. */
export function pill(state: unknown, label?: unknown) {
  const key = String(state ?? "no-evidence");
  const entry =
    key in STATE_WORDS
      ? STATE_WORDS[key as keyof typeof STATE_WORDS]
      : STATE_WORDS["no-evidence"];
  return `<span class="pill ${entry.class}">${escapeHtml(label ?? entry.word)}</span>`;
}

/** A severity chip. */
export function sev(severity: unknown) {
  const raw = String(severity ?? "");
  const safe = /^S[1-4]$/u.test(raw) ? raw : "S4";
...
export function secs(value: unknown) {
  return finite(value) ? `${Math.round(value / 1000)}s` : "—";
}

/** A short SHA in a chip. */
export function sha(value: unknown) {
  return value
...
export function issueLink(number: unknown, repoUrl?: unknown) {
  if (!number) return "—";
 ...
 */
export function sparkline(words: unknown[], label: string) {
  const width = 3;
  const gap = 1;
  const height = 22;
  const bars = words
...
export function budgetBar(observed: unknown, budget: unknown) {
  const ratio =
    finite(budget) && budget > 0 && finite(observed) ? observed / budget : 0;
...
export function table(head: unknown, body: unknown, id = "") {
  const cells = Array.isArray(head) ? head : [];
  return `<div class="tablewrap"><table${id ? ` id="${id}"` : ""}><thead><tr>${cells
    .map((cell) => `<th>${cell}</th>`)
    .join("")}</tr></thead><tbody>${body}</tbody></table></div>`;
}

/** A collapsed evidence card. */
export function details(summary: unknown, note: unknown, body: unknown) {
