...
/** §6. */
export function renderCoverage(model: ReportModel) {
  const coverage = dict(model.coverage);
  const platforms = bags(coverage.platforms);
  const rows = bags(coverage.rows);
  const header = `<div class="h">app</div>${platforms.map((column) => `<div class="h">${escapeHtml(column.label)}</div>`).join("")}`;

  const rungBody = rows
    .map((row) => {
      const cells = bags(row.cells)
        .map((cell) => {
          if (cell.na) {
  ...
  const stateBody = rows
    .map((row) => {
      const states = bags(row.states);
      const owned = states.filter((state) => state.owned).length;
      const boxes = states
        .map((state) => {
          const name = String(state.state ?? "");
          return `<span class="st-box${state.owned ? " on" : ""}" title="${escapeHtml(name)}">${escapeHtml(STATE_LETTERS[name] ?? name.slice(0, 1))}</span>`;
        })
        .join("");
      return `<div class="app">${escapeHtml(row.app)}</div><div class="cell passed" style="grid-column:2 / -1"><span class="rung">${owned} / ${states.length}</span><span class="states">${boxes}</span></div>`;
    })
    .join("");

  const verbBody = rows
    .map((row) => {
      const verbs = dict(row.verbs);
      const chips = ["create", "read", "update", "delete", "share"]
        .map((verb) => {
          const initial = verb[0]?.toUpperCase() ?? "";
          return `<span class="verb${verbs[verb] === 0 ? " zero" : ""}"><b>${initial}</b> ${verbs[verb]}</span>`;
        })
        .join("");
...
/** §7. */
export function renderPromises(model: ReportModel) {
  const promises = dict(model.promises);
  const qualities = bags(promises.qualities);
  const surfaces = bags(promises.surfaces);
  const cells = items(promises.cells);
  const counts = dict(promises.counts);
  const header = `<div class="h"></div>${surfaces.map((surface) => `<div class="h">${escapeHtml(surface.label)}</div>`).join("")}`;
  const body = qualities
    .map((quality, row) => {
      const line = bags(cells[row])
        .map((cell) => {
          const word =
            cell.state === "no-evidence" ? "no evidence" : cell.state;
          const lanes = items(cell.lanes);
          const title = `${quality.label}: ${word}${lanes.length > 0 ? ` · ${lanes.join(", ")}` : cell.reason ? ` · ${cell.reason}` : ""}`;
          ...
          return `<div class="cell ${cls}" title="${escapeHtml(title)}"><span class="st">${escapeHtml(word)}</span>${
            lanes.length > 0
              ? `<span class="ln">${escapeHtml(lanes[0])}</span>`
              : ""
          }</div>`;
        })
        .join("");
