...
/** §8. */
export function renderAdversaries(model: ReportModel) {
  const adv = dict(model.adversaries);
  const seeds = bags(adv.seeds);
  const mutation = bags(adv.mutation);
  const fuzzTargets = bags(adv.fuzzTargets);
  const fuzz = bags(adv.fuzz);
  const engines = bags(adv.engines);
  const scoreBySeed = new Map(mutation.map((row) => [row.id, row]));
  const mutationRows = seeds
    .map((seed) => {
      const observed = dict(scoreBySeed.get(seed.id));
      const score = observed.score;
      const floor = observed.floor;
      const low = finite(score) && finite(floor) && score < floor;
      ...
      }</td><td class="num">${finite(floor) ? floor : "—"}</td><td class="num">${observed.survived ?? "—"}</td></tr>`;
    })
    .join("");
  ...
  const fuzzRows = fuzzTargets
    .map((target) => {
      const observed = dict(fuzzByTarget.get(target.id));
  ...
  const engineRows = engines
    .map((engine) => {
      ...
      return `<tr><td class="mono">${escapeHtml(engine.id)}</td><td class="small">${escapeHtml(items(engine.source).join(", "))}</td><td class="mono small">${escapeHtml(engine.propertyFlow ?? "—")}</td><td>${pill(owned ? "passed" : "no-evidence")}</td><td>${owned ? "" : pill("failed", "no owner")}</td></tr>`;
    })
    .join("");
...
export function band(values: unknown[]) {
...
function chart(trend: unknown) {
  const row = dict(trend);
  const data = items(row.points).filter(finite);
  const width = 300;
  const height = 70;
  const pad = 4;
  ...
  <div class="t"><span class="name">${escapeHtml(row.name)}</span><span class="last ${tone}">${format(last)} ${escapeHtml(row.unit)}${out ? " · out of band" : ""}</span></div>
  <svg viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="${escapeHtml(row.name)}, ${data.length} candidates">
  ...
  <div class="foot"><span>${data.length} candidates · band ${format(q1)}–${format(q3)}</span><span>${finite(row.budget) ? `budget ${format(row.budget)} ${escapeHtml(row.unit)}` : "gauge · no budget"}</span></div>
</div>`;
}

/** §9. */
export function renderTrends(model: unknown) {
  const drawable = bags(dict(model).trends).filter(
    (trend) => items(dict(trend).points).length >= TREND_MINIMUM_POINTS
  );
  const body =
    drawable.length === 0
