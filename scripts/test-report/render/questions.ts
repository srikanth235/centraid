...
export function renderBlockers(model: ReportModel) {
  const rows =
...
function column(title: string, tone: string, entries: unknown, empty: string) {
  const rows = bags(entries);
  const list =
    rows.length === 0
      ...
      : rows
          ...
          .join("");
  return `<div class="card col ${tone}"><h4>${escapeHtml(title)} <span class="n">${rows.length}</span></h4><ul>${list}</ul></div>`;
}

/** §2 — six columns, computed candidate to candidate. */
export function renderSinceYesterday(model: ReportModel) {
...
${column("Budget pressure", "", since.budgetPressure, "no lane is over 80 % of its budget")}
  ...
  const from = text(model.delta.previousCandidate)
    ? `${text(model.delta.previousCandidate).slice(0, 9)} → `
    : "";
  const to = text(model.candidate?.sha)
    ? text(model.candidate?.sha).slice(0, 9)
    : "this run";
  return section(
    "changed",
    "Question two",
...
export function renderAttention(model: ReportModel) {
  ...
  <td class="owner${row.owner ? "" : " none"}">${row.owner ? escapeHtml(row.owner) : "unowned — claim"}</td>
  <td class="age ${Number(row.ageHours) > 24 ? "over" : "ok"}">${row.ageDays === null ? `${row.ageHours}h` : `${row.ageDays}d`}</td>
