...
export function renderLaneBoard(model: ReportModel) {
                ...
                (entry) =>
                  `<div class="c"><span>${escapeHtml(entry.id)}</span><span>${escapeHtml(entry.verdict)}${
                    finite(entry.durationMs)
                      ...
                      : ""
                  }${Number(entry.attempts) > 1 ? ` · ${entry.attempts} attempts` : ""}</span></div>`
              )
              .join("");
  ...
  <td>${pill(row.verdict)}${finite(row.durationMs) && row.verdict !== "parked" ? `<span class="desc">${ms(row.durationMs)}</span>` : ""}</td>
  <td>${sparkline(items(row.history), String(row.lane))}</td>
...
export function renderJourneys(model: ReportModel) {
  const rows = model.journeys
    .map((suite) => {
      const flows = bags(suite.flows);
      const observedP95 = flows.reduce(
        (total: number, flow) => total + Number(flow.budgetMs ?? 0),
        0
      );
      const slack =
        Number(suite.budgetMs) > 0 &&
        observedP95 > 0 &&
        Number(suite.budgetMs) > 1.5 * observedP95;
      const head = `<tr class="suite"><td colspan="7"><b>${escapeHtml(suite.id)}</b> · rung ${suite.rung} · ${flows.length} flows<span class="sb">${
        suite.parked
      ...
      const body = flows
        .map((flow) => {
          const observed = dict(model.caseResults?.get(String(flow.id)));
          const verdict =
  ...
  const alarmInfo = dict(model.alarm);
  const alarm = model.alarm
    ? `<div class="alarm"><span class="bell"></span><span><b>Alarm sounded</b> on ${escapeHtml(alarmInfo.last)} · ${escapeHtml(alarmInfo.what)} · next: ${escapeHtml(alarmInfo.next)}</span></div>`
