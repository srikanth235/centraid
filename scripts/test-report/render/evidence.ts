...
/** §10. */
export function renderEvidence(model: ReportModel) {
  const panels = dict(model.evidencePanels);
  ...
  const consent = table(
    ...
    bags(panels.consentLedger)
      .map(
        (layer) =>
          `<tr><td>${escapeHtml(layer.label)}</td><td class="mono">${escapeHtml(items(layer.enforcement).join(", "))}</td><td class="small">${escapeHtml(dict(layer.adversary).owner ?? "—")}</td><td>${items(layer.seats).length} / 3</td><td>${pill(
            dict(model.consentVerdicts)[String(layer.id)] ?? "no-evidence"
          )}</td></tr>`
      )
      .join("")
  );
  ...
  const joins = table(
    ...
    bags(panels.joinLaws)
      .map(
        (law) =>
          `<tr><td class="mono">${escapeHtml(law.id)}</td><td>${escapeHtml(law.kind)}</td><td>${items(law.seats).length}</td><td class="num">${law.cases ?? "—"}</td><td>${pill(
            dict(model.joinVerdicts)[String(law.id)] ?? "no-evidence"
          )}</td></tr>`
      )
      .join("")
  );

  const inventory = dict(panels.inventory);
  const parks = table(
  ...
  const observations = bags(panels.fieldObservations)
    .map(
      (entry) =>
        `<li><b>${escapeHtml(entry.title ?? entry.text ?? "")}</b>${entry.detail ? ` ${escapeHtml(entry.detail)}` : ""}<span class="age">${
          finite(entry.ageDays)
            ? `${entry.ageDays}d${Number(entry.ageDays) > 60 ? " · rung-5 red" : ""}`
            : ""
        }</span></li>`
    )
    .join("");
  ...
  const body = [
    details(
      "Coverage floors",
      `${items(panels.coverageFloors).length} scopes · ${items(panels.ratchetCandidates).length} sustained-headroom ratchet candidates`,
      floors
    ),
    details(
      "Consent ledger",
      `${items(panels.consentLedger).length} layers`,
    ...
    details("Join laws", `${items(panels.joinLaws).length} laws`, joins),
    details(
      "Inventory",
    ...
    details("Parks", `${items(panels.parks).length} lanes`, parks),
    details(
      "Field observations",
      `from QUALITY.md · ${items(panels.fieldObservations).length} open`,
...
/** §11. */
export function renderHowToRead(model: ReportModel) {
