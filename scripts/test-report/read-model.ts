 ...
 * @param {{rows: Loose[], parks: Loose[], today: string}} input the claims file, the derived views, the evidence directories and the run's metadata
 */
export function computeVerdict({
  ...
  const oldestPark = parks.reduce<number | null>((oldest, park) => {
  ...
  if (blocking.length > 0 || parkOverflow || parkTooOld) {
    ...
    if (blocking.length > 0) {
      const worst = [...blocking].sort(
        (left, right) =>
          (SEVERITY_RANK[String(left.severity)] ?? 9) -
          (SEVERITY_RANK[String(right.severity)] ?? 9)
      )[0];
      if (worst) {
 ...
 */
export function buildModel(input: unknown): ReportModel {
  const source = dict(input);
  const claims = dict(source.claims);
  const derived = dict(source.derived);
  const evidence =
    source.evidence instanceof Map
      ? (source.evidence as Map<string, unknown>)
      : new Map<string, unknown>();
  const evidenceErrors = Array.isArray(source.evidenceErrors)
    ? source.evidenceErrors
    : [];
  const previousEvidence =
    source.previousEvidence instanceof Map
      ? (source.previousEvidence as Map<string, unknown>)
      : new Map<string, unknown>();
  const candidate = dict(source.candidate);
  const history = Array.isArray(source.history) ? source.history : [];
  const generatedAt = String(source.generatedAt ?? "1970-01-01T00:00:00Z");
  const run = dict(source.run);
  const scope = source.scope ?? "nightly";
  const budgetMinutes = source.budgetMinutes ?? 90;
  const quality = dict(source.quality);

  const today = generatedAt.slice(0, 10);
  const laneRegistry = Array.isArray(claims.lanes) ? claims.lanes : [];
  ...
  const registered = new Set(bags(laneRegistry).map((lane) => lane.id));
  ...
  const minutesUsed = Math.round(
    [...evidence.values()].reduce(
      (total: number, entry) => total + Number(dict(entry).durationMs ?? 0),
      0
    ) / 60_000
  );
  ...
  return {
    ...
    verdict,
    delta: {
      previousVerdict: dict(history.at(-1)).verdict ?? null,
    ...
    repoUrl: undefined,
    links: {},
    caseResults: new Map<string, unknown>(),
  } as unknown as ReportModel;
}

export { laneSeverity, SEVERITY_RANK } from "./model/severity.ts";
