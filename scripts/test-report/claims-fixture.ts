 ...
 *
 * `validate-claims-app-axes.test.mjs` (the #839 app-shaped axes) and
 * `validate-app-scenarios.test.mjs` each break one rule at a time against the
 * SAME well-formed baseline, so the baseline lives here rather than being
 * copied into each file. It was `matrix-fixture.mjs` until #915 replaced
 * `tests/matrix.json` with `tests/claims.json`; the app-axis blocks it models
 * are the ones that moved across unchanged.
 */
...
export type ClaimsFixture = Loose & {
  seats: Array<Loose & { id: string; label?: string; doctrine?: string }>;
  flows: Loose[];
  appSeats: Loose & {
    apps: Array<Loose & { id: string; seats: Record<string, Loose> }>;
  };
  appStates: Loose & {
    trackingIssue?: string;
    states: Array<Loose & { id: string; label?: string }>;
    apps: Array<Loose & { id: string; states: Record<string, Loose> }>;
  };
  appScenarios: Loose & {
    trackingIssue?: string;
    layers?: Loose[];
  ...
  engineRegistry: Array<
    Loose & {
      ...
      label?: string;
      source?: string[];
      propertyFlow?: unknown;
      mutationSeed?: unknown;
      appEngineColumn?: boolean;
    }
  >;
  consentLedger: Array<
    Loose & {
