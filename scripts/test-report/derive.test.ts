...
import { loadClaims, validateClaims } from "./claims-schema.ts";
...
describe("the claims file", () => {
  ...
  test("declares the 11 × 10 vocabulary the promises grid joins on", () => {
    ...
    expect(claims).not.toBeNull();
    const vocabulary = dict(dict(claims).vocabulary);
    expect(items(vocabulary.qualities)).toHaveLength(11);
    expect(items(vocabulary.surfaces)).toHaveLength(10);
  });
  ...
  test("SABOTAGE: a lane tagged with a quality nobody declared is rejected", () => {
    ...
    const broken = {
      ...claims,
      lanes: [
        { ...dict(items(dict(claims).lanes)[0]), qualities: ["telepathy"] },
      ],
    };
  ...
  test("SABOTAGE: a claim with no demonstrated-red date is rejected", () => {
    ...
    const broken = {
      ...
      claims: [
        {
          ...dict(items(dict(claims).claims)[0]),
          demonstratedRed: { date: null },
        },
      ],
    };
    expect(validateClaims(broken).errors.join(" ")).toContain(
