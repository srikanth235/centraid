...
function addCitation(
  ...
  citations.get(number)?.push(String(where));
}
...
export function collectCitations(
  ...
  if (Array.isArray(node)) {
    for (const [index, child] of node.entries())
      collectCitations(
        child,
        `${where}[${isRecord(child) ? String(child.id ?? index) : index}]`,
        citations
      );
    return citations;
  }
  if (!isRecord(node)) return citations;
 ...
 */
export function declaredOpenIssues(matrix: Loose): number[] {
  return Object.entries(dict(matrix.trackingIssues))
    .filter(([, record]) => dict(record).state === "open")
...
export async function resolveIssueStates(
  issues: number[],
  {
    ...
    token,
    repo = REPO,
  }: { fetchImpl: FetchLike; token: string; repo?: string }
): Promise<{
  states: Map<number, string>;
  unreachable: Array<{ issue: number; reason: string }>;
}> {
  ...
  const resolved = await Promise.all(
    issues.map(async (issue) => {
      ...
      try {
        ...
        const body = dict((await response.json()) as unknown);
        if (typeof body.state !== "string")
          return { issue, reason: `${url} returned no issue state` };
        return { issue, state: body.state };
      } catch (error: unknown) {
        return { issue, reason: `${url} — ${errorMessage(error)}` };
      }
    })
  );
...
export function reportCitationErrors({
  ...
  for (const issue of [...citations.keys()].sort((a, b) => a - b)) {
    ...
    errors.push(
      `citation #${issue} is CLOSED but still marks a live exception at ${(
        citations.get(issue) ?? []
      )
        .sort()
 ...
 *
 * @param {{ sources: Record<string, object>, matrix: Loose, token?: string, fetchImpl?: typeof fetch, repo?: string }} options the parsed ledgers keyed by path, the matrix, the API token, and an optional injected fetch
...
export async function validateOpenCitations({
  ...
  fetchImpl?: FetchLike;
  repo?: string;
}): Promise<{ errors: string[]; checked: number }> {
