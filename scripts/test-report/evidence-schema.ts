...
function caseErrors(
  ...
  if (!has(CASE_VERDICTS, entry.verdict)) {
  ...
  if (
    ...
    !(isCount(entry.attempts) && Number(entry.attempts) >= 1)
  ) {
...
function parkedErrors(parked: unknown, push: (message: string) => void) {
  ...
  if (!Number.isInteger(parked.issue) || Number(parked.issue) <= 0) {
    push("parked.issue must be a positive issue number");
  }
}

/** Collect the errors in the `tags` block. */
function tagErrors(tags: unknown, push: (message: string) => void) {
 ...
 */
export function validateEvidence(obj: unknown): {
  ok: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (!has(RUNGS, obj.rung)) {
    push(`rung must be one of ${RUNGS.join(" | ")}`);
  }
  if (!has(PLATFORMS, obj.platform)) {
  ...
  if (
    ...
    Date.parse(String(obj.finishedAt)) < Date.parse(String(obj.startedAt))
  ) {
    push("finishedAt must not precede startedAt");
  }
  if (!has(VERDICTS, obj.verdict)) {
 ...
 */
export function evidenceFileName(lane: string) {
  return `${lane}.json`;
}
