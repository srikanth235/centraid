...
export const SEVERITY_RANK: Record<string, number> = Object.freeze({
  S1: 0,
  S2: 1,
  S3: 2,
  S4: 3,
 ...
 * @param {Loose[]} claimRows every claim row in the claims file
 */
export function laneSeverity(lane: unknown, claimRows: unknown[] = []) {
  const row = dict(lane);
  if (has(SEVERITIES, row.severity)) return String(row.severity);
  const owned = claimRows.map(dict).filter((claim) => claim.lane === row.id);
  if (owned.length > 0) {
    return owned
      .map((claim) => String(claim.severity ?? "S4"))
      .sort(
        (left, right) =>
          (SEVERITY_RANK[left] ?? 9) - (SEVERITY_RANK[right] ?? 9)
      )
      .at(0);
  }
  return row.status === "advisory" ? "S4" : "S2";
}
