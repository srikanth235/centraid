 ...
 */
export function declaredTestTitles(source: string): string[] {
  const lines = String(source ?? "").split("\n");
  const titles = [];
  for (let index = 0; index < lines.length; index += 1) {
    if (!/^\s*(?:test|it)(?:\.each\(|\.for\(|\()/u.test(lines[index] ?? ""))
      continue;
 ...
 */
export function runnerFlowList(source: string): string[] | null {
  ...
  return [...body.matchAll(/"(?<file>[^"]+)"/gu)].map(
 ...
 */
export function runnerBudgetMinutes(source: string): number | null {
  const match = /const BUDGET_MS = (?<minutes>[\d_]+) \* 60_000/u.exec(
    String(source ?? "")
  );
  return match
    ? Number((match.groups?.minutes ?? "").replaceAll("_", ""))
    : null;
}

/** Read a repo-relative source, or null when it does not exist. */
function sourceReader(cwd: string) {
 ...
 */
function validateJoinLaws(
  matrix: Loose,
  read: (file: string) => string | null,
  flowIds: Set<string>
): string[] {
  const errors: string[] = [];
  const laws = bags(matrix.joinLaws);
  if (!Array.isArray(matrix.joinLaws) || !laws.length) {
    return ["matrix has no joinLaws registry (grid E has nothing to derive)"];
  }
  const seatIds = new Set(
    bags(matrix.seats).map((seat) => String(seat.id ?? ""))
  );
  ...
  for (const law of laws) {
    ...
    seen.add(law.id);
    if (law.flow != null && !flowIds.has(String(law.flow))) {
      errors.push(`join law ${law.id} references unknown flow ${law.flow}`);
    }
    for (const seat of items(law.seats).map(String)) {
      if (!seatIds.has(seat))
        errors.push(`join law ${law.id} references unknown seat ${seat}`);
    }
    const owner = String(law.owner ?? "");
    if (!byOwner.has(owner)) byOwner.set(owner, []);
    byOwner.get(owner)?.push(law);
  }
  ...
  for (const [owner, owned] of byOwner) {
    ...
    for (const law of owned) {
      if (!titles.includes(String(law.testName ?? ""))) {
        errors.push(
 ...
 */
function validateJourneyCompleteness(
  suites: Array<{
    id: string;
    runner?: string | null;
    flows: string[];
  ...
  for (const file of flowFiles) {
    ...
    const status = dict(dict(roster.flows)[file]).status;
  ...
  for (const file of scheduled) {
    if (!flowFiles.includes(String(file))) {
      errors.push(
...
function referencedFiles(matrix: Loose) {
  return new Set(
    bags(matrix.joinLaws)
      .map((law) => law.owner)
 ...
 */
export async function validateReportRegistries(
  matrix: Loose,
  options: Loose | undefined = {}
): Promise<string[]> {
  const cwd = String(options.root ?? process.cwd());
  const readSource =
    typeof options.readSource === "function"
      ? (options.readSource as (
          file: string
  ...
  const [sources, discovered] = await Promise.all([
    ...
    options.flowFiles
      ...
      : fromAsync(glob(JOURNEY_FLOW_GLOB, { cwd })),
  ]);
  ...
  const flowFiles = items(discovered)
    .map((file) => String(file).replaceAll("\\", "/"))
    .sort();
  const flowIds = new Set(
    bags(matrix.flows).map((flow) => String(flow.id ?? ""))
  );
  const suites = Array.isArray(options.suites)
    ? (options.suites as {
        id: string;
        runner: string;
        flows: string[];
    ...
    : readSuiteRunners();
  const roster = isRecord(options.roster)
    ? options.roster
    : dict(await loadRoster());
  return [
