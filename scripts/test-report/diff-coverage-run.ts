...
export function parseArgs(argv: string[]): {
  ...
  const out: { base: string | null; dependents: boolean } = {
    base: null,
    dependents: false,
  };
  for (let i = 0; i < argv.length; i++) {
    ...
    if (current === "--base" && next) {
      out.base = next;
      i += 1;
    } else if (current === "--dependents") out.dependents = true;
  }
  return out;
}
 ...
 */
export function workspaceDirOf(filePath: string): string | null {
 ...
 */
export function projectNameOf(dir: string): string | null {
 ...
 */
export function vitestProjectNames(names: string[]): string[] {
 ...
 */
function dependentsOf(baseRef: string): string[] | null {
  const res = spawnSync(
    "bun",
