...
export function fixtureOptions(overrides: Record<string, unknown> = {}) {
  return parseFlags(
    [
      "--evidence",
      String(overrides.evidence ?? path.join(FIXTURES, "evidence")),
      "--evidence-previous",
...
export async function renderFixture(overrides: Record<string, unknown> = {}) {
