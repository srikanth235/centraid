...
export function parseStep(command: string) {
  ...
  for (const match of command.matchAll(
    ...
    flags[match.groups?.flag ?? ""] = (match.groups?.value ?? "").replace(
      /^"|"$/gu,
      ""
    );
  }
 ...
 * each invocation.
 */
export function resolveLanes(raw: unknown, block: unknown): string[] {
  const text = String(raw ?? "");
  if (!text) return [];
  if (!text.includes("$")) return [text];
  const base = text.split(/-?\$/u)[0]?.replace(/-$/u, "") ?? "";
  if (base && !base.includes("$")) return [base];
  const literals = [
    ...String(block).matchAll(/"(?<lane>[a-z0-9][a-z0-9._-]*):/gu),
  ].map((match) => match.groups?.lane ?? "");
...
export function checkEvidenceMapping({
  workflows,
  lanes,
}: {
  workflows?: unknown;
  ...
  const registry = new Map(bags(lanes).map((lane) => [String(lane.id), lane]));
  const wired = new Set<string>();

  for (const [file, source] of entries(workflows)) {
    for (const step of stepsIn(String(source))) {
...
function readWorkflows(dir: string) {
  ...
  if (!claims || claimErrors.length > 0) {
    ...
    if (errors.length > 0) {
      ...
      process.stdout.write(
        `evidence-mapping: ${lanes.length} registered lanes, every evidence step mapped\n`
      );
    }
  }
}
