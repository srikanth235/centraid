 ...
 */
function secretExpr(name: string): string {
  return `\${{ secrets.${name} }}`;
}

function lintFixture(edits: Record<string, (text: string) => string> = {}) {
...
describe("validate-release-wiring", () => {
  ...
  test("rejects an aggregator that no longer fails closed on empty results", () => {
    const errors = lintFixture({
      "release.yml": (text: string) =>
        text.replace("refusing to pass", "passing"),
    });
    expect(errors).toContain(
  ...
  test("rejects a lane that grows its own trigger", () => {
    const errors = lintFixture({
      "lane-release-companion.yml": (text: string) =>
        text.replace(
          "on:\n  workflow_call:",
          "on:\n  workflow_call:\n  push:\n    branches: [main]"
        ),
