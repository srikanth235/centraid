 ...
 */
export function isInstrumentableSource(filePath: string): boolean {
...
export function lineHits(
  ...
  const entry = dict(findCoverageEntry(coverageMap, filePath));
  if (!findCoverageEntry(coverageMap, filePath)) return null;
  const statementMap = dict(entry.statementMap);
  const s = dict(entry.s);
  let hits = 0;
  let matched = false;
  for (const [id, rawLoc] of Object.entries(statementMap)) {
    const loc = dict(rawLoc);
    const startPos = dict(loc.start);
    if (startPos.line === undefined) continue;
    const start = Number(startPos.line);
    const end = Number(dict(loc.end).line ?? start);
    if (line >= start && line <= end) {
      matched = true;
      hits = Math.max(hits, Number(s[id] ?? 0));
    }
  }
  // Prefer statement map; fall back to line-level map when present (some reporters).
  if (!matched && isRecord(entry.l)) {
    const lmap = dict(entry.l);
    if (lmap[String(line)] !== undefined) return Number(lmap[String(line)]);
  }
 ...
 */
export function scoreDiffCoverage(
  changed: Map<string, Set<number>>,
  coverageMap: Record<string, unknown>,
  opts: { filter?: (path: string) => boolean } | undefined = {}
): {
  total: number;
  covered: number;
  uncovered: Array<{ file: string; line: number; hits: number | null }>;
  percent: number;
} {
...
export function evaluateDiffCoverage(
  ...
  threshold: number,
  approvedDeviation?: string | null
): { ok: boolean; reason: string; messages: string[] } {
  if (score.total === 0) {
    return {
      ok: true,
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (current === "--base" && next) {
      out.base = next;
      i += 1;
    } else if (current === "--threshold" && next) {
      out.threshold = Number(next);
      i += 1;
    } else if (current === "--coverage" && next) {
      out.coverage = next;
      i += 1;
    } else if (current === "--help" || current === "-h") out.help = true;
  }
  return out;
}
