...
export function totalSites(sites: Record<string, unknown>) {
...
export function topOffenders(sites: unknown, limit = 5) {
  return Object.entries(dict(sites))
...
export function validateSleepInventory(
  inventory: unknown,
  ...
  const measured = Number(totalSites(sites));
  ...
  if (!Number.isInteger(budget)) {
    ...
    errors.push(
      `fixed-sleep budget exceeded: ${measured} sites against a budget of ${budget} (+${measured - Number(budget)}). The budget is down-only — ${REMEDY}. Top offenders: ${topOffenders(sites).join(", ")}`
    );
  } else if (measured < Number(budget)) {
    errors.push(
...
export function reconcileInventory(
  ...
  const measured = Number(totalSites(sites));
  const current = dict(inventory)._budget;
  const budget = Number.isInteger(current)
    ? Math.min(Number(current), measured)
    : measured;
  return { ...dict(inventory), _budget: budget, sites };
}
