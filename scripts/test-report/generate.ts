...
export function parseFlags(
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    flags[current.slice(2)] = argv[index + 1];
  }
  const at = (value: string | undefined, fallback: string) =>
...
export async function collectModel(
  options: ReturnType<typeof parseFlags>,
  now: Date = new Date()
) {
  ...
  const model: ReportModel = buildModel({
    claims,
  ...
  for (const entry of tonight.lanes.values()) {
    for (const observed of items(dict(entry).cases)) {
      const row = dict(observed);
      model.caseResults?.set(String(row.id), row);
    }
  }
  return model;
}
...
export function buildSummary(model: ReportModel) {
  const lamp = dict(model.verdict);
  const counts = dict(model.counts);
  return {
    schema: 1,
    verdict: lamp.verdict,
    why: lamp.why,
    flip: lamp.flip,
    ...
    deltas: {
      ...
      noEvidence: counts["no-evidence"] ?? 0,
      newRed: dict(model.delta).newRed,
      newGreen: dict(model.delta).newGreen,
      previousVerdict: dict(model.delta).previousVerdict,
    },
