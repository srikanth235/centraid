...
function live(css: string) {
  return css.replaceAll(/\/\*[\s\S]*?\*\//gu, "");
}

function declared(css: string) {
  return new Set(
...
function referenced(css: string) {
  return new Set(
...
): { body: string; query: string; selector: string }[] {
  const found: { body: string; query: string; selector: string }[] = [];
  let cursor = 0;
...
function contrast(one: string, other: string) {
  const ranked = [luminance(one), luminance(other)].sort(
    (left, right) => right - left
  );
  const bright = ranked[0] ?? 0;
  const dim = ranked[1] ?? 0;
  return (bright + 0.05) / (dim + 0.05);
}

/** The rung a declaration names: `var(--nw-okbg)` -> `--nw-okbg`. */
function rung(value: unknown) {
  return /^var\(\s*(?<name>--[\w-]+)\s*\)$/u.exec(String(value ?? ""))?.groups
    ?.name;
}
...
function modifiers(selector: string) {
...
const CELL_RULES = [
  ...
  .flatMap((hit) =>
    (hit.groups?.selectors ?? "")
      ...
      .filter((selector: string) => /^\.cell(?:\.[\w-]+)*$/u.test(selector))
      .map((selector: string) => ({ body: hit.groups?.body ?? "", selector }))
  )
  ...
  test("separates two states inside a family, or says why it does not", () => {
    ...
    for (const [left, leftBody] of treatments) {
      for (const [right, rightBody] of treatments) {
        ...
        const oneTreatment =
          REGISTER[left as keyof typeof REGISTER].fact ===
            REGISTER[right as keyof typeof REGISTER].fact ||
          WORD_SEPARATED.some(
