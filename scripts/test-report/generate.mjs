...
function collectCoverage(coverageSummary, floorConfig) {
  ...
  return filterFloorConfigEntries(floorConfig).map(([scope, floor]) => {
    ...
    const entries = coverageSummary
      ? Object.entries(coverageSummary).filter(
...
function collectMutationRows(scoresArtifact, floorConfig) {
  ...
  const ids = new Set([...floorEntries.map(([id]) => id), ...byId.keys()]);
...
function render(reportModel) {
  const data = JSON.stringify(reportModel).replaceAll("<", "\\u003c");
  const dimensionHeaders = reportModel.matrix.dimensions
    .map(
      (dimension) =>
        `<th scope="col"><span>${escapeHtml(dimension.label)}</span><small>${escapeHtml(dimension.lane)}</small></th>`
    )
    .join("");
  const rows = reportModel.matrix.surfaces
    .map((surface, rowIndex) => {
      const surfaceCells = reportModel.cells.filter(
    ...
    .join("");
  const coverageRowsHtml = reportModel.coverageRows
    .map((row) => {
      const lineState =
        row.lines == null
          ? "missing"
    ...
    .join("");
  const mutationRowsHtml = (reportModel.mutationRows ?? []).length
    ? (reportModel.mutationRows ?? [])
        .map((row) => {
          const state =
            row.score == null
              ? "missing"
  ...
  const runtimeRows = reportModel.packageRuntime.length
    ? reportModel.packageRuntime
        .map(
          (row) =>
  ...
  const slowRows = reportModel.slowest.length
    ? reportModel.slowest
        .map(
          (row, index) =>
  ...
  const durableSeries = reportModel.healthHistory ?? [];
  const durableTrends =
    durableSeries.length > 1
      ? [
          ...
          .join("")
      : "";
  const laneTrends = reportModel.laneResults.length
    ? reportModel.laneResults
        .map(
          (result) =>
  ...
  if (reportModel.reportScope === "main") {
    honestyBanners.push(
  ...
  if (
    reportModel.summary.silentAllClear &&
    reportModel.summary.jobReconciliation
  ) {
    honestyBanners.push(
      `<p class="lede" style="color:var(--red)">${escapeHtml(reportModel.summary.jobReconciliation)}</p>`
    );
  }
  if (reportModel.summary.unmappedEvidence) {
    honestyBanners.push(
      `<p class="lede" style="color:var(--amber)">Unmapped e2e evidence: ${reportModel.summary.unmappedEvidence}${
        (reportModel.summary.unmappedFailed ?? []).length
          ? ` (${(reportModel.summary.unmappedFailed ?? []).length} failed: ${escapeHtml((reportModel.summary.unmappedFailed ?? []).join(", "))})`
          : ""
      }</p>`
    );
  }
  if (reportModel.summary.cellsMissingRose) {
    honestyBanners.push(
      `<p class="lede" style="color:var(--amber)">cellsMissing rose vs prior durable history: ${reportModel.summary.cellsMissingPrior} → ${reportModel.summary.cellsMissing} (Δ+${reportModel.summary.cellsMissingDelta})</p>`
    );
  }
...
<header class="hero"><div><div class="eyebrow">Centraid · test intelligence</div><h1>Product health, with the gaps left visible.</h1><p class="lede">One view across per-PR correctness and nightly journey, performance, and scale evidence. Grey is intentional: missing proof should never disappear. Red cells failed with evidence; grey cells were not run.</p>${honestyBanners.join("")}${
    reportModel.summary.unhandledErrors
      ? `<p class="lede" style="color:var(--red)">Unhandled Vitest errors: ${reportModel.summary.unhandledErrors} — ${escapeHtml(
          (reportModel.summary.unhandledErrorMessages ?? [])
            .join(" · ")
            .slice(0, 400)
        )}</p>`
      : ""
  }</div><div class="summary"><div class="stat"><b>${reportModel.summary.passed}</b><small>evidence passed</small></div><div class="stat"><b>${reportModel.summary.failed}</b><small>evidence failed</small></div><div class="stat"><b>${reportModel.summary.cellsFailed ?? 0}</b><small>cells failed</small></div><div class="stat"><b>${reportModel.summary.cellsMissing ?? 0}</b><small>cells not run</small></div><div class="stat"><b>${reportModel.summary.unhandledErrors ?? 0}</b><small>unhandled errors</small></div><div class="stat"><b>${reportModel.summary.skippedTests}</b><small>tests skipped</small></div><div class="stat"><b>${reportModel.summary.envGated}</b><small>environment-gated</small></div><div class="stat"><b>${reportModel.summary.stale}</b><small>stale owners</small></div></div></header>
...
<p class="foot">Generated ${escapeHtml(reportModel.generatedAt)} · ${reportModel.matrix.surfaces.length} surfaces · ${reportModel.matrix.dimensions.length} dimensions · ${reportModel.matrix.flows.length} canonical flows</p></main>
...
function symbol(state) {
  return (
    { passed: "✓", failed: "×", skipped: "–", stale: "!", missing: "·" }[
      state
    ] ?? "·"
  );
}
