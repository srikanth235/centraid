  ...
  const durableTrends =
    durableSeries.length > 1
      ? [
          ...
          .join("")
      : "";
  const laneTrends = modelLocal.laneResults.length
    ? modelLocal.laneResults
        .map(
          (result) =>
...
<header class="hero"><div><div class="eyebrow">Centraid · test intelligence</div><h1>Product health, with the gaps left visible.</h1><p class="lede">One view across per-PR correctness and nightly journey, performance, and scale evidence. Grey is intentional: missing proof should never disappear. Red cells failed with evidence; grey cells were not run.</p>${honestyBanners.join("")}${
    modelLocal.summary.unhandledErrors
      ...
      : ""
  }</div><div class="summary"><div class="stat"><b>${modelLocal.summary.passed}</b><small>evidence passed</small></div><div class="stat"><b>${modelLocal.summary.failed}</b><small>evidence failed</small></div><div class="stat"><b>${modelLocal.summary.cellsFailed ?? 0}</b><small>cells failed</small></div><div class="stat"><b>${modelLocal.summary.cellsMissing ?? 0}</b><small>cells not run</small></div><div class="stat"><b>${modelLocal.summary.unhandledErrors ?? 0}</b><small>unhandled errors</small></div><div class="stat"><b>${modelLocal.summary.skippedTests}</b><small>tests skipped</small></div><div class="stat"><b>${modelLocal.summary.envGated}</b><small>environment-gated</small></div><div class="stat"><b>${modelLocal.summary.stale}</b><small>stale owners</small></div></div></header>
