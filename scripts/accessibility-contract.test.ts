  ...
  const [
    ...
    Promise.all(
      [
        ...
        async (file): Promise<[string, string]> => [file, await source(file)]
      )
    ),
    Promise.all(
      [
...
test("every mobile Pressable screen names an accessibility contract and keeps Dynamic Type enabled", async () => {
  const files = (await filesUnder("apps/mobile/src")).filter((file: string) =>
    file.endsWith(".tsx")
  );
  const sources = await Promise.all(files.map((file: string) => source(file)));
