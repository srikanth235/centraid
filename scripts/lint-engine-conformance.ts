...
import { scanRefusalGrammar } from "./lib/disabled-controls.ts";
import {
  checkActionKit,
  checkConceptSchemes,
  scanComponentExistence,
} from "./lint-engine-conformance-kit.ts";
import {
  checkPendingOverlay,
  checkSearchStatus,
  checkSelection,
} from "./lint-engine-conformance-overlay.ts";
import {
  ROOT,
  lineOf,
  literalArray,
  surfaceFiles,
} from "./lint-engine-conformance-surface.ts";
import type { SurfaceFile } from "./lint-engine-conformance-surface.ts";
import { checkDeclaredWrites } from "./lint-engine-conformance-writes.ts";

export type { SurfaceFile } from "./lint-engine-conformance-surface.ts";
export {
...
export { vaultEntityNames } from "./lint-engine-conformance-writes.ts";
...
function checkPlacement(root: string, files: SurfaceFile[]) {
  ...
  const placeable = new Set(
    registry.filter(
      (v): v is string => typeof v === "string" && /^[a-z_]+\.[a-z_]+$/u.test(v)
    )
  );
  if (placeable.size === 0) {
    throw new Error(
  ...
  for (const { label, code } of files) {
    ...
    for (const m of code.matchAll(
      ...
      if (itemType === undefined || placeable.has(itemType)) continue;
...
function checkCustody(_root: string, files: SurfaceFile[]) {
  const findings: string[] = [];
  ...
  for (const [label] of TRANSFER_INTERNAL_RATCHET) {
    const entry = files.find((f: SurfaceFile) => f.label === label);
    if (!entry) {
      findings.push(
...
function checkConsent(root: string, files: SurfaceFile[]) {
...
function checkTriage(root: string, files: SurfaceFile[]) {
  const findings: string[] = [];
  for (const { label, code } of files) {
...
function checkRefusalGrammar(root: string) {
  const findings: string[] = [];
