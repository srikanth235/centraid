...
// characters does not.
function isIconGlyphOnly(text: string) {
...
function ariaLabelContainsVisibleText(
  tagText: string,
  visibleText: string
): boolean {
 ...
 */
function tagEnd(src: string, start: number) {
  let i = start + 1;
  let braceDepth = 0;
  let quote = null;
...
function stripExpressions(src: string) {
  let out = "";
  let depth = 0;
  for (const ch of src) {
    if (ch === "{") {
 ...
 *  render visible text. */
function scanAriaLabelTargets(src: string, rel: string) {
  const findings = [];
...
function scanSvgAriaHidden(src: string, rel: string) {
  const findings = [];
