...
import { discoverFiles, lintFlowSource } from "./lint-e2e-flows.ts";
...
const repoFile = (rel: string) => path.resolve(import.meta.dirname, "..", rel);
...
test("SABOTAGE: the linter names no flow file, so none can be forgotten", () => {
  ...
  const source = readFileSync(repoFile("scripts/lint-e2e-flows.ts"), "utf8");
