...
test("rewriteWorkspaceDependencies maps workspace:* to versions and clears private", () => {
  ...
  assert.equal(deps["@centraid/core/protocol"], "0.1.0");
  assert.equal(deps.sharp, "^0.35.3");
  assert.equal(packageJson.devDependencies, undefined);
  const scripts = packageJson.scripts;
  assert.ok(scripts !== null && typeof scripts === "object");
  const scriptMap = scripts as Record<string, unknown>;
  assert.equal(scriptMap.prepack, undefined);
  assert.equal(scriptMap.test, "vitest");
...
test("topologicalPublishOrder places deps before dependents", () => {
  ...
  const order = topologicalPublishOrder(["gateway", "protocol"], (dir) => {
    const pkg = pkgs[dir];
    if (!pkg) throw new Error(`missing fixture package ${dir}`);
    return pkg;
  });
