...
import {
  collectNodeArtifacts,
  copyArtifacts,
} from "./merge-native-artifacts.ts";
import {
  NATIVE_PLATFORMS,
...
} from "./native-platforms.ts";

test("required native platforms cover linux, mac arm, windows x64", () => {
  const req = requiredNativePlatformIds();
  assert.deepEqual(
    req.sort(),
    ["darwin-arm64", "linux-x64", "win32-x64"].sort()
  );
});

test("nativeArtifactName matches loader convention", () => {
  assert.equal(
    nativeArtifactName("linux", "x64"),
    "centraid-tunnel-native.linux-x64.node"
  );
  assert.equal(
    nativeArtifactName("darwin", "arm64"),
    "centraid-tunnel-native.darwin-arm64.node"
  );
  assert.equal(
    nativeArtifactName("win32", "x64"),
    "centraid-tunnel-native.win32-x64.node"
  );
  assert.equal(
    nativeArtifactNameForId("darwin-arm64"),
    "centraid-tunnel-native.darwin-arm64.node"
  );
});
...
test("auditNativeArtifacts reports missing required", () => {
  ...
  assert.ok(
    audit.missingRequired.includes("centraid-tunnel-native.darwin-arm64.node")
  );
  assert.ok(
    audit.missingRequired.includes("centraid-tunnel-native.win32-x64.node")
  );
  assert.equal(audit.present.length, 1);
});

test("auditNativeArtifacts passes when all required present", () => {
  const files = requiredNativePlatformIds().map((id) =>
...
test("hostToPlatformId maps process-like hosts", () => {
  ...
  assert.equal(
    hostToPlatformId({ os: "darwin", arch: "arm64" }),
    "darwin-arm64"
  );
  assert.equal(
    hostToPlatformId({ os: "Windows_NT", arch: "x64" }),
    "win32-x64"
  );
...
test("collectNodeArtifacts + copyArtifacts merge flat and nested layouts", () => {
  ...
  fs.writeFileSync(
    path.join(from, "linux-x64", "centraid-tunnel-native.linux-x64.node"),
    "a"
  );
  fs.writeFileSync(
    path.join(from, "centraid-tunnel-native.darwin-arm64.node"),
    "b"
  );
  ...
  assert.ok(
    fs.existsSync(path.join(dest, "centraid-tunnel-native.linux-x64.node"))
  );
  assert.ok(
    fs.existsSync(path.join(dest, "centraid-tunnel-native.darwin-arm64.node"))
  );
