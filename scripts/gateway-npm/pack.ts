 ...
 *
 * Usage:
 *   node scripts/gateway-npm/pack.ts [--out artifacts/npm-packs] [--dry-run]
 */
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === "--out") out = path.resolve(argv[++i] ?? "");
      ...
      console.log(
        "Usage: node scripts/gateway-npm/pack.ts [--out dir] [--dry-run]"
      );
      process.exit(0);
    }
  }
...
function loadPublishSet() {
  return JSON.parse(
    fs.readFileSync(path.join(__dirname, "publish-set.json"), "utf8")
  );
}
...
function main() {
  ...
  console.log(
    `gateway-npm pack: done (${tarballs.length || order.length} package(s))`
  );
}
