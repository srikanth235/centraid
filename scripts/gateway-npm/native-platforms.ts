 ...
 */
export function nativeArtifactName(platform: string, arch: string): string {
 ...
 */
export function nativeArtifactNameForId(id: string): string {
 ...
 */
export function requiredNativePlatformIds(): string[] {
 ...
 */
export function auditNativeArtifacts(
  basenames: string[],
  opts: { requireAll?: boolean; requiredIds?: string[] } = {}
): { present: string[]; missingRequired: string[]; extra: string[] } {
  const requireAll = opts.requireAll === true;
  const requiredIds = opts.requiredIds ?? requiredNativePlatformIds();
  const expected = new Set(
    (requireAll ? NATIVE_PLATFORMS.map((p) => p.id) : requiredIds).map(
      (id: string) => nativeArtifactNameForId(id)
    )
  );
