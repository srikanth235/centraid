 ...
 *
 * Usage:
 *   node scripts/gateway-npm/merge-native-artifacts.ts --from <dir> [--require]
 *
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === "--from") from = path.resolve(argv[++i] ?? "");
      ...
      console.log(
        "Usage: node scripts/gateway-npm/merge-native-artifacts.ts --from <dir> [--require]"
      );
      process.exit(0);
    }
  }
 ...
 */
export function collectNodeArtifacts(from: string): string[] {
  const out: string[] = [];
 ...
 */
export function copyArtifacts(sources: string[], destDir: string): string[] {
