 ...
 *
 * Usage:
 *   node scripts/gateway-npm/publish.ts [--pack-dir artifacts/npm-packs] [--dry-run]
 */
...
function main() {
  ...
  if (requireToken && !token) {
    console.error(
      "gateway-npm publish: NPM_TOKEN / NODE_AUTH_TOKEN required (--require-token)"
    );
    process.exit(1);
  }
  if (!fs.existsSync(packDir)) {
    console.error(
      `gateway-npm publish: pack dir missing: ${packDir} (run pack.mjs first)`
    );
    process.exit(1);
  }
