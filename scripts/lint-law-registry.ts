...
const SELF_TEST = "scripts/lint-law-registry.test.ts";
 ...
 * silently unowned.
 */
export function collectLawTags(scanRoot = root): LawTag[] {
  const found: LawTag[] = [];
  for (const file of walk(scanRoot)) {
    ...
    for (const [index, line] of lines.entries()) {
      ...
      while (match !== null) {
        const tag = match.groups?.tag ?? match[1] ?? "";
        found.push({
          tag,
          file: rel,
          line: index + 1,
          valid: VALID_TAG_RE.test(tag),
        });
 ...
 */
type CheckLawRegistryInput = {
  laws: Record<string, LawEntry> | null | undefined;
  tags: LawTag[];
  flowIds?: string[];
  files?: string[];
};

export function checkLawRegistry(input: CheckLawRegistryInput): {
  violations: string[];
  notices: string[];
} {
  const { laws, tags, flowIds = [], files = [] } = input;
  ...
  const filesByTag = new Map<string, Map<string, number>>();
  for (const tag of tags.filter((entry) => entry.valid)) {
    const existing = filesByTag.get(tag.tag);
    if (existing) {
      if (!existing.has(tag.file)) existing.set(tag.file, tag.line);
    } else {
      filesByTag.set(tag.tag, new Map([[tag.file, tag.line]]));
    }
  }
...
function main() {
  ...
  const lawsRaw = isRecord(matrixRecord.laws) ? matrixRecord.laws : undefined;
  const laws = lawsRaw
    ? Object.fromEntries(
        Object.entries(lawsRaw).map(([tag, entry]) => [
          tag,
          isRecord(entry) ? entry : {},
        ])
      )
    : undefined;
  const flows = Array.isArray(matrixRecord.flows) ? matrixRecord.flows : [];
  const { violations, notices } = checkLawRegistry({
    laws,
    tags,
    flowIds: flows.flatMap((flow) =>
      isRecord(flow) && typeof flow.id === "string" ? [flow.id] : []
    ),
    files,
  });
  ...
  const registered = Object.keys(laws ?? {}).length;
