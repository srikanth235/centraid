#!/usr/bin/env bun
// The RTL and CJK fidelity lanes of `design-gallery.ts` (issue #839, gap G13).
//
...
// findings, so `design-gallery-fidelity.test.ts` can flip an input and prove
...
// division `design-gallery-lowering.ts` keeps: the gate script owns the
...
import type {
  ...
  FidelityRecord,
  PhysicalKey,
} from "./design-gallery-fidelity-types.ts";
...
const PHYSICAL_PAIRS: ReadonlyArray<
  readonly [string, PhysicalKey, PhysicalKey]
> = [
...
export function label(record: FidelityRecord): string {
...
function sameLength(a: string, b: string, tolerance = 0.5): boolean {
  const left = px(a);
  const right = px(b);
...
export function alignRenders(
  ...
  return first.map((a, index) => {
    ...
    if (!b) throw new Error(`fidelity: missing pair at ${index}`);
 ...
 */
export function judgeMirroredBoxes(pairs: AlignedPair[]): string[] {
 ...
 */
export function judgePhysicalAlignment(records: FidelityRecord[]): string[] {
  const findings: string[] = [];
 ...
 */
export function judgeNumericIsolation(records: FidelityRecord[]): string[] {
  const findings: string[] = [];
  for (const record of records) {
    if (!record.visible) continue;
 ...
 */
export function judgeIsolatedContainers(records: FidelityRecord[]): string[] {
  const findings: string[] = [];
 ...
 */
export function judgeBidiProbe(probe: BidiProbe): string[] {
  const findings: string[] = [];
...
export function normalizeStack(stack: string): string {
  return stack
    .split(",")
    .map((family: string) => family.trim().replaceAll(/^["']|["']$/gu, ""))
    .filter(Boolean)
    .join(", ");
}
 ...
 */
export function judgeTypeStability(pairs: AlignedPair[]): string[] {
  const findings: string[] = [];
...
export function collectInPage(input: {
  dir: string;
  host: string;
}): FidelityRecord[] {
  const { dir: direction, host: hostSelector } = input;
  const root = document.querySelector(hostSelector);
  if (!root) throw new Error(`fidelity: missing host ${hostSelector}`);
  const read = (element: Element, property: string): string =>
  ...
  return elements.map((element, index) => {
    ...
    const anchorBox = (
      html.offsetParent ?? document.documentElement
    ...
    return {
      ...
      childDisplays: Array.from(element.children).map((child: Element) =>
        read(child, "display")
      ),
...
export function probeBidiInPage(input: {
  host: string;
  numerals: string;
  word: string;
}): BidiProbe | null {
  const { host: hostSelector, numerals, word } = input;
  const root = document.querySelector(hostSelector);
  if (!root) throw new Error(`fidelity: missing host ${hostSelector}`);
  const read = (element: Element, property: string): string =>
    getComputedStyle(element).getPropertyValue(property);
  ...
  const source = Array.from(root.querySelectorAll("*")).find(
    (element: Element) =>
  ...
  const runX = (clone: HTMLElement, dateStart: number): BidiRun => {
    const text = clone.firstChild;
    if (!(text instanceof Text))
      throw new Error("fidelity: probe clone has no text node");
    const at = (start: number, end: number): number | null => {
  ...
  const build = (bare: boolean): { direction: string; measured: BidiRun } => {
...
export function localizeInPage(input: { host: string; sample: string }): void {
  const { host: hostSelector, sample: source } = input;
  const root = document.querySelector(hostSelector);
  if (!root) throw new Error(`fidelity: missing host ${hostSelector}`);
  ...
  while (node !== null) {
    ...
    if (length > 0) {
      ...
      while (text.length < take) {
        const glyph = source[cursor % source.length] ?? "";
        text += glyph;
        cursor += 1;
      }
      node.nodeValue = text;
 ...
 * output.
 */
export async function runFidelityLanes(
  page: Page,
  origin: string,
  options: {
    freezeMotion: string;
    host: string;
    legalTypeTriples: (page: Page, root: string, id: string) => Promise<void>;
    productFaceResolved: (page: Page, id: string) => Promise<void>;
    sansStack: string;
  }
): Promise<string[]> {
  const {
  ...
  await settle(page);
  const flipped = await page.evaluate((selector: string) => {
    const node = document.querySelector(selector);
    return node ? getComputedStyle(node).direction : "";
  }, host);
  if (flipped !== "rtl")
    return [
  ...
  try {
    ...
    findings.push(
      `cjk: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  return findings;
}
