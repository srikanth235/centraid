 ...
 */
export function readCandidatePointer(
 ...
 */
export function judgeCandidate(input: {
  head: string;
  candidate: string | null;
  allowUncandidated: boolean;
  reason?: string;
}): { ok: boolean; message: string } {
  const { head, candidate, allowUncandidated, reason } = input;
 ...
 */
export function assertHeadIsCandidate(options: {
  argv: string[];
  run?: (file: string, args: string[]) => string;
  ...
  const nextArg = argv[allowIndex + 1];
  const reason =
    allowUncandidated && nextArg !== undefined && !nextArg.startsWith("--")
      ? nextArg
