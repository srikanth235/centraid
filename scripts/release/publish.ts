 ...
 *
 *   node scripts/release/publish.ts --version 0.2.1 --issue 501 \
...
import { runSyncVersions } from "./sync-versions.ts";
...
if (version === undefined || !/^\d+\.\d+\.\d+$/u.test(version)) {
  console.error(
    "usage: node scripts/release/publish.ts --version X.Y.Z --issue N [--surfaces a,b] [--beta] [--dry-run] [--push]"
  );
  process.exit(2);
}
...
function extractReleaseBody() {
  ...
  return (
    (changelogSectionBody(text, releaseVersion) ?? "").trim() ||
    `Centraid ${releaseVersion}`
  );
}
...
const syncReport = runSyncVersions({
  rootDir: root,
  version,
  dryRun,
});
...
execSync(
  ...
  { cwd: root, stdio: "inherit", shell: "/bin/sh" }
);
