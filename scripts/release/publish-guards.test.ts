...
} from "./nightly-quality-blockers.ts";
 ...
 */
function makeFixtureRoot(options: Record<string, unknown> = {}) {
...
function runPublish(root, args) {
  return spawnSync(
    process.execPath,
    [path.join(root, "scripts/release/publish.ts"), ...args],
 ...
 */
function runPrepare(root: string, args) {
  return spawnSync(
    process.execPath,
    [
      path.join(root, "scripts/release/prepare.ts"),
      "--allow-uncandidated",
...
describe("release publish guards", () => {
  ...
  test("refuses a publish with no --version", () => {
    ...
    expect(result.stderr).toContain("usage: node scripts/release/publish.ts");
  });
...
describe("release prepare guards", () => {
  ...
  test("emits a publish command that still requires a real issue number", () => {
    ...
    expect(JSON.parse(result.stdout).publishCommand).toBe(
      "node scripts/release/publish.ts --version 0.4.3 --issue N --surfaces desktop,gateway-image,gateway-npm"
    );
  });
