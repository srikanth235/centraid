...
export type SurfaceCadence = "tag" | "store" | "continuous" | "sideline";
...
export const RELEASE_SURFACES: ReleaseSurface[] = [
  {
    id: "desktop",
 ...
 */
export function defaultShipSurfaceIds(): string[] {
 ...
 */
export function resolveShipSurfaces(
  ids: string[]
): { ok: true; surfaces: ReleaseSurface[] } | { ok: false; error: string } {
 ...
 */
export function buildSurfaceMatrix(opts: { shipIds?: string[] } = {}): {
  productVersionRule: string;
  protocolRule: string;
  buildNumberRule: string;
  defaultShip: string[];
  shipThisCycle: string[];
  surfaces: Array<
    ReleaseSurface & { inDefaultShip: boolean; inThisShip: boolean }
  >;
} {
