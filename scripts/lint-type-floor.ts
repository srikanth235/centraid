...
function blankCssComments(src: string) {
  return src.replace(/\/\*[\s\S]*?\*\//gu, (m: string) =>
    m.replace(/[^\n]/gu, " ")
  );
}
function blankJsComments(src: string) {
  return src
...
function scanCssFile(src: string, rel: string) {
  const findings = [];
...
function scanTsFile(src: string, rel: string) {
  const findings = [];
