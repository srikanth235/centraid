 ...
 */
export function parseVaultTables(text: string) {
...
export function discoverSourceFiles(root = ROOT): string[] {
  const out: string[] = [];
 ...
 */
type VaultSqlFile = { rel: string; text: string };
type VaultSqlAllow = Record<string, string>;

export function lintVaultSql(input: {
  files: VaultSqlFile[];
  tables: Iterable<string>;
  allow?: VaultSqlAllow;
  roles?: ReadonlyArray<{ pattern: RegExp; reason: string }>;
}): {
  findings: Array<{
    file: string;
    line: number;
  ...
  const { files, tables, allow = ALLOW_LIST, roles = TEST_ROLES } = input;
