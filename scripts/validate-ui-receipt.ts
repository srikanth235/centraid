...
function isClientSurface(file: string) {
...
function withoutUnrenderedFields(text: string): string {
  const doc: unknown = JSON.parse(text);
  if (!isRecord(doc)) return JSON.stringify(doc);
  for (const field of UNRENDERED_MANIFEST_FIELDS) {
    ...
    if (isRecord(node) && last !== undefined) delete node[last];
  }
 ...
 */
function manifestEditIsUnrendered(
  file: string,
  readText: (file: string) => string,
  readBase: ((file: string) => string | null) | undefined
): boolean {
...
type ValidateUiReceiptInput = {
  changed: string[];
  readText: (file: string) => string;
  readBase?: (file: string) => string | null;
};

export function validateUiReceipt(input: ValidateUiReceiptInput): string[] {
  const { changed, readText, readBase } = input;
  const touchesUi = changed.some((file) =>
    isSurface(file, { readText, readBase })
  );
  if (!touchesUi) return [];
  const errors: string[] = [];
  const receipts = changed.filter((file: string) =>
  ...
  for (const file of receipts) {
    ...
    for (const screenshot of text.match(SCREENSHOT_RE) ?? []) {
      ...
      const emitter = changed.find((candidate: string) => {
