...
import type { ForbiddenRule } from "./site-tokens-rules.ts";
import { FORBIDDEN, REPORT_FORBIDDEN } from "./site-tokens-rules.ts";

const ROOT = path.resolve(import.meta.dirname, "..");
const FONTS_SRC = path.join(ROOT, "packages/design/fonts");

/** The vendored bytes of one bundled face. */
const faceBytes = (fileName: string): Buffer =>
  readFileSync(path.join(FONTS_SRC, fileName));
...
function nightWatchDecls(indent: string, theme: "light" | "dark"): string {
  return [...NIGHT_WATCH_RAMP.matchAll(NW_RUNG)]
    .flatMap(({ groups }) => {
      const name = groups?.name;
      const color = groups?.[theme];
      if (!name || !color) return [];
      return [`${indent}--nw-${name}: ${color};`];
    })
    .join("\n");
}
...
function authoredFiles(dir: string, out: string[] = []): string[] {
...
function settle(abs: string, bytes: Buffer): void {
...
const declaredProps = (css: string): string[] =>
  declaredCustomProps(stripCssComments(css));
