// The BI and MO capture lanes of `design-gallery.ts`.
//
...
export type GalleryScheme = NativeScheme;
export type GallerySurface = "SH" | "SH-c" | "BI" | "MO";
...
function nativeTokenCss(scheme: NativeScheme): string {
  const theme = toNativeTheme(scheme);
  const vars: string[] = [];
  const kebab = (key: string): string =>
    key.replace(
      /(?<letter>[A-Z])/gu,
      (_match: string, letter: string) => `-${letter.toLowerCase()}`
    );
 ...
 */
function loweringSheetHtml(input: {
  surface: string;
  scheme: string;
  css: string;
}): string {
  const { surface, scheme, css } = input;
  return `<!doctype html>
...
async function paintLoweringSheet(
  page: Page,
  names: string[]
): Promise<string[]> {
  return page.evaluate((tokens: string[]) => {
    const sheet = document.querySelector(".sheet");
    if (!sheet) throw new Error("design-gallery: missing .sheet");
 ...
 * rendering half.
 */
export async function captureLowering(
  page: Page,
  origin: string,
  entry: GalleryEntry,
  assertions: GalleryAssertions
): Promise<void> {
  const css =
