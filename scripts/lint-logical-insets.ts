 ...
 */
function styleSheetRanges(code: string): Array<[number, number]> {
  const ranges = [];
  ...
  while (match !== null) {
    ...
    ranges.push([match.index, index] as [number, number]);
...
export function scanSource(source: string, label = "<source>"): string[] {
...
export function scanLogicalInsets(root = ROOT): string[] {
