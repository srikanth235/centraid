...
import { stripComments } from "./lint-e2e-wiring.reach.ts";
...
export type CorpusApp = { id: string; seedable: boolean };

export function stateVarietyProblems(matrix: unknown): string[] {
  const problems: string[] = [];
  const record = isRecord(matrix) ? matrix : {};
  const appStates = isRecord(record.appStates) ? record.appStates : {};
  const apps = Array.isArray(appStates.apps) ? appStates.apps : [];
  for (const app of apps) {
    if (!isRecord(app) || typeof app.id !== "string") continue;
    const states = isRecord(app.states) ? app.states : {};
    for (const [state, cell] of Object.entries(states)) {
      const owner = isRecord(cell) ? cell.owner : undefined;
 ...
 */
type CorpusProblemsInput = {
  apps: CorpusApp[];
  flows: string[];
  readFile: (rel: string) => string;
};

export function corpusProblems(input: CorpusProblemsInput): string[] {
  const { apps, flows, readFile } = input;
  const problems: string[] = [];
...
export function discoverApps(root = ROOT): CorpusApp[] {
