...
type NamedImport = {
  spec: string;
  names: Array<{ imported: string; local: string; typeOnly: boolean }>;
};
type NamespaceImport = { spec: string; local: string; typeOnly: boolean };
type Reexport = {
  spec: string;
  star: boolean;
...
type ExportListEntry = { local: string; exported: string; typeOnly: boolean };

/** Syntax-level extraction of one module's imports, exports, and identifier usage. */
export function parseModule(absPath: string, text: string) {
  ...
  const info = {
    imports: [] as NamedImport[],
    namespaceImports: [] as NamespaceImport[],
    dynamicImportSpecs: [] as string[],
    reexports: [] as Reexport[],
    exportList: [] as ExportListEntry[],
    localExports: new Map<string, "value" | "type">(),
    ...
    defaultLocal: undefined as string | undefined,
    declKinds: new Map<string, "value" | "type">(),
    valueUse: new Map<string, number>(),
    typeUse: new Map<string, number>(),
    nsValueUse: new Map<string, Map<string, number>>(),
    nsTypeUse: new Map<string, Map<string, number>>(),
  };

  const hasExportModifier = (stmt: ts.Node): boolean =>
  ...
  const hasDefaultModifier = (stmt: ts.Node): boolean =>
    ts.canHaveModifiers(stmt) &&
    (ts
      .getModifiers(stmt)
  ...
  const bindingNames = (name: ts.BindingName, out: string[]): string[] => {
  ...
  const declaredNameNodes = new Set<ts.Node>();
  const collectNameNodes = (name: ts.BindingName): void => {
  ...
  const recordUse = (name: string, inType: boolean): void => {
  ...
  const recordNsUse = (
    ...
    let inner = target.get(nsName);
    if (!inner) {
      inner = new Map<string, number>();
      target.set(nsName, inner);
    }
    bump(inner, member);
  };

  const visit = (node: ts.Node, inTypeBefore: boolean): void => {
...
export type ModuleInfo = ReturnType<typeof parseModule>;
