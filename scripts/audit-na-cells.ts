...
export function ageInDays(reviewed: unknown, now = Date.now()): number | null {
...
export type NaCell = Record<string, unknown> & { grid?: string };
export type NaFinding = { rule: string; message: string };
export type NaRegister = { cells?: Record<string, unknown> };
 ...
 */
export function collectNaCells(matrix: unknown): Map<string, NaCell> {
  const cells = new Map<string, NaCell>();
  const record = isRecord(matrix) ? matrix : {};
  const seatsLayer = isRecord(record.appSeats) ? record.appSeats : {};
  const seatApps = Array.isArray(seatsLayer.apps) ? seatsLayer.apps : [];
  for (const app of seatApps) {
    if (!isRecord(app) || typeof app.id !== "string") continue;
    const seats = isRecord(app.seats) ? app.seats : {};
    for (const [seat, cell] of Object.entries(seats)) {
      if (!isRecord(cell) || cell.status === "owned") continue;
      cells.set(`appSeats.${app.id}.${seat}`, { ...cell, grid: "appSeats" });
    }
  }
  const enginesLayer = isRecord(record.appEngines) ? record.appEngines : {};
  const engineApps = Array.isArray(enginesLayer.apps) ? enginesLayer.apps : [];
  for (const app of engineApps) {
    if (!isRecord(app) || typeof app.id !== "string") continue;
    const engines = isRecord(app.engines) ? app.engines : {};
    for (const [engine, cell] of Object.entries(engines)) {
      if (!isRecord(cell) || cell.status === "pass") continue;
...
type AuditNaCellsInput = {
  cells: Map<string, NaCell>;
  register: NaRegister;
  gateExists: (gate: unknown) => boolean;
  docAnchorExists: (citation: string) => boolean;
  now?: number;
};

/** The rule engine, pure over an injected world. */
export function auditNaCells(input: AuditNaCellsInput): NaFinding[] {
  const {
    cells,
    register,
    gateExists,
  ...
  for (const [key, row] of Object.entries(rows)) {
    ...
    if (!(typeof row.restated === "string" && row.restated.length > 40)) {
      fail(
        "restated",
    ...
    const citations = [row.citation].filter(
...
function selfTest() {
  ...
  const base = {
    cells,
    gateExists: (gate: unknown) => gate === "real.test.ts",
    docAnchorExists: (citation: string) => citation === "docs/x.md#anchor",
...
function main() {
  ...
  const claims: unknown = JSON.parse(read(CLAIMS_PATH));
  const claimsRecord = isRecord(claims) ? claims : {};
  const register: NaRegister = {
    cells: isRecord(claimsRecord.naCells) ? claimsRecord.naCells : {},
  };
  ...
  const gateExists = (gate: unknown): boolean =>
    typeof gate === "string" &&
    existsSync(path.resolve(ROOT, gate.split("#")[0] ?? gate));
  const docAnchorExists = (citation: string) => {
  ...
  const prohibitions = rows.filter(
    (row) => isRecord(row) && row.kind === "prohibition"
  );
  console.log(
