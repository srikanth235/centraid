...
const isTestFile = (name: string) => /\.test\.(?:mjs|ts)$/u.test(name);
...
const isFlowFile = (rel: string) => rel.startsWith(`${SCAN_DIRS[0]}/`);
...
function stepValue(lines: string[], i: number): string | null {
  const line = lines[i];
  ...
  for (let j = i + 1; j < lines.length && j <= i + 4; j += 1) {
    ...
    const child = /^\s*(?:text|visible)\s*:(?<rest>.*)$/u.exec(childLine);
    if (!child) {
      // Stop at a dedent back to sibling level — we have left this step.
      const indentMatch = childLine.match(/^\s*/u);
      if (
        /^\s*-\s/u.test(childLine) &&
        (indentMatch?.[0].length ?? 0) <= baseIndent
      )
        break;
      continue;
    }
...
function literalOrInterp(s: string): string | null {
  if (!s) return null;
...
const isInterp = (v: string): boolean => v.startsWith("${");
...
const asPlain = (v: string): string =>
  isInterp(v) ? v : v.replace(/\.\*$/u, "").trim();
...
function observes(typed: string, a: string | null): boolean {
...
function isAllowed(lines: string[], i: number, rule: string) {
  const marker = new RegExp(`#\\s*e2e-lint-allow:\\s*${rule}\\b`, "u");
  const current = lines[i];
  if (current !== undefined && marker.test(current)) return true;
 ...
 */
export function launcherAwaitFindings(text: string) {
  const findings = [];
  ...
  for (const segment of segments.slice(1)) {
    ...
    if (waitAt !== -1 && waitAt < (tile.index ?? 0)) continue;
    findings.push({
 ...
 */
export function lintFlowSource(text: string) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (cmd === undefined || !ALL_CMDS.has(cmd)) continue;
  ...
  for (let s = 0; s < steps.length; s += 1) {
    ...
    if (INPUT_CMDS.has(step.cmd) && step.value != null) {
      ...
      for (let t = s + 1; t < steps.length; t += 1) {
        ...
        if (
          ASSERT_CMDS.has(later.cmd) &&
          step.value !== null &&
          observes(step.value, later.value)
        ) {
          observed = true;
          break;
        }
