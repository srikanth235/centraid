...
import { parseModule } from "./share-reachability-parse.ts";
import type { ModuleInfo } from "./share-reachability-parse.ts";
...
export function isTestPath(relPath: unknown): boolean {
...
function buildWorkspaceMap(root: string): Map<string, string> {
...
type ExportOrigin = { file: string; name: string; typeOnly: boolean };

type TargetRecord = {
  file: string;
  local: string;
  reachers: { production: string[]; typeOnly: string[]; test: string[] };
};

type ReacherBucket = keyof TargetRecord["reachers"];

export function runShareReachability(root: string, config: unknown) {
  const configRecord = isRecord(config) ? config : {};
  const moduleGlobs = Array.isArray(configRecord.modules)
    ? configRecord.modules.filter((g): g is string => typeof g === "string")
    : [];
  const moduleRes = moduleGlobs.map((g) => globToRegExp(g));
  const workspaceByName = buildWorkspaceMap(root);

  const files = new Map<string, ModuleInfo>();
  const relOf = (abs: string) => path.relative(root, abs).replaceAll("\\", "/");
  for (const abs of listSourceFiles(root)) {
    files.set(abs, parseModule(abs, readFileSync(abs, "utf8")));
  }

  const tryFile = (abs: string) => {
  ...
  const exportedNamesMemo = new Map<string, Set<string>>();
  const exportedNamesOf = (
    abs: string,
    seen = new Set<string>()
  ): Set<string> => {
    const memoized = exportedNamesMemo.get(abs);
  ...
  const originMemo = new Map<string, ExportOrigin | null>();
  const exportOrigin = (
    abs: string,
    name: string,
    seen = new Set<string>()
    ...
    const memoizedOrigin = originMemo.get(key);
    if (originMemo.has(key)) return memoizedOrigin ?? null;
    if (seen.has(key)) return null;
    seen.add(key);
    const info = files.get(abs);
    let origin: ExportOrigin | null = null;
    if (!info) {
      ...
      if (listed) {
        ...
        let followed: ExportOrigin | null = null;
  ...
  const targetKeyOf = (origin: ExportOrigin | null): string | null => {
  ...
  for (const [abs, info] of files) {
    ...
    const record = (key: string, bucket: ReacherBucket): void => {
  ...
  const allowlistRaw = Array.isArray(configRecord.allowlist)
    ? configRecord.allowlist
    : [];
  const allowlist = allowlistRaw.filter(isRecord);
  const allowlisted = new Map(allowlist.map((e) => [e.capability, e] as const));
  const offenses: Array<{ capability: string; why: string }> = [];
  ...
  const staleAllowlist = allowlist
    .map((e) => e.capability)
    .filter(
      (cap): cap is string => typeof cap === "string" && !failing.has(cap)
    );
...
function newTarget(file: string, local: string): TargetRecord {
  return {
    file,
    local,
    reachers: { production: [], typeOnly: [], test: [] },
  };
}
...
function main() {
  ...
  const allowCount =
    isRecord(config) && Array.isArray(config.allowlist)
