...
test("buildSbom is deterministic and derives its serial from content", () => {
  ...
  assert.equal(typeof first.serialNumber, "string");
  assert.match(String(first.serialNumber), /^urn:uuid:[0-9a-f-]+$/u);
...
test("buildSbom carries integrity hashes as hex CycloneDX hashes", () => {
  ...
  const leftPad = bom.components.find(
    (component) => isRecord(component) && component.name === "left-pad"
  );
  assert.ok(isRecord(leftPad));
...
test("buildProvenance sorts subjects and refuses an empty subject set", () => {
  ...
  assert.deepEqual(
    statement.subject.map((s) => (isRecord(s) ? s.name : undefined)),
    ["a.exe", "b.dmg"]
  );
  assert.throws(() =>
    buildProvenance({
...
test("verifyProvenance accepts real digests and REFUSES every way they can lie", () => {
  ...
  const aExe = actual["a.exe"];
  assert.ok(aExe);
  const absent = verifyProvenance(statement, { "a.exe": aExe });
