 ...
 */
export function parseBunLock(text: string): {
  ...
  for (const match of text.matchAll(entry)) {
    ...
    packages.push({
      name,
      version,
      integrity: integrityMatch?.groups?.["hash"] ?? null,
      workspace,
    });
  }
...
export function purlFor(name: string, version: string) {
...
export function verifySbom(
...
): { ok: boolean; missing: string[]; extra: string[]; errors: string[] } {
  const errors: string[] = [];
  ...
  const listed = new Set(
    (Array.isArray(bom.components) ? bom.components : []).flatMap((c) => {
      if (
        !isRecord(c) ||
...
export function verifyProvenance(
  ...
  for (const subject of subjects) {
    if (!isRecord(subject)) {
      reasons.push("subject (unnamed) carries no sha256");
      continue;
    }
    const digest = isRecord(subject.digest) ? subject.digest : {};
    const claimed = digest.sha256;
    const name = typeof subject.name === "string" ? subject.name : "(unnamed)";
    if (typeof claimed !== "string") {
      reasons.push(`subject ${name} carries no sha256`);
      continue;
    }
    const real = actual[name];
    if (real === undefined)
      reasons.push(`subject ${name} is not present in the artifact set`);
    else if (real !== claimed)
      reasons.push(
        `subject ${name} digest mismatch (attested ${claimed.slice(0, 12)}…, actual ${real.slice(0, 12)}…)`
      );
  }
  const covered = new Set(
    subjects.flatMap((s) =>
      isRecord(s) && typeof s.name === "string" ? [s.name] : []
    )
  );
...
export function buildReleaseManifest(
  version: string,
  artifacts: Array<{ name: string; sha512: string }>
) {
  return {
...
export function keyIdFor(publicKeyBase64: string) {
...
export function verifyDocument(
  ...
  if (!isRecord(envelope) || envelope.schema !== RELEASE_SIGNATURE_SCHEMA)
