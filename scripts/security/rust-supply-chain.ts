 ...
 *
 * Usage:  node scripts/security/rust-supply-chain.ts [--require] [--root <dir>]
 */
...
export const RUST_SUPPLY_CHAIN_TOOLS = Object.freeze([
  ...
  Object.freeze({
    ...
    argsFor: (/** @type {string} */ root: string) => [
      "deny",
      "--all-features",
      "--config",
 ...
 */
export function discoverLockedCrates(root: string): string[] {
  const found: string[] = [];
...
export function decideExit(
  records: RunRecord[],
  context: { require: boolean; crates: string[] }
...
export function runRustSupplyChain(
  options: {
    root?: string;
    require?: boolean;
    run?: RunFn;
...
): { records: RunRecord[]; crates: string[]; lines: string[] } {
  ...
  for (const tool of RUST_SUPPLY_CHAIN_TOOLS) {
    ...
    if (availability !== "available") {
      ...
      lines.push(
        ...
        `    then re-run : node scripts/security/rust-supply-chain.ts`,
...
if (
  ...
  const flagged = process.argv[rootFlag + 1];
  const root =
    rootFlag === -1 || flagged === undefined
      ? DEFAULT_ROOT
