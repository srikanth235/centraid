...
function fingerprintWith(
  target: ReturnType<typeof entry>,
...
const CLEAN_ENTRY: FingerprintedPackage = {
  ...entry("known", { postinstall: "node x.js" }),
  digest: "d1",
  signals: [],
};
const CLEAN_OBSERVED = [CLEAN_ENTRY];
...
test("REFUSAL: same version, different install script — the republish attack", () => {
  const result = auditLifecycle({
    observed: [{ ...CLEAN_ENTRY, digest: "d-changed" }],
    ledger: CLEAN_LEDGER,
    trustedDependencies: [],
  });
  assert.equal(result.ok, false);
...
test("REFUSAL: a version bump forces a re-review rather than inheriting the pin", () => {
  const result = auditLifecycle({
    observed: [{ ...CLEAN_ENTRY, version: "1.0.1" }],
    ledger: CLEAN_LEDGER,
    trustedDependencies: [],
  });
  assert.match(
...
test("REFUSAL: a newly-raised behaviour signal on an already-ledgered package", () => {
  const result = auditLifecycle({
    observed: [{ ...CLEAN_ENTRY, signals: ["network-fetch"] }],
    ledger: CLEAN_LEDGER,
    trustedDependencies: [],
  });
  assert.match(
...
test("REFUSAL: `--print-ledger` output does not self-approve", () => {
  ...
  // placeholder — not a promise this file makes.
  const fresh = generated.fresh;
  assert.ok(fresh);
  assert.match(fresh.reason, /TODO/u);
  const result = auditLifecycle({
    observed,
