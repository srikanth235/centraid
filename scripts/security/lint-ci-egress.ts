 ...
 *
 * Usage:  node scripts/security/lint-ci-egress.ts
 ...
 */
export function executesDependencyCode(source: string): boolean {
 ...
 */
export function hardenRunnerSteps(
  source: string
): Array<{ ref: string; policy: string | null }> {
  const lines = source.split("\n");
  const steps: Array<{ ref: string; policy: string | null }> = [];
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (let scan = index + 1; scan < lines.length; scan += 1) {
      ...
      if (scanLine === undefined || /^\s*-\s/u.test(scanLine)) break;
      const match = /^\s*egress-policy:\s*(?<policy>\S+)/u.exec(scanLine);
      const found = match?.groups?.policy;
      if (found !== undefined) {
        policy = found.replaceAll(/["']/gu, "");
        break;
      }
    }
    steps.push({ ref, policy });
  }
  return steps;
}
...
export function auditEgress(input: {
  ...
  ledger: Record<string, { reason?: string }>;
}): {
  ok: boolean;
  problems: string[];
  hardened: string[];
...
function main() {
  ...
  const ledgerWorkflows =
    isRecord(ledgerRaw) && isRecord(ledgerRaw.workflows)
      ? Object.fromEntries(
          Object.entries(ledgerRaw.workflows).map(([file, entry]) => [
            file,
            {
              reason:
      ...
      : {};
  const result = auditEgress({ workflows, ledger: ledgerWorkflows });
