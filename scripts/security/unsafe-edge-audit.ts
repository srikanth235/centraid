 ...
 *
 * Usage:  node scripts/security/unsafe-edge-audit.ts [--root <dir>]
 ...
 */
export function hasSafetyNote(lines: string[], index: number): boolean {
...
export function scanRustSource(
  source: string,
  file: string
): { file: string; line: number; text: string; justified: boolean }[] {
 ...
 */
function collectRustFiles(dir: string, acc: string[] = []): string[] {
  let entries;
  try {
 ...
 */
export function discoverCrates(root: string): string[] {
  const found: string[] = [];
  /** @param {string} dir Directory to walk. */
 ...
 */
export function auditUnsafeEdges(
  options: { root?: string; ledger?: Record<string, number> } = {}
): { ok: boolean; findings: string[]; counts: Record<string, number> } {
  const root = options.root ?? DEFAULT_ROOT;
  const parsedLedger: unknown = JSON.parse(readFileSync(LEDGER, "utf8"));
  const ledgerRecord =
    parsedLedger && typeof parsedLedger === "object" && "crates" in parsedLedger
      ? (parsedLedger as { crates: Record<string, number> }).crates
      : {};
  const ledger =
    (options.ledger as Record<string, number> | undefined) ?? ledgerRecord;
