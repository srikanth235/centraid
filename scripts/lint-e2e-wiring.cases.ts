 ...
 */
type WiringSelfTestDirs = {
  FLOWS_DIR: string;
  LANE_PREAMBLE: string;
  MOBILE_DIR: string;
  SEEDER: string;
};

export type WiringSelfTestCase = {
  name: string;
  roster: unknown;
  matrix?: unknown;
  files?: Record<string, string>;
  flows?: string[];
  apps?: Array<{ id: string; seedable: boolean }>;
  want: string[];
};

export function wiringSelfTestCases(dirs: WiringSelfTestDirs): {
  apps: Array<{ id: string; seedable: boolean }>;
  cases: WiringSelfTestCase[];
  files: Record<string, string>;
  flows: string[];
  ...
  const { FLOWS_DIR, LANE_PREAMBLE, MOBILE_DIR, SEEDER } = dirs;
  const files = {
    "wf.yml": [
      "jobs:",
      "  gate:",
  ...
  const readFile = (
    rel: string,
    tree: Record<string, string> = files
  ): string => {
    const contents = tree[rel];
    if (contents === undefined) throw new Error(`missing fixture ${rel}`);
    return contents;
  };
  ...
  const row = (
    ...
    const spec = suite ? suites[suite as keyof typeof suites] : undefined;
    return {
      status: "scheduled",
      suite: suite ? [suite] : [],
      rungs: spec ? spec.rungs : [],
      platform: spec ? spec.platform : [],
      budgetMs: 60_000,
      claim,
      ...extra,
    };
  };
  const loose = (extra: Record<string, unknown> = {}) =>
  ...
  const suitesPlus = (members: unknown) => ({
    ...suites,
    z: {
      budgetMs: 720_000,
