...
async function schemaFiles(dir: string) {
...
export async function schemaFingerprint(schemaDir: string): Promise<string> {
...
if (process.argv[1] === import.meta.filename) {
  ...
  const configRecord =
    typeof config === "object" && config !== null
      ...
      : {};
  const exportOwner =
    typeof configRecord.exportOwner === "string"
      ? configRecord.exportOwner
      : "";
  if (current !== configRecord.schemaFingerprint) {
    if (!schemaChanged || !changed.has(exportOwner)) {
      console.error(
        `schema/export ratchet: schema fingerprint changed (${current}); touch ${exportOwner} in the same PR and update tests/schema-export-fingerprint.json after auditing export completeness`
      );
      process.exit(1);
    }
    console.error(
