...
import {
  ...
  reconcileRatchet,
  verifyRatchet,
} from "./check-comment-density-ratchet.ts";
import { commentOnlyDiff } from "./comment-only-diff.ts";
import { lintCommentBlocks } from "./lint-comment-blocks.ts";
...
const codeLine = (i: number) => `const value${i} = ${i};`;
...
const measured = (root: string, files: string[]) =>
  measureTree({ root, files }).measured;
...
test("RED: a pinned file whose comment share rises fails verification", (t) => {
  ...
  assert.match(
    failures[0] ?? "",
    /^src\/a\.ts — comment share rose 1\.00% -> \d/u
  );
...
test("a lowered share passes, and --write lowers the pin to it", (t) => {
  ...
  if (!pin) throw new Error("expected re-pin");
...
test("--write prunes a pinned file that no longer exists on disk", (t) => {
  ...
  assert.deepEqual(verifyRatchet(baseline, now), []);
  assert.deepEqual(
    Object.keys(reconcileRatchet(baseline, now).next.files ?? {}),
    ["src/a.ts"]
  );
});
...
function gitFixture(t: TestContext, contents: string) {
