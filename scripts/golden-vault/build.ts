...
function seededIds(seed: string) {
  let counter = 0;
  return () => {
    counter += 1;
...
async function seedCorpus(db: VaultHandle, nextId: () => string) {
  const vaultMod = (await import(
    path.join(root, "packages/vault/dist/index.js")
  )) as {
    bootstrapVault: (
      db: VaultHandle,
      opts: { vaultName: string; ownerName: string }
    ) => { ownerPartyId: string; concepts: Record<string, string> };
  };
  const { bootstrapVault } = vaultMod;
  ...
  const contentFor = (label: string, index: number) => {
...
async function main() {
  ...
  const vaultMod = (await import(
    ...
    openVaultDb: (opts: { dir: string; synchronous: string }) => VaultHandle;
    ONTOLOGY_VERSION: string;
    snapshotVault: (
      db: VaultHandle["vault"]
    ) => Record<string, { rows: number }>;
  };
  const { openVaultDb, ONTOLOGY_VERSION, snapshotVault } = vaultMod;
  ...
  try {
    ...
    const userVersionRaw: unknown = db.vault
      .prepare("PRAGMA user_version")
      .get();
    const userVersion =
      typeof userVersionRaw === "object" &&
      userVersionRaw !== null &&
      "user_version" in userVersionRaw
        ? userVersionRaw.user_version
        : undefined;
