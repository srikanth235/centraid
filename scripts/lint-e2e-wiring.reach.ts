...
export type ReadFile = (rel: string) => string;
 ...
 * roster. */
export function isRunnerPath(rel: string) {
 ...
 */
export function jobBlock(yaml: string, job: string): string | null {
  const lines = yaml.split("\n");
  const start = lines.indexOf(`  ${job}:`);
  if (start === -1) return null;
  for (let i = start + 1; i < lines.length; i += 1) {
    const line = lines[i];
    if (line !== undefined && /^ {2}\S/u.test(line))
...
export function directInvocations(
  chunk: string
): Array<{ target: string; line: string }> {
  return [...stripComments(chunk).matchAll(INVOKE_RE)].flatMap((m) => {
    const target = m.groups?.target;
    if (!target || m.input === undefined || m.index === undefined) return [];
    return [{ target, line: lineAt(m.input, m.index) }];
  });
}
 ...
 */
export function invocationSelector(
  line: string
): { rung: number; platform: string; suite?: string } | undefined {
  ...
  const rungValue = rung.groups?.rung;
  const platformValue = platform.groups?.platform;
  if (rungValue === undefined || platformValue === undefined) return undefined;
  return {
    rung: Number(rungValue),
    platform: platformValue,
    ...(suite?.groups?.suite ? { suite: suite.groups.suite } : {}),
  };
}
 ...
 */
export function shimSelector(source: string) {
  const call =
...
export function resolveReach(
  lanes: Record<string, WiringLane>,
  readFile: ReadFile,
  roster?: Roster
): {
  flowLanes: Map<string, Set<string>>;
  runnerLanes: Map<string, Set<string>>;
} {
  const flowLanes = new Map<string, Set<string>>();
  const runnerLanes = new Map<string, Set<string>>();
  const add = (
    ...
    if (existing) existing.add(lane);
    else map.set(key, new Set([lane]));
  };
